organization := "org.scalablytyped"
name := "lesgo"
version := "0.7-dt-20220817Z-169332"
scalaVersion := "3.2.0"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "2.4.2",
  "org.scalablytyped" %%% "aws-lambda" % "8.10-dt-20221018Z-801ab1",
  "org.scalablytyped" %%% "aws-sdk" % "2.1255.0-8f0fa9",
  "org.scalablytyped" %%% "data-api-client" % "1.2-dt-20220906Z-1cf83b",
  "org.scalablytyped" %%% "elastic__elasticsearch" % "8.5.0-c2610e",
  "org.scalablytyped" %%% "elastic__transport" % "8.2.0-6b4500",
  "org.scalablytyped" %%% "hpagent" % "1.0.0-2944ca",
  "org.scalablytyped" %%% "jsonwebtoken" % "8.5-dt-20220824Z-c593ce",
  "org.scalablytyped" %%% "memcached" % "2.2-dt-20211202Z-02c80a",
  "org.scalablytyped" %%% "mysql2" % "2.3.3-3647e7",
  "org.scalablytyped" %%% "node" % "18.11-dt-20221101Z-def929",
  "org.scalablytyped" %%% "std" % "4.9-5f989f",
  "org.scalablytyped" %%% "undici" % "5.12.0-6050b1")
publishArtifact in packageDoc := false
scalacOptions ++= List("-encoding", "utf-8", "-feature", "-language:implicitConversions", "-language:higherKinds", "-language:existentials", "-no-indent", "-source:future")
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
