package typings.nodePdftk

import typings.nodePdftk.mod.Letter
import typings.nodePdftk.mod.Permission
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nodePdftkStrings {
  
  @js.native
  sealed trait A
    extends StObject
       with Letter
  inline def A: A = "A".asInstanceOf[A]
  
  @js.native
  sealed trait AllFeatures
    extends StObject
       with Permission
  inline def AllFeatures: AllFeatures = "AllFeatures".asInstanceOf[AllFeatures]
  
  @js.native
  sealed trait Assembly
    extends StObject
       with Permission
  inline def Assembly: Assembly = "Assembly".asInstanceOf[Assembly]
  
  @js.native
  sealed trait B
    extends StObject
       with Letter
  inline def B: B = "B".asInstanceOf[B]
  
  @js.native
  sealed trait C
    extends StObject
       with Letter
  inline def C: C = "C".asInstanceOf[C]
  
  @js.native
  sealed trait CopyContents
    extends StObject
       with Permission
  inline def CopyContents: CopyContents = "CopyContents".asInstanceOf[CopyContents]
  
  @js.native
  sealed trait D
    extends StObject
       with Letter
  inline def D: D = "D".asInstanceOf[D]
  
  @js.native
  sealed trait DegradedPrintin
    extends StObject
       with Permission
  inline def DegradedPrintin: DegradedPrintin = "DegradedPrintin".asInstanceOf[DegradedPrintin]
  
  @js.native
  sealed trait E
    extends StObject
       with Letter
  inline def E: E = "E".asInstanceOf[E]
  
  @js.native
  sealed trait F
    extends StObject
       with Letter
  inline def F: F = "F".asInstanceOf[F]
  
  @js.native
  sealed trait FillIn
    extends StObject
       with Permission
  inline def FillIn: FillIn = "FillIn".asInstanceOf[FillIn]
  
  @js.native
  sealed trait G
    extends StObject
       with Letter
  inline def G: G = "G".asInstanceOf[G]
  
  @js.native
  sealed trait H
    extends StObject
       with Letter
  inline def H: H = "H".asInstanceOf[H]
  
  @js.native
  sealed trait I
    extends StObject
       with Letter
  inline def I: I = "I".asInstanceOf[I]
  
  @js.native
  sealed trait J
    extends StObject
       with Letter
  inline def J: J = "J".asInstanceOf[J]
  
  @js.native
  sealed trait K
    extends StObject
       with Letter
  inline def K: K = "K".asInstanceOf[K]
  
  @js.native
  sealed trait L
    extends StObject
       with Letter
  inline def L: L = "L".asInstanceOf[L]
  
  @js.native
  sealed trait M
    extends StObject
       with Letter
  inline def M: M = "M".asInstanceOf[M]
  
  @js.native
  sealed trait ModifyAnnotations
    extends StObject
       with Permission
  inline def ModifyAnnotations: ModifyAnnotations = "ModifyAnnotations".asInstanceOf[ModifyAnnotations]
  
  @js.native
  sealed trait ModifyContents
    extends StObject
       with Permission
  inline def ModifyContents: ModifyContents = "ModifyContents".asInstanceOf[ModifyContents]
  
  @js.native
  sealed trait N
    extends StObject
       with Letter
  inline def N: N = "N".asInstanceOf[N]
  
  @js.native
  sealed trait O
    extends StObject
       with Letter
  inline def O: O = "O".asInstanceOf[O]
  
  @js.native
  sealed trait P
    extends StObject
       with Letter
  inline def P: P = "P".asInstanceOf[P]
  
  @js.native
  sealed trait Printing
    extends StObject
       with Permission
  inline def Printing: Printing = "Printing".asInstanceOf[Printing]
  
  @js.native
  sealed trait Q
    extends StObject
       with Letter
  inline def Q: Q = "Q".asInstanceOf[Q]
  
  @js.native
  sealed trait R
    extends StObject
       with Letter
  inline def R: R = "R".asInstanceOf[R]
  
  @js.native
  sealed trait S
    extends StObject
       with Letter
  inline def S: S = "S".asInstanceOf[S]
  
  @js.native
  sealed trait ScreenReaders
    extends StObject
       with Permission
  inline def ScreenReaders: ScreenReaders = "ScreenReaders".asInstanceOf[ScreenReaders]
  
  @js.native
  sealed trait T
    extends StObject
       with Letter
  inline def T: T = "T".asInstanceOf[T]
  
  @js.native
  sealed trait U
    extends StObject
       with Letter
  inline def U: U = "U".asInstanceOf[U]
  
  @js.native
  sealed trait V
    extends StObject
       with Letter
  inline def V: V = "V".asInstanceOf[V]
  
  @js.native
  sealed trait W
    extends StObject
       with Letter
  inline def W: W = "W".asInstanceOf[W]
  
  @js.native
  sealed trait X
    extends StObject
       with Letter
  inline def X: X = "X".asInstanceOf[X]
  
  @js.native
  sealed trait Y
    extends StObject
       with Letter
  inline def Y: Y = "Y".asInstanceOf[Y]
  
  @js.native
  sealed trait Z
    extends StObject
       with Letter
  inline def Z: Z = "Z".asInstanceOf[Z]
}
