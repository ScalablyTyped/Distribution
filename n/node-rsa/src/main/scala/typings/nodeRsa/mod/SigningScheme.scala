package typings.nodeRsa.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.nodeRsa.nodeRsaStrings.pkcs1
  - typings.nodeRsa.nodeRsaStrings.pss
*/
trait SigningScheme extends StObject
object SigningScheme {
  
  inline def pkcs1: typings.nodeRsa.nodeRsaStrings.pkcs1 = "pkcs1".asInstanceOf[typings.nodeRsa.nodeRsaStrings.pkcs1]
  
  inline def pss: typings.nodeRsa.nodeRsaStrings.pss = "pss".asInstanceOf[typings.nodeRsa.nodeRsaStrings.pss]
}
