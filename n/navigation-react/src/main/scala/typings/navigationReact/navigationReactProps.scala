package typings.navigationReact

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait navigationReactProps {
  @scala.inline
  def FluentLinkProps: typings.navigationReact.mod.FluentLinkProps.type = typings.navigationReact.mod.FluentLinkProps
  type FluentLinkProps = typings.navigationReact.mod.FluentLinkProps
  @scala.inline
  def NavigationBackLinkProps: typings.navigationReact.mod.NavigationBackLinkProps.type = typings.navigationReact.mod.NavigationBackLinkProps
  type NavigationBackLinkProps = typings.navigationReact.mod.NavigationBackLinkProps
  @scala.inline
  def NavigationHandlerProps: AnonStateNavigator.type = typings.navigationReact.AnonStateNavigator
  type NavigationHandlerProps = AnonStateNavigator
  @scala.inline
  def NavigationLinkProps: typings.navigationReact.mod.NavigationLinkProps.type = typings.navigationReact.mod.NavigationLinkProps
  type NavigationLinkProps = typings.navigationReact.mod.NavigationLinkProps
  @scala.inline
  def RefreshLinkProps: typings.navigationReact.mod.RefreshLinkProps.type = typings.navigationReact.mod.RefreshLinkProps
  type RefreshLinkProps = typings.navigationReact.mod.RefreshLinkProps
}

