package typings
package nodeDashZendeskLib.nodeDashZendeskMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Client extends js.Object {
  var accountsettings: js.Any
  var activitystream: js.Any
  var attachments: nodeDashZendeskLib.nodeDashZendeskMod.AttachmentsNs.Methods
  var brand: js.Any
  var categories: js.Any
  var customagentroles: js.Any
  var dynamiccontent: js.Any
  var forums: js.Any
  var forumsubscriptions: js.Any
  var groupmemberships: js.Any
  var groups: js.Any
  var helpers: js.Any
  var imports: js.Any
  var installations: js.Any
  var jobstatuses: nodeDashZendeskLib.nodeDashZendeskMod.JobStatusesNs.Methods
  var locales: js.Any
  var macros: nodeDashZendeskLib.nodeDashZendeskMod.MacrosNs.Methods
  var oauthtokens: js.Any
  var organizationfields: js.Any
  var organizationmemberships: js.Any
  var organizations: js.Any
  var policies: js.Any
  var requests: nodeDashZendeskLib.nodeDashZendeskMod.RequestsNs.Methods
  var satisfactionratings: js.Any
  var search: js.Any
  var sessions: js.Any
  var sharingagreement: js.Any
  var suspendedtickets: js.Any
  var tags: js.Any
  var targets: js.Any
  var ticketaudits: js.Any
  var ticketevents: js.Any
  var ticketexport: js.Any
  var ticketfields: js.Any
  var ticketforms: js.Any
  var ticketimport: js.Any
  var ticketmetrics: js.Any
  var tickets: nodeDashZendeskLib.nodeDashZendeskMod.TicketsNs.Methods
  var topiccomments: js.Any
  var topics: js.Any
  var topicsubscriptions: js.Any
  var topicvotes: js.Any
  var triggers: js.Any
  var userfields: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.FieldsNs.Methods
  var useridentities: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.IdentitiesNs.Methods
  var users: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.Methods
  var views: js.Any
}

object Client {
  @scala.inline
  def apply(
    accountsettings: js.Any,
    activitystream: js.Any,
    attachments: nodeDashZendeskLib.nodeDashZendeskMod.AttachmentsNs.Methods,
    brand: js.Any,
    categories: js.Any,
    customagentroles: js.Any,
    dynamiccontent: js.Any,
    forums: js.Any,
    forumsubscriptions: js.Any,
    groupmemberships: js.Any,
    groups: js.Any,
    helpers: js.Any,
    imports: js.Any,
    installations: js.Any,
    jobstatuses: nodeDashZendeskLib.nodeDashZendeskMod.JobStatusesNs.Methods,
    locales: js.Any,
    macros: nodeDashZendeskLib.nodeDashZendeskMod.MacrosNs.Methods,
    oauthtokens: js.Any,
    organizationfields: js.Any,
    organizationmemberships: js.Any,
    organizations: js.Any,
    policies: js.Any,
    requests: nodeDashZendeskLib.nodeDashZendeskMod.RequestsNs.Methods,
    satisfactionratings: js.Any,
    search: js.Any,
    sessions: js.Any,
    sharingagreement: js.Any,
    suspendedtickets: js.Any,
    tags: js.Any,
    targets: js.Any,
    ticketaudits: js.Any,
    ticketevents: js.Any,
    ticketexport: js.Any,
    ticketfields: js.Any,
    ticketforms: js.Any,
    ticketimport: js.Any,
    ticketmetrics: js.Any,
    tickets: nodeDashZendeskLib.nodeDashZendeskMod.TicketsNs.Methods,
    topiccomments: js.Any,
    topics: js.Any,
    topicsubscriptions: js.Any,
    topicvotes: js.Any,
    triggers: js.Any,
    userfields: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.FieldsNs.Methods,
    useridentities: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.IdentitiesNs.Methods,
    users: nodeDashZendeskLib.nodeDashZendeskMod.UsersNs.Methods,
    views: js.Any
  ): Client = {
    val __obj = js.Dynamic.literal(accountsettings = accountsettings, activitystream = activitystream, attachments = attachments, brand = brand, categories = categories, customagentroles = customagentroles, dynamiccontent = dynamiccontent, forums = forums, forumsubscriptions = forumsubscriptions, groupmemberships = groupmemberships, groups = groups, helpers = helpers, imports = imports, installations = installations, jobstatuses = jobstatuses, locales = locales, macros = macros, oauthtokens = oauthtokens, organizationfields = organizationfields, organizationmemberships = organizationmemberships, organizations = organizations, policies = policies, requests = requests, satisfactionratings = satisfactionratings, search = search, sessions = sessions, sharingagreement = sharingagreement, suspendedtickets = suspendedtickets, tags = tags, targets = targets, ticketaudits = ticketaudits, ticketevents = ticketevents, ticketexport = ticketexport, ticketfields = ticketfields, ticketforms = ticketforms, ticketimport = ticketimport, ticketmetrics = ticketmetrics, tickets = tickets, topiccomments = topiccomments, topics = topics, topicsubscriptions = topicsubscriptions, topicvotes = topicvotes, triggers = triggers, userfields = userfields, useridentities = useridentities, users = users, views = views)
  
    __obj.asInstanceOf[Client]
  }
}

