organization := "org.scalablytyped"
name := "node-red"
version := "1.1-dt-20200925Z-defd31"
scalaVersion := "3.0.1"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "2.4.2",
  "org.scalablytyped" %%% "ace" % "0.0-unknown-dt-20201013Z-4c4b4f",
  "org.scalablytyped" %%% "body-parser" % "1.19-dt-20200210Z-ad3420",
  "org.scalablytyped" %%% "connect" % "v3.4.0-dt-20201002Z-4e45b6",
  "org.scalablytyped" %%% "cors" % "2.8-dt-20201010Z-111a26",
  "org.scalablytyped" %%% "express" % "4.17-dt-20201111Z-2d5437",
  "org.scalablytyped" %%% "express-serve-static-core" % "4.17-dt-20200921Z-44f8de",
  "org.scalablytyped" %%% "jquery" % "3.5-dt-20201028Z-b65a9c",
  "org.scalablytyped" %%% "jsonata" % "1.8.4-0a165e",
  "org.scalablytyped" %%% "mime" % "2.0-dt-20200722Z-162477",
  "org.scalablytyped" %%% "node" % "14.14-dt-20201119Z-11c649",
  "org.scalablytyped" %%% "node-red__editor-api" % "1.1-dt-20200925Z-5f822e",
  "org.scalablytyped" %%% "node-red__editor-client" % "1.1-dt-20201028Z-e8161f",
  "org.scalablytyped" %%% "node-red__registry" % "1.1-dt-20200925Z-bfed9a",
  "org.scalablytyped" %%% "node-red__runtime" % "1.1-dt-20201028Z-7cdbcb",
  "org.scalablytyped" %%% "node-red__util" % "1.1-dt-20200925Z-c2880f",
  "org.scalablytyped" %%% "passport" % "1.0-dt-20201112Z-82de57",
  "org.scalablytyped" %%% "qs" % "6.9-dt-20201002Z-64e6f9",
  "org.scalablytyped" %%% "range-parser" % "1.2-dt-20201002Z-329d84",
  "org.scalablytyped" %%% "serve-static" % "1.13-dt-20201117Z-bcf4d7",
  "org.scalablytyped" %%% "sizzle" % "2.3-dt-20201002Z-00b6a0",
  "org.scalablytyped" %%% "std" % "4.1-f7e8ab")
publishArtifact in packageDoc := false
scalacOptions ++= List("-encoding", "utf-8", "-feature", "-language:implicitConversions", "-language:higherKinds", "-language:existentials", "-no-indent")
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
