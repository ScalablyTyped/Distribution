...
object buttonTimerMod {
  
  inline def apply(button: Any): Unit = ^.asInstanceOf[js.Dynamic].apply(button.asInstanceOf[js.Any]).asInstanceOf[Unit]
  ...
  object ButtonTimer {
    ...
    inline def apply(button: Any, enabled: Boolean, interval: Double, propertiesToSync_ : Any): ButtonTimer = {
