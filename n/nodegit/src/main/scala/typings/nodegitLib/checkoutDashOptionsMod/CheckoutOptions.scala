package typings
package nodegitLib.checkoutDashOptionsMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("nodegit/checkout-options", "CheckoutOptions")
@js.native
class CheckoutOptions ()
  extends /* key */ ScalablyTyped.runtime.StringDictionary[js.Any] {
  var ancestorLabel: js.UndefOr[java.lang.String] = js.native
  var baseline: js.UndefOr[nodegitLib.treeMod.Tree] = js.native
  var baselineIndex: js.UndefOr[nodegitLib.nodegitMod.Index] = js.native
  var checkoutStrategy: js.UndefOr[scala.Double] = js.native
  var dirMode: js.UndefOr[scala.Double] = js.native
  var disableFilters: js.UndefOr[scala.Double] = js.native
  var fileMode: js.UndefOr[scala.Double] = js.native
  var fileOpenFlags: js.UndefOr[scala.Double] = js.native
  var notifyCb: js.UndefOr[js.Any] = js.native
  var notifyFlags: js.UndefOr[scala.Double] = js.native
  var notifyPayload: js.UndefOr[scala.Nothing] = js.native
  var ourLabel: js.UndefOr[java.lang.String] = js.native
  var paths: js.UndefOr[
    nodegitLib.strDashArrayMod.Strarray | java.lang.String | js.Array[java.lang.String]
  ] = js.native
  var perfdataCb: js.UndefOr[js.Any] = js.native
  var perfdataPayload: js.UndefOr[scala.Nothing] = js.native
  var progressCb: js.UndefOr[js.Any] = js.native
  var progressPayload: js.UndefOr[scala.Nothing] = js.native
  var targetDirectory: js.UndefOr[java.lang.String] = js.native
  var theirLabel: js.UndefOr[java.lang.String] = js.native
  var version: js.UndefOr[scala.Double] = js.native
}

