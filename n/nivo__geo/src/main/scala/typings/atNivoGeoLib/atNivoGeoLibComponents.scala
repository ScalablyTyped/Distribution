package typings
package atNivoGeoLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object atNivoGeoLibComponents {
  @scala.inline
  def Choropleth: reactLib.reactMod.ComponentType[
    atNivoGeoLib.atNivoGeoMod.ChoroplethProps with atNivoCoreLib.atNivoCoreMod.Dimensions
  ] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.Choropleth].asInstanceOf[reactLib.reactMod.ComponentType[
  atNivoGeoLib.atNivoGeoMod.ChoroplethProps with atNivoCoreLib.atNivoCoreMod.Dimensions]]
  @scala.inline
  def ChoroplethCanvas: reactLib.reactMod.ComponentType[
    atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps with atNivoCoreLib.atNivoCoreMod.Dimensions
  ] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.ChoroplethCanvas].asInstanceOf[reactLib.reactMod.ComponentType[
  atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps with atNivoCoreLib.atNivoCoreMod.Dimensions]]
  @scala.inline
  def GeoMap: reactLib.reactMod.ComponentType[
    atNivoGeoLib.atNivoGeoMod.GeoMapProps with atNivoCoreLib.atNivoCoreMod.Dimensions
  ] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.GeoMap].asInstanceOf[reactLib.reactMod.ComponentType[
  atNivoGeoLib.atNivoGeoMod.GeoMapProps with atNivoCoreLib.atNivoCoreMod.Dimensions]]
  @scala.inline
  def GeoMapCanvas: reactLib.reactMod.ComponentType[
    atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps with atNivoCoreLib.atNivoCoreMod.Dimensions
  ] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.GeoMapCanvas].asInstanceOf[reactLib.reactMod.ComponentType[
  atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps with atNivoCoreLib.atNivoCoreMod.Dimensions]]
  @scala.inline
  def ResponsiveChoropleth: reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.ChoroplethProps] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.ResponsiveChoropleth].asInstanceOf[reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.ChoroplethProps]]
  @scala.inline
  def ResponsiveChoroplethCanvas: reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.ResponsiveChoroplethCanvas].asInstanceOf[reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps]]
  @scala.inline
  def ResponsiveGeoMap: reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.GeoMapProps] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.ResponsiveGeoMap].asInstanceOf[reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.GeoMapProps]]
  @scala.inline
  def ResponsiveGeoMapCanvas: reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps] = js.constructorOf[atNivoGeoLib.atNivoGeoMod.ResponsiveGeoMapCanvas].asInstanceOf[reactLib.reactMod.ComponentType[atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps]]
  @scala.inline
  def ChoroplethCanvasProps: atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps.type = atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps
  type ChoroplethCanvasProps = atNivoGeoLib.atNivoGeoMod.ChoroplethCanvasProps
  type ChoroplethProps = atNivoGeoLib.atNivoGeoMod.ChoroplethProps
  @scala.inline
  def GeoMapCanvasProps: atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps.type = atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps
  type GeoMapCanvasProps = atNivoGeoLib.atNivoGeoMod.GeoMapCanvasProps
  type GeoMapProps = atNivoGeoLib.atNivoGeoMod.GeoMapProps
}

