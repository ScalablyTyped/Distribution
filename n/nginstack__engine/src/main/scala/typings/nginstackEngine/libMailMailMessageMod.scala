...
object libMailMailMessageMod {
  ...
  object MailMessage {
    ...
    inline def apply(decode: String => Unit, header: MailMessageHeader, messagesParts: js.Array[Any]): MailMessage = {
  ...
  type MailMessageHeader = typings.nginstackEngine.libMailMailMessageHeaderMod.^
}
