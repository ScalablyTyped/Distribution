...
object Anon_AmpBindInitData {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(ampBindInitData = ampBindInitData, buildId = buildId, canonicalBase = canonicalBase, documentMiddlewareEnabled = documentMiddlewareEnabled, generateEtags = generateEtags, hasCssMode = hasCssMode, poweredByHeader = poweredByHeader, staticMarkup = staticMarkup)
