organization := "org.scalablytyped"
name := "fastify-static"
version := "2.4.0-406888"
scalaVersion := "2.12.8"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "1.0.0",
  "org.scala-js" %%% "scalajs-dom" % "0.9.6",
  "org.scalablytyped" %%% "ajv" % "6.10.0-4b0839",
  "org.scalablytyped" %%% "archy" % "0.0-unknown-dt-20190322Z-751faa",
  "org.scalablytyped" %%% "avvio" % "6.2.1-6bb39f",
  "org.scalablytyped" %%% "debug" % "4.1-dt-20190328Z-74c562",
  "org.scalablytyped" %%% "deepmerge" % "3.2.0-a1baa5",
  "org.scalablytyped" %%% "fast-deep-equal" % "2.0.1-42ef87",
  "org.scalablytyped" %%% "fast-json-stable-stringify" % "2.0-dt-20181224Z-82585c",
  "org.scalablytyped" %%% "fast-json-stringify" % "1.14.0-548fc3",
  "org.scalablytyped" %%% "fastify" % "2.2.0-336651",
  "org.scalablytyped" %%% "fastify-plugin" % "1.5.0-979a5c",
  "org.scalablytyped" %%% "fastq" % "1.6.0-da4a5e",
  "org.scalablytyped" %%% "find-my-way" % "2.0.1-1f6122",
  "org.scalablytyped" %%% "glob" % "7.1-dt-20180920Z-3f0045",
  "org.scalablytyped" %%% "light-my-request" % "3.3.0-830533",
  "org.scalablytyped" %%% "mime" % "2.0-dt-20190205Z-2631f3",
  "org.scalablytyped" %%% "minimatch" % "3.0-dt-20180214Z-e6c927",
  "org.scalablytyped" %%% "node" % "11.13-dt-20190408Z-6fa8bf",
  "org.scalablytyped" %%% "pino" % "5.8-dt-20190319Z-bcc3f1",
  "org.scalablytyped" %%% "proxy-addr" % "2.0-dt-20180214Z-f7647b",
  "org.scalablytyped" %%% "punycode" % "2.1-dt-20181205Z-63baaa",
  "org.scalablytyped" %%% "readable-stream" % "2.3-dt-20190227Z-2b0048",
  "org.scalablytyped" %%% "safe-buffer" % "5.1.2-32e430",
  "org.scalablytyped" %%% "semver" % "6.0-dt-20190330Z-1b8b1f",
  "org.scalablytyped" %%% "send" % "v0.14.1-dt-20190322Z-bb7c0f",
  "org.scalablytyped" %%% "sonic-boom" % "0.6-dt-20190212Z-0e519c",
  "org.scalablytyped" %%% "std" % "3.4-d52857",
  "org.scalablytyped" %%% "uri-js" % "4.2.2-8cad7e")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
        