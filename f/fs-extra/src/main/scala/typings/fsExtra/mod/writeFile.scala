...
object writeFile extends js.Object {
  ...
  @JSName("__promisify__")
  def promisify(path: Double, data: js.Any, options: typings.node.fsMod.WriteFileOptions): js.Promise[Unit] = js.native
  /**
    * Asynchronously writes data to a file, replacing the file if it already exists.
    * @param path A path to a file. If a URL is provided, it must use the `file:` protocol.
    * URL support is _experimental_.
  ...
  @JSName("__promisify__")
  def promisify(path: PathLike, data: js.Any, options: typings.node.fsMod.WriteFileOptions): js.Promise[Unit] = js.native
}
