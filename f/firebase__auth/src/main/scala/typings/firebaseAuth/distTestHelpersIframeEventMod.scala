...
import typings.firebaseAuth.anon.PartialAuthEventUrlResponse
...
object distTestHelpersIframeEventMod {
  ...
  inline def authEvent(event: PartialAuthEventUrlResponse): AuthEvent = ^.asInstanceOf[js.Dynamic].applyDynamic("authEvent")(event.asInstanceOf[js.Any]).asInstanceOf[AuthEvent]
}
