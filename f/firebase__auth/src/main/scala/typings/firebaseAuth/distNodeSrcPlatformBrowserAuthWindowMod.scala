...
object distNodeSrcPlatformBrowserAuthWindowMod {
  ...
  /* Inlined {  grecaptcha :@firebase/auth.@firebase/auth/dist/node/src/platform_browser/recaptcha/recaptcha.Recaptcha | undefined,   ___jsl :std.Record<string, any> | undefined,   gapi :{readonly load (features : 'gapi.iframes', options : @firebase/auth.gapi.LoadOptions | @firebase/auth.gapi.LoadCallback | undefined): void,  readonly iframes :{  Context :new (): @firebase/auth.gapi.iframes.Context,   Iframe :new (): @firebase/auth.gapi.iframes.Iframe,  readonly CROSS_ORIGIN_IFRAMES_FILTER :@firebase/auth.gapi.iframes.IframesFilter, readonly getContext (): @firebase/auth.gapi.iframes.Context}} | undefined, [callback: string] : (args : ...unknown): void} & {[ T in keyof std.Window ]: std.Window[T]} */
  trait AuthWindow
    extends StObject
