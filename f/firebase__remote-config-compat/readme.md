
# Scala.js typings for firebase__remote-config-compat

Typings are for version 0.1.12

## Library description:
The compatibility package of Remote Config

|                    |                 |
| ------------------ | :-------------: |
| Full name          | @firebase/remote-config-compat |
| Keywords           | - |
| # releases         | 454 |
| # dependents       | 0.0 |
| # downloads        | 2643551.0 |
| # stars            | ? |

## Links
- [Homepage](https://github.com/firebase/firebase-js-sdk#readme)
- [Bugs](https://github.com/firebase/firebase-js-sdk/issues)
- [Repository](https://github.com/firebase/firebase-js-sdk)
- [Npm](https://www.npmjs.com/package/%40firebase%2Fremote-config-compat)
    


## Note
This library has been generated from typescript code from first party type definitions.

Provided with :purple_heart: from [ScalablyTyped](https://github.com/oyvindberg/ScalablyTyped)

## Usage
See [the main readme](../../readme.md) for instructions.


