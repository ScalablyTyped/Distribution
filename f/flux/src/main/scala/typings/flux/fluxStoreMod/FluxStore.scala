...
object FluxStore {
  ...
  implicit class FluxStoreOps[Self <: FluxStore[_], TPayload] (val x: Self with FluxStore[TPayload]) extends AnyVal {
    ...
    @scala.inline
    def set__dispatcher(value: typings.flux.dispatcherMod.^[TPayload]): Self = this.set("__dispatcher", value.asInstanceOf[js.Any])
    ...
    @scala.inline
    def setGetDispatcher(value: () => typings.flux.dispatcherMod.^[TPayload]): Self = this.set("getDispatcher", js.Any.fromFunction0(value))
