...
/* RemoveMultipleInheritance: Dropped parents List(fabricLib.fabricDashImplMod.ICanvasOptions because Would inherit conflicting mutable fields List(allowTouchScrolling, controlsAboveOverlay, backgroundColor, includeDefaultValues, imageSmoothingEnabled, backgroundImage, stateful, overlayImageLeft, freeDrawingColor, overlayImage, viewportTransform, backgroundImageStretch, freeDrawingLineWidth, backgroundImageOpacity, renderOnAddRemove, overlayImageTop, enableRetinaScaling, preserveObjectStacking, clipTo, overlayColor))*/
