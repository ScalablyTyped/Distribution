...
trait NullSchema
  extends BaseSchema
     with Schema {
  var `type`: fastDashJsonDashStringifyLib.fastDashJsonDashStringifyLibStrings.`null`
}

object NullSchema {
  @scala.inline
  def apply(
    `type`: fastDashJsonDashStringifyLib.fastDashJsonDashStringifyLibStrings.`null`,
