...
trait NumberSchema
  extends BaseSchema
     with Schema {
  var `type`: fastDashJsonDashStringifyLib.fastDashJsonDashStringifyLibStrings.number
}

object NumberSchema {
  @scala.inline
  def apply(
    `type`: fastDashJsonDashStringifyLib.fastDashJsonDashStringifyLibStrings.number,
