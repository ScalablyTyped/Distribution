package typings.flickity

import typings.flickity.mod.FlickityEvents
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object flickityStrings {
  
  @js.native
  sealed trait bgLazyLoad
    extends StObject
       with FlickityEvents
  inline def bgLazyLoad: bgLazyLoad = "bgLazyLoad".asInstanceOf[bgLazyLoad]
  
  @js.native
  sealed trait cellSelect
    extends StObject
       with FlickityEvents
  inline def cellSelect: cellSelect = "cellSelect".asInstanceOf[cellSelect]
  
  @js.native
  sealed trait change
    extends StObject
       with FlickityEvents
  inline def change: change = "change".asInstanceOf[change]
  
  @js.native
  sealed trait destroy
    extends StObject
       with FlickityEvents
  inline def destroy: destroy = "destroy".asInstanceOf[destroy]
  
  @js.native
  sealed trait dragEnd
    extends StObject
       with FlickityEvents
  inline def dragEnd: dragEnd = "dragEnd".asInstanceOf[dragEnd]
  
  @js.native
  sealed trait dragMove
    extends StObject
       with FlickityEvents
  inline def dragMove: dragMove = "dragMove".asInstanceOf[dragMove]
  
  @js.native
  sealed trait dragStart
    extends StObject
       with FlickityEvents
  inline def dragStart: dragStart = "dragStart".asInstanceOf[dragStart]
  
  @js.native
  sealed trait fullscreenChange
    extends StObject
       with FlickityEvents
  inline def fullscreenChange: fullscreenChange = "fullscreenChange".asInstanceOf[fullscreenChange]
  
  @js.native
  sealed trait lazyLoad
    extends StObject
       with FlickityEvents
  inline def lazyLoad: lazyLoad = "lazyLoad".asInstanceOf[lazyLoad]
  
  @js.native
  sealed trait pointerDown
    extends StObject
       with FlickityEvents
  inline def pointerDown: pointerDown = "pointerDown".asInstanceOf[pointerDown]
  
  @js.native
  sealed trait pointerMove
    extends StObject
       with FlickityEvents
  inline def pointerMove: pointerMove = "pointerMove".asInstanceOf[pointerMove]
  
  @js.native
  sealed trait pointerUp
    extends StObject
       with FlickityEvents
  inline def pointerUp: pointerUp = "pointerUp".asInstanceOf[pointerUp]
  
  @js.native
  sealed trait scroll
    extends StObject
       with FlickityEvents
  inline def scroll: scroll = "scroll".asInstanceOf[scroll]
  
  @js.native
  sealed trait select
    extends StObject
       with FlickityEvents
  inline def select: select = "select".asInstanceOf[select]
  
  @js.native
  sealed trait settle
    extends StObject
       with FlickityEvents
  inline def settle: settle = "settle".asInstanceOf[settle]
  
  @js.native
  sealed trait staticClick
    extends StObject
       with FlickityEvents
  inline def staticClick: staticClick = "staticClick".asInstanceOf[staticClick]
}
