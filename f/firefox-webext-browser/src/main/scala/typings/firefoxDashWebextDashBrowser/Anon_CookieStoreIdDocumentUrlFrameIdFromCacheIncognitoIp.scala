...
object Anon_CookieStoreIdDocumentUrlFrameIdFromCacheIncognitoIp {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(frameId = frameId, fromCache = fromCache, method = method, parentFrameId = parentFrameId, requestId = requestId, statusCode = statusCode, statusLine = statusLine, tabId = tabId, timeStamp = timeStamp, url = url, urlClassification = urlClassification)
