package typings.firebaseDatabase

import typings.firebaseDatabase.distSrcCoreViewFilterNodeFilterMod.NodeFilter
import typings.firebaseDatabase.distSrcCoreViewQueryParamsMod.QueryParams
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object distSrcCoreViewFilterLimitedFilterMod {
  
  @JSImport("@firebase/database/dist/src/core/view/filter/LimitedFilter", "LimitedFilter")
  @js.native
  open class LimitedFilter protected ()
    extends StObject
       with NodeFilter {
    def this(params: QueryParams) = this()
    
    /* private */ val endIsInclusive_ : Any = js.native
    
    /* private */ var fullLimitUpdateChild_ : Any = js.native
    
    /* private */ val index_ : Any = js.native
    
    /* private */ val limit_ : Any = js.native
    
    /* private */ val rangedFilter_ : Any = js.native
    
    /* private */ val reverse_ : Any = js.native
    
    /* private */ val startIsInclusive_ : Any = js.native
    
    /* private */ var withinDirectionalEnd: Any = js.native
    
    /* private */ var withinDirectionalStart: Any = js.native
    
    /* private */ var withinEndPost: Any = js.native
    
    /* private */ var withinStartPost: Any = js.native
  }
}
