...
trait PackagedProductDefinitionPackagingContainedItem
  extends StObject
...
object PackagedProductDefinitionPackagingContainedItem {
  
  inline def apply(item: CodeableReference): PackagedProductDefinitionPackagingContainedItem = {
    val __obj = js.Dynamic.literal(item = item.asInstanceOf[js.Any])
    __obj.asInstanceOf[PackagedProductDefinitionPackagingContainedItem]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: PackagedProductDefinitionPackagingContainedItem] (val x: Self) extends AnyVal {
