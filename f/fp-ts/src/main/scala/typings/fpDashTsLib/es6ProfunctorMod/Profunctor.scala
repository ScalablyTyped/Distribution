package typings
package fpDashTsLib.es6ProfunctorMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Profunctor[F] extends js.Object {
  val URI: F
  def map[L, A, B](fa: fpDashTsLib.es6HKTMod.HKT2[F, L, A], f: js.Function1[/* a */ A, B]): fpDashTsLib.es6HKTMod.HKT2[F, L, B]
  def promap[A, B, C, D](
    fbc: fpDashTsLib.es6HKTMod.HKT2[F, B, C],
    f: js.Function1[/* a */ A, B],
    g: js.Function1[/* c */ C, D]
  ): fpDashTsLib.es6HKTMod.HKT2[F, A, D]
}

object Profunctor {
  @scala.inline
  def apply[F](
    URI: F,
    map: (fpDashTsLib.es6HKTMod.HKT2[F, js.Any, js.Any], js.Function1[js.Any, js.Any]) => fpDashTsLib.es6HKTMod.HKT2[F, js.Any, js.Any],
    promap: (fpDashTsLib.es6HKTMod.HKT2[F, js.Any, js.Any], js.Function1[js.Any, js.Any], js.Function1[js.Any, js.Any]) => fpDashTsLib.es6HKTMod.HKT2[F, js.Any, js.Any]
  ): Profunctor[F] = {
    val __obj = js.Dynamic.literal(URI = URI.asInstanceOf[js.Any], map = js.Any.fromFunction2(map), promap = js.Any.fromFunction3(promap))
  
    __obj.asInstanceOf[Profunctor[F]]
  }
}

