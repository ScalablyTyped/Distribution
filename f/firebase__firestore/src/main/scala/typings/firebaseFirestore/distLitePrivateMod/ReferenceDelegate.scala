...
trait ReferenceDelegate extends StObject {
  ...
  def addReference(
    ...
    targetId: TargetId,
    doc: /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any
...
object ReferenceDelegate {
  ...
  inline def apply(
    addReference: (PersistenceTransaction, TargetId, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit],
    markPotentiallyOrphaned: (PersistenceTransaction, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit],
    removeReference: (PersistenceTransaction, TargetId, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit],
    removeTarget: (PersistenceTransaction, TargetData) => PersistencePromise[Unit],
    updateLimboDocument: (PersistenceTransaction, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit]
  ): ReferenceDelegate = {
  ...
  implicit open class MutableBuilder[Self <: ReferenceDelegate] (val x: Self) extends AnyVal {
    ...
    inline def setAddReference(
      value: (PersistenceTransaction, TargetId, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit]
    ): Self = StObject.set(x, "addReference", js.Any.fromFunction3(value))
    
    inline def setMarkPotentiallyOrphaned(
      value: (PersistenceTransaction, /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify DocumentKey */ Any) => PersistencePromise[Unit]
