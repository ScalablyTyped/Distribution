...
object distFirestoreSrcLocalBundleCacheMod {
  ...
  object BundleCache {
    ...
    inline def apply(
      ...
      saveBundleMetadata: (PersistenceTransaction, typings.firebaseFirestore.distFirestoreSrcProtosFirestoreBundleProtoMod.BundleMetadata) => PersistencePromise[Unit],
      saveNamedQuery: (PersistenceTransaction, typings.firebaseFirestore.distFirestoreSrcProtosFirestoreBundleProtoMod.NamedQuery) => PersistencePromise[Unit]
    ): BundleCache = {
    ...
    implicit open class MutableBuilder[Self <: BundleCache] (val x: Self) extends AnyVal {
      ...
      inline def setSaveBundleMetadata(
        value: (PersistenceTransaction, typings.firebaseFirestore.distFirestoreSrcProtosFirestoreBundleProtoMod.BundleMetadata) => PersistencePromise[Unit]
      ): Self = StObject.set(x, "saveBundleMetadata", js.Any.fromFunction2(value))
      
      inline def setSaveNamedQuery(
        value: (PersistenceTransaction, typings.firebaseFirestore.distFirestoreSrcProtosFirestoreBundleProtoMod.NamedQuery) => PersistencePromise[Unit]
