...
object distLiteFirestoreSrcLocalBundleCacheMod {
  ...
  object BundleCache {
    ...
    inline def apply(
      ...
      saveBundleMetadata: (PersistenceTransaction, typings.firebaseFirestore.distLiteFirestoreSrcProtosFirestoreBundleProtoMod.BundleMetadata) => PersistencePromise[Unit],
      saveNamedQuery: (PersistenceTransaction, typings.firebaseFirestore.distLiteFirestoreSrcProtosFirestoreBundleProtoMod.NamedQuery) => PersistencePromise[Unit]
    ): BundleCache = {
    ...
    extension [Self <: BundleCache](x: Self) {
      ...
      inline def setSaveBundleMetadata(
        value: (PersistenceTransaction, typings.firebaseFirestore.distLiteFirestoreSrcProtosFirestoreBundleProtoMod.BundleMetadata) => PersistencePromise[Unit]
      ): Self = StObject.set(x, "saveBundleMetadata", js.Any.fromFunction2(value))
      
      inline def setSaveNamedQuery(
        value: (PersistenceTransaction, typings.firebaseFirestore.distLiteFirestoreSrcProtosFirestoreBundleProtoMod.NamedQuery) => PersistencePromise[Unit]
