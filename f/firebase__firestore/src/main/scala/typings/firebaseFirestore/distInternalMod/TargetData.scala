...
trait TargetData extends StObject {
  ...
  /**
    ...
    * snapshot version.
    */
  def withResumeToken(resumeToken: ByteString, snapshotVersion: SnapshotVersion): TargetData
...
object TargetData {
  ...
  inline def apply(
    ...
    withResumeToken: (ByteString, SnapshotVersion) => TargetData,
  ...
  extension [Self <: TargetData](x: Self) {
    ...
    inline def setWithResumeToken(value: (ByteString, SnapshotVersion) => TargetData): Self = StObject.set(x, "withResumeToken", js.Any.fromFunction2(value))
