...
object s3Ns extends js.Object {
  ...
  /* RemoveMultipleInheritance: Dropped parents List(fineDashUploaderLib.libS3Mod.s3Ns.S3CoreOptions because Would inherit conflicting mutable fields List(chunking, messages, resume, request, workarounds, paste, extraButtons, camera, scaling, text, blobs, debug, maxConnections, cors, button, formatFileName, autoUpload, disableCancelForFormUploads, deleteFile, session, multiple, form, callbacks, warnBeforeUnload, validation))*/
