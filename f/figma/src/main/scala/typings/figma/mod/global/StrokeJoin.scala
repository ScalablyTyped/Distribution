package typings.figma.mod.global

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.figma.figmaStrings.MITER
  - typings.figma.figmaStrings.BEVEL
  - typings.figma.figmaStrings.ROUND
*/
trait StrokeJoin extends StObject
object StrokeJoin {
  
  inline def BEVEL: typings.figma.figmaStrings.BEVEL = "BEVEL".asInstanceOf[typings.figma.figmaStrings.BEVEL]
  
  inline def MITER: typings.figma.figmaStrings.MITER = "MITER".asInstanceOf[typings.figma.figmaStrings.MITER]
  
  inline def ROUND: typings.figma.figmaStrings.ROUND = "ROUND".asInstanceOf[typings.figma.figmaStrings.ROUND]
}
