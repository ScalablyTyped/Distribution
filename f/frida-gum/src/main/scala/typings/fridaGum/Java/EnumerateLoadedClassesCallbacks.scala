...
object EnumerateLoadedClassesCallbacks {
  ...
  inline def apply(onComplete: () => Unit, onMatch: (String, NativePointer) => Unit): EnumerateLoadedClassesCallbacks = {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: EnumerateLoadedClassesCallbacks] (val x: Self) extends AnyVal {
    
    inline def setOnComplete(value: () => Unit): Self = StObject.set(x, "onComplete", js.Any.fromFunction0(value))
    
    inline def setOnMatch(value: (String, NativePointer) => Unit): Self = StObject.set(x, "onMatch", js.Any.fromFunction2(value))
  }
}
