package typings.fridaDashGum

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* import warning: RemoveMultipleInheritance.findNewParents newComments Dropped parents 
- typings.fridaDashGum.CpuContext because Already inherited */ trait MipsCpuContext extends PortableCpuContext {
  var a0: NativePointer
  var a1: NativePointer
  var a2: NativePointer
  var a3: NativePointer
  var at: NativePointer
  var fp: NativePointer
  var gp: NativePointer
  var hi: NativePointer
  var k0: NativePointer
  var k1: NativePointer
  var lo: NativePointer
  var ra: NativePointer
  var s0: NativePointer
  var s1: NativePointer
  var s2: NativePointer
  var s3: NativePointer
  var s4: NativePointer
  var s5: NativePointer
  var s6: NativePointer
  var s7: NativePointer
  var t0: NativePointer
  var t1: NativePointer
  var t2: NativePointer
  var t3: NativePointer
  var t4: NativePointer
  var t5: NativePointer
  var t6: NativePointer
  var t7: NativePointer
  var t8: NativePointer
  var t9: NativePointer
  var v0: NativePointer
  var v1: NativePointer
}

object MipsCpuContext {
  @scala.inline
  def apply(
    a0: NativePointer,
    a1: NativePointer,
    a2: NativePointer,
    a3: NativePointer,
    at: NativePointer,
    fp: NativePointer,
    gp: NativePointer,
    hi: NativePointer,
    k0: NativePointer,
    k1: NativePointer,
    lo: NativePointer,
    pc: NativePointer,
    ra: NativePointer,
    s0: NativePointer,
    s1: NativePointer,
    s2: NativePointer,
    s3: NativePointer,
    s4: NativePointer,
    s5: NativePointer,
    s6: NativePointer,
    s7: NativePointer,
    sp: NativePointer,
    t0: NativePointer,
    t1: NativePointer,
    t2: NativePointer,
    t3: NativePointer,
    t4: NativePointer,
    t5: NativePointer,
    t6: NativePointer,
    t7: NativePointer,
    t8: NativePointer,
    t9: NativePointer,
    v0: NativePointer,
    v1: NativePointer
  ): MipsCpuContext = {
    val __obj = js.Dynamic.literal(a0 = a0, a1 = a1, a2 = a2, a3 = a3, at = at, fp = fp, gp = gp, hi = hi, k0 = k0, k1 = k1, lo = lo, pc = pc, ra = ra, s0 = s0, s1 = s1, s2 = s2, s3 = s3, s4 = s4, s5 = s5, s6 = s6, s7 = s7, sp = sp, t0 = t0, t1 = t1, t2 = t2, t3 = t3, t4 = t4, t5 = t5, t6 = t6, t7 = t7, t8 = t8, t9 = t9, v0 = v0, v1 = v1)
  
    __obj.asInstanceOf[MipsCpuContext]
  }
}

