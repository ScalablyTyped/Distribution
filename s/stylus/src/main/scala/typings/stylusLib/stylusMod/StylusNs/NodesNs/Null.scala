...
object Null {
  ...
  def apply(
    ...
    toJSON: js.Function0[stylusLib.Anon_ColumnFilenameLinenoTypeNumber]
  ): Null = {
    val __obj = js.Dynamic.literal(clone = clone, coerce = coerce, column = column, eval = eval, filename = filename, first = first, hash = hash, isNull = isNull, lineno = lineno, nodeName = nodeName, operate = operate, shouldCoerce = shouldCoerce, toBoolean = toBoolean, toExpression = toExpression, toJSON = toJSON)
