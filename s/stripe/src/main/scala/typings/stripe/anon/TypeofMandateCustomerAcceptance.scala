...
trait TypeofMandateCustomerAcceptance extends StObject {
  
  val CustomerAcceptance: Any
  
  val PaymentMethodDetails: TypeofPaymentMethodDetailAcssDebit
}
object TypeofMandateCustomerAcceptance {
  
  inline def apply(CustomerAcceptance: Any, PaymentMethodDetails: TypeofPaymentMethodDetailAcssDebit): TypeofMandateCustomerAcceptance = {
    ...
    __obj.asInstanceOf[TypeofMandateCustomerAcceptance]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TypeofMandateCustomerAcceptance] (val x: Self) extends AnyVal {
