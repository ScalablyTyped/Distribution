...
/* Inlined sigstore.sigstore/dist/types/utility.WithRequired<@sigstore/protobuf-specs.@sigstore/protobuf-specs.ArtifactVerificationOptions, 'ctlogOptions'> & {  signers :std.Extract<@sigstore/protobuf-specs.@sigstore/protobuf-specs.ArtifactVerificationOptions['signers'], {  $case :'certificateIdentities'}> | undefined} */
trait CAArtifactVerificationOptions extends StObject {
...
object CAArtifactVerificationOptions {
  
  inline def apply(): CAArtifactVerificationOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[CAArtifactVerificationOptions]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: CAArtifactVerificationOptions] (val x: Self) extends AnyVal {
