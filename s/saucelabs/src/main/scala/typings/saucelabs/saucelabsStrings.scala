package typings.saucelabs

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object saucelabsStrings {
  
  @js.native
  sealed trait ABORTED extends StObject
  inline def ABORTED: ABORTED = "ABORTED".asInstanceOf[ABORTED]
  
  @js.native
  sealed trait ANDROID extends StObject
  inline def ANDROID: ANDROID = "ANDROID".asInstanceOf[ANDROID]
  
  @js.native
  sealed trait ANDROID_INSTRUMENTATION extends StObject
  inline def ANDROID_INSTRUMENTATION: ANDROID_INSTRUMENTATION = "ANDROID_INSTRUMENTATION".asInstanceOf[ANDROID_INSTRUMENTATION]
  
  @js.native
  sealed trait ANY extends StObject
  inline def ANY: ANY = "ANY".asInstanceOf[ANY]
  
  @js.native
  sealed trait APPIUM extends StObject
  inline def APPIUM: APPIUM = "APPIUM".asInstanceOf[APPIUM]
  
  @js.native
  sealed trait ARM extends StObject
  inline def ARM: ARM = "ARM".asInstanceOf[ARM]
  
  @js.native
  sealed trait ASCENDING extends StObject
  inline def ASCENDING: ASCENDING = "ASCENDING".asInstanceOf[ASCENDING]
  
  @js.native
  sealed trait AVAILABLE extends StObject
  inline def AVAILABLE: AVAILABLE = "AVAILABLE".asInstanceOf[AVAILABLE]
  
  @js.native
  sealed trait BATCH extends StObject
  inline def BATCH: BATCH = "BATCH".asInstanceOf[BATCH]
  
  @js.native
  sealed trait BROKEN extends StObject
  inline def BROKEN: BROKEN = "BROKEN".asInstanceOf[BROKEN]
  
  @js.native
  sealed trait CANCELED extends StObject
  inline def CANCELED: CANCELED = "CANCELED".asInstanceOf[CANCELED]
  
  @js.native
  sealed trait DESCENDING extends StObject
  inline def DESCENDING: DESCENDING = "DESCENDING".asInstanceOf[DESCENDING]
  
  @js.native
  sealed trait EDGE extends StObject
  inline def EDGE: EDGE = "EDGE".asInstanceOf[EDGE]
  
  @js.native
  sealed trait ERROR extends StObject
  inline def ERROR: ERROR = "ERROR".asInstanceOf[ERROR]
  
  @js.native
  sealed trait ESPRESSO extends StObject
  inline def ESPRESSO: ESPRESSO = "ESPRESSO".asInstanceOf[ESPRESSO]
  
  @js.native
  sealed trait FAILURE extends StObject
  inline def FAILURE: FAILURE = "FAILURE".asInstanceOf[FAILURE]
  
  @js.native
  sealed trait FULL extends StObject
  inline def FULL: FULL = "FULL".asInstanceOf[FULL]
  
  @js.native
  sealed trait GPRS extends StObject
  inline def GPRS: GPRS = "GPRS".asInstanceOf[GPRS]
  
  @js.native
  sealed trait GSM extends StObject
  inline def GSM: GSM = "GSM".asInstanceOf[GSM]
  
  @js.native
  sealed trait HDCSD extends StObject
  inline def HDCSD: HDCSD = "HDCSD".asInstanceOf[HDCSD]
  
  @js.native
  sealed trait HSDPA extends StObject
  inline def HSDPA: HSDPA = "HSDPA".asInstanceOf[HSDPA]
  
  @js.native
  sealed trait INSTALL_LAUNCH extends StObject
  inline def INSTALL_LAUNCH: INSTALL_LAUNCH = "INSTALL_LAUNCH".asInstanceOf[INSTALL_LAUNCH]
  
  @js.native
  sealed trait IN_USE extends StObject
  inline def IN_USE: IN_USE = "IN_USE".asInstanceOf[IN_USE]
  
  @js.native
  sealed trait IOS extends StObject
  inline def IOS: IOS = "IOS".asInstanceOf[IOS]
  
  @js.native
  sealed trait IPAD extends StObject
  inline def IPAD: IPAD = "IPAD".asInstanceOf[IPAD]
  
  @js.native
  sealed trait IPHONE extends StObject
  inline def IPHONE: IPHONE = "IPHONE".asInstanceOf[IPHONE]
  
  @js.native
  sealed trait LANDSCAPE extends StObject
  inline def LANDSCAPE: LANDSCAPE = "LANDSCAPE".asInstanceOf[LANDSCAPE]
  
  @js.native
  sealed trait MANUAL extends StObject
  inline def MANUAL: MANUAL = "MANUAL".asInstanceOf[MANUAL]
  
  @js.native
  sealed trait NONE extends StObject
  inline def NONE: NONE = "NONE".asInstanceOf[NONE]
  
  @js.native
  sealed trait OFFLINE extends StObject
  inline def OFFLINE: OFFLINE = "OFFLINE".asInstanceOf[OFFLINE]
  
  @js.native
  sealed trait OK extends StObject
  inline def OK: OK = "OK".asInstanceOf[OK]
  
  @js.native
  sealed trait PIRANHA extends StObject
  inline def PIRANHA: PIRANHA = "PIRANHA".asInstanceOf[PIRANHA]
  
  @js.native
  sealed trait PORTRAIT extends StObject
  inline def PORTRAIT: PORTRAIT = "PORTRAIT".asInstanceOf[PORTRAIT]
  
  @js.native
  sealed trait REBOOTING extends StObject
  inline def REBOOTING: REBOOTING = "REBOOTING".asInstanceOf[REBOOTING]
  
  @js.native
  sealed trait ROBOTIUM extends StObject
  inline def ROBOTIUM: ROBOTIUM = "ROBOTIUM".asInstanceOf[ROBOTIUM]
  
  @js.native
  sealed trait SCREENSHOTS extends StObject
  inline def SCREENSHOTS: SCREENSHOTS = "SCREENSHOTS".asInstanceOf[SCREENSHOTS]
  
  @js.native
  sealed trait SKIPPED extends StObject
  inline def SKIPPED: SKIPPED = "SKIPPED".asInstanceOf[SKIPPED]
  
  @js.native
  sealed trait STRESS_TEST extends StObject
  inline def STRESS_TEST: STRESS_TEST = "STRESS_TEST".asInstanceOf[STRESS_TEST]
  
  @js.native
  sealed trait SUCCESS extends StObject
  inline def SUCCESS: SUCCESS = "SUCCESS".asInstanceOf[SUCCESS]
  
  @js.native
  sealed trait TENKOD extends StObject
  inline def TENKOD: TENKOD = "TENKOD".asInstanceOf[TENKOD]
  
  @js.native
  sealed trait UMTS extends StObject
  inline def UMTS: UMTS = "UMTS".asInstanceOf[UMTS]
  
  @js.native
  sealed trait UNDEFINED extends StObject
  inline def UNDEFINED: UNDEFINED = "UNDEFINED".asInstanceOf[UNDEFINED]
  
  @js.native
  sealed trait UNKNOWN extends StObject
  inline def UNKNOWN: UNKNOWN = "UNKNOWN".asInstanceOf[UNKNOWN]
  
  @js.native
  sealed trait UNPLUGGED extends StObject
  inline def UNPLUGGED: UNPLUGGED = "UNPLUGGED".asInstanceOf[UNPLUGGED]
  
  @js.native
  sealed trait WARNING extends StObject
  inline def WARNING: WARNING = "WARNING".asInstanceOf[WARNING]
  
  @js.native
  sealed trait X86 extends StObject
  inline def X86: X86 = "X86".asInstanceOf[X86]
  
  @js.native
  sealed trait XCUITEST extends StObject
  inline def XCUITEST: XCUITEST = "XCUITEST".asInstanceOf[XCUITEST]
  
  @js.native
  sealed trait all extends StObject
  inline def all: all = "all".asInstanceOf[all]
  
  @js.native
  sealed trait appium_ extends StObject
  inline def appium_ : appium_ = "appium".asInstanceOf[appium_]
  
  @js.native
  sealed trait baseline extends StObject
  inline def baseline: baseline = "baseline".asInstanceOf[baseline]
  
  @js.native
  sealed trait complete extends StObject
  inline def complete: complete = "complete".asInstanceOf[complete]
  
  @js.native
  sealed trait error_ extends StObject
  inline def error_ : error_ = "error".asInstanceOf[error_]
  
  @js.native
  sealed trait eu extends StObject
  inline def eu: eu = "eu".asInstanceOf[eu]
  
  @js.native
  sealed trait `eu-central-1` extends StObject
  inline def `eu-central-1`: `eu-central-1` = "eu-central-1".asInstanceOf[`eu-central-1`]
  
  @js.native
  sealed trait failed extends StObject
  inline def failed: failed = "failed".asInstanceOf[failed]
  
  @js.native
  sealed trait marketing extends StObject
  inline def marketing: marketing = "marketing".asInstanceOf[marketing]
  
  @js.native
  sealed trait new_regime extends StObject
  inline def new_regime: new_regime = "new_regime".asInstanceOf[new_regime]
  
  @js.native
  sealed trait passed extends StObject
  inline def passed: passed = "passed".asInstanceOf[passed]
  
  @js.native
  sealed trait public extends StObject
  inline def public: public = "public".asInstanceOf[public]
  
  @js.native
  sealed trait `public restricted` extends StObject
  inline def `public restricted`: `public restricted` = ("public restricted").asInstanceOf[`public restricted`]
  
  @js.native
  sealed trait share extends StObject
  inline def share: share = "share".asInstanceOf[share]
  
  @js.native
  sealed trait staging extends StObject
  inline def staging: staging = "staging".asInstanceOf[staging]
  
  @js.native
  sealed trait team extends StObject
  inline def team: team = "team".asInstanceOf[team]
  
  @js.native
  sealed trait us extends StObject
  inline def us: us = "us".asInstanceOf[us]
  
  @js.native
  sealed trait `us-east-1` extends StObject
  inline def `us-east-1`: `us-east-1` = "us-east-1".asInstanceOf[`us-east-1`]
  
  @js.native
  sealed trait `us-west-1` extends StObject
  inline def `us-west-1`: `us-west-1` = "us-west-1".asInstanceOf[`us-west-1`]
  
  @js.native
  sealed trait webdriver extends StObject
  inline def webdriver: webdriver = "webdriver".asInstanceOf[webdriver]
}
