package typings.styledDashSystem.styledDashSystemMod

import org.scalablytyped.runtime.StringDictionary
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("styled-system", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  val alignContent: styleFn = js.native
  val alignItems: styleFn = js.native
  val alignSelf: styleFn = js.native
  val background: styleFn = js.native
  val backgroundColor: styleFn = js.native
  val backgroundImage: styleFn = js.native
  val backgroundPosition: styleFn = js.native
  val backgroundRepeat: styleFn = js.native
  val backgroundSize: styleFn = js.native
  val border: styleFn = js.native
  val borderBottom: styleFn = js.native
  val borderColor: styleFn = js.native
  val borderLeft: styleFn = js.native
  val borderRadius: styleFn = js.native
  val borderRight: styleFn = js.native
  val borderStyle: styleFn = js.native
  val borderTop: styleFn = js.native
  val borderWidth: styleFn = js.native
  val borders: styleFn = js.native
  val bottom: styleFn = js.native
  val boxShadow: styleFn = js.native
  val buttonStyle: styleFn = js.native
  val color: styleFn = js.native
  val colorStyle: styleFn = js.native
  val display: styleFn = js.native
  val flex: styleFn = js.native
  val flexBasis: styleFn = js.native
  val flexDirection: styleFn = js.native
  val flexWrap: styleFn = js.native
  val flexbox: styleFn = js.native
  val fontFamily: styleFn = js.native
  val fontSize: styleFn = js.native
  val fontStyle: styleFn = js.native
  val fontWeight: styleFn = js.native
  val grid: styleFn = js.native
  val gridArea: styleFn = js.native
  val gridAutoColumns: styleFn = js.native
  val gridAutoFlow: styleFn = js.native
  val gridAutoRows: styleFn = js.native
  val gridColumn: styleFn = js.native
  val gridColumnGap: styleFn = js.native
  val gridGap: styleFn = js.native
  val gridRow: styleFn = js.native
  val gridRowGap: styleFn = js.native
  val gridTemplateAreas: styleFn = js.native
  val gridTemplateColumns: styleFn = js.native
  val gridTemplateRows: styleFn = js.native
  val height: styleFn = js.native
  val justifyContent: styleFn = js.native
  val justifyItems: styleFn = js.native
  val justifySelf: styleFn = js.native
  val layout: styleFn = js.native
  val left: styleFn = js.native
  val letterSpacing: styleFn = js.native
  val lineHeight: styleFn = js.native
  val margin: styleFn = js.native
  val marginBottom: styleFn = js.native
  val marginLeft: styleFn = js.native
  val marginRight: styleFn = js.native
  val marginTop: styleFn = js.native
  val maxHeight: styleFn = js.native
  val maxWidth: styleFn = js.native
  val minHeight: styleFn = js.native
  val minWidth: styleFn = js.native
  val opacity: styleFn = js.native
  val order: styleFn = js.native
  val overflow: styleFn = js.native
  val padding: styleFn = js.native
  val paddingBottom: styleFn = js.native
  val paddingLeft: styleFn = js.native
  val paddingRight: styleFn = js.native
  val paddingTop: styleFn = js.native
  val position: styleFn = js.native
  val right: styleFn = js.native
  val shadow: styleFn = js.native
  val size: styleFn = js.native
  val space: styleFn = js.native
  val styles: StylesProps = js.native
  val textAlign: styleFn = js.native
  val textColor: styleFn = js.native
  val textShadow: styleFn = js.native
  val textStyle: styleFn = js.native
  val top: styleFn = js.native
  val typography: styleFn = js.native
  val verticalAlign: styleFn = js.native
  val width: styleFn = js.native
  val zIndex: styleFn = js.native
  def compose(parsers: styleFn*): styleFn = js.native
  def createParser(config: ConfigStyle): styleFn = js.native
  def createStyleFunction(args: ConfigStyle): styleFn = js.native
  def get(obj: js.Any, paths: (String | Double)*): js.Any = js.native
  def getPx(n: js.Any, scale: js.Any): String = js.native
  def style[N, S](// tslint:disable-next-line no-unnecessary-generics
  args: LowLevelStyleFunctionArguments[N, S]): StringDictionary[String] = js.native
  def system(styleDefinitions: Config): styleFn = js.native
  def variant(props: VariantArgs): js.Function1[/* repeated */ js.Any, _] = js.native
}

