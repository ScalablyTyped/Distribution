package typings.std

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.std.stdStrings.include
  - typings.std.stdStrings.omit
  - typings.std.stdStrings.`same-origin`
*/
trait RequestCredentials extends StObject
object RequestCredentials {
  
  inline def include: typings.std.stdStrings.include = "include".asInstanceOf[typings.std.stdStrings.include]
  
  inline def omit: typings.std.stdStrings.omit = "omit".asInstanceOf[typings.std.stdStrings.omit]
  
  inline def `same-origin`: typings.std.stdStrings.`same-origin` = "same-origin".asInstanceOf[typings.std.stdStrings.`same-origin`]
}
