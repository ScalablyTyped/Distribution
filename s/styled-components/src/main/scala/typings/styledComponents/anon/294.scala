...
@js.native
trait `294`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[ellipse, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[ellipse, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
