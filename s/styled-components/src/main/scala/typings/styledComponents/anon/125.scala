...
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feDropShadow, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feDropShadow, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
