...
@js.native
trait `617`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[script, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[script, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
