...
@js.native
trait `319`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feTurbulence, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feTurbulence, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
