...
@js.native
trait `507`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[linearGradient, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[linearGradient, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
