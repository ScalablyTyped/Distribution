...
@js.native
trait `480`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feComposite, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feComposite, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
