...
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feMergeNode, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feMergeNode, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
