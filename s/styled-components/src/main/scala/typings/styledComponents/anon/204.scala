...
@js.native
trait `204`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[figcaption, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[figcaption, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
