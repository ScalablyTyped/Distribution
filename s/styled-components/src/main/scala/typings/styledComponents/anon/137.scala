...
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[fePointLight, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[fePointLight, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
