...
@js.native
trait `656`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feColorMatrix, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feColorMatrix, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
