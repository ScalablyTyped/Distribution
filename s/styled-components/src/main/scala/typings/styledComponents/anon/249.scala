...
@js.native
trait `249`[OuterProps, Environment /* <: web | native */, // used for styled<{}>().attrs() so attrs() gets the generic prop context
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[progress, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[progress, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
