...
OuterStatics] extends StObject {
  ...
  def apply[Props, Statics](initialStyles: Styles[OuterProps & Props], interpolations: (Interpolation[OuterProps & Props])*): ReturnType[
    (IStyledNativeComponentFactory[feMorphology, OuterProps & Props, OuterStatics & Statics]) | (IStyledComponentFactory[feMorphology, OuterProps & Props, OuterStatics & Statics])
  ] = js.native
