package typings.storybookComponents

import typings.std.Element
import typings.std.HTMLElement
import typings.std.Partial
import typings.storybookComponents.anon.PartialModifieranyany
import typings.storybookComponents.anon.PartialOptions
import typings.storybookComponents.popperjsCoreLibCreatePopperMod.PopperGeneratorArgs
import typings.storybookComponents.popperjsCoreLibEnumsMod.BasePlacement
import typings.storybookComponents.popperjsCoreLibEnumsMod.ModifierPhases_
import typings.storybookComponents.popperjsCoreLibEnumsMod.Placement
import typings.storybookComponents.popperjsCoreLibEnumsMod.VariationPlacement
import typings.storybookComponents.popperjsCoreLibModifiersApplyStylesMod.ApplyStylesModifier
import typings.storybookComponents.popperjsCoreLibModifiersArrowMod.ArrowModifier
import typings.storybookComponents.popperjsCoreLibModifiersComputeStylesMod.ComputeStylesModifier
import typings.storybookComponents.popperjsCoreLibModifiersEventListenersMod.EventListenersModifier
import typings.storybookComponents.popperjsCoreLibModifiersFlipMod.FlipModifier
import typings.storybookComponents.popperjsCoreLibModifiersHideMod.HideModifier
import typings.storybookComponents.popperjsCoreLibModifiersOffsetMod.OffsetModifier
import typings.storybookComponents.popperjsCoreLibModifiersPopperOffsetsMod.PopperOffsetsModifier
import typings.storybookComponents.popperjsCoreLibModifiersPreventOverflowMod.PreventOverflowModifier
import typings.storybookComponents.popperjsCoreLibTypesMod.Instance
import typings.storybookComponents.popperjsCoreLibTypesMod.OptionsGeneric
import typings.storybookComponents.popperjsCoreLibTypesMod.SideObject
import typings.storybookComponents.popperjsCoreLibTypesMod.State
import typings.storybookComponents.popperjsCoreLibTypesMod.VirtualElement
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object popperjsCoreLibIndexMod {
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", JSImport.Namespace)
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "afterMain")
  @js.native
  val afterMain: typings.storybookComponents.storybookComponentsStrings.afterMain = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "afterRead")
  @js.native
  val afterRead: typings.storybookComponents.storybookComponentsStrings.afterRead = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "afterWrite")
  @js.native
  val afterWrite: typings.storybookComponents.storybookComponentsStrings.afterWrite = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "applyStyles")
  @js.native
  val applyStyles: ApplyStylesModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "arrow")
  @js.native
  val arrow: ArrowModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "auto")
  @js.native
  val auto: typings.storybookComponents.storybookComponentsStrings.auto = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "basePlacements")
  @js.native
  val basePlacements: js.Array[BasePlacement] = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "beforeMain")
  @js.native
  val beforeMain: typings.storybookComponents.storybookComponentsStrings.beforeMain = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "beforeRead")
  @js.native
  val beforeRead: typings.storybookComponents.storybookComponentsStrings.beforeRead = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "beforeWrite")
  @js.native
  val beforeWrite: typings.storybookComponents.storybookComponentsStrings.beforeWrite = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "bottom")
  @js.native
  val bottom: typings.storybookComponents.storybookComponentsStrings.bottom = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "clippingParents")
  @js.native
  val clippingParents: typings.storybookComponents.storybookComponentsStrings.clippingParents = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "computeStyles")
  @js.native
  val computeStyles: ComputeStylesModifier = js.native
  
  inline def createPopper[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopper")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopper[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopper")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopper[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopper")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopper[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopper")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  
  inline def createPopperBase[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperBase")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperBase[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperBase")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperBase[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperBase")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperBase[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperBase")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  
  inline def createPopperLite[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperLite")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperLite[TModifier /* <: PartialModifieranyany */](reference: Element, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperLite")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperLite[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperLite")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any])).asInstanceOf[Instance]
  inline def createPopperLite[TModifier /* <: PartialModifieranyany */](reference: VirtualElement, popper: HTMLElement, options: Partial[OptionsGeneric[TModifier]]): Instance = (^.asInstanceOf[js.Dynamic].applyDynamic("createPopperLite")(reference.asInstanceOf[js.Any], popper.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[Instance]
  
  inline def detectOverflow(state: State): SideObject = ^.asInstanceOf[js.Dynamic].applyDynamic("detectOverflow")(state.asInstanceOf[js.Any]).asInstanceOf[SideObject]
  inline def detectOverflow(state: State, options: PartialOptions): SideObject = (^.asInstanceOf[js.Dynamic].applyDynamic("detectOverflow")(state.asInstanceOf[js.Any], options.asInstanceOf[js.Any])).asInstanceOf[SideObject]
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "end")
  @js.native
  val end: typings.storybookComponents.storybookComponentsStrings.end = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "eventListeners")
  @js.native
  val eventListeners: EventListenersModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "flip")
  @js.native
  val flip: FlipModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "hide")
  @js.native
  val hide: HideModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "left")
  @js.native
  val left: typings.storybookComponents.storybookComponentsStrings.left = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "main")
  @js.native
  val main: typings.storybookComponents.storybookComponentsStrings.main = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "modifierPhases")
  @js.native
  val modifierPhases: js.Array[ModifierPhases_] = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "offset")
  @js.native
  val offset: OffsetModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "placements")
  @js.native
  val placements: js.Array[Placement] = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "popper")
  @js.native
  val popper: typings.storybookComponents.storybookComponentsStrings.popper = js.native
  
  inline def popperGenerator(): js.Function3[
    /* reference */ Element | VirtualElement, 
    /* popper */ HTMLElement, 
    /* options */ js.UndefOr[Partial[OptionsGeneric[PartialModifieranyany]]], 
    Instance
  ] = ^.asInstanceOf[js.Dynamic].applyDynamic("popperGenerator")().asInstanceOf[js.Function3[
    /* reference */ Element | VirtualElement, 
    /* popper */ HTMLElement, 
    /* options */ js.UndefOr[Partial[OptionsGeneric[PartialModifieranyany]]], 
    Instance
  ]]
  inline def popperGenerator(generatorOptions: PopperGeneratorArgs): js.Function3[
    /* reference */ Element | VirtualElement, 
    /* popper */ HTMLElement, 
    /* options */ js.UndefOr[Partial[OptionsGeneric[PartialModifieranyany]]], 
    Instance
  ] = ^.asInstanceOf[js.Dynamic].applyDynamic("popperGenerator")(generatorOptions.asInstanceOf[js.Any]).asInstanceOf[js.Function3[
    /* reference */ Element | VirtualElement, 
    /* popper */ HTMLElement, 
    /* options */ js.UndefOr[Partial[OptionsGeneric[PartialModifieranyany]]], 
    Instance
  ]]
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "popperOffsets")
  @js.native
  val popperOffsets: PopperOffsetsModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "preventOverflow")
  @js.native
  val preventOverflow: PreventOverflowModifier = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "read")
  @js.native
  val read: typings.storybookComponents.storybookComponentsStrings.read = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "reference")
  @js.native
  val reference: typings.storybookComponents.storybookComponentsStrings.reference = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "right")
  @js.native
  val right: typings.storybookComponents.storybookComponentsStrings.right = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "start")
  @js.native
  val start: typings.storybookComponents.storybookComponentsStrings.start = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "top")
  @js.native
  val top: typings.storybookComponents.storybookComponentsStrings.top = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "variationPlacements")
  @js.native
  val variationPlacements: js.Array[VariationPlacement] = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "viewport")
  @js.native
  val viewport: typings.storybookComponents.storybookComponentsStrings.viewport = js.native
  
  @JSImport("@storybook/components/dist/ts3.9/_modules/@popperjs-core-lib-index", "write")
  @js.native
  val write: typings.storybookComponents.storybookComponentsStrings.write = js.native
}
