package typings.atStardustDashUiReactDashComponentDashEventDashListener

import typings.atStardustDashUiReactDashComponentDashEventDashListener.distEsTypesMod.EventListenerOptions
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@stardust-ui/react-component-event-listener/dist/es/useEventListener", JSImport.Namespace)
@js.native
object distEsUseEventListenerMod extends js.Object {
  def default[T /* <: /* import warning: LimitUnionLength.enterTypeRef Was union type with length 91 */ js.Any */](options: EventListenerOptions[T]): Unit = js.native
}

