...
trait AddEventListener extends StObject {
  ...
  def getSlate(): typings.slate.distInterfacesEditorMod.Editor
}
object AddEventListener {
  
  inline def apply(
    addEventListener: EditorChangeHandler => js.Function0[Unit],
    getSlate: () => typings.slate.distInterfacesEditorMod.Editor
  ): AddEventListener = {
  ...
  extension [Self <: AddEventListener](x: Self) {
    ...
    inline def setGetSlate(value: () => typings.slate.distInterfacesEditorMod.Editor): Self = StObject.set(x, "getSlate", js.Any.fromFunction0(value))
  }
}
