package typings.stringReplaceLoader.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait ReplaceEntries extends Options {
  var multiple: js.Array[ReplaceEntry] = js.native
}

object ReplaceEntries {
  @scala.inline
  def apply(multiple: js.Array[ReplaceEntry]): ReplaceEntries = {
    val __obj = js.Dynamic.literal(multiple = multiple.asInstanceOf[js.Any])
    __obj.asInstanceOf[ReplaceEntries]
  }
  @scala.inline
  implicit class ReplaceEntriesOps[Self <: ReplaceEntries] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def set(key: String, value: js.Any): Self = {
        x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
        x
    }
    @scala.inline
    def setMultipleVarargs(value: ReplaceEntry*): Self = this.set("multiple", js.Array(value :_*))
    @scala.inline
    def setMultiple(value: js.Array[ReplaceEntry]): Self = this.set("multiple", value.asInstanceOf[js.Any])
  }
  
}

