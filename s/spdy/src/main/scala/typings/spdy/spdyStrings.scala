package typings.spdy

import typings.spdy.mod.server.Protocol
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object spdyStrings {
  
  @js.native
  sealed trait h2
    extends StObject
       with Protocol
  inline def h2: h2 = "h2".asInstanceOf[h2]
  
  @js.native
  sealed trait httpSlash1Dot0
    extends StObject
       with Protocol
  inline def httpSlash1Dot0: httpSlash1Dot0 = "http/1.0".asInstanceOf[httpSlash1Dot0]
  
  @js.native
  sealed trait httpSlash1Dot1
    extends StObject
       with Protocol
  inline def httpSlash1Dot1: httpSlash1Dot1 = "http/1.1".asInstanceOf[httpSlash1Dot1]
  
  @js.native
  sealed trait spdySlash2
    extends StObject
       with Protocol
  inline def spdySlash2: spdySlash2 = "spdy/2".asInstanceOf[spdySlash2]
  
  @js.native
  sealed trait spdySlash3
    extends StObject
       with Protocol
  inline def spdySlash3: spdySlash3 = "spdy/3".asInstanceOf[spdySlash3]
  
  @js.native
  sealed trait spdySlash3Dot1
    extends StObject
       with Protocol
  inline def spdySlash3Dot1: spdySlash3Dot1 = "spdy/3.1".asInstanceOf[spdySlash3Dot1]
}
