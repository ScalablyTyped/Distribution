package typings
package semanticDashUiDashReactLib.distCommonjsElementsButtonButtonContentMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("semantic-ui-react/dist/commonjs/elements/Button/ButtonContent", JSImport.Namespace)
@js.native
object distCommonjsElementsButtonButtonContentModMembers extends js.Object {
  val default: reactLib.reactMod.ReactNs.StatelessComponent[ButtonContentProps] = js.native
}

