package typings.storybookUi.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait `111` extends StObject {
  
  var `1`: Boolean = js.native
  
  var `1-11`: Boolean = js.native
  
  var `1-12`: Boolean = js.native
  
  var `1-12-121`: Boolean = js.native
  
  var `1-12-122`: Boolean = js.native
  
  var `2`: Boolean = js.native
  
  var `2-21`: Boolean = js.native
  
  var `2-22`: Boolean = js.native
  
  var `3`: Boolean = js.native
  
  var `3-31`: Boolean = js.native
  
  var `3-32`: Boolean = js.native
  
  var `3-32-321`: Boolean = js.native
  
  var `3-32-322`: Boolean = js.native
}
object `111` {
  
  @scala.inline
  def apply(
    `1`: Boolean,
    `1-11`: Boolean,
    `1-12`: Boolean,
    `1-12-121`: Boolean,
    `1-12-122`: Boolean,
    `2`: Boolean,
    `2-21`: Boolean,
    `2-22`: Boolean,
    `3`: Boolean,
    `3-31`: Boolean,
    `3-32`: Boolean,
    `3-32-321`: Boolean,
    `3-32-322`: Boolean
  ): `111` = {
    val __obj = js.Dynamic.literal()
    __obj.updateDynamic("1")(`1`.asInstanceOf[js.Any])
    __obj.updateDynamic("1-11")(`1-11`.asInstanceOf[js.Any])
    __obj.updateDynamic("1-12")(`1-12`.asInstanceOf[js.Any])
    __obj.updateDynamic("1-12-121")(`1-12-121`.asInstanceOf[js.Any])
    __obj.updateDynamic("1-12-122")(`1-12-122`.asInstanceOf[js.Any])
    __obj.updateDynamic("2")(`2`.asInstanceOf[js.Any])
    __obj.updateDynamic("2-21")(`2-21`.asInstanceOf[js.Any])
    __obj.updateDynamic("2-22")(`2-22`.asInstanceOf[js.Any])
    __obj.updateDynamic("3")(`3`.asInstanceOf[js.Any])
    __obj.updateDynamic("3-31")(`3-31`.asInstanceOf[js.Any])
    __obj.updateDynamic("3-32")(`3-32`.asInstanceOf[js.Any])
    __obj.updateDynamic("3-32-321")(`3-32-321`.asInstanceOf[js.Any])
    __obj.updateDynamic("3-32-322")(`3-32-322`.asInstanceOf[js.Any])
    __obj.asInstanceOf[`111`]
  }
  
  @scala.inline
  implicit class `111MutableBuilder`[Self <: `111`] (val x: Self) extends AnyVal {
    
    @scala.inline
    def set1(value: Boolean): Self = StObject.set(x, "1", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set1-11`(value: Boolean): Self = StObject.set(x, "1-11", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set1-12`(value: Boolean): Self = StObject.set(x, "1-12", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set1-12-121`(value: Boolean): Self = StObject.set(x, "1-12-121", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set1-12-122`(value: Boolean): Self = StObject.set(x, "1-12-122", value.asInstanceOf[js.Any])
    
    @scala.inline
    def set2(value: Boolean): Self = StObject.set(x, "2", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set2-21`(value: Boolean): Self = StObject.set(x, "2-21", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set2-22`(value: Boolean): Self = StObject.set(x, "2-22", value.asInstanceOf[js.Any])
    
    @scala.inline
    def set3(value: Boolean): Self = StObject.set(x, "3", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set3-31`(value: Boolean): Self = StObject.set(x, "3-31", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set3-32`(value: Boolean): Self = StObject.set(x, "3-32", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set3-32-321`(value: Boolean): Self = StObject.set(x, "3-32-321", value.asInstanceOf[js.Any])
    
    @scala.inline
    def `set3-32-322`(value: Boolean): Self = StObject.set(x, "3-32-322", value.asInstanceOf[js.Any])
  }
}
