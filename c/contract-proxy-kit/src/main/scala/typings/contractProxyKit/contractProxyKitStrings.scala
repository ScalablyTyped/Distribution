package typings.contractProxyKit

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object contractProxyKitStrings {
  @js.native
  sealed trait `4Dot0Dot0-betaDot3` extends js.Object
  
  @js.native
  sealed trait CALL_EXCEPTION extends js.Object
  
  @js.native
  sealed trait INVALID_ARGUMENT extends js.Object
  
  @js.native
  sealed trait MISSING_ARGUMENT extends js.Object
  
  @js.native
  sealed trait MISSING_NEW extends js.Object
  
  @js.native
  sealed trait NOT_IMPLEMENTED extends js.Object
  
  @js.native
  sealed trait NUMERIC_FAULT extends js.Object
  
  @js.native
  sealed trait UNEXPECTED_ARGUMENT extends js.Object
  
  @js.native
  sealed trait UNKNOWN_ERROR extends js.Object
  
  @js.native
  sealed trait UNSUPPORTED_OPERATION extends js.Object
  
  @js.native
  sealed trait mSlash44ApostropheSlash60ApostropheSlash0ApostropheSlash0Slash0 extends js.Object
  
  @js.native
  sealed trait node extends js.Object
  
  @js.native
  sealed trait Ξ extends js.Object
  
  @scala.inline
  def `4Dot0Dot0-betaDot3`: `4Dot0Dot0-betaDot3` = "4.0.0-beta.3".asInstanceOf[`4Dot0Dot0-betaDot3`]
  @scala.inline
  def CALL_EXCEPTION: CALL_EXCEPTION = "CALL_EXCEPTION".asInstanceOf[CALL_EXCEPTION]
  @scala.inline
  def INVALID_ARGUMENT: INVALID_ARGUMENT = "INVALID_ARGUMENT".asInstanceOf[INVALID_ARGUMENT]
  @scala.inline
  def MISSING_ARGUMENT: MISSING_ARGUMENT = "MISSING_ARGUMENT".asInstanceOf[MISSING_ARGUMENT]
  @scala.inline
  def MISSING_NEW: MISSING_NEW = "MISSING_NEW".asInstanceOf[MISSING_NEW]
  @scala.inline
  def NOT_IMPLEMENTED: NOT_IMPLEMENTED = "NOT_IMPLEMENTED".asInstanceOf[NOT_IMPLEMENTED]
  @scala.inline
  def NUMERIC_FAULT: NUMERIC_FAULT = "NUMERIC_FAULT".asInstanceOf[NUMERIC_FAULT]
  @scala.inline
  def UNEXPECTED_ARGUMENT: UNEXPECTED_ARGUMENT = "UNEXPECTED_ARGUMENT".asInstanceOf[UNEXPECTED_ARGUMENT]
  @scala.inline
  def UNKNOWN_ERROR: UNKNOWN_ERROR = "UNKNOWN_ERROR".asInstanceOf[UNKNOWN_ERROR]
  @scala.inline
  def UNSUPPORTED_OPERATION: UNSUPPORTED_OPERATION = "UNSUPPORTED_OPERATION".asInstanceOf[UNSUPPORTED_OPERATION]
  @scala.inline
  def mSlash44ApostropheSlash60ApostropheSlash0ApostropheSlash0Slash0: mSlash44ApostropheSlash60ApostropheSlash0ApostropheSlash0Slash0 = "m/44'/60'/0'/0/0".asInstanceOf[mSlash44ApostropheSlash60ApostropheSlash0ApostropheSlash0Slash0]
  @scala.inline
  def node: node = "node".asInstanceOf[node]
  @scala.inline
  def Ξ: Ξ = "\u039E".asInstanceOf[Ξ]
}

