...
object htmlWriter {
  ...
  inline def apply(
    ...
    setRules: (String, StringDictionary[Any]) => Unit,
    text: String => Unit,
    write: String => Unit
  ): htmlWriter = {
  ...
  extension [Self <: htmlWriter](x: Self) {
    ...
    inline def setSetRules(value: (String, StringDictionary[Any]) => Unit): Self = StObject.set(x, "setRules", js.Any.fromFunction2(value))
  }
}
