package typings.csstype.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.csstype.csstypeStrings.capitalize
  - typings.csstype.csstypeStrings.`full-size-kana`
  - typings.csstype.csstypeStrings.`full-width`
  - typings.csstype.csstypeStrings.lowercase
  - typings.csstype.csstypeStrings.none
  - typings.csstype.csstypeStrings.uppercase
  - typings.csstype.csstypeStrings.`-moz-initial`
  - typings.csstype.csstypeStrings.inherit
  - typings.csstype.csstypeStrings.initial
  - typings.csstype.csstypeStrings.revert
  - typings.csstype.csstypeStrings.unset
*/
trait TextTransformProperty extends StObject
object TextTransformProperty {
  
  inline def `-moz-initial`: typings.csstype.csstypeStrings.`-moz-initial` = "-moz-initial".asInstanceOf[typings.csstype.csstypeStrings.`-moz-initial`]
  
  inline def capitalize: typings.csstype.csstypeStrings.capitalize = "capitalize".asInstanceOf[typings.csstype.csstypeStrings.capitalize]
  
  inline def `full-size-kana`: typings.csstype.csstypeStrings.`full-size-kana` = "full-size-kana".asInstanceOf[typings.csstype.csstypeStrings.`full-size-kana`]
  
  inline def `full-width`: typings.csstype.csstypeStrings.`full-width` = "full-width".asInstanceOf[typings.csstype.csstypeStrings.`full-width`]
  
  inline def inherit: typings.csstype.csstypeStrings.inherit = "inherit".asInstanceOf[typings.csstype.csstypeStrings.inherit]
  
  inline def initial: typings.csstype.csstypeStrings.initial = "initial".asInstanceOf[typings.csstype.csstypeStrings.initial]
  
  inline def lowercase: typings.csstype.csstypeStrings.lowercase = "lowercase".asInstanceOf[typings.csstype.csstypeStrings.lowercase]
  
  inline def none: typings.csstype.csstypeStrings.none = "none".asInstanceOf[typings.csstype.csstypeStrings.none]
  
  inline def revert: typings.csstype.csstypeStrings.revert = "revert".asInstanceOf[typings.csstype.csstypeStrings.revert]
  
  inline def unset: typings.csstype.csstypeStrings.unset = "unset".asInstanceOf[typings.csstype.csstypeStrings.unset]
  
  inline def uppercase: typings.csstype.csstypeStrings.uppercase = "uppercase".asInstanceOf[typings.csstype.csstypeStrings.uppercase]
}
