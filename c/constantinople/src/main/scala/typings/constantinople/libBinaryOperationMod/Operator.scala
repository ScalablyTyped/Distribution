package typings.constantinople.libBinaryOperationMod

import typings.constantinople.constantinopleStrings.`-`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typings.constantinople.constantinopleStrings.`+`
  - typings.constantinople.constantinopleStrings.`-`
  - typings.constantinople.constantinopleStrings.`/`
  - typings.constantinople.constantinopleStrings.`%`
  - typings.constantinople.constantinopleStrings.`*`
  - typings.constantinople.constantinopleStrings.`**`
  - typings.constantinople.constantinopleStrings.`&`
  - typings.constantinople.constantinopleStrings.`|`
  - typings.constantinople.constantinopleStrings.`>>`
  - typings.constantinople.constantinopleStrings.`>>>`
  - typings.constantinople.constantinopleStrings.`<<`
  - typings.constantinople.constantinopleStrings.`_backtick^_backtick`
  - typings.constantinople.constantinopleStrings.`==`
  - typings.constantinople.constantinopleStrings.`===`
  - typings.constantinople.constantinopleStrings.`!=`
  - typings.constantinople.constantinopleStrings.`!==`
  - typings.constantinople.constantinopleStrings.in
  - typings.constantinople.constantinopleStrings.instanceof
  - typings.constantinople.constantinopleStrings.`>`
  - typings.constantinople.constantinopleStrings.`<`
  - typings.constantinople.constantinopleStrings.`>=`
  - typings.constantinople.constantinopleStrings.`<=`
*/
trait Operator extends js.Object

object Operator {
  @scala.inline
  def `!=`: typings.constantinople.constantinopleStrings.`!=` = this.cast("!=")
  @scala.inline
  def `!==`: typings.constantinople.constantinopleStrings.`!==` = this.cast("!==")
  @scala.inline
  def `%`: typings.constantinople.constantinopleStrings.`%` = this.cast("%")
  @scala.inline
  def `&`: typings.constantinople.constantinopleStrings.`&` = this.cast("&")
  @scala.inline
  def `*`: typings.constantinople.constantinopleStrings.`*` = this.cast("*")
  @scala.inline
  def `**`: typings.constantinople.constantinopleStrings.`**` = this.cast("**")
  @scala.inline
  def `+`: typings.constantinople.constantinopleStrings.`+` = this.cast("+")
  @JSName("-")
  @scala.inline
  def `-_`: `-` = this.cast("-")
  @scala.inline
  def `/`: typings.constantinople.constantinopleStrings.`/` = this.cast("/")
  @scala.inline
  def `<`: typings.constantinople.constantinopleStrings.`<` = this.cast("<")
  @scala.inline
  def `<<`: typings.constantinople.constantinopleStrings.`<<` = this.cast("<<")
  @scala.inline
  def `<=`: typings.constantinople.constantinopleStrings.`<=` = this.cast("<=")
  @scala.inline
  def `==`: typings.constantinople.constantinopleStrings.`==` = this.cast("==")
  @scala.inline
  def `===`: typings.constantinople.constantinopleStrings.`===` = this.cast("===")
  @scala.inline
  def `>`: typings.constantinople.constantinopleStrings.`>` = this.cast(">")
  @scala.inline
  def `>=`: typings.constantinople.constantinopleStrings.`>=` = this.cast(">=")
  @scala.inline
  def `>>`: typings.constantinople.constantinopleStrings.`>>` = this.cast(">>")
  @scala.inline
  def `>>>`: typings.constantinople.constantinopleStrings.`>>>` = this.cast(">>>")
  @scala.inline
  def `_backtick^_backtick`: typings.constantinople.constantinopleStrings.`_backtick^_backtick` = this.cast("^")
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
  @scala.inline
  def in: typings.constantinople.constantinopleStrings.in = this.cast("in")
  @scala.inline
  def instanceof: typings.constantinople.constantinopleStrings.instanceof = this.cast("instanceof")
  @scala.inline
  def `|`: typings.constantinople.constantinopleStrings.`|` = this.cast("|")
}

