...
trait AnonDataEventHandle[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
    ...
    */
  def handle(
    event: (TriggeredEvent[TTarget, TData, _, _]) with (AnonHandleObjHandleObject[TTarget, TData]),
    data: TData*
  ): Unit
}

object AnonDataEventHandle {
  @scala.inline
  ...
  ): AnonDataEventHandle[TTarget, TData] = {
    val __obj = js.Dynamic.literal(handle = js.Any.fromFunction2(handle))
  
    __obj.asInstanceOf[AnonDataEventHandle[TTarget, TData]]
  }
}
