...
trait Anon_AddHandleObj[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
...
object Anon_AddHandleObj {
  @scala.inline
  def apply[TTarget, TData](add: (TTarget, HandleObject[TTarget, TData]) => Unit): Anon_AddHandleObj[TTarget, TData] = {
    val __obj = js.Dynamic.literal(add = js.Any.fromFunction2(add))
  
    __obj.asInstanceOf[Anon_AddHandleObj[TTarget, TData]]
  }
}
