...
trait AnonData[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
...
object AnonData {
  @scala.inline
  def apply[TTarget, TData](setup: (TData, String, EventHandler[TTarget, TData]) => Unit | `false`): AnonData[TTarget, TData] = {
    val __obj = js.Dynamic.literal(setup = js.Any.fromFunction3(setup))
  
    __obj.asInstanceOf[AnonData[TTarget, TData]]
  }
}
