package typings.cordovaDotPluginsDotDiagnostic

import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ACCESS_COARSE_LOCATION
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ACCESS_FINE_LOCATION
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ADD_VOICEMAIL
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.BODY_SENSORS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.CALL_PHONE
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.CAMERA
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.GET_ACCOUNTS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.PROCESS_OUTGOING_CALLS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CALENDAR
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CALL_LOG
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CONTACTS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_EXTERNAL_STORAGE
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_PHONE_STATE
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_SMS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_MMS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_SMS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_WAP_PUSH
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECORD_AUDIO
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.SEND_SMS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.USE_SIP
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CALENDAR
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CALL_LOG
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CONTACTS
import typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_EXTERNAL_STORAGE
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_ACCESSCOARSELOCATION extends js.Object {
  var ACCESS_COARSE_LOCATION: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ACCESS_COARSE_LOCATION
  var ACCESS_FINE_LOCATION: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ACCESS_FINE_LOCATION
  var ADD_VOICEMAIL: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.ADD_VOICEMAIL
  var BODY_SENSORS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.BODY_SENSORS
  var CALL_PHONE: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.CALL_PHONE
  var CAMERA: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.CAMERA
  var GET_ACCOUNTS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.GET_ACCOUNTS
  var PROCESS_OUTGOING_CALLS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.PROCESS_OUTGOING_CALLS
  var READ_CALENDAR: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CALENDAR
  var READ_CALL_LOG: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CALL_LOG
  var READ_CONTACTS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_CONTACTS
  var READ_EXTERNAL_STORAGE: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_EXTERNAL_STORAGE
  var READ_PHONE_STATE: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_PHONE_STATE
  var READ_SMS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.READ_SMS
  var RECEIVE_MMS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_MMS
  var RECEIVE_SMS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_SMS
  var RECEIVE_WAP_PUSH: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECEIVE_WAP_PUSH
  var RECORD_AUDIO: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.RECORD_AUDIO
  var SEND_SMS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.SEND_SMS
  var USE_SIP: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.USE_SIP
  var WRITE_CALENDAR: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CALENDAR
  var WRITE_CALL_LOG: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CALL_LOG
  var WRITE_CONTACTS: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_CONTACTS
  var WRITE_EXTERNAL_STORAGE: typings.cordovaDotPluginsDotDiagnostic.cordovaDotPluginsDotDiagnosticStrings.WRITE_EXTERNAL_STORAGE
}

object Anon_ACCESSCOARSELOCATION {
  @scala.inline
  def apply(
    ACCESS_COARSE_LOCATION: ACCESS_COARSE_LOCATION,
    ACCESS_FINE_LOCATION: ACCESS_FINE_LOCATION,
    ADD_VOICEMAIL: ADD_VOICEMAIL,
    BODY_SENSORS: BODY_SENSORS,
    CALL_PHONE: CALL_PHONE,
    CAMERA: CAMERA,
    GET_ACCOUNTS: GET_ACCOUNTS,
    PROCESS_OUTGOING_CALLS: PROCESS_OUTGOING_CALLS,
    READ_CALENDAR: READ_CALENDAR,
    READ_CALL_LOG: READ_CALL_LOG,
    READ_CONTACTS: READ_CONTACTS,
    READ_EXTERNAL_STORAGE: READ_EXTERNAL_STORAGE,
    READ_PHONE_STATE: READ_PHONE_STATE,
    READ_SMS: READ_SMS,
    RECEIVE_MMS: RECEIVE_MMS,
    RECEIVE_SMS: RECEIVE_SMS,
    RECEIVE_WAP_PUSH: RECEIVE_WAP_PUSH,
    RECORD_AUDIO: RECORD_AUDIO,
    SEND_SMS: SEND_SMS,
    USE_SIP: USE_SIP,
    WRITE_CALENDAR: WRITE_CALENDAR,
    WRITE_CALL_LOG: WRITE_CALL_LOG,
    WRITE_CONTACTS: WRITE_CONTACTS,
    WRITE_EXTERNAL_STORAGE: WRITE_EXTERNAL_STORAGE
  ): Anon_ACCESSCOARSELOCATION = {
    val __obj = js.Dynamic.literal(ACCESS_COARSE_LOCATION = ACCESS_COARSE_LOCATION, ACCESS_FINE_LOCATION = ACCESS_FINE_LOCATION, ADD_VOICEMAIL = ADD_VOICEMAIL, BODY_SENSORS = BODY_SENSORS, CALL_PHONE = CALL_PHONE, CAMERA = CAMERA, GET_ACCOUNTS = GET_ACCOUNTS, PROCESS_OUTGOING_CALLS = PROCESS_OUTGOING_CALLS, READ_CALENDAR = READ_CALENDAR, READ_CALL_LOG = READ_CALL_LOG, READ_CONTACTS = READ_CONTACTS, READ_EXTERNAL_STORAGE = READ_EXTERNAL_STORAGE, READ_PHONE_STATE = READ_PHONE_STATE, READ_SMS = READ_SMS, RECEIVE_MMS = RECEIVE_MMS, RECEIVE_SMS = RECEIVE_SMS, RECEIVE_WAP_PUSH = RECEIVE_WAP_PUSH, RECORD_AUDIO = RECORD_AUDIO, SEND_SMS = SEND_SMS, USE_SIP = USE_SIP, WRITE_CALENDAR = WRITE_CALENDAR, WRITE_CALL_LOG = WRITE_CALL_LOG, WRITE_CONTACTS = WRITE_CONTACTS, WRITE_EXTERNAL_STORAGE = WRITE_EXTERNAL_STORAGE)
  
    __obj.asInstanceOf[Anon_ACCESSCOARSELOCATION]
  }
}

