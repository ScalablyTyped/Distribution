...
object Anon_ACCESSCOARSELOCATIONACCESSFINELOCATION {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(CALENDAR = CALENDAR, CAMERA = CAMERA, CONTACTS = CONTACTS, LOCATION = LOCATION, MICROPHONE = MICROPHONE, PHONE = PHONE, SENSORS = SENSORS, SMS = SMS, STORAGE = STORAGE)
