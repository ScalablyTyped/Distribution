package typings.codemirror.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait EditorSelectionChange extends StObject {
  
  var origin: js.UndefOr[String] = js.undefined
  
  var ranges: js.Array[Range]
  
  def update(ranges: js.Array[Range]): Unit
}
object EditorSelectionChange {
  
  inline def apply(ranges: js.Array[Range], update: js.Array[Range] => Unit): EditorSelectionChange = {
    val __obj = js.Dynamic.literal(ranges = ranges.asInstanceOf[js.Any], update = js.Any.fromFunction1(update))
    __obj.asInstanceOf[EditorSelectionChange]
  }
  
  extension [Self <: EditorSelectionChange](x: Self) {
    
    inline def setOrigin(value: String): Self = StObject.set(x, "origin", value.asInstanceOf[js.Any])
    
    inline def setOriginUndefined: Self = StObject.set(x, "origin", js.undefined)
    
    inline def setRanges(value: js.Array[Range]): Self = StObject.set(x, "ranges", value.asInstanceOf[js.Any])
    
    inline def setRangesVarargs(value: Range*): Self = StObject.set(x, "ranges", js.Array(value :_*))
    
    inline def setUpdate(value: js.Array[Range] => Unit): Self = StObject.set(x, "update", js.Any.fromFunction1(value))
  }
}
