package typings.cordovaPluginContacts

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object cordovaPluginContactsStrings {
  
  @js.native
  sealed trait addresses
    extends StObject
       with ContactFieldType
  inline def addresses: addresses = "addresses".asInstanceOf[addresses]
  
  @js.native
  sealed trait birthday
    extends StObject
       with ContactFieldType
  inline def birthday: birthday = "birthday".asInstanceOf[birthday]
  
  @js.native
  sealed trait categories
    extends StObject
       with ContactFieldType
  inline def categories: categories = "categories".asInstanceOf[categories]
  
  @js.native
  sealed trait country
    extends StObject
       with ContactFieldType
  inline def country: country = "country".asInstanceOf[country]
  
  @js.native
  sealed trait department
    extends StObject
       with ContactFieldType
  inline def department: department = "department".asInstanceOf[department]
  
  @js.native
  sealed trait displayName
    extends StObject
       with ContactFieldType
  inline def displayName: displayName = "displayName".asInstanceOf[displayName]
  
  @js.native
  sealed trait emails
    extends StObject
       with ContactFieldType
  inline def emails: emails = "emails".asInstanceOf[emails]
  
  @js.native
  sealed trait familyName
    extends StObject
       with ContactFieldType
  inline def familyName: familyName = "familyName".asInstanceOf[familyName]
  
  @js.native
  sealed trait formatted
    extends StObject
       with ContactFieldType
  inline def formatted: formatted = "formatted".asInstanceOf[formatted]
  
  @js.native
  sealed trait givenName
    extends StObject
       with ContactFieldType
  inline def givenName: givenName = "givenName".asInstanceOf[givenName]
  
  @js.native
  sealed trait honorificPrefix
    extends StObject
       with ContactFieldType
  inline def honorificPrefix: honorificPrefix = "honorificPrefix".asInstanceOf[honorificPrefix]
  
  @js.native
  sealed trait honorificSuffix
    extends StObject
       with ContactFieldType
  inline def honorificSuffix: honorificSuffix = "honorificSuffix".asInstanceOf[honorificSuffix]
  
  @js.native
  sealed trait id
    extends StObject
       with ContactFieldType
  inline def id: id = "id".asInstanceOf[id]
  
  @js.native
  sealed trait ims
    extends StObject
       with ContactFieldType
  inline def ims: ims = "ims".asInstanceOf[ims]
  
  @js.native
  sealed trait locality
    extends StObject
       with ContactFieldType
  inline def locality: locality = "locality".asInstanceOf[locality]
  
  @js.native
  sealed trait middleName
    extends StObject
       with ContactFieldType
  inline def middleName: middleName = "middleName".asInstanceOf[middleName]
  
  @js.native
  sealed trait name
    extends StObject
       with ContactFieldType
  inline def name: name = "name".asInstanceOf[name]
  
  @js.native
  sealed trait nickname
    extends StObject
       with ContactFieldType
  inline def nickname: nickname = "nickname".asInstanceOf[nickname]
  
  @js.native
  sealed trait note
    extends StObject
       with ContactFieldType
  inline def note: note = "note".asInstanceOf[note]
  
  @js.native
  sealed trait organizations
    extends StObject
       with ContactFieldType
  inline def organizations: organizations = "organizations".asInstanceOf[organizations]
  
  @js.native
  sealed trait phoneNumbers
    extends StObject
       with ContactFieldType
  inline def phoneNumbers: phoneNumbers = "phoneNumbers".asInstanceOf[phoneNumbers]
  
  @js.native
  sealed trait photos
    extends StObject
       with ContactFieldType
  inline def photos: photos = "photos".asInstanceOf[photos]
  
  @js.native
  sealed trait postalCode
    extends StObject
       with ContactFieldType
  inline def postalCode: postalCode = "postalCode".asInstanceOf[postalCode]
  
  @js.native
  sealed trait region
    extends StObject
       with ContactFieldType
  inline def region: region = "region".asInstanceOf[region]
  
  @js.native
  sealed trait streetAddress
    extends StObject
       with ContactFieldType
  inline def streetAddress: streetAddress = "streetAddress".asInstanceOf[streetAddress]
  
  @js.native
  sealed trait title
    extends StObject
       with ContactFieldType
  inline def title: title = "title".asInstanceOf[title]
  
  @js.native
  sealed trait urls
    extends StObject
       with ContactFieldType
  inline def urls: urls = "urls".asInstanceOf[urls]
}
