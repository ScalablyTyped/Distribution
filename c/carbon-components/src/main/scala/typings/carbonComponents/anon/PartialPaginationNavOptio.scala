package typings.carbonComponents.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Inlined std.Partial<carbon-components.carbon-components/components/pagination-nav/pagination-nav.PaginationNavOptions> */
trait PartialPaginationNavOptio extends StObject {
  
  var attribActive: js.UndefOr[String] = js.undefined
  
  var attribPage: js.UndefOr[String] = js.undefined
  
  var classActive: js.UndefOr[String] = js.undefined
  
  var classDisabled: js.UndefOr[String] = js.undefined
  
  var selectorInit: js.UndefOr[String] = js.undefined
  
  var selectorPageActive: js.UndefOr[String] = js.undefined
  
  var selectorPageButton: js.UndefOr[String] = js.undefined
  
  var selectorPageElement: js.UndefOr[String] = js.undefined
  
  var selectorPageNext: js.UndefOr[String] = js.undefined
  
  var selectorPagePrevious: js.UndefOr[String] = js.undefined
  
  var selectorPageSelect: js.UndefOr[String] = js.undefined
}
object PartialPaginationNavOptio {
  
  inline def apply(): PartialPaginationNavOptio = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[PartialPaginationNavOptio]
  }
  
  extension [Self <: PartialPaginationNavOptio](x: Self) {
    
    inline def setAttribActive(value: String): Self = StObject.set(x, "attribActive", value.asInstanceOf[js.Any])
    
    inline def setAttribActiveUndefined: Self = StObject.set(x, "attribActive", js.undefined)
    
    inline def setAttribPage(value: String): Self = StObject.set(x, "attribPage", value.asInstanceOf[js.Any])
    
    inline def setAttribPageUndefined: Self = StObject.set(x, "attribPage", js.undefined)
    
    inline def setClassActive(value: String): Self = StObject.set(x, "classActive", value.asInstanceOf[js.Any])
    
    inline def setClassActiveUndefined: Self = StObject.set(x, "classActive", js.undefined)
    
    inline def setClassDisabled(value: String): Self = StObject.set(x, "classDisabled", value.asInstanceOf[js.Any])
    
    inline def setClassDisabledUndefined: Self = StObject.set(x, "classDisabled", js.undefined)
    
    inline def setSelectorInit(value: String): Self = StObject.set(x, "selectorInit", value.asInstanceOf[js.Any])
    
    inline def setSelectorInitUndefined: Self = StObject.set(x, "selectorInit", js.undefined)
    
    inline def setSelectorPageActive(value: String): Self = StObject.set(x, "selectorPageActive", value.asInstanceOf[js.Any])
    
    inline def setSelectorPageActiveUndefined: Self = StObject.set(x, "selectorPageActive", js.undefined)
    
    inline def setSelectorPageButton(value: String): Self = StObject.set(x, "selectorPageButton", value.asInstanceOf[js.Any])
    
    inline def setSelectorPageButtonUndefined: Self = StObject.set(x, "selectorPageButton", js.undefined)
    
    inline def setSelectorPageElement(value: String): Self = StObject.set(x, "selectorPageElement", value.asInstanceOf[js.Any])
    
    inline def setSelectorPageElementUndefined: Self = StObject.set(x, "selectorPageElement", js.undefined)
    
    inline def setSelectorPageNext(value: String): Self = StObject.set(x, "selectorPageNext", value.asInstanceOf[js.Any])
    
    inline def setSelectorPageNextUndefined: Self = StObject.set(x, "selectorPageNext", js.undefined)
    
    inline def setSelectorPagePrevious(value: String): Self = StObject.set(x, "selectorPagePrevious", value.asInstanceOf[js.Any])
    
    inline def setSelectorPagePreviousUndefined: Self = StObject.set(x, "selectorPagePrevious", js.undefined)
    
    inline def setSelectorPageSelect(value: String): Self = StObject.set(x, "selectorPageSelect", value.asInstanceOf[js.Any])
    
    inline def setSelectorPageSelectUndefined: Self = StObject.set(x, "selectorPageSelect", js.undefined)
  }
}
