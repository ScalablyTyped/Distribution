...
trait TypeoftabGroups extends StObject {
  ...
  def move(groupId: Double, moveProperties: MoveProperties): js.Promise[TabGroup] = js.native
  def move(
    groupId: Double,
  ...
  def update(groupId: Double, updateProperties: UpdateProperties): js.Promise[TabGroup] = js.native
  def update(
    groupId: Double,
