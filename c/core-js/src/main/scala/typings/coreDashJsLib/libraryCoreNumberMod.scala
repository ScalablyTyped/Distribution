package typings
package coreDashJsLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("core-js/library/core/number", JSImport.Namespace)
@js.native
object libraryCoreNumberMod extends js.Object {
  var EPSILON: scala.Double = js.native
  var MAX_SAFE_INTEGER: scala.Double = js.native
  var MIN_SAFE_INTEGER: scala.Double = js.native
  def acosh(x: scala.Double): scala.Double = js.native
  def asinh(x: scala.Double): scala.Double = js.native
  def atanh(x: scala.Double): scala.Double = js.native
  def cbrt(x: scala.Double): scala.Double = js.native
  def clz32(x: scala.Double): scala.Double = js.native
  def cosh(x: scala.Double): scala.Double = js.native
  def expm1(x: scala.Double): scala.Double = js.native
  def fround(x: scala.Double): scala.Double = js.native
  def hypot(values: scala.Double*): scala.Double = js.native
  def imul(x: scala.Double, y: scala.Double): scala.Double = js.native
  def isFinite(number: scala.Double): scala.Boolean = js.native
  def isInteger(number: scala.Double): scala.Boolean = js.native
  def isNaN(number: scala.Double): scala.Boolean = js.native
  def isSafeInteger(number: scala.Double): scala.Boolean = js.native
  def log10(x: scala.Double): scala.Double = js.native
  def log1p(x: scala.Double): scala.Double = js.native
  def log2(x: scala.Double): scala.Double = js.native
  def parseFloat(string: java.lang.String): scala.Double = js.native
  def parseInt(string: java.lang.String): scala.Double = js.native
  def parseInt(string: java.lang.String, radix: scala.Double): scala.Double = js.native
  def random(): scala.Double = js.native
  def random(lim: scala.Double): scala.Double = js.native
  def sign(x: scala.Double): scala.Double = js.native
  def sinh(x: scala.Double): scala.Double = js.native
  def tanh(x: scala.Double): scala.Double = js.native
  def trunc(x: scala.Double): scala.Double = js.native
}

