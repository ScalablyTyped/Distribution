package typings.carbonComponentsReact

import typings.carbonComponentsReact.libComponentsTileTileMod.ClickableTileProps
import typings.carbonComponentsReact.libComponentsTileTileMod.ExpandableTileProps
import typings.carbonComponentsReact.libComponentsTileTileMod.SelectableTileProps
import typings.carbonComponentsReact.libComponentsTileTileMod.TileAboveTheFoldContentProps
import typings.carbonComponentsReact.libComponentsTileTileMod.TileBelowTheFoldContentProps
import typings.carbonComponentsReact.libComponentsTileTileMod.TileProps
import typings.react.mod.FC
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libComponentsTileMod {
  
  @JSImport("carbon-components-react/lib/components/Tile", "ClickableTile")
  @js.native
  open class ClickableTile protected ()
    extends typings.carbonComponentsReact.libComponentsTileTileMod.ClickableTile {
    def this(props: ClickableTileProps) = this()
    /**
      * @deprecated
      * @see https://legacy.reactjs.org/docs/legacy-context.html
      */
    def this(props: ClickableTileProps, context: Any) = this()
  }
  
  @JSImport("carbon-components-react/lib/components/Tile", "ExpandableTile")
  @js.native
  open class ExpandableTile protected ()
    extends typings.carbonComponentsReact.libComponentsTileTileMod.ExpandableTile {
    def this(props: ExpandableTileProps) = this()
    /**
      * @deprecated
      * @see https://legacy.reactjs.org/docs/legacy-context.html
      */
    def this(props: ExpandableTileProps, context: Any) = this()
  }
  
  @JSImport("carbon-components-react/lib/components/Tile", "SelectableTile")
  @js.native
  val SelectableTile: FC[SelectableTileProps] = js.native
  
  @JSImport("carbon-components-react/lib/components/Tile", "Tile")
  @js.native
  open class Tile protected ()
    extends typings.carbonComponentsReact.libComponentsTileTileMod.Tile {
    def this(props: TileProps) = this()
    /**
      * @deprecated
      * @see https://legacy.reactjs.org/docs/legacy-context.html
      */
    def this(props: TileProps, context: Any) = this()
  }
  
  @JSImport("carbon-components-react/lib/components/Tile", "TileAboveTheFoldContent")
  @js.native
  open class TileAboveTheFoldContent protected ()
    extends typings.carbonComponentsReact.libComponentsTileTileMod.TileAboveTheFoldContent {
    def this(props: TileAboveTheFoldContentProps) = this()
    /**
      * @deprecated
      * @see https://legacy.reactjs.org/docs/legacy-context.html
      */
    def this(props: TileAboveTheFoldContentProps, context: Any) = this()
  }
  
  @JSImport("carbon-components-react/lib/components/Tile", "TileBelowTheFoldContent")
  @js.native
  open class TileBelowTheFoldContent protected ()
    extends typings.carbonComponentsReact.libComponentsTileTileMod.TileBelowTheFoldContent {
    def this(props: TileBelowTheFoldContentProps) = this()
    /**
      * @deprecated
      * @see https://legacy.reactjs.org/docs/legacy-context.html
      */
    def this(props: TileBelowTheFoldContentProps, context: Any) = this()
  }
}
