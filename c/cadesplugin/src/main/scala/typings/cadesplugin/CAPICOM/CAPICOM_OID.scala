package typings.cadesplugin.CAPICOM

import typings.cadesplugin.cadespluginNumbers.`0`
import typings.cadesplugin.cadespluginNumbers.`100`
import typings.cadesplugin.cadespluginNumbers.`101`
import typings.cadesplugin.cadespluginNumbers.`102`
import typings.cadesplugin.cadespluginNumbers.`103`
import typings.cadesplugin.cadespluginNumbers.`104`
import typings.cadesplugin.cadespluginNumbers.`105`
import typings.cadesplugin.cadespluginNumbers.`106`
import typings.cadesplugin.cadespluginNumbers.`107`
import typings.cadesplugin.cadespluginNumbers.`108`
import typings.cadesplugin.cadespluginNumbers.`109`
import typings.cadesplugin.cadespluginNumbers.`10`
import typings.cadesplugin.cadespluginNumbers.`110`
import typings.cadesplugin.cadespluginNumbers.`111`
import typings.cadesplugin.cadespluginNumbers.`112`
import typings.cadesplugin.cadespluginNumbers.`113`
import typings.cadesplugin.cadespluginNumbers.`114`
import typings.cadesplugin.cadespluginNumbers.`115`
import typings.cadesplugin.cadespluginNumbers.`116`
import typings.cadesplugin.cadespluginNumbers.`117`
import typings.cadesplugin.cadespluginNumbers.`118`
import typings.cadesplugin.cadespluginNumbers.`119`
import typings.cadesplugin.cadespluginNumbers.`11`
import typings.cadesplugin.cadespluginNumbers.`120`
import typings.cadesplugin.cadespluginNumbers.`121`
import typings.cadesplugin.cadespluginNumbers.`122`
import typings.cadesplugin.cadespluginNumbers.`123`
import typings.cadesplugin.cadespluginNumbers.`124`
import typings.cadesplugin.cadespluginNumbers.`125`
import typings.cadesplugin.cadespluginNumbers.`12`
import typings.cadesplugin.cadespluginNumbers.`13`
import typings.cadesplugin.cadespluginNumbers.`14`
import typings.cadesplugin.cadespluginNumbers.`15`
import typings.cadesplugin.cadespluginNumbers.`16`
import typings.cadesplugin.cadespluginNumbers.`17`
import typings.cadesplugin.cadespluginNumbers.`18`
import typings.cadesplugin.cadespluginNumbers.`19`
import typings.cadesplugin.cadespluginNumbers.`1`
import typings.cadesplugin.cadespluginNumbers.`20`
import typings.cadesplugin.cadespluginNumbers.`21`
import typings.cadesplugin.cadespluginNumbers.`22`
import typings.cadesplugin.cadespluginNumbers.`23`
import typings.cadesplugin.cadespluginNumbers.`24`
import typings.cadesplugin.cadespluginNumbers.`25`
import typings.cadesplugin.cadespluginNumbers.`26`
import typings.cadesplugin.cadespluginNumbers.`2`
import typings.cadesplugin.cadespluginNumbers.`3`
import typings.cadesplugin.cadespluginNumbers.`4`
import typings.cadesplugin.cadespluginNumbers.`5`
import typings.cadesplugin.cadespluginNumbers.`6`
import typings.cadesplugin.cadespluginNumbers.`7`
import typings.cadesplugin.cadespluginNumbers.`8`
import typings.cadesplugin.cadespluginNumbers.`9`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait CAPICOM_OID extends OIDPlugin {
  
  val CAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION: `23` = js.native
  
  val CAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION: `25` = js.native
  
  val CAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION: `24` = js.native
  
  val CAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION: `26` = js.native
  
  val CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION: `19` = js.native
  
  val CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION: `1` = js.native
  
  val CAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION: `14` = js.native
  
  val CAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION: `8` = js.native
  
  val CAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION: `22` = js.native
  
  val CAPICOM_OID_CERT_POLICIES_95_EXTENSION: `3` = js.native
  
  val CAPICOM_OID_CERT_POLICIES_EXTENSION: `17` = js.native
  
  val CAPICOM_OID_CLIENT_AUTH_EKU: `101` = js.native
  
  val CAPICOM_OID_CODE_SIGNING_EKU: `102` = js.native
  
  val CAPICOM_OID_CRL_DIST_POINTS_EXTENSION: `16` = js.native
  
  val CAPICOM_OID_CTL_USAGE_SIGNING_EKU: `108` = js.native
  
  val CAPICOM_OID_DIGITAL_RIGHTS_EKU: `120` = js.native
  
  val CAPICOM_OID_EFS_RECOVERY_EKU: `112` = js.native
  
  val CAPICOM_OID_EMAIL_PROTECTION_EKU: `103` = js.native
  
  val CAPICOM_OID_EMBEDED_NT_CRYPTO_EKU: `116` = js.native
  
  val CAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU: `111` = js.native
  
  val CAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION: `21` = js.native
  
  val CAPICOM_OID_IPSEC_END_SYSTEM_EKU: `104` = js.native
  
  val CAPICOM_OID_IPSEC_TUNNEL_EKU: `105` = js.native
  
  val CAPICOM_OID_IPSEC_USER_EKU: `106` = js.native
  
  val CAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION: `13` = js.native
  
  val CAPICOM_OID_ISSUER_ALT_NAME_EXTENSION: `7` = js.native
  
  val CAPICOM_OID_KEY_ATTRIBUTES_EXTENSION: `2` = js.native
  
  val CAPICOM_OID_KEY_RECOVERY_EKU: `119` = js.native
  
  val CAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION: `4` = js.native
  
  val CAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION: `5` = js.native
  
  val CAPICOM_OID_LICENSES_EKU: `121` = js.native
  
  val CAPICOM_OID_LICENSE_SERVER_EKU: `122` = js.native
  
  val CAPICOM_OID_NAME_CONSTRAINTS_EXTENSION: `15` = js.native
  
  val CAPICOM_OID_NT5_CRYPTO_EKU: `114` = js.native
  
  val CAPICOM_OID_OEM_WHQL_CRYPTO_EKU: `115` = js.native
  
  val CAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS: `124` = js.native
  
  val CAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE: `125` = js.native
  
  val CAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION: `20` = js.native
  
  val CAPICOM_OID_POLICY_MAPPINGS_EXTENSION: `18` = js.native
  
  val CAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION: `11` = js.native
  
  val CAPICOM_OID_QUALIFIED_SUBORDINATION_EKU: `118` = js.native
  
  val CAPICOM_OID_ROOT_LIST_SIGNER_EKU: `117` = js.native
  
  val CAPICOM_OID_SERVER_AUTH_EKU: `100` = js.native
  
  val CAPICOM_OID_SERVER_GATED_CRYPTO_EKU: `110` = js.native
  
  val CAPICOM_OID_SMART_CARD_LOGON_EKU: `123` = js.native
  
  val CAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION: `12` = js.native
  
  val CAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION: `6` = js.native
  
  val CAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION: `9` = js.native
  
  val CAPICOM_OID_TIME_STAMPING_EKU: `107` = js.native
  
  val CAPICOM_OID_TIME_STAMP_SIGNING_EKU: `109` = js.native
  
  val CAPICOM_OID_WHQL_CRYPTO_EKU: `113` = js.native
}
object CAPICOM_OID {
  
  @scala.inline
  def apply(
    CAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION: `23`,
    CAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION: `25`,
    CAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION: `24`,
    CAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION: `26`,
    CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION: `19`,
    CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION: `1`,
    CAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION: `14`,
    CAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION: `8`,
    CAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION: `22`,
    CAPICOM_OID_CERT_POLICIES_95_EXTENSION: `3`,
    CAPICOM_OID_CERT_POLICIES_EXTENSION: `17`,
    CAPICOM_OID_CLIENT_AUTH_EKU: `101`,
    CAPICOM_OID_CODE_SIGNING_EKU: `102`,
    CAPICOM_OID_CRL_DIST_POINTS_EXTENSION: `16`,
    CAPICOM_OID_CTL_USAGE_SIGNING_EKU: `108`,
    CAPICOM_OID_DIGITAL_RIGHTS_EKU: `120`,
    CAPICOM_OID_EFS_RECOVERY_EKU: `112`,
    CAPICOM_OID_EMAIL_PROTECTION_EKU: `103`,
    CAPICOM_OID_EMBEDED_NT_CRYPTO_EKU: `116`,
    CAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU: `111`,
    CAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION: `21`,
    CAPICOM_OID_IPSEC_END_SYSTEM_EKU: `104`,
    CAPICOM_OID_IPSEC_TUNNEL_EKU: `105`,
    CAPICOM_OID_IPSEC_USER_EKU: `106`,
    CAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION: `13`,
    CAPICOM_OID_ISSUER_ALT_NAME_EXTENSION: `7`,
    CAPICOM_OID_KEY_ATTRIBUTES_EXTENSION: `2`,
    CAPICOM_OID_KEY_RECOVERY_EKU: `119`,
    CAPICOM_OID_KEY_USAGE_EXTENSION: `10`,
    CAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION: `4`,
    CAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION: `5`,
    CAPICOM_OID_LICENSES_EKU: `121`,
    CAPICOM_OID_LICENSE_SERVER_EKU: `122`,
    CAPICOM_OID_NAME_CONSTRAINTS_EXTENSION: `15`,
    CAPICOM_OID_NT5_CRYPTO_EKU: `114`,
    CAPICOM_OID_OEM_WHQL_CRYPTO_EKU: `115`,
    CAPICOM_OID_OTHER: `0`,
    CAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS: `124`,
    CAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE: `125`,
    CAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION: `20`,
    CAPICOM_OID_POLICY_MAPPINGS_EXTENSION: `18`,
    CAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION: `11`,
    CAPICOM_OID_QUALIFIED_SUBORDINATION_EKU: `118`,
    CAPICOM_OID_ROOT_LIST_SIGNER_EKU: `117`,
    CAPICOM_OID_SERVER_AUTH_EKU: `100`,
    CAPICOM_OID_SERVER_GATED_CRYPTO_EKU: `110`,
    CAPICOM_OID_SMART_CARD_LOGON_EKU: `123`,
    CAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION: `12`,
    CAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION: `6`,
    CAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION: `9`,
    CAPICOM_OID_TIME_STAMPING_EKU: `107`,
    CAPICOM_OID_TIME_STAMP_SIGNING_EKU: `109`,
    CAPICOM_OID_WHQL_CRYPTO_EKU: `113`
  ): CAPICOM_OID = {
    val __obj = js.Dynamic.literal(CAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION = CAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION = CAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION = CAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION = CAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION = CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION = CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION = CAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION = CAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION = CAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_CERT_POLICIES_95_EXTENSION = CAPICOM_OID_CERT_POLICIES_95_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_CERT_POLICIES_EXTENSION = CAPICOM_OID_CERT_POLICIES_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_CLIENT_AUTH_EKU = CAPICOM_OID_CLIENT_AUTH_EKU.asInstanceOf[js.Any], CAPICOM_OID_CODE_SIGNING_EKU = CAPICOM_OID_CODE_SIGNING_EKU.asInstanceOf[js.Any], CAPICOM_OID_CRL_DIST_POINTS_EXTENSION = CAPICOM_OID_CRL_DIST_POINTS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_CTL_USAGE_SIGNING_EKU = CAPICOM_OID_CTL_USAGE_SIGNING_EKU.asInstanceOf[js.Any], CAPICOM_OID_DIGITAL_RIGHTS_EKU = CAPICOM_OID_DIGITAL_RIGHTS_EKU.asInstanceOf[js.Any], CAPICOM_OID_EFS_RECOVERY_EKU = CAPICOM_OID_EFS_RECOVERY_EKU.asInstanceOf[js.Any], CAPICOM_OID_EMAIL_PROTECTION_EKU = CAPICOM_OID_EMAIL_PROTECTION_EKU.asInstanceOf[js.Any], CAPICOM_OID_EMBEDED_NT_CRYPTO_EKU = CAPICOM_OID_EMBEDED_NT_CRYPTO_EKU.asInstanceOf[js.Any], CAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU = CAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU.asInstanceOf[js.Any], CAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION = CAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_IPSEC_END_SYSTEM_EKU = CAPICOM_OID_IPSEC_END_SYSTEM_EKU.asInstanceOf[js.Any], CAPICOM_OID_IPSEC_TUNNEL_EKU = CAPICOM_OID_IPSEC_TUNNEL_EKU.asInstanceOf[js.Any], CAPICOM_OID_IPSEC_USER_EKU = CAPICOM_OID_IPSEC_USER_EKU.asInstanceOf[js.Any], CAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION = CAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_ISSUER_ALT_NAME_EXTENSION = CAPICOM_OID_ISSUER_ALT_NAME_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_KEY_ATTRIBUTES_EXTENSION = CAPICOM_OID_KEY_ATTRIBUTES_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_KEY_RECOVERY_EKU = CAPICOM_OID_KEY_RECOVERY_EKU.asInstanceOf[js.Any], CAPICOM_OID_KEY_USAGE_EXTENSION = CAPICOM_OID_KEY_USAGE_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION = CAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION = CAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_LICENSES_EKU = CAPICOM_OID_LICENSES_EKU.asInstanceOf[js.Any], CAPICOM_OID_LICENSE_SERVER_EKU = CAPICOM_OID_LICENSE_SERVER_EKU.asInstanceOf[js.Any], CAPICOM_OID_NAME_CONSTRAINTS_EXTENSION = CAPICOM_OID_NAME_CONSTRAINTS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_NT5_CRYPTO_EKU = CAPICOM_OID_NT5_CRYPTO_EKU.asInstanceOf[js.Any], CAPICOM_OID_OEM_WHQL_CRYPTO_EKU = CAPICOM_OID_OEM_WHQL_CRYPTO_EKU.asInstanceOf[js.Any], CAPICOM_OID_OTHER = CAPICOM_OID_OTHER.asInstanceOf[js.Any], CAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS = CAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS.asInstanceOf[js.Any], CAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE = CAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE.asInstanceOf[js.Any], CAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION = CAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_POLICY_MAPPINGS_EXTENSION = CAPICOM_OID_POLICY_MAPPINGS_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION = CAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_QUALIFIED_SUBORDINATION_EKU = CAPICOM_OID_QUALIFIED_SUBORDINATION_EKU.asInstanceOf[js.Any], CAPICOM_OID_ROOT_LIST_SIGNER_EKU = CAPICOM_OID_ROOT_LIST_SIGNER_EKU.asInstanceOf[js.Any], CAPICOM_OID_SERVER_AUTH_EKU = CAPICOM_OID_SERVER_AUTH_EKU.asInstanceOf[js.Any], CAPICOM_OID_SERVER_GATED_CRYPTO_EKU = CAPICOM_OID_SERVER_GATED_CRYPTO_EKU.asInstanceOf[js.Any], CAPICOM_OID_SMART_CARD_LOGON_EKU = CAPICOM_OID_SMART_CARD_LOGON_EKU.asInstanceOf[js.Any], CAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION = CAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION = CAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION = CAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION.asInstanceOf[js.Any], CAPICOM_OID_TIME_STAMPING_EKU = CAPICOM_OID_TIME_STAMPING_EKU.asInstanceOf[js.Any], CAPICOM_OID_TIME_STAMP_SIGNING_EKU = CAPICOM_OID_TIME_STAMP_SIGNING_EKU.asInstanceOf[js.Any], CAPICOM_OID_WHQL_CRYPTO_EKU = CAPICOM_OID_WHQL_CRYPTO_EKU.asInstanceOf[js.Any])
    __obj.asInstanceOf[CAPICOM_OID]
  }
  
  @scala.inline
  implicit class CAPICOM_OIDMutableBuilder[Self <: CAPICOM_OID] (val x: Self) extends AnyVal {
    
    @scala.inline
    def setCAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION(value: `23`): Self = StObject.set(x, "CAPICOM_OID_APPLICATION_CERT_POLICIES_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION(value: `25`): Self = StObject.set(x, "CAPICOM_OID_APPLICATION_POLICY_CONSTRAINTS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION(value: `24`): Self = StObject.set(x, "CAPICOM_OID_APPLICATION_POLICY_MAPPINGS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION(value: `26`): Self = StObject.set(x, "CAPICOM_OID_AUTHORITY_INFO_ACCESS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION(value: `19`): Self = StObject.set(x, "CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER2_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION(value: `1`): Self = StObject.set(x, "CAPICOM_OID_AUTHORITY_KEY_IDENTIFIER_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION(value: `14`): Self = StObject.set(x, "CAPICOM_OID_BASIC_CONSTRAINTS2_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION(value: `8`): Self = StObject.set(x, "CAPICOM_OID_BASIC_CONSTRAINTS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION(value: `22`): Self = StObject.set(x, "CAPICOM_OID_CERTIFICATE_TEMPLATE_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CERT_POLICIES_95_EXTENSION(value: `3`): Self = StObject.set(x, "CAPICOM_OID_CERT_POLICIES_95_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CERT_POLICIES_EXTENSION(value: `17`): Self = StObject.set(x, "CAPICOM_OID_CERT_POLICIES_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CLIENT_AUTH_EKU(value: `101`): Self = StObject.set(x, "CAPICOM_OID_CLIENT_AUTH_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CODE_SIGNING_EKU(value: `102`): Self = StObject.set(x, "CAPICOM_OID_CODE_SIGNING_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CRL_DIST_POINTS_EXTENSION(value: `16`): Self = StObject.set(x, "CAPICOM_OID_CRL_DIST_POINTS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_CTL_USAGE_SIGNING_EKU(value: `108`): Self = StObject.set(x, "CAPICOM_OID_CTL_USAGE_SIGNING_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_DIGITAL_RIGHTS_EKU(value: `120`): Self = StObject.set(x, "CAPICOM_OID_DIGITAL_RIGHTS_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_EFS_RECOVERY_EKU(value: `112`): Self = StObject.set(x, "CAPICOM_OID_EFS_RECOVERY_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_EMAIL_PROTECTION_EKU(value: `103`): Self = StObject.set(x, "CAPICOM_OID_EMAIL_PROTECTION_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_EMBEDED_NT_CRYPTO_EKU(value: `116`): Self = StObject.set(x, "CAPICOM_OID_EMBEDED_NT_CRYPTO_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU(value: `111`): Self = StObject.set(x, "CAPICOM_OID_ENCRYPTING_FILE_SYSTEM_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION(value: `21`): Self = StObject.set(x, "CAPICOM_OID_ENHANCED_KEY_USAGE_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_IPSEC_END_SYSTEM_EKU(value: `104`): Self = StObject.set(x, "CAPICOM_OID_IPSEC_END_SYSTEM_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_IPSEC_TUNNEL_EKU(value: `105`): Self = StObject.set(x, "CAPICOM_OID_IPSEC_TUNNEL_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_IPSEC_USER_EKU(value: `106`): Self = StObject.set(x, "CAPICOM_OID_IPSEC_USER_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION(value: `13`): Self = StObject.set(x, "CAPICOM_OID_ISSUER_ALT_NAME2_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_ISSUER_ALT_NAME_EXTENSION(value: `7`): Self = StObject.set(x, "CAPICOM_OID_ISSUER_ALT_NAME_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_KEY_ATTRIBUTES_EXTENSION(value: `2`): Self = StObject.set(x, "CAPICOM_OID_KEY_ATTRIBUTES_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_KEY_RECOVERY_EKU(value: `119`): Self = StObject.set(x, "CAPICOM_OID_KEY_RECOVERY_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION(value: `4`): Self = StObject.set(x, "CAPICOM_OID_KEY_USAGE_RESTRICTION_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION(value: `5`): Self = StObject.set(x, "CAPICOM_OID_LEGACY_POLICY_MAPPINGS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_LICENSES_EKU(value: `121`): Self = StObject.set(x, "CAPICOM_OID_LICENSES_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_LICENSE_SERVER_EKU(value: `122`): Self = StObject.set(x, "CAPICOM_OID_LICENSE_SERVER_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_NAME_CONSTRAINTS_EXTENSION(value: `15`): Self = StObject.set(x, "CAPICOM_OID_NAME_CONSTRAINTS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_NT5_CRYPTO_EKU(value: `114`): Self = StObject.set(x, "CAPICOM_OID_NT5_CRYPTO_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_OEM_WHQL_CRYPTO_EKU(value: `115`): Self = StObject.set(x, "CAPICOM_OID_OEM_WHQL_CRYPTO_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS(value: `124`): Self = StObject.set(x, "CAPICOM_OID_PKIX_POLICY_QUALIFIER_CPS", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE(value: `125`): Self = StObject.set(x, "CAPICOM_OID_PKIX_POLICY_QUALIFIER_USERNOTICE", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION(value: `20`): Self = StObject.set(x, "CAPICOM_OID_POLICY_CONSTRAINTS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_POLICY_MAPPINGS_EXTENSION(value: `18`): Self = StObject.set(x, "CAPICOM_OID_POLICY_MAPPINGS_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION(value: `11`): Self = StObject.set(x, "CAPICOM_OID_PRIVATEKEY_USAGE_PERIOD_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_QUALIFIED_SUBORDINATION_EKU(value: `118`): Self = StObject.set(x, "CAPICOM_OID_QUALIFIED_SUBORDINATION_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_ROOT_LIST_SIGNER_EKU(value: `117`): Self = StObject.set(x, "CAPICOM_OID_ROOT_LIST_SIGNER_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SERVER_AUTH_EKU(value: `100`): Self = StObject.set(x, "CAPICOM_OID_SERVER_AUTH_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SERVER_GATED_CRYPTO_EKU(value: `110`): Self = StObject.set(x, "CAPICOM_OID_SERVER_GATED_CRYPTO_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SMART_CARD_LOGON_EKU(value: `123`): Self = StObject.set(x, "CAPICOM_OID_SMART_CARD_LOGON_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION(value: `12`): Self = StObject.set(x, "CAPICOM_OID_SUBJECT_ALT_NAME2_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION(value: `6`): Self = StObject.set(x, "CAPICOM_OID_SUBJECT_ALT_NAME_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION(value: `9`): Self = StObject.set(x, "CAPICOM_OID_SUBJECT_KEY_IDENTIFIER_EXTENSION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_TIME_STAMPING_EKU(value: `107`): Self = StObject.set(x, "CAPICOM_OID_TIME_STAMPING_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_TIME_STAMP_SIGNING_EKU(value: `109`): Self = StObject.set(x, "CAPICOM_OID_TIME_STAMP_SIGNING_EKU", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_OID_WHQL_CRYPTO_EKU(value: `113`): Self = StObject.set(x, "CAPICOM_OID_WHQL_CRYPTO_EKU", value.asInstanceOf[js.Any])
  }
}
