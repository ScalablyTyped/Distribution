package typings.cadesplugin.CAPICOM

import typings.cadesplugin.cadespluginNumbers.`0`
import typings.cadesplugin.cadespluginNumbers.`10`
import typings.cadesplugin.cadespluginNumbers.`11`
import typings.cadesplugin.cadespluginNumbers.`12`
import typings.cadesplugin.cadespluginNumbers.`13`
import typings.cadesplugin.cadespluginNumbers.`14`
import typings.cadesplugin.cadespluginNumbers.`15`
import typings.cadesplugin.cadespluginNumbers.`16`
import typings.cadesplugin.cadespluginNumbers.`17`
import typings.cadesplugin.cadespluginNumbers.`18`
import typings.cadesplugin.cadespluginNumbers.`19`
import typings.cadesplugin.cadespluginNumbers.`1`
import typings.cadesplugin.cadespluginNumbers.`20`
import typings.cadesplugin.cadespluginNumbers.`21`
import typings.cadesplugin.cadespluginNumbers.`22`
import typings.cadesplugin.cadespluginNumbers.`23`
import typings.cadesplugin.cadespluginNumbers.`24`
import typings.cadesplugin.cadespluginNumbers.`25`
import typings.cadesplugin.cadespluginNumbers.`26`
import typings.cadesplugin.cadespluginNumbers.`27`
import typings.cadesplugin.cadespluginNumbers.`28`
import typings.cadesplugin.cadespluginNumbers.`29`
import typings.cadesplugin.cadespluginNumbers.`2`
import typings.cadesplugin.cadespluginNumbers.`30`
import typings.cadesplugin.cadespluginNumbers.`32767`
import typings.cadesplugin.cadespluginNumbers.`32768`
import typings.cadesplugin.cadespluginNumbers.`3`
import typings.cadesplugin.cadespluginNumbers.`4`
import typings.cadesplugin.cadespluginNumbers.`5`
import typings.cadesplugin.cadespluginNumbers.`64`
import typings.cadesplugin.cadespluginNumbers.`65535`
import typings.cadesplugin.cadespluginNumbers.`65`
import typings.cadesplugin.cadespluginNumbers.`66`
import typings.cadesplugin.cadespluginNumbers.`6`
import typings.cadesplugin.cadespluginNumbers.`7`
import typings.cadesplugin.cadespluginNumbers.`8`
import typings.cadesplugin.cadespluginNumbers.`9`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait CAPICOM_PROPID extends PropIDPlugin {
  
  val CAPICOM_PROPID_ACCESS_STATE: `14` = js.native
  
  val CAPICOM_PROPID_ARCHIVED: `19` = js.native
  
  val CAPICOM_PROPID_ARCHIVED_KEY_HASH: `65` = js.native
  
  val CAPICOM_PROPID_AUTO_ENROLL: `21` = js.native
  
  val CAPICOM_PROPID_CROSS_CERT_DIST_POINTS: `23` = js.native
  
  val CAPICOM_PROPID_CTL_USAGE: `9` = js.native
  
  val CAPICOM_PROPID_DATE_STAMP: `27` = js.native
  
  val CAPICOM_PROPID_DESCRIPTION: `13` = js.native
  
  val CAPICOM_PROPID_EFS: `17` = js.native
  
  val CAPICOM_PROPID_ENROLLMENT: `26` = js.native
  
  val CAPICOM_PROPID_EXTENDED_ERROR_INFO: `30` = js.native
  
  val CAPICOM_PROPID_FIRST_RESERVED: `66` = js.native
  
  val CAPICOM_PROPID_FIRST_USER: `32768` = js.native
  
  val CAPICOM_PROPID_FORTEZZA_DATA: `18` = js.native
  
  val CAPICOM_PROPID_FRIENDLY_NAME: `11` = js.native
  
  val CAPICOM_PROPID_HASH_PROP: `3` = js.native
  
  val CAPICOM_PROPID_IE30_RESERVED: `7` = js.native
  
  val CAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH: `24` = js.native
  
  val CAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH: `28` = js.native
  
  val CAPICOM_PROPID_KEY_CONTEXT: `5` = js.native
  
  val CAPICOM_PROPID_KEY_IDENTIFIER: `20` = js.native
  
  val CAPICOM_PROPID_KEY_PROV_HANDLE: `1` = js.native
  
  val CAPICOM_PROPID_KEY_PROV_INFO: `2` = js.native
  
  val CAPICOM_PROPID_KEY_SPEC: `6` = js.native
  
  val CAPICOM_PROPID_LAST_RESERVED: `32767` = js.native
  
  val CAPICOM_PROPID_LAST_USER: `65535` = js.native
  
  val CAPICOM_PROPID_MD5_HASH: `4` = js.native
  
  val CAPICOM_PROPID_NEXT_UPDATE_LOCATION: `10` = js.native
  
  val CAPICOM_PROPID_PUBKEY_ALG_PARA: `22` = js.native
  
  val CAPICOM_PROPID_PUBKEY_HASH_RESERVED: `8` = js.native
  
  val CAPICOM_PROPID_PVK_FILE: `12` = js.native
  
  val CAPICOM_PROPID_RENEWAL: `64` = js.native
  
  val CAPICOM_PROPID_SHA1_HASH: `3` = js.native
  
  val CAPICOM_PROPID_SIGNATURE_HASH: `15` = js.native
  
  val CAPICOM_PROPID_SMART_CARD_DATA: `16` = js.native
  
  val CAPICOM_PROPID_SUBJECT_NAME_MD5_HASH: `29` = js.native
  
  val CAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH: `25` = js.native
  
  val CAPICOM_PROPID_UNKNOWN: `0` = js.native
}
object CAPICOM_PROPID {
  
  @scala.inline
  def apply(
    CAPICOM_PROPID_ACCESS_STATE: `14`,
    CAPICOM_PROPID_ARCHIVED: `19`,
    CAPICOM_PROPID_ARCHIVED_KEY_HASH: `65`,
    CAPICOM_PROPID_AUTO_ENROLL: `21`,
    CAPICOM_PROPID_CROSS_CERT_DIST_POINTS: `23`,
    CAPICOM_PROPID_CTL_USAGE: `9`,
    CAPICOM_PROPID_DATE_STAMP: `27`,
    CAPICOM_PROPID_DESCRIPTION: `13`,
    CAPICOM_PROPID_EFS: `17`,
    CAPICOM_PROPID_ENHKEY_USAGE: `9`,
    CAPICOM_PROPID_ENROLLMENT: `26`,
    CAPICOM_PROPID_EXTENDED_ERROR_INFO: `30`,
    CAPICOM_PROPID_FIRST_RESERVED: `66`,
    CAPICOM_PROPID_FIRST_USER: `32768`,
    CAPICOM_PROPID_FORTEZZA_DATA: `18`,
    CAPICOM_PROPID_FRIENDLY_NAME: `11`,
    CAPICOM_PROPID_HASH_PROP: `3`,
    CAPICOM_PROPID_IE30_RESERVED: `7`,
    CAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH: `24`,
    CAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH: `28`,
    CAPICOM_PROPID_KEY_CONTEXT: `5`,
    CAPICOM_PROPID_KEY_IDENTIFIER: `20`,
    CAPICOM_PROPID_KEY_PROV_HANDLE: `1`,
    CAPICOM_PROPID_KEY_PROV_INFO: `2`,
    CAPICOM_PROPID_KEY_SPEC: `6`,
    CAPICOM_PROPID_LAST_RESERVED: `32767`,
    CAPICOM_PROPID_LAST_USER: `65535`,
    CAPICOM_PROPID_MD5_HASH: `4`,
    CAPICOM_PROPID_NEXT_UPDATE_LOCATION: `10`,
    CAPICOM_PROPID_PUBKEY_ALG_PARA: `22`,
    CAPICOM_PROPID_PUBKEY_HASH_RESERVED: `8`,
    CAPICOM_PROPID_PVK_FILE: `12`,
    CAPICOM_PROPID_RENEWAL: `64`,
    CAPICOM_PROPID_SHA1_HASH: `3`,
    CAPICOM_PROPID_SIGNATURE_HASH: `15`,
    CAPICOM_PROPID_SMART_CARD_DATA: `16`,
    CAPICOM_PROPID_SUBJECT_NAME_MD5_HASH: `29`,
    CAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH: `25`,
    CAPICOM_PROPID_UNKNOWN: `0`
  ): CAPICOM_PROPID = {
    val __obj = js.Dynamic.literal(CAPICOM_PROPID_ACCESS_STATE = CAPICOM_PROPID_ACCESS_STATE.asInstanceOf[js.Any], CAPICOM_PROPID_ARCHIVED = CAPICOM_PROPID_ARCHIVED.asInstanceOf[js.Any], CAPICOM_PROPID_ARCHIVED_KEY_HASH = CAPICOM_PROPID_ARCHIVED_KEY_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_AUTO_ENROLL = CAPICOM_PROPID_AUTO_ENROLL.asInstanceOf[js.Any], CAPICOM_PROPID_CROSS_CERT_DIST_POINTS = CAPICOM_PROPID_CROSS_CERT_DIST_POINTS.asInstanceOf[js.Any], CAPICOM_PROPID_CTL_USAGE = CAPICOM_PROPID_CTL_USAGE.asInstanceOf[js.Any], CAPICOM_PROPID_DATE_STAMP = CAPICOM_PROPID_DATE_STAMP.asInstanceOf[js.Any], CAPICOM_PROPID_DESCRIPTION = CAPICOM_PROPID_DESCRIPTION.asInstanceOf[js.Any], CAPICOM_PROPID_EFS = CAPICOM_PROPID_EFS.asInstanceOf[js.Any], CAPICOM_PROPID_ENHKEY_USAGE = CAPICOM_PROPID_ENHKEY_USAGE.asInstanceOf[js.Any], CAPICOM_PROPID_ENROLLMENT = CAPICOM_PROPID_ENROLLMENT.asInstanceOf[js.Any], CAPICOM_PROPID_EXTENDED_ERROR_INFO = CAPICOM_PROPID_EXTENDED_ERROR_INFO.asInstanceOf[js.Any], CAPICOM_PROPID_FIRST_RESERVED = CAPICOM_PROPID_FIRST_RESERVED.asInstanceOf[js.Any], CAPICOM_PROPID_FIRST_USER = CAPICOM_PROPID_FIRST_USER.asInstanceOf[js.Any], CAPICOM_PROPID_FORTEZZA_DATA = CAPICOM_PROPID_FORTEZZA_DATA.asInstanceOf[js.Any], CAPICOM_PROPID_FRIENDLY_NAME = CAPICOM_PROPID_FRIENDLY_NAME.asInstanceOf[js.Any], CAPICOM_PROPID_HASH_PROP = CAPICOM_PROPID_HASH_PROP.asInstanceOf[js.Any], CAPICOM_PROPID_IE30_RESERVED = CAPICOM_PROPID_IE30_RESERVED.asInstanceOf[js.Any], CAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH = CAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH = CAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_KEY_CONTEXT = CAPICOM_PROPID_KEY_CONTEXT.asInstanceOf[js.Any], CAPICOM_PROPID_KEY_IDENTIFIER = CAPICOM_PROPID_KEY_IDENTIFIER.asInstanceOf[js.Any], CAPICOM_PROPID_KEY_PROV_HANDLE = CAPICOM_PROPID_KEY_PROV_HANDLE.asInstanceOf[js.Any], CAPICOM_PROPID_KEY_PROV_INFO = CAPICOM_PROPID_KEY_PROV_INFO.asInstanceOf[js.Any], CAPICOM_PROPID_KEY_SPEC = CAPICOM_PROPID_KEY_SPEC.asInstanceOf[js.Any], CAPICOM_PROPID_LAST_RESERVED = CAPICOM_PROPID_LAST_RESERVED.asInstanceOf[js.Any], CAPICOM_PROPID_LAST_USER = CAPICOM_PROPID_LAST_USER.asInstanceOf[js.Any], CAPICOM_PROPID_MD5_HASH = CAPICOM_PROPID_MD5_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_NEXT_UPDATE_LOCATION = CAPICOM_PROPID_NEXT_UPDATE_LOCATION.asInstanceOf[js.Any], CAPICOM_PROPID_PUBKEY_ALG_PARA = CAPICOM_PROPID_PUBKEY_ALG_PARA.asInstanceOf[js.Any], CAPICOM_PROPID_PUBKEY_HASH_RESERVED = CAPICOM_PROPID_PUBKEY_HASH_RESERVED.asInstanceOf[js.Any], CAPICOM_PROPID_PVK_FILE = CAPICOM_PROPID_PVK_FILE.asInstanceOf[js.Any], CAPICOM_PROPID_RENEWAL = CAPICOM_PROPID_RENEWAL.asInstanceOf[js.Any], CAPICOM_PROPID_SHA1_HASH = CAPICOM_PROPID_SHA1_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_SIGNATURE_HASH = CAPICOM_PROPID_SIGNATURE_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_SMART_CARD_DATA = CAPICOM_PROPID_SMART_CARD_DATA.asInstanceOf[js.Any], CAPICOM_PROPID_SUBJECT_NAME_MD5_HASH = CAPICOM_PROPID_SUBJECT_NAME_MD5_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH = CAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH.asInstanceOf[js.Any], CAPICOM_PROPID_UNKNOWN = CAPICOM_PROPID_UNKNOWN.asInstanceOf[js.Any])
    __obj.asInstanceOf[CAPICOM_PROPID]
  }
  
  @scala.inline
  implicit class CAPICOM_PROPIDOps[Self <: CAPICOM_PROPID] (val x: Self) extends AnyVal {
    
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    
    @scala.inline
    def set(key: String, value: js.Any): Self = {
      x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
      x
    }
    
    @scala.inline
    def setCAPICOM_PROPID_ACCESS_STATE(value: `14`): Self = this.set("CAPICOM_PROPID_ACCESS_STATE", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_ARCHIVED(value: `19`): Self = this.set("CAPICOM_PROPID_ARCHIVED", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_ARCHIVED_KEY_HASH(value: `65`): Self = this.set("CAPICOM_PROPID_ARCHIVED_KEY_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_AUTO_ENROLL(value: `21`): Self = this.set("CAPICOM_PROPID_AUTO_ENROLL", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_CROSS_CERT_DIST_POINTS(value: `23`): Self = this.set("CAPICOM_PROPID_CROSS_CERT_DIST_POINTS", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_CTL_USAGE(value: `9`): Self = this.set("CAPICOM_PROPID_CTL_USAGE", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_DATE_STAMP(value: `27`): Self = this.set("CAPICOM_PROPID_DATE_STAMP", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_DESCRIPTION(value: `13`): Self = this.set("CAPICOM_PROPID_DESCRIPTION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_EFS(value: `17`): Self = this.set("CAPICOM_PROPID_EFS", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_ENROLLMENT(value: `26`): Self = this.set("CAPICOM_PROPID_ENROLLMENT", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_EXTENDED_ERROR_INFO(value: `30`): Self = this.set("CAPICOM_PROPID_EXTENDED_ERROR_INFO", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_FIRST_RESERVED(value: `66`): Self = this.set("CAPICOM_PROPID_FIRST_RESERVED", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_FIRST_USER(value: `32768`): Self = this.set("CAPICOM_PROPID_FIRST_USER", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_FORTEZZA_DATA(value: `18`): Self = this.set("CAPICOM_PROPID_FORTEZZA_DATA", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_FRIENDLY_NAME(value: `11`): Self = this.set("CAPICOM_PROPID_FRIENDLY_NAME", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_HASH_PROP(value: `3`): Self = this.set("CAPICOM_PROPID_HASH_PROP", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_IE30_RESERVED(value: `7`): Self = this.set("CAPICOM_PROPID_IE30_RESERVED", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH(value: `24`): Self = this.set("CAPICOM_PROPID_ISSUER_PUBLIC_KEY_MD5_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH(value: `28`): Self = this.set("CAPICOM_PROPID_ISSUER_SERIAL_NUMBER_MD5_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_KEY_CONTEXT(value: `5`): Self = this.set("CAPICOM_PROPID_KEY_CONTEXT", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_KEY_IDENTIFIER(value: `20`): Self = this.set("CAPICOM_PROPID_KEY_IDENTIFIER", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_KEY_PROV_HANDLE(value: `1`): Self = this.set("CAPICOM_PROPID_KEY_PROV_HANDLE", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_KEY_PROV_INFO(value: `2`): Self = this.set("CAPICOM_PROPID_KEY_PROV_INFO", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_KEY_SPEC(value: `6`): Self = this.set("CAPICOM_PROPID_KEY_SPEC", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_LAST_RESERVED(value: `32767`): Self = this.set("CAPICOM_PROPID_LAST_RESERVED", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_LAST_USER(value: `65535`): Self = this.set("CAPICOM_PROPID_LAST_USER", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_MD5_HASH(value: `4`): Self = this.set("CAPICOM_PROPID_MD5_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_NEXT_UPDATE_LOCATION(value: `10`): Self = this.set("CAPICOM_PROPID_NEXT_UPDATE_LOCATION", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_PUBKEY_ALG_PARA(value: `22`): Self = this.set("CAPICOM_PROPID_PUBKEY_ALG_PARA", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_PUBKEY_HASH_RESERVED(value: `8`): Self = this.set("CAPICOM_PROPID_PUBKEY_HASH_RESERVED", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_PVK_FILE(value: `12`): Self = this.set("CAPICOM_PROPID_PVK_FILE", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_RENEWAL(value: `64`): Self = this.set("CAPICOM_PROPID_RENEWAL", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_SHA1_HASH(value: `3`): Self = this.set("CAPICOM_PROPID_SHA1_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_SIGNATURE_HASH(value: `15`): Self = this.set("CAPICOM_PROPID_SIGNATURE_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_SMART_CARD_DATA(value: `16`): Self = this.set("CAPICOM_PROPID_SMART_CARD_DATA", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_SUBJECT_NAME_MD5_HASH(value: `29`): Self = this.set("CAPICOM_PROPID_SUBJECT_NAME_MD5_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH(value: `25`): Self = this.set("CAPICOM_PROPID_SUBJECT_PUBLIC_KEY_MD5_HASH", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCAPICOM_PROPID_UNKNOWN(value: `0`): Self = this.set("CAPICOM_PROPID_UNKNOWN", value.asInstanceOf[js.Any])
  }
}
