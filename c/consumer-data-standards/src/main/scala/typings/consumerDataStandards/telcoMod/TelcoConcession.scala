...
object TelcoConcession {
  
  inline def apply(displayName: String, startDate: String, `type`: CONCESSION | REBATE | GRANT): TelcoConcession = {
    ...
    __obj.asInstanceOf[TelcoConcession]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TelcoConcession] (val x: Self) extends AnyVal {
