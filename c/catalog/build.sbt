organization := "org.scalablytyped"
name := "catalog"
version := "3.6.0-84ba8b"
scalaVersion := "2.12.8"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "1.0.0",
  "org.scala-js" %%% "scalajs-dom" % "0.9.6",
  "org.scalablytyped" %%% "ansi-styles" % "3.2.1-dt-20181017Z-94ad9b",
  "org.scalablytyped" %%% "anymatch" % "1.3-dt-20180214Z-a09216",
  "org.scalablytyped" %%% "args" % "3.0-dt-20180214Z-2d583a",
  "org.scalablytyped" %%% "autoprefixer" % "9.1-dt-20181107Z-10d1ff",
  "org.scalablytyped" %%% "babel-core" % "6.25-dt-20180611Z-d2db0b",
  "org.scalablytyped" %%% "babel-generator" % "6.25-dt-20180611Z-c0d92f",
  "org.scalablytyped" %%% "babel-template" % "6.25-dt-20180611Z-18b9db",
  "org.scalablytyped" %%% "babel-traverse" % "6.25-dt-20180611Z-157ca0",
  "org.scalablytyped" %%% "babel-types" % "7.0-dt-20180613Z-619284",
  "org.scalablytyped" %%% "babylon" % "6.16-dt-20181029Z-8c0e04",
  "org.scalablytyped" %%% "body-parser" % "1.17-dt-20180425Z-322647",
  "org.scalablytyped" %%% "chalk" % "2.4.2-4d2e54",
  "org.scalablytyped" %%% "clean-css" % "4.2-dt-20190130Z-155766",
  "org.scalablytyped" %%% "color-name" % "1.1-dt-20180214Z-048d4d",
  "org.scalablytyped" %%% "connect" % "v3.4.0-dt-20180417Z-fba25e",
  "org.scalablytyped" %%% "create-emotion" % "10.0.7-940780",
  "org.scalablytyped" %%% "create-react-class" % "15.6-dt-20180806Z-0d6ee9",
  "org.scalablytyped" %%% "csstype" % "2.6.2-522be0",
  "org.scalablytyped" %%% "d3-color" % "1.2-dt-20180520Z-a46f6d",
  "org.scalablytyped" %%% "detect-port" % "1.1-dt-20180214Z-1d1adb",
  "org.scalablytyped" %%% "emotion__cache" % "10.0.7-db5388",
  "org.scalablytyped" %%% "emotion__serialize" % "0.11.4-b8fd8b",
  "org.scalablytyped" %%% "emotion__sheet" % "0.9.2-487614",
  "org.scalablytyped" %%% "emotion__utils" % "0.11.1-ffeb9e",
  "org.scalablytyped" %%% "escape-string-regexp" % "1.0-dt-20180721Z-964066",
  "org.scalablytyped" %%% "eslint" % "4.16-dt-20190128Z-0126f0",
  "org.scalablytyped" %%% "estree" % "0.0-unknown-dt-20180417Z-2db6a8",
  "org.scalablytyped" %%% "express" % "4.16-dt-20190122Z-aabd2b",
  "org.scalablytyped" %%% "express-serve-static-core" % "4.16-dt-20190122Z-f1d2dc",
  "org.scalablytyped" %%% "extract-text-webpack-plugin" % "3.0-dt-20180509Z-c8a72e",
  "org.scalablytyped" %%% "friendly-errors-webpack-plugin" % "0.1-dt-20180214Z-6cc51a",
  "org.scalablytyped" %%% "history" % "4.7.2-dt-20181017Z-bd3077",
  "org.scalablytyped" %%% "html-minifier" % "3.5-dt-20190124Z-0324ae",
  "org.scalablytyped" %%% "html-webpack-plugin" % "3.2-dt-20181121Z-350ce1",
  "org.scalablytyped" %%% "http-proxy" % "1.16-dt-20180514Z-05224f",
  "org.scalablytyped" %%% "http-proxy-middleware" % "0.19-dt-20190114Z-3775cd",
  "org.scalablytyped" %%% "js-base64" % "2.3-dt-20180214Z-0003cc",
  "org.scalablytyped" %%% "js-yaml" % "3.12-dt-20190118Z-9bda5b",
  "org.scalablytyped" %%% "json-schema" % "7.0-dt-20190201Z-5b2cd9",
  "org.scalablytyped" %%% "marked" % "0.6-dt-20190102Z-216b98",
  "org.scalablytyped" %%% "mime" % "2.0-dt-20190205Z-54d3f4",
  "org.scalablytyped" %%% "node" % "11.9-dt-20190211Z-6a64f2",
  "org.scalablytyped" %%% "node-fetch" % "2.1-dt-20190205Z-5b74a4",
  "org.scalablytyped" %%% "postcss" % "5.2.18-ed6fd2",
  "org.scalablytyped" %%% "prismjs" % "1.9-dt-20180214Z-b31fed",
  "org.scalablytyped" %%% "prop-types" % "15.5-dt-20181212Z-8cdf18",
  "org.scalablytyped" %%% "raf" % "3.4-dt-20190103Z-159450",
  "org.scalablytyped" %%% "ramda" % "0.25-dt-20190206Z-fc958a",
  "org.scalablytyped" %%% "range-parser" % "1.2-dt-20181016Z-c7c39d",
  "org.scalablytyped" %%% "react" % "16.8-dt-20190206Z-d66e8f",
  "org.scalablytyped" %%% "react-dev-utils" % "7.0-dt-20190128Z-ae33e1",
  "org.scalablytyped" %%% "react-document-title" % "2.0-dt-20180803Z-3fc857",
  "org.scalablytyped" %%% "react-dom" % "16.8-dt-20190206Z-7c7ba9",
  "org.scalablytyped" %%% "react-router" % "4.4-dt-20190103Z-28d5ae",
  "org.scalablytyped" %%% "relateurl" % "v0.2.6-dt-20180214Z-07f5e7",
  "org.scalablytyped" %%% "serve-static" % "1.13-dt-20180428Z-308d11",
  "org.scalablytyped" %%% "source-map" % "0.7.3-cdf339",
  "org.scalablytyped" %%% "srcset" % "1.0-dt-20190114Z-66d09e",
  "org.scalablytyped" %%% "std" % "3.3-0ec965",
  "org.scalablytyped" %%% "strip-indent" % "2.0-dt-20181217Z-77d9e5",
  "org.scalablytyped" %%% "supports-color" % "5.3-dt-20180409Z-20da60",
  "org.scalablytyped" %%% "tapable" % "v1.0.0-dt-20180713Z-2fd0f1",
  "org.scalablytyped" %%% "uglify-js" % "3.0-dt-20181015Z-876284",
  "org.scalablytyped" %%% "unfetch" % "3.1.2-31361a",
  "org.scalablytyped" %%% "webpack" % "4.4-dt-20190116Z-6291f4",
  "org.scalablytyped" %%% "webpack-dev-server" % "3.1-dt-20190129Z-5ebb6b",
  "org.scalablytyped" %%% "webpack-manifest-plugin" % "2.0-dt-20181216Z-0e4ce4")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
        