organization := "org.scalablytyped"
name := "catalog"
version := "3.6.0-9170a9"
scalaVersion := "2.12.8"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "1.0.0",
  "org.scala-js" %%% "scalajs-dom" % "0.9.6",
  "org.scalablytyped" %%% "ansi-styles" % "3.2.1-dt-20181017Z-15d04a",
  "org.scalablytyped" %%% "anymatch" % "1.3-dt-20190212Z-d73b57",
  "org.scalablytyped" %%% "args" % "3.0-dt-20180214Z-d330bc",
  "org.scalablytyped" %%% "autoprefixer" % "9.4-dt-20190227Z-09a153",
  "org.scalablytyped" %%% "babel-core" % "6.25-dt-20190212Z-5f4950",
  "org.scalablytyped" %%% "babel-generator" % "6.25-dt-20190212Z-5df629",
  "org.scalablytyped" %%% "babel-template" % "6.25-dt-20190212Z-3697ff",
  "org.scalablytyped" %%% "babel-traverse" % "6.25-dt-20190212Z-e17a99",
  "org.scalablytyped" %%% "babel-types" % "7.0-dt-20190218Z-7ad93b",
  "org.scalablytyped" %%% "babylon" % "6.16-dt-20190212Z-010229",
  "org.scalablytyped" %%% "body-parser" % "1.17-dt-20180425Z-1ca7dd",
  "org.scalablytyped" %%% "chalk" % "2.4.2-6ee70e",
  "org.scalablytyped" %%% "clean-css" % "4.2-dt-20190130Z-6d5fe0",
  "org.scalablytyped" %%% "color-name" % "1.1-dt-20190212Z-1a1d0f",
  "org.scalablytyped" %%% "connect" % "v3.4.0-dt-20180417Z-c3324d",
  "org.scalablytyped" %%% "create-emotion" % "10.0.7-fadd08",
  "org.scalablytyped" %%% "create-react-class" % "15.6-dt-20180806Z-079941",
  "org.scalablytyped" %%% "csstype" % "2.6.3-60f3ea",
  "org.scalablytyped" %%% "d3-color" % "1.2-dt-20190212Z-c24282",
  "org.scalablytyped" %%% "detect-port" % "1.1-dt-20180214Z-96123e",
  "org.scalablytyped" %%% "emotion__cache" % "10.0.7-a901f7",
  "org.scalablytyped" %%% "emotion__serialize" % "0.11.4-aa01a6",
  "org.scalablytyped" %%% "emotion__sheet" % "0.9.2-3f3db3",
  "org.scalablytyped" %%% "emotion__utils" % "0.11.1-002494",
  "org.scalablytyped" %%% "escape-string-regexp" % "1.0-dt-20180721Z-38137e",
  "org.scalablytyped" %%% "eslint" % "4.16-dt-20190128Z-a02a91",
  "org.scalablytyped" %%% "estree" % "0.0-unknown-dt-20180417Z-756eb7",
  "org.scalablytyped" %%% "express" % "4.16-dt-20190122Z-147e65",
  "org.scalablytyped" %%% "express-serve-static-core" % "4.16-dt-20190122Z-6ff32e",
  "org.scalablytyped" %%% "extract-text-webpack-plugin" % "3.0-dt-20190212Z-53fd8f",
  "org.scalablytyped" %%% "friendly-errors-webpack-plugin" % "0.1-dt-20180214Z-43f100",
  "org.scalablytyped" %%% "history" % "4.7.2-dt-20190226Z-e317ff",
  "org.scalablytyped" %%% "html-minifier" % "3.5-dt-20190212Z-74b6b8",
  "org.scalablytyped" %%% "html-webpack-plugin" % "3.2-dt-20181121Z-511c96",
  "org.scalablytyped" %%% "http-proxy" % "1.16-dt-20180514Z-1a1dc6",
  "org.scalablytyped" %%% "http-proxy-middleware" % "0.19-dt-20190114Z-f9dd2e",
  "org.scalablytyped" %%% "js-base64" % "2.3-dt-20180214Z-87d4fa",
  "org.scalablytyped" %%% "js-yaml" % "3.12-dt-20190118Z-f1d446",
  "org.scalablytyped" %%% "json-schema" % "7.0-dt-20190212Z-f1101a",
  "org.scalablytyped" %%% "marked" % "0.6-dt-20190305Z-69ec69",
  "org.scalablytyped" %%% "mime" % "2.0-dt-20190205Z-5cf9d4",
  "org.scalablytyped" %%% "node" % "11.10-dt-20190302Z-6be09d",
  "org.scalablytyped" %%% "node-fetch" % "2.1-dt-20190205Z-9a6119",
  "org.scalablytyped" %%% "postcss" % "5.2.18-9ba2a6",
  "org.scalablytyped" %%% "prismjs" % "1.9-dt-20190212Z-9302b9",
  "org.scalablytyped" %%% "prop-types" % "15.7-dt-20190226Z-0b0b5e",
  "org.scalablytyped" %%% "raf" % "3.4-dt-20190103Z-6bf0c5",
  "org.scalablytyped" %%% "ramda" % "0.25-dt-20190228Z-295efd",
  "org.scalablytyped" %%% "range-parser" % "1.2-dt-20181016Z-aeafd9",
  "org.scalablytyped" %%% "react" % "16.8-dt-20190306Z-d5c6e1",
  "org.scalablytyped" %%% "react-dev-utils" % "7.0-dt-20190128Z-cacb56",
  "org.scalablytyped" %%% "react-document-title" % "2.0-dt-20180803Z-2c3d52",
  "org.scalablytyped" %%% "react-dom" % "16.8-dt-20190213Z-580844",
  "org.scalablytyped" %%% "react-router" % "4.4-dt-20190212Z-228a6b",
  "org.scalablytyped" %%% "relateurl" % "v0.2.6-dt-20180214Z-c930b1",
  "org.scalablytyped" %%% "serve-static" % "1.13-dt-20180428Z-96e810",
  "org.scalablytyped" %%% "source-map" % "0.7.3-39089e",
  "org.scalablytyped" %%% "srcset" % "1.0-dt-20190114Z-9623fd",
  "org.scalablytyped" %%% "std" % "3.3-05be79",
  "org.scalablytyped" %%% "strip-indent" % "2.0-dt-20181217Z-bddd26",
  "org.scalablytyped" %%% "supports-color" % "5.3-dt-20180409Z-bedf31",
  "org.scalablytyped" %%% "tapable" % "v1.0.0-dt-20180713Z-59c854",
  "org.scalablytyped" %%% "uglify-js" % "3.0-dt-20181015Z-4f8c7c",
  "org.scalablytyped" %%% "unfetch" % "3.1.2-ad62b6",
  "org.scalablytyped" %%% "webpack" % "4.4-dt-20190222Z-9215f6",
  "org.scalablytyped" %%% "webpack-dev-server" % "3.1-dt-20190129Z-78e73f",
  "org.scalablytyped" %%% "webpack-manifest-plugin" % "2.0-dt-20181216Z-d47bbd")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
        