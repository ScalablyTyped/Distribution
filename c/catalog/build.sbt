organization := "com.scalablytyped"
name := "catalog"
version := "3.6.0-567362"
scalaVersion := "2.12.7"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "runtime" % "1.0.0-M1",
  "com.scalablytyped" %%% "ansi-styles" % "3.2.1-dt-20181018Z-16b038",
  "com.scalablytyped" %%% "anymatch" % "1.3-dt-20180910Z-335bc8",
  "com.scalablytyped" %%% "args" % "3.0-dt-20180910Z-a1ba16",
  "com.scalablytyped" %%% "autoprefixer" % "9.1-dt-20181109Z-7e622a",
  "com.scalablytyped" %%% "babel-core" % "6.25-dt-20180910Z-d0a22e",
  "com.scalablytyped" %%% "babel-generator" % "6.25-dt-20180910Z-3a02f7",
  "com.scalablytyped" %%% "babel-template" % "6.25-dt-20180910Z-db16a8",
  "com.scalablytyped" %%% "babel-traverse" % "6.25-dt-20180910Z-ad41f5",
  "com.scalablytyped" %%% "babel-types" % "7.0-dt-20180910Z-5654be",
  "com.scalablytyped" %%% "babylon" % "6.16-dt-20181109Z-efe294",
  "com.scalablytyped" %%% "body-parser" % "1.17-dt-20180910Z-6ca2c5",
  "com.scalablytyped" %%% "chalk" % "2.4.1-ea2afe",
  "com.scalablytyped" %%% "clean-css" % "v3.4.9-dt-20180910Z-f6c40e",
  "com.scalablytyped" %%% "color-name" % "1.1-dt-20180910Z-566c3e",
  "com.scalablytyped" %%% "connect" % "v3.4.0-dt-20180910Z-feceb2",
  "com.scalablytyped" %%% "create-emotion" % "9.2.12-7c261d",
  "com.scalablytyped" %%% "create-react-class" % "15.6-dt-20180910Z-f33d33",
  "com.scalablytyped" %%% "csstype" % "2.5.7-eccde3",
  "com.scalablytyped" %%% "d3-color" % "1.2-dt-20180910Z-0414a8",
  "com.scalablytyped" %%% "detect-port" % "1.1-dt-20180910Z-ac56b6",
  "com.scalablytyped" %%% "escape-string-regexp" % "1.0-dt-20180910Z-76e9e4",
  "com.scalablytyped" %%% "express" % "4.16-dt-20180910Z-4d8764",
  "com.scalablytyped" %%% "express-serve-static-core" % "4.16-dt-20180910Z-43b19a",
  "com.scalablytyped" %%% "extract-text-webpack-plugin" % "3.0-dt-20180910Z-4b7e0a",
  "com.scalablytyped" %%% "friendly-errors-webpack-plugin" % "0.1-dt-20180910Z-1f7ebc",
  "com.scalablytyped" %%% "history" % "4.7.2-dt-20181018Z-c0d74d",
  "com.scalablytyped" %%% "html-minifier" % "3.5-dt-20180910Z-df7c8b",
  "com.scalablytyped" %%% "html-webpack-plugin" % "3.2-dt-20180910Z-8ce015",
  "com.scalablytyped" %%% "http-proxy" % "1.16-dt-20180910Z-7a5494",
  "com.scalablytyped" %%% "http-proxy-middleware" % "0.19-dt-20181120Z-799832",
  "com.scalablytyped" %%% "js-base64" % "2.3-dt-20180910Z-b4a065",
  "com.scalablytyped" %%% "js-yaml" % "3.11-dt-20180910Z-fd3121",
  "com.scalablytyped" %%% "marked" % "0.4-dt-20181002Z-2dc896",
  "com.scalablytyped" %%% "mime" % "2.0-dt-20180910Z-f9ab87",
  "com.scalablytyped" %%% "node" % "10.12-dt-20181116Z-cadd20",
  "com.scalablytyped" %%% "node-fetch" % "2.1-dt-20181120Z-1f4e97",
  "com.scalablytyped" %%% "postcss" % "5.2.18-7d3203",
  "com.scalablytyped" %%% "prismjs" % "1.9-dt-20180910Z-ea4f86",
  "com.scalablytyped" %%% "prop-types" % "15.5-dt-20180927Z-7b533e",
  "com.scalablytyped" %%% "ramda" % "0.25-dt-20181120Z-96bbd8",
  "com.scalablytyped" %%% "range-parser" % "1.2-dt-20180910Z-dfa7aa",
  "com.scalablytyped" %%% "react" % "16.7-dt-20181116Z-44bb36",
  "com.scalablytyped" %%% "react-document-title" % "2.0-dt-20180910Z-85f906",
  "com.scalablytyped" %%% "react-dom" % "16.0-dt-20181009Z-140611",
  "com.scalablytyped" %%% "react-router" % "4.4-dt-20181109Z-ab18e0",
  "com.scalablytyped" %%% "relateurl" % "v0.2.6-dt-20180910Z-32e068",
  "com.scalablytyped" %%% "serve-static" % "1.13-dt-20180910Z-5d47cf",
  "com.scalablytyped" %%% "source-map" % "0.7.3-2a06d1",
  "com.scalablytyped" %%% "std" % "3.1.3-0b55f4",
  "com.scalablytyped" %%% "stylis" % "3.5.3-88e110",
  "com.scalablytyped" %%% "supports-color" % "5.3-dt-20180910Z-f11a37",
  "com.scalablytyped" %%% "tapable" % "v1.0.0-dt-20180910Z-aab6b2",
  "com.scalablytyped" %%% "uglify-js" % "3.0-dt-20181018Z-3bc20d",
  "com.scalablytyped" %%% "unfetch" % "3.1.2-20ee2f",
  "com.scalablytyped" %%% "webpack" % "4.4-dt-20181116Z-01d3de",
  "com.scalablytyped" %%% "webpack-dev-server" % "3.1-dt-20180917Z-6ba4e3",
  "com.scalablytyped" %%% "webpack-manifest-plugin" % "1.3-dt-20180910Z-958b41",
  "org.scala-js" %%% "scalajs-dom" % "0.9.6")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
        