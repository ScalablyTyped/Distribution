package typings.qlikVisualizationextensions

import typings.qlikVisualizationextensions.BackendAPI.DimensionType
import typings.qlikVisualizationextensions.BackendAPI.FieldAttributesType
import typings.qlikVisualizationextensions.BackendAPI.GroupingType
import typings.qlikVisualizationextensions.BackendAPI.PatchType
import typings.qlikVisualizationextensions.BackendAPI.SortIndicatorType
import typings.qlikVisualizationextensions.BackendAPI.StateType
import typings.qlikVisualizationextensions.ExtensionAPI.ExpressionType
import typings.qlikVisualizationextensions.ExtensionAPI.SelectionModeType
import typings.qlikVisualizationextensions.NavigationAPI.NavigationErrorType
import typings.qlikVisualizationextensions.NavigationAPI.NavigationModeType
import typings.qlikVisualizationextensions.VisualizationAPI.VisualizationType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object qlikVisualizationextensionsStrings {
  
  @js.native
  sealed trait A
    extends StObject
       with FieldAttributesType
       with SortIndicatorType
       with StateType
  inline def A: A = "A".asInstanceOf[A]
  
  @js.native
  sealed trait ANALYSIS
    extends StObject
       with NavigationModeType
  inline def ANALYSIS: ANALYSIS = "ANALYSIS".asInstanceOf[ANALYSIS]
  
  @js.native
  sealed trait Add
    extends StObject
       with PatchType
  inline def Add: Add = "Add".asInstanceOf[Add]
  
  @js.native
  sealed trait C
    extends StObject
       with GroupingType
  inline def C: C = "C".asInstanceOf[C]
  
  @js.native
  sealed trait CONFIRM
    extends StObject
       with SelectionModeType
  inline def CONFIRM: CONFIRM = "CONFIRM".asInstanceOf[CONFIRM]
  
  @js.native
  sealed trait D
    extends StObject
       with DimensionType
       with FieldAttributesType
       with SortIndicatorType
       with StateType
  inline def D: D = "D".asInstanceOf[D]
  
  @js.native
  sealed trait EDIT
    extends StObject
       with NavigationModeType
  inline def EDIT: EDIT = "EDIT".asInstanceOf[EDIT]
  
  @js.native
  sealed trait F
    extends StObject
       with FieldAttributesType
  inline def F: F = "F".asInstanceOf[F]
  
  @js.native
  sealed trait H
    extends StObject
       with GroupingType
  inline def H: H = "H".asInstanceOf[H]
  
  @js.native
  sealed trait I
    extends StObject
       with FieldAttributesType
  inline def I: I = "I".asInstanceOf[I]
  
  @js.native
  sealed trait IV
    extends StObject
       with FieldAttributesType
  inline def IV: IV = "IV".asInstanceOf[IV]
  
  @js.native
  sealed trait L
    extends StObject
       with StateType
  inline def L: L = "L".asInstanceOf[L]
  
  @js.native
  sealed trait M
    extends StObject
       with FieldAttributesType
  inline def M: M = "M".asInstanceOf[M]
  
  @js.native
  sealed trait N
    extends StObject
       with DimensionType
       with GroupingType
       with SortIndicatorType
  inline def N: N = "N".asInstanceOf[N]
  
  @js.native
  sealed trait NOCURRENTSHEET
    extends StObject
       with NavigationErrorType
  inline def NOCURRENTSHEET: NOCURRENTSHEET = "NOCURRENTSHEET".asInstanceOf[NOCURRENTSHEET]
  
  @js.native
  sealed trait NOSUCHSHEET
    extends StObject
       with NavigationErrorType
  inline def NOSUCHSHEET: NOSUCHSHEET = "NOSUCHSHEET".asInstanceOf[NOSUCHSHEET]
  
  @js.native
  sealed trait NOSUCHSTORY
    extends StObject
       with NavigationErrorType
  inline def NOSUCHSTORY: NOSUCHSTORY = "NOSUCHSTORY".asInstanceOf[NOSUCHSTORY]
  
  @js.native
  sealed trait O
    extends StObject
       with StateType
  inline def O: O = "O".asInstanceOf[O]
  
  @js.native
  sealed trait QUICK
    extends StObject
       with SelectionModeType
  inline def QUICK: QUICK = "QUICK".asInstanceOf[QUICK]
  
  @js.native
  sealed trait R
    extends StObject
       with FieldAttributesType
  inline def R: R = "R".asInstanceOf[R]
  
  @js.native
  sealed trait Remove
    extends StObject
       with PatchType
  inline def Remove: Remove = "Remove".asInstanceOf[Remove]
  
  @js.native
  sealed trait Replace
    extends StObject
       with PatchType
  inline def Replace: Replace = "Replace".asInstanceOf[Replace]
  
  @js.native
  sealed trait S
    extends StObject
       with StateType
  inline def S: S = "S".asInstanceOf[S]
  
  @js.native
  sealed trait T
    extends StObject
       with DimensionType
       with FieldAttributesType
  inline def T: T = "T".asInstanceOf[T]
  
  @js.native
  sealed trait TS
    extends StObject
       with FieldAttributesType
  inline def TS: TS = "TS".asInstanceOf[TS]
  
  @js.native
  sealed trait U
    extends StObject
       with FieldAttributesType
  inline def U: U = "U".asInstanceOf[U]
  
  @js.native
  sealed trait X
    extends StObject
       with StateType
  inline def X: X = "X".asInstanceOf[X]
  
  @js.native
  sealed trait XL
    extends StObject
       with StateType
  inline def XL: XL = "XL".asInstanceOf[XL]
  
  @js.native
  sealed trait XS
    extends StObject
       with StateType
  inline def XS: XS = "XS".asInstanceOf[XS]
  
  @js.native
  sealed trait _empty
    extends StObject
       with ExpressionType
  inline def _empty: _empty = "".asInstanceOf[_empty]
  
  @js.native
  sealed trait accordion extends StObject
  inline def accordion: accordion = "accordion".asInstanceOf[accordion]
  
  @js.native
  sealed trait addons extends StObject
  inline def addons: addons = "addons".asInstanceOf[addons]
  
  @js.native
  sealed trait always
    extends StObject
       with ExpressionType
  inline def always: always = "always".asInstanceOf[always]
  
  @js.native
  sealed trait analysis_ extends StObject
  inline def analysis_ : analysis_ = "analysis".asInstanceOf[analysis_]
  
  @js.native
  sealed trait barchart
    extends StObject
       with VisualizationType
  inline def barchart: barchart = "barchart".asInstanceOf[barchart]
  
  @js.native
  sealed trait button extends StObject
  inline def button: button = "button".asInstanceOf[button]
  
  @js.native
  sealed trait buttongroup extends StObject
  inline def buttongroup: buttongroup = "buttongroup".asInstanceOf[buttongroup]
  
  @js.native
  sealed trait changed extends StObject
  inline def changed: changed = "changed".asInstanceOf[changed]
  
  @js.native
  sealed trait checkbox extends StObject
  inline def checkbox: checkbox = "checkbox".asInstanceOf[checkbox]
  
  @js.native
  sealed trait closed extends StObject
  inline def closed: closed = "closed".asInstanceOf[closed]
  
  @js.native
  sealed trait `color-picker` extends StObject
  inline def `color-picker`: `color-picker` = "color-picker".asInstanceOf[`color-picker`]
  
  @js.native
  sealed trait combochart
    extends StObject
       with VisualizationType
  inline def combochart: combochart = "combochart".asInstanceOf[combochart]
  
  @js.native
  sealed trait dimensions extends StObject
  inline def dimensions: dimensions = "dimensions".asInstanceOf[dimensions]
  
  @js.native
  sealed trait dropdown extends StObject
  inline def dropdown: dropdown = "dropdown".asInstanceOf[dropdown]
  
  @js.native
  sealed trait edit_ extends StObject
  inline def edit_ : edit_ = "edit".asInstanceOf[edit_]
  
  @js.native
  sealed trait `extension`
    extends StObject
       with VisualizationType
  inline def `extension`: `extension` = "extension".asInstanceOf[`extension`]
  
  @js.native
  sealed trait gauge
    extends StObject
       with VisualizationType
  inline def gauge: gauge = "gauge".asInstanceOf[gauge]
  
  @js.native
  sealed trait items extends StObject
  inline def items: items = "items".asInstanceOf[items]
  
  @js.native
  sealed trait kpi
    extends StObject
       with VisualizationType
  inline def kpi: kpi = "kpi".asInstanceOf[kpi]
  
  @js.native
  sealed trait linechart
    extends StObject
       with VisualizationType
  inline def linechart: linechart = "linechart".asInstanceOf[linechart]
  
  @js.native
  sealed trait link extends StObject
  inline def link: link = "link".asInstanceOf[link]
  
  @js.native
  sealed trait listbox
    extends StObject
       with VisualizationType
  inline def listbox: listbox = "listbox".asInstanceOf[listbox]
  
  @js.native
  sealed trait measures extends StObject
  inline def measures: measures = "measures".asInstanceOf[measures]
  
  @js.native
  sealed trait media extends StObject
  inline def media: media = "media".asInstanceOf[media]
  
  @js.native
  sealed trait optional
    extends StObject
       with ExpressionType
  inline def optional: optional = "optional".asInstanceOf[optional]
  
  @js.native
  sealed trait piechart
    extends StObject
       with VisualizationType
  inline def piechart: piechart = "piechart".asInstanceOf[piechart]
  
  @js.native
  sealed trait `pivot-table`
    extends StObject
       with VisualizationType
  inline def `pivot-table`: `pivot-table` = "pivot-table".asInstanceOf[`pivot-table`]
  
  @js.native
  sealed trait radiobuttons extends StObject
  inline def radiobuttons: radiobuttons = "radiobuttons".asInstanceOf[radiobuttons]
  
  @js.native
  sealed trait scatterplot
    extends StObject
       with VisualizationType
  inline def scatterplot: scatterplot = "scatterplot".asInstanceOf[scatterplot]
  
  @js.native
  sealed trait settings extends StObject
  inline def settings: settings = "settings".asInstanceOf[settings]
  
  @js.native
  sealed trait slider extends StObject
  inline def slider: slider = "slider".asInstanceOf[slider]
  
  @js.native
  sealed trait sorting extends StObject
  inline def sorting: sorting = "sorting".asInstanceOf[sorting]
  
  @js.native
  sealed trait switch extends StObject
  inline def switch: switch = "switch".asInstanceOf[switch]
  
  @js.native
  sealed trait table
    extends StObject
       with VisualizationType
  inline def table: table = "table".asInstanceOf[table]
  
  @js.native
  sealed trait text extends StObject
  inline def text: text = "text".asInstanceOf[text]
  
  @js.native
  sealed trait textarea extends StObject
  inline def textarea: textarea = "textarea".asInstanceOf[textarea]
  
  @js.native
  sealed trait treemap
    extends StObject
       with VisualizationType
  inline def treemap: treemap = "treemap".asInstanceOf[treemap]
}
