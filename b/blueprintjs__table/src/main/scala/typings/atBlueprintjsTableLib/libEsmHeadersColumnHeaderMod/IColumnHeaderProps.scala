...
trait IColumnHeaderProps
  ...
  /**
    ...
    */
  @JSName("cellRenderer")
  var cellRenderer_Original: IColumnHeaderRenderer = js.native
  /**
  ...
  /**
    ...
    */
  @JSName("onColumnWidthChanged")
  var onColumnWidthChanged_Original: atBlueprintjsTableLib.libEsmInteractionsResizableMod.IIndexedResizeCallback = js.native
  /**
    * A IColumnHeaderRenderer that, for each `<Column>`, will delegate to:
    * 1. The `columnHeaderCellRenderer` method from the `<Column>`
    * 2. A `<ColumnHeaderCell>` using the `name` prop from the `<Column>`
  ...
  /**
    * A callback invoked when user is done resizing the column
    */
  def onColumnWidthChanged(index: scala.Double, size: scala.Double): scala.Unit = js.native
}
