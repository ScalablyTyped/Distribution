...
object TextboxOptions {
  ...
  def apply(
    ...
    if (!js.isUndefined(censor)) __obj.updateDynamic("censor")(censor)
    ...
    if (!js.isUndefined(inputOnFocus)) __obj.updateDynamic("inputOnFocus")(inputOnFocus)
    ...
    if (!js.isUndefined(secret)) __obj.updateDynamic("secret")(secret)
