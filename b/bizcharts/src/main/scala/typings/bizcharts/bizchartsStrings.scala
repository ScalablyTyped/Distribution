package typings.bizcharts

import typings.bizcharts.mod.AlignXType
import typings.bizcharts.mod.AlignYType
import typings.bizcharts.mod.CoordType
import typings.bizcharts.mod.CrosshairsType
import typings.bizcharts.mod.FacetType
import typings.bizcharts.mod.GeomAdjustType
import typings.bizcharts.mod.GeomType
import typings.bizcharts.mod.LegendLayoutType
import typings.bizcharts.mod.LegendPositionType
import typings.bizcharts.mod.MarkerType
import typings.bizcharts.mod.PositionType
import typings.bizcharts.mod.triggerOnType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object bizchartsStrings {
  
  @js.native
  sealed trait area
    extends StObject
       with GeomType
  inline def area: area = "area".asInstanceOf[area]
  
  @js.native
  sealed trait areaStack
    extends StObject
       with GeomType
  inline def areaStack: areaStack = "areaStack".asInstanceOf[areaStack]
  
  @js.native
  sealed trait bottom
    extends StObject
       with AlignYType
       with LegendPositionType
       with PositionType
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait `bottom-center`
    extends StObject
       with LegendPositionType
  inline def `bottom-center`: `bottom-center` = "bottom-center".asInstanceOf[`bottom-center`]
  
  @js.native
  sealed trait `bottom-left`
    extends StObject
       with LegendPositionType
  inline def `bottom-left`: `bottom-left` = "bottom-left".asInstanceOf[`bottom-left`]
  
  @js.native
  sealed trait `bottom-right`
    extends StObject
       with LegendPositionType
  inline def `bottom-right`: `bottom-right` = "bottom-right".asInstanceOf[`bottom-right`]
  
  @js.native
  sealed trait bowtie
    extends StObject
       with MarkerType
  inline def bowtie: bowtie = "bowtie".asInstanceOf[bowtie]
  
  @js.native
  sealed trait circle
    extends StObject
       with FacetType
       with MarkerType
  inline def circle: circle = "circle".asInstanceOf[circle]
  
  @js.native
  sealed trait click
    extends StObject
       with triggerOnType
  inline def click: click = "click".asInstanceOf[click]
  
  @js.native
  sealed trait cross
    extends StObject
       with CrosshairsType
       with MarkerType
  inline def cross: cross = "cross".asInstanceOf[cross]
  
  @js.native
  sealed trait diamond
    extends StObject
       with MarkerType
  inline def diamond: diamond = "diamond".asInstanceOf[diamond]
  
  @js.native
  sealed trait dodge
    extends StObject
       with GeomAdjustType
  inline def dodge: dodge = "dodge".asInstanceOf[dodge]
  
  @js.native
  sealed trait downward extends StObject
  inline def downward: downward = "downward".asInstanceOf[downward]
  
  @js.native
  sealed trait edge
    extends StObject
       with GeomType
  inline def edge: edge = "edge".asInstanceOf[edge]
  
  @js.native
  sealed trait heatmap
    extends StObject
       with GeomType
  inline def heatmap: heatmap = "heatmap".asInstanceOf[heatmap]
  
  @js.native
  sealed trait helix
    extends StObject
       with CoordType
  inline def helix: helix = "helix".asInstanceOf[helix]
  
  @js.native
  sealed trait hexagon
    extends StObject
       with MarkerType
  inline def hexagon: hexagon = "hexagon".asInstanceOf[hexagon]
  
  @js.native
  sealed trait hollowBowtie
    extends StObject
       with MarkerType
  inline def hollowBowtie: hollowBowtie = "hollowBowtie".asInstanceOf[hollowBowtie]
  
  @js.native
  sealed trait hollowCircle
    extends StObject
       with MarkerType
  inline def hollowCircle: hollowCircle = "hollowCircle".asInstanceOf[hollowCircle]
  
  @js.native
  sealed trait hollowDiamond
    extends StObject
       with MarkerType
  inline def hollowDiamond: hollowDiamond = "hollowDiamond".asInstanceOf[hollowDiamond]
  
  @js.native
  sealed trait hollowHexagon
    extends StObject
       with MarkerType
  inline def hollowHexagon: hollowHexagon = "hollowHexagon".asInstanceOf[hollowHexagon]
  
  @js.native
  sealed trait hollowSquare
    extends StObject
       with MarkerType
  inline def hollowSquare: hollowSquare = "hollowSquare".asInstanceOf[hollowSquare]
  
  @js.native
  sealed trait hollowTriangle
    extends StObject
       with MarkerType
  inline def hollowTriangle: hollowTriangle = "hollowTriangle".asInstanceOf[hollowTriangle]
  
  @js.native
  sealed trait `hollowTriangle-down`
    extends StObject
       with MarkerType
  inline def `hollowTriangle-down`: `hollowTriangle-down` = "hollowTriangle-down".asInstanceOf[`hollowTriangle-down`]
  
  @js.native
  sealed trait horizontal
    extends StObject
       with LegendLayoutType
  inline def horizontal: horizontal = "horizontal".asInstanceOf[horizontal]
  
  @js.native
  sealed trait hyphen
    extends StObject
       with MarkerType
  inline def hyphen: hyphen = "hyphen".asInstanceOf[hyphen]
  
  @js.native
  sealed trait interval
    extends StObject
       with GeomType
  inline def interval: interval = "interval".asInstanceOf[interval]
  
  @js.native
  sealed trait intervalDodge
    extends StObject
       with GeomType
  inline def intervalDodge: intervalDodge = "intervalDodge".asInstanceOf[intervalDodge]
  
  @js.native
  sealed trait intervalStack
    extends StObject
       with GeomType
  inline def intervalStack: intervalStack = "intervalStack".asInstanceOf[intervalStack]
  
  @js.native
  sealed trait intervalSymmetric
    extends StObject
       with GeomType
  inline def intervalSymmetric: intervalSymmetric = "intervalSymmetric".asInstanceOf[intervalSymmetric]
  
  @js.native
  sealed trait jitter
    extends StObject
       with GeomAdjustType
  inline def jitter: jitter = "jitter".asInstanceOf[jitter]
  
  @js.native
  sealed trait left
    extends StObject
       with AlignXType
       with LegendPositionType
       with PositionType
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait `left-bottom`
    extends StObject
       with LegendPositionType
  inline def `left-bottom`: `left-bottom` = "left-bottom".asInstanceOf[`left-bottom`]
  
  @js.native
  sealed trait `left-center`
    extends StObject
       with LegendPositionType
  inline def `left-center`: `left-center` = "left-center".asInstanceOf[`left-center`]
  
  @js.native
  sealed trait `left-top`
    extends StObject
       with LegendPositionType
  inline def `left-top`: `left-top` = "left-top".asInstanceOf[`left-top`]
  
  @js.native
  sealed trait line
    extends StObject
       with GeomType
       with MarkerType
  inline def line: line = "line".asInstanceOf[line]
  
  @js.native
  sealed trait lineStack
    extends StObject
       with GeomType
  inline def lineStack: lineStack = "lineStack".asInstanceOf[lineStack]
  
  @js.native
  sealed trait list
    extends StObject
       with FacetType
  inline def list: list = "list".asInstanceOf[list]
  
  @js.native
  sealed trait matrix
    extends StObject
       with FacetType
  inline def matrix: matrix = "matrix".asInstanceOf[matrix]
  
  @js.native
  sealed trait middle
    extends StObject
       with AlignXType
       with AlignYType
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait mirror
    extends StObject
       with FacetType
  inline def mirror: mirror = "mirror".asInstanceOf[mirror]
  
  @js.native
  sealed trait mousemove
    extends StObject
       with triggerOnType
  inline def mousemove: mousemove = "mousemove".asInstanceOf[mousemove]
  
  @js.native
  sealed trait multiple extends StObject
  inline def multiple: multiple = "multiple".asInstanceOf[multiple]
  
  @js.native
  sealed trait none
    extends StObject
       with triggerOnType
  inline def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait path
    extends StObject
       with GeomType
  inline def path: path = "path".asInstanceOf[path]
  
  @js.native
  sealed trait plus
    extends StObject
       with MarkerType
  inline def plus: plus = "plus".asInstanceOf[plus]
  
  @js.native
  sealed trait point
    extends StObject
       with GeomType
  inline def point: point = "point".asInstanceOf[point]
  
  @js.native
  sealed trait pointDodge
    extends StObject
       with GeomType
  inline def pointDodge: pointDodge = "pointDodge".asInstanceOf[pointDodge]
  
  @js.native
  sealed trait pointJitter
    extends StObject
       with GeomType
  inline def pointJitter: pointJitter = "pointJitter".asInstanceOf[pointJitter]
  
  @js.native
  sealed trait pointStack
    extends StObject
       with GeomType
  inline def pointStack: pointStack = "pointStack".asInstanceOf[pointStack]
  
  @js.native
  sealed trait polar
    extends StObject
       with CoordType
  inline def polar: polar = "polar".asInstanceOf[polar]
  
  @js.native
  sealed trait polygon
    extends StObject
       with GeomType
  inline def polygon: polygon = "polygon".asInstanceOf[polygon]
  
  @js.native
  sealed trait rect
    extends StObject
       with CoordType
       with CrosshairsType
       with FacetType
  inline def rect: rect = "rect".asInstanceOf[rect]
  
  @js.native
  sealed trait right
    extends StObject
       with AlignXType
       with LegendPositionType
       with PositionType
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait `right-bottom`
    extends StObject
       with LegendPositionType
  inline def `right-bottom`: `right-bottom` = "right-bottom".asInstanceOf[`right-bottom`]
  
  @js.native
  sealed trait `right-center`
    extends StObject
       with LegendPositionType
  inline def `right-center`: `right-center` = "right-center".asInstanceOf[`right-center`]
  
  @js.native
  sealed trait `right-top`
    extends StObject
       with LegendPositionType
  inline def `right-top`: `right-top` = "right-top".asInstanceOf[`right-top`]
  
  @js.native
  sealed trait schema
    extends StObject
       with GeomType
  inline def schema: schema = "schema".asInstanceOf[schema]
  
  @js.native
  sealed trait schemaDodge
    extends StObject
       with GeomType
  inline def schemaDodge: schemaDodge = "schemaDodge".asInstanceOf[schemaDodge]
  
  @js.native
  sealed trait single extends StObject
  inline def single: single = "single".asInstanceOf[single]
  
  @js.native
  sealed trait square
    extends StObject
       with MarkerType
  inline def square: square = "square".asInstanceOf[square]
  
  @js.native
  sealed trait stack
    extends StObject
       with GeomAdjustType
  inline def stack: stack = "stack".asInstanceOf[stack]
  
  @js.native
  sealed trait symmetric
    extends StObject
       with GeomAdjustType
  inline def symmetric: symmetric = "symmetric".asInstanceOf[symmetric]
  
  @js.native
  sealed trait theta
    extends StObject
       with CoordType
  inline def theta: theta = "theta".asInstanceOf[theta]
  
  @js.native
  sealed trait tick
    extends StObject
       with MarkerType
  inline def tick: tick = "tick".asInstanceOf[tick]
  
  @js.native
  sealed trait top
    extends StObject
       with AlignYType
       with LegendPositionType
       with PositionType
  inline def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait `top-center`
    extends StObject
       with LegendPositionType
  inline def `top-center`: `top-center` = "top-center".asInstanceOf[`top-center`]
  
  @js.native
  sealed trait `top-left`
    extends StObject
       with LegendPositionType
  inline def `top-left`: `top-left` = "top-left".asInstanceOf[`top-left`]
  
  @js.native
  sealed trait `top-right`
    extends StObject
       with LegendPositionType
  inline def `top-right`: `top-right` = "top-right".asInstanceOf[`top-right`]
  
  @js.native
  sealed trait tree
    extends StObject
       with FacetType
  inline def tree: tree = "tree".asInstanceOf[tree]
  
  @js.native
  sealed trait triangle
    extends StObject
       with MarkerType
  inline def triangle: triangle = "triangle".asInstanceOf[triangle]
  
  @js.native
  sealed trait `triangle-down`
    extends StObject
       with MarkerType
  inline def `triangle-down`: `triangle-down` = "triangle-down".asInstanceOf[`triangle-down`]
  
  @js.native
  sealed trait upward extends StObject
  inline def upward: upward = "upward".asInstanceOf[upward]
  
  @js.native
  sealed trait vertical
    extends StObject
       with LegendLayoutType
  inline def vertical: vertical = "vertical".asInstanceOf[vertical]
  
  @js.native
  sealed trait x
    extends StObject
       with CrosshairsType
  inline def x: x = "x".asInstanceOf[x]
  
  @js.native
  sealed trait y
    extends StObject
       with CrosshairsType
  inline def y: y = "y".asInstanceOf[y]
}
