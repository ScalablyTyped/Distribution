...
/* Inlined {  $initialTime :number,   $timeElapsed :number} & baseui.baseui/button.SharedStyleProps */
trait initialTimenumbertimeElap extends StObject {
...
object initialTimenumbertimeElap {
  
  inline def apply($initialTime: Double, $timeElapsed: Double): initialTimenumbertimeElap = {
    val __obj = js.Dynamic.literal($initialTime = $initialTime.asInstanceOf[js.Any], $timeElapsed = $timeElapsed.asInstanceOf[js.Any])
  ...
  implicit open class MutableBuilder[Self <: initialTimenumbertimeElap] (val x: Self) extends AnyVal {
    ...
    inline def set$initialTime(value: Double): Self = StObject.set(x, "$initialTime", value.asInstanceOf[js.Any])
    ...
    inline def set$timeElapsed(value: Double): Self = StObject.set(x, "$timeElapsed", value.asInstanceOf[js.Any])
  }
}
