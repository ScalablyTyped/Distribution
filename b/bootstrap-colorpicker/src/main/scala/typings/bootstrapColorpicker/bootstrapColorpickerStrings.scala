package typings.bootstrapColorpicker

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object bootstrapColorpickerStrings {
  
  @js.native
  sealed trait changeColor
    extends StObject
       with ColorPickerEvents
  inline def changeColor: changeColor = "changeColor".asInstanceOf[changeColor]
  
  @js.native
  sealed trait colorpicker extends StObject
  inline def colorpicker: colorpicker = "colorpicker".asInstanceOf[colorpicker]
  
  @js.native
  sealed trait create
    extends StObject
       with ColorPickerEvents
  inline def create: create = "create".asInstanceOf[create]
  
  @js.native
  sealed trait destroy
    extends StObject
       with ColorPickerEvents
  inline def destroy: destroy = "destroy".asInstanceOf[destroy]
  
  @js.native
  sealed trait disable
    extends StObject
       with ColorPickerEvents
  inline def disable: disable = "disable".asInstanceOf[disable]
  
  @js.native
  sealed trait enable
    extends StObject
       with ColorPickerEvents
  inline def enable: enable = "enable".asInstanceOf[enable]
  
  @js.native
  sealed trait getValue extends StObject
  inline def getValue: getValue = "getValue".asInstanceOf[getValue]
  
  @js.native
  sealed trait hex
    extends StObject
       with ColorFormat
  inline def hex: hex = "hex".asInstanceOf[hex]
  
  @js.native
  sealed trait hide extends StObject
  inline def hide: hide = "hide".asInstanceOf[hide]
  
  @js.native
  sealed trait hidePicker
    extends StObject
       with ColorPickerEvents
  inline def hidePicker: hidePicker = "hidePicker".asInstanceOf[hidePicker]
  
  @js.native
  sealed trait left
    extends StObject
       with ColorPickerAlignOptions
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait reposition extends StObject
  inline def reposition: reposition = "reposition".asInstanceOf[reposition]
  
  @js.native
  sealed trait rgb
    extends StObject
       with ColorFormat
  inline def rgb: rgb = "rgb".asInstanceOf[rgb]
  
  @js.native
  sealed trait rgba
    extends StObject
       with ColorFormat
  inline def rgba: rgba = "rgba".asInstanceOf[rgba]
  
  @js.native
  sealed trait right
    extends StObject
       with ColorPickerAlignOptions
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait setValue extends StObject
  inline def setValue: setValue = "setValue".asInstanceOf[setValue]
  
  @js.native
  sealed trait show extends StObject
  inline def show: show = "show".asInstanceOf[show]
  
  @js.native
  sealed trait showPicker
    extends StObject
       with ColorPickerEvents
  inline def showPicker: showPicker = "showPicker".asInstanceOf[showPicker]
  
  @js.native
  sealed trait update extends StObject
  inline def update: update = "update".asInstanceOf[update]
}
