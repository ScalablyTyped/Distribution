package typings.atBandagedbdBdapi

import typings.react.reactMod.ComponentType
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object atBandagedbdBdapiComponents extends atBandagedbdBdapiProps {
  @scala.inline
  def BdApiModuleReactFragment: ComponentType[BdApiModuleReactFragmentProps] = typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.Fragment.asInstanceOf[typings.react.reactMod.ComponentType[BdApiModuleReactFragmentProps]]
  @scala.inline
  def BdApiModuleReactProfiler: ComponentType[BdApiModuleReactProfilerProps] = typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.Profiler.asInstanceOf[typings.react.reactMod.ComponentType[BdApiModuleReactProfilerProps]]
  @scala.inline
  def BdApiModuleReactPureComponent[P, S, SS]: ComponentType[P] = js.constructorOf[typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.PureComponent[P, S, SS]].asInstanceOf[typings.react.reactMod.ComponentType[P]]
  @scala.inline
  def BdApiModuleReactStrictMode: ComponentType[BdApiModuleReactStrictModeProps] = typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.StrictMode.asInstanceOf[typings.react.reactMod.ComponentType[BdApiModuleReactStrictModeProps]]
  @scala.inline
  def BdApiModuleReactSuspense: ComponentType[BdApiModuleReactSuspenseProps] = typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.Suspense.asInstanceOf[typings.react.reactMod.ComponentType[BdApiModuleReactSuspenseProps]]
  @scala.inline
  def BdApiModuleReactSuspenseList: ComponentType[BdApiModuleReactSuspenseListProps] = typings.atBandagedbdBdapi.atBandagedbdBdapiMod.BdApiModule.React.SuspenseList.asInstanceOf[typings.react.reactMod.ComponentType[BdApiModuleReactSuspenseListProps]]
}

