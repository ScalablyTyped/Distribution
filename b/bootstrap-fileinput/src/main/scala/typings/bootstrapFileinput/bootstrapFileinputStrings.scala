package typings.bootstrapFileinput

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object bootstrapFileinputStrings {
  @js.native
  sealed trait addToStack extends js.Object
  
  @js.native
  sealed trait any extends js.Object
  
  @js.native
  sealed trait audio extends js.Object
  
  @js.native
  sealed trait cancel extends js.Object
  
  @js.native
  sealed trait clear extends js.Object
  
  @js.native
  sealed trait clearFileStack extends js.Object
  
  @js.native
  sealed trait destroy extends js.Object
  
  @js.native
  sealed trait disable extends js.Object
  
  @js.native
  sealed trait enable extends js.Object
  
  @js.native
  sealed trait flash extends js.Object
  
  @js.native
  sealed trait getExif extends js.Object
  
  @js.native
  sealed trait getFileList extends js.Object
  
  @js.native
  sealed trait getFileStack extends js.Object
  
  @js.native
  sealed trait getFilesCount extends js.Object
  
  @js.native
  sealed trait getFrames extends js.Object
  
  @js.native
  sealed trait getPreview extends js.Object
  
  @js.native
  sealed trait height extends js.Object
  
  @js.native
  sealed trait html extends js.Object
  
  @js.native
  sealed trait image extends js.Object
  
  @js.native
  sealed trait lock extends js.Object
  
  @js.native
  sealed trait `object` extends js.Object
  
  @js.native
  sealed trait pause extends js.Object
  
  @js.native
  sealed trait readFiles extends js.Object
  
  @js.native
  sealed trait refresh extends js.Object
  
  @js.native
  sealed trait reset extends js.Object
  
  @js.native
  sealed trait resume extends js.Object
  
  @js.native
  sealed trait text extends js.Object
  
  @js.native
  sealed trait unlock extends js.Object
  
  @js.native
  sealed trait upload extends js.Object
  
  @js.native
  sealed trait video extends js.Object
  
  @js.native
  sealed trait width extends js.Object
  
  @js.native
  sealed trait zoom extends js.Object
  
  @scala.inline
  def addToStack: addToStack = "addToStack".asInstanceOf[addToStack]
  @scala.inline
  def any: any = "any".asInstanceOf[any]
  @scala.inline
  def audio: audio = "audio".asInstanceOf[audio]
  @scala.inline
  def cancel: cancel = "cancel".asInstanceOf[cancel]
  @scala.inline
  def clear: clear = "clear".asInstanceOf[clear]
  @scala.inline
  def clearFileStack: clearFileStack = "clearFileStack".asInstanceOf[clearFileStack]
  @scala.inline
  def destroy: destroy = "destroy".asInstanceOf[destroy]
  @scala.inline
  def disable: disable = "disable".asInstanceOf[disable]
  @scala.inline
  def enable: enable = "enable".asInstanceOf[enable]
  @scala.inline
  def flash: flash = "flash".asInstanceOf[flash]
  @scala.inline
  def getExif: getExif = "getExif".asInstanceOf[getExif]
  @scala.inline
  def getFileList: getFileList = "getFileList".asInstanceOf[getFileList]
  @scala.inline
  def getFileStack: getFileStack = "getFileStack".asInstanceOf[getFileStack]
  @scala.inline
  def getFilesCount: getFilesCount = "getFilesCount".asInstanceOf[getFilesCount]
  @scala.inline
  def getFrames: getFrames = "getFrames".asInstanceOf[getFrames]
  @scala.inline
  def getPreview: getPreview = "getPreview".asInstanceOf[getPreview]
  @scala.inline
  def height: height = "height".asInstanceOf[height]
  @scala.inline
  def html: html = "html".asInstanceOf[html]
  @scala.inline
  def image: image = "image".asInstanceOf[image]
  @scala.inline
  def lock: lock = "lock".asInstanceOf[lock]
  @scala.inline
  def `object`: `object` = "object".asInstanceOf[`object`]
  @scala.inline
  def pause: pause = "pause".asInstanceOf[pause]
  @scala.inline
  def readFiles: readFiles = "readFiles".asInstanceOf[readFiles]
  @scala.inline
  def refresh: refresh = "refresh".asInstanceOf[refresh]
  @scala.inline
  def reset: reset = "reset".asInstanceOf[reset]
  @scala.inline
  def resume: resume = "resume".asInstanceOf[resume]
  @scala.inline
  def text: text = "text".asInstanceOf[text]
  @scala.inline
  def unlock: unlock = "unlock".asInstanceOf[unlock]
  @scala.inline
  def upload: upload = "upload".asInstanceOf[upload]
  @scala.inline
  def video: video = "video".asInstanceOf[video]
  @scala.inline
  def width: width = "width".asInstanceOf[width]
  @scala.inline
  def zoom: zoom = "zoom".asInstanceOf[zoom]
}

