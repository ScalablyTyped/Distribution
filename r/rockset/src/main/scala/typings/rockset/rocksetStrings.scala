package typings.rockset

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object rocksetStrings {
  
  @js.native
  sealed trait ACTIVE extends StObject
  @scala.inline
  def ACTIVE: ACTIVE = "ACTIVE".asInstanceOf[ACTIVE]
  
  @js.native
  sealed trait ADDED extends StObject
  @scala.inline
  def ADDED: ADDED = "ADDED".asInstanceOf[ADDED]
  
  @js.native
  sealed trait ALREADYEXISTS extends StObject
  @scala.inline
  def ALREADYEXISTS: ALREADYEXISTS = "ALREADYEXISTS".asInstanceOf[ALREADYEXISTS]
  
  @js.native
  sealed trait AUTHEXCEPTION extends StObject
  @scala.inline
  def AUTHEXCEPTION: AUTHEXCEPTION = "AUTHEXCEPTION".asInstanceOf[AUTHEXCEPTION]
  
  @js.native
  sealed trait BASIC extends StObject
  @scala.inline
  def BASIC: BASIC = "BASIC".asInstanceOf[BASIC]
  
  @js.native
  sealed trait CATCHINGUP extends StObject
  @scala.inline
  def CATCHINGUP: CATCHINGUP = "CATCHINGUP".asInstanceOf[CATCHINGUP]
  
  @js.native
  sealed trait COMPLETED extends StObject
  @scala.inline
  def COMPLETED: COMPLETED = "COMPLETED".asInstanceOf[COMPLETED]
  
  @js.native
  sealed trait CONNECTION_ERROR extends StObject
  @scala.inline
  def CONNECTION_ERROR: CONNECTION_ERROR = "CONNECTION_ERROR".asInstanceOf[CONNECTION_ERROR]
  
  @js.native
  sealed trait CREATED extends StObject
  @scala.inline
  def CREATED: CREATED = "CREATED".asInstanceOf[CREATED]
  
  @js.native
  sealed trait DELETED extends StObject
  @scala.inline
  def DELETED: DELETED = "DELETED".asInstanceOf[DELETED]
  
  @js.native
  sealed trait DEPENDENTRESOURCES extends StObject
  @scala.inline
  def DEPENDENTRESOURCES: DEPENDENTRESOURCES = "DEPENDENTRESOURCES".asInstanceOf[DEPENDENTRESOURCES]
  
  @js.native
  sealed trait ENTERPRISE extends StObject
  @scala.inline
  def ENTERPRISE: ENTERPRISE = "ENTERPRISE".asInstanceOf[ENTERPRISE]
  
  @js.native
  sealed trait ERROR extends StObject
  @scala.inline
  def ERROR: ERROR = "ERROR".asInstanceOf[ERROR]
  
  @js.native
  sealed trait FAIL extends StObject
  @scala.inline
  def FAIL: FAIL = "FAIL".asInstanceOf[FAIL]
  
  @js.native
  sealed trait FORBIDDEN extends StObject
  @scala.inline
  def FORBIDDEN: FORBIDDEN = "FORBIDDEN".asInstanceOf[FORBIDDEN]
  
  @js.native
  sealed trait FREE extends StObject
  @scala.inline
  def FREE: FREE = "FREE".asInstanceOf[FREE]
  
  @js.native
  sealed trait INITIALIZING extends StObject
  @scala.inline
  def INITIALIZING: INITIALIZING = "INITIALIZING".asInstanceOf[INITIALIZING]
  
  @js.native
  sealed trait INTEGRATION_NOT_FOUND extends StObject
  @scala.inline
  def INTEGRATION_NOT_FOUND: INTEGRATION_NOT_FOUND = "INTEGRATION_NOT_FOUND".asInstanceOf[INTEGRATION_NOT_FOUND]
  
  @js.native
  sealed trait INTERNALERROR extends StObject
  @scala.inline
  def INTERNALERROR: INTERNALERROR = "INTERNALERROR".asInstanceOf[INTERNALERROR]
  
  @js.native
  sealed trait INVALIDINPUT extends StObject
  @scala.inline
  def INVALIDINPUT: INVALIDINPUT = "INVALIDINPUT".asInstanceOf[INVALIDINPUT]
  
  @js.native
  sealed trait NOTFOUND extends StObject
  @scala.inline
  def NOTFOUND: NOTFOUND = "NOTFOUND".asInstanceOf[NOTFOUND]
  
  @js.native
  sealed trait NOTIMPLEMENTEDYET extends StObject
  @scala.inline
  def NOTIMPLEMENTEDYET: NOTIMPLEMENTEDYET = "NOTIMPLEMENTEDYET".asInstanceOf[NOTIMPLEMENTEDYET]
  
  @js.native
  sealed trait NOT_READY extends StObject
  @scala.inline
  def NOT_READY: NOT_READY = "NOT_READY".asInstanceOf[NOT_READY]
  
  @js.native
  sealed trait PASS extends StObject
  @scala.inline
  def PASS: PASS = "PASS".asInstanceOf[PASS]
  
  @js.native
  sealed trait PAUSED extends StObject
  @scala.inline
  def PAUSED: PAUSED = "PAUSED".asInstanceOf[PAUSED]
  
  @js.native
  sealed trait PAUSING extends StObject
  @scala.inline
  def PAUSING: PAUSING = "PAUSING".asInstanceOf[PAUSING]
  
  @js.native
  sealed trait PRO extends StObject
  @scala.inline
  def PRO: PRO = "PRO".asInstanceOf[PRO]
  
  @js.native
  sealed trait PROCESSING extends StObject
  @scala.inline
  def PROCESSING: PROCESSING = "PROCESSING".asInstanceOf[PROCESSING]
  
  @js.native
  sealed trait QUERY_ERROR extends StObject
  @scala.inline
  def QUERY_ERROR: QUERY_ERROR = "QUERY_ERROR".asInstanceOf[QUERY_ERROR]
  
  @js.native
  sealed trait QUERY_TIMEOUT extends StObject
  @scala.inline
  def QUERY_TIMEOUT: QUERY_TIMEOUT = "QUERY_TIMEOUT".asInstanceOf[QUERY_TIMEOUT]
  
  @js.native
  sealed trait READY extends StObject
  @scala.inline
  def READY: READY = "READY".asInstanceOf[READY]
  
  @js.native
  sealed trait REPLACED extends StObject
  @scala.inline
  def REPLACED: REPLACED = "REPLACED".asInstanceOf[REPLACED]
  
  @js.native
  sealed trait RESOURCEEXCEEDED extends StObject
  @scala.inline
  def RESOURCEEXCEEDED: RESOURCEEXCEEDED = "RESOURCEEXCEEDED".asInstanceOf[RESOURCEEXCEEDED]
  
  @js.native
  sealed trait RESUMING extends StObject
  @scala.inline
  def RESUMING: RESUMING = "RESUMING".asInstanceOf[RESUMING]
  
  @js.native
  sealed trait ROLE_NOT_FOUND extends StObject
  @scala.inline
  def ROLE_NOT_FOUND: ROLE_NOT_FOUND = "ROLE_NOT_FOUND".asInstanceOf[ROLE_NOT_FOUND]
  
  @js.native
  sealed trait SKIP extends StObject
  @scala.inline
  def SKIP: SKIP = "SKIP".asInstanceOf[SKIP]
  
  @js.native
  sealed trait TRIAL extends StObject
  @scala.inline
  def TRIAL: TRIAL = "TRIAL".asInstanceOf[TRIAL]
  
  @js.native
  sealed trait TRIAL_EXPIRED extends StObject
  @scala.inline
  def TRIAL_EXPIRED: TRIAL_EXPIRED = "TRIAL_EXPIRED".asInstanceOf[TRIAL_EXPIRED]
  
  @js.native
  sealed trait UNKNOWN extends StObject
  @scala.inline
  def UNKNOWN: UNKNOWN = "UNKNOWN".asInstanceOf[UNKNOWN]
  
  @js.native
  sealed trait VERSIONEXCEPTION extends StObject
  @scala.inline
  def VERSIONEXCEPTION: VERSIONEXCEPTION = "VERSIONEXCEPTION".asInstanceOf[VERSIONEXCEPTION]
  
  @js.native
  sealed trait WATCHING extends StObject
  @scala.inline
  def WATCHING: WATCHING = "WATCHING".asInstanceOf[WATCHING]
}
