...
trait RedirectResult
  ...
  var `type`: typings.remixRunRouter.utilsMod.ResultType.redirect
}
object RedirectResult {
  ...
  extension [Self <: RedirectResult](x: Self) {
    ...
    inline def setType(value: typings.remixRunRouter.utilsMod.ResultType.redirect): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
