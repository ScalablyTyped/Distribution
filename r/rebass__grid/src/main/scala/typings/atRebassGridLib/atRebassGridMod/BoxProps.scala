...
- Dropped {[ P in {[ P in keyof react.react.ComponentPropsWithRef<'div'> ]: P} & {[ P in 'color' | 'is' ]: never} & {[x: string] : never, [x: number] : never}[keyof react.react.ComponentPropsWithRef<'div'>] ]: react.react.ComponentPropsWithRef<'div'>[P]} */ trait BoxProps extends CommonProps {
