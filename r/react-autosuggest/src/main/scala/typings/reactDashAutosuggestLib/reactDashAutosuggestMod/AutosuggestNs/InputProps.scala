...
- Dropped {[ P in {[ P in keyof react.react.React.InputHTMLAttributes<any> ]: P} & {[ P in 'onChange' | 'onBlur' ]: never} & {[x: string] : never, [x: number] : never}[keyof react.react.React.InputHTMLAttributes<any>] ]: react.react.React.InputHTMLAttributes<any>[P]} */ trait InputProps[TSuggestion]
