...
import typings.reactNativePaper.anon.themeDeepPartialThemeundeAccessibilityElementsHidden
...
object helperTextMod extends Shortcut {
  ...
  val default: ComponentType[PickOmitOmitPickPickAnima] & (NonReactStatics[
    (ComponentType[
      (Omit[
        Omit[
          PickPickAnimatedPropsText | themeDeepPartialThemeundeAccessibilityElementsHidden, 
          padding
        ], 
        `type`
      ]) & Type
  ...
  /* This means you don't have to write `default`, but can instead just say `helperTextMod.foo` */
