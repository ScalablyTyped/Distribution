package typings.reactMdAppBar

import typings.reactMdAppBar.appBarMod.AppBarHeight
import typings.reactMdAppBar.appBarMod.AppBarPosition
import typings.reactMdAppBar.appBarMod.AppBarTheme
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object reactMdAppBarStrings {
  
  @js.native
  sealed trait default
    extends StObject
       with AppBarTheme
  inline def default: default = "default".asInstanceOf[default]
  
  @js.native
  sealed trait bottom
    extends StObject
       with AppBarPosition
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait clear
    extends StObject
       with AppBarTheme
  inline def clear: clear = "clear".asInstanceOf[clear]
  
  @js.native
  sealed trait dense
    extends StObject
       with AppBarHeight
  inline def dense: dense = "dense".asInstanceOf[dense]
  
  @js.native
  sealed trait none
    extends StObject
       with AppBarHeight
  inline def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait normal
    extends StObject
       with AppBarHeight
  inline def normal: normal = "normal".asInstanceOf[normal]
  
  @js.native
  sealed trait primary
    extends StObject
       with AppBarTheme
  inline def primary: primary = "primary".asInstanceOf[primary]
  
  @js.native
  sealed trait prominent
    extends StObject
       with AppBarHeight
  inline def prominent: prominent = "prominent".asInstanceOf[prominent]
  
  @js.native
  sealed trait `prominent-dense`
    extends StObject
       with AppBarHeight
  inline def `prominent-dense`: `prominent-dense` = "prominent-dense".asInstanceOf[`prominent-dense`]
  
  @js.native
  sealed trait secondary
    extends StObject
       with AppBarTheme
  inline def secondary: secondary = "secondary".asInstanceOf[secondary]
  
  @js.native
  sealed trait top
    extends StObject
       with AppBarPosition
  inline def top: top = "top".asInstanceOf[top]
}
