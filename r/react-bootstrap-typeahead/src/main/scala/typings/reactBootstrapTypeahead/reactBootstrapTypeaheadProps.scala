package typings.reactBootstrapTypeahead

import typings.reactBootstrapTypeahead.mod.HighligherProps
import typings.reactBootstrapTypeahead.mod.TypeaheadModel
import typings.reactBootstrapTypeahead.mod.TypeaheadMulitInputWithHocProps
import typings.reactBootstrapTypeahead.mod.TypeaheadSingleInputWithHocProps
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait reactBootstrapTypeaheadProps {
  @scala.inline
  def AsyncTypeaheadProps: typings.reactBootstrapTypeahead.mod.AsyncTypeaheadProps.type = typings.reactBootstrapTypeahead.mod.AsyncTypeaheadProps
  type AsyncTypeaheadProps[T /* <: TypeaheadModel */] = typings.reactBootstrapTypeahead.mod.AsyncTypeaheadProps[T]
  @scala.inline
  def AutosizeInputProps: typings.reactBootstrapTypeahead.mod.AutosizeInputProps.type = typings.reactBootstrapTypeahead.mod.AutosizeInputProps
  type AutosizeInputProps = typings.reactBootstrapTypeahead.mod.AutosizeInputProps
  @scala.inline
  def BaseMenuItemProps: typings.reactBootstrapTypeahead.mod.BaseMenuItemProps.type = typings.reactBootstrapTypeahead.mod.BaseMenuItemProps
  type BaseMenuItemProps = typings.reactBootstrapTypeahead.mod.BaseMenuItemProps
  @scala.inline
  def ClearButtonProps: typings.reactBootstrapTypeahead.mod.ClearButtonProps.type = typings.reactBootstrapTypeahead.mod.ClearButtonProps
  type ClearButtonProps = typings.reactBootstrapTypeahead.mod.ClearButtonProps
  @scala.inline
  def HighlighterProps: HighligherProps.type = typings.reactBootstrapTypeahead.mod.HighligherProps
  type HighlighterProps = HighligherProps
  @scala.inline
  def LoaderProps: typings.reactBootstrapTypeahead.mod.LoaderProps.type = typings.reactBootstrapTypeahead.mod.LoaderProps
  type LoaderProps = typings.reactBootstrapTypeahead.mod.LoaderProps
  @scala.inline
  def MenuProps: typings.reactBootstrapTypeahead.mod.MenuProps.type = typings.reactBootstrapTypeahead.mod.MenuProps
  type MenuProps = typings.reactBootstrapTypeahead.mod.MenuProps
  type MenuHeaderProps = typings.reactBootstrapTypeahead.mod.MenuHeaderProps
  @scala.inline
  def MenuItemProps: typings.reactBootstrapTypeahead.mod.MenuItemProps.type = typings.reactBootstrapTypeahead.mod.MenuItemProps
  type MenuItemProps[T /* <: TypeaheadModel */] = typings.reactBootstrapTypeahead.mod.MenuItemProps[T]
  @scala.inline
  def OverlayProps: typings.reactBootstrapTypeahead.mod.OverlayProps.type = typings.reactBootstrapTypeahead.mod.OverlayProps
  type OverlayProps = typings.reactBootstrapTypeahead.mod.OverlayProps
  @scala.inline
  def TokenProps: typings.reactBootstrapTypeahead.mod.TokenProps.type = typings.reactBootstrapTypeahead.mod.TokenProps
  type TokenProps = typings.reactBootstrapTypeahead.mod.TokenProps
  @scala.inline
  def TypeaheadProps: typings.reactBootstrapTypeahead.mod.TypeaheadProps.type = typings.reactBootstrapTypeahead.mod.TypeaheadProps
  type TypeaheadProps[T /* <: TypeaheadModel */] = typings.reactBootstrapTypeahead.mod.TypeaheadProps[T]
  @scala.inline
  def TypeaheadInputMultiProps: TypeaheadMulitInputWithHocProps.type = typings.reactBootstrapTypeahead.mod.TypeaheadMulitInputWithHocProps
  type TypeaheadInputMultiProps[T /* <: TypeaheadModel */] = TypeaheadMulitInputWithHocProps[T]
  @scala.inline
  def TypeaheadInputSingleProps: TypeaheadSingleInputWithHocProps.type = typings.reactBootstrapTypeahead.mod.TypeaheadSingleInputWithHocProps
  type TypeaheadInputSingleProps[T /* <: TypeaheadModel */] = TypeaheadSingleInputWithHocProps[T]
  @scala.inline
  def TypeaheadMenuProps: typings.reactBootstrapTypeahead.mod.TypeaheadMenuProps.type = typings.reactBootstrapTypeahead.mod.TypeaheadMenuProps
  type TypeaheadMenuProps[T /* <: TypeaheadModel */] = typings.reactBootstrapTypeahead.mod.TypeaheadMenuProps[T]
}

