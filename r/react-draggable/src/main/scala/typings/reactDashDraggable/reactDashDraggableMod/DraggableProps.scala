...
object DraggableProps {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(allowAnyClick = allowAnyClick, axis = axis.asInstanceOf[js.Any], bounds = bounds.asInstanceOf[js.Any], cancel = cancel, defaultClassName = defaultClassName, defaultClassNameDragged = defaultClassNameDragged, defaultClassNameDragging = defaultClassNameDragging, defaultPosition = defaultPosition, disabled = disabled, enableUserSelectHack = enableUserSelectHack, grid = grid, handle = handle, offsetParent = offsetParent, onDrag = js.Any.fromFunction2(onDrag), onMouseDown = js.Any.fromFunction1(onMouseDown), onStart = js.Any.fromFunction2(onStart), onStop = js.Any.fromFunction2(onStop), position = position, positionOffset = positionOffset, scale = scale)
