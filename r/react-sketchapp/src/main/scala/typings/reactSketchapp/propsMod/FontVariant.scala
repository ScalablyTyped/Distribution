package typings.reactSketchapp.propsMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.reactSketchapp.reactSketchappStrings.normal
  - typings.reactSketchapp.reactSketchappStrings.`small-caps`
*/
trait FontVariant extends StObject
object FontVariant {
  
  inline def normal: typings.reactSketchapp.reactSketchappStrings.normal = "normal".asInstanceOf[typings.reactSketchapp.reactSketchappStrings.normal]
  
  inline def `small-caps`: typings.reactSketchapp.reactSketchappStrings.`small-caps` = "small-caps".asInstanceOf[typings.reactSketchapp.reactSketchappStrings.`small-caps`]
}
