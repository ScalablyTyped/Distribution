...
/* Inlined recharts.recharts.EventAttributes & std.Partial<recharts.recharts.PresentationAttributes<number | string, number | string>> & {  x1  :number,   y1  :number,   x2  :number,   y2  :number} */
trait EventAttributesPartialPreAlignmentBaseline extends js.Object {
...
object EventAttributesPartialPreAlignmentBaseline {
  @scala.inline
  def apply(
    x1: Double,
    x2: Double,
  ...
  ): EventAttributesPartialPreAlignmentBaseline = {
