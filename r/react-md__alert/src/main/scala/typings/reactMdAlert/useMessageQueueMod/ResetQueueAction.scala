package typings.reactMdAlert.useMessageQueueMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait ResetQueueAction
  extends MessageActions[js.Any] {
  var `type`: /* "RESET_QUEUE" */ String = js.native
}

object ResetQueueAction {
  @scala.inline
  def apply(`type`: /* "RESET_QUEUE" */ String): ResetQueueAction = {
    val __obj = js.Dynamic.literal()
    __obj.updateDynamic("type")(`type`.asInstanceOf[js.Any])
    __obj.asInstanceOf[ResetQueueAction]
  }
  @scala.inline
  implicit class ResetQueueActionOps[Self <: ResetQueueAction] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def set(key: String, value: js.Any): Self = {
        x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
        x
    }
    @scala.inline
    def setType(value: /* "RESET_QUEUE" */ String): Self = this.set("type", value.asInstanceOf[js.Any])
  }
  
}

