...
/* Inlined react-native.react-native.PlatformConstants & {  Version :number,   Release :string,   Serial :string,   Fingerprint :string,   Model :string,   Brand :string,   Manufacturer :string,   ServerHost :string | undefined,   uiMode :'car' | 'desk' | 'normal' | 'tv' | 'watch' | 'unknown'} */
