...
object QuadReduceIteratee {
  @scala.inline
  def apply[A, /* <: typings.rdfJs.mod.BaseQuad */ Q](run: (A, Q, Dataset[Q, Q]) => A): QuadReduceIteratee[A, Q] = {
    val __obj = js.Dynamic.literal(run = js.Any.fromFunction3(run))
    __obj.asInstanceOf[QuadReduceIteratee[A, Q]]
  }
  @scala.inline
  implicit class QuadReduceIterateeOps[Self <: QuadReduceIteratee[_, _], A, /* <: typings.rdfJs.mod.BaseQuad */ Q] (val x: Self with (QuadReduceIteratee[A, Q])) extends AnyVal {
    @scala.inline
