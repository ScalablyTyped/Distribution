package typings.reactDashNativeDashFbsdk

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait reactDashNativeDashFbsdkProps {
  @scala.inline
  def LikeViewProps: typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LikeViewProps.type = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LikeViewProps
  type LikeViewProps = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LikeViewProps
  @scala.inline
  def LoginButtonProps: typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LoginButtonProps.type = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LoginButtonProps
  type LoginButtonProps = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.LoginButtonProps
  @scala.inline
  def SendButtonProps: typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.SendButtonProps.type = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.SendButtonProps
  type SendButtonProps = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.SendButtonProps
  @scala.inline
  def ShareButtonProps: typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.ShareButtonProps.type = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.ShareButtonProps
  type ShareButtonProps = typings.reactDashNativeDashFbsdk.reactDashNativeDashFbsdkMod.ShareButtonProps
}

