organization := "org.scalablytyped"
name := "react-plotly_dot_js"
version := "2.2-dt-20181012Z-aebff4"
scalaVersion := "2.12.8"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "2.0.0",
  "org.scalablytyped" %%% "csstype" % "2.6.5-8a2cd0",
  "org.scalablytyped" %%% "d3-array" % "2.0-dt-20190221Z-2c798c",
  "org.scalablytyped" %%% "d3-axis" % "1.0-dt-20190212Z-3eef22",
  "org.scalablytyped" %%% "d3-brush" % "1.0-dt-20190212Z-5a4b25",
  "org.scalablytyped" %%% "d3-chord" % "1.0-dt-20190212Z-fd43ab",
  "org.scalablytyped" %%% "d3-collection" % "1.0-dt-20190212Z-286cf1",
  "org.scalablytyped" %%% "d3-color" % "1.2-dt-20190212Z-beed98",
  "org.scalablytyped" %%% "d3-contour" % "1.3-dt-20190205Z-3bb8a1",
  "org.scalablytyped" %%% "d3-dispatch" % "1.0-dt-20190212Z-d0968f",
  "org.scalablytyped" %%% "d3-drag" % "1.2-dt-20190212Z-f4b422",
  "org.scalablytyped" %%% "d3-dsv" % "1.0-dt-20190326Z-454688",
  "org.scalablytyped" %%% "d3-ease" % "1.0-dt-20190212Z-56cdfd",
  "org.scalablytyped" %%% "d3-force" % "1.2-dt-20190212Z-93a33e",
  "org.scalablytyped" %%% "d3-format" % "1.3-dt-20190212Z-9467d1",
  "org.scalablytyped" %%% "d3-geo" % "1.11-dt-20190212Z-14f8ed",
  "org.scalablytyped" %%% "d3-hierarchy" % "1.1-dt-20190212Z-1ff4bc",
  "org.scalablytyped" %%% "d3-interpolate" % "1.3-dt-20190212Z-031a0d",
  "org.scalablytyped" %%% "d3-path" % "1.0-dt-20190212Z-646c38",
  "org.scalablytyped" %%% "d3-quadtree" % "1.0-dt-20190212Z-767b25",
  "org.scalablytyped" %%% "d3-random" % "1.1-dt-20190212Z-dc6420",
  "org.scalablytyped" %%% "d3-scale" % "2.1-dt-20190212Z-6de445",
  "org.scalablytyped" %%% "d3-selection" % "1.4-dt-20190212Z-8a2abc",
  "org.scalablytyped" %%% "d3-shape" % "1.3-dt-20190212Z-721b66",
  "org.scalablytyped" %%% "d3-time" % "1.0-dt-20190212Z-63d757",
  "org.scalablytyped" %%% "d3-time-format" % "2.1-dt-20190212Z-0d2021",
  "org.scalablytyped" %%% "d3-timer" % "1.0-dt-20190212Z-3085c7",
  "org.scalablytyped" %%% "d3-transition" % "1.1-dt-20190212Z-71d1d1",
  "org.scalablytyped" %%% "d3-voronoi" % "1.1-dt-20190212Z-2a198c",
  "org.scalablytyped" %%% "d3-zoom" % "1.7-dt-20190212Z-cab052",
  "org.scalablytyped" %%% "geojson" % "7946.0-dt-20190319Z-854a0c",
  "org.scalablytyped" %%% "plotly_dot_js" % "1.44-dt-20190528Z-7c4d1a",
  "org.scalablytyped" %%% "prop-types" % "15.7-dt-20190417Z-c843c9",
  "org.scalablytyped" %%% "react" % "16.8-dt-20190528Z-1fde1d",
  "org.scalablytyped" %%% "std" % "3.5-f010ac")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
resolvers += Resolver.bintrayRepo("oyvindberg", "ScalablyTyped")
        