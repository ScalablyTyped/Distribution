...
trait TypeofGETRANGE extends StObject {
  
  val FIRST_KEY_INDEX: /* 1 */ Double
  
  val IS_READ_ONLY: /* true */ Boolean
  
  def transformArguments(key: RedisCommandArgument, start: Double, end: Double): RedisCommandArguments
  
  def transformReply(): RedisCommandArgument
}
object TypeofGETRANGE {
  ...
  inline def apply(
    ...
    __obj.asInstanceOf[TypeofGETRANGE]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TypeofGETRANGE] (val x: Self) extends AnyVal {
