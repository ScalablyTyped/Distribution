...
trait TypeofLREM extends StObject {
  
  val FIRST_KEY_INDEX: /* 1 */ Double
  
  def transformArguments(key: RedisCommandArgument, count: Double, element: RedisCommandArgument): RedisCommandArguments
...
object TypeofLREM {
  ...
  inline def apply(
    ...
    __obj.asInstanceOf[TypeofLREM]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TypeofLREM] (val x: Self) extends AnyVal {
