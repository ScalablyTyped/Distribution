...
trait TypeofimportedINCRBY extends StObject {
  
  val FIRST_KEY_INDEX: /* 1 */ Double
  
  def transformArguments(key: RedisCommandArgument, increment: Double): RedisCommandArguments
  
  def transformReply(): Double
}
object TypeofimportedINCRBY {
  
  inline def apply(
    FIRST_KEY_INDEX: /* 1 */ Double,
    transformArguments: (RedisCommandArgument, Double) => RedisCommandArguments,
    transformReply: () => Double
  ): TypeofimportedINCRBY = {
    ...
    __obj.asInstanceOf[TypeofimportedINCRBY]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TypeofimportedINCRBY] (val x: Self) extends AnyVal {
