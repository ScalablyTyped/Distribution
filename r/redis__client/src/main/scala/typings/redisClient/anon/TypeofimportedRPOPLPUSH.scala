...
trait TypeofimportedRPOPLPUSH extends StObject {
  
  val FIRST_KEY_INDEX: /* 1 */ Double
  
  def transformArguments(source: RedisCommandArgument, destination: RedisCommandArgument): RedisCommandArguments
  
  def transformReply(): RedisCommandArgument | Null
}
object TypeofimportedRPOPLPUSH {
  ...
  inline def apply(
    ...
    __obj.asInstanceOf[TypeofimportedRPOPLPUSH]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: TypeofimportedRPOPLPUSH] (val x: Self) extends AnyVal {
