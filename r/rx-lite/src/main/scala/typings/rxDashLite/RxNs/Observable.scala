...
import typings.rxDashLite.Anon_Reason
import typings.rxDashLite.Anon_ReasonRejectPromise
...
trait Observable[T] extends IObservable[T] {
  ...
  def toPromise(promiseCtor: Anon_ReasonRejectPromise[T]): IPromise[T] = js.native
   // alias for where
  /**
    * Converts an existing observable sequence to an ES6 Compatible Promise
    * @example
    ...
    * @returns An ES6 compatible promise with the last value from the observable sequence.
    */
  def toPromise[TPromise /* <: IPromise[T] */](promiseCtor: Anon_Reason[T, TPromise]): TPromise = js.native
