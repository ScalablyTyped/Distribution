package typings.reactDashTagcloud

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait reactDashTagcloudProps {
  @scala.inline
  def ReactTagCloudTagCloudProps: typings.reactDashTagcloud.ReactTagCloud.TagCloudProps.type = typings.reactDashTagcloud.ReactTagCloud.TagCloudProps
  type ReactTagCloudTagCloudProps = typings.reactDashTagcloud.ReactTagCloud.TagCloudProps
  @scala.inline
  def TagCloudProps: typings.reactDashTagcloud.ReactTagCloud.TagCloudProps.type = typings.reactDashTagcloud.ReactTagCloud.TagCloudProps
  type TagCloudProps = typings.reactDashTagcloud.ReactTagCloud.TagCloudProps
}

