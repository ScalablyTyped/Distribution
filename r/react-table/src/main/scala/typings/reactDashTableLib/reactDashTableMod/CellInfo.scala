...
- Dropped {[ P in 'resized' ]: react-table.react-table.ControlledStateOverrideProps<D>[P]} */ trait CellInfo[D] extends RowInfo[D] {
...
object CellInfo {
  ...
  def apply[D](
    ...
    val __obj = js.Dynamic.literal(aggregated = aggregated, classes = classes, column = column, columnProps = columnProps, expander = expander, groupedByPivot = groupedByPivot, index = index, isExpanded = isExpanded, level = level, maxWidth = maxWidth, nestingPath = nestingPath, original = original.asInstanceOf[js.Any], page = page, pageSize = pageSize, pivoted = pivoted, row = row, rowValues = rowValues, show = show, styles = styles, subRows = subRows, tdProps = tdProps, value = value.asInstanceOf[js.Any], viewIndex = viewIndex, width = width)
  
    __obj.asInstanceOf[CellInfo[D]]
  }
}
