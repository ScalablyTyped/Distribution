package typings
package reduxDashPersistLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object reduxDashPersistLibStrings {
  @js.native
  sealed trait blacklist extends js.Object
  
  @js.native
  sealed trait `persist/FLUSH` extends js.Object
  
  @js.native
  sealed trait `persist/PAUSE` extends js.Object
  
  @js.native
  sealed trait `persist/PERSIST` extends js.Object
  
  @js.native
  sealed trait `persist/PURGE` extends js.Object
  
  @js.native
  sealed trait `persist/REGISTER` extends js.Object
  
  @js.native
  sealed trait `persist/REHYDRATE` extends js.Object
  
  @js.native
  sealed trait `persist:` extends js.Object
  
  @js.native
  sealed trait `redux-persist/es/REGISTER` extends js.Object
  
  @js.native
  sealed trait `redux-persist/es/REHYDRATE` extends js.Object
  
  @js.native
  sealed trait whitelist extends js.Object
  
  @scala.inline
  def blacklist: blacklist = "blacklist".asInstanceOf[blacklist]
  @scala.inline
  def `persist/FLUSH`: `persist/FLUSH` = "persist/FLUSH".asInstanceOf[`persist/FLUSH`]
  @scala.inline
  def `persist/PAUSE`: `persist/PAUSE` = "persist/PAUSE".asInstanceOf[`persist/PAUSE`]
  @scala.inline
  def `persist/PERSIST`: `persist/PERSIST` = "persist/PERSIST".asInstanceOf[`persist/PERSIST`]
  @scala.inline
  def `persist/PURGE`: `persist/PURGE` = "persist/PURGE".asInstanceOf[`persist/PURGE`]
  @scala.inline
  def `persist/REGISTER`: `persist/REGISTER` = "persist/REGISTER".asInstanceOf[`persist/REGISTER`]
  @scala.inline
  def `persist/REHYDRATE`: `persist/REHYDRATE` = "persist/REHYDRATE".asInstanceOf[`persist/REHYDRATE`]
  @scala.inline
  def `persist:`: `persist:` = "persist:".asInstanceOf[`persist:`]
  @scala.inline
  def `redux-persist/es/REGISTER`: `redux-persist/es/REGISTER` = "redux-persist/es/REGISTER".asInstanceOf[`redux-persist/es/REGISTER`]
  @scala.inline
  def `redux-persist/es/REHYDRATE`: `redux-persist/es/REHYDRATE` = "redux-persist/es/REHYDRATE".asInstanceOf[`redux-persist/es/REHYDRATE`]
  @scala.inline
  def whitelist: whitelist = "whitelist".asInstanceOf[whitelist]
}

