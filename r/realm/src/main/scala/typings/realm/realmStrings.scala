package typings.realm

import typings.realm.Realm.App.Sync.LogLevel
import typings.realm.Realm.OpenRealmBehaviorType
import typings.realm.Realm.OpenRealmTimeOutBehavior
import typings.realm.Realm.ProgressDirection
import typings.realm.Realm.ProgressMode
import typings.realm.Realm.Services.MongoDB.OperationType
import typings.realm.Realm.SessionStopPolicy
import typings.realm.Realm._PropertyType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object realmStrings {
  
  @js.native
  sealed trait RealmOptionalParMappedModel extends StObject
  inline def RealmOptionalParMappedModel: RealmOptionalParMappedModel = "RealmOptionalParMappedModel".asInstanceOf[RealmOptionalParMappedModel]
  
  @js.native
  sealed trait _id extends StObject
  inline def _id: _id = "_id".asInstanceOf[_id]
  
  @js.native
  sealed trait _objectId extends StObject
  inline def _objectId: _objectId = "_objectId".asInstanceOf[_objectId]
  
  @js.native
  sealed trait active extends StObject
  inline def active: active = "active".asInstanceOf[active]
  
  @js.native
  sealed trait addListener extends StObject
  inline def addListener: addListener = "addListener".asInstanceOf[addListener]
  
  @js.native
  sealed trait `after-upload`
    extends StObject
       with SessionStopPolicy
  inline def `after-upload`: `after-upload` = "after-upload".asInstanceOf[`after-upload`]
  
  @js.native
  sealed trait all
    extends StObject
       with LogLevel
  inline def all: all = "all".asInstanceOf[all]
  
  @js.native
  sealed trait bool
    extends StObject
       with _PropertyType
  inline def bool: bool = "bool".asInstanceOf[bool]
  
  @js.native
  sealed trait change extends StObject
  inline def change: change = "change".asInstanceOf[change]
  
  @js.native
  sealed trait data
    extends StObject
       with _PropertyType
  inline def data: data = "data".asInstanceOf[data]
  
  @js.native
  sealed trait date
    extends StObject
       with _PropertyType
  inline def date: date = "date".asInstanceOf[date]
  
  @js.native
  sealed trait debug
    extends StObject
       with LogLevel
  inline def debug: debug = "debug".asInstanceOf[debug]
  
  @js.native
  sealed trait decimal128
    extends StObject
       with _PropertyType
  inline def decimal128: decimal128 = "decimal128".asInstanceOf[decimal128]
  
  @js.native
  sealed trait delete
    extends StObject
       with OperationType
  inline def delete: delete = "delete".asInstanceOf[delete]
  
  @js.native
  sealed trait detail
    extends StObject
       with LogLevel
  inline def detail: detail = "detail".asInstanceOf[detail]
  
  @js.native
  sealed trait double
    extends StObject
       with _PropertyType
  inline def double: double = "double".asInstanceOf[double]
  
  @js.native
  sealed trait download
    extends StObject
       with ProgressDirection
  inline def download: download = "download".asInstanceOf[download]
  
  @js.native
  sealed trait downloadBeforeOpen
    extends StObject
       with OpenRealmBehaviorType
  inline def downloadBeforeOpen: downloadBeforeOpen = "downloadBeforeOpen".asInstanceOf[downloadBeforeOpen]
  
  @js.native
  sealed trait drop
    extends StObject
       with OperationType
  inline def drop: drop = "drop".asInstanceOf[drop]
  
  @js.native
  sealed trait dropDatabase
    extends StObject
       with OperationType
  inline def dropDatabase: dropDatabase = "dropDatabase".asInstanceOf[dropDatabase]
  
  @js.native
  sealed trait entries extends StObject
  inline def entries: entries = "entries".asInstanceOf[entries]
  
  @js.native
  sealed trait error
    extends StObject
       with LogLevel
  inline def error: error = "error".asInstanceOf[error]
  
  @js.native
  sealed trait fatal
    extends StObject
       with LogLevel
  inline def fatal: fatal = "fatal".asInstanceOf[fatal]
  
  @js.native
  sealed trait float
    extends StObject
       with _PropertyType
  inline def float: float = "float".asInstanceOf[float]
  
  @js.native
  sealed trait forCurrentlyOutstandingWork
    extends StObject
       with ProgressMode
  inline def forCurrentlyOutstandingWork: forCurrentlyOutstandingWork = "forCurrentlyOutstandingWork".asInstanceOf[forCurrentlyOutstandingWork]
  
  @js.native
  sealed trait immediately
    extends StObject
       with SessionStopPolicy
  inline def immediately: immediately = "immediately".asInstanceOf[immediately]
  
  @js.native
  sealed trait inactive extends StObject
  inline def inactive: inactive = "inactive".asInstanceOf[inactive]
  
  @js.native
  sealed trait info
    extends StObject
       with LogLevel
  inline def info: info = "info".asInstanceOf[info]
  
  @js.native
  sealed trait insert
    extends StObject
       with OperationType
  inline def insert: insert = "insert".asInstanceOf[insert]
  
  @js.native
  sealed trait int
    extends StObject
       with _PropertyType
  inline def int: int = "int".asInstanceOf[int]
  
  @js.native
  sealed trait invalid extends StObject
  inline def invalid: invalid = "invalid".asInstanceOf[invalid]
  
  @js.native
  sealed trait invalidate
    extends StObject
       with OperationType
  inline def invalidate: invalidate = "invalidate".asInstanceOf[invalidate]
  
  @js.native
  sealed trait isValid extends StObject
  inline def isValid: isValid = "isValid".asInstanceOf[isValid]
  
  @js.native
  sealed trait keys extends StObject
  inline def keys: keys = "keys".asInstanceOf[keys]
  
  @js.native
  sealed trait linkingObjects
    extends StObject
       with _PropertyType
  inline def linkingObjects: linkingObjects = "linkingObjects".asInstanceOf[linkingObjects]
  
  @js.native
  sealed trait linkingObjectsCount extends StObject
  inline def linkingObjectsCount: linkingObjectsCount = "linkingObjectsCount".asInstanceOf[linkingObjectsCount]
  
  @js.native
  sealed trait list
    extends StObject
       with _PropertyType
  inline def list: list = "list".asInstanceOf[list]
  
  @js.native
  sealed trait never
    extends StObject
       with SessionStopPolicy
  inline def never: never = "never".asInstanceOf[never]
  
  @js.native
  sealed trait objectId
    extends StObject
       with _PropertyType
  inline def objectId: objectId = "objectId".asInstanceOf[objectId]
  
  @js.native
  sealed trait objectSchema extends StObject
  inline def objectSchema: objectSchema = "objectSchema".asInstanceOf[objectSchema]
  
  @js.native
  sealed trait off
    extends StObject
       with LogLevel
  inline def off: off = "off".asInstanceOf[off]
  
  @js.native
  sealed trait openImmediately
    extends StObject
       with OpenRealmBehaviorType
  inline def openImmediately: openImmediately = "openImmediately".asInstanceOf[openImmediately]
  
  @js.native
  sealed trait openLocalRealm
    extends StObject
       with OpenRealmTimeOutBehavior
  inline def openLocalRealm: openLocalRealm = "openLocalRealm".asInstanceOf[openLocalRealm]
  
  @js.native
  sealed trait removeAllListeners extends StObject
  inline def removeAllListeners: removeAllListeners = "removeAllListeners".asInstanceOf[removeAllListeners]
  
  @js.native
  sealed trait removeListener extends StObject
  inline def removeListener: removeListener = "removeListener".asInstanceOf[removeListener]
  
  @js.native
  sealed trait rename
    extends StObject
       with OperationType
  inline def rename: rename = "rename".asInstanceOf[rename]
  
  @js.native
  sealed trait replace
    extends StObject
       with OperationType
  inline def replace: replace = "replace".asInstanceOf[replace]
  
  @js.native
  sealed trait reportIndefinitely
    extends StObject
       with ProgressMode
  inline def reportIndefinitely: reportIndefinitely = "reportIndefinitely".asInstanceOf[reportIndefinitely]
  
  @js.native
  sealed trait schema extends StObject
  inline def schema: schema = "schema".asInstanceOf[schema]
  
  @js.native
  sealed trait string
    extends StObject
       with _PropertyType
  inline def string: string = "string".asInstanceOf[string]
  
  @js.native
  sealed trait throwException
    extends StObject
       with OpenRealmTimeOutBehavior
  inline def throwException: throwException = "throwException".asInstanceOf[throwException]
  
  @js.native
  sealed trait toJSON extends StObject
  inline def toJSON: toJSON = "toJSON".asInstanceOf[toJSON]
  
  @js.native
  sealed trait trace
    extends StObject
       with LogLevel
  inline def trace: trace = "trace".asInstanceOf[trace]
  
  @js.native
  sealed trait update
    extends StObject
       with OperationType
  inline def update: update = "update".asInstanceOf[update]
  
  @js.native
  sealed trait upload
    extends StObject
       with ProgressDirection
  inline def upload: upload = "upload".asInstanceOf[upload]
  
  @js.native
  sealed trait warn
    extends StObject
       with LogLevel
  inline def warn: warn = "warn".asInstanceOf[warn]
}
