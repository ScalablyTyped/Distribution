package typings
package atReachRouterLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object atReachRouterLibComponents {
  @scala.inline
  def Link[TState]: reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LinkProps[TState]] = js.constructorOf[atReachRouterLib.atReachRouterMod.Link[TState]].asInstanceOf[reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LinkProps[TState]]]
  @scala.inline
  def LinkProps: atReachRouterLib.atReachRouterMod.LinkProps.type = atReachRouterLib.atReachRouterMod.LinkProps
  type LinkProps[TState] = atReachRouterLib.atReachRouterMod.LinkProps[TState]
  @scala.inline
  def Location: reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LocationProps] = js.constructorOf[atReachRouterLib.atReachRouterMod.Location].asInstanceOf[reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LocationProps]]
  type LocationProps = atReachRouterLib.atReachRouterMod.LocationProps
  @scala.inline
  def LocationProvider: reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LocationProviderProps] = js.constructorOf[atReachRouterLib.atReachRouterMod.LocationProvider].asInstanceOf[reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.LocationProviderProps]]
  @scala.inline
  def LocationProviderProps: atReachRouterLib.atReachRouterMod.LocationProviderProps.type = atReachRouterLib.atReachRouterMod.LocationProviderProps
  type LocationProviderProps = atReachRouterLib.atReachRouterMod.LocationProviderProps
  @scala.inline
  def Match[TParams]: reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.MatchProps[TParams]] = js.constructorOf[atReachRouterLib.atReachRouterMod.Match[TParams]].asInstanceOf[reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.MatchProps[TParams]]]
  type MatchProps[TParams] = atReachRouterLib.atReachRouterMod.MatchProps[TParams]
  @scala.inline
  def Redirect[TState]: reactLib.reactMod.ComponentType[
    atReachRouterLib.atReachRouterMod.RouteComponentProps[atReachRouterLib.atReachRouterMod.RedirectProps[TState]]
  ] = js.constructorOf[atReachRouterLib.atReachRouterMod.Redirect[TState]].asInstanceOf[reactLib.reactMod.ComponentType[
  atReachRouterLib.atReachRouterMod.RouteComponentProps[atReachRouterLib.atReachRouterMod.RedirectProps[TState]]]]
  type RedirectProps[TParams] = atReachRouterLib.atReachRouterMod.RouteComponentProps[TParams]
  @scala.inline
  def Router: reactLib.reactMod.ComponentType[
    atReachRouterLib.atReachRouterMod.RouterProps with reactLib.reactMod.HTMLProps[stdLib.HTMLDivElement]
  ] = js.constructorOf[atReachRouterLib.atReachRouterMod.Router].asInstanceOf[reactLib.reactMod.ComponentType[
  atReachRouterLib.atReachRouterMod.RouterProps with reactLib.reactMod.HTMLProps[stdLib.HTMLDivElement]]]
  @scala.inline
  def ServerLocation: reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.ServerLocationProps] = js.constructorOf[atReachRouterLib.atReachRouterMod.ServerLocation].asInstanceOf[reactLib.reactMod.ComponentType[atReachRouterLib.atReachRouterMod.ServerLocationProps]]
  @scala.inline
  def ServerLocationProps: atReachRouterLib.atReachRouterMod.ServerLocationProps.type = atReachRouterLib.atReachRouterMod.ServerLocationProps
  type ServerLocationProps = atReachRouterLib.atReachRouterMod.ServerLocationProps
}

