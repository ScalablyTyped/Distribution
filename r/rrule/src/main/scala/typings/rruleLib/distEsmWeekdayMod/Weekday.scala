package typings
package rruleLib.distEsmWeekdayMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("rrule/dist/esm/weekday", "Weekday")
@js.native
class Weekday protected () extends js.Object {
  def this(weekday: scala.Double) = this()
  def this(weekday: scala.Double, n: scala.Double) = this()
  val n: js.UndefOr[scala.Double] = js.native
  val weekday: scala.Double = js.native
  def equals(other: Weekday): scala.Boolean = js.native
  def getJsWeekday(): scala.Double = js.native
  def nth(n: scala.Double): Weekday = js.native
}

