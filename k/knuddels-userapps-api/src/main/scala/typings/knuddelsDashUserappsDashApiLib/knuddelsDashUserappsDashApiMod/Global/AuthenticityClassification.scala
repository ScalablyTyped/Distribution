package typings
package knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/**
	 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html
	 */
@JSGlobal("AuthenticityClassification")
@js.native
class AuthenticityClassification () extends js.Object {
  /**
  		 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html#method_getDisplayText
  		 * @since AppServer 94663, ChatServer 94663
  		 */
  def getDisplayText(): java.lang.String = js.native
}

/* static members */
@JSGlobal("AuthenticityClassification")
@js.native
object AuthenticityClassification extends js.Object {
  /**
  		 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html#property_ServiceNotAvailable
  		 */
  val ServiceNotAvailable: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.AuthenticityClassification = js.native
  /**
  		 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html#property_Trusted
  		 */
  val Trusted: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.AuthenticityClassification = js.native
  /**
  		 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html#property_Unknown
  		 */
  val Unknown: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.AuthenticityClassification = js.native
  /**
  		 * @see https://developer.knuddels.de/docs/classes/AuthenticityClassification.html#property_VeryTrusted
  		 */
  val VeryTrusted: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.AuthenticityClassification = js.native
}

