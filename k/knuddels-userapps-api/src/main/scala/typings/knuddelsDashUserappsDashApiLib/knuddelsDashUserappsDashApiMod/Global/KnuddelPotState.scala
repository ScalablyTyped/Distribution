package typings
package knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/**
	 * @see https://developer.knuddels.de/docs/classes/KnuddelPotState.html
	 */
@JSGlobal("KnuddelPotState")
@js.native
class KnuddelPotState () extends js.Object

/* static members */
@JSGlobal("KnuddelPotState")
@js.native
object KnuddelPotState extends js.Object {
  /**
  		 * @see https://developer.knuddels.de/docs/classes/KnuddelPotState.html#property_Closed
  		 */
  val Closed: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.KnuddelPotState = js.native
  /**
  		 * @see https://developer.knuddels.de/docs/classes/KnuddelPotState.html#property_Open
  		 */
  val Open: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.KnuddelPotState = js.native
  /**
  		 * @see https://developer.knuddels.de/docs/classes/KnuddelPotState.html#property_Sealed
  		 */
  val Sealed: knuddelsDashUserappsDashApiLib.knuddelsDashUserappsDashApiMod.Global.KnuddelPotState = js.native
}

