package typings
package kendoDashUiLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_ActionSheet extends js.Object {
  var ActionSheet: Anon_ElementExtendFnOptionsProtoActionSheet
  var BackButton: Anon_ElementExtendFnOptionsProtoBackButton
  var Button: Anon_ElementExtendFnOptionsProtoButton
  var ButtonGroup: Anon_ElementExtendFnOptionsProtoButtonGroup
  var Collapsible: Anon_ElementExtendFnOptionsProtoCollapsible
  var DetailButton: Anon_ElementExtendFnOptionsProtoDetailButton
  var Drawer: Anon_ElementExtendFnOptionsProtoDrawer
  var Layout: Anon_ElementExtendFnOptionsProtoLayout
  var ListView: Anon_ElementExtendFnOptionsProtoListViewListViewOptions
  var Loader: Anon_ElementExtendFnOptionsProtoLoader
  var ModalView: Anon_ElementExtendFnOptionsProtoModalView
  var NavBar: Anon_ElementExtendFnOptionsProtoNavBar
  var Pane: Anon_ElementExtendFnOptionsProtoObjectPane
  var PopOver: Anon_ElementExtendFnOptionsProtoObjectPopOver
  var ScrollView: Anon_ElementExtendFnOptionsProtoObjectScrollViewScrollViewOptions
  var Scroller: Anon_ElementExtendFnOptionsProtoObjectScroller
  var SplitView: Anon_ElementExtendFnOptionsProtoObjectSplitView
  var Switch: Anon_ElementExtendFnOptionsProtoObjectSwitchSwitchOptions
  var TabStrip: Anon_ElementExtendFnOptionsProtoObjectTabStripTabStripOptions
  var View: Anon_ElementExtendFnOptionsProtoObjectView
  var Widget: org.scalablytyped.runtime.Instantiable0[kendoDashUiLib.kendoNs.mobileNs.uiNs.Widget]
}

