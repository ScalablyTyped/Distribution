package typings.kendoUi.global.kendo.mobile.ui

import typings.kendoUi.kendo.mobile.ui.PaneOptions
import typings.std.Element
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("kendo.mobile.ui.Pane")
@js.native
class Pane protected ()
  extends typings.kendoUi.kendo.mobile.ui.Pane {
  def this(element: Element) = this()
  def this(element: Element, options: PaneOptions) = this()
}

/* static members */
@JSGlobal("kendo.mobile.ui.Pane")
@js.native
object Pane extends js.Object {
  var fn: typings.kendoUi.kendo.mobile.ui.Pane = js.native
  def extend(proto: js.Object): typings.kendoUi.kendo.mobile.ui.Pane = js.native
}

