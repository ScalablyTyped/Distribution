...
object PivotGridMessagesFieldMenu {
  ...
  inline def apply(): PivotGridMessagesFieldMenu = {
  ...
  implicit open class MutableBuilder[Self <: PivotGridMessagesFieldMenu] (val x: Self) extends AnyVal {
    ...
    inline def setOperators(value: PivotGridMessagesFieldMenuOperators): Self = StObject.set(x, "operators", value.asInstanceOf[js.Any])
