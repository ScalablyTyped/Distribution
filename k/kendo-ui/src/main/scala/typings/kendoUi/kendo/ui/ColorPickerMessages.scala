package typings.kendoUi.kendo.ui

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait ColorPickerMessages extends StObject {
  
  @JSName("apply")
  var apply: js.UndefOr[String] = js.undefined
  
  var blue: js.UndefOr[String] = js.undefined
  
  var cancel: js.UndefOr[String] = js.undefined
  
  var clearColor: js.UndefOr[String] = js.undefined
  
  var contrastRatio: js.UndefOr[String] = js.undefined
  
  var fail: js.UndefOr[String] = js.undefined
  
  var gradient: js.UndefOr[String] = js.undefined
  
  var green: js.UndefOr[String] = js.undefined
  
  var hex: js.UndefOr[String] = js.undefined
  
  var palette: js.UndefOr[String] = js.undefined
  
  var pass: js.UndefOr[String] = js.undefined
  
  var previewInput: js.UndefOr[String] = js.undefined
  
  var red: js.UndefOr[String] = js.undefined
  
  var toggleFormat: js.UndefOr[String] = js.undefined
}
object ColorPickerMessages {
  
  inline def apply(): ColorPickerMessages = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[ColorPickerMessages]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: ColorPickerMessages] (val x: Self) extends AnyVal {
    
    inline def setApply(value: String): Self = StObject.set(x, "apply", value.asInstanceOf[js.Any])
    
    inline def setApplyUndefined: Self = StObject.set(x, "apply", js.undefined)
    
    inline def setBlue(value: String): Self = StObject.set(x, "blue", value.asInstanceOf[js.Any])
    
    inline def setBlueUndefined: Self = StObject.set(x, "blue", js.undefined)
    
    inline def setCancel(value: String): Self = StObject.set(x, "cancel", value.asInstanceOf[js.Any])
    
    inline def setCancelUndefined: Self = StObject.set(x, "cancel", js.undefined)
    
    inline def setClearColor(value: String): Self = StObject.set(x, "clearColor", value.asInstanceOf[js.Any])
    
    inline def setClearColorUndefined: Self = StObject.set(x, "clearColor", js.undefined)
    
    inline def setContrastRatio(value: String): Self = StObject.set(x, "contrastRatio", value.asInstanceOf[js.Any])
    
    inline def setContrastRatioUndefined: Self = StObject.set(x, "contrastRatio", js.undefined)
    
    inline def setFail(value: String): Self = StObject.set(x, "fail", value.asInstanceOf[js.Any])
    
    inline def setFailUndefined: Self = StObject.set(x, "fail", js.undefined)
    
    inline def setGradient(value: String): Self = StObject.set(x, "gradient", value.asInstanceOf[js.Any])
    
    inline def setGradientUndefined: Self = StObject.set(x, "gradient", js.undefined)
    
    inline def setGreen(value: String): Self = StObject.set(x, "green", value.asInstanceOf[js.Any])
    
    inline def setGreenUndefined: Self = StObject.set(x, "green", js.undefined)
    
    inline def setHex(value: String): Self = StObject.set(x, "hex", value.asInstanceOf[js.Any])
    
    inline def setHexUndefined: Self = StObject.set(x, "hex", js.undefined)
    
    inline def setPalette(value: String): Self = StObject.set(x, "palette", value.asInstanceOf[js.Any])
    
    inline def setPaletteUndefined: Self = StObject.set(x, "palette", js.undefined)
    
    inline def setPass(value: String): Self = StObject.set(x, "pass", value.asInstanceOf[js.Any])
    
    inline def setPassUndefined: Self = StObject.set(x, "pass", js.undefined)
    
    inline def setPreviewInput(value: String): Self = StObject.set(x, "previewInput", value.asInstanceOf[js.Any])
    
    inline def setPreviewInputUndefined: Self = StObject.set(x, "previewInput", js.undefined)
    
    inline def setRed(value: String): Self = StObject.set(x, "red", value.asInstanceOf[js.Any])
    
    inline def setRedUndefined: Self = StObject.set(x, "red", js.undefined)
    
    inline def setToggleFormat(value: String): Self = StObject.set(x, "toggleFormat", value.asInstanceOf[js.Any])
    
    inline def setToggleFormatUndefined: Self = StObject.set(x, "toggleFormat", js.undefined)
  }
}
