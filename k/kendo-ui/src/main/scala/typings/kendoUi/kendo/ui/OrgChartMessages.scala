package typings.kendoUi.kendo.ui

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait OrgChartMessages extends StObject {
  
  var cancel: js.UndefOr[String] = js.undefined
  
  var collapse: js.UndefOr[String] = js.undefined
  
  var create: js.UndefOr[String] = js.undefined
  
  var destroy: js.UndefOr[String] = js.undefined
  
  var destroyContent: js.UndefOr[String] = js.undefined
  
  var destroyTitle: js.UndefOr[String] = js.undefined
  
  var edit: js.UndefOr[String] = js.undefined
  
  var expand: js.UndefOr[String] = js.undefined
  
  var label: js.UndefOr[String] = js.undefined
  
  var menuLabel: js.UndefOr[String] = js.undefined
  
  var name: js.UndefOr[String] = js.undefined
  
  var none: js.UndefOr[String] = js.undefined
  
  var parent: js.UndefOr[String] = js.undefined
  
  var save: js.UndefOr[String] = js.undefined
  
  var title: js.UndefOr[String] = js.undefined
  
  var uploadAvatar: js.UndefOr[String] = js.undefined
}
object OrgChartMessages {
  
  inline def apply(): OrgChartMessages = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[OrgChartMessages]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: OrgChartMessages] (val x: Self) extends AnyVal {
    
    inline def setCancel(value: String): Self = StObject.set(x, "cancel", value.asInstanceOf[js.Any])
    
    inline def setCancelUndefined: Self = StObject.set(x, "cancel", js.undefined)
    
    inline def setCollapse(value: String): Self = StObject.set(x, "collapse", value.asInstanceOf[js.Any])
    
    inline def setCollapseUndefined: Self = StObject.set(x, "collapse", js.undefined)
    
    inline def setCreate(value: String): Self = StObject.set(x, "create", value.asInstanceOf[js.Any])
    
    inline def setCreateUndefined: Self = StObject.set(x, "create", js.undefined)
    
    inline def setDestroy(value: String): Self = StObject.set(x, "destroy", value.asInstanceOf[js.Any])
    
    inline def setDestroyContent(value: String): Self = StObject.set(x, "destroyContent", value.asInstanceOf[js.Any])
    
    inline def setDestroyContentUndefined: Self = StObject.set(x, "destroyContent", js.undefined)
    
    inline def setDestroyTitle(value: String): Self = StObject.set(x, "destroyTitle", value.asInstanceOf[js.Any])
    
    inline def setDestroyTitleUndefined: Self = StObject.set(x, "destroyTitle", js.undefined)
    
    inline def setDestroyUndefined: Self = StObject.set(x, "destroy", js.undefined)
    
    inline def setEdit(value: String): Self = StObject.set(x, "edit", value.asInstanceOf[js.Any])
    
    inline def setEditUndefined: Self = StObject.set(x, "edit", js.undefined)
    
    inline def setExpand(value: String): Self = StObject.set(x, "expand", value.asInstanceOf[js.Any])
    
    inline def setExpandUndefined: Self = StObject.set(x, "expand", js.undefined)
    
    inline def setLabel(value: String): Self = StObject.set(x, "label", value.asInstanceOf[js.Any])
    
    inline def setLabelUndefined: Self = StObject.set(x, "label", js.undefined)
    
    inline def setMenuLabel(value: String): Self = StObject.set(x, "menuLabel", value.asInstanceOf[js.Any])
    
    inline def setMenuLabelUndefined: Self = StObject.set(x, "menuLabel", js.undefined)
    
    inline def setName(value: String): Self = StObject.set(x, "name", value.asInstanceOf[js.Any])
    
    inline def setNameUndefined: Self = StObject.set(x, "name", js.undefined)
    
    inline def setNone(value: String): Self = StObject.set(x, "none", value.asInstanceOf[js.Any])
    
    inline def setNoneUndefined: Self = StObject.set(x, "none", js.undefined)
    
    inline def setParent(value: String): Self = StObject.set(x, "parent", value.asInstanceOf[js.Any])
    
    inline def setParentUndefined: Self = StObject.set(x, "parent", js.undefined)
    
    inline def setSave(value: String): Self = StObject.set(x, "save", value.asInstanceOf[js.Any])
    
    inline def setSaveUndefined: Self = StObject.set(x, "save", js.undefined)
    
    inline def setTitle(value: String): Self = StObject.set(x, "title", value.asInstanceOf[js.Any])
    
    inline def setTitleUndefined: Self = StObject.set(x, "title", js.undefined)
    
    inline def setUploadAvatar(value: String): Self = StObject.set(x, "uploadAvatar", value.asInstanceOf[js.Any])
    
    inline def setUploadAvatarUndefined: Self = StObject.set(x, "uploadAvatar", js.undefined)
  }
}
