package typings
package atPulumiAwsLib.getBillingServiceAccountMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/getBillingServiceAccount", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  def getBillingServiceAccount(): js.Promise[GetBillingServiceAccountResult] with GetBillingServiceAccountResult = js.native
  def getBillingServiceAccount(opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetBillingServiceAccountResult] with GetBillingServiceAccountResult = js.native
}

