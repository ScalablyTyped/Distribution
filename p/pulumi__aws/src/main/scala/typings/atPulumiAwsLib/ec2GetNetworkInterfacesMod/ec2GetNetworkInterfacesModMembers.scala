package typings
package atPulumiAwsLib.ec2GetNetworkInterfacesMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/ec2/getNetworkInterfaces", JSImport.Namespace)
@js.native
object ec2GetNetworkInterfacesModMembers extends js.Object {
  def getNetworkInterfaces(): js.Promise[GetNetworkInterfacesResult] = js.native
  def getNetworkInterfaces(args: GetNetworkInterfacesArgs): js.Promise[GetNetworkInterfacesResult] = js.native
  def getNetworkInterfaces(args: GetNetworkInterfacesArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetNetworkInterfacesResult] = js.native
}

