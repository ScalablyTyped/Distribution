package typings
package atPulumiAwsLib.iamGetPolicyDocumentMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/iam/getPolicyDocument", JSImport.Namespace)
@js.native
object iamGetPolicyDocumentModMembers extends js.Object {
  def getPolicyDocument(): js.Promise[GetPolicyDocumentResult] = js.native
  def getPolicyDocument(args: GetPolicyDocumentArgs): js.Promise[GetPolicyDocumentResult] = js.native
  def getPolicyDocument(args: GetPolicyDocumentArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetPolicyDocumentResult] = js.native
}

