package typings
package atPulumiAwsLib.efsGetFileSystemMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/efs/getFileSystem", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  def getFileSystem(): js.Promise[GetFileSystemResult] with GetFileSystemResult = js.native
  def getFileSystem(args: GetFileSystemArgs): js.Promise[GetFileSystemResult] with GetFileSystemResult = js.native
  def getFileSystem(args: GetFileSystemArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetFileSystemResult] with GetFileSystemResult = js.native
}

