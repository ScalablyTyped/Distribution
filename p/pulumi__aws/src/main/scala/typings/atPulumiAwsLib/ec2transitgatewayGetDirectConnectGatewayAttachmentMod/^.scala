package typings
package atPulumiAwsLib.ec2transitgatewayGetDirectConnectGatewayAttachmentMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/ec2transitgateway/getDirectConnectGatewayAttachment", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  def getDirectConnectGatewayAttachment(args: GetDirectConnectGatewayAttachmentArgs): js.Promise[GetDirectConnectGatewayAttachmentResult] = js.native
  def getDirectConnectGatewayAttachment(args: GetDirectConnectGatewayAttachmentArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetDirectConnectGatewayAttachmentResult] = js.native
}

