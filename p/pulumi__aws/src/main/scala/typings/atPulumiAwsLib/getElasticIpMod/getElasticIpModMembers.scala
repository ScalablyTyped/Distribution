package typings
package atPulumiAwsLib.getElasticIpMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/getElasticIp", JSImport.Namespace)
@js.native
object getElasticIpModMembers extends js.Object {
  def getElasticIp(): js.Promise[GetElasticIpResult] = js.native
  def getElasticIp(args: GetElasticIpArgs): js.Promise[GetElasticIpResult] = js.native
  def getElasticIp(args: GetElasticIpArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetElasticIpResult] = js.native
}

