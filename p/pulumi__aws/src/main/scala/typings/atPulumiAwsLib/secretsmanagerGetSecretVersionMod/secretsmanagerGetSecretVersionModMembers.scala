package typings
package atPulumiAwsLib.secretsmanagerGetSecretVersionMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/secretsmanager/getSecretVersion", JSImport.Namespace)
@js.native
object secretsmanagerGetSecretVersionModMembers extends js.Object {
  def getSecretVersion(args: GetSecretVersionArgs): js.Promise[GetSecretVersionResult] = js.native
  def getSecretVersion(args: GetSecretVersionArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetSecretVersionResult] = js.native
}

