package typings
package atPulumiAwsLib.ec2GetNetworkAclsMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@pulumi/aws/ec2/getNetworkAcls", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  def getNetworkAcls(): js.Promise[GetNetworkAclsResult] with GetNetworkAclsResult = js.native
  def getNetworkAcls(args: GetNetworkAclsArgs): js.Promise[GetNetworkAclsResult] with GetNetworkAclsResult = js.native
  def getNetworkAcls(args: GetNetworkAclsArgs, opts: atPulumiPulumiLib.invokeMod.InvokeOptions): js.Promise[GetNetworkAclsResult] with GetNetworkAclsResult = js.native
}

