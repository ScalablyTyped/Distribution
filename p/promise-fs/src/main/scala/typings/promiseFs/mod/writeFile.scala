...
object writeFile extends js.Object {
  def apply(path: Double, data: js.Any): js.Promise[Unit] = js.native
  def apply(path: Double, data: js.Any, callback: NoParamCallback): Unit = js.native
  def apply(path: Double, data: js.Any, options: WriteFileOptions): js.Promise[Unit] = js.native
  ...
  def apply(path: PathLike, data: js.Any, options: WriteFileOptions): js.Promise[Unit] = js.native
  /**
    * Asynchronously writes data to a file, replacing the file if it already exists.
    * @param path A path to a file. If a URL is provided, it must use the `file:` protocol.
    * URL support is _experimental_.
