...
object IDefaults {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(application_name = application_name, binary = binary, client_encoding = client_encoding, connect_timeout = connect_timeout, connectionString = connectionString, database = database, fallback_application_name = fallback_application_name, host = host, idleTimeoutMillis = idleTimeoutMillis, keepalives = keepalives, keepalives_idle = keepalives_idle, max = max, parseInputDatesAsUTC = parseInputDatesAsUTC, password = password.asInstanceOf[js.Any], port = port, query_timeout = query_timeout.asInstanceOf[js.Any], rows = rows, ssl = ssl.asInstanceOf[js.Any], statement_timeout = statement_timeout.asInstanceOf[js.Any], user = user)
