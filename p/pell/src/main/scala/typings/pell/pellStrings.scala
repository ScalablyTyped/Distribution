package typings.pell

import typings.pell.mod.pellBuiltinAction
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object pellStrings {
  
  @js.native
  sealed trait bold
    extends StObject
       with pellBuiltinAction
  inline def bold: bold = "bold".asInstanceOf[bold]
  
  @js.native
  sealed trait code
    extends StObject
       with pellBuiltinAction
  inline def code: code = "code".asInstanceOf[code]
  
  @js.native
  sealed trait heading1
    extends StObject
       with pellBuiltinAction
  inline def heading1: heading1 = "heading1".asInstanceOf[heading1]
  
  @js.native
  sealed trait heading2
    extends StObject
       with pellBuiltinAction
  inline def heading2: heading2 = "heading2".asInstanceOf[heading2]
  
  @js.native
  sealed trait image
    extends StObject
       with pellBuiltinAction
  inline def image: image = "image".asInstanceOf[image]
  
  @js.native
  sealed trait italic
    extends StObject
       with pellBuiltinAction
  inline def italic: italic = "italic".asInstanceOf[italic]
  
  @js.native
  sealed trait line
    extends StObject
       with pellBuiltinAction
  inline def line: line = "line".asInstanceOf[line]
  
  @js.native
  sealed trait link
    extends StObject
       with pellBuiltinAction
  inline def link: link = "link".asInstanceOf[link]
  
  @js.native
  sealed trait olist
    extends StObject
       with pellBuiltinAction
  inline def olist: olist = "olist".asInstanceOf[olist]
  
  @js.native
  sealed trait paragraph
    extends StObject
       with pellBuiltinAction
  inline def paragraph: paragraph = "paragraph".asInstanceOf[paragraph]
  
  @js.native
  sealed trait quote
    extends StObject
       with pellBuiltinAction
  inline def quote: quote = "quote".asInstanceOf[quote]
  
  @js.native
  sealed trait strikethrough
    extends StObject
       with pellBuiltinAction
  inline def strikethrough: strikethrough = "strikethrough".asInstanceOf[strikethrough]
  
  @js.native
  sealed trait ulist
    extends StObject
       with pellBuiltinAction
  inline def ulist: ulist = "ulist".asInstanceOf[ulist]
  
  @js.native
  sealed trait underline
    extends StObject
       with pellBuiltinAction
  inline def underline: underline = "underline".asInstanceOf[underline]
}
