...
trait Identifier
  ...
  @JSName("type")
  var type_Identifier: typings.postcssSelectorParser.postcssSelectorParserStrings.id = js.native
}
...
object Identifier {
  ...
  def apply(
    ...
    sourceIndex: Double,
    spaces: Spaces,
    `type`: typings.postcssSelectorParser.postcssSelectorParserStrings.id,
    value: String
  ): Identifier = {
  ...
  implicit class IdentifierOps[Self <: Identifier] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setType(value: typings.postcssSelectorParser.postcssSelectorParserStrings.id): Self = this.set("type", value.asInstanceOf[js.Any])
  }
