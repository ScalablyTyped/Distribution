...
object volumeAttachmentMod {
  ...
  object VolumeAttachmentArgs {
    ...
    @scala.inline
    def apply(spec: Input[typings.pulumiKubernetes.inputMod.storage.v1alpha1.VolumeAttachmentSpec]): VolumeAttachmentArgs = {
    ...
    implicit class VolumeAttachmentArgsMutableBuilder[Self <: VolumeAttachmentArgs] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setSpec(value: Input[typings.pulumiKubernetes.inputMod.storage.v1alpha1.VolumeAttachmentSpec]): Self = StObject.set(x, "spec", value.asInstanceOf[js.Any])
    }
  }
}
