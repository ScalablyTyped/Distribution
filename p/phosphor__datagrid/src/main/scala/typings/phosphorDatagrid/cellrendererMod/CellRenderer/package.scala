...
package object CellRenderer {
  /**
    ...
    */
  type ConfigFunc[T] = js.Function1[/* config */ typings.phosphorDatagrid.cellrendererMod.CellRenderer.ICellConfig, T]
  /**
    * A type alias for a cell renderer config option.
    *
    * A config option can be a static value or a config function.
    */
  type ConfigOption[T] = T | typings.phosphorDatagrid.cellrendererMod.CellRenderer.ConfigFunc[T]
}
