package typings.pgTypes.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Inlined {[ key in pg-types.pg-types.builtinsTypes ]: pg-types.pg-types.TypeId} */
@js.native
trait TypesBuiltins extends js.Object {
  var ABSTIME: TypeId = js.native
  var ACLITEM: TypeId = js.native
  var BIT: TypeId = js.native
  var BOOL: TypeId = js.native
  var BPCHAR: TypeId = js.native
  var BYTEA: TypeId = js.native
  var CHAR: TypeId = js.native
  var CID: TypeId = js.native
  var CIDR: TypeId = js.native
  var CIRCLE: TypeId = js.native
  var DATE: TypeId = js.native
  var FLOAT4: TypeId = js.native
  var FLOAT8: TypeId = js.native
  var GTSVECTOR: TypeId = js.native
  var INET: TypeId = js.native
  var INT2: TypeId = js.native
  var INT4: TypeId = js.native
  var INT8: TypeId = js.native
  var INTERVAL: TypeId = js.native
  var JSON: TypeId = js.native
  var JSONB: TypeId = js.native
  var MACADDR: TypeId = js.native
  var MACADDR8: TypeId = js.native
  var MONEY: TypeId = js.native
  var NUMERIC: TypeId = js.native
  var OID: TypeId = js.native
  var PATH: TypeId = js.native
  var PG_DEPENDENCIES: TypeId = js.native
  var PG_LSN: TypeId = js.native
  var PG_NDISTINCT: TypeId = js.native
  var PG_NODE_TREE: TypeId = js.native
  var POLYGON: TypeId = js.native
  var REFCURSOR: TypeId = js.native
  var REGCLASS: TypeId = js.native
  var REGCONFIG: TypeId = js.native
  var REGDICTIONARY: TypeId = js.native
  var REGNAMESPACE: TypeId = js.native
  var REGOPER: TypeId = js.native
  var REGOPERATOR: TypeId = js.native
  var REGPROC: TypeId = js.native
  var REGPROCEDURE: TypeId = js.native
  var REGROLE: TypeId = js.native
  var REGTYPE: TypeId = js.native
  var RELTIME: TypeId = js.native
  var SMGR: TypeId = js.native
  var TEXT: TypeId = js.native
  var TID: TypeId = js.native
  var TIME: TypeId = js.native
  var TIMESTAMP: TypeId = js.native
  var TIMESTAMPTZ: TypeId = js.native
  var TIMETZ: TypeId = js.native
  var TINTERVAL: TypeId = js.native
  var TSQUERY: TypeId = js.native
  var TSVECTOR: TypeId = js.native
  var TXID_SNAPSHOT: TypeId = js.native
  var UUID: TypeId = js.native
  var VARBIT: TypeId = js.native
  var VARCHAR: TypeId = js.native
  var XID: TypeId = js.native
  var XML: TypeId = js.native
}

object TypesBuiltins {
  @scala.inline
  def apply(
    ABSTIME: TypeId,
    ACLITEM: TypeId,
    BIT: TypeId,
    BOOL: TypeId,
    BPCHAR: TypeId,
    BYTEA: TypeId,
    CHAR: TypeId,
    CID: TypeId,
    CIDR: TypeId,
    CIRCLE: TypeId,
    DATE: TypeId,
    FLOAT4: TypeId,
    FLOAT8: TypeId,
    GTSVECTOR: TypeId,
    INET: TypeId,
    INT2: TypeId,
    INT4: TypeId,
    INT8: TypeId,
    INTERVAL: TypeId,
    JSON: TypeId,
    JSONB: TypeId,
    MACADDR: TypeId,
    MACADDR8: TypeId,
    MONEY: TypeId,
    NUMERIC: TypeId,
    OID: TypeId,
    PATH: TypeId,
    PG_DEPENDENCIES: TypeId,
    PG_LSN: TypeId,
    PG_NDISTINCT: TypeId,
    PG_NODE_TREE: TypeId,
    POLYGON: TypeId,
    REFCURSOR: TypeId,
    REGCLASS: TypeId,
    REGCONFIG: TypeId,
    REGDICTIONARY: TypeId,
    REGNAMESPACE: TypeId,
    REGOPER: TypeId,
    REGOPERATOR: TypeId,
    REGPROC: TypeId,
    REGPROCEDURE: TypeId,
    REGROLE: TypeId,
    REGTYPE: TypeId,
    RELTIME: TypeId,
    SMGR: TypeId,
    TEXT: TypeId,
    TID: TypeId,
    TIME: TypeId,
    TIMESTAMP: TypeId,
    TIMESTAMPTZ: TypeId,
    TIMETZ: TypeId,
    TINTERVAL: TypeId,
    TSQUERY: TypeId,
    TSVECTOR: TypeId,
    TXID_SNAPSHOT: TypeId,
    UUID: TypeId,
    VARBIT: TypeId,
    VARCHAR: TypeId,
    XID: TypeId,
    XML: TypeId
  ): TypesBuiltins = {
    val __obj = js.Dynamic.literal(ABSTIME = ABSTIME.asInstanceOf[js.Any], ACLITEM = ACLITEM.asInstanceOf[js.Any], BIT = BIT.asInstanceOf[js.Any], BOOL = BOOL.asInstanceOf[js.Any], BPCHAR = BPCHAR.asInstanceOf[js.Any], BYTEA = BYTEA.asInstanceOf[js.Any], CHAR = CHAR.asInstanceOf[js.Any], CID = CID.asInstanceOf[js.Any], CIDR = CIDR.asInstanceOf[js.Any], CIRCLE = CIRCLE.asInstanceOf[js.Any], DATE = DATE.asInstanceOf[js.Any], FLOAT4 = FLOAT4.asInstanceOf[js.Any], FLOAT8 = FLOAT8.asInstanceOf[js.Any], GTSVECTOR = GTSVECTOR.asInstanceOf[js.Any], INET = INET.asInstanceOf[js.Any], INT2 = INT2.asInstanceOf[js.Any], INT4 = INT4.asInstanceOf[js.Any], INT8 = INT8.asInstanceOf[js.Any], INTERVAL = INTERVAL.asInstanceOf[js.Any], JSON = JSON.asInstanceOf[js.Any], JSONB = JSONB.asInstanceOf[js.Any], MACADDR = MACADDR.asInstanceOf[js.Any], MACADDR8 = MACADDR8.asInstanceOf[js.Any], MONEY = MONEY.asInstanceOf[js.Any], NUMERIC = NUMERIC.asInstanceOf[js.Any], OID = OID.asInstanceOf[js.Any], PATH = PATH.asInstanceOf[js.Any], PG_DEPENDENCIES = PG_DEPENDENCIES.asInstanceOf[js.Any], PG_LSN = PG_LSN.asInstanceOf[js.Any], PG_NDISTINCT = PG_NDISTINCT.asInstanceOf[js.Any], PG_NODE_TREE = PG_NODE_TREE.asInstanceOf[js.Any], POLYGON = POLYGON.asInstanceOf[js.Any], REFCURSOR = REFCURSOR.asInstanceOf[js.Any], REGCLASS = REGCLASS.asInstanceOf[js.Any], REGCONFIG = REGCONFIG.asInstanceOf[js.Any], REGDICTIONARY = REGDICTIONARY.asInstanceOf[js.Any], REGNAMESPACE = REGNAMESPACE.asInstanceOf[js.Any], REGOPER = REGOPER.asInstanceOf[js.Any], REGOPERATOR = REGOPERATOR.asInstanceOf[js.Any], REGPROC = REGPROC.asInstanceOf[js.Any], REGPROCEDURE = REGPROCEDURE.asInstanceOf[js.Any], REGROLE = REGROLE.asInstanceOf[js.Any], REGTYPE = REGTYPE.asInstanceOf[js.Any], RELTIME = RELTIME.asInstanceOf[js.Any], SMGR = SMGR.asInstanceOf[js.Any], TEXT = TEXT.asInstanceOf[js.Any], TID = TID.asInstanceOf[js.Any], TIME = TIME.asInstanceOf[js.Any], TIMESTAMP = TIMESTAMP.asInstanceOf[js.Any], TIMESTAMPTZ = TIMESTAMPTZ.asInstanceOf[js.Any], TIMETZ = TIMETZ.asInstanceOf[js.Any], TINTERVAL = TINTERVAL.asInstanceOf[js.Any], TSQUERY = TSQUERY.asInstanceOf[js.Any], TSVECTOR = TSVECTOR.asInstanceOf[js.Any], TXID_SNAPSHOT = TXID_SNAPSHOT.asInstanceOf[js.Any], UUID = UUID.asInstanceOf[js.Any], VARBIT = VARBIT.asInstanceOf[js.Any], VARCHAR = VARCHAR.asInstanceOf[js.Any], XID = XID.asInstanceOf[js.Any], XML = XML.asInstanceOf[js.Any])
    __obj.asInstanceOf[TypesBuiltins]
  }
  @scala.inline
  implicit class TypesBuiltinsOps[Self <: TypesBuiltins] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def set(key: String, value: js.Any): Self = {
        x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
        x
    }
    @scala.inline
    def setABSTIME(value: TypeId): Self = this.set("ABSTIME", value.asInstanceOf[js.Any])
    @scala.inline
    def setACLITEM(value: TypeId): Self = this.set("ACLITEM", value.asInstanceOf[js.Any])
    @scala.inline
    def setBIT(value: TypeId): Self = this.set("BIT", value.asInstanceOf[js.Any])
    @scala.inline
    def setBOOL(value: TypeId): Self = this.set("BOOL", value.asInstanceOf[js.Any])
    @scala.inline
    def setBPCHAR(value: TypeId): Self = this.set("BPCHAR", value.asInstanceOf[js.Any])
    @scala.inline
    def setBYTEA(value: TypeId): Self = this.set("BYTEA", value.asInstanceOf[js.Any])
    @scala.inline
    def setCHAR(value: TypeId): Self = this.set("CHAR", value.asInstanceOf[js.Any])
    @scala.inline
    def setCID(value: TypeId): Self = this.set("CID", value.asInstanceOf[js.Any])
    @scala.inline
    def setCIDR(value: TypeId): Self = this.set("CIDR", value.asInstanceOf[js.Any])
    @scala.inline
    def setCIRCLE(value: TypeId): Self = this.set("CIRCLE", value.asInstanceOf[js.Any])
    @scala.inline
    def setDATE(value: TypeId): Self = this.set("DATE", value.asInstanceOf[js.Any])
    @scala.inline
    def setFLOAT4(value: TypeId): Self = this.set("FLOAT4", value.asInstanceOf[js.Any])
    @scala.inline
    def setFLOAT8(value: TypeId): Self = this.set("FLOAT8", value.asInstanceOf[js.Any])
    @scala.inline
    def setGTSVECTOR(value: TypeId): Self = this.set("GTSVECTOR", value.asInstanceOf[js.Any])
    @scala.inline
    def setINET(value: TypeId): Self = this.set("INET", value.asInstanceOf[js.Any])
    @scala.inline
    def setINT2(value: TypeId): Self = this.set("INT2", value.asInstanceOf[js.Any])
    @scala.inline
    def setINT4(value: TypeId): Self = this.set("INT4", value.asInstanceOf[js.Any])
    @scala.inline
    def setINT8(value: TypeId): Self = this.set("INT8", value.asInstanceOf[js.Any])
    @scala.inline
    def setINTERVAL(value: TypeId): Self = this.set("INTERVAL", value.asInstanceOf[js.Any])
    @scala.inline
    def setJSON(value: TypeId): Self = this.set("JSON", value.asInstanceOf[js.Any])
    @scala.inline
    def setJSONB(value: TypeId): Self = this.set("JSONB", value.asInstanceOf[js.Any])
    @scala.inline
    def setMACADDR(value: TypeId): Self = this.set("MACADDR", value.asInstanceOf[js.Any])
    @scala.inline
    def setMACADDR8(value: TypeId): Self = this.set("MACADDR8", value.asInstanceOf[js.Any])
    @scala.inline
    def setMONEY(value: TypeId): Self = this.set("MONEY", value.asInstanceOf[js.Any])
    @scala.inline
    def setNUMERIC(value: TypeId): Self = this.set("NUMERIC", value.asInstanceOf[js.Any])
    @scala.inline
    def setOID(value: TypeId): Self = this.set("OID", value.asInstanceOf[js.Any])
    @scala.inline
    def setPATH(value: TypeId): Self = this.set("PATH", value.asInstanceOf[js.Any])
    @scala.inline
    def setPG_DEPENDENCIES(value: TypeId): Self = this.set("PG_DEPENDENCIES", value.asInstanceOf[js.Any])
    @scala.inline
    def setPG_LSN(value: TypeId): Self = this.set("PG_LSN", value.asInstanceOf[js.Any])
    @scala.inline
    def setPG_NDISTINCT(value: TypeId): Self = this.set("PG_NDISTINCT", value.asInstanceOf[js.Any])
    @scala.inline
    def setPG_NODE_TREE(value: TypeId): Self = this.set("PG_NODE_TREE", value.asInstanceOf[js.Any])
    @scala.inline
    def setPOLYGON(value: TypeId): Self = this.set("POLYGON", value.asInstanceOf[js.Any])
    @scala.inline
    def setREFCURSOR(value: TypeId): Self = this.set("REFCURSOR", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGCLASS(value: TypeId): Self = this.set("REGCLASS", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGCONFIG(value: TypeId): Self = this.set("REGCONFIG", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGDICTIONARY(value: TypeId): Self = this.set("REGDICTIONARY", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGNAMESPACE(value: TypeId): Self = this.set("REGNAMESPACE", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGOPER(value: TypeId): Self = this.set("REGOPER", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGOPERATOR(value: TypeId): Self = this.set("REGOPERATOR", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGPROC(value: TypeId): Self = this.set("REGPROC", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGPROCEDURE(value: TypeId): Self = this.set("REGPROCEDURE", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGROLE(value: TypeId): Self = this.set("REGROLE", value.asInstanceOf[js.Any])
    @scala.inline
    def setREGTYPE(value: TypeId): Self = this.set("REGTYPE", value.asInstanceOf[js.Any])
    @scala.inline
    def setRELTIME(value: TypeId): Self = this.set("RELTIME", value.asInstanceOf[js.Any])
    @scala.inline
    def setSMGR(value: TypeId): Self = this.set("SMGR", value.asInstanceOf[js.Any])
    @scala.inline
    def setTEXT(value: TypeId): Self = this.set("TEXT", value.asInstanceOf[js.Any])
    @scala.inline
    def setTID(value: TypeId): Self = this.set("TID", value.asInstanceOf[js.Any])
    @scala.inline
    def setTIME(value: TypeId): Self = this.set("TIME", value.asInstanceOf[js.Any])
    @scala.inline
    def setTIMESTAMP(value: TypeId): Self = this.set("TIMESTAMP", value.asInstanceOf[js.Any])
    @scala.inline
    def setTIMESTAMPTZ(value: TypeId): Self = this.set("TIMESTAMPTZ", value.asInstanceOf[js.Any])
    @scala.inline
    def setTIMETZ(value: TypeId): Self = this.set("TIMETZ", value.asInstanceOf[js.Any])
    @scala.inline
    def setTINTERVAL(value: TypeId): Self = this.set("TINTERVAL", value.asInstanceOf[js.Any])
    @scala.inline
    def setTSQUERY(value: TypeId): Self = this.set("TSQUERY", value.asInstanceOf[js.Any])
    @scala.inline
    def setTSVECTOR(value: TypeId): Self = this.set("TSVECTOR", value.asInstanceOf[js.Any])
    @scala.inline
    def setTXID_SNAPSHOT(value: TypeId): Self = this.set("TXID_SNAPSHOT", value.asInstanceOf[js.Any])
    @scala.inline
    def setUUID(value: TypeId): Self = this.set("UUID", value.asInstanceOf[js.Any])
    @scala.inline
    def setVARBIT(value: TypeId): Self = this.set("VARBIT", value.asInstanceOf[js.Any])
    @scala.inline
    def setVARCHAR(value: TypeId): Self = this.set("VARCHAR", value.asInstanceOf[js.Any])
    @scala.inline
    def setXID(value: TypeId): Self = this.set("XID", value.asInstanceOf[js.Any])
    @scala.inline
    def setXML(value: TypeId): Self = this.set("XML", value.asInstanceOf[js.Any])
  }
  
}

