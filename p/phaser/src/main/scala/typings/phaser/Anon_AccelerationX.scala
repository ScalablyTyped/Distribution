...
object Anon_AccelerationX {
  ...
  def apply(
    ...
    if (!js.isUndefined(allowGravity)) __obj.updateDynamic("allowGravity")(allowGravity)
    if (!js.isUndefined(allowRotation)) __obj.updateDynamic("allowRotation")(allowRotation)
    ...
    if (!js.isUndefined(immovable)) __obj.updateDynamic("immovable")(immovable)
