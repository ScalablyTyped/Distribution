package typings.pixiDotJs

import typings.pixiDotJs.PIXINs.interactionNs.InteractionMouseEvents
import typings.pixiDotJs.PIXINs.interactionNs.InteractionPixiEvents
import typings.pixiDotJs.PIXINs.interactionNs.InteractionPointerEvents
import typings.pixiDotJs.PIXINs.interactionNs.InteractionTouchEvents
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object pixiDotJsStrings {
  @js.native
  sealed trait UniformDataMap extends js.Object
  
  @js.native
  sealed trait added extends InteractionPixiEvents
  
  @js.native
  sealed trait click extends InteractionMouseEvents
  
  @js.native
  sealed trait complete extends js.Object
  
  @js.native
  sealed trait context extends js.Object
  
  @js.native
  sealed trait dispose extends js.Object
  
  @js.native
  sealed trait error extends js.Object
  
  @js.native
  sealed trait `high-performance` extends js.Object
  
  @js.native
  sealed trait load extends js.Object
  
  @js.native
  sealed trait loaded extends js.Object
  
  @js.native
  sealed trait mousedown extends InteractionMouseEvents
  
  @js.native
  sealed trait mousemove extends InteractionMouseEvents
  
  @js.native
  sealed trait mouseout extends InteractionMouseEvents
  
  @js.native
  sealed trait mouseover extends InteractionMouseEvents
  
  @js.native
  sealed trait mouseup extends InteractionMouseEvents
  
  @js.native
  sealed trait mouseupoutside extends InteractionMouseEvents
  
  @js.native
  sealed trait pointercancel extends InteractionPointerEvents
  
  @js.native
  sealed trait pointerdown extends InteractionPointerEvents
  
  @js.native
  sealed trait pointermove extends InteractionPointerEvents
  
  @js.native
  sealed trait pointerout extends InteractionPointerEvents
  
  @js.native
  sealed trait pointerover extends InteractionPointerEvents
  
  @js.native
  sealed trait pointertap extends InteractionPointerEvents
  
  @js.native
  sealed trait pointerup extends InteractionPointerEvents
  
  @js.native
  sealed trait pointerupoutside extends InteractionPointerEvents
  
  @js.native
  sealed trait postrender extends js.Object
  
  @js.native
  sealed trait prerender extends js.Object
  
  @js.native
  sealed trait progress extends js.Object
  
  @js.native
  sealed trait removed extends InteractionPixiEvents
  
  @js.native
  sealed trait rightclick extends InteractionMouseEvents
  
  @js.native
  sealed trait rightdown extends InteractionMouseEvents
  
  @js.native
  sealed trait rightup extends InteractionMouseEvents
  
  @js.native
  sealed trait rightupoutside extends InteractionMouseEvents
  
  @js.native
  sealed trait start extends js.Object
  
  @js.native
  sealed trait tap extends InteractionTouchEvents
  
  @js.native
  sealed trait touchcancel extends InteractionTouchEvents
  
  @js.native
  sealed trait touchend extends InteractionTouchEvents
  
  @js.native
  sealed trait touchendoutside extends InteractionTouchEvents
  
  @js.native
  sealed trait touchmove extends InteractionTouchEvents
  
  @js.native
  sealed trait touchstart extends InteractionTouchEvents
  
  @js.native
  sealed trait update extends js.Object
  
  @scala.inline
  def UniformDataMap: UniformDataMap = "UniformDataMap".asInstanceOf[UniformDataMap]
  @scala.inline
  def added: added = "added".asInstanceOf[added]
  @scala.inline
  def click: click = "click".asInstanceOf[click]
  @scala.inline
  def complete: complete = "complete".asInstanceOf[complete]
  @scala.inline
  def context: context = "context".asInstanceOf[context]
  @scala.inline
  def dispose: dispose = "dispose".asInstanceOf[dispose]
  @scala.inline
  def error: error = "error".asInstanceOf[error]
  @scala.inline
  def `high-performance`: `high-performance` = "high-performance".asInstanceOf[`high-performance`]
  @scala.inline
  def load: load = "load".asInstanceOf[load]
  @scala.inline
  def loaded: loaded = "loaded".asInstanceOf[loaded]
  @scala.inline
  def mousedown: mousedown = "mousedown".asInstanceOf[mousedown]
  @scala.inline
  def mousemove: mousemove = "mousemove".asInstanceOf[mousemove]
  @scala.inline
  def mouseout: mouseout = "mouseout".asInstanceOf[mouseout]
  @scala.inline
  def mouseover: mouseover = "mouseover".asInstanceOf[mouseover]
  @scala.inline
  def mouseup: mouseup = "mouseup".asInstanceOf[mouseup]
  @scala.inline
  def mouseupoutside: mouseupoutside = "mouseupoutside".asInstanceOf[mouseupoutside]
  @scala.inline
  def pointercancel: pointercancel = "pointercancel".asInstanceOf[pointercancel]
  @scala.inline
  def pointerdown: pointerdown = "pointerdown".asInstanceOf[pointerdown]
  @scala.inline
  def pointermove: pointermove = "pointermove".asInstanceOf[pointermove]
  @scala.inline
  def pointerout: pointerout = "pointerout".asInstanceOf[pointerout]
  @scala.inline
  def pointerover: pointerover = "pointerover".asInstanceOf[pointerover]
  @scala.inline
  def pointertap: pointertap = "pointertap".asInstanceOf[pointertap]
  @scala.inline
  def pointerup: pointerup = "pointerup".asInstanceOf[pointerup]
  @scala.inline
  def pointerupoutside: pointerupoutside = "pointerupoutside".asInstanceOf[pointerupoutside]
  @scala.inline
  def postrender: postrender = "postrender".asInstanceOf[postrender]
  @scala.inline
  def prerender: prerender = "prerender".asInstanceOf[prerender]
  @scala.inline
  def progress: progress = "progress".asInstanceOf[progress]
  @scala.inline
  def removed: removed = "removed".asInstanceOf[removed]
  @scala.inline
  def rightclick: rightclick = "rightclick".asInstanceOf[rightclick]
  @scala.inline
  def rightdown: rightdown = "rightdown".asInstanceOf[rightdown]
  @scala.inline
  def rightup: rightup = "rightup".asInstanceOf[rightup]
  @scala.inline
  def rightupoutside: rightupoutside = "rightupoutside".asInstanceOf[rightupoutside]
  @scala.inline
  def start: start = "start".asInstanceOf[start]
  @scala.inline
  def tap: tap = "tap".asInstanceOf[tap]
  @scala.inline
  def touchcancel: touchcancel = "touchcancel".asInstanceOf[touchcancel]
  @scala.inline
  def touchend: touchend = "touchend".asInstanceOf[touchend]
  @scala.inline
  def touchendoutside: touchendoutside = "touchendoutside".asInstanceOf[touchendoutside]
  @scala.inline
  def touchmove: touchmove = "touchmove".asInstanceOf[touchmove]
  @scala.inline
  def touchstart: touchstart = "touchstart".asInstanceOf[touchstart]
  @scala.inline
  def update: update = "update".asInstanceOf[update]
}

