package typings.plotlyJs

import typings.plotlyJs.anon.Align
import typings.plotlyJs.anon.Line
import typings.plotlyJs.anon.Width
import typings.plotlyJs.plotlyJsStrings.dot_
import typings.plotlyJs.plotlyJsStrings.end
import typings.plotlyJs.plotlyJsStrings.legendonly
import typings.plotlyJs.plotlyJsStrings.middle
import typings.plotlyJs.plotlyJsStrings.ohcl
import typings.plotlyJs.plotlyJsStrings.start
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libTracesOhclMod {
  
  /* Rewritten from type alias, can be one of: 
    - typings.plotlyJs.plotlyJsStrings.solid
    - typings.plotlyJs.plotlyJsStrings.dot_
    - typings.plotlyJs.plotlyJsStrings.dash
    - typings.plotlyJs.plotlyJsStrings.longdash
    - typings.plotlyJs.plotlyJsStrings.dashdot
    - typings.plotlyJs.plotlyJsStrings.longdashdot
  */
  trait Dash extends StObject
  object Dash {
    
    inline def dash: typings.plotlyJs.plotlyJsStrings.dash = "dash".asInstanceOf[typings.plotlyJs.plotlyJsStrings.dash]
    
    inline def dashdot: typings.plotlyJs.plotlyJsStrings.dashdot = "dashdot".asInstanceOf[typings.plotlyJs.plotlyJsStrings.dashdot]
    
    inline def dot: dot_ = "dot".asInstanceOf[dot_]
    
    inline def longdash: typings.plotlyJs.plotlyJsStrings.longdash = "longdash".asInstanceOf[typings.plotlyJs.plotlyJsStrings.longdash]
    
    inline def longdashdot: typings.plotlyJs.plotlyJsStrings.longdashdot = "longdashdot".asInstanceOf[typings.plotlyJs.plotlyJsStrings.longdashdot]
    
    inline def solid: typings.plotlyJs.plotlyJsStrings.solid = "solid".asInstanceOf[typings.plotlyJs.plotlyJsStrings.solid]
  }
  
  /* Rewritten from type alias, can be one of: 
    - typings.plotlyJs.plotlyJsStrings.x
    - typings.plotlyJs.plotlyJsStrings.y
    - typings.plotlyJs.plotlyJsStrings.z
    - typings.plotlyJs.plotlyJsStrings.text
    - typings.plotlyJs.plotlyJsStrings.name
    - typings.plotlyJs.plotlyJsStrings.xPlussigny
    - typings.plotlyJs.plotlyJsStrings.xPlussignz
    - typings.plotlyJs.plotlyJsStrings.xPlussigntext
    - typings.plotlyJs.plotlyJsStrings.xPlussignname
    - typings.plotlyJs.plotlyJsStrings.yPlussignz
    - typings.plotlyJs.plotlyJsStrings.yPlussigntext
    - typings.plotlyJs.plotlyJsStrings.yPlussignname
    - typings.plotlyJs.plotlyJsStrings.zPlussigntext
    - typings.plotlyJs.plotlyJsStrings.zPlussignname
    - typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignz
    - typings.plotlyJs.plotlyJsStrings.xPlussignyPlussigntext
    - typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignname
    - typings.plotlyJs.plotlyJsStrings.yPlussignzPlussigntext
    - typings.plotlyJs.plotlyJsStrings.yPlussignzPlussignname
    - typings.plotlyJs.plotlyJsStrings.zPlussigntextPlussignname
    - typings.plotlyJs.plotlyJsStrings.all
    - typings.plotlyJs.plotlyJsStrings.none
    - typings.plotlyJs.plotlyJsStrings.skip
  */
  trait HoverInfo extends StObject
  object HoverInfo {
    
    inline def all: typings.plotlyJs.plotlyJsStrings.all = "all".asInstanceOf[typings.plotlyJs.plotlyJsStrings.all]
    
    inline def name: typings.plotlyJs.plotlyJsStrings.name = "name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.name]
    
    inline def none: typings.plotlyJs.plotlyJsStrings.none = "none".asInstanceOf[typings.plotlyJs.plotlyJsStrings.none]
    
    inline def skip: typings.plotlyJs.plotlyJsStrings.skip = "skip".asInstanceOf[typings.plotlyJs.plotlyJsStrings.skip]
    
    inline def text: typings.plotlyJs.plotlyJsStrings.text = "text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.text]
    
    inline def x: typings.plotlyJs.plotlyJsStrings.x = "x".asInstanceOf[typings.plotlyJs.plotlyJsStrings.x]
    
    inline def xPlussignname: typings.plotlyJs.plotlyJsStrings.xPlussignname = "x+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussignname]
    
    inline def xPlussigntext: typings.plotlyJs.plotlyJsStrings.xPlussigntext = "x+text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussigntext]
    
    inline def xPlussigny: typings.plotlyJs.plotlyJsStrings.xPlussigny = "x+y".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussigny]
    
    inline def xPlussignyPlussignname: typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignname = "x+y+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignname]
    
    inline def xPlussignyPlussigntext: typings.plotlyJs.plotlyJsStrings.xPlussignyPlussigntext = "x+y+text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussignyPlussigntext]
    
    inline def xPlussignyPlussignz: typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignz = "x+y+z".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussignyPlussignz]
    
    inline def xPlussignz: typings.plotlyJs.plotlyJsStrings.xPlussignz = "x+z".asInstanceOf[typings.plotlyJs.plotlyJsStrings.xPlussignz]
    
    inline def y: typings.plotlyJs.plotlyJsStrings.y = "y".asInstanceOf[typings.plotlyJs.plotlyJsStrings.y]
    
    inline def yPlussignname: typings.plotlyJs.plotlyJsStrings.yPlussignname = "y+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.yPlussignname]
    
    inline def yPlussigntext: typings.plotlyJs.plotlyJsStrings.yPlussigntext = "y+text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.yPlussigntext]
    
    inline def yPlussignz: typings.plotlyJs.plotlyJsStrings.yPlussignz = "y+z".asInstanceOf[typings.plotlyJs.plotlyJsStrings.yPlussignz]
    
    inline def yPlussignzPlussignname: typings.plotlyJs.plotlyJsStrings.yPlussignzPlussignname = "y+z+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.yPlussignzPlussignname]
    
    inline def yPlussignzPlussigntext: typings.plotlyJs.plotlyJsStrings.yPlussignzPlussigntext = "y+z+text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.yPlussignzPlussigntext]
    
    inline def z: typings.plotlyJs.plotlyJsStrings.z = "z".asInstanceOf[typings.plotlyJs.plotlyJsStrings.z]
    
    inline def zPlussignname: typings.plotlyJs.plotlyJsStrings.zPlussignname = "z+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zPlussignname]
    
    inline def zPlussigntext: typings.plotlyJs.plotlyJsStrings.zPlussigntext = "z+text".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zPlussigntext]
    
    inline def zPlussigntextPlussignname: typings.plotlyJs.plotlyJsStrings.zPlussigntextPlussignname = "z+text+name".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zPlussigntextPlussignname]
  }
  
  trait OhclData extends StObject {
    
    var close: js.Array[Double]
    
    // TODO: further refine
    var customdata: js.Array[Any]
    
    var decreasing: Line
    
    var high: js.Array[Double]
    
    var hoverinfo: HoverInfo
    
    var hoverlabel: Align
    
    var hovertext: String | js.Array[String]
    
    var ids: js.Array[String]
    
    // TODO: further refine
    var increasing: Line
    
    var legendgroup: String
    
    // TODO: further refine;
    var line: Width
    
    var low: js.Array[Double]
    
    var meta: Any
    
    var name: String
    
    var opacity: Double
    
    var open: js.Array[Double]
    
    var selectedpoints: Any
    
    var showlegend: Boolean
    
    var text: String | js.Array[String]
    
    var tickwidth: Double
    
    var `type`: ohcl
    
    var uirevision: Any
    
    var visible: Boolean | legendonly
    
    var x: js.Array[String]
    
    // TODO: further refine
    var xaxis: String
    
    var xcalendar: XCalendar
    
    // TODO: should depend on the layout
    var xperiod: Any
    
    var xperiod0: Any
    
    // TODO: further refine
    var xperiodalignment: start | middle | end
    
    // TODO: should depend on the layout
    var yaxis: String
  }
  object OhclData {
    
    inline def apply(
      close: js.Array[Double],
      customdata: js.Array[Any],
      decreasing: Line,
      high: js.Array[Double],
      hoverinfo: HoverInfo,
      hoverlabel: Align,
      hovertext: String | js.Array[String],
      ids: js.Array[String],
      increasing: Line,
      legendgroup: String,
      line: Width,
      low: js.Array[Double],
      meta: Any,
      name: String,
      opacity: Double,
      open: js.Array[Double],
      selectedpoints: Any,
      showlegend: Boolean,
      text: String | js.Array[String],
      tickwidth: Double,
      uirevision: Any,
      visible: Boolean | legendonly,
      x: js.Array[String],
      xaxis: String,
      xcalendar: XCalendar,
      xperiod: Any,
      xperiod0: Any,
      xperiodalignment: start | middle | end,
      yaxis: String
    ): OhclData = {
      val __obj = js.Dynamic.literal(close = close.asInstanceOf[js.Any], customdata = customdata.asInstanceOf[js.Any], decreasing = decreasing.asInstanceOf[js.Any], high = high.asInstanceOf[js.Any], hoverinfo = hoverinfo.asInstanceOf[js.Any], hoverlabel = hoverlabel.asInstanceOf[js.Any], hovertext = hovertext.asInstanceOf[js.Any], ids = ids.asInstanceOf[js.Any], increasing = increasing.asInstanceOf[js.Any], legendgroup = legendgroup.asInstanceOf[js.Any], line = line.asInstanceOf[js.Any], low = low.asInstanceOf[js.Any], meta = meta.asInstanceOf[js.Any], name = name.asInstanceOf[js.Any], opacity = opacity.asInstanceOf[js.Any], open = open.asInstanceOf[js.Any], selectedpoints = selectedpoints.asInstanceOf[js.Any], showlegend = showlegend.asInstanceOf[js.Any], text = text.asInstanceOf[js.Any], tickwidth = tickwidth.asInstanceOf[js.Any], uirevision = uirevision.asInstanceOf[js.Any], visible = visible.asInstanceOf[js.Any], x = x.asInstanceOf[js.Any], xaxis = xaxis.asInstanceOf[js.Any], xcalendar = xcalendar.asInstanceOf[js.Any], xperiod = xperiod.asInstanceOf[js.Any], xperiod0 = xperiod0.asInstanceOf[js.Any], xperiodalignment = xperiodalignment.asInstanceOf[js.Any], yaxis = yaxis.asInstanceOf[js.Any])
      __obj.updateDynamic("type")("ohcl")
      __obj.asInstanceOf[OhclData]
    }
    
    extension [Self <: OhclData](x: Self) {
      
      inline def setClose(value: js.Array[Double]): Self = StObject.set(x, "close", value.asInstanceOf[js.Any])
      
      inline def setCloseVarargs(value: Double*): Self = StObject.set(x, "close", js.Array(value*))
      
      inline def setCustomdata(value: js.Array[Any]): Self = StObject.set(x, "customdata", value.asInstanceOf[js.Any])
      
      inline def setCustomdataVarargs(value: Any*): Self = StObject.set(x, "customdata", js.Array(value*))
      
      inline def setDecreasing(value: Line): Self = StObject.set(x, "decreasing", value.asInstanceOf[js.Any])
      
      inline def setHigh(value: js.Array[Double]): Self = StObject.set(x, "high", value.asInstanceOf[js.Any])
      
      inline def setHighVarargs(value: Double*): Self = StObject.set(x, "high", js.Array(value*))
      
      inline def setHoverinfo(value: HoverInfo): Self = StObject.set(x, "hoverinfo", value.asInstanceOf[js.Any])
      
      inline def setHoverlabel(value: Align): Self = StObject.set(x, "hoverlabel", value.asInstanceOf[js.Any])
      
      inline def setHovertext(value: String | js.Array[String]): Self = StObject.set(x, "hovertext", value.asInstanceOf[js.Any])
      
      inline def setHovertextVarargs(value: String*): Self = StObject.set(x, "hovertext", js.Array(value*))
      
      inline def setIds(value: js.Array[String]): Self = StObject.set(x, "ids", value.asInstanceOf[js.Any])
      
      inline def setIdsVarargs(value: String*): Self = StObject.set(x, "ids", js.Array(value*))
      
      inline def setIncreasing(value: Line): Self = StObject.set(x, "increasing", value.asInstanceOf[js.Any])
      
      inline def setLegendgroup(value: String): Self = StObject.set(x, "legendgroup", value.asInstanceOf[js.Any])
      
      inline def setLine(value: Width): Self = StObject.set(x, "line", value.asInstanceOf[js.Any])
      
      inline def setLow(value: js.Array[Double]): Self = StObject.set(x, "low", value.asInstanceOf[js.Any])
      
      inline def setLowVarargs(value: Double*): Self = StObject.set(x, "low", js.Array(value*))
      
      inline def setMeta(value: Any): Self = StObject.set(x, "meta", value.asInstanceOf[js.Any])
      
      inline def setName(value: String): Self = StObject.set(x, "name", value.asInstanceOf[js.Any])
      
      inline def setOpacity(value: Double): Self = StObject.set(x, "opacity", value.asInstanceOf[js.Any])
      
      inline def setOpen(value: js.Array[Double]): Self = StObject.set(x, "open", value.asInstanceOf[js.Any])
      
      inline def setOpenVarargs(value: Double*): Self = StObject.set(x, "open", js.Array(value*))
      
      inline def setSelectedpoints(value: Any): Self = StObject.set(x, "selectedpoints", value.asInstanceOf[js.Any])
      
      inline def setShowlegend(value: Boolean): Self = StObject.set(x, "showlegend", value.asInstanceOf[js.Any])
      
      inline def setText(value: String | js.Array[String]): Self = StObject.set(x, "text", value.asInstanceOf[js.Any])
      
      inline def setTextVarargs(value: String*): Self = StObject.set(x, "text", js.Array(value*))
      
      inline def setTickwidth(value: Double): Self = StObject.set(x, "tickwidth", value.asInstanceOf[js.Any])
      
      inline def setType(value: ohcl): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
      
      inline def setUirevision(value: Any): Self = StObject.set(x, "uirevision", value.asInstanceOf[js.Any])
      
      inline def setVisible(value: Boolean | legendonly): Self = StObject.set(x, "visible", value.asInstanceOf[js.Any])
      
      inline def setX(value: js.Array[String]): Self = StObject.set(x, "x", value.asInstanceOf[js.Any])
      
      inline def setXVarargs(value: String*): Self = StObject.set(x, "x", js.Array(value*))
      
      inline def setXaxis(value: String): Self = StObject.set(x, "xaxis", value.asInstanceOf[js.Any])
      
      inline def setXcalendar(value: XCalendar): Self = StObject.set(x, "xcalendar", value.asInstanceOf[js.Any])
      
      inline def setXperiod(value: Any): Self = StObject.set(x, "xperiod", value.asInstanceOf[js.Any])
      
      inline def setXperiod0(value: Any): Self = StObject.set(x, "xperiod0", value.asInstanceOf[js.Any])
      
      inline def setXperiodalignment(value: start | middle | end): Self = StObject.set(x, "xperiodalignment", value.asInstanceOf[js.Any])
      
      inline def setYaxis(value: String): Self = StObject.set(x, "yaxis", value.asInstanceOf[js.Any])
    }
  }
  
  /* Rewritten from type alias, can be one of: 
    - typings.plotlyJs.plotlyJsStrings.gregorian
    - typings.plotlyJs.plotlyJsStrings.chinese
    - typings.plotlyJs.plotlyJsStrings.coptic
    - typings.plotlyJs.plotlyJsStrings.discworld
    - typings.plotlyJs.plotlyJsStrings.ethiopian
    - typings.plotlyJs.plotlyJsStrings.hebrew
    - typings.plotlyJs.plotlyJsStrings.islamic
    - typings.plotlyJs.plotlyJsStrings.julian
    - typings.plotlyJs.plotlyJsStrings.mayan
    - typings.plotlyJs.plotlyJsStrings.nanakshahi
    - typings.plotlyJs.plotlyJsStrings.nepali
    - typings.plotlyJs.plotlyJsStrings.persian
    - typings.plotlyJs.plotlyJsStrings.jalali
    - typings.plotlyJs.plotlyJsStrings.taiwan
    - typings.plotlyJs.plotlyJsStrings.thai
    - typings.plotlyJs.plotlyJsStrings.ummalqura
  */
  trait XCalendar extends StObject
  object XCalendar {
    
    inline def chinese: typings.plotlyJs.plotlyJsStrings.chinese = "chinese".asInstanceOf[typings.plotlyJs.plotlyJsStrings.chinese]
    
    inline def coptic: typings.plotlyJs.plotlyJsStrings.coptic = "coptic".asInstanceOf[typings.plotlyJs.plotlyJsStrings.coptic]
    
    inline def discworld: typings.plotlyJs.plotlyJsStrings.discworld = "discworld".asInstanceOf[typings.plotlyJs.plotlyJsStrings.discworld]
    
    inline def ethiopian: typings.plotlyJs.plotlyJsStrings.ethiopian = "ethiopian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.ethiopian]
    
    inline def gregorian: typings.plotlyJs.plotlyJsStrings.gregorian = "gregorian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.gregorian]
    
    inline def hebrew: typings.plotlyJs.plotlyJsStrings.hebrew = "hebrew".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hebrew]
    
    inline def islamic: typings.plotlyJs.plotlyJsStrings.islamic = "islamic".asInstanceOf[typings.plotlyJs.plotlyJsStrings.islamic]
    
    inline def jalali: typings.plotlyJs.plotlyJsStrings.jalali = "jalali".asInstanceOf[typings.plotlyJs.plotlyJsStrings.jalali]
    
    inline def julian: typings.plotlyJs.plotlyJsStrings.julian = "julian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.julian]
    
    inline def mayan: typings.plotlyJs.plotlyJsStrings.mayan = "mayan".asInstanceOf[typings.plotlyJs.plotlyJsStrings.mayan]
    
    inline def nanakshahi: typings.plotlyJs.plotlyJsStrings.nanakshahi = "nanakshahi".asInstanceOf[typings.plotlyJs.plotlyJsStrings.nanakshahi]
    
    inline def nepali: typings.plotlyJs.plotlyJsStrings.nepali = "nepali".asInstanceOf[typings.plotlyJs.plotlyJsStrings.nepali]
    
    inline def persian: typings.plotlyJs.plotlyJsStrings.persian = "persian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.persian]
    
    inline def taiwan: typings.plotlyJs.plotlyJsStrings.taiwan = "taiwan".asInstanceOf[typings.plotlyJs.plotlyJsStrings.taiwan]
    
    inline def thai: typings.plotlyJs.plotlyJsStrings.thai = "thai".asInstanceOf[typings.plotlyJs.plotlyJsStrings.thai]
    
    inline def ummalqura: typings.plotlyJs.plotlyJsStrings.ummalqura = "ummalqura".asInstanceOf[typings.plotlyJs.plotlyJsStrings.ummalqura]
  }
}
