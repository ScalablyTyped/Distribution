package typings.plotlyJs.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.plotlyJs.plotlyJsStrings.lasso2d
  - typings.plotlyJs.plotlyJsStrings.select2d
  - typings.plotlyJs.plotlyJsStrings.sendDataToCloud
  - typings.plotlyJs.plotlyJsStrings.zoom2d
  - typings.plotlyJs.plotlyJsStrings.pan2d
  - typings.plotlyJs.plotlyJsStrings.zoomIn2d
  - typings.plotlyJs.plotlyJsStrings.zoomOut2d
  - typings.plotlyJs.plotlyJsStrings.autoScale2d
  - typings.plotlyJs.plotlyJsStrings.resetScale2d
  - typings.plotlyJs.plotlyJsStrings.hoverClosestCartesian
  - typings.plotlyJs.plotlyJsStrings.hoverCompareCartesian
  - typings.plotlyJs.plotlyJsStrings.zoom3d
  - typings.plotlyJs.plotlyJsStrings.pan3d
  - typings.plotlyJs.plotlyJsStrings.orbitRotation
  - typings.plotlyJs.plotlyJsStrings.tableRotation
  - typings.plotlyJs.plotlyJsStrings.resetCameraDefault3d
  - typings.plotlyJs.plotlyJsStrings.resetCameraLastSave3d
  - typings.plotlyJs.plotlyJsStrings.hoverClosest3d
  - typings.plotlyJs.plotlyJsStrings.zoomInGeo
  - typings.plotlyJs.plotlyJsStrings.zoomOutGeo
  - typings.plotlyJs.plotlyJsStrings.resetGeo
  - typings.plotlyJs.plotlyJsStrings.hoverClosestGeo
  - typings.plotlyJs.plotlyJsStrings.hoverClosestGl2d
  - typings.plotlyJs.plotlyJsStrings.hoverClosestPie
  - typings.plotlyJs.plotlyJsStrings.toggleHover
  - typings.plotlyJs.plotlyJsStrings.toImage
  - typings.plotlyJs.plotlyJsStrings.resetViews
  - typings.plotlyJs.plotlyJsStrings.toggleSpikelines
*/
trait ModeBarDefaultButtons extends StObject
object ModeBarDefaultButtons {
  
  inline def autoScale2d: typings.plotlyJs.plotlyJsStrings.autoScale2d = "autoScale2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.autoScale2d]
  
  inline def hoverClosest3d: typings.plotlyJs.plotlyJsStrings.hoverClosest3d = "hoverClosest3d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverClosest3d]
  
  inline def hoverClosestCartesian: typings.plotlyJs.plotlyJsStrings.hoverClosestCartesian = "hoverClosestCartesian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverClosestCartesian]
  
  inline def hoverClosestGeo: typings.plotlyJs.plotlyJsStrings.hoverClosestGeo = "hoverClosestGeo".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverClosestGeo]
  
  inline def hoverClosestGl2d: typings.plotlyJs.plotlyJsStrings.hoverClosestGl2d = "hoverClosestGl2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverClosestGl2d]
  
  inline def hoverClosestPie: typings.plotlyJs.plotlyJsStrings.hoverClosestPie = "hoverClosestPie".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverClosestPie]
  
  inline def hoverCompareCartesian: typings.plotlyJs.plotlyJsStrings.hoverCompareCartesian = "hoverCompareCartesian".asInstanceOf[typings.plotlyJs.plotlyJsStrings.hoverCompareCartesian]
  
  inline def lasso2d: typings.plotlyJs.plotlyJsStrings.lasso2d = "lasso2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.lasso2d]
  
  inline def orbitRotation: typings.plotlyJs.plotlyJsStrings.orbitRotation = "orbitRotation".asInstanceOf[typings.plotlyJs.plotlyJsStrings.orbitRotation]
  
  inline def pan2d: typings.plotlyJs.plotlyJsStrings.pan2d = "pan2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.pan2d]
  
  inline def pan3d: typings.plotlyJs.plotlyJsStrings.pan3d = "pan3d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.pan3d]
  
  inline def resetCameraDefault3d: typings.plotlyJs.plotlyJsStrings.resetCameraDefault3d = "resetCameraDefault3d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.resetCameraDefault3d]
  
  inline def resetCameraLastSave3d: typings.plotlyJs.plotlyJsStrings.resetCameraLastSave3d = "resetCameraLastSave3d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.resetCameraLastSave3d]
  
  inline def resetGeo: typings.plotlyJs.plotlyJsStrings.resetGeo = "resetGeo".asInstanceOf[typings.plotlyJs.plotlyJsStrings.resetGeo]
  
  inline def resetScale2d: typings.plotlyJs.plotlyJsStrings.resetScale2d = "resetScale2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.resetScale2d]
  
  inline def resetViews: typings.plotlyJs.plotlyJsStrings.resetViews = "resetViews".asInstanceOf[typings.plotlyJs.plotlyJsStrings.resetViews]
  
  inline def select2d: typings.plotlyJs.plotlyJsStrings.select2d = "select2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.select2d]
  
  inline def sendDataToCloud: typings.plotlyJs.plotlyJsStrings.sendDataToCloud = "sendDataToCloud".asInstanceOf[typings.plotlyJs.plotlyJsStrings.sendDataToCloud]
  
  inline def tableRotation: typings.plotlyJs.plotlyJsStrings.tableRotation = "tableRotation".asInstanceOf[typings.plotlyJs.plotlyJsStrings.tableRotation]
  
  inline def toImage: typings.plotlyJs.plotlyJsStrings.toImage = "toImage".asInstanceOf[typings.plotlyJs.plotlyJsStrings.toImage]
  
  inline def toggleHover: typings.plotlyJs.plotlyJsStrings.toggleHover = "toggleHover".asInstanceOf[typings.plotlyJs.plotlyJsStrings.toggleHover]
  
  inline def toggleSpikelines: typings.plotlyJs.plotlyJsStrings.toggleSpikelines = "toggleSpikelines".asInstanceOf[typings.plotlyJs.plotlyJsStrings.toggleSpikelines]
  
  inline def zoom2d: typings.plotlyJs.plotlyJsStrings.zoom2d = "zoom2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoom2d]
  
  inline def zoom3d: typings.plotlyJs.plotlyJsStrings.zoom3d = "zoom3d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoom3d]
  
  inline def zoomIn2d: typings.plotlyJs.plotlyJsStrings.zoomIn2d = "zoomIn2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoomIn2d]
  
  inline def zoomInGeo: typings.plotlyJs.plotlyJsStrings.zoomInGeo = "zoomInGeo".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoomInGeo]
  
  inline def zoomOut2d: typings.plotlyJs.plotlyJsStrings.zoomOut2d = "zoomOut2d".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoomOut2d]
  
  inline def zoomOutGeo: typings.plotlyJs.plotlyJsStrings.zoomOutGeo = "zoomOutGeo".asInstanceOf[typings.plotlyJs.plotlyJsStrings.zoomOutGeo]
}
