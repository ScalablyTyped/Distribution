package typings.plotlyJs.anon

import typings.std.ArrayLike
import typings.std.Date
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait Typeofbisect extends StObject {
  
  def apply(array: ArrayLike[String], x: String): Double = js.native
  def apply(array: ArrayLike[String], x: String, lo: Double): Double = js.native
  def apply(array: ArrayLike[String], x: String, lo: Double, hi: Double): Double = js.native
  def apply(array: ArrayLike[String], x: String, lo: Unit, hi: Double): Double = js.native
  def apply(array: ArrayLike[Double], x: Double): Double = js.native
  def apply(array: ArrayLike[Double], x: Double, lo: Double): Double = js.native
  def apply(array: ArrayLike[Double], x: Double, lo: Double, hi: Double): Double = js.native
  def apply(array: ArrayLike[Double], x: Double, lo: Unit, hi: Double): Double = js.native
  def apply(array: ArrayLike[Date], x: Date): Double = js.native
  def apply(array: ArrayLike[Date], x: Date, lo: Double): Double = js.native
  def apply(array: ArrayLike[Date], x: Date, lo: Double, hi: Double): Double = js.native
  def apply(array: ArrayLike[Date], x: Date, lo: Unit, hi: Double): Double = js.native
}
