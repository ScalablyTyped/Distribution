package typings.angularCommon.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait BYNGMD extends StObject {
  
  var BYN: js.Array[js.UndefOr[String]]
  
  var GMD: js.Array[String]
  
  var GNF: js.Array[js.UndefOr[String]]
  
  var JPY: js.Array[String]
  
  var NGN: js.Array[String]
  
  var PGK: js.Array[String]
  
  var PHP: js.Array[String]
  
  var USD: js.Array[String]
  
  var XAF: js.Array[String]
  
  var XOF: js.Array[String]
}
object BYNGMD {
  
  inline def apply(
    BYN: js.Array[js.UndefOr[String]],
    GMD: js.Array[String],
    GNF: js.Array[js.UndefOr[String]],
    JPY: js.Array[String],
    NGN: js.Array[String],
    PGK: js.Array[String],
    PHP: js.Array[String],
    USD: js.Array[String],
    XAF: js.Array[String],
    XOF: js.Array[String]
  ): BYNGMD = {
    val __obj = js.Dynamic.literal(BYN = BYN.asInstanceOf[js.Any], GMD = GMD.asInstanceOf[js.Any], GNF = GNF.asInstanceOf[js.Any], JPY = JPY.asInstanceOf[js.Any], NGN = NGN.asInstanceOf[js.Any], PGK = PGK.asInstanceOf[js.Any], PHP = PHP.asInstanceOf[js.Any], USD = USD.asInstanceOf[js.Any], XAF = XAF.asInstanceOf[js.Any], XOF = XOF.asInstanceOf[js.Any])
    __obj.asInstanceOf[BYNGMD]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: BYNGMD] (val x: Self) extends AnyVal {
    
    inline def setBYN(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "BYN", value.asInstanceOf[js.Any])
    
    inline def setBYNVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "BYN", js.Array(value*))
    
    inline def setGMD(value: js.Array[String]): Self = StObject.set(x, "GMD", value.asInstanceOf[js.Any])
    
    inline def setGMDVarargs(value: String*): Self = StObject.set(x, "GMD", js.Array(value*))
    
    inline def setGNF(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "GNF", value.asInstanceOf[js.Any])
    
    inline def setGNFVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "GNF", js.Array(value*))
    
    inline def setJPY(value: js.Array[String]): Self = StObject.set(x, "JPY", value.asInstanceOf[js.Any])
    
    inline def setJPYVarargs(value: String*): Self = StObject.set(x, "JPY", js.Array(value*))
    
    inline def setNGN(value: js.Array[String]): Self = StObject.set(x, "NGN", value.asInstanceOf[js.Any])
    
    inline def setNGNVarargs(value: String*): Self = StObject.set(x, "NGN", js.Array(value*))
    
    inline def setPGK(value: js.Array[String]): Self = StObject.set(x, "PGK", value.asInstanceOf[js.Any])
    
    inline def setPGKVarargs(value: String*): Self = StObject.set(x, "PGK", js.Array(value*))
    
    inline def setPHP(value: js.Array[String]): Self = StObject.set(x, "PHP", value.asInstanceOf[js.Any])
    
    inline def setPHPVarargs(value: String*): Self = StObject.set(x, "PHP", js.Array(value*))
    
    inline def setUSD(value: js.Array[String]): Self = StObject.set(x, "USD", value.asInstanceOf[js.Any])
    
    inline def setUSDVarargs(value: String*): Self = StObject.set(x, "USD", js.Array(value*))
    
    inline def setXAF(value: js.Array[String]): Self = StObject.set(x, "XAF", value.asInstanceOf[js.Any])
    
    inline def setXAFVarargs(value: String*): Self = StObject.set(x, "XAF", js.Array(value*))
    
    inline def setXOF(value: js.Array[String]): Self = StObject.set(x, "XOF", value.asInstanceOf[js.Any])
    
    inline def setXOFVarargs(value: String*): Self = StObject.set(x, "XOF", js.Array(value*))
  }
}
