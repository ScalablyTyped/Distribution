package typings.antdMobileRn

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonAlignItemsBackgroundColorBorderRadius extends js.Object {
  var alignItems: String
  var backgroundColor: String
  var borderRadius: Double
  var height: Double
  var justifyContent: String
  var overflow: String
  var position: String
  var right: Double
  var top: Double
  var width: Double
}

object AnonAlignItemsBackgroundColorBorderRadius {
  @scala.inline
  def apply(
    alignItems: String,
    backgroundColor: String,
    borderRadius: Double,
    height: Double,
    justifyContent: String,
    overflow: String,
    position: String,
    right: Double,
    top: Double,
    width: Double
  ): AnonAlignItemsBackgroundColorBorderRadius = {
    val __obj = js.Dynamic.literal(alignItems = alignItems.asInstanceOf[js.Any], backgroundColor = backgroundColor.asInstanceOf[js.Any], borderRadius = borderRadius.asInstanceOf[js.Any], height = height.asInstanceOf[js.Any], justifyContent = justifyContent.asInstanceOf[js.Any], overflow = overflow.asInstanceOf[js.Any], position = position.asInstanceOf[js.Any], right = right.asInstanceOf[js.Any], top = top.asInstanceOf[js.Any], width = width.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[AnonAlignItemsBackgroundColorBorderRadius]
  }
}

