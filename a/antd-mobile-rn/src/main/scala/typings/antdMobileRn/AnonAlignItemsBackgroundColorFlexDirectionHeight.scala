package typings.antdMobileRn

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonAlignItemsBackgroundColorFlexDirectionHeight extends js.Object {
  var alignItems: String
  var backgroundColor: String
  var flexDirection: String
  var height: Double
  var overflow: String
}

object AnonAlignItemsBackgroundColorFlexDirectionHeight {
  @scala.inline
  def apply(
    alignItems: String,
    backgroundColor: String,
    flexDirection: String,
    height: Double,
    overflow: String
  ): AnonAlignItemsBackgroundColorFlexDirectionHeight = {
    val __obj = js.Dynamic.literal(alignItems = alignItems.asInstanceOf[js.Any], backgroundColor = backgroundColor.asInstanceOf[js.Any], flexDirection = flexDirection.asInstanceOf[js.Any], height = height.asInstanceOf[js.Any], overflow = overflow.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[AnonAlignItemsBackgroundColorFlexDirectionHeight]
  }
}

