...
- Dropped {[ P in std.Exclude<'className' | 'collapsed' | 'flatMenuKeys' | 'isMobile' | 'menuData' | 'mode' | 'openKeys' | 'style' | 'theme' | 'menuItemRender' | 'location' | 'staticContext' | 'history' | 'computedMatch' | 'match' | 'route' | 'onClick' | 'subMenuCloseDelay' | 'getPopupContainer' | 'selectedKeys' | 'defaultOpenKeys' | 'onOpenChange' | 'inlineCollapsed' | 'focusable' | 'defaultSelectedKeys' | 'prefixCls' | 'overflowedIndicator' | 'subMenuOpenDelay' | 'selectable' | 'id' | 'onSelect' | 'onDeselect' | 'multiple' | 'inlineIndent' | 'openTransitionName' | 'onMouseEnter' | 'openAnimation' | 'forceSubMenuRender' | 'menu' | 'navTheme' | 'contentWidth' | 'layout' | 'title' | 'fixedHeader' | 'iconfontUrl' | 'fixSiderbar' | 'autoHideHeader', ['onCollapse']> ]: @ant-design/pro-layout.@ant-design/pro-layout/es/SiderMenu/BaseMenu.BaseMenuProps[P]} */ trait SiderMenuProps extends js.Object {
