...
object esStyleMod {
  ...
  @JSImport("@ant-design/pro-layout/es/style", JSImport.Namespace)
  @js.native
  ...
  trait ProLayoutToken
    extends StObject
       with ProAliasToken {
    
    var componentCls: String
  }
  object ProLayoutToken {
    
    inline def apply(ProAliasToken: ProAliasToken, componentCls: String): ProLayoutToken = {
      ...
      __obj.asInstanceOf[ProLayoutToken]
    }
    
    @scala.inline
    implicit open class MutableBuilder[Self <: ProLayoutToken] (val x: Self) extends AnyVal {
