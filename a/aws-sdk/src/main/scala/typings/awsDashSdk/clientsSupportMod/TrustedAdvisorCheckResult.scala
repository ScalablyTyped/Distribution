...
object TrustedAdvisorCheckResult {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(categorySpecificSummary = categorySpecificSummary, checkId = checkId, flaggedResources = flaggedResources, resourcesSummary = resourcesSummary, status = status, timestamp = timestamp)
