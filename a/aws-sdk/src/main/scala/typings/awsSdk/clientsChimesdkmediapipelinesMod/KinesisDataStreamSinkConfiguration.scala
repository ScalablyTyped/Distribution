package typings.awsSdk.clientsChimesdkmediapipelinesMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait KinesisDataStreamSinkConfiguration extends StObject {
  
  /**
    * The ARN of the sink.
    */
  var InsightsTarget: js.UndefOr[Arn] = js.undefined
}
object KinesisDataStreamSinkConfiguration {
  
  inline def apply(): KinesisDataStreamSinkConfiguration = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[KinesisDataStreamSinkConfiguration]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: KinesisDataStreamSinkConfiguration] (val x: Self) extends AnyVal {
    
    inline def setInsightsTarget(value: Arn): Self = StObject.set(x, "InsightsTarget", value.asInstanceOf[js.Any])
    
    inline def setInsightsTargetUndefined: Self = StObject.set(x, "InsightsTarget", js.undefined)
  }
}
