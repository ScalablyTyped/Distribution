package typings.awsSdk.clientsImagebuilderMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait CvssScore extends StObject {
  
  /**
    * The CVSS base score.
    */
  var baseScore: js.UndefOr[NonNegativeDouble] = js.undefined
  
  /**
    * The vector string of the CVSS score.
    */
  var scoringVector: js.UndefOr[NonEmptyString] = js.undefined
  
  /**
    * The source of the CVSS score.
    */
  var source: js.UndefOr[NonEmptyString] = js.undefined
  
  /**
    * The CVSS version that generated the score.
    */
  var version: js.UndefOr[NonEmptyString] = js.undefined
}
object CvssScore {
  
  inline def apply(): CvssScore = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[CvssScore]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: CvssScore] (val x: Self) extends AnyVal {
    
    inline def setBaseScore(value: NonNegativeDouble): Self = StObject.set(x, "baseScore", value.asInstanceOf[js.Any])
    
    inline def setBaseScoreUndefined: Self = StObject.set(x, "baseScore", js.undefined)
    
    inline def setScoringVector(value: NonEmptyString): Self = StObject.set(x, "scoringVector", value.asInstanceOf[js.Any])
    
    inline def setScoringVectorUndefined: Self = StObject.set(x, "scoringVector", js.undefined)
    
    inline def setSource(value: NonEmptyString): Self = StObject.set(x, "source", value.asInstanceOf[js.Any])
    
    inline def setSourceUndefined: Self = StObject.set(x, "source", js.undefined)
    
    inline def setVersion(value: NonEmptyString): Self = StObject.set(x, "version", value.asInstanceOf[js.Any])
    
    inline def setVersionUndefined: Self = StObject.set(x, "version", js.undefined)
  }
}
