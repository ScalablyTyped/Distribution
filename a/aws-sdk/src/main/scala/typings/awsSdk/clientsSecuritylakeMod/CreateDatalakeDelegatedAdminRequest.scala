package typings.awsSdk.clientsSecuritylakeMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait CreateDatalakeDelegatedAdminRequest extends StObject {
  
  /**
    * The Amazon Web Services account ID of the Security Lake delegated administrator.
    */
  var account: SafeString
}
object CreateDatalakeDelegatedAdminRequest {
  
  inline def apply(account: SafeString): CreateDatalakeDelegatedAdminRequest = {
    val __obj = js.Dynamic.literal(account = account.asInstanceOf[js.Any])
    __obj.asInstanceOf[CreateDatalakeDelegatedAdminRequest]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: CreateDatalakeDelegatedAdminRequest] (val x: Self) extends AnyVal {
    
    inline def setAccount(value: SafeString): Self = StObject.set(x, "account", value.asInstanceOf[js.Any])
  }
}
