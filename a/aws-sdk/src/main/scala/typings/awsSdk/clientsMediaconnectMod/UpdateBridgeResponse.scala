package typings.awsSdk.clientsMediaconnectMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait UpdateBridgeResponse extends StObject {
  
  var Bridge: js.UndefOr[typings.awsSdk.clientsMediaconnectMod.Bridge] = js.undefined
}
object UpdateBridgeResponse {
  
  inline def apply(): UpdateBridgeResponse = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[UpdateBridgeResponse]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: UpdateBridgeResponse] (val x: Self) extends AnyVal {
    
    inline def setBridge(value: Bridge): Self = StObject.set(x, "Bridge", value.asInstanceOf[js.Any])
    
    inline def setBridgeUndefined: Self = StObject.set(x, "Bridge", js.undefined)
  }
}
