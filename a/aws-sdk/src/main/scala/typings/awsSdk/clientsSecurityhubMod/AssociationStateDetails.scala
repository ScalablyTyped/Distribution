package typings.awsSdk.clientsSecurityhubMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait AssociationStateDetails extends StObject {
  
  /**
    *  The state of the association. 
    */
  var State: js.UndefOr[NonEmptyString] = js.undefined
  
  /**
    *  The status message, if applicable. 
    */
  var StatusMessage: js.UndefOr[NonEmptyString] = js.undefined
}
object AssociationStateDetails {
  
  inline def apply(): AssociationStateDetails = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[AssociationStateDetails]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: AssociationStateDetails] (val x: Self) extends AnyVal {
    
    inline def setState(value: NonEmptyString): Self = StObject.set(x, "State", value.asInstanceOf[js.Any])
    
    inline def setStateUndefined: Self = StObject.set(x, "State", js.undefined)
    
    inline def setStatusMessage(value: NonEmptyString): Self = StObject.set(x, "StatusMessage", value.asInstanceOf[js.Any])
    
    inline def setStatusMessageUndefined: Self = StObject.set(x, "StatusMessage", js.undefined)
  }
}
