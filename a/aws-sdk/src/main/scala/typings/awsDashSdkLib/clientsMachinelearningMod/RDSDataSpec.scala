...
object RDSDataSpec {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(DatabaseCredentials = DatabaseCredentials, DatabaseInformation = DatabaseInformation, ResourceRole = ResourceRole, S3StagingLocation = S3StagingLocation, SecurityGroupIds = SecurityGroupIds, SelectSqlQuery = SelectSqlQuery, ServiceRole = ServiceRole, SubnetId = SubnetId)
    ...
    if (DataSchemaUri != null) __obj.updateDynamic("DataSchemaUri")(DataSchemaUri)
