...
trait PointCloudLayerView extends LayerView {
  ...
  /**
    ...
    *
    * @param target The point(s) to highlight. A graphic representing a point to highlight can be obtained by using [SceneView.hitTest()](https://developers.arcgis.com/javascript/latest/api-reference/esri-views-SceneView.html#hitTest).
    *
    */
