package typings.arcgisJsApi

import org.scalablytyped.runtime.Shortcut
import typings.arcgisJsApi.esri.NetworkUrl
import typings.arcgisJsApi.esri.NetworkUrlConstructor
import typings.arcgisJsApi.esri.NetworkUrlProperties
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object esriRestSupportNetworkUrlMod extends Shortcut {
  
  @JSImport("esri/rest/support/NetworkUrl", JSImport.Namespace)
  @js.native
  val ^ : js.Object & NetworkUrlConstructor = js.native
  
  /* This class was inferred from a value with a constructor, it was renamed because a distinct type already exists with the same name. */
  @JSImport("esri/rest/support/NetworkUrl", JSImport.Namespace)
  @js.native
  /**
    * An input type for Route, Closest Facility, and Service Area solvers.
    *
    * [Read more...](https://developers.arcgis.com/javascript/latest/api-reference/esri-rest-support-NetworkUrl.html)
    */
  open class Class ()
    extends StObject
       with NetworkUrl {
    def this(properties: NetworkUrlProperties) = this()
    
    /**
      * Converts an instance of this class to its [ArcGIS portal JSON](https://developers.arcgis.com/documentation/common-data-types/geometry-objects.htm) representation.
      *
      * [Read more...](https://developers.arcgis.com/javascript/latest/api-reference/esri-core-JSONSupport.html#toJSON)
      */
    /* CompleteClass */
    override def toJSON(): Any = js.native
  }
  
  type _To = js.Object & NetworkUrlConstructor
  
  /* This means you don't have to write `^`, but can instead just say `esriRestSupportNetworkUrlMod.foo` */
  override def _to: js.Object & NetworkUrlConstructor = ^
}
