...
object `type` {
  @scala.inline
  def apply(
    createPCClassRenderer: typeCreatePCClassRendererParams => IPromise[PCClassRendererResult],
    createRenderer: typeCreateRendererParams => IPromise[typeRendererResult]
  ): `type` = {
