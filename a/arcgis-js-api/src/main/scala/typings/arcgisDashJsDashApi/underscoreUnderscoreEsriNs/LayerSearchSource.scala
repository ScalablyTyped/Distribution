...
/**
  ...
  */
class LayerSearchSourceCls () extends LayerSearchSource {
...
object LayerSearchSource {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(autoNavigate = autoNavigate, clone = js.Any.fromFunction0(clone), displayField = displayField, exactMatch = exactMatch, filter = filter, getResults = js.Any.fromFunction1(getResults), getSuggestions = js.Any.fromFunction1(getSuggestions), layer = layer, maxResults = maxResults, maxSuggestions = maxSuggestions, minSuggestCharacters = minSuggestCharacters, name = name, outFields = outFields, placeholder = placeholder, popupEnabled = popupEnabled, popupTemplate = popupTemplate, prefix = prefix, resultGraphicEnabled = resultGraphicEnabled, resultSymbol = resultSymbol, searchFields = searchFields, searchTemplate = searchTemplate, suffix = suffix, suggestionTemplate = suggestionTemplate, suggestionsEnabled = suggestionsEnabled, toJSON = js.Any.fromFunction0(toJSON), withinViewEnabled = withinViewEnabled, zoomScale = zoomScale)
  
    __obj.asInstanceOf[LayerSearchSource]
  }
}
