package typings
package arcgisDashJsDashApiLib.esriTasksGeometryServiceMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("esri/tasks/GeometryService", JSImport.Namespace)
@js.native
/**
  * Represents a geometry service resource exposed by the ArcGIS REST API. It is used to perform various operations on geometries such as project, simplify, buffer, and relationships.
  *
  * [Read more...](https://developers.arcgis.com/javascript/latest/api-reference/esri-tasks-GeometryService.html)
  */
class Class ()
  extends arcgisDashJsDashApiLib.underscoreUnderscoreEsriNs.GeometryService {
  def this(properties: arcgisDashJsDashApiLib.underscoreUnderscoreEsriNs.GeometryServiceProperties) = this()
}

