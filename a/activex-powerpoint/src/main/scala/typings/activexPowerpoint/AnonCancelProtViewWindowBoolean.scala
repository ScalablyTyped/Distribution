package typings.activexPowerpoint

import typings.activexPowerpoint.PowerPoint.ProtectedViewWindow
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonCancelProtViewWindowBoolean extends js.Object {
  var Cancel: Boolean
  val ProtViewWindow: ProtectedViewWindow
}

object AnonCancelProtViewWindowBoolean {
  @scala.inline
  def apply(Cancel: Boolean, ProtViewWindow: ProtectedViewWindow): AnonCancelProtViewWindowBoolean = {
    val __obj = js.Dynamic.literal(Cancel = Cancel.asInstanceOf[js.Any], ProtViewWindow = ProtViewWindow.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[AnonCancelProtViewWindowBoolean]
  }
}

