package typings.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.HTMLStyleSheetRule")
@js.native
class HTMLStyleSheetRule protected ()
  extends StObject
     with typings.activexMshtml.MSHTML.HTMLStyleSheetRule {
  
  /* CompleteClass */
  @JSName("MSHTML.HTMLStyleSheetRule_typekey")
  var MSHTMLDotHTMLStyleSheetRule_typekey: typings.activexMshtml.MSHTML.HTMLStyleSheetRule = js.native
  
  /* CompleteClass */
  override val constructor: js.Any = js.native
  
  /* CompleteClass */
  var cssText: String = js.native
  
  /* CompleteClass */
  var ie9_selectorText: String = js.native
  
  /* CompleteClass */
  override def msGetSpecificity(index: Double): Double = js.native
  
  /* CompleteClass */
  override val parentRule: typings.activexMshtml.MSHTML.IHTMLCSSRule = js.native
  
  /* CompleteClass */
  override val parentStyleSheet: typings.activexMshtml.MSHTML.IHTMLStyleSheet = js.native
  
  /* CompleteClass */
  override val readOnly: Boolean = js.native
  
  /* CompleteClass */
  var selectorText: String = js.native
  
  /* CompleteClass */
  override val style: typings.activexMshtml.MSHTML.IHTMLRuleStyle = js.native
  
  /* CompleteClass */
  override val `type`: Double = js.native
}
