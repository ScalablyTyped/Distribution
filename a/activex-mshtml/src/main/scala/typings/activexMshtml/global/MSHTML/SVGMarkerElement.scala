package typings.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.SVGMarkerElement")
@js.native
/* private */ open class SVGMarkerElement ()
  extends StObject
     with typings.activexMshtml.MSHTML.SVGMarkerElement {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.SVGMarkerElement_typekey")
  var MSHTMLDotSVGMarkerElement_typekey: typings.activexMshtml.MSHTML.SVGMarkerElement = js.native
  
  /* CompleteClass */
  override val className: typings.activexMshtml.MSHTML.SVGAnimatedString = js.native
  
  /* CompleteClass */
  override val externalResourcesRequired: typings.activexMshtml.MSHTML.SVGAnimatedBoolean = js.native
  
  /* CompleteClass */
  var focusable: typings.activexMshtml.MSHTML.SVGAnimatedEnumeration = js.native
  
  /* CompleteClass */
  var markerHeight: typings.activexMshtml.MSHTML.SVGAnimatedLength = js.native
  
  /* CompleteClass */
  var markerUnits: typings.activexMshtml.MSHTML.SVGAnimatedEnumeration = js.native
  
  /* CompleteClass */
  var markerWidth: typings.activexMshtml.MSHTML.SVGAnimatedLength = js.native
  
  /* CompleteClass */
  var orientAngle: typings.activexMshtml.MSHTML.SVGAnimatedAngle = js.native
  
  /* CompleteClass */
  var orientType: typings.activexMshtml.MSHTML.SVGAnimatedEnumeration = js.native
  
  /* CompleteClass */
  var ownerSVGElement: typings.activexMshtml.MSHTML.ISVGSVGElement = js.native
  
  /* CompleteClass */
  var preserveAspectRatio: typings.activexMshtml.MSHTML.SVGAnimatedPreserveAspectRatio = js.native
  
  /* CompleteClass */
  var refX: typings.activexMshtml.MSHTML.SVGAnimatedLength = js.native
  
  /* CompleteClass */
  var refY: typings.activexMshtml.MSHTML.SVGAnimatedLength = js.native
  
  /* CompleteClass */
  override def setOrientToAngle(pSVGAngle: typings.activexMshtml.MSHTML.SVGAngle): Unit = js.native
  
  /* CompleteClass */
  override def setOrientToAuto(): Unit = js.native
  
  /* CompleteClass */
  override val viewBox: typings.activexMshtml.MSHTML.SVGAnimatedRect = js.native
  
  /* CompleteClass */
  var viewportElement: typings.activexMshtml.MSHTML.ISVGElement = js.native
  
  /* CompleteClass */
  var xmlbase: String = js.native
  
  /* CompleteClass */
  var xmllang: String = js.native
  
  /* CompleteClass */
  var xmlspace: String = js.native
}
