...
import typings.atlaskitTokens.anon.Interaction12
...
trait InteractionColorTokenSchema[BaseToken] extends StObject {
  ...
  var color: Interaction12[BaseToken]
}
object InteractionColorTokenSchema {
  
  inline def apply[BaseToken](color: Interaction12[BaseToken]): InteractionColorTokenSchema[BaseToken] = {
  ...
  extension [Self <: InteractionColorTokenSchema[?], BaseToken](x: Self & InteractionColorTokenSchema[BaseToken]) {
    ...
    inline def setColor(value: Interaction12[BaseToken]): Self = StObject.set(x, "color", value.asInstanceOf[js.Any])
  }
}
