...
import typings.atlaskitTokens.anon.BorderBoldBrand
...
trait BorderColorTokenSchema[BaseToken] extends StObject {
  ...
  var color: BorderBoldBrand[BaseToken]
}
object BorderColorTokenSchema {
  
  inline def apply[BaseToken](color: BorderBoldBrand[BaseToken]): BorderColorTokenSchema[BaseToken] = {
  ...
  implicit open class MutableBuilder[Self <: BorderColorTokenSchema[?], BaseToken] (val x: Self & BorderColorTokenSchema[BaseToken]) extends AnyVal {
    ...
    inline def setColor(value: BorderBoldBrand[BaseToken]): Self = StObject.set(x, "color", value.asInstanceOf[js.Any])
  }
}
