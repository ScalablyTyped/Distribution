package typings.activexWord.global.Word

import typings.activexWord.Word.WdFieldType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.MailMergeField")
@js.native
/* private */ open class MailMergeField ()
  extends StObject
     with typings.activexWord.Word.MailMergeField {
  
  /* CompleteClass */
  override val Application: typings.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var Code: typings.activexWord.Word.Range = js.native
  
  /* CompleteClass */
  override def Copy(): Unit = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Cut(): Unit = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  var Locked: Boolean = js.native
  
  /* CompleteClass */
  override val Next: typings.activexWord.Word.MailMergeField = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override val Previous: typings.activexWord.Word.MailMergeField = js.native
  
  /* CompleteClass */
  override def Select(): Unit = js.native
  
  /* CompleteClass */
  override val Type: WdFieldType = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.MailMergeField_typekey")
  var WordDotMailMergeField_typekey: typings.activexWord.Word.MailMergeField = js.native
}
