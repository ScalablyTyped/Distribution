package typings.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.AutoCorrect")
@js.native
/* private */ class AutoCorrect ()
  extends StObject
     with typings.activexWord.Word.AutoCorrect {
  
  /* CompleteClass */
  override val Application: typings.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var CorrectCapsLock: Boolean = js.native
  
  /* CompleteClass */
  var CorrectDays: Boolean = js.native
  
  /* CompleteClass */
  var CorrectHangulAndAlphabet: Boolean = js.native
  
  /* CompleteClass */
  var CorrectInitialCaps: Boolean = js.native
  
  /* CompleteClass */
  var CorrectKeyboardSetting: Boolean = js.native
  
  /* CompleteClass */
  var CorrectSentenceCaps: Boolean = js.native
  
  /* CompleteClass */
  var CorrectTableCells: Boolean = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  var DisplayAutoCorrectOptions: Boolean = js.native
  
  /* CompleteClass */
  override val Entries: typings.activexWord.Word.AutoCorrectEntries = js.native
  
  /* CompleteClass */
  var FirstLetterAutoAdd: Boolean = js.native
  
  /* CompleteClass */
  override val FirstLetterExceptions: typings.activexWord.Word.FirstLetterExceptions = js.native
  
  /* CompleteClass */
  var HangulAndAlphabetAutoAdd: Boolean = js.native
  
  /* CompleteClass */
  override val HangulAndAlphabetExceptions: typings.activexWord.Word.HangulAndAlphabetExceptions = js.native
  
  /* CompleteClass */
  var OtherCorrectionsAutoAdd: Boolean = js.native
  
  /* CompleteClass */
  override val OtherCorrectionsExceptions: typings.activexWord.Word.OtherCorrectionsExceptions = js.native
  
  /* CompleteClass */
  override val Parent: js.Any = js.native
  
  /* CompleteClass */
  var ReplaceText: Boolean = js.native
  
  /* CompleteClass */
  var ReplaceTextFromSpellingChecker: Boolean = js.native
  
  /* CompleteClass */
  var TwoInitialCapsAutoAdd: Boolean = js.native
  
  /* CompleteClass */
  override val TwoInitialCapsExceptions: typings.activexWord.Word.TwoInitialCapsExceptions = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.AutoCorrect_typekey")
  var WordDotAutoCorrect_typekey: typings.activexWord.Word.AutoCorrect = js.native
}
