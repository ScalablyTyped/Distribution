package typings
package activexDashWordLib.WordNs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("Word.Application")
@js.native
class Application protected () extends js.Object {
  val ActiveDocument: Document = js.native
  val ActiveEncryptionSession: scala.Double = js.native
  var ActivePrinter: java.lang.String = js.native
  val ActiveProtectedViewWindow: ProtectedViewWindow = js.native
  val ActiveWindow: Window = js.native
  val AddIns: activexDashWordLib.WordNs.AddIns = js.native
  val AnswerWizard: activexDashOfficeLib.OfficeNs.AnswerWizard = js.native
  val Application: activexDashWordLib.WordNs.Application = js.native
  val ArbitraryXMLSupportAvailable: scala.Boolean = js.native
  val Assistance: activexDashOfficeLib.OfficeNs.IAssistance = js.native
  val Assistant: activexDashOfficeLib.OfficeNs.Assistant = js.native
  val AutoCaptions: activexDashWordLib.WordNs.AutoCaptions = js.native
  val AutoCorrect: activexDashWordLib.WordNs.AutoCorrect = js.native
  val AutoCorrectEmail: AutoCorrect = js.native
  var AutomationSecurity: activexDashOfficeLib.OfficeNs.MsoAutomationSecurity = js.native
  val BackgroundPrintingStatus: scala.Double = js.native
  val BackgroundSavingStatus: scala.Double = js.native
  val Bibliography: activexDashWordLib.WordNs.Bibliography = js.native
  var BrowseExtraFileTypes: java.lang.String = js.native
  val Browser: activexDashWordLib.WordNs.Browser = js.native
  val Build: java.lang.String = js.native
  val BuildFeatureCrew: java.lang.String = js.native
  val BuildFull: java.lang.String = js.native
  @JSName("COMAddIns")
  val COMAddIns_Original: activexDashOfficeLib.OfficeNs.COMAddIns = js.native
  val CapsLock: scala.Boolean = js.native
  var Caption: java.lang.String = js.native
  val CaptionLabels: activexDashWordLib.WordNs.CaptionLabels = js.native
  var CheckLanguage: scala.Boolean = js.native
  @JSName("CommandBars")
  val CommandBars_Original: activexDashOfficeLib.OfficeNs.CommandBars = js.native
  val Creator: scala.Double = js.native
  val CustomDictionaries: Dictionaries = js.native
  var CustomizationContext: js.Any = js.native
  var DefaultLegalBlackline: scala.Boolean = js.native
  var DefaultSaveFormat: java.lang.String = js.native
  var DefaultTableSeparator: java.lang.String = js.native
  val Dialogs: activexDashWordLib.WordNs.Dialogs = js.native
  var DisplayAlerts: WdAlertLevel = js.native
  var DisplayAutoCompleteTips: scala.Boolean = js.native
  var DisplayDocumentInformationPanel: scala.Boolean = js.native
  var DisplayRecentFiles: scala.Boolean = js.native
  var DisplayScreenTips: scala.Boolean = js.native
  var DisplayScrollBars: scala.Boolean = js.native
  var DisplayStatusBar: scala.Boolean = js.native
  val Documents: activexDashWordLib.WordNs.Documents = js.native
  var DontResetInsertionPointProperties: scala.Boolean = js.native
  val Dummy1: scala.Boolean = js.native
  val EmailOptions: activexDashWordLib.WordNs.EmailOptions = js.native
  var EmailTemplate: java.lang.String = js.native
  var EnableCancelKey: WdEnableCancelKey = js.native
  var FeatureInstall: activexDashOfficeLib.OfficeNs.MsoFeatureInstall = js.native
  val FileConverters: activexDashWordLib.WordNs.FileConverters = js.native
  val FileSearch: activexDashOfficeLib.OfficeNs.FileSearch = js.native
  var FileValidation: activexDashOfficeLib.OfficeNs.MsoFileValidationMode = js.native
  val FocusInMailHeader: scala.Boolean = js.native
  val FontNames: activexDashWordLib.WordNs.FontNames = js.native
  val HangulHanjaDictionaries: HangulHanjaConversionDictionaries = js.native
  var Height: scala.Double = js.native
  val IsSandboxed: scala.Boolean = js.native
  val KeyBindings: activexDashWordLib.WordNs.KeyBindings = js.native
  val LandscapeFontNames: FontNames = js.native
  val Language: activexDashOfficeLib.OfficeNs.MsoLanguageID = js.native
  val LanguageSettings: activexDashOfficeLib.OfficeNs.LanguageSettings = js.native
  val Languages: activexDashWordLib.WordNs.Languages = js.native
  var Left: scala.Double = js.native
  val ListGalleries: activexDashWordLib.WordNs.ListGalleries = js.native
  val MAPIAvailable: scala.Boolean = js.native
  val MacroContainer: js.Any = js.native
  val MailMessage: activexDashWordLib.WordNs.MailMessage = js.native
  val MailSystem: WdMailSystem = js.native
  val MailingLabel: activexDashWordLib.WordNs.MailingLabel = js.native
  val MathCoprocessorAvailable: scala.Boolean = js.native
  val MouseAvailable: scala.Boolean = js.native
  val Name: java.lang.String = js.native
  val NewDocument: activexDashOfficeLib.OfficeNs.NewFile = js.native
  val NormalTemplate: Template = js.native
  val NumLock: scala.Boolean = js.native
  val OMathAutoCorrect: activexDashWordLib.WordNs.OMathAutoCorrect = js.native
  var OpenAttachmentsInFullScreen: scala.Boolean = js.native
  val Options: activexDashWordLib.WordNs.Options = js.native
  val Parent: js.Any = js.native
  val Path: java.lang.String = js.native
  val PathSeparator: java.lang.String = js.native
  val PickerDialog: activexDashOfficeLib.OfficeNs.PickerDialog = js.native
  val PortraitFontNames: FontNames = js.native
  var PrintPreview: scala.Boolean = js.native
  val ProtectedViewWindows: activexDashWordLib.WordNs.ProtectedViewWindows = js.native
  val RecentFiles: activexDashWordLib.WordNs.RecentFiles = js.native
  var RestrictLinkedStyles: scala.Boolean = js.native
  var ScreenUpdating: scala.Boolean = js.native
  val Selection: activexDashWordLib.WordNs.Selection = js.native
  var ShowStartupDialog: scala.Boolean = js.native
  var ShowStylePreviews: scala.Boolean = js.native
  var ShowVisualBasicEditor: scala.Boolean = js.native
  var ShowWindowsInTaskbar: scala.Boolean = js.native
  @JSName("SmartArtColors")
  val SmartArtColors_Original: activexDashOfficeLib.OfficeNs.SmartArtColors = js.native
  @JSName("SmartArtLayouts")
  val SmartArtLayouts_Original: activexDashOfficeLib.OfficeNs.SmartArtLayouts = js.native
  @JSName("SmartArtQuickStyles")
  val SmartArtQuickStyles_Original: activexDashOfficeLib.OfficeNs.SmartArtQuickStyles = js.native
  val SmartTagRecognizers: activexDashWordLib.WordNs.SmartTagRecognizers = js.native
  val SmartTagTypes: activexDashWordLib.WordNs.SmartTagTypes = js.native
  val SpecialMode: scala.Boolean = js.native
  var StartupPath: java.lang.String = js.native
  var StatusBar: java.lang.String = js.native
  val System: activexDashWordLib.WordNs.System = js.native
  val TaskPanes: activexDashWordLib.WordNs.TaskPanes = js.native
  val Tasks: activexDashWordLib.WordNs.Tasks = js.native
  val Templates: activexDashWordLib.WordNs.Templates = js.native
  var Top: scala.Double = js.native
  val UndoRecord: activexDashWordLib.WordNs.UndoRecord = js.native
  val UsableHeight: scala.Double = js.native
  val UsableWidth: scala.Double = js.native
  var UserAddress: java.lang.String = js.native
  val UserControl: scala.Boolean = js.native
  var UserInitials: java.lang.String = js.native
  var UserName: java.lang.String = js.native
  val VBE: activexDashVbideLib.VBIDENs.VBE = js.native
  val Version: java.lang.String = js.native
  var Visible: scala.Boolean = js.native
  var Width: scala.Double = js.native
  var WindowState: WdWindowState = js.native
  val Windows: activexDashWordLib.WordNs.Windows = js.native
  var `Word.Application_typekey`: Application = js.native
  val WordBasic: js.Any = js.native
  val XMLNamespaces: activexDashWordLib.WordNs.XMLNamespaces = js.native
  def Activate(): scala.Unit = js.native
  def AddAddress(
    TagID: activexDashWordLib.SafeArray[java.lang.String],
    Value: activexDashWordLib.SafeArray[java.lang.String]
  ): scala.Unit = js.native
  def AutomaticChange(): scala.Unit = js.native
  def BuildKeyCode(Arg1: WdKey): scala.Double = js.native
  def BuildKeyCode(Arg1: WdKey, Arg2: js.Any): scala.Double = js.native
  def BuildKeyCode(Arg1: WdKey, Arg2: js.Any, Arg3: js.Any): scala.Double = js.native
  def BuildKeyCode(Arg1: WdKey, Arg2: js.Any, Arg3: js.Any, Arg4: js.Any): scala.Double = js.native
  def COMAddIns(Index: java.lang.String): activexDashOfficeLib.OfficeNs.COMAddIn = js.native
  def COMAddIns(Index: scala.Double): activexDashOfficeLib.OfficeNs.COMAddIn = js.native
  def CentimetersToPoints(Centimeters: scala.Double): scala.Double = js.native
  def ChangeFileOpenDirectory(Path: java.lang.String): scala.Unit = js.native
  def CheckGrammar(String: java.lang.String): scala.Boolean = js.native
  def CheckSpelling(
    Word: java.lang.String,
    CustomDictionary: js.UndefOr[js.Any],
    IgnoreUppercase: js.UndefOr[js.Any],
    MainDictionary: js.UndefOr[js.Any],
    CustomDictionary2: js.UndefOr[js.Any],
    CustomDictionary3: js.UndefOr[js.Any],
    CustomDictionary4: js.UndefOr[js.Any],
    CustomDictionary5: js.UndefOr[js.Any],
    CustomDictionary6: js.UndefOr[js.Any],
    CustomDictionary7: js.UndefOr[js.Any],
    CustomDictionary8: js.UndefOr[js.Any],
    CustomDictionary9: js.UndefOr[js.Any],
    CustomDictionary10: js.UndefOr[js.Any]
  ): scala.Boolean = js.native
  def CleanString(String: java.lang.String): java.lang.String = js.native
  def CommandBars(Index: java.lang.String): activexDashOfficeLib.OfficeNs.CommandBar = js.native
  def CommandBars(Index: scala.Double): activexDashOfficeLib.OfficeNs.CommandBar = js.native
  /**
    * @param Word.WdCompareDestination [Destination=2]
    * @param Word.WdGranularity [Granularity=1]
    * @param boolean [CompareFormatting=true]
    * @param boolean [CompareCaseChanges=true]
    * @param boolean [CompareWhitespace=true]
    * @param boolean [CompareTables=true]
    * @param boolean [CompareHeaders=true]
    * @param boolean [CompareFootnotes=true]
    * @param boolean [CompareTextboxes=true]
    * @param boolean [CompareFields=true]
    * @param boolean [CompareComments=true]
    * @param boolean [CompareMoves=true]
    * @param string [RevisedAuthor='']
    * @param boolean [IgnoreAllComparisonWarnings=false]
    */
  def CompareDocuments(
    OriginalDocument: Document,
    RevisedDocument: Document,
    Destination: js.UndefOr[WdCompareDestination],
    Granularity: js.UndefOr[WdGranularity],
    CompareFormatting: js.UndefOr[scala.Boolean],
    CompareCaseChanges: js.UndefOr[scala.Boolean],
    CompareWhitespace: js.UndefOr[scala.Boolean],
    CompareTables: js.UndefOr[scala.Boolean],
    CompareHeaders: js.UndefOr[scala.Boolean],
    CompareFootnotes: js.UndefOr[scala.Boolean],
    CompareTextboxes: js.UndefOr[scala.Boolean],
    CompareFields: js.UndefOr[scala.Boolean],
    CompareComments: js.UndefOr[scala.Boolean],
    CompareMoves: js.UndefOr[scala.Boolean],
    RevisedAuthor: js.UndefOr[java.lang.String],
    IgnoreAllComparisonWarnings: js.UndefOr[scala.Boolean]
  ): Document = js.native
  def DDEExecute(Channel: scala.Double, Command: java.lang.String): scala.Unit = js.native
  def DDEInitiate(App: java.lang.String, Topic: java.lang.String): scala.Double = js.native
  def DDEPoke(Channel: scala.Double, Item: java.lang.String, Data: java.lang.String): scala.Unit = js.native
  def DDERequest(Channel: scala.Double, Item: java.lang.String): java.lang.String = js.native
  def DDETerminate(Channel: scala.Double): scala.Unit = js.native
  def DDETerminateAll(): scala.Unit = js.native
  def DefaultWebOptions(): activexDashWordLib.WordNs.DefaultWebOptions = js.native
  def DiscussionSupport(Range: js.Any, cid: js.Any, piCSE: js.Any): scala.Unit = js.native
  def Dummy2(): scala.Boolean = js.native
  def Dummy4(): scala.Unit = js.native
  def FileDialog(FileDialogType: activexDashOfficeLib.OfficeNs.MsoFileDialogType): activexDashOfficeLib.OfficeNs.FileDialog = js.native
  def FindKey(KeyCode: scala.Double): KeyBinding = js.native
  def FindKey(KeyCode: scala.Double, KeyCode2: js.Any): KeyBinding = js.native
  def GetAddress(
    Name: js.UndefOr[js.Any],
    AddressProperties: js.UndefOr[js.Any],
    UseAutoText: js.UndefOr[js.Any],
    DisplaySelectDialog: js.UndefOr[js.Any],
    SelectDialog: js.UndefOr[js.Any],
    CheckNamesDialog: js.UndefOr[js.Any],
    RecentAddressesChoice: js.UndefOr[js.Any],
    UpdateRecentAddresses: js.UndefOr[js.Any]
  ): java.lang.String = js.native
  def GetDefaultTheme(DocumentType: WdDocumentMedium): java.lang.String = js.native
  def GetSpellingSuggestions(
    Word: java.lang.String,
    CustomDictionary: js.UndefOr[js.Any],
    IgnoreUppercase: js.UndefOr[js.Any],
    MainDictionary: js.UndefOr[js.Any],
    SuggestionMode: js.UndefOr[js.Any],
    CustomDictionary2: js.UndefOr[js.Any],
    CustomDictionary3: js.UndefOr[js.Any],
    CustomDictionary4: js.UndefOr[js.Any],
    CustomDictionary5: js.UndefOr[js.Any],
    CustomDictionary6: js.UndefOr[js.Any],
    CustomDictionary7: js.UndefOr[js.Any],
    CustomDictionary8: js.UndefOr[js.Any],
    CustomDictionary9: js.UndefOr[js.Any],
    CustomDictionary10: js.UndefOr[js.Any]
  ): SpellingSuggestions = js.native
  def GoBack(): scala.Unit = js.native
  def GoForward(): scala.Unit = js.native
  def Help(HelpType: js.Any): scala.Unit = js.native
  def HelpTool(): scala.Unit = js.native
  def InchesToPoints(Inches: scala.Double): scala.Double = js.native
  def International(Index: WdInternationalIndex): js.Any = js.native
  def IsObjectValid(Object: js.Any): scala.Boolean = js.native
  def KeyString(KeyCode: scala.Double): java.lang.String = js.native
  def KeyString(KeyCode: scala.Double, KeyCode2: js.Any): java.lang.String = js.native
  /** @param number [LangId=0] */
  def Keyboard(): scala.Double = js.native
  def Keyboard(LangId: scala.Double): scala.Double = js.native
  def KeyboardBidi(): scala.Unit = js.native
  def KeyboardLatin(): scala.Unit = js.native
  def KeysBoundTo(KeyCategory: WdKeyCategory, Command: java.lang.String): activexDashWordLib.WordNs.KeysBoundTo = js.native
  def KeysBoundTo(KeyCategory: WdKeyCategory, Command: java.lang.String, CommandParameter: js.Any): activexDashWordLib.WordNs.KeysBoundTo = js.native
  def LinesToPoints(Lines: scala.Double): scala.Double = js.native
  def ListCommands(ListAllCommands: scala.Boolean): scala.Unit = js.native
  def LoadMasterList(FileName: java.lang.String): scala.Unit = js.native
  def LookupNameProperties(Name: java.lang.String): scala.Unit = js.native
  /**
    * @param Word.WdCompareDestination [Destination=2]
    * @param Word.WdGranularity [Granularity=1]
    * @param boolean [CompareFormatting=true]
    * @param boolean [CompareCaseChanges=true]
    * @param boolean [CompareWhitespace=true]
    * @param boolean [CompareTables=true]
    * @param boolean [CompareHeaders=true]
    * @param boolean [CompareFootnotes=true]
    * @param boolean [CompareTextboxes=true]
    * @param boolean [CompareFields=true]
    * @param boolean [CompareComments=true]
    * @param boolean [CompareMoves=true]
    * @param string [OriginalAuthor='']
    * @param string [RevisedAuthor='']
    * @param Word.WdMergeFormatFrom [FormatFrom=2]
    */
  def MergeDocuments(
    OriginalDocument: Document,
    RevisedDocument: Document,
    Destination: js.UndefOr[WdCompareDestination],
    Granularity: js.UndefOr[WdGranularity],
    CompareFormatting: js.UndefOr[scala.Boolean],
    CompareCaseChanges: js.UndefOr[scala.Boolean],
    CompareWhitespace: js.UndefOr[scala.Boolean],
    CompareTables: js.UndefOr[scala.Boolean],
    CompareHeaders: js.UndefOr[scala.Boolean],
    CompareFootnotes: js.UndefOr[scala.Boolean],
    CompareTextboxes: js.UndefOr[scala.Boolean],
    CompareFields: js.UndefOr[scala.Boolean],
    CompareComments: js.UndefOr[scala.Boolean],
    CompareMoves: js.UndefOr[scala.Boolean],
    OriginalAuthor: js.UndefOr[java.lang.String],
    RevisedAuthor: js.UndefOr[java.lang.String],
    FormatFrom: js.UndefOr[WdMergeFormatFrom]
  ): Document = js.native
  def MillimetersToPoints(Millimeters: scala.Double): scala.Double = js.native
  def MountVolume(Zone: java.lang.String, Server: java.lang.String, Volume: java.lang.String): scala.Double = js.native
  def MountVolume(Zone: java.lang.String, Server: java.lang.String, Volume: java.lang.String, User: js.Any): scala.Double = js.native
  def MountVolume(
    Zone: java.lang.String,
    Server: java.lang.String,
    Volume: java.lang.String,
    User: js.Any,
    UserPassword: js.Any
  ): scala.Double = js.native
  def MountVolume(
    Zone: java.lang.String,
    Server: java.lang.String,
    Volume: java.lang.String,
    User: js.Any,
    UserPassword: js.Any,
    VolumePassword: js.Any
  ): scala.Double = js.native
  def Move(Left: scala.Double, Top: scala.Double): scala.Unit = js.native
  def NewWindow(): Window = js.native
  def NextLetter(): scala.Unit = js.native
  def OnTime(When: js.Any, Name: java.lang.String): scala.Unit = js.native
  def OnTime(When: js.Any, Name: java.lang.String, Tolerance: js.Any): scala.Unit = js.native
  def OrganizerCopy(
    Source: java.lang.String,
    Destination: java.lang.String,
    Name: java.lang.String,
    Object: WdOrganizerObject
  ): scala.Unit = js.native
  def OrganizerDelete(Source: java.lang.String, Name: java.lang.String, Object: WdOrganizerObject): scala.Unit = js.native
  def OrganizerRename(
    Source: java.lang.String,
    Name: java.lang.String,
    NewName: java.lang.String,
    Object: WdOrganizerObject
  ): scala.Unit = js.native
  def PicasToPoints(Picas: scala.Double): scala.Double = js.native
  def PixelsToPoints(Pixels: scala.Double): scala.Double = js.native
  def PixelsToPoints(Pixels: scala.Double, fVertical: js.Any): scala.Double = js.native
  def PointsToCentimeters(Points: scala.Double): scala.Double = js.native
  def PointsToInches(Points: scala.Double): scala.Double = js.native
  def PointsToLines(Points: scala.Double): scala.Double = js.native
  def PointsToMillimeters(Points: scala.Double): scala.Double = js.native
  def PointsToPicas(Points: scala.Double): scala.Double = js.native
  def PointsToPixels(Points: scala.Double): scala.Double = js.native
  def PointsToPixels(Points: scala.Double, fVertical: js.Any): scala.Double = js.native
  def PrintOut(
    Background: js.UndefOr[js.Any],
    Append: js.UndefOr[js.Any],
    Range: js.UndefOr[js.Any],
    OutputFileName: js.UndefOr[js.Any],
    From: js.UndefOr[js.Any],
    To: js.UndefOr[js.Any],
    Item: js.UndefOr[js.Any],
    Copies: js.UndefOr[js.Any],
    Pages: js.UndefOr[js.Any],
    PageType: js.UndefOr[js.Any],
    PrintToFile: js.UndefOr[js.Any],
    Collate: js.UndefOr[js.Any],
    FileName: js.UndefOr[js.Any],
    ActivePrinterMacGX: js.UndefOr[js.Any],
    ManualDuplexPrint: js.UndefOr[js.Any],
    PrintZoomColumn: js.UndefOr[js.Any],
    PrintZoomRow: js.UndefOr[js.Any],
    PrintZoomPaperWidth: js.UndefOr[js.Any],
    PrintZoomPaperHeight: js.UndefOr[js.Any]
  ): scala.Unit = js.native
  def PrintOut2000(
    Background: js.UndefOr[js.Any],
    Append: js.UndefOr[js.Any],
    Range: js.UndefOr[js.Any],
    OutputFileName: js.UndefOr[js.Any],
    From: js.UndefOr[js.Any],
    To: js.UndefOr[js.Any],
    Item: js.UndefOr[js.Any],
    Copies: js.UndefOr[js.Any],
    Pages: js.UndefOr[js.Any],
    PageType: js.UndefOr[js.Any],
    PrintToFile: js.UndefOr[js.Any],
    Collate: js.UndefOr[js.Any],
    FileName: js.UndefOr[js.Any],
    ActivePrinterMacGX: js.UndefOr[js.Any],
    ManualDuplexPrint: js.UndefOr[js.Any],
    PrintZoomColumn: js.UndefOr[js.Any],
    PrintZoomRow: js.UndefOr[js.Any],
    PrintZoomPaperWidth: js.UndefOr[js.Any],
    PrintZoomPaperHeight: js.UndefOr[js.Any]
  ): scala.Unit = js.native
  def PrintOutOld(
    Background: js.UndefOr[js.Any],
    Append: js.UndefOr[js.Any],
    Range: js.UndefOr[js.Any],
    OutputFileName: js.UndefOr[js.Any],
    From: js.UndefOr[js.Any],
    To: js.UndefOr[js.Any],
    Item: js.UndefOr[js.Any],
    Copies: js.UndefOr[js.Any],
    Pages: js.UndefOr[js.Any],
    PageType: js.UndefOr[js.Any],
    PrintToFile: js.UndefOr[js.Any],
    Collate: js.UndefOr[js.Any],
    FileName: js.UndefOr[js.Any],
    ActivePrinterMacGX: js.UndefOr[js.Any],
    ManualDuplexPrint: js.UndefOr[js.Any]
  ): scala.Unit = js.native
  def ProductCode(): java.lang.String = js.native
  def PutFocusInMailHeader(): scala.Unit = js.native
  def Quit(): scala.Unit = js.native
  def Quit(SaveChanges: js.Any): scala.Unit = js.native
  def Quit(SaveChanges: js.Any, OriginalFormat: js.Any): scala.Unit = js.native
  def Quit(SaveChanges: js.Any, OriginalFormat: js.Any, RouteDocument: js.Any): scala.Unit = js.native
  def Repeat(): scala.Boolean = js.native
  def Repeat(Times: js.Any): scala.Boolean = js.native
  def ResetIgnoreAll(): scala.Unit = js.native
  def Resize(Width: scala.Double, Height: scala.Double): scala.Unit = js.native
  def Run(
    MacroName: java.lang.String,
    varg1: js.UndefOr[js.Any],
    varg2: js.UndefOr[js.Any],
    varg3: js.UndefOr[js.Any],
    varg4: js.UndefOr[js.Any],
    varg5: js.UndefOr[js.Any],
    varg6: js.UndefOr[js.Any],
    varg7: js.UndefOr[js.Any],
    varg8: js.UndefOr[js.Any],
    varg9: js.UndefOr[js.Any],
    varg10: js.UndefOr[js.Any],
    varg11: js.UndefOr[js.Any],
    varg12: js.UndefOr[js.Any],
    varg13: js.UndefOr[js.Any],
    varg14: js.UndefOr[js.Any],
    varg15: js.UndefOr[js.Any],
    varg16: js.UndefOr[js.Any],
    varg17: js.UndefOr[js.Any],
    varg18: js.UndefOr[js.Any],
    varg19: js.UndefOr[js.Any],
    varg20: js.UndefOr[js.Any],
    varg21: js.UndefOr[js.Any],
    varg22: js.UndefOr[js.Any],
    varg23: js.UndefOr[js.Any],
    varg24: js.UndefOr[js.Any],
    varg25: js.UndefOr[js.Any],
    varg26: js.UndefOr[js.Any],
    varg27: js.UndefOr[js.Any],
    varg28: js.UndefOr[js.Any],
    varg29: js.UndefOr[js.Any],
    varg30: js.UndefOr[js.Any]
  ): js.Any = js.native
  def RunOld(MacroName: java.lang.String): scala.Unit = js.native
  def ScreenRefresh(): scala.Unit = js.native
  def SendFax(): scala.Unit = js.native
  def SetDefaultTheme(Name: java.lang.String, DocumentType: WdDocumentMedium): scala.Unit = js.native
  def ShowClipboard(): scala.Unit = js.native
  def ShowMe(): scala.Unit = js.native
  def SmartArtColors(Index: java.lang.String): activexDashOfficeLib.OfficeNs.SmartArtColor = js.native
  def SmartArtColors(Index: scala.Double): activexDashOfficeLib.OfficeNs.SmartArtColor = js.native
  def SmartArtLayouts(Index: java.lang.String): activexDashOfficeLib.OfficeNs.SmartArtLayout = js.native
  def SmartArtLayouts(Index: scala.Double): activexDashOfficeLib.OfficeNs.SmartArtLayout = js.native
  def SmartArtQuickStyles(Index: java.lang.String): activexDashOfficeLib.OfficeNs.SmartArtQuickStyle = js.native
  def SmartArtQuickStyles(Index: scala.Double): activexDashOfficeLib.OfficeNs.SmartArtQuickStyle = js.native
  def SubstituteFont(UnavailableFont: java.lang.String, SubstituteFont: java.lang.String): scala.Unit = js.native
  def SynonymInfo(Word: java.lang.String): activexDashWordLib.WordNs.SynonymInfo = js.native
  def SynonymInfo(Word: java.lang.String, LanguageID: js.Any): activexDashWordLib.WordNs.SynonymInfo = js.native
  def ThreeWayMerge(
    LocalDocument: Document,
    ServerDocument: Document,
    BaseDocument: Document,
    FavorSource: scala.Boolean
  ): scala.Unit = js.native
  def ToggleKeyboard(): scala.Unit = js.native
}

