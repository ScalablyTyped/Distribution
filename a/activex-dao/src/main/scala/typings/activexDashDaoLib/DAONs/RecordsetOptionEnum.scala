package typings
package activexDashDaoLib.DAONs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - activexDashDaoLib.activexDashDaoLibNumbers.`8`
  - activexDashDaoLib.activexDashDaoLibNumbers.`32`
  - activexDashDaoLib.activexDashDaoLibNumbers.`2`
  - activexDashDaoLib.activexDashDaoLibNumbers.`1`
  - activexDashDaoLib.activexDashDaoLibNumbers.`2048`
  - activexDashDaoLib.activexDashDaoLibNumbers.`128`
  - activexDashDaoLib.activexDashDaoLibNumbers.`256`
  - activexDashDaoLib.activexDashDaoLibNumbers.`16`
  - activexDashDaoLib.activexDashDaoLibNumbers.`4`
  - activexDashDaoLib.activexDashDaoLibNumbers.`1024`
  - activexDashDaoLib.activexDashDaoLibNumbers.`512`
  - activexDashDaoLib.activexDashDaoLibNumbers.`64`
*/
trait RecordsetOptionEnum extends js.Object

