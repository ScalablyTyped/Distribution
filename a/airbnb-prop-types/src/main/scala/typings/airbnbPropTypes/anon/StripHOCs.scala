package typings.airbnbPropTypes.anon

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait StripHOCs extends js.Object {
  var stripHOCs: js.Array[String] = js.native
}

object StripHOCs {
  @scala.inline
  def apply(stripHOCs: js.Array[String]): StripHOCs = {
    val __obj = js.Dynamic.literal(stripHOCs = stripHOCs.asInstanceOf[js.Any])
    __obj.asInstanceOf[StripHOCs]
  }
  @scala.inline
  implicit class StripHOCsOps[Self <: StripHOCs] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def set(key: String, value: js.Any): Self = {
        x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
        x
    }
    @scala.inline
    def setStripHOCsVarargs(value: String*): Self = this.set("stripHOCs", js.Array(value :_*))
    @scala.inline
    def setStripHOCs(value: js.Array[String]): Self = this.set("stripHOCs", value.asInstanceOf[js.Any])
  }
  
}

