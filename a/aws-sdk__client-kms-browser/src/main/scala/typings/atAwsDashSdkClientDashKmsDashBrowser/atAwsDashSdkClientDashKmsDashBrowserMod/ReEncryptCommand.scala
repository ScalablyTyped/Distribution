package typings.atAwsDashSdkClientDashKmsDashBrowser.atAwsDashSdkClientDashKmsDashBrowserMod

import typings.atAwsDashSdkClientDashKmsDashBrowser.typesReEncryptInputMod.ReEncryptInput
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@aws-sdk/client-kms-browser", "ReEncryptCommand")
@js.native
class ReEncryptCommand protected ()
  extends typings.atAwsDashSdkClientDashKmsDashBrowser.commandsReEncryptCommandMod.ReEncryptCommand {
  def this(input: ReEncryptInput) = this()
}

