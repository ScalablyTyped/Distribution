package typings.activexDashExcel.ExcelNs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("Excel.Slicer")
@js.native
class Slicer protected () extends js.Object {
  val ActiveItem: SlicerItem = js.native
  val Application: typings.activexDashExcel.ExcelNs.Application = js.native
  var Caption: String = js.native
  var ColumnWidth: Double = js.native
  val Creator: XlCreator = js.native
  var DisableMoveResizeUI: Boolean = js.native
  var DisplayHeader: Boolean = js.native
  var `Excel.Slicer_typekey`: Slicer = js.native
  var Height: Double = js.native
  var Left: Double = js.native
  var Locked: Boolean = js.native
  var Name: String = js.native
  var NumberOfColumns: Double = js.native
  val Parent: js.Any = js.native
  var RowHeight: Double = js.native
  val Shape: typings.activexDashExcel.ExcelNs.Shape = js.native
  val SlicerCache: typings.activexDashExcel.ExcelNs.SlicerCache = js.native
  val SlicerCacheLevel: typings.activexDashExcel.ExcelNs.SlicerCacheLevel = js.native
  var Style: js.Any = js.native
  var Top: Double = js.native
  var Width: Double = js.native
  def Copy(): Unit = js.native
  def Cut(): Unit = js.native
  def Delete(): Unit = js.native
}

