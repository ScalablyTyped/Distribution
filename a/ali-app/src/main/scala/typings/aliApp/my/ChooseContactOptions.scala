...
object ChooseContactOptions {
  ...
  inline def apply(chooseType: single | multi | String, success: ContactsDicArray => Unit): ChooseContactOptions = {
  ...
  implicit open class MutableBuilder[Self <: ChooseContactOptions] (val x: Self) extends AnyVal {
    ...
    inline def setSuccess(value: ContactsDicArray => Unit): Self = StObject.set(x, "success", js.Any.fromFunction1(value))
  }
}
