package typings.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.IMsoChartArea")
@js.native
/* private */ class IMsoChartArea ()
  extends StObject
     with typings.activexOffice.Office.IMsoChartArea {
  
  /* CompleteClass */
  override val Application: js.Any = js.native
  
  /* CompleteClass */
  var AutoScaleFont: js.Any = js.native
  
  /* CompleteClass */
  override val Border: typings.activexOffice.Office.IMsoBorder = js.native
  
  /* CompleteClass */
  override def Clear(): js.Any = js.native
  
  /* CompleteClass */
  override def ClearContents(): js.Any = js.native
  
  /* CompleteClass */
  override def ClearFormats(): js.Any = js.native
  
  /* CompleteClass */
  override def Copy(): js.Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val Fill: typings.activexOffice.Office.ChartFillFormat = js.native
  
  /* CompleteClass */
  override val Font: typings.activexOffice.Office.ChartFont = js.native
  
  /* CompleteClass */
  override val Format: typings.activexOffice.Office.IMsoChartFormat = js.native
  
  /* CompleteClass */
  var Height: Double = js.native
  
  /* CompleteClass */
  override val Interior: typings.activexOffice.Office.IMsoInterior = js.native
  
  /* CompleteClass */
  var Left: Double = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.IMsoChartArea_typekey")
  var OfficeDotIMsoChartArea_typekey: typings.activexOffice.Office.IMsoChartArea = js.native
  
  /* CompleteClass */
  override val Parent: js.Any = js.native
  
  /* CompleteClass */
  var RoundedCorners: Boolean = js.native
  
  /* CompleteClass */
  override def Select(): js.Any = js.native
  
  /* CompleteClass */
  var Shadow: Boolean = js.native
  
  /* CompleteClass */
  var Top: Double = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
}
