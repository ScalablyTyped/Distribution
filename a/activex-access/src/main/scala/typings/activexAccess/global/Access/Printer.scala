package typings.activexAccess.global.Access

import typings.activexAccess.Access.AcPrintColor
import typings.activexAccess.Access.AcPrintDuplex
import typings.activexAccess.Access.AcPrintItemLayout
import typings.activexAccess.Access.AcPrintObjQuality
import typings.activexAccess.Access.AcPrintOrientation
import typings.activexAccess.Access.AcPrintPaperBin
import typings.activexAccess.Access.AcPrintPaperSize
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Access.Printer")
@js.native
class Printer protected ()
  extends StObject
     with typings.activexAccess.Access.Printer {
  
  /* CompleteClass */
  @JSName("Access.Printer_typekey")
  var AccessDotPrinter_typekey: typings.activexAccess.Access.Printer = js.native
  
  /* CompleteClass */
  var BottomMargin: Double = js.native
  
  /* CompleteClass */
  var ColorMode: AcPrintColor = js.native
  
  /* CompleteClass */
  var ColumnSpacing: Double = js.native
  
  /* CompleteClass */
  var Copies: Double = js.native
  
  /* CompleteClass */
  var DataOnly: Boolean = js.native
  
  /* CompleteClass */
  var DefaultSize: Boolean = js.native
  
  /* CompleteClass */
  override val DeviceName: String = js.native
  
  /* CompleteClass */
  override val DriverName: String = js.native
  
  /* CompleteClass */
  var Duplex: AcPrintDuplex = js.native
  
  /* CompleteClass */
  override def IsMemberSafe(dispid: Double): Boolean = js.native
  
  /* CompleteClass */
  var ItemLayout: AcPrintItemLayout = js.native
  
  /* CompleteClass */
  var ItemSizeHeight: Double = js.native
  
  /* CompleteClass */
  var ItemSizeWidth: Double = js.native
  
  /* CompleteClass */
  var ItemsAcross: Double = js.native
  
  /* CompleteClass */
  var LeftMargin: Double = js.native
  
  /* CompleteClass */
  var Orientation: AcPrintOrientation = js.native
  
  /* CompleteClass */
  var PaperBin: AcPrintPaperBin = js.native
  
  /* CompleteClass */
  var PaperSize: AcPrintPaperSize = js.native
  
  /* CompleteClass */
  override val Port: String = js.native
  
  /* CompleteClass */
  var PrintQuality: AcPrintObjQuality = js.native
  
  /* CompleteClass */
  var RightMargin: Double = js.native
  
  /* CompleteClass */
  var RowSpacing: Double = js.native
  
  /* CompleteClass */
  var TopMargin: Double = js.native
}
