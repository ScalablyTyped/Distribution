...
object ssoSilentRequestMod {
  
  /* Inlined std.Partial<std.Omit<@azure/msal-common.@azure/msal-common.CommonAuthorizationUrlRequest, 'responseMode' | 'codeChallenge' | 'codeChallengeMethod' | 'requestedClaimsHash' | 'nativeBroker'>> */
  trait SsoSilentRequest
    extends StObject
       with _EventPayload {
  ...
  object SsoSilentRequest {
    
    inline def apply(): SsoSilentRequest = {
      val __obj = js.Dynamic.literal()
      __obj.asInstanceOf[SsoSilentRequest]
    }
    
    extension [Self <: SsoSilentRequest](x: Self) {
