package typings.activexShdocvw

import typings.activexShdocvw.SHDocVw.BrowserBarConstants
import typings.activexShdocvw.SHDocVw.BrowserNavConstants
import typings.activexShdocvw.SHDocVw.CommandStateChangeConstants
import typings.activexShdocvw.SHDocVw.OLECMDEXECOPT
import typings.activexShdocvw.SHDocVw.OLECMDF
import typings.activexShdocvw.SHDocVw.OLECMDID
import typings.activexShdocvw.SHDocVw.RefreshConstants
import typings.activexShdocvw.SHDocVw.SecureLockIconConstants
import typings.activexShdocvw.SHDocVw.ShellWindowFindWindowOptions
import typings.activexShdocvw.SHDocVw.ShellWindowTypeConstants
import typings.activexShdocvw.SHDocVw.tagREADYSTATE
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object activexShdocvwInts {
  
  @js.native
  sealed trait `-1`
    extends StObject
       with CommandStateChangeConstants
  inline def `-1`: `-1` = -1.asInstanceOf[`-1`]
  
  @js.native
  sealed trait `0`
    extends StObject
       with OLECMDEXECOPT
       with RefreshConstants
       with SecureLockIconConstants
       with ShellWindowTypeConstants
       with tagREADYSTATE
  inline def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `0x0009`
    extends StObject
       with BrowserBarConstants
  inline def `0x0009`: `0x0009` = 0x0009.asInstanceOf[`0x0009`]
  
  @js.native
  sealed trait `0x000A`
    extends StObject
       with BrowserBarConstants
  inline def `0x000A`: `0x000A` = 0x000A.asInstanceOf[`0x000A`]
  
  @js.native
  sealed trait `0x000B`
    extends StObject
       with BrowserBarConstants
  inline def `0x000B`: `0x000B` = 0x000B.asInstanceOf[`0x000B`]
  
  @js.native
  sealed trait `1`
    extends StObject
       with BrowserNavConstants
       with CommandStateChangeConstants
       with OLECMDEXECOPT
       with OLECMDF
       with OLECMDID
       with RefreshConstants
       with SecureLockIconConstants
       with ShellWindowFindWindowOptions
       with ShellWindowTypeConstants
       with tagREADYSTATE
  inline def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `10`
    extends StObject
       with OLECMDID
  inline def `10`: `10` = 10.asInstanceOf[`10`]
  
  @js.native
  sealed trait `1024`
    extends StObject
       with BrowserNavConstants
  inline def `1024`: `1024` = 1024.asInstanceOf[`1024`]
  
  @js.native
  sealed trait `11`
    extends StObject
       with OLECMDID
  inline def `11`: `11` = 11.asInstanceOf[`11`]
  
  @js.native
  sealed trait `12`
    extends StObject
       with OLECMDID
  inline def `12`: `12` = 12.asInstanceOf[`12`]
  
  @js.native
  sealed trait `128`
    extends StObject
       with BrowserNavConstants
  inline def `128`: `128` = 128.asInstanceOf[`128`]
  
  @js.native
  sealed trait `13`
    extends StObject
       with OLECMDID
  inline def `13`: `13` = 13.asInstanceOf[`13`]
  
  @js.native
  sealed trait `14`
    extends StObject
       with OLECMDID
  inline def `14`: `14` = 14.asInstanceOf[`14`]
  
  @js.native
  sealed trait `15`
    extends StObject
       with OLECMDID
  inline def `15`: `15` = 15.asInstanceOf[`15`]
  
  @js.native
  sealed trait `16`
    extends StObject
       with BrowserNavConstants
       with OLECMDF
       with OLECMDID
  inline def `16`: `16` = 16.asInstanceOf[`16`]
  
  @js.native
  sealed trait `16384`
    extends StObject
       with BrowserNavConstants
  inline def `16384`: `16384` = 16384.asInstanceOf[`16384`]
  
  @js.native
  sealed trait `17`
    extends StObject
       with OLECMDID
  inline def `17`: `17` = 17.asInstanceOf[`17`]
  
  @js.native
  sealed trait `18`
    extends StObject
       with OLECMDID
  inline def `18`: `18` = 18.asInstanceOf[`18`]
  
  @js.native
  sealed trait `19`
    extends StObject
       with OLECMDID
  inline def `19`: `19` = 19.asInstanceOf[`19`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with BrowserNavConstants
       with CommandStateChangeConstants
       with OLECMDEXECOPT
       with OLECMDF
       with OLECMDID
       with SecureLockIconConstants
       with ShellWindowFindWindowOptions
       with ShellWindowTypeConstants
       with tagREADYSTATE
  inline def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `20`
    extends StObject
       with OLECMDID
  inline def `20`: `20` = 20.asInstanceOf[`20`]
  
  @js.native
  sealed trait `2048`
    extends StObject
       with BrowserNavConstants
  inline def `2048`: `2048` = 2048.asInstanceOf[`2048`]
  
  @js.native
  sealed trait `21`
    extends StObject
       with OLECMDID
  inline def `21`: `21` = 21.asInstanceOf[`21`]
  
  @js.native
  sealed trait `22`
    extends StObject
       with OLECMDID
  inline def `22`: `22` = 22.asInstanceOf[`22`]
  
  @js.native
  sealed trait `23`
    extends StObject
       with OLECMDID
  inline def `23`: `23` = 23.asInstanceOf[`23`]
  
  @js.native
  sealed trait `24`
    extends StObject
       with OLECMDID
  inline def `24`: `24` = 24.asInstanceOf[`24`]
  
  @js.native
  sealed trait `25`
    extends StObject
       with OLECMDID
  inline def `25`: `25` = 25.asInstanceOf[`25`]
  
  @js.native
  sealed trait `256`
    extends StObject
       with BrowserNavConstants
  inline def `256`: `256` = 256.asInstanceOf[`256`]
  
  @js.native
  sealed trait `26`
    extends StObject
       with OLECMDID
  inline def `26`: `26` = 26.asInstanceOf[`26`]
  
  @js.native
  sealed trait `27`
    extends StObject
       with OLECMDID
  inline def `27`: `27` = 27.asInstanceOf[`27`]
  
  @js.native
  sealed trait `28`
    extends StObject
       with OLECMDID
  inline def `28`: `28` = 28.asInstanceOf[`28`]
  
  @js.native
  sealed trait `29`
    extends StObject
       with OLECMDID
  inline def `29`: `29` = 29.asInstanceOf[`29`]
  
  @js.native
  sealed trait `3`
    extends StObject
       with OLECMDEXECOPT
       with OLECMDID
       with RefreshConstants
       with SecureLockIconConstants
       with tagREADYSTATE
  inline def `3`: `3` = 3.asInstanceOf[`3`]
  
  @js.native
  sealed trait `30`
    extends StObject
       with OLECMDID
  inline def `30`: `30` = 30.asInstanceOf[`30`]
  
  @js.native
  sealed trait `31`
    extends StObject
       with OLECMDID
  inline def `31`: `31` = 31.asInstanceOf[`31`]
  
  @js.native
  sealed trait `32`
    extends StObject
       with BrowserNavConstants
       with OLECMDF
       with OLECMDID
  inline def `32`: `32` = 32.asInstanceOf[`32`]
  
  @js.native
  sealed trait `32768`
    extends StObject
       with BrowserNavConstants
  inline def `32768`: `32768` = 32768.asInstanceOf[`32768`]
  
  @js.native
  sealed trait `33`
    extends StObject
       with OLECMDID
  inline def `33`: `33` = 33.asInstanceOf[`33`]
  
  @js.native
  sealed trait `34`
    extends StObject
       with OLECMDID
  inline def `34`: `34` = 34.asInstanceOf[`34`]
  
  @js.native
  sealed trait `35`
    extends StObject
       with OLECMDID
  inline def `35`: `35` = 35.asInstanceOf[`35`]
  
  @js.native
  sealed trait `36`
    extends StObject
       with OLECMDID
  inline def `36`: `36` = 36.asInstanceOf[`36`]
  
  @js.native
  sealed trait `37`
    extends StObject
       with OLECMDID
  inline def `37`: `37` = 37.asInstanceOf[`37`]
  
  @js.native
  sealed trait `38`
    extends StObject
       with OLECMDID
  inline def `38`: `38` = 38.asInstanceOf[`38`]
  
  @js.native
  sealed trait `39`
    extends StObject
       with OLECMDID
  inline def `39`: `39` = 39.asInstanceOf[`39`]
  
  @js.native
  sealed trait `4`
    extends StObject
       with OLECMDF
       with OLECMDID
       with SecureLockIconConstants
       with ShellWindowFindWindowOptions
       with ShellWindowTypeConstants
       with tagREADYSTATE
  inline def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `40`
    extends StObject
       with OLECMDID
  inline def `40`: `40` = 40.asInstanceOf[`40`]
  
  @js.native
  sealed trait `4096`
    extends StObject
       with BrowserNavConstants
  inline def `4096`: `4096` = 4096.asInstanceOf[`4096`]
  
  @js.native
  sealed trait `41`
    extends StObject
       with OLECMDID
  inline def `41`: `41` = 41.asInstanceOf[`41`]
  
  @js.native
  sealed trait `42`
    extends StObject
       with OLECMDID
  inline def `42`: `42` = 42.asInstanceOf[`42`]
  
  @js.native
  sealed trait `43`
    extends StObject
       with OLECMDID
  inline def `43`: `43` = 43.asInstanceOf[`43`]
  
  @js.native
  sealed trait `44`
    extends StObject
       with OLECMDID
  inline def `44`: `44` = 44.asInstanceOf[`44`]
  
  @js.native
  sealed trait `45`
    extends StObject
       with OLECMDID
  inline def `45`: `45` = 45.asInstanceOf[`45`]
  
  @js.native
  sealed trait `46`
    extends StObject
       with OLECMDID
  inline def `46`: `46` = 46.asInstanceOf[`46`]
  
  @js.native
  sealed trait `47`
    extends StObject
       with OLECMDID
  inline def `47`: `47` = 47.asInstanceOf[`47`]
  
  @js.native
  sealed trait `48`
    extends StObject
       with OLECMDID
  inline def `48`: `48` = 48.asInstanceOf[`48`]
  
  @js.native
  sealed trait `49`
    extends StObject
       with OLECMDID
  inline def `49`: `49` = 49.asInstanceOf[`49`]
  
  @js.native
  sealed trait `5`
    extends StObject
       with OLECMDID
       with SecureLockIconConstants
  inline def `5`: `5` = 5.asInstanceOf[`5`]
  
  @js.native
  sealed trait `50`
    extends StObject
       with OLECMDID
  inline def `50`: `50` = 50.asInstanceOf[`50`]
  
  @js.native
  sealed trait `51`
    extends StObject
       with OLECMDID
  inline def `51`: `51` = 51.asInstanceOf[`51`]
  
  @js.native
  sealed trait `512`
    extends StObject
       with BrowserNavConstants
  inline def `512`: `512` = 512.asInstanceOf[`512`]
  
  @js.native
  sealed trait `52`
    extends StObject
       with OLECMDID
  inline def `52`: `52` = 52.asInstanceOf[`52`]
  
  @js.native
  sealed trait `55`
    extends StObject
       with OLECMDID
  inline def `55`: `55` = 55.asInstanceOf[`55`]
  
  @js.native
  sealed trait `56`
    extends StObject
       with OLECMDID
  inline def `56`: `56` = 56.asInstanceOf[`56`]
  
  @js.native
  sealed trait `57`
    extends StObject
       with OLECMDID
  inline def `57`: `57` = 57.asInstanceOf[`57`]
  
  @js.native
  sealed trait `58`
    extends StObject
       with OLECMDID
  inline def `58`: `58` = 58.asInstanceOf[`58`]
  
  @js.native
  sealed trait `59`
    extends StObject
       with OLECMDID
  inline def `59`: `59` = 59.asInstanceOf[`59`]
  
  @js.native
  sealed trait `6`
    extends StObject
       with OLECMDID
       with SecureLockIconConstants
  inline def `6`: `6` = 6.asInstanceOf[`6`]
  
  @js.native
  sealed trait `60`
    extends StObject
       with OLECMDID
  inline def `60`: `60` = 60.asInstanceOf[`60`]
  
  @js.native
  sealed trait `61`
    extends StObject
       with OLECMDID
  inline def `61`: `61` = 61.asInstanceOf[`61`]
  
  @js.native
  sealed trait `62`
    extends StObject
       with OLECMDID
  inline def `62`: `62` = 62.asInstanceOf[`62`]
  
  @js.native
  sealed trait `63`
    extends StObject
       with OLECMDID
  inline def `63`: `63` = 63.asInstanceOf[`63`]
  
  @js.native
  sealed trait `64`
    extends StObject
       with BrowserNavConstants
       with OLECMDID
  inline def `64`: `64` = 64.asInstanceOf[`64`]
  
  @js.native
  sealed trait `65`
    extends StObject
       with OLECMDID
  inline def `65`: `65` = 65.asInstanceOf[`65`]
  
  @js.native
  sealed trait `65536`
    extends StObject
       with BrowserNavConstants
  inline def `65536`: `65536` = 65536.asInstanceOf[`65536`]
  
  @js.native
  sealed trait `66`
    extends StObject
       with OLECMDID
  inline def `66`: `66` = 66.asInstanceOf[`66`]
  
  @js.native
  sealed trait `67`
    extends StObject
       with OLECMDID
  inline def `67`: `67` = 67.asInstanceOf[`67`]
  
  @js.native
  sealed trait `68`
    extends StObject
       with OLECMDID
  inline def `68`: `68` = 68.asInstanceOf[`68`]
  
  @js.native
  sealed trait `69`
    extends StObject
       with OLECMDID
  inline def `69`: `69` = 69.asInstanceOf[`69`]
  
  @js.native
  sealed trait `7`
    extends StObject
       with OLECMDID
  inline def `7`: `7` = 7.asInstanceOf[`7`]
  
  @js.native
  sealed trait `70`
    extends StObject
       with OLECMDID
  inline def `70`: `70` = 70.asInstanceOf[`70`]
  
  @js.native
  sealed trait `71`
    extends StObject
       with OLECMDID
  inline def `71`: `71` = 71.asInstanceOf[`71`]
  
  @js.native
  sealed trait `72`
    extends StObject
       with OLECMDID
  inline def `72`: `72` = 72.asInstanceOf[`72`]
  
  @js.native
  sealed trait `73`
    extends StObject
       with OLECMDID
  inline def `73`: `73` = 73.asInstanceOf[`73`]
  
  @js.native
  sealed trait `74`
    extends StObject
       with OLECMDID
  inline def `74`: `74` = 74.asInstanceOf[`74`]
  
  @js.native
  sealed trait `75`
    extends StObject
       with OLECMDID
  inline def `75`: `75` = 75.asInstanceOf[`75`]
  
  @js.native
  sealed trait `76`
    extends StObject
       with OLECMDID
  inline def `76`: `76` = 76.asInstanceOf[`76`]
  
  @js.native
  sealed trait `77`
    extends StObject
       with OLECMDID
  inline def `77`: `77` = 77.asInstanceOf[`77`]
  
  @js.native
  sealed trait `78`
    extends StObject
       with OLECMDID
  inline def `78`: `78` = 78.asInstanceOf[`78`]
  
  @js.native
  sealed trait `79`
    extends StObject
       with OLECMDID
  inline def `79`: `79` = 79.asInstanceOf[`79`]
  
  @js.native
  sealed trait `8`
    extends StObject
       with OLECMDF
       with OLECMDID
       with ShellWindowTypeConstants
  inline def `8`: `8` = 8.asInstanceOf[`8`]
  
  @js.native
  sealed trait `80`
    extends StObject
       with OLECMDID
  inline def `80`: `80` = 80.asInstanceOf[`80`]
  
  @js.native
  sealed trait `81`
    extends StObject
       with OLECMDID
  inline def `81`: `81` = 81.asInstanceOf[`81`]
  
  @js.native
  sealed trait `8192`
    extends StObject
       with BrowserNavConstants
  inline def `8192`: `8192` = 8192.asInstanceOf[`8192`]
  
  @js.native
  sealed trait `82`
    extends StObject
       with OLECMDID
  inline def `82`: `82` = 82.asInstanceOf[`82`]
  
  @js.native
  sealed trait `83`
    extends StObject
       with OLECMDID
  inline def `83`: `83` = 83.asInstanceOf[`83`]
  
  @js.native
  sealed trait `84`
    extends StObject
       with OLECMDID
  inline def `84`: `84` = 84.asInstanceOf[`84`]
  
  @js.native
  sealed trait `85`
    extends StObject
       with OLECMDID
  inline def `85`: `85` = 85.asInstanceOf[`85`]
  
  @js.native
  sealed trait `9`
    extends StObject
       with OLECMDID
  inline def `9`: `9` = 9.asInstanceOf[`9`]
}
