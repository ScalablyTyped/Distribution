...
/* Inlined react.react.CSSProperties & std.Partial<std.Record<'--border-color' | '--background-color' | '--text-color' | '--border-radius', string>> */
trait CSSPropertiesPartialRecorBackgroundcolor extends StObject {
...
object CSSPropertiesPartialRecorBackgroundcolor {
  
  inline def apply(): CSSPropertiesPartialRecorBackgroundcolor = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[CSSPropertiesPartialRecorBackgroundcolor]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: CSSPropertiesPartialRecorBackgroundcolor] (val x: Self) extends AnyVal {
