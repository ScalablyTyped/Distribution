package typings
package atAngularCoreLib.srcCoreMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@angular/core/src/core", "ɵRender3NgModuleRef")
@js.native
class ɵRender3NgModuleRef[T] protected ()
  extends atAngularCoreLib.srcCoreUnderscoreRender3UnderscorePrivateUnderscoreExportMod.ɵRender3NgModuleRef[T] {
  def this(ngModuleType: atAngularCoreLib.srcTypeMod.Type[T]) = this()
  def this(ngModuleType: atAngularCoreLib.srcTypeMod.Type[T], _parent: atAngularCoreLib.srcDiInjectorMod.Injector) = this()
}

