package typings.aphrodite

import typings.aphrodite.typingsCssPropertiesMod.CSSWideKeyword
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object aphroditeStrings {
  
  @js.native
  sealed trait `-ms-autohiding-scrollbar` extends StObject
  inline def `-ms-autohiding-scrollbar`: `-ms-autohiding-scrollbar` = "-ms-autohiding-scrollbar".asInstanceOf[`-ms-autohiding-scrollbar`]
  
  @js.native
  sealed trait ColonColonafter extends StObject
  inline def ColonColonafter: ColonColonafter = "::after".asInstanceOf[ColonColonafter]
  
  @js.native
  sealed trait ColonColonbackdrop extends StObject
  inline def ColonColonbackdrop: ColonColonbackdrop = "::backdrop".asInstanceOf[ColonColonbackdrop]
  
  @js.native
  sealed trait ColonColonbefore extends StObject
  inline def ColonColonbefore: ColonColonbefore = "::before".asInstanceOf[ColonColonbefore]
  
  @js.native
  sealed trait `ColonColonfirst-letter` extends StObject
  inline def `ColonColonfirst-letter`: `ColonColonfirst-letter` = "::first-letter".asInstanceOf[`ColonColonfirst-letter`]
  
  @js.native
  sealed trait `ColonColonfirst-line` extends StObject
  inline def `ColonColonfirst-line`: `ColonColonfirst-line` = "::first-line".asInstanceOf[`ColonColonfirst-line`]
  
  @js.native
  sealed trait ColonColonselection extends StObject
  inline def ColonColonselection: ColonColonselection = "::selection".asInstanceOf[ColonColonselection]
  
  @js.native
  sealed trait Colonactive extends StObject
  inline def Colonactive: Colonactive = ":active".asInstanceOf[Colonactive]
  
  @js.native
  sealed trait Colonafter extends StObject
  inline def Colonafter: Colonafter = ":after".asInstanceOf[Colonafter]
  
  @js.native
  sealed trait `Colonany-link` extends StObject
  inline def `Colonany-link`: `Colonany-link` = ":any-link".asInstanceOf[`Colonany-link`]
  
  @js.native
  sealed trait Colonbefore extends StObject
  inline def Colonbefore: Colonbefore = ":before".asInstanceOf[Colonbefore]
  
  @js.native
  sealed trait Colonchecked extends StObject
  inline def Colonchecked: Colonchecked = ":checked".asInstanceOf[Colonchecked]
  
  @js.native
  sealed trait Colondefault extends StObject
  inline def Colondefault: Colondefault = ":default".asInstanceOf[Colondefault]
  
  @js.native
  sealed trait ColondirLeftparenthesisltrRightparenthesis extends StObject
  inline def ColondirLeftparenthesisltrRightparenthesis: ColondirLeftparenthesisltrRightparenthesis = ":dir(ltr)".asInstanceOf[ColondirLeftparenthesisltrRightparenthesis]
  
  @js.native
  sealed trait ColondirLeftparenthesisrtlRightparenthesis extends StObject
  inline def ColondirLeftparenthesisrtlRightparenthesis: ColondirLeftparenthesisrtlRightparenthesis = ":dir(rtl)".asInstanceOf[ColondirLeftparenthesisrtlRightparenthesis]
  
  @js.native
  sealed trait Colondisabled extends StObject
  inline def Colondisabled: Colondisabled = ":disabled".asInstanceOf[Colondisabled]
  
  @js.native
  sealed trait Colonempty extends StObject
  inline def Colonempty: Colonempty = ":empty".asInstanceOf[Colonempty]
  
  @js.native
  sealed trait Colonenabled extends StObject
  inline def Colonenabled: Colonenabled = ":enabled".asInstanceOf[Colonenabled]
  
  @js.native
  sealed trait Colonfirst extends StObject
  inline def Colonfirst: Colonfirst = ":first".asInstanceOf[Colonfirst]
  
  @js.native
  sealed trait `Colonfirst-child` extends StObject
  inline def `Colonfirst-child`: `Colonfirst-child` = ":first-child".asInstanceOf[`Colonfirst-child`]
  
  @js.native
  sealed trait `Colonfirst-letter` extends StObject
  inline def `Colonfirst-letter`: `Colonfirst-letter` = ":first-letter".asInstanceOf[`Colonfirst-letter`]
  
  @js.native
  sealed trait `Colonfirst-line` extends StObject
  inline def `Colonfirst-line`: `Colonfirst-line` = ":first-line".asInstanceOf[`Colonfirst-line`]
  
  @js.native
  sealed trait `Colonfirst-of-type` extends StObject
  inline def `Colonfirst-of-type`: `Colonfirst-of-type` = ":first-of-type".asInstanceOf[`Colonfirst-of-type`]
  
  @js.native
  sealed trait Colonfocus extends StObject
  inline def Colonfocus: Colonfocus = ":focus".asInstanceOf[Colonfocus]
  
  @js.native
  sealed trait `Colonfocus-within` extends StObject
  inline def `Colonfocus-within`: `Colonfocus-within` = ":focus-within".asInstanceOf[`Colonfocus-within`]
  
  @js.native
  sealed trait Colonfullscreen extends StObject
  inline def Colonfullscreen: Colonfullscreen = ":fullscreen".asInstanceOf[Colonfullscreen]
  
  @js.native
  sealed trait Colonhover extends StObject
  inline def Colonhover: Colonhover = ":hover".asInstanceOf[Colonhover]
  
  @js.native
  sealed trait `Colonin-range` extends StObject
  inline def `Colonin-range`: `Colonin-range` = ":in-range".asInstanceOf[`Colonin-range`]
  
  @js.native
  sealed trait Colonindeterminate extends StObject
  inline def Colonindeterminate: Colonindeterminate = ":indeterminate".asInstanceOf[Colonindeterminate]
  
  @js.native
  sealed trait Coloninvalid extends StObject
  inline def Coloninvalid: Coloninvalid = ":invalid".asInstanceOf[Coloninvalid]
  
  @js.native
  sealed trait ColonlangLeftparenthesisdeRightparenthesis extends StObject
  inline def ColonlangLeftparenthesisdeRightparenthesis: ColonlangLeftparenthesisdeRightparenthesis = ":lang(de)".asInstanceOf[ColonlangLeftparenthesisdeRightparenthesis]
  
  @js.native
  sealed trait ColonlangLeftparenthesisenRightparenthesis extends StObject
  inline def ColonlangLeftparenthesisenRightparenthesis: ColonlangLeftparenthesisenRightparenthesis = ":lang(en)".asInstanceOf[ColonlangLeftparenthesisenRightparenthesis]
  
  @js.native
  sealed trait ColonlangLeftparenthesisesRightparenthesis extends StObject
  inline def ColonlangLeftparenthesisesRightparenthesis: ColonlangLeftparenthesisesRightparenthesis = ":lang(es)".asInstanceOf[ColonlangLeftparenthesisesRightparenthesis]
  
  @js.native
  sealed trait ColonlangLeftparenthesisfrRightparenthesis extends StObject
  inline def ColonlangLeftparenthesisfrRightparenthesis: ColonlangLeftparenthesisfrRightparenthesis = ":lang(fr)".asInstanceOf[ColonlangLeftparenthesisfrRightparenthesis]
  
  @js.native
  sealed trait ColonlangLeftparenthesisruRightparenthesis extends StObject
  inline def ColonlangLeftparenthesisruRightparenthesis: ColonlangLeftparenthesisruRightparenthesis = ":lang(ru)".asInstanceOf[ColonlangLeftparenthesisruRightparenthesis]
  
  @js.native
  sealed trait ColonlangLeftparenthesiszhRightparenthesis extends StObject
  inline def ColonlangLeftparenthesiszhRightparenthesis: ColonlangLeftparenthesiszhRightparenthesis = ":lang(zh)".asInstanceOf[ColonlangLeftparenthesiszhRightparenthesis]
  
  @js.native
  sealed trait `Colonlast-child` extends StObject
  inline def `Colonlast-child`: `Colonlast-child` = ":last-child".asInstanceOf[`Colonlast-child`]
  
  @js.native
  sealed trait `Colonlast-of-type` extends StObject
  inline def `Colonlast-of-type`: `Colonlast-of-type` = ":last-of-type".asInstanceOf[`Colonlast-of-type`]
  
  @js.native
  sealed trait Colonleft extends StObject
  inline def Colonleft: Colonleft = ":left".asInstanceOf[Colonleft]
  
  @js.native
  sealed trait Colonlink extends StObject
  inline def Colonlink: Colonlink = ":link".asInstanceOf[Colonlink]
  
  @js.native
  sealed trait `Colonnth-childLeftparenthesisevenRightparenthesis` extends StObject
  inline def `Colonnth-childLeftparenthesisevenRightparenthesis`: `Colonnth-childLeftparenthesisevenRightparenthesis` = ":nth-child(even)".asInstanceOf[`Colonnth-childLeftparenthesisevenRightparenthesis`]
  
  @js.native
  sealed trait `Colonnth-childLeftparenthesisoddRightparenthesis` extends StObject
  inline def `Colonnth-childLeftparenthesisoddRightparenthesis`: `Colonnth-childLeftparenthesisoddRightparenthesis` = ":nth-child(odd)".asInstanceOf[`Colonnth-childLeftparenthesisoddRightparenthesis`]
  
  @js.native
  sealed trait `Colonnth-last-childLeftparenthesisevenRightparenthesis` extends StObject
  inline def `Colonnth-last-childLeftparenthesisevenRightparenthesis`: `Colonnth-last-childLeftparenthesisevenRightparenthesis` = ":nth-last-child(even)".asInstanceOf[`Colonnth-last-childLeftparenthesisevenRightparenthesis`]
  
  @js.native
  sealed trait `Colonnth-last-childLeftparenthesisoddRightparenthesis` extends StObject
  inline def `Colonnth-last-childLeftparenthesisoddRightparenthesis`: `Colonnth-last-childLeftparenthesisoddRightparenthesis` = ":nth-last-child(odd)".asInstanceOf[`Colonnth-last-childLeftparenthesisoddRightparenthesis`]
  
  @js.native
  sealed trait `Colononly-child` extends StObject
  inline def `Colononly-child`: `Colononly-child` = ":only-child".asInstanceOf[`Colononly-child`]
  
  @js.native
  sealed trait `Colononly-of-type` extends StObject
  inline def `Colononly-of-type`: `Colononly-of-type` = ":only-of-type".asInstanceOf[`Colononly-of-type`]
  
  @js.native
  sealed trait Colonoptional extends StObject
  inline def Colonoptional: Colonoptional = ":optional".asInstanceOf[Colonoptional]
  
  @js.native
  sealed trait `Colonout-of-range` extends StObject
  inline def `Colonout-of-range`: `Colonout-of-range` = ":out-of-range".asInstanceOf[`Colonout-of-range`]
  
  @js.native
  sealed trait `Colonplaceholder-shown` extends StObject
  inline def `Colonplaceholder-shown`: `Colonplaceholder-shown` = ":placeholder-shown".asInstanceOf[`Colonplaceholder-shown`]
  
  @js.native
  sealed trait `Colonread-only` extends StObject
  inline def `Colonread-only`: `Colonread-only` = ":read-only".asInstanceOf[`Colonread-only`]
  
  @js.native
  sealed trait `Colonread-write` extends StObject
  inline def `Colonread-write`: `Colonread-write` = ":read-write".asInstanceOf[`Colonread-write`]
  
  @js.native
  sealed trait Colonrequired extends StObject
  inline def Colonrequired: Colonrequired = ":required".asInstanceOf[Colonrequired]
  
  @js.native
  sealed trait Colonright extends StObject
  inline def Colonright: Colonright = ":right".asInstanceOf[Colonright]
  
  @js.native
  sealed trait Colonroot extends StObject
  inline def Colonroot: Colonroot = ":root".asInstanceOf[Colonroot]
  
  @js.native
  sealed trait Colonscope extends StObject
  inline def Colonscope: Colonscope = ":scope".asInstanceOf[Colonscope]
  
  @js.native
  sealed trait Colontarget extends StObject
  inline def Colontarget: Colontarget = ":target".asInstanceOf[Colontarget]
  
  @js.native
  sealed trait Colonvalid extends StObject
  inline def Colonvalid: Colonvalid = ":valid".asInstanceOf[Colonvalid]
  
  @js.native
  sealed trait Colonvisited extends StObject
  inline def Colonvisited: Colonvisited = ":visited".asInstanceOf[Colonvisited]
  
  @js.native
  sealed trait absolute extends StObject
  inline def absolute: absolute = "absolute".asInstanceOf[absolute]
  
  @js.native
  sealed trait active extends StObject
  inline def active: active = "active".asInstanceOf[active]
  
  @js.native
  sealed trait adjacent extends StObject
  inline def adjacent: adjacent = "adjacent".asInstanceOf[adjacent]
  
  @js.native
  sealed trait `after-edge` extends StObject
  inline def `after-edge`: `after-edge` = "after-edge".asInstanceOf[`after-edge`]
  
  @js.native
  sealed trait alignContent extends StObject
  inline def alignContent: alignContent = "alignContent".asInstanceOf[alignContent]
  
  @js.native
  sealed trait alignItems extends StObject
  inline def alignItems: alignItems = "alignItems".asInstanceOf[alignItems]
  
  @js.native
  sealed trait alignSelf extends StObject
  inline def alignSelf: alignSelf = "alignSelf".asInstanceOf[alignSelf]
  
  @js.native
  sealed trait alignmentAdjust extends StObject
  inline def alignmentAdjust: alignmentAdjust = "alignmentAdjust".asInstanceOf[alignmentAdjust]
  
  @js.native
  sealed trait alignmentBaseline extends StObject
  inline def alignmentBaseline: alignmentBaseline = "alignmentBaseline".asInstanceOf[alignmentBaseline]
  
  @js.native
  sealed trait all extends StObject
  inline def all: all = "all".asInstanceOf[all]
  
  @js.native
  sealed trait `allow-end` extends StObject
  inline def `allow-end`: `allow-end` = "allow-end".asInstanceOf[`allow-end`]
  
  @js.native
  sealed trait alphabetic extends StObject
  inline def alphabetic: alphabetic = "alphabetic".asInstanceOf[alphabetic]
  
  @js.native
  sealed trait alternate extends StObject
  inline def alternate: alternate = "alternate".asInstanceOf[alternate]
  
  @js.native
  sealed trait `alternate-reverse` extends StObject
  inline def `alternate-reverse`: `alternate-reverse` = "alternate-reverse".asInstanceOf[`alternate-reverse`]
  
  @js.native
  sealed trait always extends StObject
  inline def always: always = "always".asInstanceOf[always]
  
  @js.native
  sealed trait animationDelay extends StObject
  inline def animationDelay: animationDelay = "animationDelay".asInstanceOf[animationDelay]
  
  @js.native
  sealed trait animationDirection extends StObject
  inline def animationDirection: animationDirection = "animationDirection".asInstanceOf[animationDirection]
  
  @js.native
  sealed trait animationIterationCount extends StObject
  inline def animationIterationCount: animationIterationCount = "animationIterationCount".asInstanceOf[animationIterationCount]
  
  @js.native
  sealed trait animationName extends StObject
  inline def animationName: animationName = "animationName".asInstanceOf[animationName]
  
  @js.native
  sealed trait animationPlayState extends StObject
  inline def animationPlayState: animationPlayState = "animationPlayState".asInstanceOf[animationPlayState]
  
  @js.native
  sealed trait appearance extends StObject
  inline def appearance: appearance = "appearance".asInstanceOf[appearance]
  
  @js.native
  sealed trait auto extends StObject
  inline def auto: auto = "auto".asInstanceOf[auto]
  
  @js.native
  sealed trait avoid extends StObject
  inline def avoid: avoid = "avoid".asInstanceOf[avoid]
  
  @js.native
  sealed trait `avoid-column` extends StObject
  inline def `avoid-column`: `avoid-column` = "avoid-column".asInstanceOf[`avoid-column`]
  
  @js.native
  sealed trait `avoid-page` extends StObject
  inline def `avoid-page`: `avoid-page` = "avoid-page".asInstanceOf[`avoid-page`]
  
  @js.native
  sealed trait `avoid-region` extends StObject
  inline def `avoid-region`: `avoid-region` = "avoid-region".asInstanceOf[`avoid-region`]
  
  @js.native
  sealed trait backfaceVisibility extends StObject
  inline def backfaceVisibility: backfaceVisibility = "backfaceVisibility".asInstanceOf[backfaceVisibility]
  
  @js.native
  sealed trait background extends StObject
  inline def background: background = "background".asInstanceOf[background]
  
  @js.native
  sealed trait backgroundAttachment extends StObject
  inline def backgroundAttachment: backgroundAttachment = "backgroundAttachment".asInstanceOf[backgroundAttachment]
  
  @js.native
  sealed trait backgroundBlendMode extends StObject
  inline def backgroundBlendMode: backgroundBlendMode = "backgroundBlendMode".asInstanceOf[backgroundBlendMode]
  
  @js.native
  sealed trait backgroundColor extends StObject
  inline def backgroundColor: backgroundColor = "backgroundColor".asInstanceOf[backgroundColor]
  
  @js.native
  sealed trait backgroundComposite extends StObject
  inline def backgroundComposite: backgroundComposite = "backgroundComposite".asInstanceOf[backgroundComposite]
  
  @js.native
  sealed trait backgroundImage extends StObject
  inline def backgroundImage: backgroundImage = "backgroundImage".asInstanceOf[backgroundImage]
  
  @js.native
  sealed trait backgroundOrigin extends StObject
  inline def backgroundOrigin: backgroundOrigin = "backgroundOrigin".asInstanceOf[backgroundOrigin]
  
  @js.native
  sealed trait backgroundPosition extends StObject
  inline def backgroundPosition: backgroundPosition = "backgroundPosition".asInstanceOf[backgroundPosition]
  
  @js.native
  sealed trait backgroundRepeat extends StObject
  inline def backgroundRepeat: backgroundRepeat = "backgroundRepeat".asInstanceOf[backgroundRepeat]
  
  @js.native
  sealed trait balance extends StObject
  inline def balance: balance = "balance".asInstanceOf[balance]
  
  @js.native
  sealed trait baseline extends StObject
  inline def baseline: baseline = "baseline".asInstanceOf[baseline]
  
  @js.native
  sealed trait baselineShift extends StObject
  inline def baselineShift: baselineShift = "baselineShift".asInstanceOf[baselineShift]
  
  @js.native
  sealed trait `before-edge` extends StObject
  inline def `before-edge`: `before-edge` = "before-edge".asInstanceOf[`before-edge`]
  
  @js.native
  sealed trait behavior extends StObject
  inline def behavior: behavior = "behavior".asInstanceOf[behavior]
  
  @js.native
  sealed trait `bidi-override` extends StObject
  inline def `bidi-override`: `bidi-override` = "bidi-override".asInstanceOf[`bidi-override`]
  
  @js.native
  sealed trait block extends StObject
  inline def block: block = "block".asInstanceOf[block]
  
  @js.native
  sealed trait `block flow` extends StObject
  inline def `block flow`: `block flow` = ("block flow").asInstanceOf[`block flow`]
  
  @js.native
  sealed trait bold extends StObject
  inline def bold: bold = "bold".asInstanceOf[bold]
  
  @js.native
  sealed trait bolder extends StObject
  inline def bolder: bolder = "bolder".asInstanceOf[bolder]
  
  @js.native
  sealed trait border extends StObject
  inline def border: border = "border".asInstanceOf[border]
  
  @js.native
  sealed trait `border-box` extends StObject
  inline def `border-box`: `border-box` = "border-box".asInstanceOf[`border-box`]
  
  @js.native
  sealed trait borderBottom extends StObject
  inline def borderBottom: borderBottom = "borderBottom".asInstanceOf[borderBottom]
  
  @js.native
  sealed trait borderBottomColor extends StObject
  inline def borderBottomColor: borderBottomColor = "borderBottomColor".asInstanceOf[borderBottomColor]
  
  @js.native
  sealed trait borderBottomLeftRadius extends StObject
  inline def borderBottomLeftRadius: borderBottomLeftRadius = "borderBottomLeftRadius".asInstanceOf[borderBottomLeftRadius]
  
  @js.native
  sealed trait borderBottomRightRadius extends StObject
  inline def borderBottomRightRadius: borderBottomRightRadius = "borderBottomRightRadius".asInstanceOf[borderBottomRightRadius]
  
  @js.native
  sealed trait borderBottomStyle extends StObject
  inline def borderBottomStyle: borderBottomStyle = "borderBottomStyle".asInstanceOf[borderBottomStyle]
  
  @js.native
  sealed trait borderBottomWidth extends StObject
  inline def borderBottomWidth: borderBottomWidth = "borderBottomWidth".asInstanceOf[borderBottomWidth]
  
  @js.native
  sealed trait borderCollapse extends StObject
  inline def borderCollapse: borderCollapse = "borderCollapse".asInstanceOf[borderCollapse]
  
  @js.native
  sealed trait borderColor extends StObject
  inline def borderColor: borderColor = "borderColor".asInstanceOf[borderColor]
  
  @js.native
  sealed trait borderCornerShape extends StObject
  inline def borderCornerShape: borderCornerShape = "borderCornerShape".asInstanceOf[borderCornerShape]
  
  @js.native
  sealed trait borderImageSource extends StObject
  inline def borderImageSource: borderImageSource = "borderImageSource".asInstanceOf[borderImageSource]
  
  @js.native
  sealed trait borderImageWidth extends StObject
  inline def borderImageWidth: borderImageWidth = "borderImageWidth".asInstanceOf[borderImageWidth]
  
  @js.native
  sealed trait borderLeft extends StObject
  inline def borderLeft: borderLeft = "borderLeft".asInstanceOf[borderLeft]
  
  @js.native
  sealed trait borderLeftColor extends StObject
  inline def borderLeftColor: borderLeftColor = "borderLeftColor".asInstanceOf[borderLeftColor]
  
  @js.native
  sealed trait borderLeftStyle extends StObject
  inline def borderLeftStyle: borderLeftStyle = "borderLeftStyle".asInstanceOf[borderLeftStyle]
  
  @js.native
  sealed trait borderLeftWidth extends StObject
  inline def borderLeftWidth: borderLeftWidth = "borderLeftWidth".asInstanceOf[borderLeftWidth]
  
  @js.native
  sealed trait borderRadius extends StObject
  inline def borderRadius: borderRadius = "borderRadius".asInstanceOf[borderRadius]
  
  @js.native
  sealed trait borderRight extends StObject
  inline def borderRight: borderRight = "borderRight".asInstanceOf[borderRight]
  
  @js.native
  sealed trait borderRightColor extends StObject
  inline def borderRightColor: borderRightColor = "borderRightColor".asInstanceOf[borderRightColor]
  
  @js.native
  sealed trait borderRightStyle extends StObject
  inline def borderRightStyle: borderRightStyle = "borderRightStyle".asInstanceOf[borderRightStyle]
  
  @js.native
  sealed trait borderRightWidth extends StObject
  inline def borderRightWidth: borderRightWidth = "borderRightWidth".asInstanceOf[borderRightWidth]
  
  @js.native
  sealed trait borderSpacing extends StObject
  inline def borderSpacing: borderSpacing = "borderSpacing".asInstanceOf[borderSpacing]
  
  @js.native
  sealed trait borderStyle extends StObject
  inline def borderStyle: borderStyle = "borderStyle".asInstanceOf[borderStyle]
  
  @js.native
  sealed trait borderTop extends StObject
  inline def borderTop: borderTop = "borderTop".asInstanceOf[borderTop]
  
  @js.native
  sealed trait borderTopColor extends StObject
  inline def borderTopColor: borderTopColor = "borderTopColor".asInstanceOf[borderTopColor]
  
  @js.native
  sealed trait borderTopLeftRadius extends StObject
  inline def borderTopLeftRadius: borderTopLeftRadius = "borderTopLeftRadius".asInstanceOf[borderTopLeftRadius]
  
  @js.native
  sealed trait borderTopRightRadius extends StObject
  inline def borderTopRightRadius: borderTopRightRadius = "borderTopRightRadius".asInstanceOf[borderTopRightRadius]
  
  @js.native
  sealed trait borderTopStyle extends StObject
  inline def borderTopStyle: borderTopStyle = "borderTopStyle".asInstanceOf[borderTopStyle]
  
  @js.native
  sealed trait borderTopWidth extends StObject
  inline def borderTopWidth: borderTopWidth = "borderTopWidth".asInstanceOf[borderTopWidth]
  
  @js.native
  sealed trait borderWidth extends StObject
  inline def borderWidth: borderWidth = "borderWidth".asInstanceOf[borderWidth]
  
  @js.native
  sealed trait both extends StObject
  inline def both: both = "both".asInstanceOf[both]
  
  @js.native
  sealed trait bottom extends StObject
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait boxAlign extends StObject
  inline def boxAlign: boxAlign = "boxAlign".asInstanceOf[boxAlign]
  
  @js.native
  sealed trait boxDecorationBreak extends StObject
  inline def boxDecorationBreak: boxDecorationBreak = "boxDecorationBreak".asInstanceOf[boxDecorationBreak]
  
  @js.native
  sealed trait boxDirection extends StObject
  inline def boxDirection: boxDirection = "boxDirection".asInstanceOf[boxDirection]
  
  @js.native
  sealed trait boxFlex extends StObject
  inline def boxFlex: boxFlex = "boxFlex".asInstanceOf[boxFlex]
  
  @js.native
  sealed trait boxFlexGroup extends StObject
  inline def boxFlexGroup: boxFlexGroup = "boxFlexGroup".asInstanceOf[boxFlexGroup]
  
  @js.native
  sealed trait boxLineProgression extends StObject
  inline def boxLineProgression: boxLineProgression = "boxLineProgression".asInstanceOf[boxLineProgression]
  
  @js.native
  sealed trait boxLines extends StObject
  inline def boxLines: boxLines = "boxLines".asInstanceOf[boxLines]
  
  @js.native
  sealed trait boxOrdinalGroup extends StObject
  inline def boxOrdinalGroup: boxOrdinalGroup = "boxOrdinalGroup".asInstanceOf[boxOrdinalGroup]
  
  @js.native
  sealed trait boxShadow extends StObject
  inline def boxShadow: boxShadow = "boxShadow".asInstanceOf[boxShadow]
  
  @js.native
  sealed trait boxSizing extends StObject
  inline def boxSizing: boxSizing = "boxSizing".asInstanceOf[boxSizing]
  
  @js.native
  sealed trait break extends StObject
  inline def break: break = "break".asInstanceOf[break]
  
  @js.native
  sealed trait `break-all` extends StObject
  inline def `break-all`: `break-all` = "break-all".asInstanceOf[`break-all`]
  
  @js.native
  sealed trait `break-word` extends StObject
  inline def `break-word`: `break-word` = "break-word".asInstanceOf[`break-word`]
  
  @js.native
  sealed trait breakAfter extends StObject
  inline def breakAfter: breakAfter = "breakAfter".asInstanceOf[breakAfter]
  
  @js.native
  sealed trait breakBefore extends StObject
  inline def breakBefore: breakBefore = "breakBefore".asInstanceOf[breakBefore]
  
  @js.native
  sealed trait breakInside extends StObject
  inline def breakInside: breakInside = "breakInside".asInstanceOf[breakInside]
  
  @js.native
  sealed trait capitalize extends StObject
  inline def capitalize: capitalize = "capitalize".asInstanceOf[capitalize]
  
  @js.native
  sealed trait center extends StObject
  inline def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait central extends StObject
  inline def central: central = "central".asInstanceOf[central]
  
  @js.native
  sealed trait clear extends StObject
  inline def clear: clear = "clear".asInstanceOf[clear]
  
  @js.native
  sealed trait clip extends StObject
  inline def clip: clip = "clip".asInstanceOf[clip]
  
  @js.native
  sealed trait clipRule extends StObject
  inline def clipRule: clipRule = "clipRule".asInstanceOf[clipRule]
  
  @js.native
  sealed trait clone extends StObject
  
  @js.native
  sealed trait collapse extends StObject
  inline def collapse: collapse = "collapse".asInstanceOf[collapse]
  
  @js.native
  sealed trait color extends StObject
  inline def color: color = "color".asInstanceOf[color]
  
  @js.native
  sealed trait `color-burn` extends StObject
  inline def `color-burn`: `color-burn` = "color-burn".asInstanceOf[`color-burn`]
  
  @js.native
  sealed trait `color-dodge` extends StObject
  inline def `color-dodge`: `color-dodge` = "color-dodge".asInstanceOf[`color-dodge`]
  
  @js.native
  sealed trait column extends StObject
  inline def column: column = "column".asInstanceOf[column]
  
  @js.native
  sealed trait `column nowrap` extends StObject
  inline def `column nowrap`: `column nowrap` = ("column nowrap").asInstanceOf[`column nowrap`]
  
  @js.native
  sealed trait `column wrap` extends StObject
  inline def `column wrap`: `column wrap` = ("column wrap").asInstanceOf[`column wrap`]
  
  @js.native
  sealed trait `column wrap-reverse` extends StObject
  inline def `column wrap-reverse`: `column wrap-reverse` = ("column wrap-reverse").asInstanceOf[`column wrap-reverse`]
  
  @js.native
  sealed trait `column-reverse` extends StObject
  inline def `column-reverse`: `column-reverse` = "column-reverse".asInstanceOf[`column-reverse`]
  
  @js.native
  sealed trait `column-reverse nowrap` extends StObject
  inline def `column-reverse nowrap`: `column-reverse nowrap` = ("column-reverse nowrap").asInstanceOf[`column-reverse nowrap`]
  
  @js.native
  sealed trait `column-reverse wrap` extends StObject
  inline def `column-reverse wrap`: `column-reverse wrap` = ("column-reverse wrap").asInstanceOf[`column-reverse wrap`]
  
  @js.native
  sealed trait `column-reverse wrap-reverse` extends StObject
  inline def `column-reverse wrap-reverse`: `column-reverse wrap-reverse` = ("column-reverse wrap-reverse").asInstanceOf[`column-reverse wrap-reverse`]
  
  @js.native
  sealed trait columnCount extends StObject
  inline def columnCount: columnCount = "columnCount".asInstanceOf[columnCount]
  
  @js.native
  sealed trait columnFill extends StObject
  inline def columnFill: columnFill = "columnFill".asInstanceOf[columnFill]
  
  @js.native
  sealed trait columnGap extends StObject
  inline def columnGap: columnGap = "columnGap".asInstanceOf[columnGap]
  
  @js.native
  sealed trait columnRule extends StObject
  inline def columnRule: columnRule = "columnRule".asInstanceOf[columnRule]
  
  @js.native
  sealed trait columnRuleColor extends StObject
  inline def columnRuleColor: columnRuleColor = "columnRuleColor".asInstanceOf[columnRuleColor]
  
  @js.native
  sealed trait columnRuleWidth extends StObject
  inline def columnRuleWidth: columnRuleWidth = "columnRuleWidth".asInstanceOf[columnRuleWidth]
  
  @js.native
  sealed trait columnSpan extends StObject
  inline def columnSpan: columnSpan = "columnSpan".asInstanceOf[columnSpan]
  
  @js.native
  sealed trait columnWidth extends StObject
  inline def columnWidth: columnWidth = "columnWidth".asInstanceOf[columnWidth]
  
  @js.native
  sealed trait columns extends StObject
  inline def columns: columns = "columns".asInstanceOf[columns]
  
  @js.native
  sealed trait condensed extends StObject
  inline def condensed: condensed = "condensed".asInstanceOf[condensed]
  
  @js.native
  sealed trait `content-box` extends StObject
  inline def `content-box`: `content-box` = "content-box".asInstanceOf[`content-box`]
  
  @js.native
  sealed trait contents extends StObject
  inline def contents: contents = "contents".asInstanceOf[contents]
  
  @js.native
  sealed trait counterIncrement extends StObject
  inline def counterIncrement: counterIncrement = "counterIncrement".asInstanceOf[counterIncrement]
  
  @js.native
  sealed trait counterReset extends StObject
  inline def counterReset: counterReset = "counterReset".asInstanceOf[counterReset]
  
  @js.native
  sealed trait cue extends StObject
  inline def cue: cue = "cue".asInstanceOf[cue]
  
  @js.native
  sealed trait cueAfter extends StObject
  inline def cueAfter: cueAfter = "cueAfter".asInstanceOf[cueAfter]
  
  @js.native
  sealed trait cursor extends StObject
  inline def cursor: cursor = "cursor".asInstanceOf[cursor]
  
  @js.native
  sealed trait darken extends StObject
  inline def darken: darken = "darken".asInstanceOf[darken]
  
  @js.native
  sealed trait dashed extends StObject
  inline def dashed: dashed = "dashed".asInstanceOf[dashed]
  
  @js.native
  sealed trait difference extends StObject
  inline def difference: difference = "difference".asInstanceOf[difference]
  
  @js.native
  sealed trait direction extends StObject
  inline def direction: direction = "direction".asInstanceOf[direction]
  
  @js.native
  sealed trait disabled extends StObject
  inline def disabled: disabled = "disabled".asInstanceOf[disabled]
  
  @js.native
  sealed trait display extends StObject
  inline def display: display = "display".asInstanceOf[display]
  
  @js.native
  sealed trait distribute extends StObject
  inline def distribute: distribute = "distribute".asInstanceOf[distribute]
  
  @js.native
  sealed trait dotted extends StObject
  inline def dotted: dotted = "dotted".asInstanceOf[dotted]
  
  @js.native
  sealed trait double extends StObject
  inline def double: double = "double".asInstanceOf[double]
  
  @js.native
  sealed trait embed extends StObject
  inline def embed: embed = "embed".asInstanceOf[embed]
  
  @js.native
  sealed trait enabled extends StObject
  inline def enabled: enabled = "enabled".asInstanceOf[enabled]
  
  @js.native
  sealed trait end extends StObject
  inline def end: end = "end".asInstanceOf[end]
  
  @js.native
  sealed trait evenodd extends StObject
  inline def evenodd: evenodd = "evenodd".asInstanceOf[evenodd]
  
  @js.native
  sealed trait exclusion extends StObject
  inline def exclusion: exclusion = "exclusion".asInstanceOf[exclusion]
  
  @js.native
  sealed trait expanded extends StObject
  inline def expanded: expanded = "expanded".asInstanceOf[expanded]
  
  @js.native
  sealed trait `extra-condensed` extends StObject
  inline def `extra-condensed`: `extra-condensed` = "extra-condensed".asInstanceOf[`extra-condensed`]
  
  @js.native
  sealed trait `extra-expanded` extends StObject
  inline def `extra-expanded`: `extra-expanded` = "extra-expanded".asInstanceOf[`extra-expanded`]
  
  @js.native
  sealed trait fill extends StObject
  inline def fill: fill = "fill".asInstanceOf[fill]
  
  @js.native
  sealed trait fillOpacity extends StObject
  inline def fillOpacity: fillOpacity = "fillOpacity".asInstanceOf[fillOpacity]
  
  @js.native
  sealed trait fillRule extends StObject
  inline def fillRule: fillRule = "fillRule".asInstanceOf[fillRule]
  
  @js.native
  sealed trait filter extends StObject
  inline def filter: filter = "filter".asInstanceOf[filter]
  
  @js.native
  sealed trait fixed extends StObject
  inline def fixed: fixed = "fixed".asInstanceOf[fixed]
  
  @js.native
  sealed trait flat extends StObject
  inline def flat: flat = "flat".asInstanceOf[flat]
  
  @js.native
  sealed trait flex extends StObject
  inline def flex: flex = "flex".asInstanceOf[flex]
  
  @js.native
  sealed trait `flex run-in` extends StObject
  inline def `flex run-in`: `flex run-in` = ("flex run-in").asInstanceOf[`flex run-in`]
  
  @js.native
  sealed trait `flex-end` extends StObject
  inline def `flex-end`: `flex-end` = "flex-end".asInstanceOf[`flex-end`]
  
  @js.native
  sealed trait `flex-start` extends StObject
  inline def `flex-start`: `flex-start` = "flex-start".asInstanceOf[`flex-start`]
  
  @js.native
  sealed trait flexAlign extends StObject
  inline def flexAlign: flexAlign = "flexAlign".asInstanceOf[flexAlign]
  
  @js.native
  sealed trait flexBasis extends StObject
  inline def flexBasis: flexBasis = "flexBasis".asInstanceOf[flexBasis]
  
  @js.native
  sealed trait flexDirection extends StObject
  inline def flexDirection: flexDirection = "flexDirection".asInstanceOf[flexDirection]
  
  @js.native
  sealed trait flexFlow extends StObject
  inline def flexFlow: flexFlow = "flexFlow".asInstanceOf[flexFlow]
  
  @js.native
  sealed trait flexGrow extends StObject
  inline def flexGrow: flexGrow = "flexGrow".asInstanceOf[flexGrow]
  
  @js.native
  sealed trait flexItemAlign extends StObject
  inline def flexItemAlign: flexItemAlign = "flexItemAlign".asInstanceOf[flexItemAlign]
  
  @js.native
  sealed trait flexLinePack extends StObject
  inline def flexLinePack: flexLinePack = "flexLinePack".asInstanceOf[flexLinePack]
  
  @js.native
  sealed trait flexOrder extends StObject
  inline def flexOrder: flexOrder = "flexOrder".asInstanceOf[flexOrder]
  
  @js.native
  sealed trait flexShrink extends StObject
  inline def flexShrink: flexShrink = "flexShrink".asInstanceOf[flexShrink]
  
  @js.native
  sealed trait flexWrap extends StObject
  inline def flexWrap: flexWrap = "flexWrap".asInstanceOf[flexWrap]
  
  @js.native
  sealed trait float extends StObject
  inline def float: float = "float".asInstanceOf[float]
  
  @js.native
  sealed trait flow extends StObject
  inline def flow: flow = "flow".asInstanceOf[flow]
  
  @js.native
  sealed trait `flow list-item block` extends StObject
  inline def `flow list-item block`: `flow list-item block` = ("flow list-item block").asInstanceOf[`flow list-item block`]
  
  @js.native
  sealed trait `flow-root` extends StObject
  inline def `flow-root`: `flow-root` = "flow-root".asInstanceOf[`flow-root`]
  
  @js.native
  sealed trait flowFrom extends StObject
  inline def flowFrom: flowFrom = "flowFrom".asInstanceOf[flowFrom]
  
  @js.native
  sealed trait font extends StObject
  inline def font: font = "font".asInstanceOf[font]
  
  @js.native
  sealed trait fontFamily extends StObject
  inline def fontFamily: fontFamily = "fontFamily".asInstanceOf[fontFamily]
  
  @js.native
  sealed trait fontKerning extends StObject
  inline def fontKerning: fontKerning = "fontKerning".asInstanceOf[fontKerning]
  
  @js.native
  sealed trait fontSize extends StObject
  inline def fontSize: fontSize = "fontSize".asInstanceOf[fontSize]
  
  @js.native
  sealed trait fontSizeAdjust extends StObject
  inline def fontSizeAdjust: fontSizeAdjust = "fontSizeAdjust".asInstanceOf[fontSizeAdjust]
  
  @js.native
  sealed trait fontStretch extends StObject
  inline def fontStretch: fontStretch = "fontStretch".asInstanceOf[fontStretch]
  
  @js.native
  sealed trait fontStyle extends StObject
  inline def fontStyle: fontStyle = "fontStyle".asInstanceOf[fontStyle]
  
  @js.native
  sealed trait fontSynthesis extends StObject
  inline def fontSynthesis: fontSynthesis = "fontSynthesis".asInstanceOf[fontSynthesis]
  
  @js.native
  sealed trait fontVariant extends StObject
  inline def fontVariant: fontVariant = "fontVariant".asInstanceOf[fontVariant]
  
  @js.native
  sealed trait fontVariantAlternates extends StObject
  inline def fontVariantAlternates: fontVariantAlternates = "fontVariantAlternates".asInstanceOf[fontVariantAlternates]
  
  @js.native
  sealed trait fontWeight extends StObject
  inline def fontWeight: fontWeight = "fontWeight".asInstanceOf[fontWeight]
  
  @js.native
  sealed trait `full-width` extends StObject
  inline def `full-width`: `full-width` = "full-width".asInstanceOf[`full-width`]
  
  @js.native
  sealed trait geometricPrecision extends StObject
  inline def geometricPrecision: geometricPrecision = "geometricPrecision".asInstanceOf[geometricPrecision]
  
  @js.native
  sealed trait grid extends StObject
  inline def grid: grid = "grid".asInstanceOf[grid]
  
  @js.native
  sealed trait gridArea extends StObject
  inline def gridArea: gridArea = "gridArea".asInstanceOf[gridArea]
  
  @js.native
  sealed trait gridColumn extends StObject
  inline def gridColumn: gridColumn = "gridColumn".asInstanceOf[gridColumn]
  
  @js.native
  sealed trait gridColumnEnd extends StObject
  inline def gridColumnEnd: gridColumnEnd = "gridColumnEnd".asInstanceOf[gridColumnEnd]
  
  @js.native
  sealed trait gridColumnStart extends StObject
  inline def gridColumnStart: gridColumnStart = "gridColumnStart".asInstanceOf[gridColumnStart]
  
  @js.native
  sealed trait gridRow extends StObject
  inline def gridRow: gridRow = "gridRow".asInstanceOf[gridRow]
  
  @js.native
  sealed trait gridRowEnd extends StObject
  inline def gridRowEnd: gridRowEnd = "gridRowEnd".asInstanceOf[gridRowEnd]
  
  @js.native
  sealed trait gridRowPosition extends StObject
  inline def gridRowPosition: gridRowPosition = "gridRowPosition".asInstanceOf[gridRowPosition]
  
  @js.native
  sealed trait gridRowSpan extends StObject
  inline def gridRowSpan: gridRowSpan = "gridRowSpan".asInstanceOf[gridRowSpan]
  
  @js.native
  sealed trait gridTemplateAreas extends StObject
  inline def gridTemplateAreas: gridTemplateAreas = "gridTemplateAreas".asInstanceOf[gridTemplateAreas]
  
  @js.native
  sealed trait gridTemplateColumns extends StObject
  inline def gridTemplateColumns: gridTemplateColumns = "gridTemplateColumns".asInstanceOf[gridTemplateColumns]
  
  @js.native
  sealed trait gridTemplateRows extends StObject
  inline def gridTemplateRows: gridTemplateRows = "gridTemplateRows".asInstanceOf[gridTemplateRows]
  
  @js.native
  sealed trait groove extends StObject
  inline def groove: groove = "groove".asInstanceOf[groove]
  
  @js.native
  sealed trait hanging extends StObject
  inline def hanging: hanging = "hanging".asInstanceOf[hanging]
  
  @js.native
  sealed trait `hard-light` extends StObject
  inline def `hard-light`: `hard-light` = "hard-light".asInstanceOf[`hard-light`]
  
  @js.native
  sealed trait height extends StObject
  inline def height: height = "height".asInstanceOf[height]
  
  @js.native
  sealed trait hidden extends StObject
  inline def hidden: hidden = "hidden".asInstanceOf[hidden]
  
  @js.native
  sealed trait `horizontal-tb` extends StObject
  inline def `horizontal-tb`: `horizontal-tb` = "horizontal-tb".asInstanceOf[`horizontal-tb`]
  
  @js.native
  sealed trait hue extends StObject
  inline def hue: hue = "hue".asInstanceOf[hue]
  
  @js.native
  sealed trait hyphenateLimitChars extends StObject
  inline def hyphenateLimitChars: hyphenateLimitChars = "hyphenateLimitChars".asInstanceOf[hyphenateLimitChars]
  
  @js.native
  sealed trait hyphenateLimitLines extends StObject
  inline def hyphenateLimitLines: hyphenateLimitLines = "hyphenateLimitLines".asInstanceOf[hyphenateLimitLines]
  
  @js.native
  sealed trait hyphenateLimitZone extends StObject
  inline def hyphenateLimitZone: hyphenateLimitZone = "hyphenateLimitZone".asInstanceOf[hyphenateLimitZone]
  
  @js.native
  sealed trait hyphens extends StObject
  inline def hyphens: hyphens = "hyphens".asInstanceOf[hyphens]
  
  @js.native
  sealed trait ideographic extends StObject
  inline def ideographic: ideographic = "ideographic".asInstanceOf[ideographic]
  
  @js.native
  sealed trait ignore extends StObject
  inline def ignore: ignore = "ignore".asInstanceOf[ignore]
  
  @js.native
  sealed trait imeMode extends StObject
  inline def imeMode: imeMode = "imeMode".asInstanceOf[imeMode]
  
  @js.native
  sealed trait inactive extends StObject
  inline def inactive: inactive = "inactive".asInstanceOf[inactive]
  
  @js.native
  sealed trait infinite extends StObject
  inline def infinite: infinite = "infinite".asInstanceOf[infinite]
  
  @js.native
  sealed trait inherit
    extends StObject
       with CSSWideKeyword
  inline def inherit: inherit = "inherit".asInstanceOf[inherit]
  
  @js.native
  sealed trait initial
    extends StObject
       with CSSWideKeyword
  inline def initial: initial = "initial".asInstanceOf[initial]
  
  @js.native
  sealed trait `inline` extends StObject
  inline def `inline`: `inline` = "inline".asInstanceOf[`inline`]
  
  @js.native
  sealed trait `inline table` extends StObject
  inline def `inline table`: `inline table` = ("inline table").asInstanceOf[`inline table`]
  
  @js.native
  sealed trait `inline-block` extends StObject
  inline def `inline-block`: `inline-block` = "inline-block".asInstanceOf[`inline-block`]
  
  @js.native
  sealed trait `inline-end` extends StObject
  inline def `inline-end`: `inline-end` = "inline-end".asInstanceOf[`inline-end`]
  
  @js.native
  sealed trait `inline-flex` extends StObject
  inline def `inline-flex`: `inline-flex` = "inline-flex".asInstanceOf[`inline-flex`]
  
  @js.native
  sealed trait `inline-grid` extends StObject
  inline def `inline-grid`: `inline-grid` = "inline-grid".asInstanceOf[`inline-grid`]
  
  @js.native
  sealed trait `inline-list-item` extends StObject
  inline def `inline-list-item`: `inline-list-item` = "inline-list-item".asInstanceOf[`inline-list-item`]
  
  @js.native
  sealed trait `inline-start` extends StObject
  inline def `inline-start`: `inline-start` = "inline-start".asInstanceOf[`inline-start`]
  
  @js.native
  sealed trait `inline-table` extends StObject
  inline def `inline-table`: `inline-table` = "inline-table".asInstanceOf[`inline-table`]
  
  @js.native
  sealed trait inset extends StObject
  inline def inset: inset = "inset".asInstanceOf[inset]
  
  @js.native
  sealed trait inside extends StObject
  inline def inside: inside = "inside".asInstanceOf[inside]
  
  @js.native
  sealed trait `inter-character` extends StObject
  inline def `inter-character`: `inter-character` = "inter-character".asInstanceOf[`inter-character`]
  
  @js.native
  sealed trait `inter-cluster` extends StObject
  inline def `inter-cluster`: `inter-cluster` = "inter-cluster".asInstanceOf[`inter-cluster`]
  
  @js.native
  sealed trait `inter-ideograph` extends StObject
  inline def `inter-ideograph`: `inter-ideograph` = "inter-ideograph".asInstanceOf[`inter-ideograph`]
  
  @js.native
  sealed trait `inter-word` extends StObject
  inline def `inter-word`: `inter-word` = "inter-word".asInstanceOf[`inter-word`]
  
  @js.native
  sealed trait isolate extends StObject
  inline def isolate: isolate = "isolate".asInstanceOf[isolate]
  
  @js.native
  sealed trait `isolate-override` extends StObject
  inline def `isolate-override`: `isolate-override` = "isolate-override".asInstanceOf[`isolate-override`]
  
  @js.native
  sealed trait italic extends StObject
  inline def italic: italic = "italic".asInstanceOf[italic]
  
  @js.native
  sealed trait justify extends StObject
  inline def justify: justify = "justify".asInstanceOf[justify]
  
  @js.native
  sealed trait justifyContent extends StObject
  inline def justifyContent: justifyContent = "justifyContent".asInstanceOf[justifyContent]
  
  @js.native
  sealed trait kashida extends StObject
  inline def kashida: kashida = "kashida".asInstanceOf[kashida]
  
  @js.native
  sealed trait `keep-all` extends StObject
  inline def `keep-all`: `keep-all` = "keep-all".asInstanceOf[`keep-all`]
  
  @js.native
  sealed trait layoutGrid extends StObject
  inline def layoutGrid: layoutGrid = "layoutGrid".asInstanceOf[layoutGrid]
  
  @js.native
  sealed trait layoutGridChar extends StObject
  inline def layoutGridChar: layoutGridChar = "layoutGridChar".asInstanceOf[layoutGridChar]
  
  @js.native
  sealed trait layoutGridLine extends StObject
  inline def layoutGridLine: layoutGridLine = "layoutGridLine".asInstanceOf[layoutGridLine]
  
  @js.native
  sealed trait layoutGridMode extends StObject
  inline def layoutGridMode: layoutGridMode = "layoutGridMode".asInstanceOf[layoutGridMode]
  
  @js.native
  sealed trait layoutGridType extends StObject
  inline def layoutGridType: layoutGridType = "layoutGridType".asInstanceOf[layoutGridType]
  
  @js.native
  sealed trait left extends StObject
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait leftwards extends StObject
  inline def leftwards: leftwards = "leftwards".asInstanceOf[leftwards]
  
  @js.native
  sealed trait letterSpacing extends StObject
  inline def letterSpacing: letterSpacing = "letterSpacing".asInstanceOf[letterSpacing]
  
  @js.native
  sealed trait lighten extends StObject
  inline def lighten: lighten = "lighten".asInstanceOf[lighten]
  
  @js.native
  sealed trait lighter extends StObject
  inline def lighter: lighter = "lighter".asInstanceOf[lighter]
  
  @js.native
  sealed trait lineBreak extends StObject
  inline def lineBreak: lineBreak = "lineBreak".asInstanceOf[lineBreak]
  
  @js.native
  sealed trait lineClamp extends StObject
  inline def lineClamp: lineClamp = "lineClamp".asInstanceOf[lineClamp]
  
  @js.native
  sealed trait lineHeight extends StObject
  inline def lineHeight: lineHeight = "lineHeight".asInstanceOf[lineHeight]
  
  @js.native
  sealed trait `list-item` extends StObject
  inline def `list-item`: `list-item` = "list-item".asInstanceOf[`list-item`]
  
  @js.native
  sealed trait `list-item block` extends StObject
  inline def `list-item block`: `list-item block` = ("list-item block").asInstanceOf[`list-item block`]
  
  @js.native
  sealed trait `list-item block flow` extends StObject
  inline def `list-item block flow`: `list-item block flow` = ("list-item block flow").asInstanceOf[`list-item block flow`]
  
  @js.native
  sealed trait `list-item block flow-root` extends StObject
  inline def `list-item block flow-root`: `list-item block flow-root` = ("list-item block flow-root").asInstanceOf[`list-item block flow-root`]
  
  @js.native
  sealed trait `list-item flow` extends StObject
  inline def `list-item flow`: `list-item flow` = ("list-item flow").asInstanceOf[`list-item flow`]
  
  @js.native
  sealed trait `list-item flow-root` extends StObject
  inline def `list-item flow-root`: `list-item flow-root` = ("list-item flow-root").asInstanceOf[`list-item flow-root`]
  
  @js.native
  sealed trait `list-item inline` extends StObject
  inline def `list-item inline`: `list-item inline` = ("list-item inline").asInstanceOf[`list-item inline`]
  
  @js.native
  sealed trait listStyle extends StObject
  inline def listStyle: listStyle = "listStyle".asInstanceOf[listStyle]
  
  @js.native
  sealed trait listStyleImage extends StObject
  inline def listStyleImage: listStyleImage = "listStyleImage".asInstanceOf[listStyleImage]
  
  @js.native
  sealed trait listStylePosition extends StObject
  inline def listStylePosition: listStylePosition = "listStylePosition".asInstanceOf[listStylePosition]
  
  @js.native
  sealed trait listStyleType extends StObject
  inline def listStyleType: listStyleType = "listStyleType".asInstanceOf[listStyleType]
  
  @js.native
  sealed trait local extends StObject
  inline def local: local = "local".asInstanceOf[local]
  
  @js.native
  sealed trait loose extends StObject
  inline def loose: loose = "loose".asInstanceOf[loose]
  
  @js.native
  sealed trait lowercase extends StObject
  inline def lowercase: lowercase = "lowercase".asInstanceOf[lowercase]
  
  @js.native
  sealed trait ltr extends StObject
  inline def ltr: ltr = "ltr".asInstanceOf[ltr]
  
  @js.native
  sealed trait luminosity extends StObject
  inline def luminosity: luminosity = "luminosity".asInstanceOf[luminosity]
  
  @js.native
  sealed trait manual extends StObject
  inline def manual: manual = "manual".asInstanceOf[manual]
  
  @js.native
  sealed trait margin extends StObject
  inline def margin: margin = "margin".asInstanceOf[margin]
  
  @js.native
  sealed trait marginBottom extends StObject
  inline def marginBottom: marginBottom = "marginBottom".asInstanceOf[marginBottom]
  
  @js.native
  sealed trait marginLeft extends StObject
  inline def marginLeft: marginLeft = "marginLeft".asInstanceOf[marginLeft]
  
  @js.native
  sealed trait marginRight extends StObject
  inline def marginRight: marginRight = "marginRight".asInstanceOf[marginRight]
  
  @js.native
  sealed trait marginTop extends StObject
  inline def marginTop: marginTop = "marginTop".asInstanceOf[marginTop]
  
  @js.native
  sealed trait marqueeDirection extends StObject
  inline def marqueeDirection: marqueeDirection = "marqueeDirection".asInstanceOf[marqueeDirection]
  
  @js.native
  sealed trait marqueeStyle extends StObject
  inline def marqueeStyle: marqueeStyle = "marqueeStyle".asInstanceOf[marqueeStyle]
  
  @js.native
  sealed trait mask extends StObject
  inline def mask: mask = "mask".asInstanceOf[mask]
  
  @js.native
  sealed trait maskBorder extends StObject
  inline def maskBorder: maskBorder = "maskBorder".asInstanceOf[maskBorder]
  
  @js.native
  sealed trait maskBorderRepeat extends StObject
  inline def maskBorderRepeat: maskBorderRepeat = "maskBorderRepeat".asInstanceOf[maskBorderRepeat]
  
  @js.native
  sealed trait maskBorderSlice extends StObject
  inline def maskBorderSlice: maskBorderSlice = "maskBorderSlice".asInstanceOf[maskBorderSlice]
  
  @js.native
  sealed trait maskBorderSource extends StObject
  inline def maskBorderSource: maskBorderSource = "maskBorderSource".asInstanceOf[maskBorderSource]
  
  @js.native
  sealed trait maskBorderWidth extends StObject
  inline def maskBorderWidth: maskBorderWidth = "maskBorderWidth".asInstanceOf[maskBorderWidth]
  
  @js.native
  sealed trait maskClip extends StObject
  inline def maskClip: maskClip = "maskClip".asInstanceOf[maskClip]
  
  @js.native
  sealed trait maskOrigin extends StObject
  inline def maskOrigin: maskOrigin = "maskOrigin".asInstanceOf[maskOrigin]
  
  @js.native
  sealed trait `match-parent` extends StObject
  inline def `match-parent`: `match-parent` = "match-parent".asInstanceOf[`match-parent`]
  
  @js.native
  sealed trait mathematical extends StObject
  inline def mathematical: mathematical = "mathematical".asInstanceOf[mathematical]
  
  @js.native
  sealed trait maxFontSize extends StObject
  inline def maxFontSize: maxFontSize = "maxFontSize".asInstanceOf[maxFontSize]
  
  @js.native
  sealed trait maxHeight extends StObject
  inline def maxHeight: maxHeight = "maxHeight".asInstanceOf[maxHeight]
  
  @js.native
  sealed trait maxWidth extends StObject
  inline def maxWidth: maxWidth = "maxWidth".asInstanceOf[maxWidth]
  
  @js.native
  sealed trait maximum extends StObject
  inline def maximum: maximum = "maximum".asInstanceOf[maximum]
  
  @js.native
  sealed trait middle extends StObject
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait minHeight extends StObject
  inline def minHeight: minHeight = "minHeight".asInstanceOf[minHeight]
  
  @js.native
  sealed trait minWidth extends StObject
  inline def minWidth: minWidth = "minWidth".asInstanceOf[minWidth]
  
  @js.native
  sealed trait multiple extends StObject
  inline def multiple: multiple = "multiple".asInstanceOf[multiple]
  
  @js.native
  sealed trait multiply extends StObject
  inline def multiply: multiply = "multiply".asInstanceOf[multiply]
  
  @js.native
  sealed trait `no-limit` extends StObject
  inline def `no-limit`: `no-limit` = "no-limit".asInstanceOf[`no-limit`]
  
  @js.native
  sealed trait none extends StObject
  inline def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait nonzero extends StObject
  inline def nonzero: nonzero = "nonzero".asInstanceOf[nonzero]
  
  @js.native
  sealed trait normal extends StObject
  inline def normal: normal = "normal".asInstanceOf[normal]
  
  @js.native
  sealed trait nowrap extends StObject
  inline def nowrap: nowrap = "nowrap".asInstanceOf[nowrap]
  
  @js.native
  sealed trait `nowrap column` extends StObject
  inline def `nowrap column`: `nowrap column` = ("nowrap column").asInstanceOf[`nowrap column`]
  
  @js.native
  sealed trait `nowrap column-reverse` extends StObject
  inline def `nowrap column-reverse`: `nowrap column-reverse` = ("nowrap column-reverse").asInstanceOf[`nowrap column-reverse`]
  
  @js.native
  sealed trait `nowrap row` extends StObject
  inline def `nowrap row`: `nowrap row` = ("nowrap row").asInstanceOf[`nowrap row`]
  
  @js.native
  sealed trait `nowrap row-reverse` extends StObject
  inline def `nowrap row-reverse`: `nowrap row-reverse` = ("nowrap row-reverse").asInstanceOf[`nowrap row-reverse`]
  
  @js.native
  sealed trait oblique extends StObject
  inline def oblique: oblique = "oblique".asInstanceOf[oblique]
  
  @js.native
  sealed trait opacity extends StObject
  inline def opacity: opacity = "opacity".asInstanceOf[opacity]
  
  @js.native
  sealed trait optimizeLegibility extends StObject
  inline def optimizeLegibility: optimizeLegibility = "optimizeLegibility".asInstanceOf[optimizeLegibility]
  
  @js.native
  sealed trait optimizeSpeed extends StObject
  inline def optimizeSpeed: optimizeSpeed = "optimizeSpeed".asInstanceOf[optimizeSpeed]
  
  @js.native
  sealed trait order extends StObject
  inline def order: order = "order".asInstanceOf[order]
  
  @js.native
  sealed trait orphans extends StObject
  inline def orphans: orphans = "orphans".asInstanceOf[orphans]
  
  @js.native
  sealed trait outline extends StObject
  inline def outline: outline = "outline".asInstanceOf[outline]
  
  @js.native
  sealed trait outlineColor extends StObject
  inline def outlineColor: outlineColor = "outlineColor".asInstanceOf[outlineColor]
  
  @js.native
  sealed trait outlineOffset extends StObject
  inline def outlineOffset: outlineOffset = "outlineOffset".asInstanceOf[outlineOffset]
  
  @js.native
  sealed trait outset extends StObject
  inline def outset: outset = "outset".asInstanceOf[outset]
  
  @js.native
  sealed trait outside extends StObject
  inline def outside: outside = "outside".asInstanceOf[outside]
  
  @js.native
  sealed trait over extends StObject
  inline def over: over = "over".asInstanceOf[over]
  
  @js.native
  sealed trait overflow extends StObject
  inline def overflow: overflow = "overflow".asInstanceOf[overflow]
  
  @js.native
  sealed trait overflowStyle extends StObject
  inline def overflowStyle: overflowStyle = "overflowStyle".asInstanceOf[overflowStyle]
  
  @js.native
  sealed trait overflowX extends StObject
  inline def overflowX: overflowX = "overflowX".asInstanceOf[overflowX]
  
  @js.native
  sealed trait overflowY extends StObject
  inline def overflowY: overflowY = "overflowY".asInstanceOf[overflowY]
  
  @js.native
  sealed trait overlay extends StObject
  inline def overlay: overlay = "overlay".asInstanceOf[overlay]
  
  @js.native
  sealed trait padding extends StObject
  inline def padding: padding = "padding".asInstanceOf[padding]
  
  @js.native
  sealed trait `padding-box` extends StObject
  inline def `padding-box`: `padding-box` = "padding-box".asInstanceOf[`padding-box`]
  
  @js.native
  sealed trait paddingBottom extends StObject
  inline def paddingBottom: paddingBottom = "paddingBottom".asInstanceOf[paddingBottom]
  
  @js.native
  sealed trait paddingLeft extends StObject
  inline def paddingLeft: paddingLeft = "paddingLeft".asInstanceOf[paddingLeft]
  
  @js.native
  sealed trait paddingRight extends StObject
  inline def paddingRight: paddingRight = "paddingRight".asInstanceOf[paddingRight]
  
  @js.native
  sealed trait paddingTop extends StObject
  inline def paddingTop: paddingTop = "paddingTop".asInstanceOf[paddingTop]
  
  @js.native
  sealed trait page extends StObject
  inline def page: page = "page".asInstanceOf[page]
  
  @js.native
  sealed trait pageBreakAfter extends StObject
  inline def pageBreakAfter: pageBreakAfter = "pageBreakAfter".asInstanceOf[pageBreakAfter]
  
  @js.native
  sealed trait pageBreakBefore extends StObject
  inline def pageBreakBefore: pageBreakBefore = "pageBreakBefore".asInstanceOf[pageBreakBefore]
  
  @js.native
  sealed trait pageBreakInside extends StObject
  inline def pageBreakInside: pageBreakInside = "pageBreakInside".asInstanceOf[pageBreakInside]
  
  @js.native
  sealed trait painted extends StObject
  inline def painted: painted = "painted".asInstanceOf[painted]
  
  @js.native
  sealed trait pause extends StObject
  inline def pause: pause = "pause".asInstanceOf[pause]
  
  @js.native
  sealed trait pauseAfter extends StObject
  inline def pauseAfter: pauseAfter = "pauseAfter".asInstanceOf[pauseAfter]
  
  @js.native
  sealed trait pauseBefore extends StObject
  inline def pauseBefore: pauseBefore = "pauseBefore".asInstanceOf[pauseBefore]
  
  @js.native
  sealed trait paused extends StObject
  inline def paused: paused = "paused".asInstanceOf[paused]
  
  @js.native
  sealed trait perspective extends StObject
  inline def perspective: perspective = "perspective".asInstanceOf[perspective]
  
  @js.native
  sealed trait perspectiveOrigin extends StObject
  inline def perspectiveOrigin: perspectiveOrigin = "perspectiveOrigin".asInstanceOf[perspectiveOrigin]
  
  @js.native
  sealed trait plaintext extends StObject
  inline def plaintext: plaintext = "plaintext".asInstanceOf[plaintext]
  
  @js.native
  sealed trait pointerEvents extends StObject
  inline def pointerEvents: pointerEvents = "pointerEvents".asInstanceOf[pointerEvents]
  
  @js.native
  sealed trait position extends StObject
  inline def position: position = "position".asInstanceOf[position]
  
  @js.native
  sealed trait pre extends StObject
  inline def pre: pre = "pre".asInstanceOf[pre]
  
  @js.native
  sealed trait `pre-line` extends StObject
  inline def `pre-line`: `pre-line` = "pre-line".asInstanceOf[`pre-line`]
  
  @js.native
  sealed trait `pre-wrap` extends StObject
  inline def `pre-wrap`: `pre-wrap` = "pre-wrap".asInstanceOf[`pre-wrap`]
  
  @js.native
  sealed trait `preserve-3d` extends StObject
  inline def `preserve-3d`: `preserve-3d` = "preserve-3d".asInstanceOf[`preserve-3d`]
  
  @js.native
  sealed trait punctuationTrim extends StObject
  inline def punctuationTrim: punctuationTrim = "punctuationTrim".asInstanceOf[punctuationTrim]
  
  @js.native
  sealed trait quotes extends StObject
  inline def quotes: quotes = "quotes".asInstanceOf[quotes]
  
  @js.native
  sealed trait recto extends StObject
  inline def recto: recto = "recto".asInstanceOf[recto]
  
  @js.native
  sealed trait region extends StObject
  inline def region: region = "region".asInstanceOf[region]
  
  @js.native
  sealed trait regionFragment extends StObject
  inline def regionFragment: regionFragment = "regionFragment".asInstanceOf[regionFragment]
  
  @js.native
  sealed trait relative extends StObject
  inline def relative: relative = "relative".asInstanceOf[relative]
  
  @js.native
  sealed trait restAfter extends StObject
  inline def restAfter: restAfter = "restAfter".asInstanceOf[restAfter]
  
  @js.native
  sealed trait restBefore extends StObject
  inline def restBefore: restBefore = "restBefore".asInstanceOf[restBefore]
  
  @js.native
  sealed trait reverse extends StObject
  inline def reverse: reverse = "reverse".asInstanceOf[reverse]
  
  @js.native
  sealed trait ridge extends StObject
  inline def ridge: ridge = "ridge".asInstanceOf[ridge]
  
  @js.native
  sealed trait right extends StObject
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait rightwards extends StObject
  inline def rightwards: rightwards = "rightwards".asInstanceOf[rightwards]
  
  @js.native
  sealed trait row extends StObject
  inline def row: row = "row".asInstanceOf[row]
  
  @js.native
  sealed trait `row nowrap` extends StObject
  inline def `row nowrap`: `row nowrap` = ("row nowrap").asInstanceOf[`row nowrap`]
  
  @js.native
  sealed trait `row wrap` extends StObject
  inline def `row wrap`: `row wrap` = ("row wrap").asInstanceOf[`row wrap`]
  
  @js.native
  sealed trait `row wrap-reverse` extends StObject
  inline def `row wrap-reverse`: `row wrap-reverse` = ("row wrap-reverse").asInstanceOf[`row wrap-reverse`]
  
  @js.native
  sealed trait `row-reverse` extends StObject
  inline def `row-reverse`: `row-reverse` = "row-reverse".asInstanceOf[`row-reverse`]
  
  @js.native
  sealed trait `row-reverse nowrap` extends StObject
  inline def `row-reverse nowrap`: `row-reverse nowrap` = ("row-reverse nowrap").asInstanceOf[`row-reverse nowrap`]
  
  @js.native
  sealed trait `row-reverse wrap` extends StObject
  inline def `row-reverse wrap`: `row-reverse wrap` = ("row-reverse wrap").asInstanceOf[`row-reverse wrap`]
  
  @js.native
  sealed trait `row-reverse wrap-reverse` extends StObject
  inline def `row-reverse wrap-reverse`: `row-reverse wrap-reverse` = ("row-reverse wrap-reverse").asInstanceOf[`row-reverse wrap-reverse`]
  
  @js.native
  sealed trait rtl extends StObject
  inline def rtl: rtl = "rtl".asInstanceOf[rtl]
  
  @js.native
  sealed trait ruby extends StObject
  inline def ruby: ruby = "ruby".asInstanceOf[ruby]
  
  @js.native
  sealed trait `ruby-base` extends StObject
  inline def `ruby-base`: `ruby-base` = "ruby-base".asInstanceOf[`ruby-base`]
  
  @js.native
  sealed trait `ruby-base-container` extends StObject
  inline def `ruby-base-container`: `ruby-base-container` = "ruby-base-container".asInstanceOf[`ruby-base-container`]
  
  @js.native
  sealed trait `ruby-text` extends StObject
  inline def `ruby-text`: `ruby-text` = "ruby-text".asInstanceOf[`ruby-text`]
  
  @js.native
  sealed trait `ruby-text-container` extends StObject
  inline def `ruby-text-container`: `ruby-text-container` = "ruby-text-container".asInstanceOf[`ruby-text-container`]
  
  @js.native
  sealed trait rubyAlign extends StObject
  inline def rubyAlign: rubyAlign = "rubyAlign".asInstanceOf[rubyAlign]
  
  @js.native
  sealed trait rubyPosition extends StObject
  inline def rubyPosition: rubyPosition = "rubyPosition".asInstanceOf[rubyPosition]
  
  @js.native
  sealed trait `run-in` extends StObject
  inline def `run-in`: `run-in` = "run-in".asInstanceOf[`run-in`]
  
  @js.native
  sealed trait running extends StObject
  inline def running: running = "running".asInstanceOf[running]
  
  @js.native
  sealed trait saturation extends StObject
  inline def saturation: saturation = "saturation".asInstanceOf[saturation]
  
  @js.native
  sealed trait screen extends StObject
  inline def screen: screen = "screen".asInstanceOf[screen]
  
  @js.native
  sealed trait scroll extends StObject
  inline def scroll: scroll = "scroll".asInstanceOf[scroll]
  
  @js.native
  sealed trait scrollbar extends StObject
  inline def scrollbar: scrollbar = "scrollbar".asInstanceOf[scrollbar]
  
  @js.native
  sealed trait `select-after` extends StObject
  inline def `select-after`: `select-after` = "select-after".asInstanceOf[`select-after`]
  
  @js.native
  sealed trait `select-all` extends StObject
  inline def `select-all`: `select-all` = "select-all".asInstanceOf[`select-all`]
  
  @js.native
  sealed trait `select-before` extends StObject
  inline def `select-before`: `select-before` = "select-before".asInstanceOf[`select-before`]
  
  @js.native
  sealed trait `select-menu` extends StObject
  inline def `select-menu`: `select-menu` = "select-menu".asInstanceOf[`select-menu`]
  
  @js.native
  sealed trait `select-same` extends StObject
  inline def `select-same`: `select-same` = "select-same".asInstanceOf[`select-same`]
  
  @js.native
  sealed trait `semi-condensed` extends StObject
  inline def `semi-condensed`: `semi-condensed` = "semi-condensed".asInstanceOf[`semi-condensed`]
  
  @js.native
  sealed trait `semi-expanded` extends StObject
  inline def `semi-expanded`: `semi-expanded` = "semi-expanded".asInstanceOf[`semi-expanded`]
  
  @js.native
  sealed trait separate extends StObject
  inline def separate: separate = "separate".asInstanceOf[separate]
  
  @js.native
  sealed trait shapeImageThreshold extends StObject
  inline def shapeImageThreshold: shapeImageThreshold = "shapeImageThreshold".asInstanceOf[shapeImageThreshold]
  
  @js.native
  sealed trait shapeInside extends StObject
  inline def shapeInside: shapeInside = "shapeInside".asInstanceOf[shapeInside]
  
  @js.native
  sealed trait shapeMargin extends StObject
  inline def shapeMargin: shapeMargin = "shapeMargin".asInstanceOf[shapeMargin]
  
  @js.native
  sealed trait shapeOutside extends StObject
  inline def shapeOutside: shapeOutside = "shapeOutside".asInstanceOf[shapeOutside]
  
  @js.native
  sealed trait `sideways-lr` extends StObject
  inline def `sideways-lr`: `sideways-lr` = "sideways-lr".asInstanceOf[`sideways-lr`]
  
  @js.native
  sealed trait `sideways-rl` extends StObject
  inline def `sideways-rl`: `sideways-rl` = "sideways-rl".asInstanceOf[`sideways-rl`]
  
  @js.native
  sealed trait single extends StObject
  inline def single: single = "single".asInstanceOf[single]
  
  @js.native
  sealed trait slice extends StObject
  inline def slice: slice = "slice".asInstanceOf[slice]
  
  @js.native
  sealed trait `soft-light` extends StObject
  inline def `soft-light`: `soft-light` = "soft-light".asInstanceOf[`soft-light`]
  
  @js.native
  sealed trait solid extends StObject
  inline def solid: solid = "solid".asInstanceOf[solid]
  
  @js.native
  sealed trait `space-around` extends StObject
  inline def `space-around`: `space-around` = "space-around".asInstanceOf[`space-around`]
  
  @js.native
  sealed trait `space-between` extends StObject
  inline def `space-between`: `space-between` = "space-between".asInstanceOf[`space-between`]
  
  @js.native
  sealed trait `space-evenly` extends StObject
  inline def `space-evenly`: `space-evenly` = "space-evenly".asInstanceOf[`space-evenly`]
  
  @js.native
  sealed trait speak extends StObject
  inline def speak: speak = "speak".asInstanceOf[speak]
  
  @js.native
  sealed trait speakAs extends StObject
  inline def speakAs: speakAs = "speakAs".asInstanceOf[speakAs]
  
  @js.native
  sealed trait start extends StObject
  inline def start: start = "start".asInstanceOf[start]
  
  @js.native
  sealed trait static extends StObject
  inline def static: static = "static".asInstanceOf[static]
  
  @js.native
  sealed trait sticky extends StObject
  inline def sticky: sticky = "sticky".asInstanceOf[sticky]
  
  @js.native
  sealed trait stretch extends StObject
  inline def stretch: stretch = "stretch".asInstanceOf[stretch]
  
  @js.native
  sealed trait strict extends StObject
  inline def strict: strict = "strict".asInstanceOf[strict]
  
  @js.native
  sealed trait stroke extends StObject
  inline def stroke: stroke = "stroke".asInstanceOf[stroke]
  
  @js.native
  sealed trait strokeOpacity extends StObject
  inline def strokeOpacity: strokeOpacity = "strokeOpacity".asInstanceOf[strokeOpacity]
  
  @js.native
  sealed trait strokeWidth extends StObject
  inline def strokeWidth: strokeWidth = "strokeWidth".asInstanceOf[strokeWidth]
  
  @js.native
  sealed trait subgrid extends StObject
  inline def subgrid: subgrid = "subgrid".asInstanceOf[subgrid]
  
  @js.native
  sealed trait tabSize extends StObject
  inline def tabSize: tabSize = "tabSize".asInstanceOf[tabSize]
  
  @js.native
  sealed trait table extends StObject
  inline def table: table = "table".asInstanceOf[table]
  
  @js.native
  sealed trait `table-caption` extends StObject
  inline def `table-caption`: `table-caption` = "table-caption".asInstanceOf[`table-caption`]
  
  @js.native
  sealed trait `table-cell` extends StObject
  inline def `table-cell`: `table-cell` = "table-cell".asInstanceOf[`table-cell`]
  
  @js.native
  sealed trait `table-column` extends StObject
  inline def `table-column`: `table-column` = "table-column".asInstanceOf[`table-column`]
  
  @js.native
  sealed trait `table-column-group` extends StObject
  inline def `table-column-group`: `table-column-group` = "table-column-group".asInstanceOf[`table-column-group`]
  
  @js.native
  sealed trait `table-footer-group` extends StObject
  inline def `table-footer-group`: `table-footer-group` = "table-footer-group".asInstanceOf[`table-footer-group`]
  
  @js.native
  sealed trait `table-header-group` extends StObject
  inline def `table-header-group`: `table-header-group` = "table-header-group".asInstanceOf[`table-header-group`]
  
  @js.native
  sealed trait `table-row` extends StObject
  inline def `table-row`: `table-row` = "table-row".asInstanceOf[`table-row`]
  
  @js.native
  sealed trait `table-row-group` extends StObject
  inline def `table-row-group`: `table-row-group` = "table-row-group".asInstanceOf[`table-row-group`]
  
  @js.native
  sealed trait tableLayout extends StObject
  inline def tableLayout: tableLayout = "tableLayout".asInstanceOf[tableLayout]
  
  @js.native
  sealed trait `text-after-edge` extends StObject
  inline def `text-after-edge`: `text-after-edge` = "text-after-edge".asInstanceOf[`text-after-edge`]
  
  @js.native
  sealed trait `text-before-edge` extends StObject
  inline def `text-before-edge`: `text-before-edge` = "text-before-edge".asInstanceOf[`text-before-edge`]
  
  @js.native
  sealed trait textAlign extends StObject
  inline def textAlign: textAlign = "textAlign".asInstanceOf[textAlign]
  
  @js.native
  sealed trait textAlignLast extends StObject
  inline def textAlignLast: textAlignLast = "textAlignLast".asInstanceOf[textAlignLast]
  
  @js.native
  sealed trait textDecoration extends StObject
  inline def textDecoration: textDecoration = "textDecoration".asInstanceOf[textDecoration]
  
  @js.native
  sealed trait textDecorationColor extends StObject
  inline def textDecorationColor: textDecorationColor = "textDecorationColor".asInstanceOf[textDecorationColor]
  
  @js.native
  sealed trait textDecorationLine extends StObject
  inline def textDecorationLine: textDecorationLine = "textDecorationLine".asInstanceOf[textDecorationLine]
  
  @js.native
  sealed trait textDecorationLineThrough extends StObject
  inline def textDecorationLineThrough: textDecorationLineThrough = "textDecorationLineThrough".asInstanceOf[textDecorationLineThrough]
  
  @js.native
  sealed trait textDecorationNone extends StObject
  inline def textDecorationNone: textDecorationNone = "textDecorationNone".asInstanceOf[textDecorationNone]
  
  @js.native
  sealed trait textDecorationOverline extends StObject
  inline def textDecorationOverline: textDecorationOverline = "textDecorationOverline".asInstanceOf[textDecorationOverline]
  
  @js.native
  sealed trait textDecorationSkip extends StObject
  inline def textDecorationSkip: textDecorationSkip = "textDecorationSkip".asInstanceOf[textDecorationSkip]
  
  @js.native
  sealed trait textDecorationStyle extends StObject
  inline def textDecorationStyle: textDecorationStyle = "textDecorationStyle".asInstanceOf[textDecorationStyle]
  
  @js.native
  sealed trait textDecorationUnderline extends StObject
  inline def textDecorationUnderline: textDecorationUnderline = "textDecorationUnderline".asInstanceOf[textDecorationUnderline]
  
  @js.native
  sealed trait textEmphasis extends StObject
  inline def textEmphasis: textEmphasis = "textEmphasis".asInstanceOf[textEmphasis]
  
  @js.native
  sealed trait textEmphasisColor extends StObject
  inline def textEmphasisColor: textEmphasisColor = "textEmphasisColor".asInstanceOf[textEmphasisColor]
  
  @js.native
  sealed trait textEmphasisStyle extends StObject
  inline def textEmphasisStyle: textEmphasisStyle = "textEmphasisStyle".asInstanceOf[textEmphasisStyle]
  
  @js.native
  sealed trait textHeight extends StObject
  inline def textHeight: textHeight = "textHeight".asInstanceOf[textHeight]
  
  @js.native
  sealed trait textIndent extends StObject
  inline def textIndent: textIndent = "textIndent".asInstanceOf[textIndent]
  
  @js.native
  sealed trait textJustifyTrim extends StObject
  inline def textJustifyTrim: textJustifyTrim = "textJustifyTrim".asInstanceOf[textJustifyTrim]
  
  @js.native
  sealed trait textKashidaSpace extends StObject
  inline def textKashidaSpace: textKashidaSpace = "textKashidaSpace".asInstanceOf[textKashidaSpace]
  
  @js.native
  sealed trait textLineThrough extends StObject
  inline def textLineThrough: textLineThrough = "textLineThrough".asInstanceOf[textLineThrough]
  
  @js.native
  sealed trait textLineThroughColor extends StObject
  inline def textLineThroughColor: textLineThroughColor = "textLineThroughColor".asInstanceOf[textLineThroughColor]
  
  @js.native
  sealed trait textLineThroughMode extends StObject
  inline def textLineThroughMode: textLineThroughMode = "textLineThroughMode".asInstanceOf[textLineThroughMode]
  
  @js.native
  sealed trait textLineThroughStyle extends StObject
  inline def textLineThroughStyle: textLineThroughStyle = "textLineThroughStyle".asInstanceOf[textLineThroughStyle]
  
  @js.native
  sealed trait textLineThroughWidth extends StObject
  inline def textLineThroughWidth: textLineThroughWidth = "textLineThroughWidth".asInstanceOf[textLineThroughWidth]
  
  @js.native
  sealed trait textOverflow extends StObject
  inline def textOverflow: textOverflow = "textOverflow".asInstanceOf[textOverflow]
  
  @js.native
  sealed trait textOverline extends StObject
  inline def textOverline: textOverline = "textOverline".asInstanceOf[textOverline]
  
  @js.native
  sealed trait textOverlineColor extends StObject
  inline def textOverlineColor: textOverlineColor = "textOverlineColor".asInstanceOf[textOverlineColor]
  
  @js.native
  sealed trait textOverlineMode extends StObject
  inline def textOverlineMode: textOverlineMode = "textOverlineMode".asInstanceOf[textOverlineMode]
  
  @js.native
  sealed trait textOverlineStyle extends StObject
  inline def textOverlineStyle: textOverlineStyle = "textOverlineStyle".asInstanceOf[textOverlineStyle]
  
  @js.native
  sealed trait textOverlineWidth extends StObject
  inline def textOverlineWidth: textOverlineWidth = "textOverlineWidth".asInstanceOf[textOverlineWidth]
  
  @js.native
  sealed trait textRendering extends StObject
  inline def textRendering: textRendering = "textRendering".asInstanceOf[textRendering]
  
  @js.native
  sealed trait textScript extends StObject
  inline def textScript: textScript = "textScript".asInstanceOf[textScript]
  
  @js.native
  sealed trait textShadow extends StObject
  inline def textShadow: textShadow = "textShadow".asInstanceOf[textShadow]
  
  @js.native
  sealed trait textTransform extends StObject
  inline def textTransform: textTransform = "textTransform".asInstanceOf[textTransform]
  
  @js.native
  sealed trait textUnderlinePosition extends StObject
  inline def textUnderlinePosition: textUnderlinePosition = "textUnderlinePosition".asInstanceOf[textUnderlinePosition]
  
  @js.native
  sealed trait textUnderlineStyle extends StObject
  inline def textUnderlineStyle: textUnderlineStyle = "textUnderlineStyle".asInstanceOf[textUnderlineStyle]
  
  @js.native
  sealed trait top extends StObject
  inline def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait touchAction extends StObject
  inline def touchAction: touchAction = "touchAction".asInstanceOf[touchAction]
  
  @js.native
  sealed trait transform extends StObject
  inline def transform: transform = "transform".asInstanceOf[transform]
  
  @js.native
  sealed trait transformOrigin extends StObject
  inline def transformOrigin: transformOrigin = "transformOrigin".asInstanceOf[transformOrigin]
  
  @js.native
  sealed trait transformOriginZ extends StObject
  inline def transformOriginZ: transformOriginZ = "transformOriginZ".asInstanceOf[transformOriginZ]
  
  @js.native
  sealed trait transformStyle extends StObject
  inline def transformStyle: transformStyle = "transformStyle".asInstanceOf[transformStyle]
  
  @js.native
  sealed trait transition extends StObject
  inline def transition: transition = "transition".asInstanceOf[transition]
  
  @js.native
  sealed trait transitionDelay extends StObject
  inline def transitionDelay: transitionDelay = "transitionDelay".asInstanceOf[transitionDelay]
  
  @js.native
  sealed trait transitionDuration extends StObject
  inline def transitionDuration: transitionDuration = "transitionDuration".asInstanceOf[transitionDuration]
  
  @js.native
  sealed trait transitionProperty extends StObject
  inline def transitionProperty: transitionProperty = "transitionProperty".asInstanceOf[transitionProperty]
  
  @js.native
  sealed trait transitionTimingFunction extends StObject
  inline def transitionTimingFunction: transitionTimingFunction = "transitionTimingFunction".asInstanceOf[transitionTimingFunction]
  
  @js.native
  sealed trait trim extends StObject
  inline def trim: trim = "trim".asInstanceOf[trim]
  
  @js.native
  sealed trait `ultra-condensed` extends StObject
  inline def `ultra-condensed`: `ultra-condensed` = "ultra-condensed".asInstanceOf[`ultra-condensed`]
  
  @js.native
  sealed trait `ultra-expanded` extends StObject
  inline def `ultra-expanded`: `ultra-expanded` = "ultra-expanded".asInstanceOf[`ultra-expanded`]
  
  @js.native
  sealed trait under extends StObject
  inline def under: under = "under".asInstanceOf[under]
  
  @js.native
  sealed trait unicodeBidi extends StObject
  inline def unicodeBidi: unicodeBidi = "unicodeBidi".asInstanceOf[unicodeBidi]
  
  @js.native
  sealed trait unicodeRange extends StObject
  inline def unicodeRange: unicodeRange = "unicodeRange".asInstanceOf[unicodeRange]
  
  @js.native
  sealed trait unset
    extends StObject
       with CSSWideKeyword
  inline def unset: unset = "unset".asInstanceOf[unset]
  
  @js.native
  sealed trait uppercase extends StObject
  inline def uppercase: uppercase = "uppercase".asInstanceOf[uppercase]
  
  @js.native
  sealed trait `use-script` extends StObject
  inline def `use-script`: `use-script` = "use-script".asInstanceOf[`use-script`]
  
  @js.native
  sealed trait userFocus extends StObject
  inline def userFocus: userFocus = "userFocus".asInstanceOf[userFocus]
  
  @js.native
  sealed trait userInput extends StObject
  inline def userInput: userInput = "userInput".asInstanceOf[userInput]
  
  @js.native
  sealed trait verso extends StObject
  inline def verso: verso = "verso".asInstanceOf[verso]
  
  @js.native
  sealed trait `vertical-lr` extends StObject
  inline def `vertical-lr`: `vertical-lr` = "vertical-lr".asInstanceOf[`vertical-lr`]
  
  @js.native
  sealed trait `vertical-rl` extends StObject
  inline def `vertical-rl`: `vertical-rl` = "vertical-rl".asInstanceOf[`vertical-rl`]
  
  @js.native
  sealed trait verticalAlign extends StObject
  inline def verticalAlign: verticalAlign = "verticalAlign".asInstanceOf[verticalAlign]
  
  @js.native
  sealed trait visibility extends StObject
  inline def visibility: visibility = "visibility".asInstanceOf[visibility]
  
  @js.native
  sealed trait visible extends StObject
  inline def visible: visible = "visible".asInstanceOf[visible]
  
  @js.native
  sealed trait visibleFill extends StObject
  inline def visibleFill: visibleFill = "visibleFill".asInstanceOf[visibleFill]
  
  @js.native
  sealed trait visiblePainted extends StObject
  inline def visiblePainted: visiblePainted = "visiblePainted".asInstanceOf[visiblePainted]
  
  @js.native
  sealed trait visibleStroke extends StObject
  inline def visibleStroke: visibleStroke = "visibleStroke".asInstanceOf[visibleStroke]
  
  @js.native
  sealed trait voiceBalance extends StObject
  inline def voiceBalance: voiceBalance = "voiceBalance".asInstanceOf[voiceBalance]
  
  @js.native
  sealed trait voiceDuration extends StObject
  inline def voiceDuration: voiceDuration = "voiceDuration".asInstanceOf[voiceDuration]
  
  @js.native
  sealed trait voiceFamily extends StObject
  inline def voiceFamily: voiceFamily = "voiceFamily".asInstanceOf[voiceFamily]
  
  @js.native
  sealed trait voicePitch extends StObject
  inline def voicePitch: voicePitch = "voicePitch".asInstanceOf[voicePitch]
  
  @js.native
  sealed trait voiceRange extends StObject
  inline def voiceRange: voiceRange = "voiceRange".asInstanceOf[voiceRange]
  
  @js.native
  sealed trait voiceRate extends StObject
  inline def voiceRate: voiceRate = "voiceRate".asInstanceOf[voiceRate]
  
  @js.native
  sealed trait voiceStress extends StObject
  inline def voiceStress: voiceStress = "voiceStress".asInstanceOf[voiceStress]
  
  @js.native
  sealed trait voiceVolume extends StObject
  inline def voiceVolume: voiceVolume = "voiceVolume".asInstanceOf[voiceVolume]
  
  @js.native
  sealed trait whiteSpace extends StObject
  inline def whiteSpace: whiteSpace = "whiteSpace".asInstanceOf[whiteSpace]
  
  @js.native
  sealed trait whiteSpaceTreatment extends StObject
  inline def whiteSpaceTreatment: whiteSpaceTreatment = "whiteSpaceTreatment".asInstanceOf[whiteSpaceTreatment]
  
  @js.native
  sealed trait widows extends StObject
  inline def widows: widows = "widows".asInstanceOf[widows]
  
  @js.native
  sealed trait width extends StObject
  inline def width: width = "width".asInstanceOf[width]
  
  @js.native
  sealed trait wordBreak extends StObject
  inline def wordBreak: wordBreak = "wordBreak".asInstanceOf[wordBreak]
  
  @js.native
  sealed trait wordSpacing extends StObject
  inline def wordSpacing: wordSpacing = "wordSpacing".asInstanceOf[wordSpacing]
  
  @js.native
  sealed trait wordWrap extends StObject
  inline def wordWrap: wordWrap = "wordWrap".asInstanceOf[wordWrap]
  
  @js.native
  sealed trait wrap extends StObject
  inline def wrap: wrap = "wrap".asInstanceOf[wrap]
  
  @js.native
  sealed trait `wrap column` extends StObject
  inline def `wrap column`: `wrap column` = ("wrap column").asInstanceOf[`wrap column`]
  
  @js.native
  sealed trait `wrap column-reverse` extends StObject
  inline def `wrap column-reverse`: `wrap column-reverse` = ("wrap column-reverse").asInstanceOf[`wrap column-reverse`]
  
  @js.native
  sealed trait `wrap row` extends StObject
  inline def `wrap row`: `wrap row` = ("wrap row").asInstanceOf[`wrap row`]
  
  @js.native
  sealed trait `wrap row-reverse` extends StObject
  inline def `wrap row-reverse`: `wrap row-reverse` = ("wrap row-reverse").asInstanceOf[`wrap row-reverse`]
  
  @js.native
  sealed trait `wrap-reverse` extends StObject
  inline def `wrap-reverse`: `wrap-reverse` = "wrap-reverse".asInstanceOf[`wrap-reverse`]
  
  @js.native
  sealed trait `wrap-reverse column` extends StObject
  inline def `wrap-reverse column`: `wrap-reverse column` = ("wrap-reverse column").asInstanceOf[`wrap-reverse column`]
  
  @js.native
  sealed trait `wrap-reverse column-reverse` extends StObject
  inline def `wrap-reverse column-reverse`: `wrap-reverse column-reverse` = ("wrap-reverse column-reverse").asInstanceOf[`wrap-reverse column-reverse`]
  
  @js.native
  sealed trait `wrap-reverse row` extends StObject
  inline def `wrap-reverse row`: `wrap-reverse row` = ("wrap-reverse row").asInstanceOf[`wrap-reverse row`]
  
  @js.native
  sealed trait `wrap-reverse row-reverse` extends StObject
  inline def `wrap-reverse row-reverse`: `wrap-reverse row-reverse` = ("wrap-reverse row-reverse").asInstanceOf[`wrap-reverse row-reverse`]
  
  @js.native
  sealed trait wrapFlow extends StObject
  inline def wrapFlow: wrapFlow = "wrapFlow".asInstanceOf[wrapFlow]
  
  @js.native
  sealed trait wrapMargin extends StObject
  inline def wrapMargin: wrapMargin = "wrapMargin".asInstanceOf[wrapMargin]
  
  @js.native
  sealed trait wrapOption extends StObject
  inline def wrapOption: wrapOption = "wrapOption".asInstanceOf[wrapOption]
  
  @js.native
  sealed trait writingMode extends StObject
  inline def writingMode: writingMode = "writingMode".asInstanceOf[writingMode]
  
  @js.native
  sealed trait zIndex extends StObject
  inline def zIndex: zIndex = "zIndex".asInstanceOf[zIndex]
  
  @js.native
  sealed trait zoom extends StObject
  inline def zoom: zoom = "zoom".asInstanceOf[zoom]
}
