package typings.appleMapkitJsBrowser.mapkit

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

// prettier-ignore
/* Rewritten from type alias, can be one of: 
  - typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.select
  - typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.deselect
  - typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-start`
  - typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.dragging
  - typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-end`
*/
trait AnnotationEventType extends StObject
object AnnotationEventType {
  
  @scala.inline
  def deselect: typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.deselect = "deselect".asInstanceOf[typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.deselect]
  
  @scala.inline
  def `drag-end`: typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-end` = "drag-end".asInstanceOf[typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-end`]
  
  @scala.inline
  def `drag-start`: typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-start` = "drag-start".asInstanceOf[typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.`drag-start`]
  
  @scala.inline
  def dragging: typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.dragging = "dragging".asInstanceOf[typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.dragging]
  
  @scala.inline
  def select: typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.select = "select".asInstanceOf[typings.appleMapkitJsBrowser.appleMapkitJsBrowserStrings.select]
}
