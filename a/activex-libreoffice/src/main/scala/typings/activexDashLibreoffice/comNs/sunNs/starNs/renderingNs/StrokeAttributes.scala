...
trait StrokeAttributes extends js.Object {
  /**
    ...
    *
    * The first element specifies the length of the first "on" segment of the dashing, the second element the length of the first "off" segment, and so
  ...
  /**
    ...
    *
    * After every sub-stroke, i.e. after every line or curve segment within a single path polygon, a shape of this type is inserted into the stroke to glue
    * the segments together. Please note that distinct polygons within the path poly-polygon are not connected, and therefore also not joined via the shape
    * specified here.
    * @see PathJoinType
    */
  var JoinType: Double
  /**
    ...
    *
    * The entries here are relative to the {@link StrokeAttributes.StrokeWidth} attribute above, i.e. the total width of all lines and spacings will always
    * equal {@link StrokeAttributes.StrokeWidth} . The first element specifies the width of the rightmost line, when traveling from the start point of the
  ...
  /**
    ...
    *
    * This attribute is only used when {@link StrokeAttributes.JoinType} is set to {@link PathJoinType.MITER} . Should the length of a corner's diagonal
    * exceed this limit, a beveled join is used instead. This value must be positive (or 0.0, which is equivalent to setting {@link
    * StrokeAttributes.JoinType} to {@link PathJoinType.BEVEL} .
    *
