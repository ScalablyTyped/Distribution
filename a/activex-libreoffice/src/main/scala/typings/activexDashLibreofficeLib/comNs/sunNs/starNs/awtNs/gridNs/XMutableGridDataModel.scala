...
object XMutableGridDataModel {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(ColumnCount = ColumnCount, RowCount = RowCount, acquire = acquire, addEventListener = addEventListener, addGridDataListener = addGridDataListener, addRow = addRow, addRows = addRows, createClone = createClone, dispose = dispose, getCellData = getCellData, getCellToolTip = getCellToolTip, getRowData = getRowData, getRowHeading = getRowHeading, insertRow = insertRow, insertRows = insertRows, queryInterface = queryInterface, release = release, removeAllRows = removeAllRows, removeEventListener = removeEventListener, removeGridDataListener = removeGridDataListener, removeRow = removeRow, updateCellData = updateCellData, updateCellToolTip = updateCellToolTip, updateRowData = updateRowData, updateRowHeading = updateRowHeading, updateRowToolTip = updateRowToolTip)
