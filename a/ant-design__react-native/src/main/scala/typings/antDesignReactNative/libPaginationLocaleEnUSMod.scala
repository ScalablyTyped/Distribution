package typings.antDesignReactNative

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libPaginationLocaleEnUSMod {
  
  object default {
    
    @JSImport("@ant-design/react-native/lib/pagination/locale/en_US", JSImport.Default)
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("@ant-design/react-native/lib/pagination/locale/en_US", "default.nextText")
    @js.native
    def nextText: String = js.native
    inline def nextText_=(x: String): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("nextText")(x.asInstanceOf[js.Any])
    
    @JSImport("@ant-design/react-native/lib/pagination/locale/en_US", "default.prevText")
    @js.native
    def prevText: String = js.native
    inline def prevText_=(x: String): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("prevText")(x.asInstanceOf[js.Any])
  }
}
