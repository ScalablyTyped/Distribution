package typings.antDesignReactNative

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libDatePickerUtilsMod {
  
  @JSImport("@ant-design/react-native/lib/date-picker/utils", JSImport.Namespace)
  @js.native
  val ^ : js.Any = js.native
  
  inline def formatFn(instance: Any, value: js.Date): Any = (^.asInstanceOf[js.Dynamic].applyDynamic("formatFn")(instance.asInstanceOf[js.Any], value.asInstanceOf[js.Any])).asInstanceOf[Any]
  
  inline def formatProps(props: Any, value: js.Date): Any = (^.asInstanceOf[js.Dynamic].applyDynamic("formatProps")(props.asInstanceOf[js.Any], value.asInstanceOf[js.Any])).asInstanceOf[Any]
}
