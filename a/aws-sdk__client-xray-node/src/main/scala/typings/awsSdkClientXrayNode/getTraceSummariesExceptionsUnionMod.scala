package typings.awsSdkClientXrayNode

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@aws-sdk/client-xray-node/types/GetTraceSummariesExceptionsUnion", JSImport.Namespace)
@js.native
object getTraceSummariesExceptionsUnionMod extends js.Object {
  /* Rewritten from type alias, can be one of: 
    - typings.awsSdkClientXrayNode.typesInvalidRequestExceptionMod.InvalidRequestException
    - typings.awsSdkClientXrayNode.typesThrottledExceptionMod.ThrottledException
  */
  trait GetTraceSummariesExceptionsUnion extends js.Object
  
}

