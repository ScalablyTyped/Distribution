package typings.devtoolsProtocol.mod.Protocol.Audits

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteUnspecifiedTreatedAsLax
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteNoneInsecure
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteLax
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteStrict
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeInvalidSameParty
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSamePartyCrossPartyContext
  - typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeDomainNonASCII
*/
trait CookieExclusionReason extends StObject
object CookieExclusionReason {
  
  inline def ExcludeDomainNonASCII: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeDomainNonASCII = "ExcludeDomainNonASCII".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeDomainNonASCII]
  
  inline def ExcludeInvalidSameParty: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeInvalidSameParty = "ExcludeInvalidSameParty".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeInvalidSameParty]
  
  inline def ExcludeSamePartyCrossPartyContext: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSamePartyCrossPartyContext = "ExcludeSamePartyCrossPartyContext".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSamePartyCrossPartyContext]
  
  inline def ExcludeSameSiteLax: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteLax = "ExcludeSameSiteLax".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteLax]
  
  inline def ExcludeSameSiteNoneInsecure: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteNoneInsecure = "ExcludeSameSiteNoneInsecure".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteNoneInsecure]
  
  inline def ExcludeSameSiteStrict: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteStrict = "ExcludeSameSiteStrict".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteStrict]
  
  inline def ExcludeSameSiteUnspecifiedTreatedAsLax: typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteUnspecifiedTreatedAsLax = "ExcludeSameSiteUnspecifiedTreatedAsLax".asInstanceOf[typings.devtoolsProtocol.devtoolsProtocolStrings.ExcludeSameSiteUnspecifiedTreatedAsLax]
}
