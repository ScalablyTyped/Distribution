...
object distNodeTypesLegacyV46Mod {
  ...
  */
  ...
  {readonly map <U>(callbackfn : (value : unknown, index : number, array : std.Array<unknown>): U): std.Array<U>,  readonly length :number, readonly entries (): std.IterableIterator<[number, unknown]>, readonly findLast <S extends unknown>(predicate : (value : unknown, index : number, array : std.Array<unknown>): value is S): S | undefined, readonly join (): string, readonly keys (): std.IterableIterator<number>, readonly forEach (callbackfn : (value : unknown, index : number, array : std.Array<unknown>): void): void, readonly concat (items : ...std.ConcatArray<unknown>): std.Array<unknown>, readonly includes (searchElement : unknown): boolean, readonly slice (): std.Array<unknown>, readonly flat <A, D extends number = 1>(this : A): std.Array<std.FlatArray<A, D>>, readonly some (predicate : (value : unknown, index : number, array : std.Array<unknown>): unknown): boolean, readonly at (index : number): unknown | undefined, readonly filter <S extends unknown>(predicate : (value : unknown, index : number, array : std.Array<unknown>): value is S): std.Array<S>, readonly findLastIndex (predicate : (value : unknown, index : number, array : std.Array<unknown>): unknown): number, readonly toString (): string, readonly indexOf (searchElement : unknown): number, readonly reduceRight (callbackfn : (previousValue : unknown, currentValue : unknown, currentIndex : number, array : std.Array<unknown>): unknown): unknown, readonly findIndex (predicate : (value : unknown, index : number, obj : std.Array<unknown>): unknown): number, readonly reduce (callbackfn : (previousValue : unknown, currentValue : unknown, currentIndex : number, array : std.Array<unknown>): unknown): unknown, readonly values (): std.IterableIterator<unknown>, readonly flatMap <U, This = undefined>(callback : (this : This, value : unknown, index : number, array : std.Array<unknown>): U | std.ReadonlyArray<U>): std.Array<U>, readonly toLocaleString (): string, readonly every <S extends unknown>(predicate : (value : unknown, index : number, array : std.Array<unknown>): value is S): this is std.Array<S>, readonly lastIndexOf (searchElement : unknown): number, readonly find <S extends unknown>(predicate : (value : unknown, index : number, obj : std.Array<unknown>): value is S): S | undefined}]> ? deepmerge-ts.deepmerge-ts/dist/node/types/legacy/v4_6.FlatternAlias<std.Omit<deepmerge-ts.deepmerge-ts/dist/node/types/legacy/v4_6.DeepMergeRecordsDefaultHKTInternalProps<Ts, MF, M>, deepmerge-ts.deepmerge-ts/dist/node/types/legacy/v4_6.BlacklistedRecordProps>> : {}
    }}}
    */
