...
object GanttConfigOptions {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(auto_scheduling = auto_scheduling, auto_scheduling_descendant_links = auto_scheduling_descendant_links, auto_scheduling_initial = auto_scheduling_initial, auto_scheduling_move_projects = auto_scheduling_move_projects, auto_scheduling_strict = auto_scheduling_strict, auto_types = auto_types, autofit = autofit, autoscroll = autoscroll, autoscroll_speed = autoscroll_speed, autosize = autosize.asInstanceOf[js.Any], autosize_min_width = autosize_min_width, branch_loading = branch_loading, branch_loading_property = branch_loading_property, buttons_left = buttons_left, buttons_right = buttons_right, calendar_property = calendar_property, cascade_delete = cascade_delete, columns = columns, correct_work_time = correct_work_time, date_grid = date_grid, date_scale = date_scale, details_on_create = details_on_create, details_on_dblclick = details_on_dblclick, drag_lightbox = drag_lightbox, drag_links = drag_links, drag_mode = drag_mode, drag_move = drag_move, drag_progress = drag_progress, drag_project = drag_project, drag_resize = drag_resize, duration_step = duration_step, duration_unit = duration_unit, editable_property = editable_property, editor_types = editor_types, end_date = end_date, fit_tasks = fit_tasks, grid_resize = grid_resize, grid_resizer_attribute = grid_resizer_attribute, grid_resizer_column_attribute = grid_resizer_column_attribute, grid_width = grid_width, highlight_critical_path = highlight_critical_path, inherit_scale_class = inherit_scale_class, initial_scroll = initial_scroll, keep_grid_width = keep_grid_width, keyboard_navigation = keyboard_navigation, keyboard_navigation_cells = keyboard_navigation_cells, layer_attribute = layer_attribute, layout = layout, lightbox = lightbox, lightbox_additional_height = lightbox_additional_height, link_arrow_size = link_arrow_size, link_attribute = link_attribute, link_line_width = link_line_width, link_wrapper_width = link_wrapper_width, links = links, min_column_width = min_column_width, min_duration = min_duration, min_grid_column_width = min_grid_column_width, multiselect = multiselect, multiselect_one_level = multiselect_one_level, open_tree_initially = open_tree_initially, order_branch = order_branch.asInstanceOf[js.Any], order_branch_free = order_branch_free, placeholder_task = placeholder_task, preserve_scroll = preserve_scroll, prevent_default_scroll = prevent_default_scroll, quick_info_detached = quick_info_detached, quickinfo_buttons = quickinfo_buttons, readonly = readonly, readonly_property = readonly_property, redo = redo, resource_calendars = resource_calendars, resource_property = resource_property, resource_render_empty_cells = resource_render_empty_cells, resource_store = resource_store, root_id = root_id.asInstanceOf[js.Any], round_dnd_dates = round_dnd_dates, row_height = row_height, rtl = rtl, scale_height = scale_height, scale_offset_minimal = scale_offset_minimal, scale_unit = scale_unit, scroll_on_click = scroll_on_click, scroll_size = scroll_size, select_task = select_task, server_utc = server_utc, show_chart = show_chart, show_errors = show_errors, show_grid = show_grid, show_links = show_links, show_markers = show_markers, show_progress = show_progress, show_quick_info = show_quick_info, show_task_cells = show_task_cells, show_unscheduled = show_unscheduled, skip_off_time = skip_off_time, smart_rendering = smart_rendering, smart_scales = smart_scales, sort = sort, start_date = start_date, start_on_monday = start_on_monday, static_background = static_background, step = step, subscales = subscales, task_attribute = task_attribute, task_date = task_date, task_height = task_height, task_scroll_offset = task_scroll_offset, time_picker = time_picker, time_step = time_step, tooltip_hide_timeout = tooltip_hide_timeout, tooltip_offset_x = tooltip_offset_x, tooltip_offset_y = tooltip_offset_y, tooltip_timeout = tooltip_timeout, touch = touch.asInstanceOf[js.Any], touch_drag = touch_drag.asInstanceOf[js.Any], touch_feedback = touch_feedback, touch_feedback_duration = touch_feedback_duration, type_renderers = type_renderers, types = types, undo = undo, undo_actions = undo_actions, undo_steps = undo_steps, undo_types = undo_types, wai_aria_attributes = wai_aria_attributes, work_time = work_time, xml_date = xml_date)
