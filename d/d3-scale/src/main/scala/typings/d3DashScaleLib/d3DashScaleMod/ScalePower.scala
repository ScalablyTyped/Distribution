...
trait ScalePower[Range, Output] extends ScaleContinuousNumeric[Range, Output] {
  /**
    ...
    */
  def exponent(): scala.Double = js.native
  /**
    * Sets the current exponent to the given numeric value.
    * (Note that this is effectively a linear scale until you set a different exponent.)
    */
  def exponent(exponent: scala.Double): this.type = js.native
  /**
  ...
  /**
    ...
    */
  @JSName("interpolate")
  def interpolate_NewOutputScalePower[NewOutput](interpolate: InterpolatorFactory[Range, NewOutput]): ScalePower[Range, NewOutput] = js.native
}
