...
object defaults_ {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(anchors = anchors, angleSnap = angleSnap, arrows = arrows, button = button, clickMode = clickMode, clickable = clickable, copy = js.Any.fromFunction0(copy), current = current, currentHit = currentHit, disabled = disabled, highlighted = highlighted, hitHighlighted = hitHighlighted, hitNorm = hitNorm, hitSelected = hitSelected, labelSameColor = labelSameColor, norm = norm, renderHitLayer = renderHitLayer, renderHitLines = renderHitLines, selected = selected, text = text, textDisabled = textDisabled, textMode = textMode, useSelectedStyle = useSelectedStyle, zAngle = zAngle, zAxis = zAxis, zAxisEnabled = zAxisEnabled)
