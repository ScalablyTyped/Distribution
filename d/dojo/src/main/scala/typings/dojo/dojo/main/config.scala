...
object config {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addOnLoad = addOnLoad, afterOnLoad = afterOnLoad, baseUrl = baseUrl, callback = callback, debugContainerId = debugContainerId, debugHeight = debugHeight, defaultDuration = defaultDuration, deferredInstrumentation = deferredInstrumentation, deps = deps, dojoBlankHtmlUrl = dojoBlankHtmlUrl, extraLocale = extraLocale, ioPublish = ioPublish, isDebug = isDebug, locale = locale, modulePaths = modulePaths, parseOnLoad = parseOnLoad, require = require, transparentColor = transparentColor, urchin = urchin, useCustomLogger = useCustomLogger, useDeferredInstrumentation = useDeferredInstrumentation)
