package typings.glaze

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object glazeNumbers {
  
  @js.native
  sealed trait `0` extends StObject
  @scala.inline
  def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `1` extends StObject
  @scala.inline
  def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `1.25` extends StObject
  @scala.inline
  def `1.25`: `1.25` = 1.25.asInstanceOf[`1.25`]
  
  @js.native
  sealed trait `1.375` extends StObject
  @scala.inline
  def `1.375`: `1.375` = 1.375.asInstanceOf[`1.375`]
  
  @js.native
  sealed trait `1.5` extends StObject
  @scala.inline
  def `1.5`: `1.5` = 1.5.asInstanceOf[`1.5`]
  
  @js.native
  sealed trait `1.625` extends StObject
  @scala.inline
  def `1.625`: `1.625` = 1.625.asInstanceOf[`1.625`]
  
  @js.native
  sealed trait `1024` extends StObject
  @scala.inline
  def `1024`: `1024` = 1024.asInstanceOf[`1024`]
  
  @js.native
  sealed trait `1280` extends StObject
  @scala.inline
  def `1280`: `1280` = 1280.asInstanceOf[`1280`]
  
  @js.native
  sealed trait `2` extends StObject
  @scala.inline
  def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `4` extends StObject
  @scala.inline
  def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `640` extends StObject
  @scala.inline
  def `640`: `640` = 640.asInstanceOf[`640`]
  
  @js.native
  sealed trait `768` extends StObject
  @scala.inline
  def `768`: `768` = 768.asInstanceOf[`768`]
  
  @js.native
  sealed trait `8` extends StObject
  @scala.inline
  def `8`: `8` = 8.asInstanceOf[`8`]
  
  @js.native
  sealed trait `9999` extends StObject
  @scala.inline
  def `9999`: `9999` = 9999.asInstanceOf[`9999`]
}
