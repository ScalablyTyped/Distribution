package typings.gitlab.distTypesCoreTemplatesResourceAccessRequestsMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typings.gitlab.gitlabNumbers.`10`
  - typings.gitlab.gitlabNumbers.`20`
  - typings.gitlab.gitlabNumbers.`30`
  - typings.gitlab.gitlabNumbers.`40`
  - typings.gitlab.gitlabNumbers.`50`
*/
trait AccessLevel extends js.Object

object AccessLevel {
  @scala.inline
  def `10`: typings.gitlab.gitlabNumbers.`10` = this.cast(10)
  @scala.inline
  def `20`: typings.gitlab.gitlabNumbers.`20` = this.cast(20)
  @scala.inline
  def `30`: typings.gitlab.gitlabNumbers.`30` = this.cast(30)
  @scala.inline
  def `40`: typings.gitlab.gitlabNumbers.`40` = this.cast(40)
  @scala.inline
  def `50`: typings.gitlab.gitlabNumbers.`50` = this.cast(50)
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
}

