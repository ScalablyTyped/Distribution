...
trait SendAsResource extends js.Object {
  ...
  def get(request: AnonAltFieldsKey): Request_[SendAs]
  /**
  ...
  /**
    ...
    */
  def patch(request: AnonAltFieldsKey): Request_[SendAs]
  /**
    * Updates a send-as alias. If a signature is provided, Gmail will sanitize the HTML before saving it with the alias.
    *
    * Addresses other than the primary address for the account can only be updated by service account clients that have been delegated domain-wide authority.
    */
  def update(request: AnonAltFieldsKey): Request_[SendAs]
  /**
    * Sends a verification email to the specified send-as alias address. The verification status must be pending.
    *
    * This method is only available to service account clients that have been delegated domain-wide authority.
    */
  def verify(request: AnonAltFieldsKey): Request_[Unit]
}
...
object SendAsResource {
  ...
  def apply(
    ...
    get: AnonAltFieldsKey => Request_[SendAs],
    list: AnonAlt => Request_[ListSendAsResponse],
    patch: AnonAltFieldsKey => Request_[SendAs],
    smimeInfo: SmimeInfoResource,
    update: AnonAltFieldsKey => Request_[SendAs],
    verify: AnonAltFieldsKey => Request_[Unit]
  ): SendAsResource = {
