...
trait CategoryErrorCategory extends StObject {
  
  var category: ErrorCategory
  
  var level: typings.gatsbyCli.structuredErrorsTypesMod.Level
  
  def text(hasInputManifestPagePath: PagePath): String
}
object CategoryErrorCategory {
  
  inline def apply(
    category: ErrorCategory,
    level: typings.gatsbyCli.structuredErrorsTypesMod.Level,
    text: PagePath => String
    ...
    __obj.asInstanceOf[CategoryErrorCategory]
  }
  
  extension [Self <: CategoryErrorCategory](x: Self) {
    ...
    inline def setText(value: PagePath => String): Self = StObject.set(x, "text", js.Any.fromFunction1(value))
  }
}
