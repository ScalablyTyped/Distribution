...
trait TasksResource extends js.Object {
  ...
  def get(request: Fields): Request[Task]
  /** Insert a new task in a TaskQueue */
  def insert(request: Key): Request[Task]
  /** Lease 1 or more tasks from a TaskQueue. */
  def lease(request: GroupByTag): Request[Tasks]
  /** List Tasks in a TaskQueue */
  def list(request: Key): Request[Tasks2]
  /** Update tasks that are leased out of a TaskQueue. This method supports patch semantics. */
  def patch(request: NewLeaseSeconds): Request[Task]
  /** Update tasks that are leased out of a TaskQueue. */
  def update(request: NewLeaseSeconds): Request[Task]
}
