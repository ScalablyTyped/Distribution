...
open class ResourceTeamdrives protected () extends StObject {
  ...
  def insert(params: Unit, options: MethodOptions): GaxiosPromise[SchemaTeamDrive] = js.native
  ...
  /**
    ...
    * @returns A promise if used with async/await, or void if used with a callback.
    */
  def insert(params: ParamsResourceTeamdrivesInsert, options: StreamMethodOptions): GaxiosPromise[Readable] = js.native
  def insert(
    params: ParamsResourceTeamdrivesInsert,
    options: StreamMethodOptions,
    callback: BodyResponseCallback[Readable]
