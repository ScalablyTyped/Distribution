...
import typings.gestalt.gestaltStrings.queued
...
trait StatusProps extends StObject {
  ...
  var `type`: unstarted | queued | inProgress | halted | ok | problem | canceled | warning
}
object StatusProps {
  
  inline def apply(`type`: unstarted | queued | inProgress | halted | ok | problem | canceled | warning): StatusProps = {
  ...
  implicit open class MutableBuilder[Self <: StatusProps] (val x: Self) extends AnyVal {
    ...
    inline def setType(value: unstarted | queued | inProgress | halted | ok | problem | canceled | warning): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
