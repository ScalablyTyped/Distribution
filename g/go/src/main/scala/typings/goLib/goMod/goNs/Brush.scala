...
object Brush {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addColorStop = js.Any.fromFunction2(addColorStop), color = color, colorStops = colorStops, copy = js.Any.fromFunction0(copy), end = end, endRadius = endRadius, pattern = pattern, start = start, startRadius = startRadius)
