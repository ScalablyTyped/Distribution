...
class Set[T] () extends Iterable[T] {
  ...
  /**
    ...
    * @param {*} val
    */
  def add(`val`: T): scala.Boolean = js.native
  /**
  ...
  /**
    ...
    */
  def containsAll(coll: Iterable[T]): scala.Boolean = js.native
  /**
    * Returns true if any of the values of a given collection are in this Set.
    * @param {Iterable<T>} coll
    */
  def containsAny(coll: Iterable[T]): scala.Boolean = js.native
  /**
  ...
  /**
    ...
    */
  def filter(pred: js.Function1[/* x */ T, scala.Boolean]): Set[T] = js.native
  /**
