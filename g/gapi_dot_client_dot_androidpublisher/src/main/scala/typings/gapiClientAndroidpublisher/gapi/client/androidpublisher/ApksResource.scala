...
trait ApksResource extends js.Object {
  /**
    ...
    */
  def addexternallyhosted(request: EditId): Request[ApksAddExternallyHostedResponse]
  def list(request: EditId): Request[ApksListResponse]
  def upload(request: EditId): Request[Apk]
}
