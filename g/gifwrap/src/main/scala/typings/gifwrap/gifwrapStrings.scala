package typings.gifwrap

import typings.gifwrap.mod.DitherAlgorithm
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object gifwrapStrings {
  
  @js.native
  sealed trait Atkinson
    extends StObject
       with DitherAlgorithm
  inline def Atkinson: Atkinson = "Atkinson".asInstanceOf[Atkinson]
  
  @js.native
  sealed trait Burkes
    extends StObject
       with DitherAlgorithm
  inline def Burkes: Burkes = "Burkes".asInstanceOf[Burkes]
  
  @js.native
  sealed trait FalseFloydSteinberg
    extends StObject
       with DitherAlgorithm
  inline def FalseFloydSteinberg: FalseFloydSteinberg = "FalseFloydSteinberg".asInstanceOf[FalseFloydSteinberg]
  
  @js.native
  sealed trait FloydSteinberg
    extends StObject
       with DitherAlgorithm
  inline def FloydSteinberg: FloydSteinberg = "FloydSteinberg".asInstanceOf[FloydSteinberg]
  
  @js.native
  sealed trait Jarvis
    extends StObject
       with DitherAlgorithm
  inline def Jarvis: Jarvis = "Jarvis".asInstanceOf[Jarvis]
  
  @js.native
  sealed trait Sierra
    extends StObject
       with DitherAlgorithm
  inline def Sierra: Sierra = "Sierra".asInstanceOf[Sierra]
  
  @js.native
  sealed trait SierraLite
    extends StObject
       with DitherAlgorithm
  inline def SierraLite: SierraLite = "SierraLite".asInstanceOf[SierraLite]
  
  @js.native
  sealed trait Stucki
    extends StObject
       with DitherAlgorithm
  inline def Stucki: Stucki = "Stucki".asInstanceOf[Stucki]
  
  @js.native
  sealed trait TwoSierra
    extends StObject
       with DitherAlgorithm
  inline def TwoSierra: TwoSierra = "TwoSierra".asInstanceOf[TwoSierra]
}
