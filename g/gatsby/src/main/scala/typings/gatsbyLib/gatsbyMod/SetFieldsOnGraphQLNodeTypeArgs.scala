...
trait SetFieldsOnGraphQLNodeTypeArgs extends ParentSpanPluginArgs {
  var traceId: gatsbyLib.gatsbyLibStrings.`initial-setFieldsOnGraphQLNodeType`
  var `type`: gatsbyLib.Anon_Name
}
...
object SetFieldsOnGraphQLNodeTypeArgs {
  ...
  def apply(
    ...
    reporter: Reporter,
    store: Store,
    traceId: gatsbyLib.gatsbyLibStrings.`initial-setFieldsOnGraphQLNodeType`,
    tracing: Tracing,
    `type`: gatsbyLib.Anon_Name
