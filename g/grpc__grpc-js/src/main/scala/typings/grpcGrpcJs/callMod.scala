...
object callMod extends js.Object {
  ...
  - typings.grpcGrpcJs.objectStreamMod.ObjectWritable because Inheritance from two classes. Inlined _write, write, write, write, write, setDefaultEncoding, end, end, end, end, end */ @js.native
  class ClientDuplexStreamImpl[RequestType, ResponseType] protected ()
    extends Duplex
       with AnonSerialize[RequestType]
       with AnonCancel
       with EmitterAugmentation1[metadata | status | data, Metadata | ResponseType | StatusObject]
       with AnonChunk[ResponseType]
       with AnonRead[ResponseType] {
    def this(
      call: Call,
  ...
  type ClientReadableStream[ResponseType] = AnonChunk[ResponseType] with SurfaceCall with ObjectReadable[ResponseType]
  type ClientUnaryCall = SurfaceCall
  type ClientWritableStream[RequestType] = AnonSerialize[RequestType] with SurfaceCall with ObjectWritable[RequestType]
  type ServiceError = StatusObject with Error
  type SurfaceCall = AnonCancel with (EmitterAugmentation1[metadata, Metadata]) with (EmitterAugmentation1[status, StatusObject]) with EventEmitter
}
