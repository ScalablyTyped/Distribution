...
import typings.grpcGrpcJs.AnonCallback
import typings.grpcGrpcJs.AnonCancelWithStatus
...
- typings.grpcGrpcJs.objectStreamMod.IntermediateObjectReadable because var conflicts: destroyed, readable. Inlined 
- typings.grpcGrpcJs.objectStreamMod.ObjectWritable because Inheritance from two classes. Inlined _write, write, write, write, write, setDefaultEncoding, end, end, end, end, end */ @JSImport("@grpc/grpc-js/build/src/call-stream", "Http2CallStream")
@js.native
class Http2CallStream protected ()
  extends Duplex
     with AnonCancelWithStatus
     with EmitterAugmentation1[metadata | status | data, Buffer | Metadata | StatusObject]
     with AnonCallback[Buffer, WriteObject]
     with AnonRead[Buffer] {
  def this(
    methodName: String,
