...
trait Anon_Animation extends js.Object {
  ...
  var deviceBreakpoints: Anon_Computer
...
object Anon_Animation {
  ...
  def apply(
    ...
    deviceBreakpoints: Anon_Computer,
    ...
    val __obj = js.Dynamic.literal(animation = animation, borderSize = borderSize, breakpoints = breakpoints, colors = colors, control = control, debounceDelay = debounceDelay, deviceBreakpoints = deviceBreakpoints, drop = drop, edgeSize = edgeSize, elevation = elevation, focus = focus, font = font, hover = hover, input = input, opacity = opacity, selected = selected, size = size, spacing = spacing)
