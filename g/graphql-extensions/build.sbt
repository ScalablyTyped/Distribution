organization := "org.scalablytyped"
name := "graphql-extensions"
version := "0.5.4-fedbb8"
scalaVersion := "2.12.8"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.olvind" %%% "scalablytyped-runtime" % "1.0.0",
  "org.scala-js" %%% "scalajs-dom" % "0.9.6",
  "org.scalablytyped" %%% "apollo-datasource" % "0.3.1-f5931c",
  "org.scalablytyped" %%% "apollo-engine-reporting" % "1.0.7-7cff95",
  "org.scalablytyped" %%% "apollo-engine-reporting-protobuf" % "0.2.1-a09309",
  "org.scalablytyped" %%% "apollo-env" % "0.4.0-f29605",
  "org.scalablytyped" %%% "apollo-graphql" % "0.1.3-980e65",
  "org.scalablytyped" %%% "apollo-link" % "1.2.11-e13a57",
  "org.scalablytyped" %%% "apollo-server-caching" % "0.3.1-f091ae",
  "org.scalablytyped" %%% "apollo-server-core" % "2.4.8-bd58d3",
  "org.scalablytyped" %%% "apollo-server-env" % "2.2.0-92fce4",
  "org.scalablytyped" %%% "apollo-server-errors" % "2.2.1-efd118",
  "org.scalablytyped" %%% "apollo-server-plugin-base" % "0.3.7-e4e4d1",
  "org.scalablytyped" %%% "apollo-utilities" % "1.2.1-da0a30",
  "org.scalablytyped" %%% "apollographql__apollo-tools" % "0.3.5-95ba9c",
  "org.scalablytyped" %%% "apollographql__graphql-playground-html" % "1.6.6-395c0b",
  "org.scalablytyped" %%% "async-retry" % "1.2-dt-20180601Z-d843f8",
  "org.scalablytyped" %%% "backo2" % "1.0-dt-20180601Z-a8dbaf",
  "org.scalablytyped" %%% "core-js" % "2.5-dt-20180921Z-298712",
  "org.scalablytyped" %%% "deprecated-decorator" % "0.1.6-492e8d",
  "org.scalablytyped" %%% "eventemitter3" % "3.1.0-3d3736",
  "org.scalablytyped" %%% "fast-json-stable-stringify" % "2.0-dt-20181224Z-ccd0c9",
  "org.scalablytyped" %%% "graphql" % "14.0-dt-20190212Z-aa9287",
  "org.scalablytyped" %%% "graphql-subscriptions" % "1.0.0-5abef0",
  "org.scalablytyped" %%% "graphql-tag" % "2.10.1-8ad040",
  "org.scalablytyped" %%% "graphql-tools" % "4.0.4-949d7e",
  "org.scalablytyped" %%% "iterall" % "1.2.2-e4101d",
  "org.scalablytyped" %%% "lodash" % "4.14-dt-20190311Z-601106",
  "org.scalablytyped" %%% "lodash_dot_sortby" % "4.7-dt-20190214Z-615acc",
  "org.scalablytyped" %%% "long" % "4.0.0-dt-20180425Z-b30bce",
  "org.scalablytyped" %%% "lru-cache" % "5.1-dt-20190222Z-7f28fe",
  "org.scalablytyped" %%% "node" % "11.11-dt-20190318Z-f6d9fe",
  "org.scalablytyped" %%% "node-fetch" % "2.1-dt-20190205Z-512b32",
  "org.scalablytyped" %%% "protobufjs" % "6.8.8-f0c22e",
  "org.scalablytyped" %%% "protobufjs__aspromise" % "1.1.2-6d28c3",
  "org.scalablytyped" %%% "protobufjs__base64" % "1.1.2-46201f",
  "org.scalablytyped" %%% "protobufjs__codegen" % "2.0.4-63857b",
  "org.scalablytyped" %%% "protobufjs__eventemitter" % "1.1.0-fe2693",
  "org.scalablytyped" %%% "protobufjs__fetch" % "1.1.0-c2c6bd",
  "org.scalablytyped" %%% "protobufjs__float" % "1.0.2-3b892b",
  "org.scalablytyped" %%% "protobufjs__inquire" % "1.1.0-97bb93",
  "org.scalablytyped" %%% "protobufjs__path" % "1.1.2-8051ed",
  "org.scalablytyped" %%% "protobufjs__pool" % "1.1.0-7152e5",
  "org.scalablytyped" %%% "protobufjs__utf8" % "1.1.0-1df087",
  "org.scalablytyped" %%% "sha_dot_js" % "2.4-dt-20181126Z-591e2d",
  "org.scalablytyped" %%% "std" % "3.3-ec2942",
  "org.scalablytyped" %%% "subscriptions-transport-ws" % "0.9.16-fcaf37",
  "org.scalablytyped" %%% "symbol-observable" % "1.2.0-5b665c",
  "org.scalablytyped" %%% "ts-invariant" % "0.3.2-6c21c8",
  "org.scalablytyped" %%% "tslib" % "1.9.3-28a6ab",
  "org.scalablytyped" %%% "util_dot_promisify" % "1.0-dt-20180214Z-9ccedf",
  "org.scalablytyped" %%% "uuid" % "3.4-dt-20180828Z-ff94b7",
  "org.scalablytyped" %%% "ws" % "6.0-dt-20180830Z-bc1d9e",
  "org.scalablytyped" %%% "zen-observable" % "0.8-dt-20180618Z-4a9f67",
  "org.scalablytyped" %%% "zen-observable-ts" % "0.8.18-3d5d0d")
publishArtifact in packageDoc := false
scalacOptions += "-P:scalajs:sjsDefinedByDefault"
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalablyTyped"
        