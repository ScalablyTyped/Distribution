...
object FileCapabilities {
  ...
  def apply(
    ...
    if (!js.isUndefined(canChangeCopyRequiresWriterPermission)) __obj.updateDynamic("canChangeCopyRequiresWriterPermission")(canChangeCopyRequiresWriterPermission)
    if (!js.isUndefined(canChangeRestrictedDownload)) __obj.updateDynamic("canChangeRestrictedDownload")(canChangeRestrictedDownload)
