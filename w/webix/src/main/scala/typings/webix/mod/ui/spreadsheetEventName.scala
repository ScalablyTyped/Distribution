package typings.webix.mod.ui

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.webix.webixStrings.onAfterConditionSet
  - typings.webix.webixStrings.onAfterLoad
  - typings.webix.webixStrings.onAfterRangeSet
  - typings.webix.webixStrings.onAfterSelect
  - typings.webix.webixStrings.onAfterSheetShow
  - typings.webix.webixStrings.onAfterSpan
  - typings.webix.webixStrings.onAfterSplit
  - typings.webix.webixStrings.onBeforeCommentShow
  - typings.webix.webixStrings.onBeforeLoad
  - typings.webix.webixStrings.onBeforeSheetShow
  - typings.webix.webixStrings.onBeforeSpan
  - typings.webix.webixStrings.onBeforeSplit
  - typings.webix.webixStrings.onBeforeValueChange
  - typings.webix.webixStrings.onBindRequest
  - typings.webix.webixStrings.onCellChange
  - typings.webix.webixStrings.onChange
  - typings.webix.webixStrings.onCommentHide
  - typings.webix.webixStrings.onComponentInit
  - typings.webix.webixStrings.onDataParse
  - typings.webix.webixStrings.onDataSerialize
  - typings.webix.webixStrings.onDestruct
  - typings.webix.webixStrings.onLoadError
  - typings.webix.webixStrings.onMathRefresh
  - typings.webix.webixStrings.onReset
  - typings.webix.webixStrings.onSheetAdd
  - typings.webix.webixStrings.onSheetRemove
  - typings.webix.webixStrings.onSheetRename
  - typings.webix.webixStrings.onStyleSet
  - typings.webix.webixStrings.onUIEditStart
  - typings.webix.webixStrings.onUIEditStop
  - typings.webix.webixStrings.onViewInit
  - typings.webix.webixStrings.onViewShow
*/
trait spreadsheetEventName extends StObject
object spreadsheetEventName {
  
  inline def onAfterConditionSet: typings.webix.webixStrings.onAfterConditionSet = "onAfterConditionSet".asInstanceOf[typings.webix.webixStrings.onAfterConditionSet]
  
  inline def onAfterLoad: typings.webix.webixStrings.onAfterLoad = "onAfterLoad".asInstanceOf[typings.webix.webixStrings.onAfterLoad]
  
  inline def onAfterRangeSet: typings.webix.webixStrings.onAfterRangeSet = "onAfterRangeSet".asInstanceOf[typings.webix.webixStrings.onAfterRangeSet]
  
  inline def onAfterSelect: typings.webix.webixStrings.onAfterSelect = "onAfterSelect".asInstanceOf[typings.webix.webixStrings.onAfterSelect]
  
  inline def onAfterSheetShow: typings.webix.webixStrings.onAfterSheetShow = "onAfterSheetShow".asInstanceOf[typings.webix.webixStrings.onAfterSheetShow]
  
  inline def onAfterSpan: typings.webix.webixStrings.onAfterSpan = "onAfterSpan".asInstanceOf[typings.webix.webixStrings.onAfterSpan]
  
  inline def onAfterSplit: typings.webix.webixStrings.onAfterSplit = "onAfterSplit".asInstanceOf[typings.webix.webixStrings.onAfterSplit]
  
  inline def onBeforeCommentShow: typings.webix.webixStrings.onBeforeCommentShow = "onBeforeCommentShow".asInstanceOf[typings.webix.webixStrings.onBeforeCommentShow]
  
  inline def onBeforeLoad: typings.webix.webixStrings.onBeforeLoad = "onBeforeLoad".asInstanceOf[typings.webix.webixStrings.onBeforeLoad]
  
  inline def onBeforeSheetShow: typings.webix.webixStrings.onBeforeSheetShow = "onBeforeSheetShow".asInstanceOf[typings.webix.webixStrings.onBeforeSheetShow]
  
  inline def onBeforeSpan: typings.webix.webixStrings.onBeforeSpan = "onBeforeSpan".asInstanceOf[typings.webix.webixStrings.onBeforeSpan]
  
  inline def onBeforeSplit: typings.webix.webixStrings.onBeforeSplit = "onBeforeSplit".asInstanceOf[typings.webix.webixStrings.onBeforeSplit]
  
  inline def onBeforeValueChange: typings.webix.webixStrings.onBeforeValueChange = "onBeforeValueChange".asInstanceOf[typings.webix.webixStrings.onBeforeValueChange]
  
  inline def onBindRequest: typings.webix.webixStrings.onBindRequest = "onBindRequest".asInstanceOf[typings.webix.webixStrings.onBindRequest]
  
  inline def onCellChange: typings.webix.webixStrings.onCellChange = "onCellChange".asInstanceOf[typings.webix.webixStrings.onCellChange]
  
  inline def onChange: typings.webix.webixStrings.onChange = "onChange".asInstanceOf[typings.webix.webixStrings.onChange]
  
  inline def onCommentHide: typings.webix.webixStrings.onCommentHide = "onCommentHide".asInstanceOf[typings.webix.webixStrings.onCommentHide]
  
  inline def onComponentInit: typings.webix.webixStrings.onComponentInit = "onComponentInit".asInstanceOf[typings.webix.webixStrings.onComponentInit]
  
  inline def onDataParse: typings.webix.webixStrings.onDataParse = "onDataParse".asInstanceOf[typings.webix.webixStrings.onDataParse]
  
  inline def onDataSerialize: typings.webix.webixStrings.onDataSerialize = "onDataSerialize".asInstanceOf[typings.webix.webixStrings.onDataSerialize]
  
  inline def onDestruct: typings.webix.webixStrings.onDestruct = "onDestruct".asInstanceOf[typings.webix.webixStrings.onDestruct]
  
  inline def onLoadError: typings.webix.webixStrings.onLoadError = "onLoadError".asInstanceOf[typings.webix.webixStrings.onLoadError]
  
  inline def onMathRefresh: typings.webix.webixStrings.onMathRefresh = "onMathRefresh".asInstanceOf[typings.webix.webixStrings.onMathRefresh]
  
  inline def onReset: typings.webix.webixStrings.onReset = "onReset".asInstanceOf[typings.webix.webixStrings.onReset]
  
  inline def onSheetAdd: typings.webix.webixStrings.onSheetAdd = "onSheetAdd".asInstanceOf[typings.webix.webixStrings.onSheetAdd]
  
  inline def onSheetRemove: typings.webix.webixStrings.onSheetRemove = "onSheetRemove".asInstanceOf[typings.webix.webixStrings.onSheetRemove]
  
  inline def onSheetRename: typings.webix.webixStrings.onSheetRename = "onSheetRename".asInstanceOf[typings.webix.webixStrings.onSheetRename]
  
  inline def onStyleSet: typings.webix.webixStrings.onStyleSet = "onStyleSet".asInstanceOf[typings.webix.webixStrings.onStyleSet]
  
  inline def onUIEditStart: typings.webix.webixStrings.onUIEditStart = "onUIEditStart".asInstanceOf[typings.webix.webixStrings.onUIEditStart]
  
  inline def onUIEditStop: typings.webix.webixStrings.onUIEditStop = "onUIEditStop".asInstanceOf[typings.webix.webixStrings.onUIEditStop]
  
  inline def onViewInit: typings.webix.webixStrings.onViewInit = "onViewInit".asInstanceOf[typings.webix.webixStrings.onViewInit]
  
  inline def onViewShow: typings.webix.webixStrings.onViewShow = "onViewShow".asInstanceOf[typings.webix.webixStrings.onViewShow]
}
