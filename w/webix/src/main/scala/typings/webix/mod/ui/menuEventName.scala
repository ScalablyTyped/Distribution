package typings.webix.mod.ui

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.webix.webixStrings.onAfterAdd
  - typings.webix.webixStrings.onAfterContextMenu
  - typings.webix.webixStrings.onAfterDelete
  - typings.webix.webixStrings.onAfterDrop
  - typings.webix.webixStrings.onAfterLoad
  - typings.webix.webixStrings.onAfterRender
  - typings.webix.webixStrings.onAfterScroll
  - typings.webix.webixStrings.onAfterSelect
  - typings.webix.webixStrings.onAfterSort
  - typings.webix.webixStrings.onBeforeAdd
  - typings.webix.webixStrings.onBeforeContextMenu
  - typings.webix.webixStrings.onBeforeDelete
  - typings.webix.webixStrings.onBeforeDrag
  - typings.webix.webixStrings.onBeforeDragIn
  - typings.webix.webixStrings.onBeforeDrop
  - typings.webix.webixStrings.onBeforeDropOut
  - typings.webix.webixStrings.onBeforeLoad
  - typings.webix.webixStrings.onBeforeRender
  - typings.webix.webixStrings.onBeforeSelect
  - typings.webix.webixStrings.onBeforeSort
  - typings.webix.webixStrings.onBindRequest
  - typings.webix.webixStrings.onBlur
  - typings.webix.webixStrings.onDataRequest
  - typings.webix.webixStrings.onDataUpdate
  - typings.webix.webixStrings.onDestruct
  - typings.webix.webixStrings.onDragOut
  - typings.webix.webixStrings.onEnter
  - typings.webix.webixStrings.onFocus
  - typings.webix.webixStrings.onItemClick
  - typings.webix.webixStrings.onItemDblClick
  - typings.webix.webixStrings.onItemRender
  - typings.webix.webixStrings.onKeyPress
  - typings.webix.webixStrings.onLoadError
  - typings.webix.webixStrings.onLongTouch
  - typings.webix.webixStrings.onMenuItemClick
  - typings.webix.webixStrings.onMouseMove
  - typings.webix.webixStrings.onMouseMoving
  - typings.webix.webixStrings.onMouseOut
  - typings.webix.webixStrings.onPaste
  - typings.webix.webixStrings.onSelectChange
  - typings.webix.webixStrings.onSwipeX
  - typings.webix.webixStrings.onSwipeY
  - typings.webix.webixStrings.onTabFocus
  - typings.webix.webixStrings.onTimedKeyPress
  - typings.webix.webixStrings.onTouchEnd
  - typings.webix.webixStrings.onTouchMove
  - typings.webix.webixStrings.onTouchStart
  - typings.webix.webixStrings.onValidationError
  - typings.webix.webixStrings.onValidationSuccess
  - typings.webix.webixStrings.onViewResize
  - typings.webix.webixStrings.onViewShow
*/
trait menuEventName extends StObject
object menuEventName {
  
  inline def onAfterAdd: typings.webix.webixStrings.onAfterAdd = "onAfterAdd".asInstanceOf[typings.webix.webixStrings.onAfterAdd]
  
  inline def onAfterContextMenu: typings.webix.webixStrings.onAfterContextMenu = "onAfterContextMenu".asInstanceOf[typings.webix.webixStrings.onAfterContextMenu]
  
  inline def onAfterDelete: typings.webix.webixStrings.onAfterDelete = "onAfterDelete".asInstanceOf[typings.webix.webixStrings.onAfterDelete]
  
  inline def onAfterDrop: typings.webix.webixStrings.onAfterDrop = "onAfterDrop".asInstanceOf[typings.webix.webixStrings.onAfterDrop]
  
  inline def onAfterLoad: typings.webix.webixStrings.onAfterLoad = "onAfterLoad".asInstanceOf[typings.webix.webixStrings.onAfterLoad]
  
  inline def onAfterRender: typings.webix.webixStrings.onAfterRender = "onAfterRender".asInstanceOf[typings.webix.webixStrings.onAfterRender]
  
  inline def onAfterScroll: typings.webix.webixStrings.onAfterScroll = "onAfterScroll".asInstanceOf[typings.webix.webixStrings.onAfterScroll]
  
  inline def onAfterSelect: typings.webix.webixStrings.onAfterSelect = "onAfterSelect".asInstanceOf[typings.webix.webixStrings.onAfterSelect]
  
  inline def onAfterSort: typings.webix.webixStrings.onAfterSort = "onAfterSort".asInstanceOf[typings.webix.webixStrings.onAfterSort]
  
  inline def onBeforeAdd: typings.webix.webixStrings.onBeforeAdd = "onBeforeAdd".asInstanceOf[typings.webix.webixStrings.onBeforeAdd]
  
  inline def onBeforeContextMenu: typings.webix.webixStrings.onBeforeContextMenu = "onBeforeContextMenu".asInstanceOf[typings.webix.webixStrings.onBeforeContextMenu]
  
  inline def onBeforeDelete: typings.webix.webixStrings.onBeforeDelete = "onBeforeDelete".asInstanceOf[typings.webix.webixStrings.onBeforeDelete]
  
  inline def onBeforeDrag: typings.webix.webixStrings.onBeforeDrag = "onBeforeDrag".asInstanceOf[typings.webix.webixStrings.onBeforeDrag]
  
  inline def onBeforeDragIn: typings.webix.webixStrings.onBeforeDragIn = "onBeforeDragIn".asInstanceOf[typings.webix.webixStrings.onBeforeDragIn]
  
  inline def onBeforeDrop: typings.webix.webixStrings.onBeforeDrop = "onBeforeDrop".asInstanceOf[typings.webix.webixStrings.onBeforeDrop]
  
  inline def onBeforeDropOut: typings.webix.webixStrings.onBeforeDropOut = "onBeforeDropOut".asInstanceOf[typings.webix.webixStrings.onBeforeDropOut]
  
  inline def onBeforeLoad: typings.webix.webixStrings.onBeforeLoad = "onBeforeLoad".asInstanceOf[typings.webix.webixStrings.onBeforeLoad]
  
  inline def onBeforeRender: typings.webix.webixStrings.onBeforeRender = "onBeforeRender".asInstanceOf[typings.webix.webixStrings.onBeforeRender]
  
  inline def onBeforeSelect: typings.webix.webixStrings.onBeforeSelect = "onBeforeSelect".asInstanceOf[typings.webix.webixStrings.onBeforeSelect]
  
  inline def onBeforeSort: typings.webix.webixStrings.onBeforeSort = "onBeforeSort".asInstanceOf[typings.webix.webixStrings.onBeforeSort]
  
  inline def onBindRequest: typings.webix.webixStrings.onBindRequest = "onBindRequest".asInstanceOf[typings.webix.webixStrings.onBindRequest]
  
  inline def onBlur: typings.webix.webixStrings.onBlur = "onBlur".asInstanceOf[typings.webix.webixStrings.onBlur]
  
  inline def onDataRequest: typings.webix.webixStrings.onDataRequest = "onDataRequest".asInstanceOf[typings.webix.webixStrings.onDataRequest]
  
  inline def onDataUpdate: typings.webix.webixStrings.onDataUpdate = "onDataUpdate".asInstanceOf[typings.webix.webixStrings.onDataUpdate]
  
  inline def onDestruct: typings.webix.webixStrings.onDestruct = "onDestruct".asInstanceOf[typings.webix.webixStrings.onDestruct]
  
  inline def onDragOut: typings.webix.webixStrings.onDragOut = "onDragOut".asInstanceOf[typings.webix.webixStrings.onDragOut]
  
  inline def onEnter: typings.webix.webixStrings.onEnter = "onEnter".asInstanceOf[typings.webix.webixStrings.onEnter]
  
  inline def onFocus: typings.webix.webixStrings.onFocus = "onFocus".asInstanceOf[typings.webix.webixStrings.onFocus]
  
  inline def onItemClick: typings.webix.webixStrings.onItemClick = "onItemClick".asInstanceOf[typings.webix.webixStrings.onItemClick]
  
  inline def onItemDblClick: typings.webix.webixStrings.onItemDblClick = "onItemDblClick".asInstanceOf[typings.webix.webixStrings.onItemDblClick]
  
  inline def onItemRender: typings.webix.webixStrings.onItemRender = "onItemRender".asInstanceOf[typings.webix.webixStrings.onItemRender]
  
  inline def onKeyPress: typings.webix.webixStrings.onKeyPress = "onKeyPress".asInstanceOf[typings.webix.webixStrings.onKeyPress]
  
  inline def onLoadError: typings.webix.webixStrings.onLoadError = "onLoadError".asInstanceOf[typings.webix.webixStrings.onLoadError]
  
  inline def onLongTouch: typings.webix.webixStrings.onLongTouch = "onLongTouch".asInstanceOf[typings.webix.webixStrings.onLongTouch]
  
  inline def onMenuItemClick: typings.webix.webixStrings.onMenuItemClick = "onMenuItemClick".asInstanceOf[typings.webix.webixStrings.onMenuItemClick]
  
  inline def onMouseMove: typings.webix.webixStrings.onMouseMove = "onMouseMove".asInstanceOf[typings.webix.webixStrings.onMouseMove]
  
  inline def onMouseMoving: typings.webix.webixStrings.onMouseMoving = "onMouseMoving".asInstanceOf[typings.webix.webixStrings.onMouseMoving]
  
  inline def onMouseOut: typings.webix.webixStrings.onMouseOut = "onMouseOut".asInstanceOf[typings.webix.webixStrings.onMouseOut]
  
  inline def onPaste: typings.webix.webixStrings.onPaste = "onPaste".asInstanceOf[typings.webix.webixStrings.onPaste]
  
  inline def onSelectChange: typings.webix.webixStrings.onSelectChange = "onSelectChange".asInstanceOf[typings.webix.webixStrings.onSelectChange]
  
  inline def onSwipeX: typings.webix.webixStrings.onSwipeX = "onSwipeX".asInstanceOf[typings.webix.webixStrings.onSwipeX]
  
  inline def onSwipeY: typings.webix.webixStrings.onSwipeY = "onSwipeY".asInstanceOf[typings.webix.webixStrings.onSwipeY]
  
  inline def onTabFocus: typings.webix.webixStrings.onTabFocus = "onTabFocus".asInstanceOf[typings.webix.webixStrings.onTabFocus]
  
  inline def onTimedKeyPress: typings.webix.webixStrings.onTimedKeyPress = "onTimedKeyPress".asInstanceOf[typings.webix.webixStrings.onTimedKeyPress]
  
  inline def onTouchEnd: typings.webix.webixStrings.onTouchEnd = "onTouchEnd".asInstanceOf[typings.webix.webixStrings.onTouchEnd]
  
  inline def onTouchMove: typings.webix.webixStrings.onTouchMove = "onTouchMove".asInstanceOf[typings.webix.webixStrings.onTouchMove]
  
  inline def onTouchStart: typings.webix.webixStrings.onTouchStart = "onTouchStart".asInstanceOf[typings.webix.webixStrings.onTouchStart]
  
  inline def onValidationError: typings.webix.webixStrings.onValidationError = "onValidationError".asInstanceOf[typings.webix.webixStrings.onValidationError]
  
  inline def onValidationSuccess: typings.webix.webixStrings.onValidationSuccess = "onValidationSuccess".asInstanceOf[typings.webix.webixStrings.onValidationSuccess]
  
  inline def onViewResize: typings.webix.webixStrings.onViewResize = "onViewResize".asInstanceOf[typings.webix.webixStrings.onViewResize]
  
  inline def onViewShow: typings.webix.webixStrings.onViewShow = "onViewShow".asInstanceOf[typings.webix.webixStrings.onViewShow]
}
