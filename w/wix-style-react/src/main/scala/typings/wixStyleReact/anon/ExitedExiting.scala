...
trait ExitedExiting extends StObject {
...
object ExitedExiting {
  
  inline def apply(entered: HeightLeft, entering: LeftWidth, exited: HeightLeft, exiting: BottomHeight): ExitedExiting = {
    ...
    __obj.asInstanceOf[ExitedExiting]
  }
  
  extension [Self <: ExitedExiting](x: Self) {
