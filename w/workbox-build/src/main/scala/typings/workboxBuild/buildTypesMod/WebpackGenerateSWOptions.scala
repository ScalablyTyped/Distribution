...
- typings.workboxBuild.buildTypesMod.GeneratePartial because var conflicts: mode. Inlined babelPresetEnvTargets, cacheId, cleanupOutdatedCaches, clientsClaim, directoryIndex, disableDevLogs, ignoreURLParametersMatching, importScripts, inlineWorkboxRuntime, navigateFallback, navigateFallbackAllowlist, navigateFallbackDenylist, navigationPreload, offlineGoogleAnalytics, runtimeCaching, skipWaiting, sourcemap */ trait WebpackGenerateSWOptions
  extends StObject
     with BasePartial
     with WebpackPartial
     with WebpackGenerateSWPartial {
...
object WebpackGenerateSWOptions {
  
  inline def apply(): WebpackGenerateSWOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[WebpackGenerateSWOptions]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: WebpackGenerateSWOptions] (val x: Self) extends AnyVal {
