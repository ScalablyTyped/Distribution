...
object MidiStartMessage {
  ...
  inline def apply(rawData: IBuffer, timestamp: Double, `type`: MidiMessageType): MidiStartMessage = {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: MidiStartMessage] (val x: Self) extends AnyVal {
