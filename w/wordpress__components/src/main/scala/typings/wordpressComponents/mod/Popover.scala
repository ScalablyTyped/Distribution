...
object Popover extends Shortcut {
  ...
  @js.native
  val ^ : ComponentType[Props] with typings.wordpressComponents.anon.Slot = js.native
  
  type _To = ComponentType[Props] with typings.wordpressComponents.anon.Slot
  
  /* This means you don't have to write `^`, but can instead just say `Popover.foo` */
  override def _to: ComponentType[Props] with typings.wordpressComponents.anon.Slot = ^
}
