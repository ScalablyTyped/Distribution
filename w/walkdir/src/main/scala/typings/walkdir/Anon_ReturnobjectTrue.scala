package typings.walkdir

import typings.walkdir.walkdirBooleans.`true`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_ReturnobjectTrue extends js.Object {
  var return_object: `true`
}

object Anon_ReturnobjectTrue {
  @scala.inline
  def apply(return_object: `true`): Anon_ReturnobjectTrue = {
    val __obj = js.Dynamic.literal(return_object = return_object.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon_ReturnobjectTrue]
  }
}

