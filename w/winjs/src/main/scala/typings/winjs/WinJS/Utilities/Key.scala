package typings.winjs.WinJS.Utilities

import org.scalablytyped.runtime.TopLevel
import typings.winjs.WinJS.Utilities.Key.F1
import typings.winjs.WinJS.Utilities.Key.F10
import typings.winjs.WinJS.Utilities.Key.F11
import typings.winjs.WinJS.Utilities.Key.F12
import typings.winjs.WinJS.Utilities.Key.F2
import typings.winjs.WinJS.Utilities.Key.F3
import typings.winjs.WinJS.Utilities.Key.F4
import typings.winjs.WinJS.Utilities.Key.F5
import typings.winjs.WinJS.Utilities.Key.F6
import typings.winjs.WinJS.Utilities.Key.F7
import typings.winjs.WinJS.Utilities.Key.F8
import typings.winjs.WinJS.Utilities.Key.F9
import typings.winjs.WinJS.Utilities.Key.GamepadA
import typings.winjs.WinJS.Utilities.Key.GamepadB
import typings.winjs.WinJS.Utilities.Key.GamepadDPadDown
import typings.winjs.WinJS.Utilities.Key.GamepadDPadLeft
import typings.winjs.WinJS.Utilities.Key.GamepadDPadRight
import typings.winjs.WinJS.Utilities.Key.GamepadDPadUp
import typings.winjs.WinJS.Utilities.Key.GamepadLeftShoulder
import typings.winjs.WinJS.Utilities.Key.GamepadLeftThumbstick
import typings.winjs.WinJS.Utilities.Key.GamepadLeftThumbstickDown
import typings.winjs.WinJS.Utilities.Key.GamepadLeftThumbstickLeft
import typings.winjs.WinJS.Utilities.Key.GamepadLeftThumbstickRight
import typings.winjs.WinJS.Utilities.Key.GamepadLeftThumbstickUp
import typings.winjs.WinJS.Utilities.Key.GamepadLeftTrigger
import typings.winjs.WinJS.Utilities.Key.GamepadMenu
import typings.winjs.WinJS.Utilities.Key.GamepadRightShoulder
import typings.winjs.WinJS.Utilities.Key.GamepadRightThumbstick
import typings.winjs.WinJS.Utilities.Key.GamepadRightThumbstickDown
import typings.winjs.WinJS.Utilities.Key.GamepadRightThumbstickLeft
import typings.winjs.WinJS.Utilities.Key.GamepadRightThumbstickRight
import typings.winjs.WinJS.Utilities.Key.GamepadRightThumbstickUp
import typings.winjs.WinJS.Utilities.Key.GamepadRightTrigger
import typings.winjs.WinJS.Utilities.Key.GamepadView
import typings.winjs.WinJS.Utilities.Key.GamepadX
import typings.winjs.WinJS.Utilities.Key.GamepadY
import typings.winjs.WinJS.Utilities.Key.IME
import typings.winjs.WinJS.Utilities.Key.NavigationAccept
import typings.winjs.WinJS.Utilities.Key.NavigationCancel
import typings.winjs.WinJS.Utilities.Key.NavigationDown
import typings.winjs.WinJS.Utilities.Key.NavigationLeft
import typings.winjs.WinJS.Utilities.Key.NavigationMenu
import typings.winjs.WinJS.Utilities.Key.NavigationRight
import typings.winjs.WinJS.Utilities.Key.NavigationUp
import typings.winjs.WinJS.Utilities.Key.NavigationView
import typings.winjs.WinJS.Utilities.Key.a
import typings.winjs.WinJS.Utilities.Key.add
import typings.winjs.WinJS.Utilities.Key.alt
import typings.winjs.WinJS.Utilities.Key.b
import typings.winjs.WinJS.Utilities.Key.backSlash
import typings.winjs.WinJS.Utilities.Key.backspace
import typings.winjs.WinJS.Utilities.Key.browserBack
import typings.winjs.WinJS.Utilities.Key.browserForward
import typings.winjs.WinJS.Utilities.Key.c
import typings.winjs.WinJS.Utilities.Key.capsLock
import typings.winjs.WinJS.Utilities.Key.closeBracket
import typings.winjs.WinJS.Utilities.Key.comma
import typings.winjs.WinJS.Utilities.Key.ctrl
import typings.winjs.WinJS.Utilities.Key.d
import typings.winjs.WinJS.Utilities.Key.dash
import typings.winjs.WinJS.Utilities.Key.decimalPoint
import typings.winjs.WinJS.Utilities.Key.deleteKey
import typings.winjs.WinJS.Utilities.Key.divide
import typings.winjs.WinJS.Utilities.Key.downArrow
import typings.winjs.WinJS.Utilities.Key.e
import typings.winjs.WinJS.Utilities.Key.end
import typings.winjs.WinJS.Utilities.Key.enter
import typings.winjs.WinJS.Utilities.Key.equal
import typings.winjs.WinJS.Utilities.Key.escape
import typings.winjs.WinJS.Utilities.Key.f
import typings.winjs.WinJS.Utilities.Key.forwardSlash
import typings.winjs.WinJS.Utilities.Key.g
import typings.winjs.WinJS.Utilities.Key.graveAccent
import typings.winjs.WinJS.Utilities.Key.h
import typings.winjs.WinJS.Utilities.Key.home
import typings.winjs.WinJS.Utilities.Key.i
import typings.winjs.WinJS.Utilities.Key.insert
import typings.winjs.WinJS.Utilities.Key.j
import typings.winjs.WinJS.Utilities.Key.k
import typings.winjs.WinJS.Utilities.Key.l
import typings.winjs.WinJS.Utilities.Key.leftArrow
import typings.winjs.WinJS.Utilities.Key.leftWindows
import typings.winjs.WinJS.Utilities.Key.m
import typings.winjs.WinJS.Utilities.Key.menu
import typings.winjs.WinJS.Utilities.Key.multiply
import typings.winjs.WinJS.Utilities.Key.n
import typings.winjs.WinJS.Utilities.Key.num0
import typings.winjs.WinJS.Utilities.Key.num1
import typings.winjs.WinJS.Utilities.Key.num2
import typings.winjs.WinJS.Utilities.Key.num3
import typings.winjs.WinJS.Utilities.Key.num4
import typings.winjs.WinJS.Utilities.Key.num5
import typings.winjs.WinJS.Utilities.Key.num6
import typings.winjs.WinJS.Utilities.Key.num7
import typings.winjs.WinJS.Utilities.Key.num8
import typings.winjs.WinJS.Utilities.Key.num9
import typings.winjs.WinJS.Utilities.Key.numLock
import typings.winjs.WinJS.Utilities.Key.numPad0
import typings.winjs.WinJS.Utilities.Key.numPad1
import typings.winjs.WinJS.Utilities.Key.numPad2
import typings.winjs.WinJS.Utilities.Key.numPad3
import typings.winjs.WinJS.Utilities.Key.numPad4
import typings.winjs.WinJS.Utilities.Key.numPad5
import typings.winjs.WinJS.Utilities.Key.numPad6
import typings.winjs.WinJS.Utilities.Key.numPad7
import typings.winjs.WinJS.Utilities.Key.numPad8
import typings.winjs.WinJS.Utilities.Key.numPad9
import typings.winjs.WinJS.Utilities.Key.o
import typings.winjs.WinJS.Utilities.Key.openBracket
import typings.winjs.WinJS.Utilities.Key.p
import typings.winjs.WinJS.Utilities.Key.pageDown
import typings.winjs.WinJS.Utilities.Key.pageUp
import typings.winjs.WinJS.Utilities.Key.pause
import typings.winjs.WinJS.Utilities.Key.period
import typings.winjs.WinJS.Utilities.Key.q
import typings.winjs.WinJS.Utilities.Key.r
import typings.winjs.WinJS.Utilities.Key.rightArrow
import typings.winjs.WinJS.Utilities.Key.rightWindows
import typings.winjs.WinJS.Utilities.Key.s
import typings.winjs.WinJS.Utilities.Key.scrollLock
import typings.winjs.WinJS.Utilities.Key.semicolon
import typings.winjs.WinJS.Utilities.Key.shift
import typings.winjs.WinJS.Utilities.Key.singleQuote
import typings.winjs.WinJS.Utilities.Key.space
import typings.winjs.WinJS.Utilities.Key.subtract
import typings.winjs.WinJS.Utilities.Key.t
import typings.winjs.WinJS.Utilities.Key.tab
import typings.winjs.WinJS.Utilities.Key.u
import typings.winjs.WinJS.Utilities.Key.upArrow
import typings.winjs.WinJS.Utilities.Key.v
import typings.winjs.WinJS.Utilities.Key.w
import typings.winjs.WinJS.Utilities.Key.x
import typings.winjs.WinJS.Utilities.Key.y
import typings.winjs.WinJS.Utilities.Key.z
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
sealed trait Key extends js.Object

//#region Enumerations
/**
  * Defines a set of keyboard values.
  **/
@JSGlobal("WinJS.Utilities.Key")
@js.native
object Key extends js.Object {
  /**
    * The F1 key.
    **/
  @js.native
  sealed trait F1 extends Key
  
  /**
    * The F10 key.
    **/
  @js.native
  sealed trait F10 extends Key
  
  /**
    * The F11 key.
    **/
  @js.native
  sealed trait F11 extends Key
  
  /**
    * The F12 key.
    **/
  @js.native
  sealed trait F12 extends Key
  
  /**
    * The F2 key.
    **/
  @js.native
  sealed trait F2 extends Key
  
  /**
    * The F3 key.
    **/
  @js.native
  sealed trait F3 extends Key
  
  /**
    * The F4 key.
    **/
  @js.native
  sealed trait F4 extends Key
  
  /**
    * The F5 key.
    **/
  @js.native
  sealed trait F5 extends Key
  
  /**
    * The F6 key.
    **/
  @js.native
  sealed trait F6 extends Key
  
  /**
    * The F7 key.
    **/
  @js.native
  sealed trait F7 extends Key
  
  /**
    * The F8 key.
    **/
  @js.native
  sealed trait F8 extends Key
  
  /**
    * The F9 key.
    **/
  @js.native
  sealed trait F9 extends Key
  
  /**
    * The open bracket key ([).
    **/
  /**
    * The XBox One gamepad A button.
    **/
  @js.native
  sealed trait GamepadA extends Key
  
  /**
    * The XBox One gamepad B button.
    **/
  @js.native
  sealed trait GamepadB extends Key
  
  /**
    * The XBox One gamepad dpad down.
    **/
  @js.native
  sealed trait GamepadDPadDown extends Key
  
  /**
    * The XBox One gamepad dpad left.
    **/
  @js.native
  sealed trait GamepadDPadLeft extends Key
  
  /**
    * The XBox One gamepad dpad right.
    **/
  @js.native
  sealed trait GamepadDPadRight extends Key
  
  /**
    * The XBox One gamepad dpad up.
    **/
  @js.native
  sealed trait GamepadDPadUp extends Key
  
  /**
    * The XBox One gamepad left shoulder.
    **/
  @js.native
  sealed trait GamepadLeftShoulder extends Key
  
  /**
    * The XBox One gamepad left thumbstick button.
    **/
  @js.native
  sealed trait GamepadLeftThumbstick extends Key
  
  /**
    * The XBox One gamepad left thumbstick's down.
    **/
  @js.native
  sealed trait GamepadLeftThumbstickDown extends Key
  
  /**
    * The XBox One gamepad left thumbstick's left.
    **/
  @js.native
  sealed trait GamepadLeftThumbstickLeft extends Key
  
  /**
    * The XBox One gamepad left thumbstick's right.
    **/
  @js.native
  sealed trait GamepadLeftThumbstickRight extends Key
  
  /**
    * The XBox One gamepad left thumbstick's up.
    **/
  @js.native
  sealed trait GamepadLeftThumbstickUp extends Key
  
  /**
    * The XBox One gamepad left trigger.
    **/
  @js.native
  sealed trait GamepadLeftTrigger extends Key
  
  /**
    * The XBox One gamepad menu button.
    **/
  @js.native
  sealed trait GamepadMenu extends Key
  
  /**
    * The XBox One gamepad right shoulder.
    **/
  @js.native
  sealed trait GamepadRightShoulder extends Key
  
  /**
    * The XBox One gamepad right thumbstick button.
    **/
  @js.native
  sealed trait GamepadRightThumbstick extends Key
  
  /**
    * The XBox One gamepad right thumbstick's down.
    **/
  @js.native
  sealed trait GamepadRightThumbstickDown extends Key
  
  /**
    * The XBox One gamepad right thumbstick's left.
    **/
  @js.native
  sealed trait GamepadRightThumbstickLeft extends Key
  
  /**
    * The XBox One gamepad right thumbstick's right.
    **/
  @js.native
  sealed trait GamepadRightThumbstickRight extends Key
  
  /**
    * The XBox One gamepad right thumbstick's up.
    **/
  @js.native
  sealed trait GamepadRightThumbstickUp extends Key
  
  /**
    * The XBox One gamepad right trigger.
    **/
  @js.native
  sealed trait GamepadRightTrigger extends Key
  
  /**
    * The XBox One gamepad view button.
    **/
  @js.native
  sealed trait GamepadView extends Key
  
  /**
    * The XBox One gamepad X button.
    **/
  @js.native
  sealed trait GamepadX extends Key
  
  /**
    * The XBox One gamepad Y button.
    **/
  @js.native
  sealed trait GamepadY extends Key
  
  /**
    * Any IME input.
    **/
  @js.native
  sealed trait IME extends Key
  
  /**
    * The XBox One Remote navigation accept button.
    **/
  @js.native
  sealed trait NavigationAccept extends Key
  
  /**
    * The XBox One Remote navigation cancel button.
    **/
  @js.native
  sealed trait NavigationCancel extends Key
  
  /**
    * The XBox One Remote navigation down button.
    **/
  @js.native
  sealed trait NavigationDown extends Key
  
  /**
    * The XBox One Remote navigation left button.
    **/
  @js.native
  sealed trait NavigationLeft extends Key
  
  /**
    * The XBox One Remote navigation menu button.
    **/
  @js.native
  sealed trait NavigationMenu extends Key
  
  /**
    * The XBox One Remote navigation right button.
    **/
  @js.native
  sealed trait NavigationRight extends Key
  
  /**
    * The XBox One Remote navigation up button.
    **/
  @js.native
  sealed trait NavigationUp extends Key
  
  /**
    * The XBox One Remote navigation view button.
    **/
  @js.native
  sealed trait NavigationView extends Key
  
  /**
    * The a key.
    **/
  @js.native
  sealed trait a extends Key
  
  /**
    * The addition key (+).
    **/
  @js.native
  sealed trait add extends Key
  
  /**
    * The ALT key.
    **/
  @js.native
  sealed trait alt extends Key
  
  /**
    * The b key.
    **/
  @js.native
  sealed trait b extends Key
  
  /**
    * The backslash key (\).
    **/
  @js.native
  sealed trait backSlash extends Key
  
  /**
    * The BACKSPACE key.
    **/
  @js.native
  sealed trait backspace extends Key
  
  /**
    * The browser BACK key.
    **/
  @js.native
  sealed trait browserBack extends Key
  
  /**
    * The browser FORWARD key.
    **/
  @js.native
  sealed trait browserForward extends Key
  
  /**
    * The c key.
    **/
  @js.native
  sealed trait c extends Key
  
  /**
    * The CAPS LOCK key.
    **/
  @js.native
  sealed trait capsLock extends Key
  
  /**
    * The close bracket key (]).
    **/
  @js.native
  sealed trait closeBracket extends Key
  
  /**
    * The comma key (,).
    **/
  @js.native
  sealed trait comma extends Key
  
  /**
    * The CTRL key.
    **/
  @js.native
  sealed trait ctrl extends Key
  
  /**
    * The d key.
    **/
  @js.native
  sealed trait d extends Key
  
  /**
    * The dash key (-).
    **/
  @js.native
  sealed trait dash extends Key
  
  /**
    * The decimal point key (.)
    **/
  @js.native
  sealed trait decimalPoint extends Key
  
  /**
    * The DELETE key.
    **/
  @js.native
  sealed trait deleteKey extends Key
  
  /**
    * The division key (/).
    **/
  @js.native
  sealed trait divide extends Key
  
  /**
    * The DOWN ARROW key.
    **/
  @js.native
  sealed trait downArrow extends Key
  
  /**
    * The e key.
    **/
  @js.native
  sealed trait e extends Key
  
  /**
    * The END key.
    **/
  @js.native
  sealed trait end extends Key
  
  /**
    * The ENTER key.
    **/
  @js.native
  sealed trait enter extends Key
  
  /**
    * The equals key (=).
    **/
  @js.native
  sealed trait equal extends Key
  
  /**
    * The ESCAPE key.
    **/
  @js.native
  sealed trait escape extends Key
  
  /**
    * The f key.
    **/
  @js.native
  sealed trait f extends Key
  
  /**
    * The forward slash key (/).
    **/
  @js.native
  sealed trait forwardSlash extends Key
  
  /**
    * The g key.
    **/
  @js.native
  sealed trait g extends Key
  
  /**
    * The grave accent key (`).
    **/
  @js.native
  sealed trait graveAccent extends Key
  
  /**
    * The h key.
    **/
  @js.native
  sealed trait h extends Key
  
  /**
    * The HOME key.
    **/
  @js.native
  sealed trait home extends Key
  
  /**
    * The i key.
    **/
  @js.native
  sealed trait i extends Key
  
  /**
    * The INSERT key.
    **/
  @js.native
  sealed trait insert extends Key
  
  /**
    * The j key.
    **/
  @js.native
  sealed trait j extends Key
  
  /**
    * The k key.
    **/
  @js.native
  sealed trait k extends Key
  
  /**
    * The l key.
    **/
  @js.native
  sealed trait l extends Key
  
  /**
    * The LEFT ARROW key.
    **/
  @js.native
  sealed trait leftArrow extends Key
  
  /**
    * The left Windows key.
    **/
  @js.native
  sealed trait leftWindows extends Key
  
  /**
    * The m key.
    **/
  @js.native
  sealed trait m extends Key
  
  /**
    * The menu key.
    **/
  @js.native
  sealed trait menu extends Key
  
  /**
    * The multiplication key (*).
    **/
  @js.native
  sealed trait multiply extends Key
  
  /**
    * The n key.
    **/
  @js.native
  sealed trait n extends Key
  
  /**
    * The 0 key.
    **/
  @js.native
  sealed trait num0 extends Key
  
  /**
    * The 1 key.
    **/
  @js.native
  sealed trait num1 extends Key
  
  /**
    * The 2 key.
    **/
  @js.native
  sealed trait num2 extends Key
  
  /**
    * The 3 key.
    **/
  @js.native
  sealed trait num3 extends Key
  
  /**
    * The 4 key.
    **/
  @js.native
  sealed trait num4 extends Key
  
  /**
    * The 5 key.
    **/
  @js.native
  sealed trait num5 extends Key
  
  /**
    * The 6 key.
    **/
  @js.native
  sealed trait num6 extends Key
  
  /**
    * The 7 key.
    **/
  @js.native
  sealed trait num7 extends Key
  
  /**
    * The 8 key.
    **/
  @js.native
  sealed trait num8 extends Key
  
  /**
    * The 9 key.
    **/
  @js.native
  sealed trait num9 extends Key
  
  /**
    * The NUMBER LOCK key.
    **/
  @js.native
  sealed trait numLock extends Key
  
  /**
    * The 0 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad0 extends Key
  
  /**
    * The 1 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad1 extends Key
  
  /**
    * The 2 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad2 extends Key
  
  /**
    * The 3 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad3 extends Key
  
  /**
    * The 4 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad4 extends Key
  
  /**
    * The 5 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad5 extends Key
  
  /**
    * The 6 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad6 extends Key
  
  /**
    * The 7 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad7 extends Key
  
  /**
    * The 8 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad8 extends Key
  
  /**
    * The 9 key on the numerical keypad.
    **/
  @js.native
  sealed trait numPad9 extends Key
  
  /**
    * The o key.
    **/
  @js.native
  sealed trait o extends Key
  
  /**
    * The open bracket key ([).
    **/
  @js.native
  sealed trait openBracket extends Key
  
  /**
    * The p key.
    **/
  @js.native
  sealed trait p extends Key
  
  /**
    * The PAGE DOWN key.
    **/
  @js.native
  sealed trait pageDown extends Key
  
  /**
    * The PAGE UP key.
    **/
  @js.native
  sealed trait pageUp extends Key
  
  /**
    * The PAUSE key.
    **/
  @js.native
  sealed trait pause extends Key
  
  /**
    * The period key (.).
    **/
  @js.native
  sealed trait period extends Key
  
  /**
    * The q key.
    **/
  @js.native
  sealed trait q extends Key
  
  /**
    * The r key.
    **/
  @js.native
  sealed trait r extends Key
  
  /**
    * The RIGHT ARROW key.
    **/
  @js.native
  sealed trait rightArrow extends Key
  
  /**
    * The right Windows key.
    **/
  @js.native
  sealed trait rightWindows extends Key
  
  /**
    * The s key.
    **/
  @js.native
  sealed trait s extends Key
  
  /**
    * The SCROLL LOCK key.
    **/
  @js.native
  sealed trait scrollLock extends Key
  
  /**
    * The semicolon key (;).
    **/
  @js.native
  sealed trait semicolon extends Key
  
  /**
    * The SHIFT key.
    **/
  @js.native
  sealed trait shift extends Key
  
  /**
    * The single quote key (').
    **/
  @js.native
  sealed trait singleQuote extends Key
  
  /**
    * The SPACE key.
    **/
  @js.native
  sealed trait space extends Key
  
  /**
    * The subtraction key (-).
    **/
  @js.native
  sealed trait subtract extends Key
  
  /**
    * The t key.
    **/
  @js.native
  sealed trait t extends Key
  
  /**
    * The TAB key.
    **/
  @js.native
  sealed trait tab extends Key
  
  /**
    * The u key.
    **/
  @js.native
  sealed trait u extends Key
  
  /**
    * The UP ARROW key.
    **/
  @js.native
  sealed trait upArrow extends Key
  
  /**
    * The v key.
    **/
  @js.native
  sealed trait v extends Key
  
  /**
    * The w key.
    **/
  @js.native
  sealed trait w extends Key
  
  /**
    * The x key.
    **/
  @js.native
  sealed trait x extends Key
  
  /**
    * The y key.
    **/
  @js.native
  sealed trait y extends Key
  
  /**
    * The z key.
    **/
  @js.native
  sealed trait z extends Key
  
  @JSBracketAccess
  def apply(value: Double): js.UndefOr[Key with Double] = js.native
  /* 74 */ @js.native
  object F1 extends TopLevel[F1 with Double]
  
  /* 83 */ @js.native
  object F10 extends TopLevel[F10 with Double]
  
  /* 84 */ @js.native
  object F11 extends TopLevel[F11 with Double]
  
  /* 85 */ @js.native
  object F12 extends TopLevel[F12 with Double]
  
  /* 75 */ @js.native
  object F2 extends TopLevel[F2 with Double]
  
  /* 76 */ @js.native
  object F3 extends TopLevel[F3 with Double]
  
  /* 77 */ @js.native
  object F4 extends TopLevel[F4 with Double]
  
  /* 78 */ @js.native
  object F5 extends TopLevel[F5 with Double]
  
  /* 79 */ @js.native
  object F6 extends TopLevel[F6 with Double]
  
  /* 80 */ @js.native
  object F7 extends TopLevel[F7 with Double]
  
  /* 81 */ @js.native
  object F8 extends TopLevel[F8 with Double]
  
  /* 82 */ @js.native
  object F9 extends TopLevel[F9 with Double]
  
  /* 105 */ @js.native
  object GamepadA extends TopLevel[GamepadA with Double]
  
  /* 106 */ @js.native
  object GamepadB extends TopLevel[GamepadB with Double]
  
  /* 114 */ @js.native
  object GamepadDPadDown extends TopLevel[GamepadDPadDown with Double]
  
  /* 115 */ @js.native
  object GamepadDPadLeft extends TopLevel[GamepadDPadLeft with Double]
  
  /* 116 */ @js.native
  object GamepadDPadRight extends TopLevel[GamepadDPadRight with Double]
  
  /* 113 */ @js.native
  object GamepadDPadUp extends TopLevel[GamepadDPadUp with Double]
  
  /* 110 */ @js.native
  object GamepadLeftShoulder extends TopLevel[GamepadLeftShoulder with Double]
  
  /* 119 */ @js.native
  object GamepadLeftThumbstick extends TopLevel[GamepadLeftThumbstick with Double]
  
  /* 122 */ @js.native
  object GamepadLeftThumbstickDown extends TopLevel[GamepadLeftThumbstickDown with Double]
  
  /* 124 */ @js.native
  object GamepadLeftThumbstickLeft extends TopLevel[GamepadLeftThumbstickLeft with Double]
  
  /* 123 */ @js.native
  object GamepadLeftThumbstickRight extends TopLevel[GamepadLeftThumbstickRight with Double]
  
  /* 121 */ @js.native
  object GamepadLeftThumbstickUp extends TopLevel[GamepadLeftThumbstickUp with Double]
  
  /* 111 */ @js.native
  object GamepadLeftTrigger extends TopLevel[GamepadLeftTrigger with Double]
  
  /* 117 */ @js.native
  object GamepadMenu extends TopLevel[GamepadMenu with Double]
  
  /* 109 */ @js.native
  object GamepadRightShoulder extends TopLevel[GamepadRightShoulder with Double]
  
  /* 120 */ @js.native
  object GamepadRightThumbstick extends TopLevel[GamepadRightThumbstick with Double]
  
  /* 126 */ @js.native
  object GamepadRightThumbstickDown extends TopLevel[GamepadRightThumbstickDown with Double]
  
  /* 128 */ @js.native
  object GamepadRightThumbstickLeft extends TopLevel[GamepadRightThumbstickLeft with Double]
  
  /* 127 */ @js.native
  object GamepadRightThumbstickRight extends TopLevel[GamepadRightThumbstickRight with Double]
  
  /* 125 */ @js.native
  object GamepadRightThumbstickUp extends TopLevel[GamepadRightThumbstickUp with Double]
  
  /* 112 */ @js.native
  object GamepadRightTrigger extends TopLevel[GamepadRightTrigger with Double]
  
  /* 118 */ @js.native
  object GamepadView extends TopLevel[GamepadView with Double]
  
  /* 107 */ @js.native
  object GamepadX extends TopLevel[GamepadX with Double]
  
  /* 108 */ @js.native
  object GamepadY extends TopLevel[GamepadY with Double]
  
  /* 133 */ @js.native
  object IME extends TopLevel[IME with Double]
  
  /* 92 */ @js.native
  object NavigationAccept extends TopLevel[NavigationAccept with Double]
  
  /* 93 */ @js.native
  object NavigationCancel extends TopLevel[NavigationCancel with Double]
  
  /* 89 */ @js.native
  object NavigationDown extends TopLevel[NavigationDown with Double]
  
  /* 90 */ @js.native
  object NavigationLeft extends TopLevel[NavigationLeft with Double]
  
  /* 87 */ @js.native
  object NavigationMenu extends TopLevel[NavigationMenu with Double]
  
  /* 91 */ @js.native
  object NavigationRight extends TopLevel[NavigationRight with Double]
  
  /* 88 */ @js.native
  object NavigationUp extends TopLevel[NavigationUp with Double]
  
  /* 86 */ @js.native
  object NavigationView extends TopLevel[NavigationView with Double]
  
  /* 30 */ @js.native
  object a extends TopLevel[a with Double]
  
  /* 70 */ @js.native
  object add extends TopLevel[add with Double]
  
  /* 5 */ @js.native
  object alt extends TopLevel[alt with Double]
  
  /* 31 */ @js.native
  object b extends TopLevel[b with Double]
  
  /* 130 */ @js.native
  object backSlash extends TopLevel[backSlash with Double]
  
  /* 0 */ @js.native
  object backspace extends TopLevel[backspace with Double]
  
  /* 96 */ @js.native
  object browserBack extends TopLevel[browserBack with Double]
  
  /* 97 */ @js.native
  object browserForward extends TopLevel[browserForward with Double]
  
  /* 32 */ @js.native
  object c extends TopLevel[c with Double]
  
  /* 7 */ @js.native
  object capsLock extends TopLevel[capsLock with Double]
  
  /* 131 */ @js.native
  object closeBracket extends TopLevel[closeBracket with Double]
  
  /* 100 */ @js.native
  object comma extends TopLevel[comma with Double]
  
  /* 4 */ @js.native
  object ctrl extends TopLevel[ctrl with Double]
  
  /* 33 */ @js.native
  object d extends TopLevel[d with Double]
  
  /* 101 */ @js.native
  object dash extends TopLevel[dash with Double]
  
  /* 72 */ @js.native
  object decimalPoint extends TopLevel[decimalPoint with Double]
  
  /* 19 */ @js.native
  object deleteKey extends TopLevel[deleteKey with Double]
  
  /* 73 */ @js.native
  object divide extends TopLevel[divide with Double]
  
  /* 17 */ @js.native
  object downArrow extends TopLevel[downArrow with Double]
  
  /* 34 */ @js.native
  object e extends TopLevel[e with Double]
  
  /* 12 */ @js.native
  object end extends TopLevel[end with Double]
  
  /* 2 */ @js.native
  object enter extends TopLevel[enter with Double]
  
  /* 99 */ @js.native
  object equal extends TopLevel[equal with Double]
  
  /* 8 */ @js.native
  object escape extends TopLevel[escape with Double]
  
  /* 35 */ @js.native
  object f extends TopLevel[f with Double]
  
  /* 103 */ @js.native
  object forwardSlash extends TopLevel[forwardSlash with Double]
  
  /* 36 */ @js.native
  object g extends TopLevel[g with Double]
  
  /* 104 */ @js.native
  object graveAccent extends TopLevel[graveAccent with Double]
  
  /* 37 */ @js.native
  object h extends TopLevel[h with Double]
  
  /* 13 */ @js.native
  object home extends TopLevel[home with Double]
  
  /* 38 */ @js.native
  object i extends TopLevel[i with Double]
  
  /* 18 */ @js.native
  object insert extends TopLevel[insert with Double]
  
  /* 39 */ @js.native
  object j extends TopLevel[j with Double]
  
  /* 40 */ @js.native
  object k extends TopLevel[k with Double]
  
  /* 41 */ @js.native
  object l extends TopLevel[l with Double]
  
  /* 14 */ @js.native
  object leftArrow extends TopLevel[leftArrow with Double]
  
  /* 56 */ @js.native
  object leftWindows extends TopLevel[leftWindows with Double]
  
  /* 42 */ @js.native
  object m extends TopLevel[m with Double]
  
  /* 58 */ @js.native
  object menu extends TopLevel[menu with Double]
  
  /* 69 */ @js.native
  object multiply extends TopLevel[multiply with Double]
  
  /* 43 */ @js.native
  object n extends TopLevel[n with Double]
  
  /* 20 */ @js.native
  object num0 extends TopLevel[num0 with Double]
  
  /* 21 */ @js.native
  object num1 extends TopLevel[num1 with Double]
  
  /* 22 */ @js.native
  object num2 extends TopLevel[num2 with Double]
  
  /* 23 */ @js.native
  object num3 extends TopLevel[num3 with Double]
  
  /* 24 */ @js.native
  object num4 extends TopLevel[num4 with Double]
  
  /* 25 */ @js.native
  object num5 extends TopLevel[num5 with Double]
  
  /* 26 */ @js.native
  object num6 extends TopLevel[num6 with Double]
  
  /* 27 */ @js.native
  object num7 extends TopLevel[num7 with Double]
  
  /* 28 */ @js.native
  object num8 extends TopLevel[num8 with Double]
  
  /* 29 */ @js.native
  object num9 extends TopLevel[num9 with Double]
  
  /* 94 */ @js.native
  object numLock extends TopLevel[numLock with Double]
  
  /* 59 */ @js.native
  object numPad0 extends TopLevel[numPad0 with Double]
  
  /* 60 */ @js.native
  object numPad1 extends TopLevel[numPad1 with Double]
  
  /* 61 */ @js.native
  object numPad2 extends TopLevel[numPad2 with Double]
  
  /* 62 */ @js.native
  object numPad3 extends TopLevel[numPad3 with Double]
  
  /* 63 */ @js.native
  object numPad4 extends TopLevel[numPad4 with Double]
  
  /* 64 */ @js.native
  object numPad5 extends TopLevel[numPad5 with Double]
  
  /* 65 */ @js.native
  object numPad6 extends TopLevel[numPad6 with Double]
  
  /* 66 */ @js.native
  object numPad7 extends TopLevel[numPad7 with Double]
  
  /* 67 */ @js.native
  object numPad8 extends TopLevel[numPad8 with Double]
  
  /* 68 */ @js.native
  object numPad9 extends TopLevel[numPad9 with Double]
  
  /* 44 */ @js.native
  object o extends TopLevel[o with Double]
  
  /* 129 */ @js.native
  object openBracket extends TopLevel[openBracket with Double]
  
  /* 45 */ @js.native
  object p extends TopLevel[p with Double]
  
  /* 11 */ @js.native
  object pageDown extends TopLevel[pageDown with Double]
  
  /* 10 */ @js.native
  object pageUp extends TopLevel[pageUp with Double]
  
  /* 6 */ @js.native
  object pause extends TopLevel[pause with Double]
  
  /* 102 */ @js.native
  object period extends TopLevel[period with Double]
  
  /* 46 */ @js.native
  object q extends TopLevel[q with Double]
  
  /* 47 */ @js.native
  object r extends TopLevel[r with Double]
  
  /* 16 */ @js.native
  object rightArrow extends TopLevel[rightArrow with Double]
  
  /* 57 */ @js.native
  object rightWindows extends TopLevel[rightWindows with Double]
  
  /* 48 */ @js.native
  object s extends TopLevel[s with Double]
  
  /* 95 */ @js.native
  object scrollLock extends TopLevel[scrollLock with Double]
  
  /* 98 */ @js.native
  object semicolon extends TopLevel[semicolon with Double]
  
  /* 3 */ @js.native
  object shift extends TopLevel[shift with Double]
  
  /* 132 */ @js.native
  object singleQuote extends TopLevel[singleQuote with Double]
  
  /* 9 */ @js.native
  object space extends TopLevel[space with Double]
  
  /* 71 */ @js.native
  object subtract extends TopLevel[subtract with Double]
  
  /* 49 */ @js.native
  object t extends TopLevel[t with Double]
  
  /* 1 */ @js.native
  object tab extends TopLevel[tab with Double]
  
  /* 50 */ @js.native
  object u extends TopLevel[u with Double]
  
  /* 15 */ @js.native
  object upArrow extends TopLevel[upArrow with Double]
  
  /* 51 */ @js.native
  object v extends TopLevel[v with Double]
  
  /* 52 */ @js.native
  object w extends TopLevel[w with Double]
  
  /* 53 */ @js.native
  object x extends TopLevel[x with Double]
  
  /* 54 */ @js.native
  object y extends TopLevel[y with Double]
  
  /* 55 */ @js.native
  object z extends TopLevel[z with Double]
  
}

