package typings.vexdb

import typings.vexdb.requestObjectsMod.Endpoint
import typings.vexdb.requestObjectsMod.Grades
import typings.vexdb.requestObjectsMod.Programs
import typings.vexdb.requestObjectsMod.Seasons
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object vexdbStrings {
  @js.native
  sealed trait `Bridge Battle` extends Seasons
  
  @js.native
  sealed trait `Change Up` extends Seasons
  
  @js.native
  sealed trait `Clean Sweep` extends Seasons
  
  @js.native
  sealed trait Collge extends Grades
  
  @js.native
  sealed trait Elevation extends Seasons
  
  @js.native
  sealed trait Gateway extends Seasons
  
  @js.native
  sealed trait `High School` extends Grades
  
  @js.native
  sealed trait `In The Zone` extends Seasons
  
  @js.native
  sealed trait `Middle School` extends Grades
  
  @js.native
  sealed trait `Nothing But Net` extends Seasons
  
  @js.native
  sealed trait `Round Up` extends Seasons
  
  @js.native
  sealed trait `Sack Attack` extends Seasons
  
  @js.native
  sealed trait Skyrise extends Seasons
  
  @js.native
  sealed trait StarStruck extends Seasons
  
  @js.native
  sealed trait `Toss Up` extends Seasons
  
  @js.native
  sealed trait `Tower Takeover` extends Seasons
  
  @js.native
  sealed trait `Turning Point` extends Seasons
  
  @js.native
  sealed trait VEXU extends Programs
  
  @js.native
  sealed trait VRC extends Programs
  
  @js.native
  sealed trait ap extends js.Object
  
  @js.native
  sealed trait attempts extends js.Object
  
  @js.native
  sealed trait awards extends Endpoint
  
  @js.native
  sealed trait blue1 extends js.Object
  
  @js.native
  sealed trait blue2 extends js.Object
  
  @js.native
  sealed trait blue3 extends js.Object
  
  @js.native
  sealed trait bluescore extends js.Object
  
  @js.native
  sealed trait bluesit extends js.Object
  
  @js.native
  sealed trait ccwm extends js.Object
  
  @js.native
  sealed trait city extends js.Object
  
  @js.native
  sealed trait country extends js.Object
  
  @js.native
  sealed trait current extends Seasons
  
  @js.native
  sealed trait division extends js.Object
  
  @js.native
  sealed trait dpr extends js.Object
  
  @js.native
  sealed trait end extends js.Object
  
  @js.native
  sealed trait events extends Endpoint
  
  @js.native
  sealed trait fetch extends js.Object
  
  @js.native
  sealed trait field extends js.Object
  
  @js.native
  sealed trait instance extends js.Object
  
  @js.native
  sealed trait item extends js.Object
  
  @js.native
  sealed trait key extends js.Object
  
  @js.native
  sealed trait loc_address1 extends js.Object
  
  @js.native
  sealed trait loc_address2 extends js.Object
  
  @js.native
  sealed trait loc_city extends js.Object
  
  @js.native
  sealed trait loc_country extends js.Object
  
  @js.native
  sealed trait loc_postcode extends js.Object
  
  @js.native
  sealed trait loc_region extends js.Object
  
  @js.native
  sealed trait loc_venue extends js.Object
  
  @js.native
  sealed trait losses extends js.Object
  
  @js.native
  sealed trait matches extends Endpoint
  
  @js.native
  sealed trait matchnum extends js.Object
  
  @js.native
  sealed trait max_score extends js.Object
  
  @js.native
  sealed trait name extends js.Object
  
  @js.native
  sealed trait number extends js.Object
  
  @js.native
  sealed trait opr extends js.Object
  
  @js.native
  sealed trait order extends js.Object
  
  @js.native
  sealed trait organisation extends js.Object
  
  @js.native
  sealed trait prefetch extends js.Object
  
  @js.native
  sealed trait program extends js.Object
  
  @js.native
  sealed trait rank extends js.Object
  
  @js.native
  sealed trait rankings extends Endpoint
  
  @js.native
  sealed trait red1 extends js.Object
  
  @js.native
  sealed trait red2 extends js.Object
  
  @js.native
  sealed trait red3 extends js.Object
  
  @js.native
  sealed trait redscore extends js.Object
  
  @js.native
  sealed trait redsit extends js.Object
  
  @js.native
  sealed trait region extends js.Object
  
  @js.native
  sealed trait robot_name extends js.Object
  
  @js.native
  sealed trait scheduled extends js.Object
  
  @js.native
  sealed trait score extends js.Object
  
  @js.native
  sealed trait season extends js.Object
  
  @js.native
  sealed trait season_rankings extends Endpoint
  
  @js.native
  sealed trait skills extends Endpoint
  
  @js.native
  sealed trait sku extends js.Object
  
  @js.native
  sealed trait sp extends js.Object
  
  @js.native
  sealed trait start extends js.Object
  
  @js.native
  sealed trait team extends js.Object
  
  @js.native
  sealed trait team_name extends js.Object
  
  @js.native
  sealed trait teams extends Endpoint
  
  @js.native
  sealed trait ties extends js.Object
  
  @js.native
  sealed trait trsp extends js.Object
  
  @js.native
  sealed trait `type` extends js.Object
  
  @js.native
  sealed trait vrating extends js.Object
  
  @js.native
  sealed trait vrating_rank extends js.Object
  
  @js.native
  sealed trait wins extends js.Object
  
  @js.native
  sealed trait wp extends js.Object
  
  @scala.inline
  def `Bridge Battle`: `Bridge Battle` = ("Bridge Battle").asInstanceOf[`Bridge Battle`]
  @scala.inline
  def `Change Up`: `Change Up` = ("Change Up").asInstanceOf[`Change Up`]
  @scala.inline
  def `Clean Sweep`: `Clean Sweep` = ("Clean Sweep").asInstanceOf[`Clean Sweep`]
  @scala.inline
  def Collge: Collge = "Collge".asInstanceOf[Collge]
  @scala.inline
  def Elevation: Elevation = "Elevation".asInstanceOf[Elevation]
  @scala.inline
  def Gateway: Gateway = "Gateway".asInstanceOf[Gateway]
  @scala.inline
  def `High School`: `High School` = ("High School").asInstanceOf[`High School`]
  @scala.inline
  def `In The Zone`: `In The Zone` = ("In The Zone").asInstanceOf[`In The Zone`]
  @scala.inline
  def `Middle School`: `Middle School` = ("Middle School").asInstanceOf[`Middle School`]
  @scala.inline
  def `Nothing But Net`: `Nothing But Net` = ("Nothing But Net").asInstanceOf[`Nothing But Net`]
  @scala.inline
  def `Round Up`: `Round Up` = ("Round Up").asInstanceOf[`Round Up`]
  @scala.inline
  def `Sack Attack`: `Sack Attack` = ("Sack Attack").asInstanceOf[`Sack Attack`]
  @scala.inline
  def Skyrise: Skyrise = "Skyrise".asInstanceOf[Skyrise]
  @scala.inline
  def StarStruck: StarStruck = "StarStruck".asInstanceOf[StarStruck]
  @scala.inline
  def `Toss Up`: `Toss Up` = ("Toss Up").asInstanceOf[`Toss Up`]
  @scala.inline
  def `Tower Takeover`: `Tower Takeover` = ("Tower Takeover").asInstanceOf[`Tower Takeover`]
  @scala.inline
  def `Turning Point`: `Turning Point` = ("Turning Point").asInstanceOf[`Turning Point`]
  @scala.inline
  def VEXU: VEXU = "VEXU".asInstanceOf[VEXU]
  @scala.inline
  def VRC: VRC = "VRC".asInstanceOf[VRC]
  @scala.inline
  def ap: ap = "ap".asInstanceOf[ap]
  @scala.inline
  def attempts: attempts = "attempts".asInstanceOf[attempts]
  @scala.inline
  def awards: awards = "awards".asInstanceOf[awards]
  @scala.inline
  def blue1: blue1 = "blue1".asInstanceOf[blue1]
  @scala.inline
  def blue2: blue2 = "blue2".asInstanceOf[blue2]
  @scala.inline
  def blue3: blue3 = "blue3".asInstanceOf[blue3]
  @scala.inline
  def bluescore: bluescore = "bluescore".asInstanceOf[bluescore]
  @scala.inline
  def bluesit: bluesit = "bluesit".asInstanceOf[bluesit]
  @scala.inline
  def ccwm: ccwm = "ccwm".asInstanceOf[ccwm]
  @scala.inline
  def city: city = "city".asInstanceOf[city]
  @scala.inline
  def country: country = "country".asInstanceOf[country]
  @scala.inline
  def current: current = "current".asInstanceOf[current]
  @scala.inline
  def division: division = "division".asInstanceOf[division]
  @scala.inline
  def dpr: dpr = "dpr".asInstanceOf[dpr]
  @scala.inline
  def end: end = "end".asInstanceOf[end]
  @scala.inline
  def events: events = "events".asInstanceOf[events]
  @scala.inline
  def fetch: fetch = "fetch".asInstanceOf[fetch]
  @scala.inline
  def field: field = "field".asInstanceOf[field]
  @scala.inline
  def instance: instance = "instance".asInstanceOf[instance]
  @scala.inline
  def item: item = "item".asInstanceOf[item]
  @scala.inline
  def key: key = "key".asInstanceOf[key]
  @scala.inline
  def loc_address1: loc_address1 = "loc_address1".asInstanceOf[loc_address1]
  @scala.inline
  def loc_address2: loc_address2 = "loc_address2".asInstanceOf[loc_address2]
  @scala.inline
  def loc_city: loc_city = "loc_city".asInstanceOf[loc_city]
  @scala.inline
  def loc_country: loc_country = "loc_country".asInstanceOf[loc_country]
  @scala.inline
  def loc_postcode: loc_postcode = "loc_postcode".asInstanceOf[loc_postcode]
  @scala.inline
  def loc_region: loc_region = "loc_region".asInstanceOf[loc_region]
  @scala.inline
  def loc_venue: loc_venue = "loc_venue".asInstanceOf[loc_venue]
  @scala.inline
  def losses: losses = "losses".asInstanceOf[losses]
  @scala.inline
  def matches: matches = "matches".asInstanceOf[matches]
  @scala.inline
  def matchnum: matchnum = "matchnum".asInstanceOf[matchnum]
  @scala.inline
  def max_score: max_score = "max_score".asInstanceOf[max_score]
  @scala.inline
  def name: name = "name".asInstanceOf[name]
  @scala.inline
  def number: number = "number".asInstanceOf[number]
  @scala.inline
  def opr: opr = "opr".asInstanceOf[opr]
  @scala.inline
  def order: order = "order".asInstanceOf[order]
  @scala.inline
  def organisation: organisation = "organisation".asInstanceOf[organisation]
  @scala.inline
  def prefetch: prefetch = "prefetch".asInstanceOf[prefetch]
  @scala.inline
  def program: program = "program".asInstanceOf[program]
  @scala.inline
  def rank: rank = "rank".asInstanceOf[rank]
  @scala.inline
  def rankings: rankings = "rankings".asInstanceOf[rankings]
  @scala.inline
  def red1: red1 = "red1".asInstanceOf[red1]
  @scala.inline
  def red2: red2 = "red2".asInstanceOf[red2]
  @scala.inline
  def red3: red3 = "red3".asInstanceOf[red3]
  @scala.inline
  def redscore: redscore = "redscore".asInstanceOf[redscore]
  @scala.inline
  def redsit: redsit = "redsit".asInstanceOf[redsit]
  @scala.inline
  def region: region = "region".asInstanceOf[region]
  @scala.inline
  def robot_name: robot_name = "robot_name".asInstanceOf[robot_name]
  @scala.inline
  def scheduled: scheduled = "scheduled".asInstanceOf[scheduled]
  @scala.inline
  def score: score = "score".asInstanceOf[score]
  @scala.inline
  def season: season = "season".asInstanceOf[season]
  @scala.inline
  def season_rankings: season_rankings = "season_rankings".asInstanceOf[season_rankings]
  @scala.inline
  def skills: skills = "skills".asInstanceOf[skills]
  @scala.inline
  def sku: sku = "sku".asInstanceOf[sku]
  @scala.inline
  def sp: sp = "sp".asInstanceOf[sp]
  @scala.inline
  def start: start = "start".asInstanceOf[start]
  @scala.inline
  def team: team = "team".asInstanceOf[team]
  @scala.inline
  def team_name: team_name = "team_name".asInstanceOf[team_name]
  @scala.inline
  def teams: teams = "teams".asInstanceOf[teams]
  @scala.inline
  def ties: ties = "ties".asInstanceOf[ties]
  @scala.inline
  def trsp: trsp = "trsp".asInstanceOf[trsp]
  @scala.inline
  def `type`: `type` = "type".asInstanceOf[`type`]
  @scala.inline
  def vrating: vrating = "vrating".asInstanceOf[vrating]
  @scala.inline
  def vrating_rank: vrating_rank = "vrating_rank".asInstanceOf[vrating_rank]
  @scala.inline
  def wins: wins = "wins".asInstanceOf[wins]
  @scala.inline
  def wp: wp = "wp".asInstanceOf[wp]
}

