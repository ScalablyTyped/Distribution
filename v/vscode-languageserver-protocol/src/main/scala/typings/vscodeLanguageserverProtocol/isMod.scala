package typings.vscodeLanguageserverProtocol

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object isMod {
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "array")
  @js.native
  def array[T](value: js.Any): /* is std.Array<T> */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "boolean")
  @js.native
  def boolean(value: js.Any): /* is boolean */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "error")
  @js.native
  def error(value: js.Any): /* is std.Error */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "func")
  @js.native
  def func(value: js.Any): /* is std.Function */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "number")
  @js.native
  def number(value: js.Any): /* is number */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "objectLiteral")
  @js.native
  def objectLiteral(value: js.Any): /* is object */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "string")
  @js.native
  def string(value: js.Any): /* is string */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "stringArray")
  @js.native
  def stringArray(value: js.Any): /* is std.Array<string> */ Boolean = js.native
  
  @JSImport("vscode-languageserver-protocol/lib/utils/is", "typedArray")
  @js.native
  def typedArray[T](value: js.Any, check: js.Function1[/* value */ js.Any, Boolean]): /* is std.Array<T> */ Boolean = js.native
}
