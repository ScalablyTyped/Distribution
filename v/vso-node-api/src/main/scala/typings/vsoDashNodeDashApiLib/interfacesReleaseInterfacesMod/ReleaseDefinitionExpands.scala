package typings
package vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
sealed trait ReleaseDefinitionExpands extends js.Object

@JSImport("vso-node-api/interfaces/ReleaseInterfaces", "ReleaseDefinitionExpands")
@js.native
object ReleaseDefinitionExpands extends js.Object {
  @js.native
  sealed trait Artifacts
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait Environments
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait LastRelease
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait None
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait Tags
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait Triggers
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  @js.native
  sealed trait Variables
    extends vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands
  
  /* 4 */ val Artifacts: Artifacts with scala.Double = js.native
  /* 2 */ val Environments: Environments with scala.Double = js.native
  /* 64 */ val LastRelease: LastRelease with scala.Double = js.native
  /* 0 */ val None: None with scala.Double = js.native
  /* 32 */ val Tags: Tags with scala.Double = js.native
  /* 8 */ val Triggers: Triggers with scala.Double = js.native
  /* 16 */ val Variables: Variables with scala.Double = js.native
  @JSBracketAccess
  def apply(value: scala.Double): js.UndefOr[
    vsoDashNodeDashApiLib.interfacesReleaseInterfacesMod.ReleaseDefinitionExpands with scala.Double
  ] = js.native
}

