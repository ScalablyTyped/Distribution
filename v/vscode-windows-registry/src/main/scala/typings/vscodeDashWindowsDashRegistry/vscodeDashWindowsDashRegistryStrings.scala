package typings.vscodeDashWindowsDashRegistry

import typings.vscodeDashWindowsDashRegistry.vscodeDashWindowsDashRegistryMod.HKEY
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object vscodeDashWindowsDashRegistryStrings {
  @js.native
  sealed trait HKEY_CLASSES_ROOT extends HKEY
  
  @js.native
  sealed trait HKEY_CURRENT_CONFIG extends HKEY
  
  @js.native
  sealed trait HKEY_CURRENT_USER extends HKEY
  
  @js.native
  sealed trait HKEY_LOCAL_MACHINE extends HKEY
  
  @js.native
  sealed trait HKEY_USERS extends HKEY
  
  @scala.inline
  def HKEY_CLASSES_ROOT: HKEY_CLASSES_ROOT = "HKEY_CLASSES_ROOT".asInstanceOf[HKEY_CLASSES_ROOT]
  @scala.inline
  def HKEY_CURRENT_CONFIG: HKEY_CURRENT_CONFIG = "HKEY_CURRENT_CONFIG".asInstanceOf[HKEY_CURRENT_CONFIG]
  @scala.inline
  def HKEY_CURRENT_USER: HKEY_CURRENT_USER = "HKEY_CURRENT_USER".asInstanceOf[HKEY_CURRENT_USER]
  @scala.inline
  def HKEY_LOCAL_MACHINE: HKEY_LOCAL_MACHINE = "HKEY_LOCAL_MACHINE".asInstanceOf[HKEY_LOCAL_MACHINE]
  @scala.inline
  def HKEY_USERS: HKEY_USERS = "HKEY_USERS".asInstanceOf[HKEY_USERS]
}

