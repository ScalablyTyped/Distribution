...
object Highestline {
  ...
  inline def apply(highest_line: Double, lowest_line: Double, y_bottom: Double, y_top: Double): Highestline = {
  ...
  extension [Self <: Highestline](x: Self) {
    ...
    inline def setLowest_line(value: Double): Self = StObject.set(x, "lowest_line", value.asInstanceOf[js.Any])
