package typings.vscodeLanguageclient.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object InlineValueRefreshRequest {
  
  @JSImport("vscode-languageclient", "InlineValueRefreshRequest.messageDirection")
  @js.native
  val messageDirection: typings.vscodeLanguageserverProtocol.libCommonMessagesMod.MessageDirection = js.native
  
  @JSImport("vscode-languageclient", "InlineValueRefreshRequest.method")
  @js.native
  val method: /* template literal string: workspace/inlineValue/refresh */ String = js.native
  
  @JSImport("vscode-languageclient", "InlineValueRefreshRequest.type")
  @js.native
  val `type`: typings.vscodeLanguageserverProtocol.libCommonMessagesMod.ProtocolRequestType0[Unit, Unit, Unit, Unit] = js.native
}
