package typings
package vegaDashLiteLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object vegaDashLiteLibStrings {
  @js.native
  sealed trait `Autosize BACKSLASHfitBACKSLASH only works for single views and layered viewsDOT` extends js.Object
  
  @js.native
  sealed trait `Axes cannot be shared in concatenated viewsDOT` extends js.Object
  
  @js.native
  sealed trait `Axes cannot be shared in repeated viewsDOT` extends js.Object
  
  @js.native
  sealed trait AxisResolveMap extends js.Object
  
  @js.native
  sealed trait `Bar mark should not be used with point scale when rangeStep is nullDOT Please use band scale insteadDOT` extends js.Object
  
  @js.native
  sealed trait `Cannot use a fixed value of BACKSLASHrangeStepBACKSLASH when BACKSLASHautosizeBACKSLASH is BACKSLASHfitBACKSLASHDOT` extends js.Object
  
  @js.native
  sealed trait `Domains that should be unioned has conflicting sort propertiesDOT Sort will be set to trueDOT` extends js.Object
  
  @js.native
  sealed trait Empty extends js.Object
  
  @js.native
  sealed trait Flag extends js.Object
  
  @js.native
  sealed trait GuideEncodingEntry extends js.Object
  
  @js.native
  sealed trait `If BACKSLASHfromDOTfieldsBACKSLASH is not specified, BACKSLASHasBACKSLASH has to be a string that specifies the key to be used for the data from the secondary sourceDOT` extends js.Object
  
  @js.native
  sealed trait `Invalid channel for axisDOT` extends js.Object
  
  @js.native
  sealed trait `Invalid spec` extends js.Object
  
  @js.native
  sealed trait LegendComponentIndex extends js.Object
  
  @js.native
  sealed trait LegendIndex extends js.Object
  
  @js.native
  sealed trait LegendResolveMap extends js.Object
  
  @js.native
  sealed trait `Line marks cannot encode size with a non-groupby fieldDOT You may want to use trail marks insteadDOT` extends js.Object
  
  @js.native
  sealed trait ModelWithField extends js.Object
  
  @js.native
  sealed trait `Scale bindings are currently only supported for scales with unbinned, continuous domainsDOT` extends js.Object
  
  @js.native
  sealed trait ScaleComponentIndex extends js.Object
  
  @js.native
  sealed trait ScaleIndex extends js.Object
  
  @js.native
  sealed trait ScaleResolveMap extends js.Object
  
  @js.native
  sealed trait SupportedMark extends js.Object
  
  @js.native
  sealed trait `Unable to merge domains` extends js.Object
  
  @js.native
  sealed trait VgEncodeEntry extends js.Object
  
  @js.native
  sealed trait _modify extends js.Object
  
  @js.native
  sealed trait _selection_domain_ extends js.Object
  
  @js.native
  sealed trait _store extends js.Object
  
  @js.native
  sealed trait _tuple extends js.Object
  
  @js.native
  sealed trait _vgsid_ extends js.Object
  
  @js.native
  sealed trait aggregate extends js.Object
  
  @js.native
  sealed trait albers
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait albersUsa
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait alias
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait align
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait all
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgLayoutAlign
  
  @js.native
  sealed trait `all-scroll`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait angle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait area
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait ascending
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgComparatorOrder
  
  @js.native
  sealed trait auto
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait axis
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait azimuthalEqualArea
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait azimuthalEquidistant
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait band
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait bar
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait base extends js.Object
  
  @js.native
  sealed trait baseline
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait basis
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `basis-closed`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `basis-open`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait bevel
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeJoin
  
  @js.native
  sealed trait bin extends js.Object
  
  @js.native
  sealed trait `bin-linear`
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait `bin-ordinal`
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait bold
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.FontWeightString
  
  @js.native
  sealed trait boolean extends js.Object
  
  @js.native
  sealed trait bottom
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.AxisOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.TitleOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VerticalAlign
  
  @js.native
  sealed trait `bottom-left`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
  
  @js.native
  sealed trait `bottom-right`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
  
  @js.native
  sealed trait box
    extends vegaDashLiteLib.buildSrcCompositemarkBoxplotMod.BoxPlotStyle
  
  @js.native
  sealed trait `box-plot` extends js.Object
  
  @js.native
  sealed trait boxMid
    extends vegaDashLiteLib.buildSrcCompositemarkBoxplotMod.BoxPlotStyle
  
  @js.native
  sealed trait boxWhisker
    extends vegaDashLiteLib.buildSrcCompositemarkBoxplotMod.BoxPlotStyle
  
  @js.native
  sealed trait bundle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait butt
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeCap
  
  @js.native
  sealed trait cardinal
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `cardinal-closed`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `cardinal-open`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait cell
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait center
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.HorizontalAlign
       with vegaDashLiteLib.buildSrcStackMod.StackOffset
  
  @js.native
  sealed trait checkbox extends js.Object
  
  @js.native
  sealed trait circle
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait clamp extends js.Object
  
  @js.native
  sealed trait clip
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait clipAngle extends js.Object
  
  @js.native
  sealed trait clipExtent extends js.Object
  
  @js.native
  sealed trait coefficient extends js.Object
  
  @js.native
  sealed trait `col-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait collect extends js.Object
  
  @js.native
  sealed trait color
    extends vegaDashLiteLib.buildSrcChannelMod.ColorChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait column
    extends vegaDashLiteLib.buildSrcDataMod.DataSourceType
       with vegaDashLiteLib.buildSrcCompileHeaderMod.HeaderChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait concat extends js.Object
  
  @js.native
  sealed trait conicConformal
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait conicEqualArea
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait conicEquidistant
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait content extends js.Object
  
  @js.native
  sealed trait `context-menu`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait continuous extends js.Object
  
  @js.native
  sealed trait copy
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait cornerRadius
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait count extends js.Object
  
  @js.native
  sealed trait crosshair
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait csv
    extends vegaDashLiteLib.buildSrcDataMod.DataFormatType
  
  @js.native
  sealed trait cubehelix
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait `cubehelix-long`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait cume_dist
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait cursor
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait `custom domain scale cannot be unioned with default field-based domain` extends js.Object
  
  @js.native
  sealed trait data extends js.Object
  
  @js.native
  sealed trait date
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait datum extends js.Object
  
  @js.native
  sealed trait day
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
       with vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait default
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait defined
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait dense_rank
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait descending
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgComparatorOrder
  
  @js.native
  sealed trait detail extends js.Object
  
  @js.native
  sealed trait dir
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait discrete extends js.Object
  
  @js.native
  sealed trait distance extends js.Object
  
  @js.native
  sealed trait domain
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait dsv
    extends vegaDashLiteLib.buildSrcDataMod.DataFormatType
  
  @js.native
  sealed trait dx
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait dy
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait `e-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait each
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgLayoutAlign
  
  @js.native
  sealed trait ellipsis
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait encode extends js.Object
  
  @js.native
  sealed trait encoding extends js.Object
  
  @js.native
  sealed trait end
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Anchor
  
  @js.native
  sealed trait endAngle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait entryPadding extends js.Object
  
  @js.native
  sealed trait equirectangular
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait `error-bar` extends js.Object
  
  @js.native
  sealed trait `ew-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait exponent extends js.Object
  
  @js.native
  sealed trait extent extends js.Object
  
  @js.native
  sealed trait facet extends js.Object
  
  @js.native
  sealed trait fill
    extends vegaDashLiteLib.buildSrcChannelMod.ColorChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait fillOpacity
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait filled extends js.Object
  
  @js.native
  sealed trait filter extends js.Object
  
  @js.native
  sealed trait first_value
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait fit
    extends vegaDashLiteLib.buildSrcToplevelpropsMod.AutosizeType
  
  @js.native
  sealed trait flexible extends js.Object
  
  @js.native
  sealed trait flush extends js.Object
  
  @js.native
  sealed trait font
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait fontSize
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait fontStyle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait fontWeight
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait footer
    extends vegaDashLiteLib.buildSrcCompileHeaderMod.HeaderType
  
  @js.native
  sealed trait format extends js.Object
  
  @js.native
  sealed trait formula extends js.Object
  
  @js.native
  sealed trait fraction extends js.Object
  
  @js.native
  sealed trait full extends js.Object
  
  @js.native
  sealed trait functional extends js.Object
  
  @js.native
  sealed trait geojson
    extends vegaDashLiteLib.buildSrcTypeMod.GeoType
  
  @js.native
  sealed trait geopoint extends js.Object
  
  @js.native
  sealed trait geoshape
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait getHeaderProperties extends js.Object
  
  @js.native
  sealed trait global
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionResolution
  
  @js.native
  sealed trait gnomonic
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait grab
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait grabbing
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait gradient
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendType
  
  @js.native
  sealed trait greedy extends js.Object
  
  @js.native
  sealed trait grid
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait gridScale extends js.Object
  
  @js.native
  sealed trait hcl
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait `hcl-long`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait header
    extends vegaDashLiteLib.buildSrcCompileHeaderMod.HeaderType
  
  @js.native
  sealed trait height
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait help
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait horizontal
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Orient
  
  @js.native
  sealed trait hour
    extends vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait hours
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait hoursminutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait hoursminutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait href
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait hsl
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait `hsl-long`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait identifier extends js.Object
  
  @js.native
  sealed trait impute extends js.Object
  
  @js.native
  sealed trait independent
    extends vegaDashLiteLib.buildSrcResolveMod.ResolveMode
  
  @js.native
  sealed trait innerRadius
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait interpolate
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait intersect
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionResolution
  
  @js.native
  sealed trait interval
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionType
  
  @js.native
  sealed trait italic
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.FontStyle
  
  @js.native
  sealed trait json
    extends vegaDashLiteLib.buildSrcDataMod.DataFormatType
  
  @js.native
  sealed trait key
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait lab
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait labelAngle extends js.Object
  
  @js.native
  sealed trait labelBound extends js.Object
  
  @js.native
  sealed trait labelFlush extends js.Object
  
  @js.native
  sealed trait labelOverlap extends js.Object
  
  @js.native
  sealed trait labelPadding extends js.Object
  
  @js.native
  sealed trait labels
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait lag
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait last_value
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait latitude
    extends vegaDashLiteLib.buildSrcTypeMod.GeoType
       with vegaDashLiteLib.buildSrcChannelMod.GeoPositionChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait latitude2
    extends vegaDashLiteLib.buildSrcChannelMod.GeoPositionChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait layer extends js.Object
  
  @js.native
  sealed trait lead
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait left
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.AxisOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.HorizontalAlign
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.TitleOrient
  
  @js.native
  sealed trait legend extends js.Object
  
  @js.native
  sealed trait limit
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait line
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait linear
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
       with vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait `linear-closed`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait lobes extends js.Object
  
  @js.native
  sealed trait log
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait longitude
    extends vegaDashLiteLib.buildSrcTypeMod.GeoType
       with vegaDashLiteLib.buildSrcChannelMod.GeoPositionChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait longitude2
    extends vegaDashLiteLib.buildSrcChannelMod.GeoPositionChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait lookup
    extends vegaDashLiteLib.buildSrcDataMod.DataSourceType
  
  @js.native
  sealed trait ltr
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Dir
  
  @js.native
  sealed trait main
    extends vegaDashLiteLib.buildSrcDataMod.DataSourceType
  
  @js.native
  sealed trait max extends js.Object
  
  @js.native
  sealed trait maxExtent extends js.Object
  
  @js.native
  sealed trait mean extends js.Object
  
  @js.native
  sealed trait median extends js.Object
  
  @js.native
  sealed trait mercator
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait merged
    extends vegaDashLiteLib.buildSrcCompileLayoutsizeComponentMod._LayoutSize
  
  @js.native
  sealed trait mid extends js.Object
  
  @js.native
  sealed trait middle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Anchor
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VerticalAlign
  
  @js.native
  sealed trait milliseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait min extends js.Object
  
  @js.native
  sealed trait `min-max` extends js.Object
  
  @js.native
  sealed trait minExtent extends js.Object
  
  @js.native
  sealed trait minute
    extends vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait minutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait minutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait miter
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeJoin
  
  @js.native
  sealed trait monotone
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait month
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
       with vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait monthdate
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait move
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait multi
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionType
  
  @js.native
  sealed trait `n-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait `ne-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait `nesw-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait nice extends js.Object
  
  @js.native
  sealed trait `no-drop`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait nominal
    extends vegaDashLiteLib.buildSrcTypeMod.BasicType
  
  @js.native
  sealed trait none
    extends vegaDashLiteLib.buildSrcToplevelpropsMod.AutosizeType
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgLayoutAlign
  
  @js.native
  sealed trait normal
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.FontStyle
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.FontWeightString
  
  @js.native
  sealed trait normalize
    extends vegaDashLiteLib.buildSrcStackMod.StackOffset
  
  @js.native
  sealed trait `not-allowed`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait `ns-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait nth_value
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait ntile
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait number extends js.Object
  
  @js.native
  sealed trait `nw-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait `nwse-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait offset extends js.Object
  
  @js.native
  sealed trait opacity
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait order extends js.Object
  
  @js.native
  sealed trait ordinal
    extends vegaDashLiteLib.buildSrcTypeMod.BasicType
       with vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait orient
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait orthographic
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait outerRadius
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait pad
    extends vegaDashLiteLib.buildSrcToplevelpropsMod.AutosizeType
  
  @js.native
  sealed trait padding extends js.Object
  
  @js.native
  sealed trait paddingInner extends js.Object
  
  @js.native
  sealed trait paddingOuter extends js.Object
  
  @js.native
  sealed trait parallel extends js.Object
  
  @js.native
  sealed trait parent extends js.Object
  
  @js.native
  sealed trait parity extends js.Object
  
  @js.native
  sealed trait path
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait percent_rank
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait plain extends js.Object
  
  @js.native
  sealed trait point
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
       with vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait pointer
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait position extends js.Object
  
  @js.native
  sealed trait pow
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait precision extends js.Object
  
  @js.native
  sealed trait progress
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait property extends js.Object
  
  @js.native
  sealed trait quantile extends js.Object
  
  @js.native
  sealed trait quantitative
    extends vegaDashLiteLib.buildSrcTypeMod.BasicType
  
  @js.native
  sealed trait quantize extends js.Object
  
  @js.native
  sealed trait quarter
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait quartermonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait radio extends js.Object
  
  @js.native
  sealed trait radius
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait range extends js.Object
  
  @js.native
  sealed trait `range-step`
    extends vegaDashLiteLib.buildSrcCompileLayoutsizeComponentMod._LayoutSize
  
  @js.native
  sealed trait rangeStep extends js.Object
  
  @js.native
  sealed trait rank
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait ratio extends js.Object
  
  @js.native
  sealed trait raw
    extends vegaDashLiteLib.buildSrcDataMod.DataSourceType
  
  @js.native
  sealed trait rect
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait reduce extends js.Object
  
  @js.native
  sealed trait repeat extends js.Object
  
  @js.native
  sealed trait reverse extends js.Object
  
  @js.native
  sealed trait rgb
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.ScaleInterpolate
  
  @js.native
  sealed trait right
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.AxisOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.HorizontalAlign
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.TitleOrient
  
  @js.native
  sealed trait rotate extends js.Object
  
  @js.native
  sealed trait round
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeCap
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeJoin
  
  @js.native
  sealed trait row
    extends vegaDashLiteLib.buildSrcDataMod.DataSourceType
       with vegaDashLiteLib.buildSrcCompileHeaderMod.HeaderChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
  
  @js.native
  sealed trait `row-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait row_number
    extends vegaDashLiteLib.buildSrcTransformMod.WindowOnlyOp
  
  @js.native
  sealed trait rtl
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Dir
  
  @js.native
  sealed trait rule
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait `s-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait scale extends js.Object
  
  @js.native
  sealed trait scales extends js.Object
  
  @js.native
  sealed trait scheme extends js.Object
  
  @js.native
  sealed trait `se-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait second
    extends vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait seconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
  
  @js.native
  sealed trait secondsmilliseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait select extends js.Object
  
  @js.native
  sealed trait sequential
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait shape
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait shared
    extends vegaDashLiteLib.buildSrcResolveMod.ResolveMode
  
  @js.native
  sealed trait shortTimeLabels extends js.Object
  
  @js.native
  sealed trait single
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionType
  
  @js.native
  sealed trait size
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait spacing extends js.Object
  
  @js.native
  sealed trait sqrt
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait square
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.StrokeCap
  
  @js.native
  sealed trait stack extends js.Object
  
  @js.native
  sealed trait start
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Anchor
  
  @js.native
  sealed trait startAngle
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait step
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `step-after`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait `step-before`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Interpolate
  
  @js.native
  sealed trait stereographic
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait string extends js.Object
  
  @js.native
  sealed trait stroke
    extends vegaDashLiteLib.buildSrcChannelMod.ColorChannel
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeCap
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeDash
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeDashOffset
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeJoin
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeMiterLimit
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeOpacity
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait strokeWidth
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait `sw-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait symbol
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendType
  
  @js.native
  sealed trait temporal
    extends vegaDashLiteLib.buildSrcTypeMod.BasicType
  
  @js.native
  sealed trait tension
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait text
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
       with vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait theta
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait threshold extends js.Object
  
  @js.native
  sealed trait tick
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait tickCount extends js.Object
  
  @js.native
  sealed trait tickSize extends js.Object
  
  @js.native
  sealed trait ticks
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait tilt extends js.Object
  
  @js.native
  sealed trait time
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait title
    extends vegaDashLiteLib.buildSrcAxisMod.AxisPart
  
  @js.native
  sealed trait titleMaxLength extends js.Object
  
  @js.native
  sealed trait titlePadding extends js.Object
  
  @js.native
  sealed trait tooltip
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait top
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.AxisOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.TitleOrient
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VerticalAlign
  
  @js.native
  sealed trait `top-left`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
  
  @js.native
  sealed trait `top-right`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.LegendOrient
  
  @js.native
  sealed trait topojson
    extends vegaDashLiteLib.buildSrcDataMod.DataFormatType
  
  @js.native
  sealed trait trail
    extends vegaDashLiteLib.buildSrcMarkMod.Mark
  
  @js.native
  sealed trait transparent extends js.Object
  
  @js.native
  sealed trait transverseMercator
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgProjectionType
  
  @js.native
  sealed trait tsv
    extends vegaDashLiteLib.buildSrcDataMod.DataFormatType
  
  @js.native
  sealed trait `type` extends js.Object
  
  @js.native
  sealed trait unaggregated
    extends vegaDashLiteLib.buildSrcScaleMod._Domain
  
  @js.native
  sealed trait union
    extends vegaDashLiteLib.buildSrcSelectionMod.SelectionResolution
  
  @js.native
  sealed trait unit extends js.Object
  
  @js.native
  sealed trait url
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait utc
    extends vegaDashLiteLib.buildSrcScaleMod.ScaleType
  
  @js.native
  sealed trait utcdate
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcday
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utchours
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utchoursminutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utchoursminutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcmilliseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcminutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcminutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcmonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcmonthdate
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcquarter
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcquartermonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcsecondsmilliseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyear
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcSingleTimeUnit
  
  @js.native
  sealed trait utcyearmonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearmonthdate
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearmonthdatehours
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearmonthdatehoursminutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearmonthdatehoursminutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearquarter
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait utcyearquartermonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.UtcMultiTimeUnit
  
  @js.native
  sealed trait value extends js.Object
  
  @js.native
  sealed trait values extends js.Object
  
  @js.native
  sealed trait verbal extends js.Object
  
  @js.native
  sealed trait vertical
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Orient
  
  @js.native
  sealed trait `vertical-text`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait visual extends js.Object
  
  @js.native
  sealed trait `w-resize`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait wait
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait week
    extends vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait width
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait window extends js.Object
  
  @js.native
  sealed trait x
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait x2
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait xc
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait y
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait y2
    extends vegaDashLiteLib.buildSrcChannelMod.SingleDefChannel
       with vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait yc
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.VgEncodeChannel
  
  @js.native
  sealed trait year
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalSingleTimeUnit
       with vegaDashLiteLib.buildSrcScaleMod.NiceTime
  
  @js.native
  sealed trait yearmonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearmonthdate
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearmonthdatehours
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearmonthdatehoursminutes
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearmonthdatehoursminutesseconds
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearquarter
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait yearquartermonth
    extends vegaDashLiteLib.buildSrcTimeunitMod.LocalMultiTimeUnit
  
  @js.native
  sealed trait zero
    extends vegaDashLiteLib.buildSrcStackMod.StackOffset
  
  @js.native
  sealed trait zindex extends js.Object
  
  @js.native
  sealed trait `zoom-in`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @js.native
  sealed trait `zoom-out`
    extends vegaDashLiteLib.buildSrcVegaDotSchemaMod.Cursor
  
  @scala.inline
  def `Autosize BACKSLASHfitBACKSLASH only works for single views and layered viewsDOT`: `Autosize BACKSLASHfitBACKSLASH only works for single views and layered viewsDOT` = "Autosize \"fit\" only works for single views and layered views.".asInstanceOf[`Autosize BACKSLASHfitBACKSLASH only works for single views and layered viewsDOT`]
  @scala.inline
  def `Axes cannot be shared in concatenated viewsDOT`: `Axes cannot be shared in concatenated viewsDOT` = "Axes cannot be shared in concatenated views.".asInstanceOf[`Axes cannot be shared in concatenated viewsDOT`]
  @scala.inline
  def `Axes cannot be shared in repeated viewsDOT`: `Axes cannot be shared in repeated viewsDOT` = "Axes cannot be shared in repeated views.".asInstanceOf[`Axes cannot be shared in repeated viewsDOT`]
  @scala.inline
  def AxisResolveMap: AxisResolveMap = "AxisResolveMap".asInstanceOf[AxisResolveMap]
  @scala.inline
  def `Bar mark should not be used with point scale when rangeStep is nullDOT Please use band scale insteadDOT`: `Bar mark should not be used with point scale when rangeStep is nullDOT Please use band scale insteadDOT` = "Bar mark should not be used with point scale when rangeStep is null. Please use band scale instead.".asInstanceOf[`Bar mark should not be used with point scale when rangeStep is nullDOT Please use band scale insteadDOT`]
  @scala.inline
  def `Cannot use a fixed value of BACKSLASHrangeStepBACKSLASH when BACKSLASHautosizeBACKSLASH is BACKSLASHfitBACKSLASHDOT`: `Cannot use a fixed value of BACKSLASHrangeStepBACKSLASH when BACKSLASHautosizeBACKSLASH is BACKSLASHfitBACKSLASHDOT` = "Cannot use a fixed value of \"rangeStep\" when \"autosize\" is \"fit\".".asInstanceOf[`Cannot use a fixed value of BACKSLASHrangeStepBACKSLASH when BACKSLASHautosizeBACKSLASH is BACKSLASHfitBACKSLASHDOT`]
  @scala.inline
  def `Domains that should be unioned has conflicting sort propertiesDOT Sort will be set to trueDOT`: `Domains that should be unioned has conflicting sort propertiesDOT Sort will be set to trueDOT` = "Domains that should be unioned has conflicting sort properties. Sort will be set to true.".asInstanceOf[`Domains that should be unioned has conflicting sort propertiesDOT Sort will be set to trueDOT`]
  @scala.inline
  def Empty: Empty = "".asInstanceOf[Empty]
  @scala.inline
  def Flag: Flag = "Flag".asInstanceOf[Flag]
  @scala.inline
  def GuideEncodingEntry: GuideEncodingEntry = "GuideEncodingEntry".asInstanceOf[GuideEncodingEntry]
  @scala.inline
  def `If BACKSLASHfromDOTfieldsBACKSLASH is not specified, BACKSLASHasBACKSLASH has to be a string that specifies the key to be used for the data from the secondary sourceDOT`: `If BACKSLASHfromDOTfieldsBACKSLASH is not specified, BACKSLASHasBACKSLASH has to be a string that specifies the key to be used for the data from the secondary sourceDOT` = "If \"from.fields\" is not specified, \"as\" has to be a string that specifies the key to be used for the data from the secondary source.".asInstanceOf[`If BACKSLASHfromDOTfieldsBACKSLASH is not specified, BACKSLASHasBACKSLASH has to be a string that specifies the key to be used for the data from the secondary sourceDOT`]
  @scala.inline
  def `Invalid channel for axisDOT`: `Invalid channel for axisDOT` = "Invalid channel for axis.".asInstanceOf[`Invalid channel for axisDOT`]
  @scala.inline
  def `Invalid spec`: `Invalid spec` = "Invalid spec".asInstanceOf[`Invalid spec`]
  @scala.inline
  def LegendComponentIndex: LegendComponentIndex = "LegendComponentIndex".asInstanceOf[LegendComponentIndex]
  @scala.inline
  def LegendIndex: LegendIndex = "LegendIndex".asInstanceOf[LegendIndex]
  @scala.inline
  def LegendResolveMap: LegendResolveMap = "LegendResolveMap".asInstanceOf[LegendResolveMap]
  @scala.inline
  def `Line marks cannot encode size with a non-groupby fieldDOT You may want to use trail marks insteadDOT`: `Line marks cannot encode size with a non-groupby fieldDOT You may want to use trail marks insteadDOT` = "Line marks cannot encode size with a non-groupby field. You may want to use trail marks instead.".asInstanceOf[`Line marks cannot encode size with a non-groupby fieldDOT You may want to use trail marks insteadDOT`]
  @scala.inline
  def ModelWithField: ModelWithField = "ModelWithField".asInstanceOf[ModelWithField]
  @scala.inline
  def `Scale bindings are currently only supported for scales with unbinned, continuous domainsDOT`: `Scale bindings are currently only supported for scales with unbinned, continuous domainsDOT` = "Scale bindings are currently only supported for scales with unbinned, continuous domains.".asInstanceOf[`Scale bindings are currently only supported for scales with unbinned, continuous domainsDOT`]
  @scala.inline
  def ScaleComponentIndex: ScaleComponentIndex = "ScaleComponentIndex".asInstanceOf[ScaleComponentIndex]
  @scala.inline
  def ScaleIndex: ScaleIndex = "ScaleIndex".asInstanceOf[ScaleIndex]
  @scala.inline
  def ScaleResolveMap: ScaleResolveMap = "ScaleResolveMap".asInstanceOf[ScaleResolveMap]
  @scala.inline
  def SupportedMark: SupportedMark = "SupportedMark".asInstanceOf[SupportedMark]
  @scala.inline
  def `Unable to merge domains`: `Unable to merge domains` = "Unable to merge domains".asInstanceOf[`Unable to merge domains`]
  @scala.inline
  def VgEncodeEntry: VgEncodeEntry = "VgEncodeEntry".asInstanceOf[VgEncodeEntry]
  @scala.inline
  def _modify: _modify = "_modify".asInstanceOf[_modify]
  @scala.inline
  def _selection_domain_ : _selection_domain_ = "_selection_domain_".asInstanceOf[_selection_domain_]
  @scala.inline
  def _store: _store = "_store".asInstanceOf[_store]
  @scala.inline
  def _tuple: _tuple = "_tuple".asInstanceOf[_tuple]
  @scala.inline
  def _vgsid_ : _vgsid_ = "_vgsid_".asInstanceOf[_vgsid_]
  @scala.inline
  def aggregate: aggregate = "aggregate".asInstanceOf[aggregate]
  @scala.inline
  def albers: albers = "albers".asInstanceOf[albers]
  @scala.inline
  def albersUsa: albersUsa = "albersUsa".asInstanceOf[albersUsa]
  @scala.inline
  def alias: alias = "alias".asInstanceOf[alias]
  @scala.inline
  def align: align = "align".asInstanceOf[align]
  @scala.inline
  def all: all = "all".asInstanceOf[all]
  @scala.inline
  def `all-scroll`: `all-scroll` = "all-scroll".asInstanceOf[`all-scroll`]
  @scala.inline
  def angle: angle = "angle".asInstanceOf[angle]
  @scala.inline
  def area: area = "area".asInstanceOf[area]
  @scala.inline
  def ascending: ascending = "ascending".asInstanceOf[ascending]
  @scala.inline
  def auto: auto = "auto".asInstanceOf[auto]
  @scala.inline
  def axis: axis = "axis".asInstanceOf[axis]
  @scala.inline
  def azimuthalEqualArea: azimuthalEqualArea = "azimuthalEqualArea".asInstanceOf[azimuthalEqualArea]
  @scala.inline
  def azimuthalEquidistant: azimuthalEquidistant = "azimuthalEquidistant".asInstanceOf[azimuthalEquidistant]
  @scala.inline
  def band: band = "band".asInstanceOf[band]
  @scala.inline
  def bar: bar = "bar".asInstanceOf[bar]
  @scala.inline
  def base: base = "base".asInstanceOf[base]
  @scala.inline
  def baseline: baseline = "baseline".asInstanceOf[baseline]
  @scala.inline
  def basis: basis = "basis".asInstanceOf[basis]
  @scala.inline
  def `basis-closed`: `basis-closed` = "basis-closed".asInstanceOf[`basis-closed`]
  @scala.inline
  def `basis-open`: `basis-open` = "basis-open".asInstanceOf[`basis-open`]
  @scala.inline
  def bevel: bevel = "bevel".asInstanceOf[bevel]
  @scala.inline
  def bin: bin = "bin".asInstanceOf[bin]
  @scala.inline
  def `bin-linear`: `bin-linear` = "bin-linear".asInstanceOf[`bin-linear`]
  @scala.inline
  def `bin-ordinal`: `bin-ordinal` = "bin-ordinal".asInstanceOf[`bin-ordinal`]
  @scala.inline
  def bold: bold = "bold".asInstanceOf[bold]
  @scala.inline
  def boolean: boolean = "boolean".asInstanceOf[boolean]
  @scala.inline
  def bottom: bottom = "bottom".asInstanceOf[bottom]
  @scala.inline
  def `bottom-left`: `bottom-left` = "bottom-left".asInstanceOf[`bottom-left`]
  @scala.inline
  def `bottom-right`: `bottom-right` = "bottom-right".asInstanceOf[`bottom-right`]
  @scala.inline
  def box: box = "box".asInstanceOf[box]
  @scala.inline
  def `box-plot`: `box-plot` = "box-plot".asInstanceOf[`box-plot`]
  @scala.inline
  def boxMid: boxMid = "boxMid".asInstanceOf[boxMid]
  @scala.inline
  def boxWhisker: boxWhisker = "boxWhisker".asInstanceOf[boxWhisker]
  @scala.inline
  def bundle: bundle = "bundle".asInstanceOf[bundle]
  @scala.inline
  def butt: butt = "butt".asInstanceOf[butt]
  @scala.inline
  def cardinal: cardinal = "cardinal".asInstanceOf[cardinal]
  @scala.inline
  def `cardinal-closed`: `cardinal-closed` = "cardinal-closed".asInstanceOf[`cardinal-closed`]
  @scala.inline
  def `cardinal-open`: `cardinal-open` = "cardinal-open".asInstanceOf[`cardinal-open`]
  @scala.inline
  def cell: cell = "cell".asInstanceOf[cell]
  @scala.inline
  def center: center = "center".asInstanceOf[center]
  @scala.inline
  def checkbox: checkbox = "checkbox".asInstanceOf[checkbox]
  @scala.inline
  def circle: circle = "circle".asInstanceOf[circle]
  @scala.inline
  def clamp: clamp = "clamp".asInstanceOf[clamp]
  @scala.inline
  def clip: clip = "clip".asInstanceOf[clip]
  @scala.inline
  def clipAngle: clipAngle = "clipAngle".asInstanceOf[clipAngle]
  @scala.inline
  def clipExtent: clipExtent = "clipExtent".asInstanceOf[clipExtent]
  @scala.inline
  def coefficient: coefficient = "coefficient".asInstanceOf[coefficient]
  @scala.inline
  def `col-resize`: `col-resize` = "col-resize".asInstanceOf[`col-resize`]
  @scala.inline
  def collect: collect = "collect".asInstanceOf[collect]
  @scala.inline
  def color: color = "color".asInstanceOf[color]
  @scala.inline
  def column: column = "column".asInstanceOf[column]
  @scala.inline
  def concat: concat = "concat".asInstanceOf[concat]
  @scala.inline
  def conicConformal: conicConformal = "conicConformal".asInstanceOf[conicConformal]
  @scala.inline
  def conicEqualArea: conicEqualArea = "conicEqualArea".asInstanceOf[conicEqualArea]
  @scala.inline
  def conicEquidistant: conicEquidistant = "conicEquidistant".asInstanceOf[conicEquidistant]
  @scala.inline
  def content: content = "content".asInstanceOf[content]
  @scala.inline
  def `context-menu`: `context-menu` = "context-menu".asInstanceOf[`context-menu`]
  @scala.inline
  def continuous: continuous = "continuous".asInstanceOf[continuous]
  @scala.inline
  def copy: copy = "copy".asInstanceOf[copy]
  @scala.inline
  def cornerRadius: cornerRadius = "cornerRadius".asInstanceOf[cornerRadius]
  @scala.inline
  def count: count = "count".asInstanceOf[count]
  @scala.inline
  def crosshair: crosshair = "crosshair".asInstanceOf[crosshair]
  @scala.inline
  def csv: csv = "csv".asInstanceOf[csv]
  @scala.inline
  def cubehelix: cubehelix = "cubehelix".asInstanceOf[cubehelix]
  @scala.inline
  def `cubehelix-long`: `cubehelix-long` = "cubehelix-long".asInstanceOf[`cubehelix-long`]
  @scala.inline
  def cume_dist: cume_dist = "cume_dist".asInstanceOf[cume_dist]
  @scala.inline
  def cursor: cursor = "cursor".asInstanceOf[cursor]
  @scala.inline
  def `custom domain scale cannot be unioned with default field-based domain`: `custom domain scale cannot be unioned with default field-based domain` = "custom domain scale cannot be unioned with default field-based domain".asInstanceOf[`custom domain scale cannot be unioned with default field-based domain`]
  @scala.inline
  def data: data = "data".asInstanceOf[data]
  @scala.inline
  def date: date = "date".asInstanceOf[date]
  @scala.inline
  def datum: datum = "datum".asInstanceOf[datum]
  @scala.inline
  def day: day = "day".asInstanceOf[day]
  @scala.inline
  def default: default = "default".asInstanceOf[default]
  @scala.inline
  def defined: defined = "defined".asInstanceOf[defined]
  @scala.inline
  def dense_rank: dense_rank = "dense_rank".asInstanceOf[dense_rank]
  @scala.inline
  def descending: descending = "descending".asInstanceOf[descending]
  @scala.inline
  def detail: detail = "detail".asInstanceOf[detail]
  @scala.inline
  def dir: dir = "dir".asInstanceOf[dir]
  @scala.inline
  def discrete: discrete = "discrete".asInstanceOf[discrete]
  @scala.inline
  def distance: distance = "distance".asInstanceOf[distance]
  @scala.inline
  def domain: domain = "domain".asInstanceOf[domain]
  @scala.inline
  def dsv: dsv = "dsv".asInstanceOf[dsv]
  @scala.inline
  def dx: dx = "dx".asInstanceOf[dx]
  @scala.inline
  def dy: dy = "dy".asInstanceOf[dy]
  @scala.inline
  def `e-resize`: `e-resize` = "e-resize".asInstanceOf[`e-resize`]
  @scala.inline
  def each: each = "each".asInstanceOf[each]
  @scala.inline
  def ellipsis: ellipsis = "ellipsis".asInstanceOf[ellipsis]
  @scala.inline
  def encode: encode = "encode".asInstanceOf[encode]
  @scala.inline
  def encoding: encoding = "encoding".asInstanceOf[encoding]
  @scala.inline
  def end: end = "end".asInstanceOf[end]
  @scala.inline
  def endAngle: endAngle = "endAngle".asInstanceOf[endAngle]
  @scala.inline
  def entryPadding: entryPadding = "entryPadding".asInstanceOf[entryPadding]
  @scala.inline
  def equirectangular: equirectangular = "equirectangular".asInstanceOf[equirectangular]
  @scala.inline
  def `error-bar`: `error-bar` = "error-bar".asInstanceOf[`error-bar`]
  @scala.inline
  def `ew-resize`: `ew-resize` = "ew-resize".asInstanceOf[`ew-resize`]
  @scala.inline
  def exponent: exponent = "exponent".asInstanceOf[exponent]
  @scala.inline
  def extent: extent = "extent".asInstanceOf[extent]
  @scala.inline
  def facet: facet = "facet".asInstanceOf[facet]
  @scala.inline
  def fill: fill = "fill".asInstanceOf[fill]
  @scala.inline
  def fillOpacity: fillOpacity = "fillOpacity".asInstanceOf[fillOpacity]
  @scala.inline
  def filled: filled = "filled".asInstanceOf[filled]
  @scala.inline
  def filter: filter = "filter".asInstanceOf[filter]
  @scala.inline
  def first_value: first_value = "first_value".asInstanceOf[first_value]
  @scala.inline
  def fit: fit = "fit".asInstanceOf[fit]
  @scala.inline
  def flexible: flexible = "flexible".asInstanceOf[flexible]
  @scala.inline
  def flush: flush = "flush".asInstanceOf[flush]
  @scala.inline
  def font: font = "font".asInstanceOf[font]
  @scala.inline
  def fontSize: fontSize = "fontSize".asInstanceOf[fontSize]
  @scala.inline
  def fontStyle: fontStyle = "fontStyle".asInstanceOf[fontStyle]
  @scala.inline
  def fontWeight: fontWeight = "fontWeight".asInstanceOf[fontWeight]
  @scala.inline
  def footer: footer = "footer".asInstanceOf[footer]
  @scala.inline
  def format: format = "format".asInstanceOf[format]
  @scala.inline
  def formula: formula = "formula".asInstanceOf[formula]
  @scala.inline
  def fraction: fraction = "fraction".asInstanceOf[fraction]
  @scala.inline
  def full: full = "full".asInstanceOf[full]
  @scala.inline
  def functional: functional = "functional".asInstanceOf[functional]
  @scala.inline
  def geojson: geojson = "geojson".asInstanceOf[geojson]
  @scala.inline
  def geopoint: geopoint = "geopoint".asInstanceOf[geopoint]
  @scala.inline
  def geoshape: geoshape = "geoshape".asInstanceOf[geoshape]
  @scala.inline
  def getHeaderProperties: getHeaderProperties = "getHeaderProperties".asInstanceOf[getHeaderProperties]
  @scala.inline
  def global: global = "global".asInstanceOf[global]
  @scala.inline
  def gnomonic: gnomonic = "gnomonic".asInstanceOf[gnomonic]
  @scala.inline
  def grab: grab = "grab".asInstanceOf[grab]
  @scala.inline
  def grabbing: grabbing = "grabbing".asInstanceOf[grabbing]
  @scala.inline
  def gradient: gradient = "gradient".asInstanceOf[gradient]
  @scala.inline
  def greedy: greedy = "greedy".asInstanceOf[greedy]
  @scala.inline
  def grid: grid = "grid".asInstanceOf[grid]
  @scala.inline
  def gridScale: gridScale = "gridScale".asInstanceOf[gridScale]
  @scala.inline
  def hcl: hcl = "hcl".asInstanceOf[hcl]
  @scala.inline
  def `hcl-long`: `hcl-long` = "hcl-long".asInstanceOf[`hcl-long`]
  @scala.inline
  def header: header = "header".asInstanceOf[header]
  @scala.inline
  def height: height = "height".asInstanceOf[height]
  @scala.inline
  def help: help = "help".asInstanceOf[help]
  @scala.inline
  def horizontal: horizontal = "horizontal".asInstanceOf[horizontal]
  @scala.inline
  def hour: hour = "hour".asInstanceOf[hour]
  @scala.inline
  def hours: hours = "hours".asInstanceOf[hours]
  @scala.inline
  def hoursminutes: hoursminutes = "hoursminutes".asInstanceOf[hoursminutes]
  @scala.inline
  def hoursminutesseconds: hoursminutesseconds = "hoursminutesseconds".asInstanceOf[hoursminutesseconds]
  @scala.inline
  def href: href = "href".asInstanceOf[href]
  @scala.inline
  def hsl: hsl = "hsl".asInstanceOf[hsl]
  @scala.inline
  def `hsl-long`: `hsl-long` = "hsl-long".asInstanceOf[`hsl-long`]
  @scala.inline
  def identifier: identifier = "identifier".asInstanceOf[identifier]
  @scala.inline
  def impute: impute = "impute".asInstanceOf[impute]
  @scala.inline
  def independent: independent = "independent".asInstanceOf[independent]
  @scala.inline
  def innerRadius: innerRadius = "innerRadius".asInstanceOf[innerRadius]
  @scala.inline
  def interpolate: interpolate = "interpolate".asInstanceOf[interpolate]
  @scala.inline
  def intersect: intersect = "intersect".asInstanceOf[intersect]
  @scala.inline
  def interval: interval = "interval".asInstanceOf[interval]
  @scala.inline
  def italic: italic = "italic".asInstanceOf[italic]
  @scala.inline
  def json: json = "json".asInstanceOf[json]
  @scala.inline
  def key: key = "key".asInstanceOf[key]
  @scala.inline
  def lab: lab = "lab".asInstanceOf[lab]
  @scala.inline
  def labelAngle: labelAngle = "labelAngle".asInstanceOf[labelAngle]
  @scala.inline
  def labelBound: labelBound = "labelBound".asInstanceOf[labelBound]
  @scala.inline
  def labelFlush: labelFlush = "labelFlush".asInstanceOf[labelFlush]
  @scala.inline
  def labelOverlap: labelOverlap = "labelOverlap".asInstanceOf[labelOverlap]
  @scala.inline
  def labelPadding: labelPadding = "labelPadding".asInstanceOf[labelPadding]
  @scala.inline
  def labels: labels = "labels".asInstanceOf[labels]
  @scala.inline
  def lag: lag = "lag".asInstanceOf[lag]
  @scala.inline
  def last_value: last_value = "last_value".asInstanceOf[last_value]
  @scala.inline
  def latitude: latitude = "latitude".asInstanceOf[latitude]
  @scala.inline
  def latitude2: latitude2 = "latitude2".asInstanceOf[latitude2]
  @scala.inline
  def layer: layer = "layer".asInstanceOf[layer]
  @scala.inline
  def lead: lead = "lead".asInstanceOf[lead]
  @scala.inline
  def left: left = "left".asInstanceOf[left]
  @scala.inline
  def legend: legend = "legend".asInstanceOf[legend]
  @scala.inline
  def limit: limit = "limit".asInstanceOf[limit]
  @scala.inline
  def line: line = "line".asInstanceOf[line]
  @scala.inline
  def linear: linear = "linear".asInstanceOf[linear]
  @scala.inline
  def `linear-closed`: `linear-closed` = "linear-closed".asInstanceOf[`linear-closed`]
  @scala.inline
  def lobes: lobes = "lobes".asInstanceOf[lobes]
  @scala.inline
  def log: log = "log".asInstanceOf[log]
  @scala.inline
  def longitude: longitude = "longitude".asInstanceOf[longitude]
  @scala.inline
  def longitude2: longitude2 = "longitude2".asInstanceOf[longitude2]
  @scala.inline
  def lookup: lookup = "lookup".asInstanceOf[lookup]
  @scala.inline
  def ltr: ltr = "ltr".asInstanceOf[ltr]
  @scala.inline
  def main: main = "main".asInstanceOf[main]
  @scala.inline
  def max: max = "max".asInstanceOf[max]
  @scala.inline
  def maxExtent: maxExtent = "maxExtent".asInstanceOf[maxExtent]
  @scala.inline
  def mean: mean = "mean".asInstanceOf[mean]
  @scala.inline
  def median: median = "median".asInstanceOf[median]
  @scala.inline
  def mercator: mercator = "mercator".asInstanceOf[mercator]
  @scala.inline
  def merged: merged = "merged".asInstanceOf[merged]
  @scala.inline
  def mid: mid = "mid".asInstanceOf[mid]
  @scala.inline
  def middle: middle = "middle".asInstanceOf[middle]
  @scala.inline
  def milliseconds: milliseconds = "milliseconds".asInstanceOf[milliseconds]
  @scala.inline
  def min: min = "min".asInstanceOf[min]
  @scala.inline
  def `min-max`: `min-max` = "min-max".asInstanceOf[`min-max`]
  @scala.inline
  def minExtent: minExtent = "minExtent".asInstanceOf[minExtent]
  @scala.inline
  def minute: minute = "minute".asInstanceOf[minute]
  @scala.inline
  def minutes: minutes = "minutes".asInstanceOf[minutes]
  @scala.inline
  def minutesseconds: minutesseconds = "minutesseconds".asInstanceOf[minutesseconds]
  @scala.inline
  def miter: miter = "miter".asInstanceOf[miter]
  @scala.inline
  def monotone: monotone = "monotone".asInstanceOf[monotone]
  @scala.inline
  def month: month = "month".asInstanceOf[month]
  @scala.inline
  def monthdate: monthdate = "monthdate".asInstanceOf[monthdate]
  @scala.inline
  def move: move = "move".asInstanceOf[move]
  @scala.inline
  def multi: multi = "multi".asInstanceOf[multi]
  @scala.inline
  def `n-resize`: `n-resize` = "n-resize".asInstanceOf[`n-resize`]
  @scala.inline
  def `ne-resize`: `ne-resize` = "ne-resize".asInstanceOf[`ne-resize`]
  @scala.inline
  def `nesw-resize`: `nesw-resize` = "nesw-resize".asInstanceOf[`nesw-resize`]
  @scala.inline
  def nice: nice = "nice".asInstanceOf[nice]
  @scala.inline
  def `no-drop`: `no-drop` = "no-drop".asInstanceOf[`no-drop`]
  @scala.inline
  def nominal: nominal = "nominal".asInstanceOf[nominal]
  @scala.inline
  def none: none = "none".asInstanceOf[none]
  @scala.inline
  def normal: normal = "normal".asInstanceOf[normal]
  @scala.inline
  def normalize: normalize = "normalize".asInstanceOf[normalize]
  @scala.inline
  def `not-allowed`: `not-allowed` = "not-allowed".asInstanceOf[`not-allowed`]
  @scala.inline
  def `ns-resize`: `ns-resize` = "ns-resize".asInstanceOf[`ns-resize`]
  @scala.inline
  def nth_value: nth_value = "nth_value".asInstanceOf[nth_value]
  @scala.inline
  def ntile: ntile = "ntile".asInstanceOf[ntile]
  @scala.inline
  def number: number = "number".asInstanceOf[number]
  @scala.inline
  def `nw-resize`: `nw-resize` = "nw-resize".asInstanceOf[`nw-resize`]
  @scala.inline
  def `nwse-resize`: `nwse-resize` = "nwse-resize".asInstanceOf[`nwse-resize`]
  @scala.inline
  def offset: offset = "offset".asInstanceOf[offset]
  @scala.inline
  def opacity: opacity = "opacity".asInstanceOf[opacity]
  @scala.inline
  def order: order = "order".asInstanceOf[order]
  @scala.inline
  def ordinal: ordinal = "ordinal".asInstanceOf[ordinal]
  @scala.inline
  def orient: orient = "orient".asInstanceOf[orient]
  @scala.inline
  def orthographic: orthographic = "orthographic".asInstanceOf[orthographic]
  @scala.inline
  def outerRadius: outerRadius = "outerRadius".asInstanceOf[outerRadius]
  @scala.inline
  def pad: pad = "pad".asInstanceOf[pad]
  @scala.inline
  def padding: padding = "padding".asInstanceOf[padding]
  @scala.inline
  def paddingInner: paddingInner = "paddingInner".asInstanceOf[paddingInner]
  @scala.inline
  def paddingOuter: paddingOuter = "paddingOuter".asInstanceOf[paddingOuter]
  @scala.inline
  def parallel: parallel = "parallel".asInstanceOf[parallel]
  @scala.inline
  def parent: parent = "parent".asInstanceOf[parent]
  @scala.inline
  def parity: parity = "parity".asInstanceOf[parity]
  @scala.inline
  def path: path = "path".asInstanceOf[path]
  @scala.inline
  def percent_rank: percent_rank = "percent_rank".asInstanceOf[percent_rank]
  @scala.inline
  def plain: plain = "plain".asInstanceOf[plain]
  @scala.inline
  def point: point = "point".asInstanceOf[point]
  @scala.inline
  def pointer: pointer = "pointer".asInstanceOf[pointer]
  @scala.inline
  def position: position = "position".asInstanceOf[position]
  @scala.inline
  def pow: pow = "pow".asInstanceOf[pow]
  @scala.inline
  def precision: precision = "precision".asInstanceOf[precision]
  @scala.inline
  def progress: progress = "progress".asInstanceOf[progress]
  @scala.inline
  def property: property = "property".asInstanceOf[property]
  @scala.inline
  def quantile: quantile = "quantile".asInstanceOf[quantile]
  @scala.inline
  def quantitative: quantitative = "quantitative".asInstanceOf[quantitative]
  @scala.inline
  def quantize: quantize = "quantize".asInstanceOf[quantize]
  @scala.inline
  def quarter: quarter = "quarter".asInstanceOf[quarter]
  @scala.inline
  def quartermonth: quartermonth = "quartermonth".asInstanceOf[quartermonth]
  @scala.inline
  def radio: radio = "radio".asInstanceOf[radio]
  @scala.inline
  def radius: radius = "radius".asInstanceOf[radius]
  @scala.inline
  def range: range = "range".asInstanceOf[range]
  @scala.inline
  def `range-step`: `range-step` = "range-step".asInstanceOf[`range-step`]
  @scala.inline
  def rangeStep: rangeStep = "rangeStep".asInstanceOf[rangeStep]
  @scala.inline
  def rank: rank = "rank".asInstanceOf[rank]
  @scala.inline
  def ratio: ratio = "ratio".asInstanceOf[ratio]
  @scala.inline
  def raw: raw = "raw".asInstanceOf[raw]
  @scala.inline
  def rect: rect = "rect".asInstanceOf[rect]
  @scala.inline
  def reduce: reduce = "reduce".asInstanceOf[reduce]
  @scala.inline
  def repeat: repeat = "repeat".asInstanceOf[repeat]
  @scala.inline
  def reverse: reverse = "reverse".asInstanceOf[reverse]
  @scala.inline
  def rgb: rgb = "rgb".asInstanceOf[rgb]
  @scala.inline
  def right: right = "right".asInstanceOf[right]
  @scala.inline
  def rotate: rotate = "rotate".asInstanceOf[rotate]
  @scala.inline
  def round: round = "round".asInstanceOf[round]
  @scala.inline
  def row: row = "row".asInstanceOf[row]
  @scala.inline
  def `row-resize`: `row-resize` = "row-resize".asInstanceOf[`row-resize`]
  @scala.inline
  def row_number: row_number = "row_number".asInstanceOf[row_number]
  @scala.inline
  def rtl: rtl = "rtl".asInstanceOf[rtl]
  @scala.inline
  def rule: rule = "rule".asInstanceOf[rule]
  @scala.inline
  def `s-resize`: `s-resize` = "s-resize".asInstanceOf[`s-resize`]
  @scala.inline
  def scale: scale = "scale".asInstanceOf[scale]
  @scala.inline
  def scales: scales = "scales".asInstanceOf[scales]
  @scala.inline
  def scheme: scheme = "scheme".asInstanceOf[scheme]
  @scala.inline
  def `se-resize`: `se-resize` = "se-resize".asInstanceOf[`se-resize`]
  @scala.inline
  def second: second = "second".asInstanceOf[second]
  @scala.inline
  def seconds: seconds = "seconds".asInstanceOf[seconds]
  @scala.inline
  def secondsmilliseconds: secondsmilliseconds = "secondsmilliseconds".asInstanceOf[secondsmilliseconds]
  @scala.inline
  def select: select = "select".asInstanceOf[select]
  @scala.inline
  def sequential: sequential = "sequential".asInstanceOf[sequential]
  @scala.inline
  def shape: shape = "shape".asInstanceOf[shape]
  @scala.inline
  def shared: shared = "shared".asInstanceOf[shared]
  @scala.inline
  def shortTimeLabels: shortTimeLabels = "shortTimeLabels".asInstanceOf[shortTimeLabels]
  @scala.inline
  def single: single = "single".asInstanceOf[single]
  @scala.inline
  def size: size = "size".asInstanceOf[size]
  @scala.inline
  def spacing: spacing = "spacing".asInstanceOf[spacing]
  @scala.inline
  def sqrt: sqrt = "sqrt".asInstanceOf[sqrt]
  @scala.inline
  def square: square = "square".asInstanceOf[square]
  @scala.inline
  def stack: stack = "stack".asInstanceOf[stack]
  @scala.inline
  def start: start = "start".asInstanceOf[start]
  @scala.inline
  def startAngle: startAngle = "startAngle".asInstanceOf[startAngle]
  @scala.inline
  def step: step = "step".asInstanceOf[step]
  @scala.inline
  def `step-after`: `step-after` = "step-after".asInstanceOf[`step-after`]
  @scala.inline
  def `step-before`: `step-before` = "step-before".asInstanceOf[`step-before`]
  @scala.inline
  def stereographic: stereographic = "stereographic".asInstanceOf[stereographic]
  @scala.inline
  def string: string = "string".asInstanceOf[string]
  @scala.inline
  def stroke: stroke = "stroke".asInstanceOf[stroke]
  @scala.inline
  def strokeCap: strokeCap = "strokeCap".asInstanceOf[strokeCap]
  @scala.inline
  def strokeDash: strokeDash = "strokeDash".asInstanceOf[strokeDash]
  @scala.inline
  def strokeDashOffset: strokeDashOffset = "strokeDashOffset".asInstanceOf[strokeDashOffset]
  @scala.inline
  def strokeJoin: strokeJoin = "strokeJoin".asInstanceOf[strokeJoin]
  @scala.inline
  def strokeMiterLimit: strokeMiterLimit = "strokeMiterLimit".asInstanceOf[strokeMiterLimit]
  @scala.inline
  def strokeOpacity: strokeOpacity = "strokeOpacity".asInstanceOf[strokeOpacity]
  @scala.inline
  def strokeWidth: strokeWidth = "strokeWidth".asInstanceOf[strokeWidth]
  @scala.inline
  def `sw-resize`: `sw-resize` = "sw-resize".asInstanceOf[`sw-resize`]
  @scala.inline
  def symbol: symbol = "symbol".asInstanceOf[symbol]
  @scala.inline
  def temporal: temporal = "temporal".asInstanceOf[temporal]
  @scala.inline
  def tension: tension = "tension".asInstanceOf[tension]
  @scala.inline
  def text: text = "text".asInstanceOf[text]
  @scala.inline
  def theta: theta = "theta".asInstanceOf[theta]
  @scala.inline
  def threshold: threshold = "threshold".asInstanceOf[threshold]
  @scala.inline
  def tick: tick = "tick".asInstanceOf[tick]
  @scala.inline
  def tickCount: tickCount = "tickCount".asInstanceOf[tickCount]
  @scala.inline
  def tickSize: tickSize = "tickSize".asInstanceOf[tickSize]
  @scala.inline
  def ticks: ticks = "ticks".asInstanceOf[ticks]
  @scala.inline
  def tilt: tilt = "tilt".asInstanceOf[tilt]
  @scala.inline
  def time: time = "time".asInstanceOf[time]
  @scala.inline
  def title: title = "title".asInstanceOf[title]
  @scala.inline
  def titleMaxLength: titleMaxLength = "titleMaxLength".asInstanceOf[titleMaxLength]
  @scala.inline
  def titlePadding: titlePadding = "titlePadding".asInstanceOf[titlePadding]
  @scala.inline
  def tooltip: tooltip = "tooltip".asInstanceOf[tooltip]
  @scala.inline
  def top: top = "top".asInstanceOf[top]
  @scala.inline
  def `top-left`: `top-left` = "top-left".asInstanceOf[`top-left`]
  @scala.inline
  def `top-right`: `top-right` = "top-right".asInstanceOf[`top-right`]
  @scala.inline
  def topojson: topojson = "topojson".asInstanceOf[topojson]
  @scala.inline
  def trail: trail = "trail".asInstanceOf[trail]
  @scala.inline
  def transparent: transparent = "transparent".asInstanceOf[transparent]
  @scala.inline
  def transverseMercator: transverseMercator = "transverseMercator".asInstanceOf[transverseMercator]
  @scala.inline
  def tsv: tsv = "tsv".asInstanceOf[tsv]
  @scala.inline
  def `type`: `type` = "type".asInstanceOf[`type`]
  @scala.inline
  def unaggregated: unaggregated = "unaggregated".asInstanceOf[unaggregated]
  @scala.inline
  def union: union = "union".asInstanceOf[union]
  @scala.inline
  def unit: unit = "unit".asInstanceOf[unit]
  @scala.inline
  def url: url = "url".asInstanceOf[url]
  @scala.inline
  def utc: utc = "utc".asInstanceOf[utc]
  @scala.inline
  def utcdate: utcdate = "utcdate".asInstanceOf[utcdate]
  @scala.inline
  def utcday: utcday = "utcday".asInstanceOf[utcday]
  @scala.inline
  def utchours: utchours = "utchours".asInstanceOf[utchours]
  @scala.inline
  def utchoursminutes: utchoursminutes = "utchoursminutes".asInstanceOf[utchoursminutes]
  @scala.inline
  def utchoursminutesseconds: utchoursminutesseconds = "utchoursminutesseconds".asInstanceOf[utchoursminutesseconds]
  @scala.inline
  def utcmilliseconds: utcmilliseconds = "utcmilliseconds".asInstanceOf[utcmilliseconds]
  @scala.inline
  def utcminutes: utcminutes = "utcminutes".asInstanceOf[utcminutes]
  @scala.inline
  def utcminutesseconds: utcminutesseconds = "utcminutesseconds".asInstanceOf[utcminutesseconds]
  @scala.inline
  def utcmonth: utcmonth = "utcmonth".asInstanceOf[utcmonth]
  @scala.inline
  def utcmonthdate: utcmonthdate = "utcmonthdate".asInstanceOf[utcmonthdate]
  @scala.inline
  def utcquarter: utcquarter = "utcquarter".asInstanceOf[utcquarter]
  @scala.inline
  def utcquartermonth: utcquartermonth = "utcquartermonth".asInstanceOf[utcquartermonth]
  @scala.inline
  def utcseconds: utcseconds = "utcseconds".asInstanceOf[utcseconds]
  @scala.inline
  def utcsecondsmilliseconds: utcsecondsmilliseconds = "utcsecondsmilliseconds".asInstanceOf[utcsecondsmilliseconds]
  @scala.inline
  def utcyear: utcyear = "utcyear".asInstanceOf[utcyear]
  @scala.inline
  def utcyearmonth: utcyearmonth = "utcyearmonth".asInstanceOf[utcyearmonth]
  @scala.inline
  def utcyearmonthdate: utcyearmonthdate = "utcyearmonthdate".asInstanceOf[utcyearmonthdate]
  @scala.inline
  def utcyearmonthdatehours: utcyearmonthdatehours = "utcyearmonthdatehours".asInstanceOf[utcyearmonthdatehours]
  @scala.inline
  def utcyearmonthdatehoursminutes: utcyearmonthdatehoursminutes = "utcyearmonthdatehoursminutes".asInstanceOf[utcyearmonthdatehoursminutes]
  @scala.inline
  def utcyearmonthdatehoursminutesseconds: utcyearmonthdatehoursminutesseconds = "utcyearmonthdatehoursminutesseconds".asInstanceOf[utcyearmonthdatehoursminutesseconds]
  @scala.inline
  def utcyearquarter: utcyearquarter = "utcyearquarter".asInstanceOf[utcyearquarter]
  @scala.inline
  def utcyearquartermonth: utcyearquartermonth = "utcyearquartermonth".asInstanceOf[utcyearquartermonth]
  @scala.inline
  def value: value = "value".asInstanceOf[value]
  @scala.inline
  def values: values = "values".asInstanceOf[values]
  @scala.inline
  def verbal: verbal = "verbal".asInstanceOf[verbal]
  @scala.inline
  def vertical: vertical = "vertical".asInstanceOf[vertical]
  @scala.inline
  def `vertical-text`: `vertical-text` = "vertical-text".asInstanceOf[`vertical-text`]
  @scala.inline
  def visual: visual = "visual".asInstanceOf[visual]
  @scala.inline
  def `w-resize`: `w-resize` = "w-resize".asInstanceOf[`w-resize`]
  @scala.inline
  def week: week = "week".asInstanceOf[week]
  @scala.inline
  def width: width = "width".asInstanceOf[width]
  @scala.inline
  def window: window = "window".asInstanceOf[window]
  @scala.inline
  def x: x = "x".asInstanceOf[x]
  @scala.inline
  def x2: x2 = "x2".asInstanceOf[x2]
  @scala.inline
  def xc: xc = "xc".asInstanceOf[xc]
  @scala.inline
  def y: y = "y".asInstanceOf[y]
  @scala.inline
  def y2: y2 = "y2".asInstanceOf[y2]
  @scala.inline
  def yc: yc = "yc".asInstanceOf[yc]
  @scala.inline
  def year: year = "year".asInstanceOf[year]
  @scala.inline
  def yearmonth: yearmonth = "yearmonth".asInstanceOf[yearmonth]
  @scala.inline
  def yearmonthdate: yearmonthdate = "yearmonthdate".asInstanceOf[yearmonthdate]
  @scala.inline
  def yearmonthdatehours: yearmonthdatehours = "yearmonthdatehours".asInstanceOf[yearmonthdatehours]
  @scala.inline
  def yearmonthdatehoursminutes: yearmonthdatehoursminutes = "yearmonthdatehoursminutes".asInstanceOf[yearmonthdatehoursminutes]
  @scala.inline
  def yearmonthdatehoursminutesseconds: yearmonthdatehoursminutesseconds = "yearmonthdatehoursminutesseconds".asInstanceOf[yearmonthdatehoursminutesseconds]
  @scala.inline
  def yearquarter: yearquarter = "yearquarter".asInstanceOf[yearquarter]
  @scala.inline
  def yearquartermonth: yearquartermonth = "yearquartermonth".asInstanceOf[yearquartermonth]
  @scala.inline
  def zero: zero = "zero".asInstanceOf[zero]
  @scala.inline
  def zindex: zindex = "zindex".asInstanceOf[zindex]
  @scala.inline
  def `zoom-in`: `zoom-in` = "zoom-in".asInstanceOf[`zoom-in`]
  @scala.inline
  def `zoom-out`: `zoom-out` = "zoom-out".asInstanceOf[`zoom-out`]
}

