package typings
package urlDashParseLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait Anon_Parse extends js.Object {
  @JSName("parse")
  var parse_Original: urlDashParseLib.urlDashParseMod.QueryParser = js.native
  @JSName("stringify")
  var stringify_Original: urlDashParseLib.urlDashParseMod.StringifyQuery = js.native
  def parse(query: java.lang.String): js.Object = js.native
  def stringify(query: js.Object): java.lang.String = js.native
}

