package typings.zingchart

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonA extends js.Object {
  @JSName("`%A`")
  var GraveaccentPercentsignAGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%D`")
  var GraveaccentPercentsignDGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%G`")
  var GraveaccentPercentsignGGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%H`")
  var GraveaccentPercentsignHGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%M`")
  var GraveaccentPercentsignMGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%Y`")
  var GraveaccentPercentsignYGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%a`")
  var GraveaccentPercentsignaGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%d`")
  var GraveaccentPercentsigndGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%dd`")
  var GraveaccentPercentsignddGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%g`")
  var GraveaccentPercentsigngGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%h`")
  var GraveaccentPercentsignhGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%i`")
  var GraveaccentPercentsigniGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%m`")
  var GraveaccentPercentsignmGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%mm`")
  var GraveaccentPercentsignmmGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%q`")
  var GraveaccentPercentsignqGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%s`")
  var GraveaccentPercentsignsGraveaccent: js.UndefOr[js.Any] = js.undefined
  @JSName("`%y`")
  var GraveaccentPercentsignyGraveaccent: js.UndefOr[js.Any] = js.undefined
  /**
    * To format your date values. Use this attribute with the `type` value (set to `true`). Token Description `%A` Displays the ante or
    * post meridiem time in upper case letters: AM or PM. `%a` Displays the ante or post meridiem time in lower case letters: am or pm.
    * `%D` Displays the day of the week in abbreviated form: Sun, Mon, Tue, Wed, Thu, Fri. `%d` Displays the day's date without a leadin
    * g 0 if the date is single digit. `%dd` Displays the day's date with a leading 0 if the date is single digit. `%G` Displays the hou
    * r in 24-hour format without a leading 0. `%g` Displays the hour in 12-hour format without a leading 0. `%H` Displays the hour in 2
    * 4-hour format with a leading 0 if the hour is single digit. `%h` Displays the hour in 12-hour format with a leading 0 if the hour
    * is single digit. `%i` Displays the minutes. `%M` Displays the month in abbreviated form: Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, S
    * ep, Oct, Nov and Dec. `%m` Displays the month numerically without a leading 0 if the date is single digit. `%mm` Display the month
    *  numerically with a leading 0 if the month is single digit. `%q` Displays the milliseconds. `%s` Displays the seconds. `%Y` Displa
    * ys the year in 4-digit format. `%y` Displays the year in 2-digit format.
    */
  var all: js.UndefOr[String] = js.undefined
  var guide: js.UndefOr[AnonAlphaLinecolorLinestyleLinewidthVisibleAny] = js.undefined
  var item: js.UndefOr[
    AnonAlphaAngleBackgroundcolorBackgroundcolor1Backgroundcolor2BackgroundfitBackgroundimageBackgroundpositionBackgroundrepeatBoldBorderbottomBordercolor
  ] = js.undefined
}

object AnonA {
  @scala.inline
  def apply(
    GraveaccentPercentsignAGraveaccent: js.Any = null,
    GraveaccentPercentsignDGraveaccent: js.Any = null,
    GraveaccentPercentsignGGraveaccent: js.Any = null,
    GraveaccentPercentsignHGraveaccent: js.Any = null,
    GraveaccentPercentsignMGraveaccent: js.Any = null,
    GraveaccentPercentsignYGraveaccent: js.Any = null,
    GraveaccentPercentsignaGraveaccent: js.Any = null,
    GraveaccentPercentsigndGraveaccent: js.Any = null,
    GraveaccentPercentsignddGraveaccent: js.Any = null,
    GraveaccentPercentsigngGraveaccent: js.Any = null,
    GraveaccentPercentsignhGraveaccent: js.Any = null,
    GraveaccentPercentsigniGraveaccent: js.Any = null,
    GraveaccentPercentsignmGraveaccent: js.Any = null,
    GraveaccentPercentsignmmGraveaccent: js.Any = null,
    GraveaccentPercentsignqGraveaccent: js.Any = null,
    GraveaccentPercentsignsGraveaccent: js.Any = null,
    GraveaccentPercentsignyGraveaccent: js.Any = null,
    all: String = null,
    guide: AnonAlphaLinecolorLinestyleLinewidthVisibleAny = null,
    item: AnonAlphaAngleBackgroundcolorBackgroundcolor1Backgroundcolor2BackgroundfitBackgroundimageBackgroundpositionBackgroundrepeatBoldBorderbottomBordercolor = null
  ): AnonA = {
    val __obj = js.Dynamic.literal()
    if (GraveaccentPercentsignAGraveaccent != null) __obj.updateDynamic("`%A`")(GraveaccentPercentsignAGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignDGraveaccent != null) __obj.updateDynamic("`%D`")(GraveaccentPercentsignDGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignGGraveaccent != null) __obj.updateDynamic("`%G`")(GraveaccentPercentsignGGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignHGraveaccent != null) __obj.updateDynamic("`%H`")(GraveaccentPercentsignHGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignMGraveaccent != null) __obj.updateDynamic("`%M`")(GraveaccentPercentsignMGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignYGraveaccent != null) __obj.updateDynamic("`%Y`")(GraveaccentPercentsignYGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignaGraveaccent != null) __obj.updateDynamic("`%a`")(GraveaccentPercentsignaGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsigndGraveaccent != null) __obj.updateDynamic("`%d`")(GraveaccentPercentsigndGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignddGraveaccent != null) __obj.updateDynamic("`%dd`")(GraveaccentPercentsignddGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsigngGraveaccent != null) __obj.updateDynamic("`%g`")(GraveaccentPercentsigngGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignhGraveaccent != null) __obj.updateDynamic("`%h`")(GraveaccentPercentsignhGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsigniGraveaccent != null) __obj.updateDynamic("`%i`")(GraveaccentPercentsigniGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignmGraveaccent != null) __obj.updateDynamic("`%m`")(GraveaccentPercentsignmGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignmmGraveaccent != null) __obj.updateDynamic("`%mm`")(GraveaccentPercentsignmmGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignqGraveaccent != null) __obj.updateDynamic("`%q`")(GraveaccentPercentsignqGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignsGraveaccent != null) __obj.updateDynamic("`%s`")(GraveaccentPercentsignsGraveaccent.asInstanceOf[js.Any])
    if (GraveaccentPercentsignyGraveaccent != null) __obj.updateDynamic("`%y`")(GraveaccentPercentsignyGraveaccent.asInstanceOf[js.Any])
    if (all != null) __obj.updateDynamic("all")(all.asInstanceOf[js.Any])
    if (guide != null) __obj.updateDynamic("guide")(guide.asInstanceOf[js.Any])
    if (item != null) __obj.updateDynamic("item")(item.asInstanceOf[js.Any])
    __obj.asInstanceOf[AnonA]
  }
}

