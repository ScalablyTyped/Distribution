package typings
package ejDotWebDotAllLib.ejNs.ColorPickerNs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait TooltipText extends js.Object {
  /** Sets the tooltip text for the add button.
    */
  var addButton: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the basic preset.
    */
  var basic: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the candy crush preset.
    */
  var candyCrush: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the citrus preset.
    */
  var citrus: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the current color area.
    */
  var currentColor: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the flat colors preset.
    */
  var flatColors: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the misty preset.
    */
  var misty: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the mono chrome preset.
    */
  var monoChrome: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the moon light preset.
    */
  var moonLight: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the pink shades preset.
    */
  var pinkShades: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the sandy preset.
    */
  var sandy: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the sea wolf preset.
    */
  var seaWolf: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the selected color area.
    */
  var selectedColor: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the switcher button.
    */
  var switcher: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the vintage preset.
    */
  var vintage: js.UndefOr[java.lang.String] = js.undefined
  /** Sets the tooltip text for the web colors preset.
    */
  var webColors: js.UndefOr[java.lang.String] = js.undefined
}

