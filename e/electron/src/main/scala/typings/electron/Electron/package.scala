package typings.electron.Electron

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}


type Accelerator = java.lang.String

/* was `typeof imported_events.EventEmitter` */
type NodeEventEmitter = typings.node.eventsMod.EventEmitter

type TouchBarGroup = typings.node.eventsMod.EventEmitter

type TouchBarOtherItemsProxy = typings.node.eventsMod.EventEmitter

/* was `typeof NativeImage` */
type nativeImage = typings.electron.Electron.NativeImage_

/* was `typeof Session` */
type session = typings.electron.Electron.Session_

/* was `typeof WebContents` */
type webContents = typings.electron.Electron.WebContents_
