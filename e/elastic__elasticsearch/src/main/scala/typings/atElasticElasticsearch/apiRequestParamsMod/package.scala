...
package object apiRequestParamsMod {
  ...
  type Info = Generic
  type IngestProcessorGrok = Generic
  type LicenseDelete = Generic
  type LicenseGetBasicStatus = Generic
  type LicenseGetTrialStatus = Generic
  type MlDeleteExpiredData = Generic
  type MlInfo = Generic
  type Ping = Generic
