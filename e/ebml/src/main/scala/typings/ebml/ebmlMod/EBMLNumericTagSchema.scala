...
- typings.ebml.ebmlMod.EBMLTagSchema because Already inherited */ trait EBMLNumericTagSchema extends EBMLDefaultableTagSchema {
  var br: js.UndefOr[
...
object EBMLNumericTagSchema {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(description = description, level = level, name = name, range = range)
