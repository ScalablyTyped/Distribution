package typings.javascriptDashTimeDashAgo

import typings.javascriptDashTimeDashAgo.localeMod.DefaultFormats
import typings.javascriptDashTimeDashAgo.localeMod.ExtendedFormats
import typings.javascriptDashTimeDashAgo.localeMod.Formats
import typings.javascriptDashTimeDashAgo.localeMod.TimeUnit
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object javascriptDashTimeDashAgoStrings {
  @js.native
  sealed trait af extends js.Object
  
  @js.native
  sealed trait am extends js.Object
  
  @js.native
  sealed trait ar extends js.Object
  
  @js.native
  sealed trait `ar-AE` extends js.Object
  
  @js.native
  sealed trait as extends js.Object
  
  @js.native
  sealed trait ast extends js.Object
  
  @js.native
  sealed trait az extends js.Object
  
  @js.native
  sealed trait be extends js.Object
  
  @js.native
  sealed trait bg extends js.Object
  
  @js.native
  sealed trait bn extends js.Object
  
  @js.native
  sealed trait br extends js.Object
  
  @js.native
  sealed trait bs extends js.Object
  
  @js.native
  sealed trait `bs-Cyrl` extends js.Object
  
  @js.native
  sealed trait ca extends js.Object
  
  @js.native
  sealed trait ccp extends js.Object
  
  @js.native
  sealed trait ce extends js.Object
  
  @js.native
  sealed trait chr extends js.Object
  
  @js.native
  sealed trait cs extends js.Object
  
  @js.native
  sealed trait cy extends js.Object
  
  @js.native
  sealed trait da extends js.Object
  
  @js.native
  sealed trait day extends TimeUnit
  
  @js.native
  sealed trait de extends js.Object
  
  @js.native
  sealed trait dsb extends js.Object
  
  @js.native
  sealed trait dz extends js.Object
  
  @js.native
  sealed trait ee extends js.Object
  
  @js.native
  sealed trait el extends js.Object
  
  @js.native
  sealed trait en extends js.Object
  
  @js.native
  sealed trait `es-AR` extends js.Object
  
  @js.native
  sealed trait `es-MX` extends js.Object
  
  @js.native
  sealed trait `es-PY` extends js.Object
  
  @js.native
  sealed trait `es-SV` extends js.Object
  
  @js.native
  sealed trait `es-US` extends js.Object
  
  @js.native
  sealed trait et extends js.Object
  
  @js.native
  sealed trait eu extends js.Object
  
  @js.native
  sealed trait fa extends js.Object
  
  @js.native
  sealed trait few extends js.Object
  
  @js.native
  sealed trait fi extends js.Object
  
  @js.native
  sealed trait fil extends js.Object
  
  @js.native
  sealed trait fo extends js.Object
  
  @js.native
  sealed trait fr extends js.Object
  
  @js.native
  sealed trait `fr-CA` extends js.Object
  
  @js.native
  sealed trait fur extends js.Object
  
  @js.native
  sealed trait fy extends js.Object
  
  @js.native
  sealed trait ga extends js.Object
  
  @js.native
  sealed trait gd extends js.Object
  
  @js.native
  sealed trait gl extends js.Object
  
  @js.native
  sealed trait gu extends js.Object
  
  @js.native
  sealed trait he extends js.Object
  
  @js.native
  sealed trait hi extends js.Object
  
  @js.native
  sealed trait hour extends TimeUnit
  
  @js.native
  sealed trait hr extends js.Object
  
  @js.native
  sealed trait hsb extends js.Object
  
  @js.native
  sealed trait hu extends js.Object
  
  @js.native
  sealed trait hy extends js.Object
  
  @js.native
  sealed trait ia extends js.Object
  
  @js.native
  sealed trait id extends js.Object
  
  @js.native
  sealed trait is extends js.Object
  
  @js.native
  sealed trait it extends js.Object
  
  @js.native
  sealed trait ja extends js.Object
  
  @js.native
  sealed trait jgo extends js.Object
  
  @js.native
  sealed trait jv extends js.Object
  
  @js.native
  sealed trait ka extends js.Object
  
  @js.native
  sealed trait kea extends js.Object
  
  @js.native
  sealed trait kk extends js.Object
  
  @js.native
  sealed trait kl extends js.Object
  
  @js.native
  sealed trait km extends js.Object
  
  @js.native
  sealed trait kn extends js.Object
  
  @js.native
  sealed trait ko extends js.Object
  
  @js.native
  sealed trait kok extends js.Object
  
  @js.native
  sealed trait ksh extends js.Object
  
  @js.native
  sealed trait ku extends js.Object
  
  @js.native
  sealed trait ky extends js.Object
  
  @js.native
  sealed trait lb extends js.Object
  
  @js.native
  sealed trait lkt extends js.Object
  
  @js.native
  sealed trait lo extends js.Object
  
  @js.native
  sealed trait long
    extends DefaultFormats
       with Formats
  
  @js.native
  sealed trait `long-convenient`
    extends ExtendedFormats
       with Formats
  
  @js.native
  sealed trait `long-time`
    extends ExtendedFormats
       with Formats
  
  @js.native
  sealed trait lt extends js.Object
  
  @js.native
  sealed trait lv extends js.Object
  
  @js.native
  sealed trait minute extends TimeUnit
  
  @js.native
  sealed trait mk extends js.Object
  
  @js.native
  sealed trait ml extends js.Object
  
  @js.native
  sealed trait mn extends js.Object
  
  @js.native
  sealed trait month extends TimeUnit
  
  @js.native
  sealed trait mr extends js.Object
  
  @js.native
  sealed trait ms extends js.Object
  
  @js.native
  sealed trait mt extends js.Object
  
  @js.native
  sealed trait my extends js.Object
  
  @js.native
  sealed trait mzn extends js.Object
  
  @js.native
  sealed trait narrow
    extends DefaultFormats
       with Formats
  
  @js.native
  sealed trait nb extends js.Object
  
  @js.native
  sealed trait ne extends js.Object
  
  @js.native
  sealed trait nl extends js.Object
  
  @js.native
  sealed trait nn extends js.Object
  
  @js.native
  sealed trait now extends TimeUnit
  
  @js.native
  sealed trait one extends js.Object
  
  @js.native
  sealed trait or extends js.Object
  
  @js.native
  sealed trait other extends js.Object
  
  @js.native
  sealed trait pa extends js.Object
  
  @js.native
  sealed trait pl extends js.Object
  
  @js.native
  sealed trait ps extends js.Object
  
  @js.native
  sealed trait pt extends js.Object
  
  @js.native
  sealed trait qu extends js.Object
  
  @js.native
  sealed trait quarter extends TimeUnit
  
  @js.native
  sealed trait ro extends js.Object
  
  @js.native
  sealed trait ru extends js.Object
  
  @js.native
  sealed trait sah extends js.Object
  
  @js.native
  sealed trait sd extends js.Object
  
  @js.native
  sealed trait se extends js.Object
  
  @js.native
  sealed trait `se-FI` extends js.Object
  
  @js.native
  sealed trait second extends TimeUnit
  
  @js.native
  sealed trait short
    extends DefaultFormats
       with Formats
  
  @js.native
  sealed trait `short-convenient`
    extends ExtendedFormats
       with Formats
  
  @js.native
  sealed trait `short-time`
    extends ExtendedFormats
       with Formats
  
  @js.native
  sealed trait si extends js.Object
  
  @js.native
  sealed trait sk extends js.Object
  
  @js.native
  sealed trait sl extends js.Object
  
  @js.native
  sealed trait so extends js.Object
  
  @js.native
  sealed trait sq extends js.Object
  
  @js.native
  sealed trait sr extends js.Object
  
  @js.native
  sealed trait `sr-Latn` extends js.Object
  
  @js.native
  sealed trait sv extends js.Object
  
  @js.native
  sealed trait sw extends js.Object
  
  @js.native
  sealed trait ta extends js.Object
  
  @js.native
  sealed trait te extends js.Object
  
  @js.native
  sealed trait th extends js.Object
  
  @js.native
  sealed trait ti extends js.Object
  
  @js.native
  sealed trait tiny
    extends ExtendedFormats
       with Formats
  
  @js.native
  sealed trait tk extends js.Object
  
  @js.native
  sealed trait to extends js.Object
  
  @js.native
  sealed trait tr extends js.Object
  
  @js.native
  sealed trait two extends js.Object
  
  @js.native
  sealed trait ug extends js.Object
  
  @js.native
  sealed trait uk extends js.Object
  
  @js.native
  sealed trait ur extends js.Object
  
  @js.native
  sealed trait `ur-IN` extends js.Object
  
  @js.native
  sealed trait uz extends js.Object
  
  @js.native
  sealed trait `uz-Cyrl` extends js.Object
  
  @js.native
  sealed trait vi extends js.Object
  
  @js.native
  sealed trait wae extends js.Object
  
  @js.native
  sealed trait week extends TimeUnit
  
  @js.native
  sealed trait year extends TimeUnit
  
  @js.native
  sealed trait yi extends js.Object
  
  @js.native
  sealed trait yue extends js.Object
  
  @js.native
  sealed trait `yue-Hans` extends js.Object
  
  @js.native
  sealed trait zh extends js.Object
  
  @js.native
  sealed trait `zh-Hans-HK` extends js.Object
  
  @js.native
  sealed trait `zh-Hans-MO` extends js.Object
  
  @js.native
  sealed trait `zh-Hans-SG` extends js.Object
  
  @js.native
  sealed trait `zh-Hant` extends js.Object
  
  @js.native
  sealed trait `zh-Hant-HK` extends js.Object
  
  @js.native
  sealed trait `zh-Hant-MO` extends js.Object
  
  @js.native
  sealed trait zu extends js.Object
  
  @scala.inline
  def af: af = "af".asInstanceOf[af]
  @scala.inline
  def am: am = "am".asInstanceOf[am]
  @scala.inline
  def ar: ar = "ar".asInstanceOf[ar]
  @scala.inline
  def `ar-AE`: `ar-AE` = "ar-AE".asInstanceOf[`ar-AE`]
  @scala.inline
  def as: as = "as".asInstanceOf[as]
  @scala.inline
  def ast: ast = "ast".asInstanceOf[ast]
  @scala.inline
  def az: az = "az".asInstanceOf[az]
  @scala.inline
  def be: be = "be".asInstanceOf[be]
  @scala.inline
  def bg: bg = "bg".asInstanceOf[bg]
  @scala.inline
  def bn: bn = "bn".asInstanceOf[bn]
  @scala.inline
  def br: br = "br".asInstanceOf[br]
  @scala.inline
  def bs: bs = "bs".asInstanceOf[bs]
  @scala.inline
  def `bs-Cyrl`: `bs-Cyrl` = "bs-Cyrl".asInstanceOf[`bs-Cyrl`]
  @scala.inline
  def ca: ca = "ca".asInstanceOf[ca]
  @scala.inline
  def ccp: ccp = "ccp".asInstanceOf[ccp]
  @scala.inline
  def ce: ce = "ce".asInstanceOf[ce]
  @scala.inline
  def chr: chr = "chr".asInstanceOf[chr]
  @scala.inline
  def cs: cs = "cs".asInstanceOf[cs]
  @scala.inline
  def cy: cy = "cy".asInstanceOf[cy]
  @scala.inline
  def da: da = "da".asInstanceOf[da]
  @scala.inline
  def day: day = "day".asInstanceOf[day]
  @scala.inline
  def de: de = "de".asInstanceOf[de]
  @scala.inline
  def dsb: dsb = "dsb".asInstanceOf[dsb]
  @scala.inline
  def dz: dz = "dz".asInstanceOf[dz]
  @scala.inline
  def ee: ee = "ee".asInstanceOf[ee]
  @scala.inline
  def el: el = "el".asInstanceOf[el]
  @scala.inline
  def en: en = "en".asInstanceOf[en]
  @scala.inline
  def `es-AR`: `es-AR` = "es-AR".asInstanceOf[`es-AR`]
  @scala.inline
  def `es-MX`: `es-MX` = "es-MX".asInstanceOf[`es-MX`]
  @scala.inline
  def `es-PY`: `es-PY` = "es-PY".asInstanceOf[`es-PY`]
  @scala.inline
  def `es-SV`: `es-SV` = "es-SV".asInstanceOf[`es-SV`]
  @scala.inline
  def `es-US`: `es-US` = "es-US".asInstanceOf[`es-US`]
  @scala.inline
  def et: et = "et".asInstanceOf[et]
  @scala.inline
  def eu: eu = "eu".asInstanceOf[eu]
  @scala.inline
  def fa: fa = "fa".asInstanceOf[fa]
  @scala.inline
  def few: few = "few".asInstanceOf[few]
  @scala.inline
  def fi: fi = "fi".asInstanceOf[fi]
  @scala.inline
  def fil: fil = "fil".asInstanceOf[fil]
  @scala.inline
  def fo: fo = "fo".asInstanceOf[fo]
  @scala.inline
  def fr: fr = "fr".asInstanceOf[fr]
  @scala.inline
  def `fr-CA`: `fr-CA` = "fr-CA".asInstanceOf[`fr-CA`]
  @scala.inline
  def fur: fur = "fur".asInstanceOf[fur]
  @scala.inline
  def fy: fy = "fy".asInstanceOf[fy]
  @scala.inline
  def ga: ga = "ga".asInstanceOf[ga]
  @scala.inline
  def gd: gd = "gd".asInstanceOf[gd]
  @scala.inline
  def gl: gl = "gl".asInstanceOf[gl]
  @scala.inline
  def gu: gu = "gu".asInstanceOf[gu]
  @scala.inline
  def he: he = "he".asInstanceOf[he]
  @scala.inline
  def hi: hi = "hi".asInstanceOf[hi]
  @scala.inline
  def hour: hour = "hour".asInstanceOf[hour]
  @scala.inline
  def hr: hr = "hr".asInstanceOf[hr]
  @scala.inline
  def hsb: hsb = "hsb".asInstanceOf[hsb]
  @scala.inline
  def hu: hu = "hu".asInstanceOf[hu]
  @scala.inline
  def hy: hy = "hy".asInstanceOf[hy]
  @scala.inline
  def ia: ia = "ia".asInstanceOf[ia]
  @scala.inline
  def id: id = "id".asInstanceOf[id]
  @scala.inline
  def is: is = "is".asInstanceOf[is]
  @scala.inline
  def it: it = "it".asInstanceOf[it]
  @scala.inline
  def ja: ja = "ja".asInstanceOf[ja]
  @scala.inline
  def jgo: jgo = "jgo".asInstanceOf[jgo]
  @scala.inline
  def jv: jv = "jv".asInstanceOf[jv]
  @scala.inline
  def ka: ka = "ka".asInstanceOf[ka]
  @scala.inline
  def kea: kea = "kea".asInstanceOf[kea]
  @scala.inline
  def kk: kk = "kk".asInstanceOf[kk]
  @scala.inline
  def kl: kl = "kl".asInstanceOf[kl]
  @scala.inline
  def km: km = "km".asInstanceOf[km]
  @scala.inline
  def kn: kn = "kn".asInstanceOf[kn]
  @scala.inline
  def ko: ko = "ko".asInstanceOf[ko]
  @scala.inline
  def kok: kok = "kok".asInstanceOf[kok]
  @scala.inline
  def ksh: ksh = "ksh".asInstanceOf[ksh]
  @scala.inline
  def ku: ku = "ku".asInstanceOf[ku]
  @scala.inline
  def ky: ky = "ky".asInstanceOf[ky]
  @scala.inline
  def lb: lb = "lb".asInstanceOf[lb]
  @scala.inline
  def lkt: lkt = "lkt".asInstanceOf[lkt]
  @scala.inline
  def lo: lo = "lo".asInstanceOf[lo]
  @scala.inline
  def long: long = "long".asInstanceOf[long]
  @scala.inline
  def `long-convenient`: `long-convenient` = "long-convenient".asInstanceOf[`long-convenient`]
  @scala.inline
  def `long-time`: `long-time` = "long-time".asInstanceOf[`long-time`]
  @scala.inline
  def lt: lt = "lt".asInstanceOf[lt]
  @scala.inline
  def lv: lv = "lv".asInstanceOf[lv]
  @scala.inline
  def minute: minute = "minute".asInstanceOf[minute]
  @scala.inline
  def mk: mk = "mk".asInstanceOf[mk]
  @scala.inline
  def ml: ml = "ml".asInstanceOf[ml]
  @scala.inline
  def mn: mn = "mn".asInstanceOf[mn]
  @scala.inline
  def month: month = "month".asInstanceOf[month]
  @scala.inline
  def mr: mr = "mr".asInstanceOf[mr]
  @scala.inline
  def ms: ms = "ms".asInstanceOf[ms]
  @scala.inline
  def mt: mt = "mt".asInstanceOf[mt]
  @scala.inline
  def my: my = "my".asInstanceOf[my]
  @scala.inline
  def mzn: mzn = "mzn".asInstanceOf[mzn]
  @scala.inline
  def narrow: narrow = "narrow".asInstanceOf[narrow]
  @scala.inline
  def nb: nb = "nb".asInstanceOf[nb]
  @scala.inline
  def ne: ne = "ne".asInstanceOf[ne]
  @scala.inline
  def nl: nl = "nl".asInstanceOf[nl]
  @scala.inline
  def nn: nn = "nn".asInstanceOf[nn]
  @scala.inline
  def now: now = "now".asInstanceOf[now]
  @scala.inline
  def one: one = "one".asInstanceOf[one]
  @scala.inline
  def or: or = "or".asInstanceOf[or]
  @scala.inline
  def other: other = "other".asInstanceOf[other]
  @scala.inline
  def pa: pa = "pa".asInstanceOf[pa]
  @scala.inline
  def pl: pl = "pl".asInstanceOf[pl]
  @scala.inline
  def ps: ps = "ps".asInstanceOf[ps]
  @scala.inline
  def pt: pt = "pt".asInstanceOf[pt]
  @scala.inline
  def qu: qu = "qu".asInstanceOf[qu]
  @scala.inline
  def quarter: quarter = "quarter".asInstanceOf[quarter]
  @scala.inline
  def ro: ro = "ro".asInstanceOf[ro]
  @scala.inline
  def ru: ru = "ru".asInstanceOf[ru]
  @scala.inline
  def sah: sah = "sah".asInstanceOf[sah]
  @scala.inline
  def sd: sd = "sd".asInstanceOf[sd]
  @scala.inline
  def se: se = "se".asInstanceOf[se]
  @scala.inline
  def `se-FI`: `se-FI` = "se-FI".asInstanceOf[`se-FI`]
  @scala.inline
  def second: second = "second".asInstanceOf[second]
  @scala.inline
  def short: short = "short".asInstanceOf[short]
  @scala.inline
  def `short-convenient`: `short-convenient` = "short-convenient".asInstanceOf[`short-convenient`]
  @scala.inline
  def `short-time`: `short-time` = "short-time".asInstanceOf[`short-time`]
  @scala.inline
  def si: si = "si".asInstanceOf[si]
  @scala.inline
  def sk: sk = "sk".asInstanceOf[sk]
  @scala.inline
  def sl: sl = "sl".asInstanceOf[sl]
  @scala.inline
  def so: so = "so".asInstanceOf[so]
  @scala.inline
  def sq: sq = "sq".asInstanceOf[sq]
  @scala.inline
  def sr: sr = "sr".asInstanceOf[sr]
  @scala.inline
  def `sr-Latn`: `sr-Latn` = "sr-Latn".asInstanceOf[`sr-Latn`]
  @scala.inline
  def sv: sv = "sv".asInstanceOf[sv]
  @scala.inline
  def sw: sw = "sw".asInstanceOf[sw]
  @scala.inline
  def ta: ta = "ta".asInstanceOf[ta]
  @scala.inline
  def te: te = "te".asInstanceOf[te]
  @scala.inline
  def th: th = "th".asInstanceOf[th]
  @scala.inline
  def ti: ti = "ti".asInstanceOf[ti]
  @scala.inline
  def tiny: tiny = "tiny".asInstanceOf[tiny]
  @scala.inline
  def tk: tk = "tk".asInstanceOf[tk]
  @scala.inline
  def to: to = "to".asInstanceOf[to]
  @scala.inline
  def tr: tr = "tr".asInstanceOf[tr]
  @scala.inline
  def two: two = "two".asInstanceOf[two]
  @scala.inline
  def ug: ug = "ug".asInstanceOf[ug]
  @scala.inline
  def uk: uk = "uk".asInstanceOf[uk]
  @scala.inline
  def ur: ur = "ur".asInstanceOf[ur]
  @scala.inline
  def `ur-IN`: `ur-IN` = "ur-IN".asInstanceOf[`ur-IN`]
  @scala.inline
  def uz: uz = "uz".asInstanceOf[uz]
  @scala.inline
  def `uz-Cyrl`: `uz-Cyrl` = "uz-Cyrl".asInstanceOf[`uz-Cyrl`]
  @scala.inline
  def vi: vi = "vi".asInstanceOf[vi]
  @scala.inline
  def wae: wae = "wae".asInstanceOf[wae]
  @scala.inline
  def week: week = "week".asInstanceOf[week]
  @scala.inline
  def year: year = "year".asInstanceOf[year]
  @scala.inline
  def yi: yi = "yi".asInstanceOf[yi]
  @scala.inline
  def yue: yue = "yue".asInstanceOf[yue]
  @scala.inline
  def `yue-Hans`: `yue-Hans` = "yue-Hans".asInstanceOf[`yue-Hans`]
  @scala.inline
  def zh: zh = "zh".asInstanceOf[zh]
  @scala.inline
  def `zh-Hans-HK`: `zh-Hans-HK` = "zh-Hans-HK".asInstanceOf[`zh-Hans-HK`]
  @scala.inline
  def `zh-Hans-MO`: `zh-Hans-MO` = "zh-Hans-MO".asInstanceOf[`zh-Hans-MO`]
  @scala.inline
  def `zh-Hans-SG`: `zh-Hans-SG` = "zh-Hans-SG".asInstanceOf[`zh-Hans-SG`]
  @scala.inline
  def `zh-Hant`: `zh-Hant` = "zh-Hant".asInstanceOf[`zh-Hant`]
  @scala.inline
  def `zh-Hant-HK`: `zh-Hant-HK` = "zh-Hant-HK".asInstanceOf[`zh-Hant-HK`]
  @scala.inline
  def `zh-Hant-MO`: `zh-Hant-MO` = "zh-Hant-MO".asInstanceOf[`zh-Hant-MO`]
  @scala.inline
  def zu: zu = "zu".asInstanceOf[zu]
}

