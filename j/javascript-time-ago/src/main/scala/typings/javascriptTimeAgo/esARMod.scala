...
object esARMod extends Shortcut {
  ...
  object Locale {
    ...
    @scala.inline
    def apply(locale: `es-AR`, narrow: Duration, quantify: Double => one | two | few | other, short: Duration): Locale = {
    ...
    implicit class LocaleMutableBuilder[Self <: Locale] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setLocale(value: `es-AR`): Self = StObject.set(x, "locale", value.asInstanceOf[js.Any])
  ...
  /* This means you don't have to write `^`, but can instead just say `esARMod.foo` */
