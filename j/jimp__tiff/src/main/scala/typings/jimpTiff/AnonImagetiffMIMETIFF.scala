package typings.jimpTiff

import typings.jimpTiff.jimpTiffStrings.imageSlashtiff
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonImagetiffMIMETIFF extends js.Object {
  var MIME_TIFF: imageSlashtiff
}

object AnonImagetiffMIMETIFF {
  @scala.inline
  def apply(MIME_TIFF: imageSlashtiff): AnonImagetiffMIMETIFF = {
    val __obj = js.Dynamic.literal(MIME_TIFF = MIME_TIFF.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[AnonImagetiffMIMETIFF]
  }
}

