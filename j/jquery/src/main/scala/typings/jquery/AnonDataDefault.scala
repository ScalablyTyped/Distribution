...
trait AnonDataDefault[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
...
object AnonDataDefault {
  @scala.inline
  def apply[TTarget, TData](_default: (TriggeredEvent[TTarget, TData, _, _], TData) => Unit | `false`): AnonDataDefault[TTarget, TData] = {
    val __obj = js.Dynamic.literal(_default = js.Any.fromFunction2(_default))
  
    __obj.asInstanceOf[AnonDataDefault[TTarget, TData]]
  }
}
