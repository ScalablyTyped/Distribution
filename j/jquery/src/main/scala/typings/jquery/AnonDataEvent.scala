...
trait AnonDataEvent[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
    ...
    */
  def trigger(event: typings.jquery.JQuery_.Event, data: TData): Unit | `false`
}

object AnonDataEvent {
  @scala.inline
  def apply[TTarget, TData](trigger: (typings.jquery.JQuery_.Event, TData) => Unit | `false`): AnonDataEvent[TTarget, TData] = {
