...
object MouseUpEvent {
  ...
  @scala.inline
  implicit class MouseUpEventOps[Self <: MouseUpEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (MouseUpEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    ...
    @scala.inline
    def setType(value: mouseup): Self = this.set("type", value.asInstanceOf[js.Any])
  }
}
