...
object ClickEvent {
  ...
  @scala.inline
  implicit class ClickEventOps[Self <: ClickEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (ClickEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
