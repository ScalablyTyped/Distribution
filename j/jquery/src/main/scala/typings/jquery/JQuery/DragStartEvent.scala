...
object DragStartEvent {
  ...
  @scala.inline
  implicit class DragStartEventOps[Self <: DragStartEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (DragStartEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def setType(value: dragstart): Self = this.set("type", value.asInstanceOf[js.Any])
  }
