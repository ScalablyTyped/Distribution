...
object ContextMenuEvent {
  ...
  @scala.inline
  implicit class ContextMenuEventOps[Self <: ContextMenuEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (ContextMenuEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    ...
    @scala.inline
    def setType(value: contextmenu): Self = this.set("type", value.asInstanceOf[js.Any])
  }
}
