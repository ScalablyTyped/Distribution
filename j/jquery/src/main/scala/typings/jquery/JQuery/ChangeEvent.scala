...
object ChangeEvent {
  ...
  @scala.inline
  implicit class ChangeEventOps[Self <: ChangeEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (ChangeEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
