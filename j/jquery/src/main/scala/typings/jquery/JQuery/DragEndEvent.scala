...
object DragEndEvent {
  ...
  @scala.inline
  implicit class DragEndEventOps[Self <: DragEndEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (DragEndEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    ...
    @scala.inline
    def setType(value: dragend): Self = this.set("type", value.asInstanceOf[js.Any])
  }
}
