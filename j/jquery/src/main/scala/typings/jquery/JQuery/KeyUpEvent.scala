...
object KeyUpEvent {
  ...
  @scala.inline
  implicit class KeyUpEventOps[Self <: KeyUpEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (KeyUpEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    ...
    @scala.inline
    def setType(value: keyup): Self = this.set("type", value.asInstanceOf[js.Any])
  }
}
