...
object MouseOutEvent {
  ...
  @scala.inline
  implicit class MouseOutEventOps[Self <: MouseOutEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (MouseOutEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    ...
    @scala.inline
    def setType(value: mouseout): Self = this.set("type", value.asInstanceOf[js.Any])
  }
}
