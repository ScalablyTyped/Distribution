...
object TouchStartEvent {
  ...
  @scala.inline
  implicit class TouchStartEventOps[Self <: TouchStartEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (TouchStartEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def setType(value: touchstart): Self = this.set("type", value.asInstanceOf[js.Any])
  }
