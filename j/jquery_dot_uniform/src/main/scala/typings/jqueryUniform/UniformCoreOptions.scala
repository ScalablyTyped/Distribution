package typings.jqueryUniform

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait UniformCoreOptions extends StObject {
  
  var activeClass: js.UndefOr[String] = js.undefined
  
  var autoHide: js.UndefOr[Boolean] = js.undefined
  
  var buttonClass: js.UndefOr[String] = js.undefined
  
  var checkboxClass: js.UndefOr[String] = js.undefined
  
  var checkedClass: js.UndefOr[String] = js.undefined
  
  var disabledClass: js.UndefOr[String] = js.undefined
  
  var eventNamespace: js.UndefOr[String] = js.undefined
  
  var fileButtonClass: js.UndefOr[String] = js.undefined
  
  var fileButtonHtml: js.UndefOr[String] = js.undefined
  
  var fileClass: js.UndefOr[String] = js.undefined
  
  var fileDefaultHtml: js.UndefOr[String] = js.undefined
  
  var filenameClass: js.UndefOr[String] = js.undefined
  
  var focusClass: js.UndefOr[String] = js.undefined
  
  var hoverClass: js.UndefOr[String] = js.undefined
  
  var idPrefix: js.UndefOr[String] = js.undefined
  
  var inputAddTypeAsClass: js.UndefOr[Boolean] = js.undefined
  
  var inputClass: js.UndefOr[String] = js.undefined
  
  var radioClass: js.UndefOr[String] = js.undefined
  
  var resetDefaultHtml: js.UndefOr[String] = js.undefined
  
  var resetSelector: js.UndefOr[Any] = js.undefined
  
  var selectAutoWidth: js.UndefOr[Boolean] = js.undefined
  
  var selectClass: js.UndefOr[String] = js.undefined
  
  var selectMultiClass: js.UndefOr[String] = js.undefined
  
  var submitDefaultHtml: js.UndefOr[String] = js.undefined
  
  var textareaClass: js.UndefOr[String] = js.undefined
  
  var useID: js.UndefOr[Boolean] = js.undefined
  
  var wrapperClass: js.UndefOr[String] = js.undefined
}
object UniformCoreOptions {
  
  inline def apply(): UniformCoreOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[UniformCoreOptions]
  }
  
  extension [Self <: UniformCoreOptions](x: Self) {
    
    inline def setActiveClass(value: String): Self = StObject.set(x, "activeClass", value.asInstanceOf[js.Any])
    
    inline def setActiveClassUndefined: Self = StObject.set(x, "activeClass", js.undefined)
    
    inline def setAutoHide(value: Boolean): Self = StObject.set(x, "autoHide", value.asInstanceOf[js.Any])
    
    inline def setAutoHideUndefined: Self = StObject.set(x, "autoHide", js.undefined)
    
    inline def setButtonClass(value: String): Self = StObject.set(x, "buttonClass", value.asInstanceOf[js.Any])
    
    inline def setButtonClassUndefined: Self = StObject.set(x, "buttonClass", js.undefined)
    
    inline def setCheckboxClass(value: String): Self = StObject.set(x, "checkboxClass", value.asInstanceOf[js.Any])
    
    inline def setCheckboxClassUndefined: Self = StObject.set(x, "checkboxClass", js.undefined)
    
    inline def setCheckedClass(value: String): Self = StObject.set(x, "checkedClass", value.asInstanceOf[js.Any])
    
    inline def setCheckedClassUndefined: Self = StObject.set(x, "checkedClass", js.undefined)
    
    inline def setDisabledClass(value: String): Self = StObject.set(x, "disabledClass", value.asInstanceOf[js.Any])
    
    inline def setDisabledClassUndefined: Self = StObject.set(x, "disabledClass", js.undefined)
    
    inline def setEventNamespace(value: String): Self = StObject.set(x, "eventNamespace", value.asInstanceOf[js.Any])
    
    inline def setEventNamespaceUndefined: Self = StObject.set(x, "eventNamespace", js.undefined)
    
    inline def setFileButtonClass(value: String): Self = StObject.set(x, "fileButtonClass", value.asInstanceOf[js.Any])
    
    inline def setFileButtonClassUndefined: Self = StObject.set(x, "fileButtonClass", js.undefined)
    
    inline def setFileButtonHtml(value: String): Self = StObject.set(x, "fileButtonHtml", value.asInstanceOf[js.Any])
    
    inline def setFileButtonHtmlUndefined: Self = StObject.set(x, "fileButtonHtml", js.undefined)
    
    inline def setFileClass(value: String): Self = StObject.set(x, "fileClass", value.asInstanceOf[js.Any])
    
    inline def setFileClassUndefined: Self = StObject.set(x, "fileClass", js.undefined)
    
    inline def setFileDefaultHtml(value: String): Self = StObject.set(x, "fileDefaultHtml", value.asInstanceOf[js.Any])
    
    inline def setFileDefaultHtmlUndefined: Self = StObject.set(x, "fileDefaultHtml", js.undefined)
    
    inline def setFilenameClass(value: String): Self = StObject.set(x, "filenameClass", value.asInstanceOf[js.Any])
    
    inline def setFilenameClassUndefined: Self = StObject.set(x, "filenameClass", js.undefined)
    
    inline def setFocusClass(value: String): Self = StObject.set(x, "focusClass", value.asInstanceOf[js.Any])
    
    inline def setFocusClassUndefined: Self = StObject.set(x, "focusClass", js.undefined)
    
    inline def setHoverClass(value: String): Self = StObject.set(x, "hoverClass", value.asInstanceOf[js.Any])
    
    inline def setHoverClassUndefined: Self = StObject.set(x, "hoverClass", js.undefined)
    
    inline def setIdPrefix(value: String): Self = StObject.set(x, "idPrefix", value.asInstanceOf[js.Any])
    
    inline def setIdPrefixUndefined: Self = StObject.set(x, "idPrefix", js.undefined)
    
    inline def setInputAddTypeAsClass(value: Boolean): Self = StObject.set(x, "inputAddTypeAsClass", value.asInstanceOf[js.Any])
    
    inline def setInputAddTypeAsClassUndefined: Self = StObject.set(x, "inputAddTypeAsClass", js.undefined)
    
    inline def setInputClass(value: String): Self = StObject.set(x, "inputClass", value.asInstanceOf[js.Any])
    
    inline def setInputClassUndefined: Self = StObject.set(x, "inputClass", js.undefined)
    
    inline def setRadioClass(value: String): Self = StObject.set(x, "radioClass", value.asInstanceOf[js.Any])
    
    inline def setRadioClassUndefined: Self = StObject.set(x, "radioClass", js.undefined)
    
    inline def setResetDefaultHtml(value: String): Self = StObject.set(x, "resetDefaultHtml", value.asInstanceOf[js.Any])
    
    inline def setResetDefaultHtmlUndefined: Self = StObject.set(x, "resetDefaultHtml", js.undefined)
    
    inline def setResetSelector(value: Any): Self = StObject.set(x, "resetSelector", value.asInstanceOf[js.Any])
    
    inline def setResetSelectorUndefined: Self = StObject.set(x, "resetSelector", js.undefined)
    
    inline def setSelectAutoWidth(value: Boolean): Self = StObject.set(x, "selectAutoWidth", value.asInstanceOf[js.Any])
    
    inline def setSelectAutoWidthUndefined: Self = StObject.set(x, "selectAutoWidth", js.undefined)
    
    inline def setSelectClass(value: String): Self = StObject.set(x, "selectClass", value.asInstanceOf[js.Any])
    
    inline def setSelectClassUndefined: Self = StObject.set(x, "selectClass", js.undefined)
    
    inline def setSelectMultiClass(value: String): Self = StObject.set(x, "selectMultiClass", value.asInstanceOf[js.Any])
    
    inline def setSelectMultiClassUndefined: Self = StObject.set(x, "selectMultiClass", js.undefined)
    
    inline def setSubmitDefaultHtml(value: String): Self = StObject.set(x, "submitDefaultHtml", value.asInstanceOf[js.Any])
    
    inline def setSubmitDefaultHtmlUndefined: Self = StObject.set(x, "submitDefaultHtml", js.undefined)
    
    inline def setTextareaClass(value: String): Self = StObject.set(x, "textareaClass", value.asInstanceOf[js.Any])
    
    inline def setTextareaClassUndefined: Self = StObject.set(x, "textareaClass", js.undefined)
    
    inline def setUseID(value: Boolean): Self = StObject.set(x, "useID", value.asInstanceOf[js.Any])
    
    inline def setUseIDUndefined: Self = StObject.set(x, "useID", js.undefined)
    
    inline def setWrapperClass(value: String): Self = StObject.set(x, "wrapperClass", value.asInstanceOf[js.Any])
    
    inline def setWrapperClassUndefined: Self = StObject.set(x, "wrapperClass", js.undefined)
  }
}
