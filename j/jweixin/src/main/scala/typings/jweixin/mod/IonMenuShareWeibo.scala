...
trait IonMenuShareWeibo extends BaseParams {
  ...
  // 用户确认分享后执行的回调函数
  @JSName("success")
  def success_MIonMenuShareWeibo(): Unit = js.native
...
object IonMenuShareWeibo {
  ...
  @scala.inline
  def apply(cancel: () => Unit, desc: String, imgUrl: String, link: String, success: () => Unit, title: String): IonMenuShareWeibo = {
  ...
  @scala.inline
  implicit class IonMenuShareWeiboMutableBuilder[Self <: IonMenuShareWeibo] (val x: Self) extends AnyVal {
