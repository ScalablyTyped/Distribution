...
object ICodeCellModel {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(contentChanged = contentChanged, id = id, metadata = metadata, outputs = outputs, stateChanged = stateChanged, toJSON = js.Any.fromFunction0(toJSON), trusted = trusted)
