...
import typings.ipfsCoreTypes.ipfsCoreTypesStrings.ADDING_PEER
...
trait AddingPeerEvent
  ...
  var `type`: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.ADDING_PEER
}
object AddingPeerEvent {
  
  inline def apply(peer: PeerId, `type`: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.ADDING_PEER): AddingPeerEvent = {
  ...
  implicit open class MutableBuilder[Self <: AddingPeerEvent] (val x: Self) extends AnyVal {
    ...
    inline def setType(value: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.ADDING_PEER): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
