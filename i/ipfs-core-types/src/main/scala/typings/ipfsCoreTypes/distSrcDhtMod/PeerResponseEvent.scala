...
import typings.ipfsCoreTypes.ipfsCoreTypesStrings.PEER_RESPONSE
...
trait PeerResponseEvent
  ...
  var `type`: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.PEER_RESPONSE
}
object PeerResponseEvent {
  ...
  inline def apply(
    ...
    `type`: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.PEER_RESPONSE
  ): PeerResponseEvent = {
  ...
  extension [Self <: PeerResponseEvent](x: Self) {
    ...
    inline def setType(value: typings.ipfsCoreTypes.distSrcDhtMod.EventTypes.PEER_RESPONSE): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
