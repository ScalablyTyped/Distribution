package typings.ipp.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typings.ipp.ippStrings.aborted
  - typings.ipp.ippStrings.all
  - typings.ipp.ippStrings.canceled
  - typings.ipp.ippStrings.completed
  - typings.ipp.ippStrings.`not-completed`
  - typings.ipp.ippStrings.pending
  - typings.ipp.ippStrings.`pending-held`
  - typings.ipp.ippStrings.processing
  - typings.ipp.ippStrings.`processing-stopped`
  - typings.ipp.ippStrings.`proof-print`
  - typings.ipp.ippStrings.saved
*/
trait WhichJobs extends js.Object

object WhichJobs {
  @scala.inline
  def aborted: typings.ipp.ippStrings.aborted = "aborted".asInstanceOf[typings.ipp.ippStrings.aborted]
  @scala.inline
  def all: typings.ipp.ippStrings.all = "all".asInstanceOf[typings.ipp.ippStrings.all]
  @scala.inline
  def canceled: typings.ipp.ippStrings.canceled = "canceled".asInstanceOf[typings.ipp.ippStrings.canceled]
  @scala.inline
  def completed: typings.ipp.ippStrings.completed = "completed".asInstanceOf[typings.ipp.ippStrings.completed]
  @scala.inline
  def `not-completed`: typings.ipp.ippStrings.`not-completed` = "not-completed".asInstanceOf[typings.ipp.ippStrings.`not-completed`]
  @scala.inline
  def pending: typings.ipp.ippStrings.pending = "pending".asInstanceOf[typings.ipp.ippStrings.pending]
  @scala.inline
  def `pending-held`: typings.ipp.ippStrings.`pending-held` = "pending-held".asInstanceOf[typings.ipp.ippStrings.`pending-held`]
  @scala.inline
  def processing: typings.ipp.ippStrings.processing = "processing".asInstanceOf[typings.ipp.ippStrings.processing]
  @scala.inline
  def `processing-stopped`: typings.ipp.ippStrings.`processing-stopped` = "processing-stopped".asInstanceOf[typings.ipp.ippStrings.`processing-stopped`]
  @scala.inline
  def `proof-print`: typings.ipp.ippStrings.`proof-print` = "proof-print".asInstanceOf[typings.ipp.ippStrings.`proof-print`]
  @scala.inline
  def saved: typings.ipp.ippStrings.saved = "saved".asInstanceOf[typings.ipp.ippStrings.saved]
}

