...
/* Inlined std.Omit<ioredis.ioredis/built/redis/RedisOptions.RedisOptions, 'port' | 'host' | 'path' | 'sentinels' | 'retryStrategy' | 'enableOfflineQueue' | 'readOnly'> */
...
object OmitRedisOptionsporthostp {
  ...
  inline def apply(): OmitRedisOptionsporthostp = {
