package typings
package igniteDashUiLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._


trait IgVideoPlayerLocale
  extends /**
	 * Option for IgVideoPlayerLocale
	 */
/* optionName */ org.scalablytyped.runtime.StringDictionary[js.Any] {
  /**
  	 * Gets/Sets ad message text.
  	 *
  	 */
  var adMessage: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets long ad message text.
  	 *
  	 */
  var adMessageLong: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets ad message text when no duration is specified.
  	 *
  	 */
  var adMessageNoDuration: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets new ad window title.
  	 *
  	 */
  var adNewWindowTip: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets buffering label text.
  	 *
  	 */
  var buffering: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets enter fullscreen button title.
  	 *
  	 */
  var enterFullscreen: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets exit fullscreen button title.
  	 *
  	 */
  var exitFullscreen: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets live video title.
  	 *
  	 */
  var live: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets live stream video title.
  	 *
  	 */
  var liveStream: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets paused button title.
  	 *
  	 */
  var paused: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets play button title.
  	 *
  	 */
  var play: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets playing button title.
  	 *
  	 */
  var playing: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets progress label long format.
  	 *
  	 */
  var progressLabelLongFormat: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets progress label short format.
  	 *
  	 */
  var progressLabelShortFormat: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets related videos text.
  	 *
  	 */
  var relatedVideos: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets replay button text.
  	 *
  	 */
  var replayButton: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets replay button tooltip.
  	 *
  	 */
  var replayTooltip: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets skip to button title.
  	 *
  	 */
  var skipTo: js.UndefOr[scala.Boolean] = js.undefined
  /**
  	 * Gets/Sets volume button title.
  	 *
  	 */
  var volume: js.UndefOr[scala.Boolean] = js.undefined
}

