package typings
package igniteDashUiLib.InfragisticsNs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("Infragistics.Level")
@js.native
class Level () extends js.Object {
  /**
  	 * Returns the caption of the level used when displaying the name of the level to the user.
  	 *
  	 * @param value
  	 */
  def caption(value: js.Object): java.lang.String = js.native
  /**
  	 * Returns the distance of the level from the root of the level. Root level is zero (0)
  	 *
  	 * @param value
  	 */
  def depth(value: js.Object): scala.Double = js.native
  /**
  	 * Returns a user-friendly description of the level.
  	 *
  	 * @param value
  	 */
  def description(value: js.Object): java.lang.String = js.native
  /**
  	 * Returns the unique name of the dimension that contains the level.
  	 *
  	 * @param value
  	 */
  def dimensionUniqueName(value: js.Object): java.lang.String = js.native
  /**
  	 * Returns the unique name of the hierarchy that contains the level.
  	 *
  	 * @param value
  	 */
  def hierarchyUniqueName(value: js.Object): java.lang.String = js.native
  /**
  	 * Returns the ID of the attribute that the level is sorted on.
  	 *
  	 * @param value
  	 */
  def levelOrderingProperty(value: js.Object): scala.Double = js.native
  /**
  	 * Returns a value that defines how the level was sourced.
  	 *
  	 * @param value
  	 */
  def levelOrigin(value: js.Object): scala.Double = js.native
  /**
  	 * Returns the count of all members in the level.
  	 *
  	 * @param value
  	 */
  def membersCount(value: js.Object): scala.Double = js.native
  /**
  	 * Returns the name of the level.
  	 *
  	 * @param value
  	 */
  def name(value: js.Object): java.lang.String = js.native
  /**
  	 * Returns the unique name of the level.
  	 *
  	 * @param value
  	 */
  def uniqueName(value: js.Object): java.lang.String = js.native
}

