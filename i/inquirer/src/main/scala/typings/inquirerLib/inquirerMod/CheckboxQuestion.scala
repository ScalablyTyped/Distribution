...
- Dropped {[ P in 'choices' | 'filter' | 'pageSize' ]: inquirer.inquirer.QuestionOptions<A>[P]} */ trait CheckboxQuestion[A]
  extends QuestionCommon[A]
     with Question[A] {
  var `type`: inquirerLib.inquirerLibStrings.checkbox
  /**
...
object CheckboxQuestion {
  @scala.inline
  def apply[A](
    `type`: inquirerLib.inquirerLibStrings.checkbox,
    ...
    __obj.asInstanceOf[CheckboxQuestion[A]]
  }
}
