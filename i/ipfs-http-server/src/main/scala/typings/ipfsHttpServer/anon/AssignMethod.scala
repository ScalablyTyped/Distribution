...
trait AssignMethod extends StObject {
  ...
  /**
    ...
    */
  def method(request: Request[ReqRefDefaults], _h: ResponseToolkit[ReqRefDefaults]): js.Promise[Config]
}
object AssignMethod {
  
  inline def apply(
    assign: String,
    method: (Request[ReqRefDefaults], ResponseToolkit[ReqRefDefaults]) => js.Promise[Config]
  ): AssignMethod = {
  ...
  implicit open class MutableBuilder[Self <: AssignMethod] (val x: Self) extends AnyVal {
    ...
    inline def setMethod(value: (Request[ReqRefDefaults], ResponseToolkit[ReqRefDefaults]) => js.Promise[Config]): Self = StObject.set(x, "method", js.Any.fromFunction2(value))
  }
}
