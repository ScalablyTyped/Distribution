...
object CreateUpdateUser {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(anonymous = anonymous, avatar = avatar, companies = companies, created_at = created_at, custom_attributes = custom_attributes, id = id, session_count = session_count, unsubscribed_from_emails = unsubscribed_from_emails, updated_at = updated_at)
