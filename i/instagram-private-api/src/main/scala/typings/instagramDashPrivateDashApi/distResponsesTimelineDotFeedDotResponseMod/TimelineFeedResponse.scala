...
object TimelineFeedResponse {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(auto_load_more_enabled = auto_load_more_enabled, client_feed_changelist_applied = client_feed_changelist_applied, client_gap_enforcer_matrix = client_gap_enforcer_matrix, client_session_id = client_session_id, feed_items = feed_items, feed_pill_text = feed_pill_text, is_direct_v2_enabled = is_direct_v2_enabled, more_available = more_available, next_max_id = next_max_id, num_results = num_results, pagination_info = pagination_info, status = status, view_state_version = view_state_version)
