...
object TimelineFeedResponseMedia_or_ad {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(can_view_more_preview_comments = can_view_more_preview_comments, can_viewer_save = can_viewer_save, caption = caption, caption_is_edited = caption_is_edited, client_cache_key = client_cache_key, code = code, comment_count = comment_count, comment_likes_enabled = comment_likes_enabled, comment_threading_enabled = comment_threading_enabled, device_timestamp = device_timestamp.asInstanceOf[js.Any], direct_reply_to_author_enabled = direct_reply_to_author_enabled, filter_type = filter_type, has_liked = has_liked, has_more_comments = has_more_comments, id = id, inventory_source = inventory_source, is_eof = is_eof, is_seen = is_seen, like_count = like_count, max_num_visible_preview_comments = max_num_visible_preview_comments, media_type = media_type, organic_tracking_token = organic_tracking_token, photo_of_you = photo_of_you, pk = pk, preview_comments = preview_comments, taken_at = taken_at, top_likers = top_likers, user = user)
    ...
    if (ad_id != null) __obj.updateDynamic("ad_id")(ad_id)
    ...
    if (!js.isUndefined(can_viewer_reshare)) __obj.updateDynamic("can_viewer_reshare")(can_viewer_reshare)
    ...
    if (!js.isUndefined(collapse_comments)) __obj.updateDynamic("collapse_comments")(collapse_comments)
    ...
    if (fb_page_url != null) __obj.updateDynamic("fb_page_url")(fb_page_url)
    ...
    if (iTunesItem != null) __obj.updateDynamic("iTunesItem")(iTunesItem)
    if (image_versions2 != null) __obj.updateDynamic("image_versions2")(image_versions2)
    if (injected != null) __obj.updateDynamic("injected")(injected)
    if (inline_composer_display_condition != null) __obj.updateDynamic("inline_composer_display_condition")(inline_composer_display_condition)
    ...
    if (!js.isUndefined(is_sidecar_child)) __obj.updateDynamic("is_sidecar_child")(is_sidecar_child)
