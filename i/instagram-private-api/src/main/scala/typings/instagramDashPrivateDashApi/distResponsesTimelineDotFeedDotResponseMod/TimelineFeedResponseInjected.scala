...
object TimelineFeedResponseInjected {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(about_ad_params = about_ad_params, ad_id = ad_id, ad_title = ad_title, cookies = cookies, direct_share = direct_share, display_viewability_eligible = display_viewability_eligible, hide_flow_type = hide_flow_type, hide_label = hide_label, hide_reasons_v2 = hide_reasons_v2, invalidation = invalidation, is_demo = is_demo, is_holdout = is_holdout, is_leadgen_native_eligible = is_leadgen_native_eligible, label = label, lead_gen_form_id = lead_gen_form_id.asInstanceOf[js.Any], show_ad_choices = show_ad_choices, show_icon = show_icon, tracking_token = tracking_token, view_tags = view_tags)
