...
object DirectRepositoryRankedRecipientsResponseUser {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(full_name = full_name, has_anonymous_profile_picture = has_anonymous_profile_picture, is_directapp_installed = is_directapp_installed, is_private = is_private, is_using_unified_inbox_for_direct = is_using_unified_inbox_for_direct, is_verified = is_verified, pk = pk, profile_pic_id = profile_pic_id, profile_pic_url = profile_pic_url, username = username)
