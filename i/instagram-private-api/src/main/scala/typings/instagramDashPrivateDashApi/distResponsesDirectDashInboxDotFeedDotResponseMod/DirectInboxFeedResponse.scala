...
object DirectInboxFeedResponse {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(inbox = inbox, most_recent_inviter = most_recent_inviter, pending_requests_total = pending_requests_total, seq_id = seq_id, snapshot_at_ms = snapshot_at_ms, status = status)
