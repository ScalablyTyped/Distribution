...
object UserRepositoryInfoResponseUser {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(account_type = account_type, aggregate_promote_engagement = aggregate_promote_engagement, allowed_commenter_type = allowed_commenter_type, auto_expand_chaining = auto_expand_chaining, besties_count = besties_count, biography = biography, biography_with_entities = biography_with_entities, can_be_reported_as_fraud = can_be_reported_as_fraud, can_be_tagged_as_sponsor = can_be_tagged_as_sponsor, can_boost_post = can_boost_post, can_convert_to_business = can_convert_to_business, can_create_sponsor_tags = can_create_sponsor_tags, can_follow_hashtag = can_follow_hashtag, can_link_entities_in_bio = can_link_entities_in_bio, can_see_organic_insights = can_see_organic_insights, can_tag_products_from_merchants = can_tag_products_from_merchants, external_url = external_url, feed_post_reshare_disabled = feed_post_reshare_disabled, follower_count = follower_count, following_count = following_count, following_tag_count = following_tag_count, full_name = full_name, geo_media_count = geo_media_count, has_anonymous_profile_picture = has_anonymous_profile_picture, has_biography_translation = has_biography_translation, has_chaining = has_chaining, has_highlight_reels = has_highlight_reels, has_placed_orders = has_placed_orders, has_profile_video_feed = has_profile_video_feed, has_recommend_accounts = has_recommend_accounts, hd_profile_pic_url_info = hd_profile_pic_url_info, hd_profile_pic_versions = hd_profile_pic_versions, highlight_reshare_disabled = highlight_reshare_disabled, include_direct_blacklist_status = include_direct_blacklist_status, is_business = is_business, is_call_to_action_enabled = is_call_to_action_enabled, is_eligible_to_show_fb_cross_sharing_nux = is_eligible_to_show_fb_cross_sharing_nux, is_interest_account = is_interest_account, is_needy = is_needy, is_potential_business = is_potential_business, is_private = is_private, is_profile_action_needed = is_profile_action_needed, is_verified = is_verified, is_video_creator = is_video_creator, media_count = media_count, nametag = nametag, page_id_for_new_suma_biz_account = page_id_for_new_suma_biz_account, pk = pk, profile_pic_id = profile_pic_id, profile_pic_url = profile_pic_url, recently_bestied_by_count = recently_bestied_by_count, reel_auto_archive = reel_auto_archive, show_besties_badge = show_besties_badge, show_business_conversion_icon = show_business_conversion_icon, show_conversion_edit_entry = show_conversion_edit_entry, show_insights_terms = show_insights_terms, total_ar_effects = total_ar_effects, total_igtv_videos = total_igtv_videos, username = username, usertag_review_enabled = usertag_review_enabled, usertags_count = usertags_count)
