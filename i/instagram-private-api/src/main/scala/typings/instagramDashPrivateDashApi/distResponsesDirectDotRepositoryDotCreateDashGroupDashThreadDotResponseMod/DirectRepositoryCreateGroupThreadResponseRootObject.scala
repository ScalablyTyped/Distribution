...
object DirectRepositoryCreateGroupThreadResponseRootObject {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(admin_user_ids = admin_user_ids, approval_required_for_new_members = approval_required_for_new_members, archived = archived, business_thread_folder = business_thread_folder, canonical = canonical, folder = folder, has_newer = has_newer, has_older = has_older, input_mode = input_mode, inviter = inviter, is_group = is_group, is_pin = is_pin, items = items, last_activity_at = last_activity_at, last_seen_at = last_seen_at, left_users = left_users, mentions_muted = mentions_muted, muted = muted, named = named, next_cursor = next_cursor, pending = pending, pending_score = pending_score, prev_cursor = prev_cursor, read_state = read_state, status = status, thread_id = thread_id, thread_title = thread_title, thread_type = thread_type, thread_v2_id = thread_v2_id, users = users, valued_request = valued_request, vc_muted = vc_muted, viewer_id = viewer_id)
