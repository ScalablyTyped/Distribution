...
object TagFeedResponseStory {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(can_reply = can_reply, can_reshare = can_reshare, expiring_at = expiring_at, id = id, items = items, latest_reel_media = latest_reel_media, muted = muted, owner = owner, prefetch_count = prefetch_count, reel_type = reel_type, seen = seen, unique_integer_reel_id = unique_integer_reel_id)
