package typings
package istanbulDashLibDashSourceDashMapsLib.istanbulDashLibDashSourceDashMapsMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("istanbul-lib-source-maps", JSImport.Namespace)
@js.native
object istanbulDashLibDashSourceDashMapsModMembers extends js.Object {
  def createSourceMapStore(): MapStore = js.native
  def createSourceMapStore(options: stdLib.Partial[MapStoreOptions]): MapStore = js.native
}

