...
- highchartsLib.highchartsMod.HighchartsNs.IndividualSeriesOptions because var conflicts: color, cursor, innerSize, maxFontSize, minFontSize, size, visible. Inlined `type`, colorIndex, crisp, data, description, id, index, legendIndex, name, pointWidth, stack, xAxis, yAxis, zIndex */ trait PieChartSeriesOptions extends PieChart {
  /**
