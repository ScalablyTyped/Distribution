...
- highchartsLib.highchartsMod.IndividualSeriesOptions because var conflicts: color, cursor, linkedTo, maxFontSize, minFontSize, visible. Inlined size, innerSize, `type`, colorIndex, crisp, data, description, id, index, legendIndex, name, pointWidth, stack, xAxis, yAxis, zIndex */ trait AreaChartSeriesOptions extends AreaChart {
  /**
