...
- typings.highcharts.highchartsMod.PlotOrganizationOptions because var conflicts: zIndex. Inlined accessibility, allAreas, animation, boostBlending, borderColor, borderRadius, borderWidth, className, clip, color, colorAxis, colorByPoint, colorIndex, colors, compare, compareBase, compareStart, connectors, cursor, dataGrouping, dataLabels, description, dragDrop, enableMouseTracking, events, exposeElementToA11y, getExtremesFromAll, hangingIndent, includeInDataExport, joinBy, keys, label, lastPrice, lastVisiblePrice, levels, linkColor, linkedTo, linkLineWidth, linkOpacity, linkRadius, minPointLength, navigatorOptions, nodePadding, nodeWidth, opacity, point, pointDescriptionFormatter, selected, showCheckbox, showInLegend, showInNavigator, skipKeyboardNavigation, states, stickyTracking, tooltip, turboThreshold, visible */ trait SeriesOrganizationOptions
  ...
  /**
    ...
    */
  @JSName("type")
  var type_SeriesOrganizationOptions: organization
  /**
...
object SeriesOrganizationOptions {
  ...
  def apply(
    ...
    if (!js.isUndefined(curveFactor)) __obj.updateDynamic("curveFactor")(curveFactor)
