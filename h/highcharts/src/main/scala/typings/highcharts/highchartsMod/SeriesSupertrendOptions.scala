...
- typings.highcharts.highchartsMod.PlotSupertrendOptions because var conflicts: name, zIndex. Inlined accessibility, allowPointSelect, animation, animationLimit, boostBlending, boostThreshold, borderColor, borderWidth, changeTrendLine, className, clip, colorIndex, compare, compareBase, compareStart, compareToMain, connectEnds, connectNulls, connectors, cursor, dashStyle, dataGrouping, dataLabels, description, dragDrop, enableMouseTracking, events, exposeElementToA11y, fallingTrendColor, findNearestPointBy, gapSize, gapUnit, getExtremesFromAll, includeInDataExport, label, lastPrice, lastVisiblePrice, linecap, lineWidth, linkedTo, marker, opacity, params, point, pointDescriptionFormatter, risingTrendColor, selected, shadow, showCheckbox, showInLegend, skipKeyboardNavigation, softThreshold, states, step, stickyTracking, tooltip, turboThreshold, visible, zoneAxis, zones */ trait SeriesSupertrendOptions
  ...
  /**
    ...
    */
  @JSName("type")
  var type_SeriesSupertrendOptions: supertrend
  /**
...
object SeriesSupertrendOptions {
  ...
  def apply(
    ...
    if (changeTrendLine != null) __obj.updateDynamic("changeTrendLine")(changeTrendLine)
    ...
    if (risingTrendColor != null) __obj.updateDynamic("risingTrendColor")(risingTrendColor)
