...
- typings.highcharts.highchartsMod.SeriesOptions because var conflicts: zIndex. Inlined id, index, legendIndex, mapData, name, stack, treemap, `type`, xAxis, yAxis */ trait SeriesColumnrangeOptions
...
object SeriesColumnrangeOptions {
  ...
  def apply(
    ...
    if (pointRange != null) __obj.updateDynamic("pointRange")(pointRange)
