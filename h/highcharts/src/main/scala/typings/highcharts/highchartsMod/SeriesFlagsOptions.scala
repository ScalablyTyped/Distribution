...
- typings.highcharts.highchartsMod.PlotFlagsOptions because var conflicts: zIndex. Inlined accessibility, allAreas, allowOverlapX, allowPointSelect, animationLimit, boostBlending, boostThreshold, className, clip, color, colorAxis, colorIndex, colors, compare, compareBase, compareStart, connectors, crisp, cropThreshold, cursor, dataLabels, depth, description, dragDrop, edgeColor, edgeWidth, enableMouseTracking, events, exposeElementToA11y, fillColor, findNearestPointBy, getExtremesFromAll, grouping, groupPadding, groupZPadding, height, includeInDataExport, joinBy, keys, label, lastPrice, lastVisiblePrice, lineColor, lineWidth, linkedTo, maxPointWidth, minPointLength, navigatorOptions, negativeColor, onKey, onSeries, opacity, point, pointDescriptionFormatter, pointInterval, pointIntervalUnit, pointPlacement, pointRange, pointStart, selected, shadow, shape, showCheckbox, showInLegend, showInNavigator, skipKeyboardNavigation, softThreshold, stackDistance, stacking, states, stickyTracking, style, textAlign, threshold, title, tooltip, useHTML, visible, width, y, zoneAxis, zones */ trait SeriesFlagsOptions
  ...
  /**
    ...
    */
  @JSName("type")
  var type_SeriesFlagsOptions: flags
  /**
...
object SeriesFlagsOptions {
  ...
  def apply(
    ...
    if (!js.isUndefined(allowOverlapX)) __obj.updateDynamic("allowOverlapX")(allowOverlapX)
    ...
    if (onKey != null) __obj.updateDynamic("onKey")(onKey)
