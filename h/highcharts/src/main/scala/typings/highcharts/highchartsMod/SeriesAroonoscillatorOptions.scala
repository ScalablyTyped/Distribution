...
- typings.highcharts.highchartsMod.PlotAroonoscillatorOptions because var conflicts: name, zIndex. Inlined accessibility, allowPointSelect, animation, animationLimit, boostBlending, boostThreshold, borderColor, borderWidth, className, clip, color, colorIndex, compareStart, compareToMain, connectEnds, connectNulls, connectors, cropThreshold, cursor, dashStyle, dataGrouping, dataLabels, description, dragDrop, enableMouseTracking, events, exposeElementToA11y, findNearestPointBy, gapSize, gapUnit, getExtremesFromAll, includeInDataExport, label, lastPrice, lastVisiblePrice, linecap, lineWidth, linkedTo, marker, negativeColor, opacity, params, point, pointDescriptionFormatter, selected, shadow, showCheckbox, showInLegend, skipKeyboardNavigation, softThreshold, states, step, stickyTracking, threshold, tooltip, turboThreshold, visible, zoneAxis, zones */ trait SeriesAroonoscillatorOptions
  ...
  /**
    ...
    */
  @JSName("type")
  var type_SeriesAroonoscillatorOptions: aroonoscillator
  /**
...
object SeriesAroonoscillatorOptions {
  ...
  def apply(
    ...
    if (!js.isUndefined(aroonDown)) __obj.updateDynamic("aroonDown")(aroonDown)
