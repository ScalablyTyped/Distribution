...
object SeriesPpoOptions {
  ...
  inline def apply(dataParser: Unit, dataURL: Unit, `type`: String | ppo): SeriesPpoOptions = {
  ...
  implicit open class MutableBuilder[Self <: SeriesPpoOptions] (val x: Self) extends AnyVal {
    ...
    inline def setType(value: String | ppo): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
