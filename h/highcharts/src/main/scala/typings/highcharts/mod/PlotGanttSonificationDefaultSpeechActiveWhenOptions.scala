...
trait PlotGanttSonificationDefaultSpeechActiveWhenOptions extends StObject {
  
  /**
    * (Gantt) Track is only active when `prop` was above, and is now at or
    * below this value.
    *
  ...
  /**
    * (Gantt) Track is only active when `prop` was below, and is now at or
    * above this value.
    *
    * If both `crossingUp` and `crossingDown` are defined, the track is active
    * if either condition is met.
  ...
  /**
    * (Gantt) Track is only active when `prop` is below or at this value.
    */
  var max: js.UndefOr[Double] = js.undefined
  
  /**
    * (Gantt) Track is only active when `prop` is above or at this value.
    */
  var min: js.UndefOr[Double] = js.undefined
  
  /**
    * (Gantt) The point property to compare, for example `y` or `x`.
    */
  var prop: js.UndefOr[String] = js.undefined
}
object PlotGanttSonificationDefaultSpeechActiveWhenOptions {
  
  inline def apply(): PlotGanttSonificationDefaultSpeechActiveWhenOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[PlotGanttSonificationDefaultSpeechActiveWhenOptions]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: PlotGanttSonificationDefaultSpeechActiveWhenOptions] (val x: Self) extends AnyVal {
