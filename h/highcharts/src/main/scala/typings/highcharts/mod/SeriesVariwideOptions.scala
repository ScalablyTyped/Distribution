...
- typings.highcharts.mod.SeriesOptions because var conflicts: zIndex. Inlined id, index, legendIndex, mapData, name, stack, `type`, xAxis, yAxis */ trait SeriesVariwideOptions
  extends StObject
     with PlotVariwideOptions {
...
object SeriesVariwideOptions {
  ...
  inline def apply(`type`: String | variwide): SeriesVariwideOptions = {
  ...
  implicit open class MutableBuilder[Self <: SeriesVariwideOptions] (val x: Self) extends AnyVal {
    ...
    inline def setType(value: String | variwide): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
