...
trait PlotXrangeOptions extends StObject {
  ...
  /**
    * (Highcharts, Highstock, Gantt) Set the point threshold for when a series
  ...
  /**
    ...
    */
  var dataSorting: js.UndefOr[DataSortingOptionsObject | PlotXrangeDataSortingOptions] = js.undefined
  ...
  /**
    * (Highcharts, Highstock, Gantt) Determines whether the series should look
    * for the nearest point in both dimensions or just the x-dimension when
    * hovering the series. Defaults to `'xy'` for scatter series and `'x'` for
    * most other series. If the data has duplicate x-values, it is recommended
  ...
  /**
    * (Highcharts, Highstock, Gantt) Options for the point markers of line-like
  ...
  /**
    * (Highcharts, Highstock, Gantt) The color for the parts of the graph or
    * points that are below the threshold. Note that `zones` takes precedence
    * over the negative color. Using `negativeColor` is equivalent to applying
    * a zone with value of 0.
    */
...
object PlotXrangeOptions {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: PlotXrangeOptions] (val x: Self) extends AnyVal {
    ...
    inline def setDataSorting(value: DataSortingOptionsObject | PlotXrangeDataSortingOptions): Self = StObject.set(x, "dataSorting", value.asInstanceOf[js.Any])
