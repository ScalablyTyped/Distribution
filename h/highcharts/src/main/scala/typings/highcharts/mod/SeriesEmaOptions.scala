...
- typings.highcharts.mod.SeriesOptions because var conflicts: name, zIndex. Inlined id, index, legendIndex, mapData, stack, `type`, xAxis, yAxis */ trait SeriesEmaOptions
  extends StObject
     with PlotEmaOptions {
...
object SeriesEmaOptions {
  ...
  inline def apply(dataParser: Unit, dataURL: Unit, `type`: String | ema): SeriesEmaOptions = {
  ...
  implicit open class MutableBuilder[Self <: SeriesEmaOptions] (val x: Self) extends AnyVal {
    ...
    inline def setType(value: String | ema): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
