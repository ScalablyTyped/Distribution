...
object NavigatorXAxisPlotBandsEventsOptions {
  ...
  inline def apply(): NavigatorXAxisPlotBandsEventsOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[NavigatorXAxisPlotBandsEventsOptions]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: NavigatorXAxisPlotBandsEventsOptions] (val x: Self) extends AnyVal {
