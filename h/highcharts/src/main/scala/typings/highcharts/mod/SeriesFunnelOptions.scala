...
- typings.highcharts.mod.SeriesOptions because var conflicts: zIndex. Inlined id, index, legendIndex, mapData, name, stack, `type`, xAxis, yAxis */ trait SeriesFunnelOptions
  extends StObject
     with PlotFunnelOptions {
...
object SeriesFunnelOptions {
  ...
  inline def apply(dataParser: Unit, dataURL: Unit, `type`: String | funnel): SeriesFunnelOptions = {
  ...
  extension [Self <: SeriesFunnelOptions](x: Self) {
    ...
    inline def setType(value: String | funnel): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
