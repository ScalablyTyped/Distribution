package typings.microsoftGraph.mod

import typings.microsoftGraph.microsoftGraphStrings.unknown_
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.microsoftGraph.microsoftGraphStrings.unknown_
  - typings.microsoftGraph.microsoftGraphStrings.pending
  - typings.microsoftGraph.microsoftGraphStrings.partial
  - typings.microsoftGraph.microsoftGraphStrings.complete
  - typings.microsoftGraph.microsoftGraphStrings.error
*/
trait ImportedWindowsAutopilotDeviceIdentityImportStatus extends StObject
object ImportedWindowsAutopilotDeviceIdentityImportStatus {
  
  inline def complete: typings.microsoftGraph.microsoftGraphStrings.complete = "complete".asInstanceOf[typings.microsoftGraph.microsoftGraphStrings.complete]
  
  inline def error: typings.microsoftGraph.microsoftGraphStrings.error = "error".asInstanceOf[typings.microsoftGraph.microsoftGraphStrings.error]
  
  inline def partial: typings.microsoftGraph.microsoftGraphStrings.partial = "partial".asInstanceOf[typings.microsoftGraph.microsoftGraphStrings.partial]
  
  inline def pending: typings.microsoftGraph.microsoftGraphStrings.pending = "pending".asInstanceOf[typings.microsoftGraph.microsoftGraphStrings.pending]
  
  inline def unknown: unknown_ = "unknown".asInstanceOf[unknown_]
}
