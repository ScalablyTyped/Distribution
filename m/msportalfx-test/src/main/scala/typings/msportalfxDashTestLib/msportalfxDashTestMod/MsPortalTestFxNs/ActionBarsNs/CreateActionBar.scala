...
object CreateActionBar {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(baseLocator = baseLocator, click = click, createButton = createButton, getAttribute = getAttribute, getLocator = getLocator, getText = getText, isDisplayed = isDisplayed, isElementPresent = isElementPresent, isPresent = isPresent, parentLocator = parentLocator, rightClick = rightClick, sendKeys = sendKeys)
