package typings.maximMazurokGapiClientManagedidentities

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object maximMazurokGapiClientManagedidentitiesStrings {
  
  @js.native
  sealed trait Domain extends StObject
  inline def Domain: Domain = "Domain".asInstanceOf[Domain]
  
  @js.native
  sealed trait GoogleCloudSaasacceleratorManagementProvidersV1Instance extends StObject
  inline def GoogleCloudSaasacceleratorManagementProvidersV1Instance: GoogleCloudSaasacceleratorManagementProvidersV1Instance = "GoogleCloudSaasacceleratorManagementProvidersV1Instance".asInstanceOf[GoogleCloudSaasacceleratorManagementProvidersV1Instance]
  
  @js.native
  sealed trait GoogleCloudSaasacceleratorManagementProvidersV1MaintenanceSettings extends StObject
  inline def GoogleCloudSaasacceleratorManagementProvidersV1MaintenanceSettings: GoogleCloudSaasacceleratorManagementProvidersV1MaintenanceSettings = "GoogleCloudSaasacceleratorManagementProvidersV1MaintenanceSettings".asInstanceOf[GoogleCloudSaasacceleratorManagementProvidersV1MaintenanceSettings]
  
  @js.native
  sealed trait Location extends StObject
  inline def Location: Location = "Location".asInstanceOf[Location]
  
  @js.native
  sealed trait MaintenancePolicy extends StObject
  inline def MaintenancePolicy: MaintenancePolicy = "MaintenancePolicy".asInstanceOf[MaintenancePolicy]
  
  @js.native
  sealed trait Operation extends StObject
  inline def Operation: Operation = "Operation".asInstanceOf[Operation]
  
  @js.native
  sealed trait Status extends StObject
  inline def Status: Status = "Status".asInstanceOf[Status]
  
  @js.native
  sealed trait managedidentities extends StObject
  inline def managedidentities: managedidentities = "managedidentities".asInstanceOf[managedidentities]
  
  @js.native
  sealed trait v1 extends StObject
  inline def v1: v1 = "v1".asInstanceOf[v1]
}
