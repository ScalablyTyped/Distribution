package typings.markdownDashIt

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_Canclose extends js.Object {
  var can_close: Boolean
  var can_open: Boolean
  var length: Double
}

object Anon_Canclose {
  @scala.inline
  def apply(can_close: Boolean, can_open: Boolean, length: Double): Anon_Canclose = {
    val __obj = js.Dynamic.literal(can_close = can_close, can_open = can_open, length = length)
  
    __obj.asInstanceOf[Anon_Canclose]
  }
}

