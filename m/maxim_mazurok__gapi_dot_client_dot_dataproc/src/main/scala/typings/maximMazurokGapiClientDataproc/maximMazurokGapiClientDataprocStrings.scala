package typings.maximMazurokGapiClientDataproc

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object maximMazurokGapiClientDataprocStrings {
  
  @js.native
  sealed trait AutoscalingPolicy extends StObject
  inline def AutoscalingPolicy: AutoscalingPolicy = "AutoscalingPolicy".asInstanceOf[AutoscalingPolicy]
  
  @js.native
  sealed trait Batch extends StObject
  inline def Batch: Batch = "Batch".asInstanceOf[Batch]
  
  @js.native
  sealed trait BatchOperationMetadata extends StObject
  inline def BatchOperationMetadata: BatchOperationMetadata = "BatchOperationMetadata".asInstanceOf[BatchOperationMetadata]
  
  @js.native
  sealed trait Cluster extends StObject
  inline def Cluster: Cluster = "Cluster".asInstanceOf[Cluster]
  
  @js.native
  sealed trait ClusterMetrics extends StObject
  inline def ClusterMetrics: ClusterMetrics = "ClusterMetrics".asInstanceOf[ClusterMetrics]
  
  @js.native
  sealed trait ClusterOperationMetadata extends StObject
  inline def ClusterOperationMetadata: ClusterOperationMetadata = "ClusterOperationMetadata".asInstanceOf[ClusterOperationMetadata]
  
  @js.native
  sealed trait ClusterSelector extends StObject
  inline def ClusterSelector: ClusterSelector = "ClusterSelector".asInstanceOf[ClusterSelector]
  
  @js.native
  sealed trait EndpointConfig extends StObject
  inline def EndpointConfig: EndpointConfig = "EndpointConfig".asInstanceOf[EndpointConfig]
  
  @js.native
  sealed trait GceClusterConfig extends StObject
  inline def GceClusterConfig: GceClusterConfig = "GceClusterConfig".asInstanceOf[GceClusterConfig]
  
  @js.native
  sealed trait HadoopJob extends StObject
  inline def HadoopJob: HadoopJob = "HadoopJob".asInstanceOf[HadoopJob]
  
  @js.native
  sealed trait HiveJob extends StObject
  inline def HiveJob: HiveJob = "HiveJob".asInstanceOf[HiveJob]
  
  @js.native
  sealed trait IdentityConfig extends StObject
  inline def IdentityConfig: IdentityConfig = "IdentityConfig".asInstanceOf[IdentityConfig]
  
  @js.native
  sealed trait InstantiateWorkflowTemplateRequest extends StObject
  inline def InstantiateWorkflowTemplateRequest: InstantiateWorkflowTemplateRequest = "InstantiateWorkflowTemplateRequest".asInstanceOf[InstantiateWorkflowTemplateRequest]
  
  @js.native
  sealed trait Job extends StObject
  inline def Job: Job = "Job".asInstanceOf[Job]
  
  @js.native
  sealed trait JobPlacement extends StObject
  inline def JobPlacement: JobPlacement = "JobPlacement".asInstanceOf[JobPlacement]
  
  @js.native
  sealed trait KubernetesSoftwareConfig extends StObject
  inline def KubernetesSoftwareConfig: KubernetesSoftwareConfig = "KubernetesSoftwareConfig".asInstanceOf[KubernetesSoftwareConfig]
  
  @js.native
  sealed trait LoggingConfig extends StObject
  inline def LoggingConfig: LoggingConfig = "LoggingConfig".asInstanceOf[LoggingConfig]
  
  @js.native
  sealed trait ManagedCluster extends StObject
  inline def ManagedCluster: ManagedCluster = "ManagedCluster".asInstanceOf[ManagedCluster]
  
  @js.native
  sealed trait Operation extends StObject
  inline def Operation: Operation = "Operation".asInstanceOf[Operation]
  
  @js.native
  sealed trait OrderedJob extends StObject
  inline def OrderedJob: OrderedJob = "OrderedJob".asInstanceOf[OrderedJob]
  
  @js.native
  sealed trait PigJob extends StObject
  inline def PigJob: PigJob = "PigJob".asInstanceOf[PigJob]
  
  @js.native
  sealed trait PrestoJob extends StObject
  inline def PrestoJob: PrestoJob = "PrestoJob".asInstanceOf[PrestoJob]
  
  @js.native
  sealed trait PySparkJob extends StObject
  inline def PySparkJob: PySparkJob = "PySparkJob".asInstanceOf[PySparkJob]
  
  @js.native
  sealed trait RuntimeConfig extends StObject
  inline def RuntimeConfig: RuntimeConfig = "RuntimeConfig".asInstanceOf[RuntimeConfig]
  
  @js.native
  sealed trait RuntimeInfo extends StObject
  inline def RuntimeInfo: RuntimeInfo = "RuntimeInfo".asInstanceOf[RuntimeInfo]
  
  @js.native
  sealed trait SessionOperationMetadata extends StObject
  inline def SessionOperationMetadata: SessionOperationMetadata = "SessionOperationMetadata".asInstanceOf[SessionOperationMetadata]
  
  @js.native
  sealed trait SoftwareConfig extends StObject
  inline def SoftwareConfig: SoftwareConfig = "SoftwareConfig".asInstanceOf[SoftwareConfig]
  
  @js.native
  sealed trait SparkJob extends StObject
  inline def SparkJob: SparkJob = "SparkJob".asInstanceOf[SparkJob]
  
  @js.native
  sealed trait SparkRJob extends StObject
  inline def SparkRJob: SparkRJob = "SparkRJob".asInstanceOf[SparkRJob]
  
  @js.native
  sealed trait SparkSqlBatch extends StObject
  inline def SparkSqlBatch: SparkSqlBatch = "SparkSqlBatch".asInstanceOf[SparkSqlBatch]
  
  @js.native
  sealed trait SparkSqlJob extends StObject
  inline def SparkSqlJob: SparkSqlJob = "SparkSqlJob".asInstanceOf[SparkSqlJob]
  
  @js.native
  sealed trait Status extends StObject
  inline def Status: Status = "Status".asInstanceOf[Status]
  
  @js.native
  sealed trait WorkflowMetadata extends StObject
  inline def WorkflowMetadata: WorkflowMetadata = "WorkflowMetadata".asInstanceOf[WorkflowMetadata]
  
  @js.native
  sealed trait WorkflowTemplate extends StObject
  inline def WorkflowTemplate: WorkflowTemplate = "WorkflowTemplate".asInstanceOf[WorkflowTemplate]
  
  @js.native
  sealed trait dataproc extends StObject
  inline def dataproc: dataproc = "dataproc".asInstanceOf[dataproc]
  
  @js.native
  sealed trait v1 extends StObject
  inline def v1: v1 = "v1".asInstanceOf[v1]
}
