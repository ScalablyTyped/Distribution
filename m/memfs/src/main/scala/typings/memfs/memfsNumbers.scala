package typings.memfs

import typings.memfs.constantsMod.S
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object memfsNumbers {
  @js.native
  sealed trait `1` extends S
  
  @js.native
  sealed trait `1024` extends S
  
  @js.native
  sealed trait `128` extends S
  
  @js.native
  sealed trait `16` extends S
  
  @js.native
  sealed trait `2` extends S
  
  @js.native
  sealed trait `2048` extends S
  
  @js.native
  sealed trait `256` extends S
  
  @js.native
  sealed trait `32` extends S
  
  @js.native
  sealed trait `4` extends S
  
  @js.native
  sealed trait `512` extends S
  
  @js.native
  sealed trait `64` extends S
  
  @js.native
  sealed trait `8` extends S
  
  @scala.inline
  def `1`: `1` = 1.asInstanceOf[`1`]
  @scala.inline
  def `1024`: `1024` = 1024.asInstanceOf[`1024`]
  @scala.inline
  def `128`: `128` = 128.asInstanceOf[`128`]
  @scala.inline
  def `16`: `16` = 16.asInstanceOf[`16`]
  @scala.inline
  def `2`: `2` = 2.asInstanceOf[`2`]
  @scala.inline
  def `2048`: `2048` = 2048.asInstanceOf[`2048`]
  @scala.inline
  def `256`: `256` = 256.asInstanceOf[`256`]
  @scala.inline
  def `32`: `32` = 32.asInstanceOf[`32`]
  @scala.inline
  def `4`: `4` = 4.asInstanceOf[`4`]
  @scala.inline
  def `512`: `512` = 512.asInstanceOf[`512`]
  @scala.inline
  def `64`: `64` = 64.asInstanceOf[`64`]
  @scala.inline
  def `8`: `8` = 8.asInstanceOf[`8`]
}

