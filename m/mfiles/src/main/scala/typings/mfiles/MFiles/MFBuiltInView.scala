package typings.mfiles.MFiles

import typings.mfiles.mfilesNumbers.`-1`
import typings.mfiles.mfilesNumbers.`-9000`
import typings.mfiles.mfilesNumbers.`-9001`
import typings.mfiles.mfilesNumbers.`11`
import typings.mfiles.mfilesNumbers.`12`
import typings.mfiles.mfilesNumbers.`13`
import typings.mfiles.mfilesNumbers.`14`
import typings.mfiles.mfilesNumbers.`15`
import typings.mfiles.mfilesNumbers.`5`
import typings.mfiles.mfilesNumbers.`6`
import typings.mfiles.mfilesNumbers.`7`
import typings.mfiles.mfilesNumbers.`8`
import typings.mfiles.mfilesNumbers.`9`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typings.mfiles.mfilesNumbers.`5`
  - typings.mfiles.mfilesNumbers.`6`
  - typings.mfiles.mfilesNumbers.`7`
  - typings.mfiles.mfilesNumbers.`8`
  - typings.mfiles.mfilesNumbers.`9`
  - typings.mfiles.mfilesNumbers.`11`
  - typings.mfiles.mfilesNumbers.`12`
  - typings.mfiles.mfilesNumbers.`13`
  - typings.mfiles.mfilesNumbers.`14`
  - typings.mfiles.mfilesNumbers.`15`
  - typings.mfiles.mfilesNumbers.`-1`
  - typings.mfiles.mfilesNumbers.`-9000`
  - typings.mfiles.mfilesNumbers.`-9001`
*/
trait MFBuiltInView extends StObject
object MFBuiltInView {
  
  inline def AnyView: `-1` = -1.asInstanceOf[`-1`]
  
  inline def AssignedToMe: `9` = 9.asInstanceOf[`9`]
  
  inline def BuiltIn: `13` = 13.asInstanceOf[`13`]
  
  inline def ByID: `12` = 12.asInstanceOf[`12`]
  
  inline def CheckedOutToCurrentUser: `5` = 5.asInstanceOf[`5`]
  
  inline def Favorites: `15` = 15.asInstanceOf[`15`]
  
  inline def LatestSearches: `11` = 11.asInstanceOf[`11`]
  
  inline def Offline: `6` = 6.asInstanceOf[`6`]
  
  inline def OfflineCheckedOut: `-9000` = -9000.asInstanceOf[`-9000`]
  
  inline def OfflineMarkedForOfflineAvailability: `-9001` = -9001.asInstanceOf[`-9001`]
  
  inline def RecentlyAccessedByMe: `14` = 14.asInstanceOf[`14`]
  
  inline def RecentlyModifiedByMe: `7` = 7.asInstanceOf[`7`]
  
  inline def Templates: `8` = 8.asInstanceOf[`8`]
}
