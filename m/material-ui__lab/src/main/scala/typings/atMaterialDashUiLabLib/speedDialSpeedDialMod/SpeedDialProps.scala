...
- Dropped react.react.HTMLAttributes<std.HTMLDivElement> & std.Partial<any> extends any ? std.Pick<react.react.HTMLAttributes<std.HTMLDivElement> & std.Partial<any>, std.Exclude<keyof react.react.HTMLAttributes<std.HTMLDivElement> & std.Partial<any>, 'classes' | never>> : never */ trait SpeedDialProps
