...
trait Overwrite extends StObject {
  ...
  /**
    * Only available to selected merchants, for example multi-client accounts (MCAs) and their sub-accounts. When set to `True`, this option removes any existing claim on the
    * requested website and replaces it with a claim from the account that makes the request.
    */
...
object Overwrite {
  ...
  inline def apply(accountId: String, merchantId: String): Overwrite = {
