...
trait DiskFields extends StObject {
  ...
  /**
    * [Input Only] Whether to attempt an application consistent snapshot by informing the OS to prepare for the snapshot process. Currently only supported on Windows instances using
...
object DiskFields {
  ...
  @scala.inline
  def apply(disk: String, project: String, zone: String): DiskFields = {
    ...
    __obj.asInstanceOf[DiskFields]
  }
  
  @scala.inline
  implicit class DiskFieldsMutableBuilder[Self <: DiskFields] (val x: Self) extends AnyVal {
