...
trait SSLHealthCheck extends StObject {
  ...
  /**
    ...
    * named port specified in the Backend Service is used for health checking. If not specified, SSL health check follows behavior specified in port and portName fields.
    */
  ...
  /**
    * The application data to send once the SSL connection has been established (default value is empty). If both request and response are empty, the connection establishment alone will
...
object SSLHealthCheck {
  ...
  inline def apply(): SSLHealthCheck = {
