...
trait HttpRouteRuleMatch extends StObject {
  ...
  /**
    ...
    * part of the original URL. fullPathMatch must be between 1 and 1024 characters. Only one of prefixMatch, fullPathMatch or regexMatch must be specified.
    */
  ...
  /**
    ...
    * node metadata. When there is a match, the relevant routing configuration is made available to those proxies. For each metadataFilter in this list, if its filterMatchCriteria is set
    * to MATCH_ANY, at least one of the filterLabels must match the corresponding label provided in the metadata. If its filterMatchCriteria is set to MATCH_ALL, then all of its
    * filterLabels must match with corresponding labels provided in the metadata. If multiple metadataFilters are specified, all of them need to be satisfied in order to be considered a
    * match. metadataFilters specified here will be applied after those specified in ForwardingRule that refers to the UrlMap this HttpRouteRuleMatch belongs to. metadataFilters only
    * applies to Loadbalancers that have their loadBalancingScheme set to INTERNAL_SELF_MANAGED. Not supported when the URL map is bound to target gRPC proxy that has validateForProxyless
    * field set to true.
    */
  var metadataFilters: js.UndefOr[js.Array[MetadataFilter]] = js.undefined
  
  /**
    * For satisfying the matchRule condition, the request's path must begin with the specified prefixMatch. prefixMatch must begin with a /. The value must be between 1 and 1024
  ...
  /**
    * Specifies a list of query parameter match criteria, all of which must match corresponding query parameters in the request. Not supported when the URL map is bound to target gRPC
    * proxy.
    */
  ...
  /**
    ...
    * with the original URL. For regular expression grammar please see github.com/google/re2/wiki/Syntax Only one of prefixMatch, fullPathMatch or regexMatch must be specified. Note that
    * regexMatch only applies to Loadbalancers that have their loadBalancingScheme set to INTERNAL_SELF_MANAGED.
    */
  var regexMatch: js.UndefOr[String] = js.undefined
}
object HttpRouteRuleMatch {
  
  inline def apply(): HttpRouteRuleMatch = {
  ...
  implicit open class MutableBuilder[Self <: HttpRouteRuleMatch] (val x: Self) extends AnyVal {
    ...
    inline def setHeaderMatches(value: js.Array[HttpHeaderMatch]): Self = StObject.set(x, "headerMatches", value.asInstanceOf[js.Any])
    
    inline def setHeaderMatchesUndefined: Self = StObject.set(x, "headerMatches", js.undefined)
    
    inline def setHeaderMatchesVarargs(value: HttpHeaderMatch*): Self = StObject.set(x, "headerMatches", js.Array(value*))
    ...
    inline def setQueryParameterMatches(value: js.Array[HttpQueryParameterMatch]): Self = StObject.set(x, "queryParameterMatches", value.asInstanceOf[js.Any])
    
    inline def setQueryParameterMatchesUndefined: Self = StObject.set(x, "queryParameterMatches", js.undefined)
    
    inline def setQueryParameterMatchesVarargs(value: HttpQueryParameterMatch*): Self = StObject.set(x, "queryParameterMatches", js.Array(value*))
