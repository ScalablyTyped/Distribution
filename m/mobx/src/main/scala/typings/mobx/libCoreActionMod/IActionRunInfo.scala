...
object IActionRunInfo {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(actionId = actionId, notifySpy = notifySpy, parentActionId = parentActionId, prevAllowStateChanges = prevAllowStateChanges, prevAllowStateReads = prevAllowStateReads, startTime = startTime)
    if (error != null) __obj.updateDynamic("error")(error)
    if (prevDerivation != null) __obj.updateDynamic("prevDerivation")(prevDerivation)
