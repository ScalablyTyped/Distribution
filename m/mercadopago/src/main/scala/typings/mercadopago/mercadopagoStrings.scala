package typings.mercadopago

import typings.mercadopago.currencyMod.Currency
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object mercadopagoStrings {
  
  @js.native
  sealed trait AED
    extends StObject
       with Currency
  inline def AED: AED = "AED".asInstanceOf[AED]
  
  @js.native
  sealed trait AFN
    extends StObject
       with Currency
  inline def AFN: AFN = "AFN".asInstanceOf[AFN]
  
  @js.native
  sealed trait ALL
    extends StObject
       with Currency
  inline def ALL: ALL = "ALL".asInstanceOf[ALL]
  
  @js.native
  sealed trait AMD
    extends StObject
       with Currency
  inline def AMD: AMD = "AMD".asInstanceOf[AMD]
  
  @js.native
  sealed trait ANG
    extends StObject
       with Currency
  inline def ANG: ANG = "ANG".asInstanceOf[ANG]
  
  @js.native
  sealed trait AOA
    extends StObject
       with Currency
  inline def AOA: AOA = "AOA".asInstanceOf[AOA]
  
  @js.native
  sealed trait ARS
    extends StObject
       with Currency
  inline def ARS: ARS = "ARS".asInstanceOf[ARS]
  
  @js.native
  sealed trait AUD
    extends StObject
       with Currency
  inline def AUD: AUD = "AUD".asInstanceOf[AUD]
  
  @js.native
  sealed trait AWG
    extends StObject
       with Currency
  inline def AWG: AWG = "AWG".asInstanceOf[AWG]
  
  @js.native
  sealed trait AZN
    extends StObject
       with Currency
  inline def AZN: AZN = "AZN".asInstanceOf[AZN]
  
  @js.native
  sealed trait BAM
    extends StObject
       with Currency
  inline def BAM: BAM = "BAM".asInstanceOf[BAM]
  
  @js.native
  sealed trait BBD
    extends StObject
       with Currency
  inline def BBD: BBD = "BBD".asInstanceOf[BBD]
  
  @js.native
  sealed trait BDT
    extends StObject
       with Currency
  inline def BDT: BDT = "BDT".asInstanceOf[BDT]
  
  @js.native
  sealed trait BGN
    extends StObject
       with Currency
  inline def BGN: BGN = "BGN".asInstanceOf[BGN]
  
  @js.native
  sealed trait BHD
    extends StObject
       with Currency
  inline def BHD: BHD = "BHD".asInstanceOf[BHD]
  
  @js.native
  sealed trait BIF
    extends StObject
       with Currency
  inline def BIF: BIF = "BIF".asInstanceOf[BIF]
  
  @js.native
  sealed trait BMD
    extends StObject
       with Currency
  inline def BMD: BMD = "BMD".asInstanceOf[BMD]
  
  @js.native
  sealed trait BND
    extends StObject
       with Currency
  inline def BND: BND = "BND".asInstanceOf[BND]
  
  @js.native
  sealed trait BOB
    extends StObject
       with Currency
  inline def BOB: BOB = "BOB".asInstanceOf[BOB]
  
  @js.native
  sealed trait BOV
    extends StObject
       with Currency
  inline def BOV: BOV = "BOV".asInstanceOf[BOV]
  
  @js.native
  sealed trait BRL
    extends StObject
       with Currency
  inline def BRL: BRL = "BRL".asInstanceOf[BRL]
  
  @js.native
  sealed trait BSD
    extends StObject
       with Currency
  inline def BSD: BSD = "BSD".asInstanceOf[BSD]
  
  @js.native
  sealed trait BTN
    extends StObject
       with Currency
  inline def BTN: BTN = "BTN".asInstanceOf[BTN]
  
  @js.native
  sealed trait BWP
    extends StObject
       with Currency
  inline def BWP: BWP = "BWP".asInstanceOf[BWP]
  
  @js.native
  sealed trait BYN
    extends StObject
       with Currency
  inline def BYN: BYN = "BYN".asInstanceOf[BYN]
  
  @js.native
  sealed trait BZD
    extends StObject
       with Currency
  inline def BZD: BZD = "BZD".asInstanceOf[BZD]
  
  @js.native
  sealed trait CAD
    extends StObject
       with Currency
  inline def CAD: CAD = "CAD".asInstanceOf[CAD]
  
  @js.native
  sealed trait CDF
    extends StObject
       with Currency
  inline def CDF: CDF = "CDF".asInstanceOf[CDF]
  
  @js.native
  sealed trait CHE
    extends StObject
       with Currency
  inline def CHE: CHE = "CHE".asInstanceOf[CHE]
  
  @js.native
  sealed trait CHF
    extends StObject
       with Currency
  inline def CHF: CHF = "CHF".asInstanceOf[CHF]
  
  @js.native
  sealed trait CHW
    extends StObject
       with Currency
  inline def CHW: CHW = "CHW".asInstanceOf[CHW]
  
  @js.native
  sealed trait CLF
    extends StObject
       with Currency
  inline def CLF: CLF = "CLF".asInstanceOf[CLF]
  
  @js.native
  sealed trait CLP
    extends StObject
       with Currency
  inline def CLP: CLP = "CLP".asInstanceOf[CLP]
  
  @js.native
  sealed trait CNY
    extends StObject
       with Currency
  inline def CNY: CNY = "CNY".asInstanceOf[CNY]
  
  @js.native
  sealed trait COP
    extends StObject
       with Currency
  inline def COP: COP = "COP".asInstanceOf[COP]
  
  @js.native
  sealed trait COU
    extends StObject
       with Currency
  inline def COU: COU = "COU".asInstanceOf[COU]
  
  @js.native
  sealed trait CRC
    extends StObject
       with Currency
  inline def CRC: CRC = "CRC".asInstanceOf[CRC]
  
  @js.native
  sealed trait CUC
    extends StObject
       with Currency
  inline def CUC: CUC = "CUC".asInstanceOf[CUC]
  
  @js.native
  sealed trait CUP
    extends StObject
       with Currency
  inline def CUP: CUP = "CUP".asInstanceOf[CUP]
  
  @js.native
  sealed trait CVE
    extends StObject
       with Currency
  inline def CVE: CVE = "CVE".asInstanceOf[CVE]
  
  @js.native
  sealed trait CZK
    extends StObject
       with Currency
  inline def CZK: CZK = "CZK".asInstanceOf[CZK]
  
  @js.native
  sealed trait DJF
    extends StObject
       with Currency
  inline def DJF: DJF = "DJF".asInstanceOf[DJF]
  
  @js.native
  sealed trait DKK
    extends StObject
       with Currency
  inline def DKK: DKK = "DKK".asInstanceOf[DKK]
  
  @js.native
  sealed trait DOP
    extends StObject
       with Currency
  inline def DOP: DOP = "DOP".asInstanceOf[DOP]
  
  @js.native
  sealed trait DZD
    extends StObject
       with Currency
  inline def DZD: DZD = "DZD".asInstanceOf[DZD]
  
  @js.native
  sealed trait EGP
    extends StObject
       with Currency
  inline def EGP: EGP = "EGP".asInstanceOf[EGP]
  
  @js.native
  sealed trait ERN
    extends StObject
       with Currency
  inline def ERN: ERN = "ERN".asInstanceOf[ERN]
  
  @js.native
  sealed trait ETB
    extends StObject
       with Currency
  inline def ETB: ETB = "ETB".asInstanceOf[ETB]
  
  @js.native
  sealed trait EUR
    extends StObject
       with Currency
  inline def EUR: EUR = "EUR".asInstanceOf[EUR]
  
  @js.native
  sealed trait FJD
    extends StObject
       with Currency
  inline def FJD: FJD = "FJD".asInstanceOf[FJD]
  
  @js.native
  sealed trait FKP
    extends StObject
       with Currency
  inline def FKP: FKP = "FKP".asInstanceOf[FKP]
  
  @js.native
  sealed trait GBP
    extends StObject
       with Currency
  inline def GBP: GBP = "GBP".asInstanceOf[GBP]
  
  @js.native
  sealed trait GEL
    extends StObject
       with Currency
  inline def GEL: GEL = "GEL".asInstanceOf[GEL]
  
  @js.native
  sealed trait GHS
    extends StObject
       with Currency
  inline def GHS: GHS = "GHS".asInstanceOf[GHS]
  
  @js.native
  sealed trait GIP
    extends StObject
       with Currency
  inline def GIP: GIP = "GIP".asInstanceOf[GIP]
  
  @js.native
  sealed trait GMD
    extends StObject
       with Currency
  inline def GMD: GMD = "GMD".asInstanceOf[GMD]
  
  @js.native
  sealed trait GNF
    extends StObject
       with Currency
  inline def GNF: GNF = "GNF".asInstanceOf[GNF]
  
  @js.native
  sealed trait GTQ
    extends StObject
       with Currency
  inline def GTQ: GTQ = "GTQ".asInstanceOf[GTQ]
  
  @js.native
  sealed trait GYD
    extends StObject
       with Currency
  inline def GYD: GYD = "GYD".asInstanceOf[GYD]
  
  @js.native
  sealed trait HKD
    extends StObject
       with Currency
  inline def HKD: HKD = "HKD".asInstanceOf[HKD]
  
  @js.native
  sealed trait HNL
    extends StObject
       with Currency
  inline def HNL: HNL = "HNL".asInstanceOf[HNL]
  
  @js.native
  sealed trait HRK
    extends StObject
       with Currency
  inline def HRK: HRK = "HRK".asInstanceOf[HRK]
  
  @js.native
  sealed trait HTG
    extends StObject
       with Currency
  inline def HTG: HTG = "HTG".asInstanceOf[HTG]
  
  @js.native
  sealed trait HUF
    extends StObject
       with Currency
  inline def HUF: HUF = "HUF".asInstanceOf[HUF]
  
  @js.native
  sealed trait IDR
    extends StObject
       with Currency
  inline def IDR: IDR = "IDR".asInstanceOf[IDR]
  
  @js.native
  sealed trait ILS
    extends StObject
       with Currency
  inline def ILS: ILS = "ILS".asInstanceOf[ILS]
  
  @js.native
  sealed trait INC extends StObject
  inline def INC: INC = "INC".asInstanceOf[INC]
  
  @js.native
  sealed trait INR
    extends StObject
       with Currency
  inline def INR: INR = "INR".asInstanceOf[INR]
  
  @js.native
  sealed trait IQD
    extends StObject
       with Currency
  inline def IQD: IQD = "IQD".asInstanceOf[IQD]
  
  @js.native
  sealed trait IRR
    extends StObject
       with Currency
  inline def IRR: IRR = "IRR".asInstanceOf[IRR]
  
  @js.native
  sealed trait ISK
    extends StObject
       with Currency
  inline def ISK: ISK = "ISK".asInstanceOf[ISK]
  
  @js.native
  sealed trait IVA extends StObject
  inline def IVA: IVA = "IVA".asInstanceOf[IVA]
  
  @js.native
  sealed trait JMD
    extends StObject
       with Currency
  inline def JMD: JMD = "JMD".asInstanceOf[JMD]
  
  @js.native
  sealed trait JOD
    extends StObject
       with Currency
  inline def JOD: JOD = "JOD".asInstanceOf[JOD]
  
  @js.native
  sealed trait JPY
    extends StObject
       with Currency
  inline def JPY: JPY = "JPY".asInstanceOf[JPY]
  
  @js.native
  sealed trait KES
    extends StObject
       with Currency
  inline def KES: KES = "KES".asInstanceOf[KES]
  
  @js.native
  sealed trait KGS
    extends StObject
       with Currency
  inline def KGS: KGS = "KGS".asInstanceOf[KGS]
  
  @js.native
  sealed trait KHR
    extends StObject
       with Currency
  inline def KHR: KHR = "KHR".asInstanceOf[KHR]
  
  @js.native
  sealed trait KMF
    extends StObject
       with Currency
  inline def KMF: KMF = "KMF".asInstanceOf[KMF]
  
  @js.native
  sealed trait KPW
    extends StObject
       with Currency
  inline def KPW: KPW = "KPW".asInstanceOf[KPW]
  
  @js.native
  sealed trait KRW
    extends StObject
       with Currency
  inline def KRW: KRW = "KRW".asInstanceOf[KRW]
  
  @js.native
  sealed trait KWD
    extends StObject
       with Currency
  inline def KWD: KWD = "KWD".asInstanceOf[KWD]
  
  @js.native
  sealed trait KYD
    extends StObject
       with Currency
  inline def KYD: KYD = "KYD".asInstanceOf[KYD]
  
  @js.native
  sealed trait KZT
    extends StObject
       with Currency
  inline def KZT: KZT = "KZT".asInstanceOf[KZT]
  
  @js.native
  sealed trait LAK
    extends StObject
       with Currency
  inline def LAK: LAK = "LAK".asInstanceOf[LAK]
  
  @js.native
  sealed trait LBP
    extends StObject
       with Currency
  inline def LBP: LBP = "LBP".asInstanceOf[LBP]
  
  @js.native
  sealed trait LKR
    extends StObject
       with Currency
  inline def LKR: LKR = "LKR".asInstanceOf[LKR]
  
  @js.native
  sealed trait LRD
    extends StObject
       with Currency
  inline def LRD: LRD = "LRD".asInstanceOf[LRD]
  
  @js.native
  sealed trait LSL
    extends StObject
       with Currency
  inline def LSL: LSL = "LSL".asInstanceOf[LSL]
  
  @js.native
  sealed trait LYD
    extends StObject
       with Currency
  inline def LYD: LYD = "LYD".asInstanceOf[LYD]
  
  @js.native
  sealed trait MAD
    extends StObject
       with Currency
  inline def MAD: MAD = "MAD".asInstanceOf[MAD]
  
  @js.native
  sealed trait MDL
    extends StObject
       with Currency
  inline def MDL: MDL = "MDL".asInstanceOf[MDL]
  
  @js.native
  sealed trait MGA
    extends StObject
       with Currency
  inline def MGA: MGA = "MGA".asInstanceOf[MGA]
  
  @js.native
  sealed trait MKD
    extends StObject
       with Currency
  inline def MKD: MKD = "MKD".asInstanceOf[MKD]
  
  @js.native
  sealed trait MMK
    extends StObject
       with Currency
  inline def MMK: MMK = "MMK".asInstanceOf[MMK]
  
  @js.native
  sealed trait MNT
    extends StObject
       with Currency
  inline def MNT: MNT = "MNT".asInstanceOf[MNT]
  
  @js.native
  sealed trait MOP
    extends StObject
       with Currency
  inline def MOP: MOP = "MOP".asInstanceOf[MOP]
  
  @js.native
  sealed trait MRU
    extends StObject
       with Currency
  inline def MRU: MRU = "MRU".asInstanceOf[MRU]
  
  @js.native
  sealed trait MUR
    extends StObject
       with Currency
  inline def MUR: MUR = "MUR".asInstanceOf[MUR]
  
  @js.native
  sealed trait MVR
    extends StObject
       with Currency
  inline def MVR: MVR = "MVR".asInstanceOf[MVR]
  
  @js.native
  sealed trait MWK
    extends StObject
       with Currency
  inline def MWK: MWK = "MWK".asInstanceOf[MWK]
  
  @js.native
  sealed trait MXN
    extends StObject
       with Currency
  inline def MXN: MXN = "MXN".asInstanceOf[MXN]
  
  @js.native
  sealed trait MXV
    extends StObject
       with Currency
  inline def MXV: MXV = "MXV".asInstanceOf[MXV]
  
  @js.native
  sealed trait MYR
    extends StObject
       with Currency
  inline def MYR: MYR = "MYR".asInstanceOf[MYR]
  
  @js.native
  sealed trait MZN
    extends StObject
       with Currency
  inline def MZN: MZN = "MZN".asInstanceOf[MZN]
  
  @js.native
  sealed trait NAD
    extends StObject
       with Currency
  inline def NAD: NAD = "NAD".asInstanceOf[NAD]
  
  @js.native
  sealed trait NGN
    extends StObject
       with Currency
  inline def NGN: NGN = "NGN".asInstanceOf[NGN]
  
  @js.native
  sealed trait NIO
    extends StObject
       with Currency
  inline def NIO: NIO = "NIO".asInstanceOf[NIO]
  
  @js.native
  sealed trait NOK
    extends StObject
       with Currency
  inline def NOK: NOK = "NOK".asInstanceOf[NOK]
  
  @js.native
  sealed trait NPR
    extends StObject
       with Currency
  inline def NPR: NPR = "NPR".asInstanceOf[NPR]
  
  @js.native
  sealed trait NZD
    extends StObject
       with Currency
  inline def NZD: NZD = "NZD".asInstanceOf[NZD]
  
  @js.native
  sealed trait OMR
    extends StObject
       with Currency
  inline def OMR: OMR = "OMR".asInstanceOf[OMR]
  
  @js.native
  sealed trait PAB
    extends StObject
       with Currency
  inline def PAB: PAB = "PAB".asInstanceOf[PAB]
  
  @js.native
  sealed trait PEN
    extends StObject
       with Currency
  inline def PEN: PEN = "PEN".asInstanceOf[PEN]
  
  @js.native
  sealed trait PGK
    extends StObject
       with Currency
  inline def PGK: PGK = "PGK".asInstanceOf[PGK]
  
  @js.native
  sealed trait PHP
    extends StObject
       with Currency
  inline def PHP: PHP = "PHP".asInstanceOf[PHP]
  
  @js.native
  sealed trait PKR
    extends StObject
       with Currency
  inline def PKR: PKR = "PKR".asInstanceOf[PKR]
  
  @js.native
  sealed trait PLN
    extends StObject
       with Currency
  inline def PLN: PLN = "PLN".asInstanceOf[PLN]
  
  @js.native
  sealed trait PYG
    extends StObject
       with Currency
  inline def PYG: PYG = "PYG".asInstanceOf[PYG]
  
  @js.native
  sealed trait QAR
    extends StObject
       with Currency
  inline def QAR: QAR = "QAR".asInstanceOf[QAR]
  
  @js.native
  sealed trait RON
    extends StObject
       with Currency
  inline def RON: RON = "RON".asInstanceOf[RON]
  
  @js.native
  sealed trait RSD
    extends StObject
       with Currency
  inline def RSD: RSD = "RSD".asInstanceOf[RSD]
  
  @js.native
  sealed trait RUB
    extends StObject
       with Currency
  inline def RUB: RUB = "RUB".asInstanceOf[RUB]
  
  @js.native
  sealed trait RWF
    extends StObject
       with Currency
  inline def RWF: RWF = "RWF".asInstanceOf[RWF]
  
  @js.native
  sealed trait SAR
    extends StObject
       with Currency
  inline def SAR: SAR = "SAR".asInstanceOf[SAR]
  
  @js.native
  sealed trait SBD
    extends StObject
       with Currency
  inline def SBD: SBD = "SBD".asInstanceOf[SBD]
  
  @js.native
  sealed trait SCR
    extends StObject
       with Currency
  inline def SCR: SCR = "SCR".asInstanceOf[SCR]
  
  @js.native
  sealed trait SDG
    extends StObject
       with Currency
  inline def SDG: SDG = "SDG".asInstanceOf[SDG]
  
  @js.native
  sealed trait SEK
    extends StObject
       with Currency
  inline def SEK: SEK = "SEK".asInstanceOf[SEK]
  
  @js.native
  sealed trait SGD
    extends StObject
       with Currency
  inline def SGD: SGD = "SGD".asInstanceOf[SGD]
  
  @js.native
  sealed trait SHP
    extends StObject
       with Currency
  inline def SHP: SHP = "SHP".asInstanceOf[SHP]
  
  @js.native
  sealed trait SLL
    extends StObject
       with Currency
  inline def SLL: SLL = "SLL".asInstanceOf[SLL]
  
  @js.native
  sealed trait SOS
    extends StObject
       with Currency
  inline def SOS: SOS = "SOS".asInstanceOf[SOS]
  
  @js.native
  sealed trait SRD
    extends StObject
       with Currency
  inline def SRD: SRD = "SRD".asInstanceOf[SRD]
  
  @js.native
  sealed trait SSP
    extends StObject
       with Currency
  inline def SSP: SSP = "SSP".asInstanceOf[SSP]
  
  @js.native
  sealed trait STN
    extends StObject
       with Currency
  inline def STN: STN = "STN".asInstanceOf[STN]
  
  @js.native
  sealed trait SVC
    extends StObject
       with Currency
  inline def SVC: SVC = "SVC".asInstanceOf[SVC]
  
  @js.native
  sealed trait SYP
    extends StObject
       with Currency
  inline def SYP: SYP = "SYP".asInstanceOf[SYP]
  
  @js.native
  sealed trait SZL
    extends StObject
       with Currency
  inline def SZL: SZL = "SZL".asInstanceOf[SZL]
  
  @js.native
  sealed trait THB
    extends StObject
       with Currency
  inline def THB: THB = "THB".asInstanceOf[THB]
  
  @js.native
  sealed trait TJS
    extends StObject
       with Currency
  inline def TJS: TJS = "TJS".asInstanceOf[TJS]
  
  @js.native
  sealed trait TMT
    extends StObject
       with Currency
  inline def TMT: TMT = "TMT".asInstanceOf[TMT]
  
  @js.native
  sealed trait TND
    extends StObject
       with Currency
  inline def TND: TND = "TND".asInstanceOf[TND]
  
  @js.native
  sealed trait TOP
    extends StObject
       with Currency
  inline def TOP: TOP = "TOP".asInstanceOf[TOP]
  
  @js.native
  sealed trait TRY
    extends StObject
       with Currency
  inline def TRY: TRY = "TRY".asInstanceOf[TRY]
  
  @js.native
  sealed trait TTD
    extends StObject
       with Currency
  inline def TTD: TTD = "TTD".asInstanceOf[TTD]
  
  @js.native
  sealed trait TWD
    extends StObject
       with Currency
  inline def TWD: TWD = "TWD".asInstanceOf[TWD]
  
  @js.native
  sealed trait TZS
    extends StObject
       with Currency
  inline def TZS: TZS = "TZS".asInstanceOf[TZS]
  
  @js.native
  sealed trait UAH
    extends StObject
       with Currency
  inline def UAH: UAH = "UAH".asInstanceOf[UAH]
  
  @js.native
  sealed trait UGX
    extends StObject
       with Currency
  inline def UGX: UGX = "UGX".asInstanceOf[UGX]
  
  @js.native
  sealed trait USD
    extends StObject
       with Currency
  inline def USD: USD = "USD".asInstanceOf[USD]
  
  @js.native
  sealed trait USN
    extends StObject
       with Currency
  inline def USN: USN = "USN".asInstanceOf[USN]
  
  @js.native
  sealed trait UYI
    extends StObject
       with Currency
  inline def UYI: UYI = "UYI".asInstanceOf[UYI]
  
  @js.native
  sealed trait UYU
    extends StObject
       with Currency
  inline def UYU: UYU = "UYU".asInstanceOf[UYU]
  
  @js.native
  sealed trait UYW
    extends StObject
       with Currency
  inline def UYW: UYW = "UYW".asInstanceOf[UYW]
  
  @js.native
  sealed trait UZS
    extends StObject
       with Currency
  inline def UZS: UZS = "UZS".asInstanceOf[UZS]
  
  @js.native
  sealed trait VES
    extends StObject
       with Currency
  inline def VES: VES = "VES".asInstanceOf[VES]
  
  @js.native
  sealed trait VND
    extends StObject
       with Currency
  inline def VND: VND = "VND".asInstanceOf[VND]
  
  @js.native
  sealed trait VUV
    extends StObject
       with Currency
  inline def VUV: VUV = "VUV".asInstanceOf[VUV]
  
  @js.native
  sealed trait WST
    extends StObject
       with Currency
  inline def WST: WST = "WST".asInstanceOf[WST]
  
  @js.native
  sealed trait XAF
    extends StObject
       with Currency
  inline def XAF: XAF = "XAF".asInstanceOf[XAF]
  
  @js.native
  sealed trait XAG
    extends StObject
       with Currency
  inline def XAG: XAG = "XAG".asInstanceOf[XAG]
  
  @js.native
  sealed trait XAU
    extends StObject
       with Currency
  inline def XAU: XAU = "XAU".asInstanceOf[XAU]
  
  @js.native
  sealed trait XBA
    extends StObject
       with Currency
  inline def XBA: XBA = "XBA".asInstanceOf[XBA]
  
  @js.native
  sealed trait XBB
    extends StObject
       with Currency
  inline def XBB: XBB = "XBB".asInstanceOf[XBB]
  
  @js.native
  sealed trait XBC
    extends StObject
       with Currency
  inline def XBC: XBC = "XBC".asInstanceOf[XBC]
  
  @js.native
  sealed trait XBD
    extends StObject
       with Currency
  inline def XBD: XBD = "XBD".asInstanceOf[XBD]
  
  @js.native
  sealed trait XCD
    extends StObject
       with Currency
  inline def XCD: XCD = "XCD".asInstanceOf[XCD]
  
  @js.native
  sealed trait XDR
    extends StObject
       with Currency
  inline def XDR: XDR = "XDR".asInstanceOf[XDR]
  
  @js.native
  sealed trait XOF
    extends StObject
       with Currency
  inline def XOF: XOF = "XOF".asInstanceOf[XOF]
  
  @js.native
  sealed trait XPD
    extends StObject
       with Currency
  inline def XPD: XPD = "XPD".asInstanceOf[XPD]
  
  @js.native
  sealed trait XPF
    extends StObject
       with Currency
  inline def XPF: XPF = "XPF".asInstanceOf[XPF]
  
  @js.native
  sealed trait XPT
    extends StObject
       with Currency
  inline def XPT: XPT = "XPT".asInstanceOf[XPT]
  
  @js.native
  sealed trait XSU
    extends StObject
       with Currency
  inline def XSU: XSU = "XSU".asInstanceOf[XSU]
  
  @js.native
  sealed trait XTS
    extends StObject
       with Currency
  inline def XTS: XTS = "XTS".asInstanceOf[XTS]
  
  @js.native
  sealed trait XUA
    extends StObject
       with Currency
  inline def XUA: XUA = "XUA".asInstanceOf[XUA]
  
  @js.native
  sealed trait XXX
    extends StObject
       with Currency
  inline def XXX: XXX = "XXX".asInstanceOf[XXX]
  
  @js.native
  sealed trait YER
    extends StObject
       with Currency
  inline def YER: YER = "YER".asInstanceOf[YER]
  
  @js.native
  sealed trait ZAR
    extends StObject
       with Currency
  inline def ZAR: ZAR = "ZAR".asInstanceOf[ZAR]
  
  @js.native
  sealed trait ZMW
    extends StObject
       with Currency
  inline def ZMW: ZMW = "ZMW".asInstanceOf[ZMW]
  
  @js.native
  sealed trait ZWL
    extends StObject
       with Currency
  inline def ZWL: ZWL = "ZWL".asInstanceOf[ZWL]
  
  @js.native
  sealed trait all_ extends StObject
  inline def all_ : all_ = "all".asInstanceOf[all_]
  
  @js.native
  sealed trait approved extends StObject
  inline def approved: approved = "approved".asInstanceOf[approved]
  
  @js.native
  sealed trait association extends StObject
  inline def association: association = "association".asInstanceOf[association]
  
  @js.native
  sealed trait authorized extends StObject
  inline def authorized: authorized = "authorized".asInstanceOf[authorized]
  
  @js.native
  sealed trait cancelled extends StObject
  inline def cancelled: cancelled = "cancelled".asInstanceOf[cancelled]
  
  @js.native
  sealed trait charged_back extends StObject
  inline def charged_back: charged_back = "charged_back".asInstanceOf[charged_back]
  
  @js.native
  sealed trait custom extends StObject
  inline def custom: custom = "custom".asInstanceOf[custom]
  
  @js.native
  sealed trait customer extends StObject
  inline def customer: customer = "customer".asInstanceOf[customer]
  
  @js.native
  sealed trait days extends StObject
  inline def days: days = "days".asInstanceOf[days]
  
  @js.native
  sealed trait facebook_ad extends StObject
  inline def facebook_ad: facebook_ad = "facebook_ad".asInstanceOf[facebook_ad]
  
  @js.native
  sealed trait google_ad extends StObject
  inline def google_ad: google_ad = "google_ad".asInstanceOf[google_ad]
  
  @js.native
  sealed trait guest extends StObject
  inline def guest: guest = "guest".asInstanceOf[guest]
  
  @js.native
  sealed trait in_mediation extends StObject
  inline def in_mediation: in_mediation = "in_mediation".asInstanceOf[in_mediation]
  
  @js.native
  sealed trait in_process extends StObject
  inline def in_process: in_process = "in_process".asInstanceOf[in_process]
  
  @js.native
  sealed trait individual extends StObject
  inline def individual: individual = "individual".asInstanceOf[individual]
  
  @js.native
  sealed trait me2 extends StObject
  inline def me2: me2 = "me2".asInstanceOf[me2]
  
  @js.native
  sealed trait mercadolibre extends StObject
  inline def mercadolibre: mercadolibre = "mercadolibre".asInstanceOf[mercadolibre]
  
  @js.native
  sealed trait mercadopago extends StObject
  inline def mercadopago: mercadopago = "mercadopago".asInstanceOf[mercadopago]
  
  @js.native
  sealed trait months extends StObject
  inline def months: months = "months".asInstanceOf[months]
  
  @js.native
  sealed trait not_specified extends StObject
  inline def not_specified: not_specified = "not_specified".asInstanceOf[not_specified]
  
  @js.native
  sealed trait paused extends StObject
  inline def paused: paused = "paused".asInstanceOf[paused]
  
  @js.native
  sealed trait pending extends StObject
  inline def pending: pending = "pending".asInstanceOf[pending]
  
  @js.native
  sealed trait refunded extends StObject
  inline def refunded: refunded = "refunded".asInstanceOf[refunded]
  
  @js.native
  sealed trait registered extends StObject
  inline def registered: registered = "registered".asInstanceOf[registered]
  
  @js.native
  sealed trait rejected extends StObject
  inline def rejected: rejected = "rejected".asInstanceOf[rejected]
}
