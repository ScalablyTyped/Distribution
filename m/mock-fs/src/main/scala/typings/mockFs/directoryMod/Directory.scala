...
trait Directory extends Item {
  /**
    ...
    */
  def addItem[T /* <: typings.mockFs.itemMod.^ */](name: String, item: T): T
  /** Get a named item (or null if none). */
  def getItem(name: String): typings.mockFs.itemMod.^  | Null
  ...
  /**
    ...
    */
  def removeItem(name: String): typings.mockFs.itemMod.^
}
