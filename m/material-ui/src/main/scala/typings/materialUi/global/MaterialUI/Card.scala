package typings.materialUi.global.MaterialUI

import typings.materialUi.MaterialUI.Card.CardActionsProps
import typings.materialUi.MaterialUI.Card.CardExpandableProps
import typings.materialUi.MaterialUI.Card.CardHeaderProps
import typings.materialUi.MaterialUI.Card.CardMediaProps
import typings.materialUi.MaterialUI.Card.CardProps
import typings.materialUi.MaterialUI.Card.CardTextProps
import typings.materialUi.MaterialUI.Card.CardTitleProps
import typings.react.mod.Component
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("__MaterialUI.Card")
@js.native
object Card extends js.Object {
  @js.native
  class Card ()
    extends Component[CardProps, js.Object, js.Any]
  
  @js.native
  class CardActions ()
    extends Component[CardActionsProps, js.Object, js.Any]
  
  @js.native
  class CardExpandable ()
    extends Component[CardExpandableProps, js.Object, js.Any]
  
  @js.native
  class CardHeader ()
    extends Component[CardHeaderProps, js.Object, js.Any]
  
  @js.native
  class CardMedia ()
    extends Component[CardMediaProps, js.Object, js.Any]
  
  @js.native
  class CardText ()
    extends Component[CardTextProps, js.Object, js.Any]
  
  @js.native
  class CardTitle ()
    extends Component[CardTitleProps, js.Object, js.Any]
  
}

