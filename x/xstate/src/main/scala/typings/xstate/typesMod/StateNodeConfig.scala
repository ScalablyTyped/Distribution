...
object StateNodeConfig {
  @scala.inline
  def apply[TContext, /* <: typings.xstate.typesMod.StateSchema[_] */ TStateSchema, /* <: typings.xstate.typesMod.EventObject */ TEvent](): StateNodeConfig[TContext, TStateSchema, TEvent] = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[StateNodeConfig[TContext, TStateSchema, TEvent]]
  }
  @scala.inline
  implicit class StateNodeConfigOps[Self <: StateNodeConfig[_, _, _], TContext, /* <: typings.xstate.typesMod.StateSchema[_] */ TStateSchema, /* <: typings.xstate.typesMod.EventObject */ TEvent] (val x: Self with (StateNodeConfig[TContext, TStateSchema, TEvent])) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def setOnVarargs(
      value: ((/* import warning: importer.ImportType#apply Failed type conversion: {[ K in TEvent['type'] ]: xstate.xstate/lib/types.TransitionConfig<TContext, TEvent extends {  type :K}? TEvent : never> & {  event :K}}[TEvent['type']] */ js.Any) | ((TransitionConfig[TContext, TEvent]) with typings.xstate.anon.Event) | ((TransitionConfig[TContext, TEvent]) with `2`))*
