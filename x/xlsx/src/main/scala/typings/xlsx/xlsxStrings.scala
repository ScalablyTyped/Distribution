package typings.xlsx

import typings.xlsx.mod.BookType
import typings.xlsx.mod.ExcelDataType
import typings.xlsx.mod.SheetType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object xlsxStrings {
  
  @js.native
  sealed trait A extends StObject
  inline def A: A = "A".asInstanceOf[A]
  
  @js.native
  sealed trait array extends StObject
  inline def array: array = "array".asInstanceOf[array]
  
  @js.native
  sealed trait b
    extends StObject
       with ExcelDataType
  inline def b: b = "b".asInstanceOf[b]
  
  @js.native
  sealed trait base64 extends StObject
  inline def base64: base64 = "base64".asInstanceOf[base64]
  
  @js.native
  sealed trait biff2
    extends StObject
       with BookType
  inline def biff2: biff2 = "biff2".asInstanceOf[biff2]
  
  @js.native
  sealed trait biff5
    extends StObject
       with BookType
  inline def biff5: biff5 = "biff5".asInstanceOf[biff5]
  
  @js.native
  sealed trait biff8
    extends StObject
       with BookType
  inline def biff8: biff8 = "biff8".asInstanceOf[biff8]
  
  @js.native
  sealed trait binary extends StObject
  inline def binary: binary = "binary".asInstanceOf[binary]
  
  @js.native
  sealed trait buffer extends StObject
  inline def buffer: buffer = "buffer".asInstanceOf[buffer]
  
  @js.native
  sealed trait chart
    extends StObject
       with SheetType
  inline def chart: chart = "chart".asInstanceOf[chart]
  
  @js.native
  sealed trait csv
    extends StObject
       with BookType
  inline def csv: csv = "csv".asInstanceOf[csv]
  
  @js.native
  sealed trait d
    extends StObject
       with ExcelDataType
  inline def d: d = "d".asInstanceOf[d]
  
  @js.native
  sealed trait dbf
    extends StObject
       with BookType
  inline def dbf: dbf = "dbf".asInstanceOf[dbf]
  
  @js.native
  sealed trait dif
    extends StObject
       with BookType
  inline def dif: dif = "dif".asInstanceOf[dif]
  
  @js.native
  sealed trait e
    extends StObject
       with ExcelDataType
  inline def e: e = "e".asInstanceOf[e]
  
  @js.native
  sealed trait eth
    extends StObject
       with BookType
  inline def eth: eth = "eth".asInstanceOf[eth]
  
  @js.native
  sealed trait file extends StObject
  inline def file: file = "file".asInstanceOf[file]
  
  @js.native
  sealed trait fods
    extends StObject
       with BookType
  inline def fods: fods = "fods".asInstanceOf[fods]
  
  @js.native
  sealed trait html
    extends StObject
       with BookType
  inline def html: html = "html".asInstanceOf[html]
  
  @js.native
  sealed trait n
    extends StObject
       with ExcelDataType
  inline def n: n = "n".asInstanceOf[n]
  
  @js.native
  sealed trait ods
    extends StObject
       with BookType
  inline def ods: ods = "ods".asInstanceOf[ods]
  
  @js.native
  sealed trait prn
    extends StObject
       with BookType
  inline def prn: prn = "prn".asInstanceOf[prn]
  
  @js.native
  sealed trait rtf
    extends StObject
       with BookType
  inline def rtf: rtf = "rtf".asInstanceOf[rtf]
  
  @js.native
  sealed trait s
    extends StObject
       with ExcelDataType
  inline def s: s = "s".asInstanceOf[s]
  
  @js.native
  sealed trait sheet
    extends StObject
       with SheetType
  inline def sheet: sheet = "sheet".asInstanceOf[sheet]
  
  @js.native
  sealed trait slk
    extends StObject
       with BookType
  inline def slk: slk = "slk".asInstanceOf[slk]
  
  @js.native
  sealed trait string extends StObject
  inline def string: string = "string".asInstanceOf[string]
  
  @js.native
  sealed trait sylk
    extends StObject
       with BookType
  inline def sylk: sylk = "sylk".asInstanceOf[sylk]
  
  @js.native
  sealed trait txt
    extends StObject
       with BookType
  inline def txt: txt = "txt".asInstanceOf[txt]
  
  @js.native
  sealed trait xla
    extends StObject
       with BookType
  inline def xla: xla = "xla".asInstanceOf[xla]
  
  @js.native
  sealed trait xlml
    extends StObject
       with BookType
  inline def xlml: xlml = "xlml".asInstanceOf[xlml]
  
  @js.native
  sealed trait xls
    extends StObject
       with BookType
  inline def xls: xls = "xls".asInstanceOf[xls]
  
  @js.native
  sealed trait xlsb
    extends StObject
       with BookType
  inline def xlsb: xlsb = "xlsb".asInstanceOf[xlsb]
  
  @js.native
  sealed trait xlsm
    extends StObject
       with BookType
  inline def xlsm: xlsm = "xlsm".asInstanceOf[xlsm]
  
  @js.native
  sealed trait xlsx
    extends StObject
       with BookType
  inline def xlsx: xlsx = "xlsx".asInstanceOf[xlsx]
  
  @js.native
  sealed trait z
    extends StObject
       with ExcelDataType
  inline def z: z = "z".asInstanceOf[z]
}
