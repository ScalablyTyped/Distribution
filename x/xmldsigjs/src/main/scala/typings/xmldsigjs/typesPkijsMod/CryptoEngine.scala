...
import typings.xmldsigjs.xmldsigjsStrings.jwk
import typings.xmldsigjs.xmldsigjsStrings.pkcs8
import typings.xmldsigjs.xmldsigjsStrings.raw
import typings.xmldsigjs.xmldsigjsStrings.spki
...
class CryptoEngine () extends SubtleCrypto {
  ...
  def digest(algorithm: NativeAlgorithmIdentifier, data: BufferSource): js.Thenable[ArrayBuffer] = js.native
  ...
  def wrapKey(format: String, key: CryptoKey, wrappingKey: CryptoKey, wrapAlgorithm: NativeAlgorithmIdentifier): js.Thenable[ArrayBuffer] = js.native
}
