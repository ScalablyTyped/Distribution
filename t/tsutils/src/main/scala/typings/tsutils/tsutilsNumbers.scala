package typings.tsutils

import typings.tsutils.utilUsageMod.DeclarationDomain
import typings.tsutils.utilUsageMod.UsageDomain
import typings.tsutils.utilUtilMod.ImportKind
import typings.tsutils.utilUtilMod.ImportOptions
import typings.tsutils.utilUtilMod.ScopeBoundary
import typings.tsutils.utilUtilMod.SideEffectOptions
import typings.tsutils.utilUtilMod.VariableDeclarationKind
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object tsutilsNumbers {
  @js.native
  sealed trait `0`
    extends ScopeBoundary
       with SideEffectOptions
       with VariableDeclarationKind
  
  @js.native
  sealed trait `1`
    extends DeclarationDomain
       with ImportKind
       with ImportOptions
       with ScopeBoundary
       with SideEffectOptions
       with UsageDomain
       with VariableDeclarationKind
  
  @js.native
  sealed trait `16`
    extends ImportKind
       with ImportOptions
  
  @js.native
  sealed trait `18`
    extends ImportKind
       with ImportOptions
  
  @js.native
  sealed trait `2`
    extends DeclarationDomain
       with ImportKind
       with ImportOptions
       with ScopeBoundary
       with SideEffectOptions
       with UsageDomain
       with VariableDeclarationKind
  
  @js.native
  sealed trait `24`
    extends ImportKind
       with ImportOptions
  
  @js.native
  sealed trait `27` extends ImportOptions
  
  @js.native
  sealed trait `3`
    extends ImportKind
       with ImportOptions
  
  @js.native
  sealed trait `31` extends ImportOptions
  
  @js.native
  sealed trait `32` extends ImportKind
  
  @js.native
  sealed trait `4`
    extends DeclarationDomain
       with ImportKind
       with ImportOptions
       with SideEffectOptions
       with UsageDomain
  
  @js.native
  sealed trait `5` extends UsageDomain
  
  @js.native
  sealed trait `59` extends ImportKind
  
  @js.native
  sealed trait `63` extends ImportKind
  
  @js.native
  sealed trait `7`
    extends DeclarationDomain
       with UsageDomain
  
  @js.native
  sealed trait `8`
    extends DeclarationDomain
       with ImportKind
       with ImportOptions
       with UsageDomain
  
  @scala.inline
  def `0`: `0` = 0.asInstanceOf[`0`]
  @scala.inline
  def `1`: `1` = 1.asInstanceOf[`1`]
  @scala.inline
  def `16`: `16` = 16.asInstanceOf[`16`]
  @scala.inline
  def `18`: `18` = 18.asInstanceOf[`18`]
  @scala.inline
  def `2`: `2` = 2.asInstanceOf[`2`]
  @scala.inline
  def `24`: `24` = 24.asInstanceOf[`24`]
  @scala.inline
  def `27`: `27` = 27.asInstanceOf[`27`]
  @scala.inline
  def `3`: `3` = 3.asInstanceOf[`3`]
  @scala.inline
  def `31`: `31` = 31.asInstanceOf[`31`]
  @scala.inline
  def `32`: `32` = 32.asInstanceOf[`32`]
  @scala.inline
  def `4`: `4` = 4.asInstanceOf[`4`]
  @scala.inline
  def `5`: `5` = 5.asInstanceOf[`5`]
  @scala.inline
  def `59`: `59` = 59.asInstanceOf[`59`]
  @scala.inline
  def `63`: `63` = 63.asInstanceOf[`63`]
  @scala.inline
  def `7`: `7` = 7.asInstanceOf[`7`]
  @scala.inline
  def `8`: `8` = 8.asInstanceOf[`8`]
}

