package typings
package atTinajsTinaLib.atTinajsTinaMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@tinajs/tina", "Component")
@js.native
class Component () extends js.Object {
  var data: org.scalablytyped.runtime.StringDictionary[js.Any] = js.native
  def setData(data: org.scalablytyped.runtime.StringDictionary[js.Any]): scala.Unit = js.native
}

@JSImport("@tinajs/tina", "Component")
@js.native
object Component extends js.Object {
  def define(definitions: stdLib.Partial[atTinajsTinaLib.atTinajsTinaMod.ComponentDefinitions]): scala.Unit = js.native
  def mixin(definitions: stdLib.Partial[atTinajsTinaLib.atTinajsTinaMod.ComponentDefinitions]): scala.Unit = js.native
}

