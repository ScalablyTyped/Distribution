...
object `@reactNavigationStackUtilsGestureHandlerRefContextMod` extends Shortcut {
  ...
  /* This means you don't have to write `default`, but can instead just say ``@reactNavigationStackUtilsGestureHandlerRefContextMod`.foo` */
