package typings.typeorm.migrationInterfaceMod

import typings.typeorm.queryRunnerMod.QueryRunner
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait MigrationInterface extends js.Object {
  
  /**
    * Reverse the migrations.
    */
  def down(queryRunner: QueryRunner): js.Promise[_] = js.native
  
  /**
    * Optional migration name, defaults to class name.
    */
  var name: js.UndefOr[String] = js.native
  
  /**
    * Run the migrations.
    */
  def up(queryRunner: QueryRunner): js.Promise[_] = js.native
}
object MigrationInterface {
  
  @scala.inline
  def apply(down: QueryRunner => js.Promise[_], up: QueryRunner => js.Promise[_]): MigrationInterface = {
    val __obj = js.Dynamic.literal(down = js.Any.fromFunction1(down), up = js.Any.fromFunction1(up))
    __obj.asInstanceOf[MigrationInterface]
  }
  
  @scala.inline
  implicit class MigrationInterfaceOps[Self <: MigrationInterface] (val x: Self) extends AnyVal {
    
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    
    @scala.inline
    def set(key: String, value: js.Any): Self = {
      x.asInstanceOf[js.Dynamic].updateDynamic(key)(value)
      x
    }
    
    @scala.inline
    def setDown(value: QueryRunner => js.Promise[_]): Self = this.set("down", js.Any.fromFunction1(value))
    
    @scala.inline
    def setUp(value: QueryRunner => js.Promise[_]): Self = this.set("up", js.Any.fromFunction1(value))
    
    @scala.inline
    def setName(value: String): Self = this.set("name", value.asInstanceOf[js.Any])
    
    @scala.inline
    def deleteName: Self = this.set("name", js.undefined)
  }
}
