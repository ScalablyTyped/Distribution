package typings
package terminalDashKitLib.terminalMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait Impl extends js.Object {
  @JSName("alternateScreenBuffer")
  var alternateScreenBuffer_Original: Terminal = js.native
  var app: java.lang.String = js.native
  var appName: java.lang.String = js.native
  @JSName("applicationKeypad")
  var applicationKeypad_Original: Terminal = js.native
  @JSName("backDelete")
  var backDelete_Original: Terminal = js.native
  @JSName("bell")
  var bell_Original: Terminal = js.native
  @JSName("bgBlack")
  var bgBlack_Original: CTerminal = js.native
  @JSName("bgBlue")
  var bgBlue_Original: CTerminal = js.native
  @JSName("bgBrightBlack")
  var bgBrightBlack_Original: CTerminal = js.native
  @JSName("bgBrightBlue")
  var bgBrightBlue_Original: CTerminal = js.native
  @JSName("bgBrightCyan")
  var bgBrightCyan_Original: CTerminal = js.native
  @JSName("bgBrightGreen")
  var bgBrightGreen_Original: CTerminal = js.native
  @JSName("bgBrightMagenta")
  var bgBrightMagenta_Original: CTerminal = js.native
  @JSName("bgBrightRed")
  var bgBrightRed_Original: CTerminal = js.native
  @JSName("bgBrightWhite")
  var bgBrightWhite_Original: CTerminal = js.native
  @JSName("bgBrightYellow")
  var bgBrightYellow_Original: CTerminal = js.native
  @JSName("bgCyan")
  var bgCyan_Original: CTerminal = js.native
  @JSName("bgDefaultColor")
  var bgDefaultColor_Original: CTerminal = js.native
  @JSName("bgGray")
  var bgGray_Original: CTerminal = js.native
  @JSName("bgGreen")
  var bgGreen_Original: CTerminal = js.native
  @JSName("bgGrey")
  var bgGrey_Original: CTerminal = js.native
  @JSName("bgMagenta")
  var bgMagenta_Original: CTerminal = js.native
  @JSName("bgRed")
  var bgRed_Original: CTerminal = js.native
  @JSName("bgWhite")
  var bgWhite_Original: CTerminal = js.native
  @JSName("bgYellow")
  var bgYellow_Original: CTerminal = js.native
  @JSName("black")
  var black_Original: CTerminal = js.native
  @JSName("blink")
  var blink_Original: CTerminal = js.native
  @JSName("blue")
  var blue_Original: CTerminal = js.native
  @JSName("bold")
  var bold_Original: CTerminal = js.native
  @JSName("brightBlack")
  var brightBlack_Original: CTerminal = js.native
  @JSName("brightBlue")
  var brightBlue_Original: CTerminal = js.native
  @JSName("brightCyan")
  var brightCyan_Original: CTerminal = js.native
  @JSName("brightGreen")
  var brightGreen_Original: CTerminal = js.native
  @JSName("brightMagenta")
  var brightMagenta_Original: CTerminal = js.native
  @JSName("brightRed")
  var brightRed_Original: CTerminal = js.native
  @JSName("brightWhite")
  var brightWhite_Original: CTerminal = js.native
  @JSName("brightYellow")
  var brightYellow_Original: CTerminal = js.native
  @JSName("clear")
  var clear_Original: Terminal = js.native
  @JSName("cyan")
  var cyan_Original: CTerminal = js.native
  @JSName("defaultColor")
  var defaultColor_Original: CTerminal = js.native
  @JSName("dim")
  var dim_Original: CTerminal = js.native
  @JSName("eraseDisplayAbove")
  var eraseDisplayAbove_Original: Terminal = js.native
  @JSName("eraseDisplayBelow")
  var eraseDisplayBelow_Original: Terminal = js.native
  @JSName("eraseDisplay")
  var eraseDisplay_Original: Terminal = js.native
  @JSName("eraseLineAfter")
  var eraseLineAfter_Original: Terminal = js.native
  @JSName("eraseLineBefore")
  var eraseLineBefore_Original: Terminal = js.native
  @JSName("eraseLine")
  var eraseLine_Original: Terminal = js.native
  @JSName("eraseScrollback")
  var eraseScrollback_Original: Terminal = js.native
  @JSName("error")
  var error_Original: Terminal = js.native
  @JSName("focusEvent")
  var focusEvent_Original: Terminal = js.native
  var generic: java.lang.String = js.native
  @JSName("gray")
  var gray_Original: CTerminal = js.native
  @JSName("green")
  var green_Original: CTerminal = js.native
  @JSName("grey")
  var grey_Original: CTerminal = js.native
  var height: scala.Double = js.native
  @JSName("hidden")
  var hidden_Original: CTerminal = js.native
  @JSName("hideCursor")
  var hideCursor_Original: Terminal = js.native
  @JSName("inverse")
  var inverse_Original: CTerminal = js.native
  @JSName("italic")
  var italic_Original: CTerminal = js.native
  @JSName("magenta")
  var magenta_Original: CTerminal = js.native
  @JSName("mouseButton")
  var mouseButton_Original: Terminal = js.native
  @JSName("mouseDrag")
  var mouseDrag_Original: Terminal = js.native
  @JSName("mouseMotion")
  var mouseMotion_Original: Terminal = js.native
  @JSName("mouseSGR")
  var mouseSGR_Original: Terminal = js.native
  @JSName("moveTo")
  var moveTo_Original: CTerminal = js.native
  @JSName("red")
  var red_Original: CTerminal = js.native
  @JSName("requestCursorLocation")
  var requestCursorLocation_Original: Terminal = js.native
  @JSName("requestScreenSize")
  var requestScreenSize_Original: Terminal = js.native
  @JSName("resetCursorColorRgb")
  var resetCursorColorRgb_Original: Terminal = js.native
  @JSName("resetDefaultBgColorRgb")
  var resetDefaultBgColorRgb_Original: Terminal = js.native
  @JSName("resetDefaultColorRgb")
  var resetDefaultColorRgb_Original: Terminal = js.native
  @JSName("resetHighlightBgColorRgb")
  var resetHighlightBgColorRgb_Original: Terminal = js.native
  @JSName("resetScrollingRegion")
  var resetScrollingRegion_Original: Terminal = js.native
  @JSName("reset")
  var reset_Original: Terminal = js.native
  @JSName("restoreCursor")
  var restoreCursor_Original: Terminal = js.native
  @JSName("saveCursor")
  var saveCursor_Original: Terminal = js.native
  @JSName("str")
  var str_Original: Terminal = js.native
  @JSName("strike")
  var strike_Original: CTerminal = js.native
  @JSName("styleReset")
  var styleReset_Original: CTerminal = js.native
  @JSName("tabClearAll")
  var tabClearAll_Original: Terminal = js.native
  @JSName("tabClear")
  var tabClear_Original: Terminal = js.native
  @JSName("tabSet")
  var tabSet_Original: Terminal = js.native
  var termconfigFile: java.lang.String = js.native
  @JSName("underline")
  var underline_Original: CTerminal = js.native
  var up: (js.Function1[/* n */ scala.Double, Terminal]) | (js.Function2[/* n */ scala.Double, /* repeated */ js.Any, Terminal]) = js.native
  @JSName("white")
  var white_Original: CTerminal = js.native
  var width: scala.Double = js.native
  @JSName("wrap")
  var wrap_Original: CTerminal = js.native
  @JSName("yellow")
  var yellow_Original: CTerminal = js.native
  def apply(): Terminal = js.native
  def apply(args: js.Any*): Terminal = js.native
  def apply(str: java.lang.String): Terminal = js.native
  def alternateScreenBuffer(): Terminal = js.native
  def alternateScreenBuffer(args: js.Any*): Terminal = js.native
  def alternateScreenBuffer(str: java.lang.String): Terminal = js.native
  def applicationKeypad(): Terminal = js.native
  def applicationKeypad(args: js.Any*): Terminal = js.native
  def applicationKeypad(str: java.lang.String): Terminal = js.native
  def backDelete(): Terminal = js.native
  def backDelete(args: js.Any*): Terminal = js.native
  def backDelete(str: java.lang.String): Terminal = js.native
  def backwardTab(n: scala.Double): Terminal = js.native
  def bar(value: scala.Double): scala.Unit = js.native
  def bar(value: scala.Double, options: terminalDashKitLib.Anon_BarStyle): scala.Unit = js.native
  def bell(): Terminal = js.native
  def bell(args: js.Any*): Terminal = js.native
  def bell(str: java.lang.String): Terminal = js.native
  def bgBlack(): Terminal = js.native
  def bgBlack(args: js.Any*): Terminal = js.native
  def bgBlack(str: java.lang.String): Terminal = js.native
  def bgBlue(): Terminal = js.native
  def bgBlue(args: js.Any*): Terminal = js.native
  def bgBlue(str: java.lang.String): Terminal = js.native
  def bgBrightBlack(): Terminal = js.native
  def bgBrightBlack(args: js.Any*): Terminal = js.native
  def bgBrightBlack(str: java.lang.String): Terminal = js.native
  def bgBrightBlue(): Terminal = js.native
  def bgBrightBlue(args: js.Any*): Terminal = js.native
  def bgBrightBlue(str: java.lang.String): Terminal = js.native
  def bgBrightColor(register: scala.Double): Terminal = js.native
  def bgBrightCyan(): Terminal = js.native
  def bgBrightCyan(args: js.Any*): Terminal = js.native
  def bgBrightCyan(str: java.lang.String): Terminal = js.native
  def bgBrightGreen(): Terminal = js.native
  def bgBrightGreen(args: js.Any*): Terminal = js.native
  def bgBrightGreen(str: java.lang.String): Terminal = js.native
  def bgBrightMagenta(): Terminal = js.native
  def bgBrightMagenta(args: js.Any*): Terminal = js.native
  def bgBrightMagenta(str: java.lang.String): Terminal = js.native
  def bgBrightRed(): Terminal = js.native
  def bgBrightRed(args: js.Any*): Terminal = js.native
  def bgBrightRed(str: java.lang.String): Terminal = js.native
  def bgBrightWhite(): Terminal = js.native
  def bgBrightWhite(args: js.Any*): Terminal = js.native
  def bgBrightWhite(str: java.lang.String): Terminal = js.native
  def bgBrightYellow(): Terminal = js.native
  def bgBrightYellow(args: js.Any*): Terminal = js.native
  def bgBrightYellow(str: java.lang.String): Terminal = js.native
  def bgColor(register: scala.Double): Terminal = js.native
  def bgColor256(register: scala.Double): Terminal = js.native
  def bgColorGrayscale(I: scala.Double): Terminal = js.native
  def bgColorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def bgColorRgbHex(rgb: java.lang.String): Terminal = js.native
  def bgCyan(): Terminal = js.native
  def bgCyan(args: js.Any*): Terminal = js.native
  def bgCyan(str: java.lang.String): Terminal = js.native
  def bgDarkColor(register: scala.Double): Terminal = js.native
  def bgDefaultColor(): Terminal = js.native
  def bgDefaultColor(args: js.Any*): Terminal = js.native
  def bgDefaultColor(str: java.lang.String): Terminal = js.native
  def bgGray(): Terminal = js.native
  def bgGray(args: js.Any*): Terminal = js.native
  def bgGray(str: java.lang.String): Terminal = js.native
  def bgGreen(): Terminal = js.native
  def bgGreen(args: js.Any*): Terminal = js.native
  def bgGreen(str: java.lang.String): Terminal = js.native
  def bgGrey(): Terminal = js.native
  def bgGrey(args: js.Any*): Terminal = js.native
  def bgGrey(str: java.lang.String): Terminal = js.native
  def bgMagenta(): Terminal = js.native
  def bgMagenta(args: js.Any*): Terminal = js.native
  def bgMagenta(str: java.lang.String): Terminal = js.native
  def bgRed(): Terminal = js.native
  def bgRed(args: js.Any*): Terminal = js.native
  def bgRed(str: java.lang.String): Terminal = js.native
  def bgWhite(): Terminal = js.native
  def bgWhite(args: js.Any*): Terminal = js.native
  def bgWhite(str: java.lang.String): Terminal = js.native
  def bgYellow(): Terminal = js.native
  def bgYellow(args: js.Any*): Terminal = js.native
  def bgYellow(str: java.lang.String): Terminal = js.native
  def bindArgs(args: js.Any*): Terminal = js.native
  def black(): Terminal = js.native
  def black(args: js.Any*): Terminal = js.native
  def black(str: java.lang.String): Terminal = js.native
  def blink(): Terminal = js.native
  def blink(args: js.Any*): Terminal = js.native
  def blink(str: java.lang.String): Terminal = js.native
  def blue(): Terminal = js.native
  def blue(args: js.Any*): Terminal = js.native
  def blue(str: java.lang.String): Terminal = js.native
  def bold(): Terminal = js.native
  def bold(args: js.Any*): Terminal = js.native
  def bold(str: java.lang.String): Terminal = js.native
  def brightBlack(): Terminal = js.native
  def brightBlack(args: js.Any*): Terminal = js.native
  def brightBlack(str: java.lang.String): Terminal = js.native
  def brightBlue(): Terminal = js.native
  def brightBlue(args: js.Any*): Terminal = js.native
  def brightBlue(str: java.lang.String): Terminal = js.native
  def brightColor(register: scala.Double): Terminal = js.native
  def brightCyan(): Terminal = js.native
  def brightCyan(args: js.Any*): Terminal = js.native
  def brightCyan(str: java.lang.String): Terminal = js.native
  def brightGreen(): Terminal = js.native
  def brightGreen(args: js.Any*): Terminal = js.native
  def brightGreen(str: java.lang.String): Terminal = js.native
  def brightMagenta(): Terminal = js.native
  def brightMagenta(args: js.Any*): Terminal = js.native
  def brightMagenta(str: java.lang.String): Terminal = js.native
  def brightRed(): Terminal = js.native
  def brightRed(args: js.Any*): Terminal = js.native
  def brightRed(str: java.lang.String): Terminal = js.native
  def brightWhite(): Terminal = js.native
  def brightWhite(args: js.Any*): Terminal = js.native
  def brightWhite(str: java.lang.String): Terminal = js.native
  def brightYellow(): Terminal = js.native
  def brightYellow(args: js.Any*): Terminal = js.native
  def brightYellow(str: java.lang.String): Terminal = js.native
  def clear(): Terminal = js.native
  def clear(args: js.Any*): Terminal = js.native
  def clear(str: java.lang.String): Terminal = js.native
  def color(register: scala.Double): Terminal = js.native
  def color256(register: scala.Double): Terminal = js.native
  def colorGrayscale(I: scala.Double): Terminal = js.native
  def colorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def colorRgbHex(rgb: java.lang.String): Terminal = js.native
  def column(x: scala.Double): Terminal = js.native
  def cwd(uri: java.lang.String): Terminal = js.native
  def cyan(): Terminal = js.native
  def cyan(args: js.Any*): Terminal = js.native
  def cyan(str: java.lang.String): Terminal = js.native
  def darkColor(register: scala.Double): Terminal = js.native
  def defaultColor(): Terminal = js.native
  def defaultColor(args: js.Any*): Terminal = js.native
  def defaultColor(str: java.lang.String): Terminal = js.native
  def delete(n: scala.Double): Terminal = js.native
  def deleteLine(n: scala.Double): Terminal = js.native
  def dim(): Terminal = js.native
  def dim(args: js.Any*): Terminal = js.native
  def dim(str: java.lang.String): Terminal = js.native
  def down(n: scala.Double): Terminal = js.native
  def drawImage(url: java.lang.String): js.Promise[scala.Unit] = js.native
  def drawImage(url: java.lang.String, callback: Callback[scala.Unit]): scala.Unit = js.native
  def drawImage(url: java.lang.String, options: terminalDashKitLib.Anon_ShrinkAnonHeight): js.Promise[scala.Unit] = js.native
  def drawImage(
    url: java.lang.String,
    options: terminalDashKitLib.Anon_ShrinkAnonHeight,
    callback: Callback[scala.Unit]
  ): scala.Unit = js.native
  def eraseArea(x: scala.Double, y: scala.Double): Terminal = js.native
  def eraseArea(x: scala.Double, y: scala.Double, width: scala.Double): Terminal = js.native
  def eraseArea(x: scala.Double, y: scala.Double, width: scala.Double, height: scala.Double): Terminal = js.native
  def eraseDisplay(): Terminal = js.native
  def eraseDisplay(args: js.Any*): Terminal = js.native
  def eraseDisplay(str: java.lang.String): Terminal = js.native
  def eraseDisplayAbove(): Terminal = js.native
  def eraseDisplayAbove(args: js.Any*): Terminal = js.native
  def eraseDisplayAbove(str: java.lang.String): Terminal = js.native
  def eraseDisplayBelow(): Terminal = js.native
  def eraseDisplayBelow(args: js.Any*): Terminal = js.native
  def eraseDisplayBelow(str: java.lang.String): Terminal = js.native
  def eraseLine(): Terminal = js.native
  def eraseLine(args: js.Any*): Terminal = js.native
  def eraseLine(str: java.lang.String): Terminal = js.native
  def eraseLineAfter(): Terminal = js.native
  def eraseLineAfter(args: js.Any*): Terminal = js.native
  def eraseLineAfter(str: java.lang.String): Terminal = js.native
  def eraseLineBefore(): Terminal = js.native
  def eraseLineBefore(args: js.Any*): Terminal = js.native
  def eraseLineBefore(str: java.lang.String): Terminal = js.native
  def eraseScrollback(): Terminal = js.native
  def eraseScrollback(args: js.Any*): Terminal = js.native
  def eraseScrollback(str: java.lang.String): Terminal = js.native
  def error(): Terminal = js.native
  def error(args: js.Any*): Terminal = js.native
  def error(str: java.lang.String): Terminal = js.native
  def fileInput(): js.Promise[java.lang.String] = js.native
  def fileInput(options: IFileInputOptions): js.Promise[java.lang.String] = js.native
  def fileInput(options: IFileInputOptions, callback: Callback[java.lang.String]): scala.Unit = js.native
  def focusEvent(): Terminal = js.native
  def focusEvent(args: js.Any*): Terminal = js.native
  def focusEvent(str: java.lang.String): Terminal = js.native
  def forwardTab(n: scala.Double): Terminal = js.native
  def fullscreen(options: scala.Boolean): scala.Unit = js.native
  def fullscreen(options: terminalDashKitLib.Anon_NoAlternate): scala.Unit = js.native
  def getColor(register: scala.Double): scala.Unit = js.native
  def getColor(register: scala.Double, callback: Callback[terminalDashKitLib.Anon_B]): scala.Unit = js.native
  def getCursorLocation(
    callback: js.Function3[
      /* error */ js.Any, 
      /* x */ js.UndefOr[scala.Double], 
      /* y */ js.UndefOr[scala.Double], 
      scala.Unit
    ]
  ): scala.Unit = js.native
  def getPalette(register: scala.Double): scala.Unit = js.native
  def getPalette(register: scala.Double, callback: Callback[Palette]): scala.Unit = js.native
  def grabInput(options: scala.Boolean): scala.Unit = js.native
  def grabInput(options: scala.Boolean, safeCallback: scala.Boolean): scala.Unit = js.native
  def grabInput(options: terminalDashKitLib.Anon_Button): scala.Unit = js.native
  def grabInput(options: terminalDashKitLib.Anon_Button, safeCallback: scala.Boolean): scala.Unit = js.native
  def gray(): Terminal = js.native
  def gray(args: js.Any*): Terminal = js.native
  def gray(str: java.lang.String): Terminal = js.native
  def green(): Terminal = js.native
  def green(args: js.Any*): Terminal = js.native
  def green(str: java.lang.String): Terminal = js.native
  def grey(): Terminal = js.native
  def grey(args: js.Any*): Terminal = js.native
  def grey(str: java.lang.String): Terminal = js.native
  def gridMenu(menuItems: js.Array[java.lang.String]): terminalDashKitLib.Anon_PromiseGridMenuResponse = js.native
  def gridMenu(menuItems: js.Array[java.lang.String], callback: Callback[GridMenuResponse]): scala.Unit = js.native
  def gridMenu(menuItems: js.Array[java.lang.String], options: GridMenuOptions): terminalDashKitLib.Anon_PromiseGridMenuResponse = js.native
  def gridMenu(
    menuItems: js.Array[java.lang.String],
    options: GridMenuOptions,
    callback: Callback[GridMenuResponse]
  ): scala.Unit = js.native
  def hidden(): Terminal = js.native
  def hidden(args: js.Any*): Terminal = js.native
  def hidden(str: java.lang.String): Terminal = js.native
  def hideCursor(): Terminal = js.native
  def hideCursor(args: js.Any*): Terminal = js.native
  def hideCursor(str: java.lang.String): Terminal = js.native
  def iconName(str: java.lang.String): Terminal = js.native
  def inputField(): terminalDashKitLib.Anon_Abort = js.native
  def inputField(callback: Callback[java.lang.String]): scala.Unit = js.native
  def inputField(options: InputFieldOptions): terminalDashKitLib.Anon_Abort = js.native
  def inputField(options: InputFieldOptions, callback: Callback[java.lang.String]): scala.Unit = js.native
  def insert(n: scala.Double): Terminal = js.native
  def insertLine(n: scala.Double): Terminal = js.native
  def inverse(): Terminal = js.native
  def inverse(args: js.Any*): Terminal = js.native
  def inverse(str: java.lang.String): Terminal = js.native
  def italic(): Terminal = js.native
  def italic(args: js.Any*): Terminal = js.native
  def italic(str: java.lang.String): Terminal = js.native
  def left(n: scala.Double): Terminal = js.native
  def magenta(): Terminal = js.native
  def magenta(args: js.Any*): Terminal = js.native
  def magenta(str: java.lang.String): Terminal = js.native
  def markupOnly(str: java.lang.String): Terminal = js.native
  def mouseButton(): Terminal = js.native
  def mouseButton(args: js.Any*): Terminal = js.native
  def mouseButton(str: java.lang.String): Terminal = js.native
  def mouseDrag(): Terminal = js.native
  def mouseDrag(args: js.Any*): Terminal = js.native
  def mouseDrag(str: java.lang.String): Terminal = js.native
  def mouseMotion(): Terminal = js.native
  def mouseMotion(args: js.Any*): Terminal = js.native
  def mouseMotion(str: java.lang.String): Terminal = js.native
  def mouseSGR(): Terminal = js.native
  def mouseSGR(args: js.Any*): Terminal = js.native
  def mouseSGR(str: java.lang.String): Terminal = js.native
  def move(x: scala.Double, y: scala.Double): Terminal = js.native
  def moveTo(): Terminal = js.native
  def moveTo(args: js.Any*): Terminal = js.native
  def moveTo(str: java.lang.String): Terminal = js.native
  def nextLine(n: scala.Double): Terminal = js.native
  def noFormat(str: java.lang.String): Terminal = js.native
  def notify(title: java.lang.String, text: java.lang.String): Terminal = js.native
  def previousLine(n: scala.Double): Terminal = js.native
  def processExit(code: scala.Double): scala.Unit = js.native
  def progressBar(): ProgressBarController = js.native
  def progressBar(options: ProgressBarOptions): ProgressBarController = js.native
  def red(): Terminal = js.native
  def red(args: js.Any*): Terminal = js.native
  def red(str: java.lang.String): Terminal = js.native
  def requestColor(n: scala.Double): Terminal = js.native
  def requestCursorLocation(): Terminal = js.native
  def requestCursorLocation(args: js.Any*): Terminal = js.native
  def requestCursorLocation(str: java.lang.String): Terminal = js.native
  def requestScreenSize(): Terminal = js.native
  def requestScreenSize(args: js.Any*): Terminal = js.native
  def requestScreenSize(str: java.lang.String): Terminal = js.native
  def reset(): Terminal = js.native
  def reset(args: js.Any*): Terminal = js.native
  def reset(str: java.lang.String): Terminal = js.native
  def resetCursorColorRgb(): Terminal = js.native
  def resetCursorColorRgb(args: js.Any*): Terminal = js.native
  def resetCursorColorRgb(str: java.lang.String): Terminal = js.native
  def resetDefaultBgColorRgb(): Terminal = js.native
  def resetDefaultBgColorRgb(args: js.Any*): Terminal = js.native
  def resetDefaultBgColorRgb(str: java.lang.String): Terminal = js.native
  def resetDefaultColorRgb(): Terminal = js.native
  def resetDefaultColorRgb(args: js.Any*): Terminal = js.native
  def resetDefaultColorRgb(str: java.lang.String): Terminal = js.native
  def resetHighlightBgColorRgb(): Terminal = js.native
  def resetHighlightBgColorRgb(args: js.Any*): Terminal = js.native
  def resetHighlightBgColorRgb(str: java.lang.String): Terminal = js.native
  def resetScrollingRegion(): Terminal = js.native
  def resetScrollingRegion(args: js.Any*): Terminal = js.native
  def resetScrollingRegion(str: java.lang.String): Terminal = js.native
  def restoreCursor(): Terminal = js.native
  def restoreCursor(args: js.Any*): Terminal = js.native
  def restoreCursor(str: java.lang.String): Terminal = js.native
  def right(n: scala.Double): Terminal = js.native
  def saveCursor(): Terminal = js.native
  def saveCursor(args: js.Any*): Terminal = js.native
  def saveCursor(str: java.lang.String): Terminal = js.native
  def scrollDown(n: scala.Double): Terminal = js.native
  def scrollUp(n: scala.Double): Terminal = js.native
  def scrollingRegion(top: scala.Double, bottom: scala.Double): Terminal = js.native
  def setColor(register: scala.Double, rgb: terminalDashKitLib.Anon_B): scala.Unit = js.native
  def setColor(register: scala.Double, rgb: terminalDashKitLib.Anon_B, names: js.Array[java.lang.String]): scala.Unit = js.native
  def setColor(register: scala.Double, r: scala.Double, g: scala.Double, b: scala.Double): scala.Unit = js.native
  def setColor(
    register: scala.Double,
    r: scala.Double,
    g: scala.Double,
    b: scala.Double,
    names: js.Array[java.lang.String]
  ): scala.Unit = js.native
  def setCursorColor(register: scala.Double): Terminal = js.native
  def setCursorColorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def setDefaultBgColorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def setDefaultColorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def setHighlightBgColorRgb(r: scala.Double, g: scala.Double, b: scala.Double): Terminal = js.native
  def setPalette(palette: java.lang.String): scala.Unit = js.native
  def setPalette(palette: Palette): scala.Unit = js.native
  def singleColumnMenu(menuItems: js.Array[java.lang.String]): terminalDashKitLib.Anon_PromiseSingleColumnMenuResponse = js.native
  def singleColumnMenu(menuItems: js.Array[java.lang.String], callback: Callback[SingleColumnMenuResponse]): scala.Unit = js.native
  def singleColumnMenu(menuItems: js.Array[java.lang.String], options: SingleColumnMenuOptions): terminalDashKitLib.Anon_PromiseSingleColumnMenuResponse = js.native
  def singleColumnMenu(
    menuItems: js.Array[java.lang.String],
    options: SingleColumnMenuOptions,
    callback: Callback[SingleColumnMenuResponse]
  ): scala.Unit = js.native
  def singleLineMenu(menuItems: js.Array[java.lang.String]): terminalDashKitLib.Anon_Promise = js.native
  def singleLineMenu(menuItems: js.Array[java.lang.String], callback: Callback[SingleLineMenuResponse]): scala.Unit = js.native
  def singleLineMenu(menuItems: js.Array[java.lang.String], options: SingleLineMenuOptions): terminalDashKitLib.Anon_Promise = js.native
  def singleLineMenu(
    menuItems: js.Array[java.lang.String],
    options: SingleLineMenuOptions,
    callback: Callback[SingleLineMenuResponse]
  ): scala.Unit = js.native
  def singleRowMenu(menuItems: js.Array[java.lang.String]): terminalDashKitLib.Anon_Promise = js.native
  def singleRowMenu(menuItems: js.Array[java.lang.String], callback: Callback[SingleLineMenuResponse]): scala.Unit = js.native
  def singleRowMenu(menuItems: js.Array[java.lang.String], options: SingleLineMenuOptions): terminalDashKitLib.Anon_Promise = js.native
  def singleRowMenu(
    menuItems: js.Array[java.lang.String],
    options: SingleLineMenuOptions,
    callback: Callback[SingleLineMenuResponse]
  ): scala.Unit = js.native
  def slowTyping(str: java.lang.String): js.Promise[scala.Unit] = js.native
  def slowTyping(str: java.lang.String, callback: Callback[scala.Unit]): scala.Unit = js.native
  def slowTyping(str: java.lang.String, options: terminalDashKitLib.Anon_Delay): js.Promise[scala.Unit] = js.native
  def slowTyping(str: java.lang.String, options: terminalDashKitLib.Anon_Delay, callback: Callback[scala.Unit]): scala.Unit = js.native
  def str(): Terminal = js.native
  def str(args: js.Any*): Terminal = js.native
  def str(str: java.lang.String): Terminal = js.native
  def strike(): Terminal = js.native
  def strike(args: js.Any*): Terminal = js.native
  def strike(str: java.lang.String): Terminal = js.native
  def styleReset(): Terminal = js.native
  def styleReset(args: js.Any*): Terminal = js.native
  def styleReset(str: java.lang.String): Terminal = js.native
  def tabClear(): Terminal = js.native
  def tabClear(args: js.Any*): Terminal = js.native
  def tabClear(str: java.lang.String): Terminal = js.native
  def tabClearAll(): Terminal = js.native
  def tabClearAll(args: js.Any*): Terminal = js.native
  def tabClearAll(str: java.lang.String): Terminal = js.native
  def tabSet(): Terminal = js.native
  def tabSet(args: js.Any*): Terminal = js.native
  def tabSet(str: java.lang.String): Terminal = js.native
  def underline(): Terminal = js.native
  def underline(args: js.Any*): Terminal = js.native
  def underline(str: java.lang.String): Terminal = js.native
  def white(): Terminal = js.native
  def white(args: js.Any*): Terminal = js.native
  def white(str: java.lang.String): Terminal = js.native
  def windowTitle(str: java.lang.String): Terminal = js.native
  def wrap(): Terminal = js.native
  def wrap(args: js.Any*): Terminal = js.native
  def wrap(str: java.lang.String): Terminal = js.native
  def wrapColumn(): scala.Unit = js.native
  def wrapColumn(options: terminalDashKitLib.Anon_Continue): scala.Unit = js.native
  def wrapColumn(x: js.UndefOr[scala.Nothing], width: scala.Double): scala.Unit = js.native
  def wrapColumn(x: scala.Double, width: scala.Double): scala.Unit = js.native
  def yellow(): Terminal = js.native
  def yellow(args: js.Any*): Terminal = js.native
  def yellow(str: java.lang.String): Terminal = js.native
  def yesOrNo(): terminalDashKitLib.Anon_Abort = js.native
  def yesOrNo(options: YesOrNoOptions): terminalDashKitLib.Anon_Abort = js.native
  def yesOrNo(options: YesOrNoOptions, callback: Callback[scala.Boolean]): scala.Unit = js.native
}

