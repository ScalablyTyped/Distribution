...
object mod {
  ...
  object DirectoryResult {
    ...
    inline def apply(cleanup: () => Unit, path: String): DirectoryResult = {
  ...
  object FileResult {
    ...
    inline def apply(cleanup: () => Unit, fd: Double, path: String): FileResult = {
