package typings.three

import typings.three.threeMod.BufferGeometry
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object boxLineGeometryMod {
  
  @JSImport("three/examples/jsm/geometries/BoxLineGeometry", "BoxLineGeometry")
  @js.native
  open class BoxLineGeometry protected () extends BufferGeometry {
    def this(
      width: js.UndefOr[Double],
      height: js.UndefOr[Double],
      depth: js.UndefOr[Double],
      widthSegments: js.UndefOr[Double],
      heightSegments: js.UndefOr[Double],
      depthSegments: js.UndefOr[Double]
    ) = this()
  }
}
