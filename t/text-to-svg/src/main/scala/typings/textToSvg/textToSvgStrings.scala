package typings.textToSvg

import typings.textToSvg.mod.Anchor
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object textToSvgStrings {
  
  @js.native
  sealed trait baseline
    extends StObject
       with Anchor
  inline def baseline: baseline = "baseline".asInstanceOf[baseline]
  
  @js.native
  sealed trait bottom
    extends StObject
       with Anchor
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait center
    extends StObject
       with Anchor
  inline def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait `center baseline`
    extends StObject
       with Anchor
  inline def `center baseline`: `center baseline` = ("center baseline").asInstanceOf[`center baseline`]
  
  @js.native
  sealed trait `center bottom`
    extends StObject
       with Anchor
  inline def `center bottom`: `center bottom` = ("center bottom").asInstanceOf[`center bottom`]
  
  @js.native
  sealed trait `center middle`
    extends StObject
       with Anchor
  inline def `center middle`: `center middle` = ("center middle").asInstanceOf[`center middle`]
  
  @js.native
  sealed trait `center top`
    extends StObject
       with Anchor
  inline def `center top`: `center top` = ("center top").asInstanceOf[`center top`]
  
  @js.native
  sealed trait left
    extends StObject
       with Anchor
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait `left baseline`
    extends StObject
       with Anchor
  inline def `left baseline`: `left baseline` = ("left baseline").asInstanceOf[`left baseline`]
  
  @js.native
  sealed trait `left bottom`
    extends StObject
       with Anchor
  inline def `left bottom`: `left bottom` = ("left bottom").asInstanceOf[`left bottom`]
  
  @js.native
  sealed trait `left middle`
    extends StObject
       with Anchor
  inline def `left middle`: `left middle` = ("left middle").asInstanceOf[`left middle`]
  
  @js.native
  sealed trait `left top`
    extends StObject
       with Anchor
  inline def `left top`: `left top` = ("left top").asInstanceOf[`left top`]
  
  @js.native
  sealed trait middle
    extends StObject
       with Anchor
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait right
    extends StObject
       with Anchor
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait `right baseline`
    extends StObject
       with Anchor
  inline def `right baseline`: `right baseline` = ("right baseline").asInstanceOf[`right baseline`]
  
  @js.native
  sealed trait `right bottom`
    extends StObject
       with Anchor
  inline def `right bottom`: `right bottom` = ("right bottom").asInstanceOf[`right bottom`]
  
  @js.native
  sealed trait `right middle`
    extends StObject
       with Anchor
  inline def `right middle`: `right middle` = ("right middle").asInstanceOf[`right middle`]
  
  @js.native
  sealed trait `right top`
    extends StObject
       with Anchor
  inline def `right top`: `right top` = ("right top").asInstanceOf[`right top`]
  
  @js.native
  sealed trait top
    extends StObject
       with Anchor
  inline def top: top = "top".asInstanceOf[top]
}
