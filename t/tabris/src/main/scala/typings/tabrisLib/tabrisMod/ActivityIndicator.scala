...
class ActivityIndicator () extends Widget {
  ...
  /**
    ...
    */
  def off(listeners: ActivityIndicatorEvents): this.type = js.native
  /**
    ...
    */
  def on(listeners: ActivityIndicatorEvents): this.type = js.native
  /**
    * Same as `on`, but removes the listener after it has been invoked by an event.
    * @param listeners A key-value map where the keys are event types and the values are the listeners to register for these events, e.g. `{tap: onTap, scroll: onScroll}`.
    */
  def once(listeners: ActivityIndicatorEvents): this.type = js.native
}
