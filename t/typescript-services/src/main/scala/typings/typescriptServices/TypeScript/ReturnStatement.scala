...
trait ReturnStatement
  ...
  def structuralEquals(ast: ReturnStatement, includingPosition: Boolean): Boolean
}
object ReturnStatement {
  ...
  inline def apply(
    ...
    start: () => Double,
    structuralEquals: (ReturnStatement, Boolean) => Boolean,
    syntaxID: () => Double,
  ...
  implicit open class MutableBuilder[Self <: ReturnStatement] (val x: Self) extends AnyVal {
    ...
    inline def setStructuralEquals(value: (ReturnStatement, Boolean) => Boolean): Self = StObject.set(x, "structuralEquals", js.Any.fromFunction2(value))
  }
}
