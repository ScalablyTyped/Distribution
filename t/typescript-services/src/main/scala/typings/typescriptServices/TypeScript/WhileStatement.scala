...
object WhileStatement {
  ...
  def apply(
    ...
    start: () => Double,
    statement: AST,
    structuralEquals: (WhileStatement, Boolean) => Boolean,
    syntaxID: () => Double,
  ...
  implicit class WhileStatementOps[Self <: WhileStatement] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setStructuralEquals(value: (WhileStatement, Boolean) => Boolean): Self = this.set("structuralEquals", js.Any.fromFunction2(value))
  }
}
