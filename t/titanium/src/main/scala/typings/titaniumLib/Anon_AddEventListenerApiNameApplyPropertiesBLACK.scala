...
object Anon_AddEventListenerApiNameApplyPropertiesBLACK {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(BLACK = BLACK, DEFAULT = DEFAULT, WHITE = WHITE, addEventListener = addEventListener, apiName = apiName, applyProperties = applyProperties, bubbleParent = bubbleParent, fireEvent = fireEvent, getApiName = getApiName, getBubbleParent = getBubbleParent, removeEventListener = removeEventListener, setBubbleParent = setBubbleParent)
