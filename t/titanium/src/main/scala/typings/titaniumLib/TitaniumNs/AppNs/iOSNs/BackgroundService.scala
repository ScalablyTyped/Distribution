...
object BackgroundService {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addEventListener = addEventListener, apiName = apiName, applyProperties = applyProperties, bubbleParent = bubbleParent, fireEvent = fireEvent, getApiName = getApiName, getBubbleParent = getBubbleParent, getUrl = getUrl, removeEventListener = removeEventListener, setBubbleParent = setBubbleParent, setUrl = setUrl, stop = stop, unregister = unregister, url = url)
