...
object Notation {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(addEventListener = addEventListener, apiName = apiName, applyProperties = applyProperties, bubbleParent = bubbleParent, fireEvent = fireEvent, getApiName = getApiName, getBubbleParent = getBubbleParent, getPublicId = getPublicId, getSystemId = getSystemId, publicId = publicId, removeEventListener = removeEventListener, setBubbleParent = setBubbleParent, systemId = systemId)
