...
trait CardanoSignedTxWitness extends StObject {
  ...
  var `type`: typings.trezorConnect.protobufMod.CardanoTxWitnessType
}
object CardanoSignedTxWitness {
  
  inline def apply(pubKey: String, signature: String, `type`: typings.trezorConnect.protobufMod.CardanoTxWitnessType): CardanoSignedTxWitness = {
  ...
  extension [Self <: CardanoSignedTxWitness](x: Self) {
    ...
    inline def setType(value: typings.trezorConnect.protobufMod.CardanoTxWitnessType): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
