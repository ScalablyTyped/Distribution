package typings.typedoc.mod.TypeScript

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("typedoc", "TypeScript.ScriptElementKindModifier")
@js.native
object ScriptElementKindModifier extends StObject {
  
  @JSBracketAccess
  def apply(value: String): js.UndefOr[typings.typescript.mod.ScriptElementKindModifier & String] = js.native
  
  /* "abstract" */ val abstractModifier: typings.typescript.mod.ScriptElementKindModifier.abstractModifier & String = js.native
  
  /* "declare" */ val ambientModifier: typings.typescript.mod.ScriptElementKindModifier.ambientModifier & String = js.native
  
  /* ".cjs" */ val cjsModifier: typings.typescript.mod.ScriptElementKindModifier.cjsModifier & String = js.native
  
  /* ".cts" */ val ctsModifier: typings.typescript.mod.ScriptElementKindModifier.ctsModifier & String = js.native
  
  /* ".d.cts" */ val dctsModifier: typings.typescript.mod.ScriptElementKindModifier.dctsModifier & String = js.native
  
  /* "deprecated" */ val deprecatedModifier: typings.typescript.mod.ScriptElementKindModifier.deprecatedModifier & String = js.native
  
  /* ".d.mts" */ val dmtsModifier: typings.typescript.mod.ScriptElementKindModifier.dmtsModifier & String = js.native
  
  /* ".d.ts" */ val dtsModifier: typings.typescript.mod.ScriptElementKindModifier.dtsModifier & String = js.native
  
  /* "export" */ val exportedModifier: typings.typescript.mod.ScriptElementKindModifier.exportedModifier & String = js.native
  
  /* ".js" */ val jsModifier: typings.typescript.mod.ScriptElementKindModifier.jsModifier & String = js.native
  
  /* ".json" */ val jsonModifier: typings.typescript.mod.ScriptElementKindModifier.jsonModifier & String = js.native
  
  /* ".jsx" */ val jsxModifier: typings.typescript.mod.ScriptElementKindModifier.jsxModifier & String = js.native
  
  /* ".mjs" */ val mjsModifier: typings.typescript.mod.ScriptElementKindModifier.mjsModifier & String = js.native
  
  /* ".mts" */ val mtsModifier: typings.typescript.mod.ScriptElementKindModifier.mtsModifier & String = js.native
  
  /* "" */ val none: typings.typescript.mod.ScriptElementKindModifier.none & String = js.native
  
  /* "optional" */ val optionalModifier: typings.typescript.mod.ScriptElementKindModifier.optionalModifier & String = js.native
  
  /* "private" */ val privateMemberModifier: typings.typescript.mod.ScriptElementKindModifier.privateMemberModifier & String = js.native
  
  /* "protected" */ val protectedMemberModifier: typings.typescript.mod.ScriptElementKindModifier.protectedMemberModifier & String = js.native
  
  /* "public" */ val publicMemberModifier: typings.typescript.mod.ScriptElementKindModifier.publicMemberModifier & String = js.native
  
  /* "static" */ val staticModifier: typings.typescript.mod.ScriptElementKindModifier.staticModifier & String = js.native
  
  /* ".ts" */ val tsModifier: typings.typescript.mod.ScriptElementKindModifier.tsModifier & String = js.native
  
  /* ".tsx" */ val tsxModifier: typings.typescript.mod.ScriptElementKindModifier.tsxModifier & String = js.native
}
