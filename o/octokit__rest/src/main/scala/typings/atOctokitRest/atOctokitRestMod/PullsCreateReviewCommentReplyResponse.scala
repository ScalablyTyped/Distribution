...
object PullsCreateReviewCommentReplyResponse {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(_links = _links, author_association = author_association, body = body, commit_id = commit_id, created_at = created_at, diff_hunk = diff_hunk, html_url = html_url, id = id, node_id = node_id, original_commit_id = original_commit_id, original_position = original_position, path = path, position = position, pull_request_review_id = pull_request_review_id, pull_request_url = pull_request_url, updated_at = updated_at, url = url, user = user)
