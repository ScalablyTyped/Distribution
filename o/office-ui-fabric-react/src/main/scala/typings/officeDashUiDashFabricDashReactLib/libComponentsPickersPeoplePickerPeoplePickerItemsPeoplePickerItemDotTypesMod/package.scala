package typings
package officeDashUiDashFabricDashReactLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

package object libComponentsPickersPeoplePickerPeoplePickerItemsPeoplePickerItemDotTypesMod {
  type IPeoplePickerItemSelectedStyleProps = (stdLib.Required[
    stdLib.Pick[
      IPeoplePickerItemSelectedProps, 
      officeDashUiDashFabricDashReactLib.officeDashUiDashFabricDashReactLibStrings.theme
    ]
  ]) with (stdLib.Pick[
    IPeoplePickerItemSelectedProps, 
    officeDashUiDashFabricDashReactLib.officeDashUiDashFabricDashReactLibStrings.className | officeDashUiDashFabricDashReactLib.officeDashUiDashFabricDashReactLibStrings.selected
  ]) with officeDashUiDashFabricDashReactLib.Anon_Invalid
  type IPeoplePickerItemSuggestionStyleProps = (stdLib.Required[
    stdLib.Pick[
      IPeoplePickerItemSuggestionProps, 
      officeDashUiDashFabricDashReactLib.officeDashUiDashFabricDashReactLibStrings.theme
    ]
  ]) with (stdLib.Pick[
    IPeoplePickerItemSuggestionProps, 
    officeDashUiDashFabricDashReactLib.officeDashUiDashFabricDashReactLibStrings.className
  ]) with js.Object
  type IPeoplePickerItemWithMenuProps = officeDashUiDashFabricDashReactLib.libComponentsPickersPickerItemDotTypesMod.IPickerItemProps[IPersonaWithMenu]
}
