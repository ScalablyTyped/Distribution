package typings
package officeDashUiDashFabricDashReactLib

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("office-ui-fabric-react/lib/components/ColorPicker/ColorPicker", JSImport.Namespace)
@js.native
object libComponentsColorPickerColorPickerMod extends js.Object {
  def ColorPicker(
    props: officeDashUiDashFabricDashReactLib.libComponentsColorPickerColorPickerDotTypesMod.IColorPickerProps
  ): reactLib.reactMod.Global.JSXNs.Element = js.native
}

