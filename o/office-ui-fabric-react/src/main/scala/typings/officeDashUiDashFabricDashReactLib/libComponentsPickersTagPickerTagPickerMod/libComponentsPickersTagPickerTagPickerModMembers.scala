package typings
package officeDashUiDashFabricDashReactLib.libComponentsPickersTagPickerTagPickerMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("office-ui-fabric-react/lib/components/pickers/TagPicker/TagPicker", JSImport.Namespace)
@js.native
object libComponentsPickersTagPickerTagPickerModMembers extends js.Object {
  def TagPicker(props: ITagPickerProps): reactLib.reactMod.Global.JSXNs.Element = js.native
}

