package typings
package officeDashUiDashFabricDashReactLib.libComponentsChoiceGroupChoiceGroupOptionChoiceGroupOptionDotTypesMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait IChoiceGroupOptionStyleProps extends js.Object {
  var checked: js.UndefOr[scala.Boolean] = js.undefined
  var disabled: js.UndefOr[scala.Boolean] = js.undefined
  var focused: js.UndefOr[scala.Boolean] = js.undefined
  var hasIcon: js.UndefOr[scala.Boolean] = js.undefined
  var hasImage: js.UndefOr[scala.Boolean] = js.undefined
  var imageIsLarge: js.UndefOr[scala.Boolean] = js.undefined
  var theme: atUifabricStylingLib.libInterfacesIThemeMod.ITheme
}

