package typings.officeDashUiDashFabricDashReact.libPersonaMod

import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`100px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`16px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`20px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`24px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`28px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`32px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`40px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`48px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`56px`
import typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactStrings.`72px`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("office-ui-fabric-react/lib/Persona", "personaSize")
@js.native
object personaSizeNs extends js.Object {
  val size10: `20px` = js.native
  val size100: `100px` = js.native
  val size16: `16px` = js.native
  val size24: `24px` = js.native
  val size28: `28px` = js.native
  val size32: `32px` = js.native
  val size40: `40px` = js.native
  val size48: `48px` = js.native
  val size56: `56px` = js.native
  val size72: `72px` = js.native
  val size8: `20px` = js.native
}

