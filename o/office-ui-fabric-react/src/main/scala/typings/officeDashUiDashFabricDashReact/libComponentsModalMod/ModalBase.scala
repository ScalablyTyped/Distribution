package typings.officeDashUiDashFabricDashReact.libComponentsModalMod

import typings.officeDashUiDashFabricDashReact.libComponentsModalModalDotTypesMod.IModalProps
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("office-ui-fabric-react/lib/components/Modal", "ModalBase")
@js.native
class ModalBase protected ()
  extends typings.officeDashUiDashFabricDashReact.libComponentsModalModalDotBaseMod.ModalBase {
  def this(props: IModalProps) = this()
}

/* static members */
@JSImport("office-ui-fabric-react/lib/components/Modal", "ModalBase")
@js.native
object ModalBase extends js.Object {
  var defaultProps: IModalProps = js.native
}

