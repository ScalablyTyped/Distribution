package typings.officeDashUiDashFabricDashReact.officeDashUiDashFabricDashReactMod

import typings.officeDashUiDashFabricDashReact.Anon_CreateGenericItem
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("office-ui-fabric-react", "ListPeoplePickerBase")
@js.native
class ListPeoplePickerBase ()
  extends typings.officeDashUiDashFabricDashReact.libPickersMod.ListPeoplePickerBase

/* static members */
@JSImport("office-ui-fabric-react", "ListPeoplePickerBase")
@js.native
object ListPeoplePickerBase extends js.Object {
  /** Default props for ListPeoplePicker. */
  var defaultProps: Anon_CreateGenericItem = js.native
}

