...
object INavState {
  ...
  def apply(
    ...
    val __obj = js.Dynamic.literal(isGroupCollapsed = isGroupCollapsed)
    if (!js.isUndefined(isLinkExpandStateChanged)) __obj.updateDynamic("isLinkExpandStateChanged")(isLinkExpandStateChanged)
