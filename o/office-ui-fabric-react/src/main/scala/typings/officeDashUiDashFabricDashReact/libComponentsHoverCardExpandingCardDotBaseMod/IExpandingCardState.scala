package typings.officeDashUiDashFabricDashReact.libComponentsHoverCardExpandingCardDotBaseMod
...
object IExpandingCardState {
  ...
  def apply(firstFrameRendered: Boolean, needsScroll: Boolean): IExpandingCardState = {
    val __obj = js.Dynamic.literal(firstFrameRendered = firstFrameRendered, needsScroll = needsScroll)
