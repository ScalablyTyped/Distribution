...
object ActionElement {
  ...
  inline def apply(action: open | close, element: typings.std.Element, endCallback: () => Unit): ActionElement = {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: ActionElement] (val x: Self) extends AnyVal {
