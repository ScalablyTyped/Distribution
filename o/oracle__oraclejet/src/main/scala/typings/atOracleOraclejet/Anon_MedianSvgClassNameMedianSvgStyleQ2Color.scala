package typings.atOracleOraclejet

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_MedianSvgClassNameMedianSvgStyleQ2Color extends js.Object {
  var medianSvgClassName: js.UndefOr[String] = js.undefined
  var medianSvgStyle: js.UndefOr[js.Object] = js.undefined
  var q2Color: js.UndefOr[String] = js.undefined
  var q2SvgClassName: js.UndefOr[String] = js.undefined
  var q2SvgStyle: js.UndefOr[js.Object] = js.undefined
  var q3Color: js.UndefOr[String] = js.undefined
  var q3SvgClassName: js.UndefOr[String] = js.undefined
  var q3SvgStyle: js.UndefOr[js.Object] = js.undefined
  var whiskerEndLength: js.UndefOr[String] = js.undefined
  var whiskerEndSvgClassName: js.UndefOr[String] = js.undefined
  var whiskerEndSvgStyle: js.UndefOr[js.Object] = js.undefined
  var whiskerSvgClassName: js.UndefOr[String] = js.undefined
  var whiskerSvgStyle: js.UndefOr[js.Object] = js.undefined
}

object Anon_MedianSvgClassNameMedianSvgStyleQ2Color {
  @scala.inline
  def apply(
    medianSvgClassName: String = null,
    medianSvgStyle: js.Object = null,
    q2Color: String = null,
    q2SvgClassName: String = null,
    q2SvgStyle: js.Object = null,
    q3Color: String = null,
    q3SvgClassName: String = null,
    q3SvgStyle: js.Object = null,
    whiskerEndLength: String = null,
    whiskerEndSvgClassName: String = null,
    whiskerEndSvgStyle: js.Object = null,
    whiskerSvgClassName: String = null,
    whiskerSvgStyle: js.Object = null
  ): Anon_MedianSvgClassNameMedianSvgStyleQ2Color = {
    val __obj = js.Dynamic.literal()
    if (medianSvgClassName != null) __obj.updateDynamic("medianSvgClassName")(medianSvgClassName)
    if (medianSvgStyle != null) __obj.updateDynamic("medianSvgStyle")(medianSvgStyle)
    if (q2Color != null) __obj.updateDynamic("q2Color")(q2Color)
    if (q2SvgClassName != null) __obj.updateDynamic("q2SvgClassName")(q2SvgClassName)
    if (q2SvgStyle != null) __obj.updateDynamic("q2SvgStyle")(q2SvgStyle)
    if (q3Color != null) __obj.updateDynamic("q3Color")(q3Color)
    if (q3SvgClassName != null) __obj.updateDynamic("q3SvgClassName")(q3SvgClassName)
    if (q3SvgStyle != null) __obj.updateDynamic("q3SvgStyle")(q3SvgStyle)
    if (whiskerEndLength != null) __obj.updateDynamic("whiskerEndLength")(whiskerEndLength)
    if (whiskerEndSvgClassName != null) __obj.updateDynamic("whiskerEndSvgClassName")(whiskerEndSvgClassName)
    if (whiskerEndSvgStyle != null) __obj.updateDynamic("whiskerEndSvgStyle")(whiskerEndSvgStyle)
    if (whiskerSvgClassName != null) __obj.updateDynamic("whiskerSvgClassName")(whiskerSvgClassName)
    if (whiskerSvgStyle != null) __obj.updateDynamic("whiskerSvgStyle")(whiskerSvgStyle)
    __obj.asInstanceOf[Anon_MedianSvgClassNameMedianSvgStyleQ2Color]
  }
}

