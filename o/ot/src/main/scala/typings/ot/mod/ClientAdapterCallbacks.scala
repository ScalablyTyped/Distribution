...
object ClientAdapterCallbacks {
  ...
  inline def apply(blur: () => Unit, change: (TextOperation, TextOperation) => Unit, selectionChange: () => Unit): ClientAdapterCallbacks = {
  ...
  extension [Self <: ClientAdapterCallbacks](x: Self) {
    ...
    inline def setChange(value: (TextOperation, TextOperation) => Unit): Self = StObject.set(x, "change", js.Any.fromFunction2(value))
    
    inline def setSelectionChange(value: () => Unit): Self = StObject.set(x, "selectionChange", js.Any.fromFunction0(value))
  }
}
