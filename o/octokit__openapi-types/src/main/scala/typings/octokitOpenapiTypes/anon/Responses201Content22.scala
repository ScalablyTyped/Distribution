package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait Responses201Content22 extends StObject {
  
  var parameters: PathEnterprise
  
  var responses: `201Content22`
}
object Responses201Content22 {
  
  inline def apply(parameters: PathEnterprise, responses: `201Content22`): Responses201Content22 = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[Responses201Content22]
  }
  
  extension [Self <: Responses201Content22](x: Self) {
    
    inline def setParameters(value: PathEnterprise): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `201Content22`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
