package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait ParametersPath389 extends StObject {
  
  var parameters: Path389
  
  var responses: `200Content388`
}
object ParametersPath389 {
  
  inline def apply(parameters: Path389, responses: `200Content388`): ParametersPath389 = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[ParametersPath389]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: ParametersPath389] (val x: Self) extends AnyVal {
    
    inline def setParameters(value: Path389): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `200Content388`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
