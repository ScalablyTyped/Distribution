...
trait PermissionsSlug extends StObject {
  ...
  var events: js.UndefOr[
    js.Array[
      branch_protection_rule | check_run | check_suite | code_scanning_alert | commit_comment | content_reference | create | delete | deployment | deployment_review | deployment_status | deploy_key | discussion | discussion_comment | fork | gollum | issues | issue_comment | label | member_ | membership | milestone | organization_ | org_block | page_build | project | project_card | project_column | public | pull_request | pull_request_review | pull_request_review_comment | push | registry_package | release | repository_ | repository_dispatch | secret_scanning_alert | star | status | team_ | team_add | watch | workflow_dispatch | workflow_run | reminder | pull_request_review_thread
    ]
  ] = js.undefined
...
object PermissionsSlug {
  
  inline def apply(html_url: String, name: String, node_id: String): PermissionsSlug = {
    ...
    __obj.asInstanceOf[PermissionsSlug]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: PermissionsSlug] (val x: Self) extends AnyVal {
