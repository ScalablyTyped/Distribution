...
trait SenderStarredat extends StObject {
...
object SenderStarredat {
  
  inline def apply(repository: Allowupdatebranch, sender: Avatarurl): SenderStarredat = {
    val __obj = js.Dynamic.literal(action = "created", repository = repository.asInstanceOf[js.Any], sender = sender.asInstanceOf[js.Any], starred_at = null)
    __obj.asInstanceOf[SenderStarredat]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: SenderStarredat] (val x: Self) extends AnyVal {
