package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait Responses205304403 extends StObject {
  
  var parameters: PathThreadidNumber
  
  var responses: `205304403`
}
object Responses205304403 {
  
  inline def apply(parameters: PathThreadidNumber, responses: `205304403`): Responses205304403 = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[Responses205304403]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: Responses205304403] (val x: Self) extends AnyVal {
    
    inline def setParameters(value: PathThreadidNumber): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `205304403`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
