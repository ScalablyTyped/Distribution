...
trait Headrepository extends StObject {
  ...
  /** @enum {string} */
  var status: requested | in_progress | completed | queued_ | pending
...
object Headrepository {
  ...
  inline def apply(
    ...
    status: requested | in_progress | completed | queued_ | pending,
    updated_at: String,
    url: String,
    workflow_id: Double,
    workflow_url: String
    ...
    __obj.asInstanceOf[Headrepository]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: Headrepository] (val x: Self) extends AnyVal {
    ...
    inline def setStatus(value: requested | in_progress | completed | queued_ | pending): Self = StObject.set(x, "status", value.asInstanceOf[js.Any])
