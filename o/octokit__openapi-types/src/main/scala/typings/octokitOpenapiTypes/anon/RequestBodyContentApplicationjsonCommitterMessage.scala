package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait RequestBodyContentApplicationjsonCommitterMessage extends StObject {
  
  var parameters: `76`
  
  var requestBody: ContentApplicationjsonCommitterMessage
  
  var responses: `201ContentApplicationjsonAuthorCommitter`
}
object RequestBodyContentApplicationjsonCommitterMessage {
  
  inline def apply(
    parameters: `76`,
    requestBody: ContentApplicationjsonCommitterMessage,
    responses: `201ContentApplicationjsonAuthorCommitter`
  ): RequestBodyContentApplicationjsonCommitterMessage = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], requestBody = requestBody.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[RequestBodyContentApplicationjsonCommitterMessage]
  }
  
  extension [Self <: RequestBodyContentApplicationjsonCommitterMessage](x: Self) {
    
    inline def setParameters(value: `76`): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setRequestBody(value: ContentApplicationjsonCommitterMessage): Self = StObject.set(x, "requestBody", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `201ContentApplicationjsonAuthorCommitter`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
