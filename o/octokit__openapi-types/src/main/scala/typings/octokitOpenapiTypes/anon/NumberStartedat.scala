...
trait NumberStartedat extends StObject {
  ...
  /** @enum {string} */
  var status: completed | in_progress | queued_ | pending | waiting
}
object NumberStartedat {
  
  inline def apply(name: String, number: Double, status: completed | in_progress | queued_ | pending | waiting): NumberStartedat = {
    ...
    __obj.asInstanceOf[NumberStartedat]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: NumberStartedat] (val x: Self) extends AnyVal {
    ...
    inline def setStatus(value: completed | in_progress | queued_ | pending | waiting): Self = StObject.set(x, "status", value.asInstanceOf[js.Any])
  }
}
