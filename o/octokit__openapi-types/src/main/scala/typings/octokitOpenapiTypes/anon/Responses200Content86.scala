package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait Responses200Content86 extends StObject {
  
  var parameters: PathUsername
  
  var responses: `200Content86`
}
object Responses200Content86 {
  
  inline def apply(parameters: PathUsername, responses: `200Content86`): Responses200Content86 = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[Responses200Content86]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: Responses200Content86] (val x: Self) extends AnyVal {
    
    inline def setParameters(value: PathUsername): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `200Content86`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
