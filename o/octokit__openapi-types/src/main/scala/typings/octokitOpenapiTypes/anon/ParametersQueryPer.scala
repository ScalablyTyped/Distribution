package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait ParametersQueryPer extends StObject {
  
  var parameters: QueryPer
  
  var responses: `200Content359`
}
object ParametersQueryPer {
  
  inline def apply(parameters: QueryPer, responses: `200Content359`): ParametersQueryPer = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[ParametersQueryPer]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: ParametersQueryPer] (val x: Self) extends AnyVal {
    
    inline def setParameters(value: QueryPer): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `200Content359`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
