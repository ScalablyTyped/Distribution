package typings.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait ParametersPathCommitshaQueryPage extends StObject {
  
  var parameters: PathCommitshaQueryPage
  
  var responses: `200Content212`
}
object ParametersPathCommitshaQueryPage {
  
  inline def apply(parameters: PathCommitshaQueryPage, responses: `200Content212`): ParametersPathCommitshaQueryPage = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[ParametersPathCommitshaQueryPage]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: ParametersPathCommitshaQueryPage] (val x: Self) extends AnyVal {
    
    inline def setParameters(value: PathCommitshaQueryPage): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `200Content212`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
