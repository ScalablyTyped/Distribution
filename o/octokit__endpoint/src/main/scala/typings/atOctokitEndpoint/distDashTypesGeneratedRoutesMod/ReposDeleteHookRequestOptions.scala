...
trait ReposDeleteHookRequestOptions extends js.Object {
  var headers: Headers
  var method: DELETE
...
object ReposDeleteHookRequestOptions {
  @scala.inline
  def apply(headers: Headers, method: DELETE, request: EndpointRequestOptions, url: Url): ReposDeleteHookRequestOptions = {
    val __obj = js.Dynamic.literal(headers = headers, method = method, request = request, url = url)
  
    __obj.asInstanceOf[ReposDeleteHookRequestOptions]
  }
}
