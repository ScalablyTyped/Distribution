...
trait TeamsRemoveRepoRequestOptions extends js.Object {
  var headers: Headers
  var method: DELETE
...
object TeamsRemoveRepoRequestOptions {
  @scala.inline
  def apply(headers: Headers, method: DELETE, request: EndpointRequestOptions, url: Url): TeamsRemoveRepoRequestOptions = {
    val __obj = js.Dynamic.literal(headers = headers, method = method, request = request, url = url)
  
    __obj.asInstanceOf[TeamsRemoveRepoRequestOptions]
  }
}
