...
trait AppsCheckAccountIsAssociatedWithAnyStubbedRequestOptions extends js.Object {
  var headers: Headers
  var method: GET
...
object AppsCheckAccountIsAssociatedWithAnyStubbedRequestOptions {
  @scala.inline
  def apply(headers: Headers, method: GET, request: EndpointRequestOptions, url: Url): AppsCheckAccountIsAssociatedWithAnyStubbedRequestOptions = {
    val __obj = js.Dynamic.literal(headers = headers, method = method, request = request, url = url)
  
    __obj.asInstanceOf[AppsCheckAccountIsAssociatedWithAnyStubbedRequestOptions]
  }
}
