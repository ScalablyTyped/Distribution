...
trait ReposCreateFileRequestOptions extends js.Object {
  var headers: Headers
  var method: PUT
...
object ReposCreateFileRequestOptions {
  @scala.inline
  def apply(headers: Headers, method: PUT, request: EndpointRequestOptions, url: Url): ReposCreateFileRequestOptions = {
    val __obj = js.Dynamic.literal(headers = headers, method = method, request = request, url = url)
  
    __obj.asInstanceOf[ReposCreateFileRequestOptions]
  }
}
