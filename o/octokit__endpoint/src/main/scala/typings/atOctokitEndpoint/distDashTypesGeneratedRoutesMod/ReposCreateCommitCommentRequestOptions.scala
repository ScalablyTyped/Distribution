...
trait ReposCreateCommitCommentRequestOptions extends js.Object {
  var headers: Headers
  var method: POST
...
object ReposCreateCommitCommentRequestOptions {
  @scala.inline
  def apply(headers: Headers, method: POST, request: EndpointRequestOptions, url: Url): ReposCreateCommitCommentRequestOptions = {
    val __obj = js.Dynamic.literal(headers = headers, method = method, request = request, url = url)
  
    __obj.asInstanceOf[ReposCreateCommitCommentRequestOptions]
  }
}
