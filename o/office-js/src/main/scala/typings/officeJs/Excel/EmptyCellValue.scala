...
import typings.officeJs.Excel.RangeValueType.empty
...
trait EmptyCellValue extends StObject {
  ...
  /**
    ...
    * [Api set: ExcelApi 1.16]
    */
  var `type`: typings.officeJs.Excel.CellValueType.empty | Empty
}
object EmptyCellValue {
  
  inline def apply(`type`: typings.officeJs.Excel.CellValueType.empty | Empty): EmptyCellValue = {
  ...
  extension [Self <: EmptyCellValue](x: Self) {
    ...
    inline def setType(value: typings.officeJs.Excel.CellValueType.empty | Empty): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
