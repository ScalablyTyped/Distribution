...
import typings.officeJs.Excel.ErrorCellValueType.name
import typings.officeJs.Excel.RangeValueType.error
import typings.officeJs.officeJsStrings.Error
import typings.officeJs.officeJsStrings.Name
import typings.officeJs.officeJsStrings.NumbersignNAMEQuestionmark
...
object NameErrorCellValue {
  ...
  inline def apply(`type`: typings.officeJs.Excel.CellValueType.error | Error): NameErrorCellValue = {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: NameErrorCellValue] (val x: Self) extends AnyVal {
