...
import typings.officeJs.Excel.ErrorCellValueType.`null`
import typings.officeJs.Excel.RangeValueType.error
import typings.officeJs.officeJsStrings.Error
import typings.officeJs.officeJsStrings.Null
import typings.officeJs.officeJsStrings.NumbersignNULLExclamationmark
...
object NullErrorCellValue {
  ...
  inline def apply(`type`: typings.officeJs.Excel.CellValueType.error | Error): NullErrorCellValue = {
  ...
  @scala.inline
  implicit open class MutableBuilder[Self <: NullErrorCellValue] (val x: Self) extends AnyVal {
