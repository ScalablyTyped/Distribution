package typings
package openui5Lib.sapNs.uiNs.coreNs

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
sealed trait AccessibleRole extends js.Object

/**
       * Defines the accessible roles for ARIA support. This enumeration is used with the AccessibleRole
       * control property.For more information, goto "Roles for Accessible Rich Internet Applications
       * (WAI-ARIA Roles)" at the www.w3.org homepage.
      */
@JSGlobal("sap.ui.core.AccessibleRole")
@js.native
object AccessibleRole extends js.Object {
  @js.native
  sealed trait Alert
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait AlertDialog
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Application
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Banner
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Button
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Checkbox
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait ColumnHeader
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Combobox
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait ContentInfo
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Definition
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Description
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Dialog
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Directory
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Document
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Grid
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait GridCell
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Group
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Heading
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Img
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Link
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait List
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait ListItem
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Listbox
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Log
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Main
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Marquee
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Menu
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait MenuItem
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait MenuItemCheckbox
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait MenuItemRadio
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Menubar
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Navigation
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Note
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Option
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Presentation
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait ProgressBar
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Radio
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait RadioGroup
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Region
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Row
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait RowHeader
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Search
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Secondary
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait SeeAlso
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Separator
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Slider
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait SpinButton
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Status
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Tab
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Tablist
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Tabpanel
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Textbox
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Timer
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Toolbar
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Tooltip
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait Tree
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait TreeGrid
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  @js.native
  sealed trait TreeItem
    extends openui5Lib.sapNs.uiNs.coreNs.AccessibleRole
  
  val Alert: Alert with java.lang.String = js.native
  val AlertDialog: AlertDialog with java.lang.String = js.native
  val Application: Application with java.lang.String = js.native
  val Banner: Banner with java.lang.String = js.native
  val Button: Button with java.lang.String = js.native
  val Checkbox: Checkbox with java.lang.String = js.native
  val ColumnHeader: ColumnHeader with java.lang.String = js.native
  val Combobox: Combobox with java.lang.String = js.native
  val ContentInfo: ContentInfo with java.lang.String = js.native
  val Definition: Definition with java.lang.String = js.native
  val Description: Description with java.lang.String = js.native
  val Dialog: Dialog with java.lang.String = js.native
  val Directory: Directory with java.lang.String = js.native
  val Document: Document with java.lang.String = js.native
  val Grid: Grid with java.lang.String = js.native
  val GridCell: GridCell with java.lang.String = js.native
  val Group: Group with java.lang.String = js.native
  val Heading: Heading with java.lang.String = js.native
  val Img: Img with java.lang.String = js.native
  val Link: Link with java.lang.String = js.native
  val List: List with java.lang.String = js.native
  val ListItem: ListItem with java.lang.String = js.native
  val Listbox: Listbox with java.lang.String = js.native
  val Log: Log with java.lang.String = js.native
  val Main: Main with java.lang.String = js.native
  val Marquee: Marquee with java.lang.String = js.native
  val Menu: Menu with java.lang.String = js.native
  val MenuItem: MenuItem with java.lang.String = js.native
  val MenuItemCheckbox: MenuItemCheckbox with java.lang.String = js.native
  val MenuItemRadio: MenuItemRadio with java.lang.String = js.native
  val Menubar: Menubar with java.lang.String = js.native
  val Navigation: Navigation with java.lang.String = js.native
  val Note: Note with java.lang.String = js.native
  val Option: Option with java.lang.String = js.native
  val Presentation: Presentation with java.lang.String = js.native
  val ProgressBar: ProgressBar with java.lang.String = js.native
  val Radio: Radio with java.lang.String = js.native
  val RadioGroup: RadioGroup with java.lang.String = js.native
  val Region: Region with java.lang.String = js.native
  val Row: Row with java.lang.String = js.native
  val RowHeader: RowHeader with java.lang.String = js.native
  val Search: Search with java.lang.String = js.native
  val Secondary: Secondary with java.lang.String = js.native
  val SeeAlso: SeeAlso with java.lang.String = js.native
  val Separator: Separator with java.lang.String = js.native
  val Slider: Slider with java.lang.String = js.native
  val SpinButton: SpinButton with java.lang.String = js.native
  val Status: Status with java.lang.String = js.native
  val Tab: Tab with java.lang.String = js.native
  val Tablist: Tablist with java.lang.String = js.native
  val Tabpanel: Tabpanel with java.lang.String = js.native
  val Textbox: Textbox with java.lang.String = js.native
  val Timer: Timer with java.lang.String = js.native
  val Toolbar: Toolbar with java.lang.String = js.native
  val Tooltip: Tooltip with java.lang.String = js.native
  val Tree: Tree with java.lang.String = js.native
  val TreeGrid: TreeGrid with java.lang.String = js.native
  val TreeItem: TreeItem with java.lang.String = js.native
  @JSBracketAccess
  def apply(value: java.lang.String): js.UndefOr[openui5Lib.sapNs.uiNs.coreNs.AccessibleRole with java.lang.String] = js.native
}

