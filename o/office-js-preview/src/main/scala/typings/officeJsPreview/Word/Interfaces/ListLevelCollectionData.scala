package typings.officeJsPreview.Word.Interfaces

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** An interface describing the data returned by calling `listLevelCollection.toJSON()`. */
trait ListLevelCollectionData extends StObject {
  
  var items: js.UndefOr[js.Array[ListLevelData]] = js.undefined
}
object ListLevelCollectionData {
  
  inline def apply(): ListLevelCollectionData = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[ListLevelCollectionData]
  }
  
  @scala.inline
  implicit open class MutableBuilder[Self <: ListLevelCollectionData] (val x: Self) extends AnyVal {
    
    inline def setItems(value: js.Array[ListLevelData]): Self = StObject.set(x, "items", value.asInstanceOf[js.Any])
    
    inline def setItemsUndefined: Self = StObject.set(x, "items", js.undefined)
    
    inline def setItemsVarargs(value: ListLevelData*): Self = StObject.set(x, "items", js.Array(value*))
  }
}
