...
object ActionsGetOrgPublicKeyRequestOptions {
  ...
  inline def apply(headers: RequestHeaders, request: RequestRequestOptions): ActionsGetOrgPublicKeyRequestOptions = {
  ...
  extension [Self <: ActionsGetOrgPublicKeyRequestOptions](x: Self) {
    ...
    inline def setUrl(value: `SlashorgsSlashColonorgSlashactionsSlashsecretsSlashpublic-key`): Self = StObject.set(x, "url", value.asInstanceOf[js.Any])
  }
}
