...
object MarkdownRenderRequestOptions {
  ...
  inline def apply(headers: RequestHeaders, request: RequestRequestOptions): MarkdownRenderRequestOptions = {
  ...
  extension [Self <: MarkdownRenderRequestOptions](x: Self) {
    ...
    inline def setUrl(value: Slashmarkdown): Self = StObject.set(x, "url", value.asInstanceOf[js.Any])
  }
}
